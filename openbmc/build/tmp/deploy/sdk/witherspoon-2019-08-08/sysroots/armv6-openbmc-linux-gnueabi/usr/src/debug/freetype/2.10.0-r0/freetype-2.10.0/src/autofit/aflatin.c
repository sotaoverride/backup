...
af_latin_metrics_init_widths(AF_LatinMetrics metrics, FT_Face face)
{
...

    {
        ...
        while (*p)
        {
            ...
            glyph_index =
                af_shaper_get_elem(&metrics->root, shaper_buf, 0, NULL, NULL);
            if (glyph_index)
                break;
        }
        ...
        for (dim = 0; dim < AF_DIMENSION_MAX; dim++)
        {
             ...
             * function.
             */
            af_latin_hints_link_segments(hints, 0, NULL, (AF_Dimension)dim);
...
static void af_latin_sort_blue(FT_UInt count, AF_LatinBlue* table)
{
    ...
    for (i = 1; i < count; i++)
    {
        for (j = i; j > 0; j--)
        {
            ...
            if (table[j - 1]->flags &
                (AF_LATIN_BLUE_TOP | AF_LATIN_BLUE_SUB_TOP))
                a = table[j - 1]->ref.org;
            else
                a = table[j - 1]->shoot.org;

            if (table[j]->flags & (AF_LATIN_BLUE_TOP | AF_LATIN_BLUE_SUB_TOP))
                b = table[j]->ref.org;
            else
                b = table[j]->shoot.org;
    ...
    for (; bs->string != AF_BLUE_STRING_MAX; bs++)
    {
        ...
        while (*p)
        {
            ...
            for (i = 0; i < num_idx; i++)
            {
                 ...
                 */
                glyph_index = af_shaper_get_elem(&metrics->root, shaper_buf, i,
                                                 NULL, &y_offset);
                if (glyph_index == 0)
                {
                    FT_TRACE5(("  U+%04lX unavailable\n", ch));
...

                {
                    ...
                    for (nn = 0; nn < outline.n_contours;
                         first = last + 1, nn++)
                    {
                ...
                if (best_point >= 0)
                {
                    ...
                    if (FT_CURVE_TAG(outline.tags[best_point]) ==
                        FT_CURVE_TAG_ON)
                    {
                    ...
                    do
                    {
                        ...
                        /* accept a small distance or a small angle (both values
                         * are */
                        /* heuristic; value 20 corresponds to approx. 2.9
                         * degrees)   */
                        if (dist > 5)
                    ...
                    if (AF_LATIN_IS_LONG_BLUE(bs))
                    {
                        /* If this flag is set, we have an additional constraint
                         * to  */
                        /* get the blue zone distance: Find a segment of the
                         * topmost */
                        /* (or bottommost) contour that is longer than a
                         * heuristic   */
                        /* threshold.  This ensures that small bumps in the
                         * outline  */
                        /* are ignored (for example, the `vertical serifs' found
                         * in  */
                        /* many Hebrew glyph designs). */

                        /* If this segment is long enough, we are done.
                         * Otherwise,  */
                         ...
                         */
                        /* actually the one (vertically) nearest to the
                         * extremum.    */
                                ...
                                best_contour_last - best_contour_first)
                        {
                             ...
                             */
                            /* incorrect warning messages otherwise, talking
                             * about */
                            ...
                            left2right =
                                FT_BOOL(points[prev].x < points[best_point].x);
                            ...
                            do
                            {
                                ...
                                if (FT_ABS(best_y - points[first].y) >
                                    height_threshold)
                                {
                                ...
                                if (dist > 5)
                                    if (FT_ABS(points[last].x -
                                               points[first].x) <= 20 * dist)
                                    {
                                        hit = 0;
                                        continue;
                                    }
                                ...
                                {
                                    /* all constraints are met; update segment
                                     * after */
                                    /* finding its end */
                                    do
                                    {
                                        ...
                                        d = FT_ABS(points[last].y -
                                                   points[first].y);
                                        if (d > 5)
                                            if (FT_ABS(points[next].x -
                                                       points[first].x) <=
                                                20 * dist)
                                            {
        ...
        if (*blue_shoot != *blue_ref)
        {
            ...
            if ((AF_LATIN_IS_TOP_BLUE(bs) || AF_LATIN_IS_SUB_TOP_BLUE(bs)) ^
                over_ref)
            {
                *blue_ref = *blue_shoot = (shoot + ref) / 2;
...
af_latin_metrics_check_digits(AF_LatinMetrics metrics, FT_Face face)
{
    ...
    while (*p)
    {
        ...
        glyph_index =
            af_shaper_get_elem(&metrics->root, shaper_buf, 0, &advance, NULL);
        if (!glyph_index)
            continue;
...
static void af_latin_metrics_scale_dim(AF_LatinMetrics metrics,
                                       AF_Scaler scaler, AF_Dimension dim)
{
    FT_Fixed scale;
    FT_Pos delta;
    AF_LatinAxis axis;
     ...
     */
    {
        ...
        if (blue)
        {
            ...
            if (limit && ppem <= limit && ppem >= AF_PROP_INCREASE_X_HEIGHT_MIN)
                threshold = 52;
            ...
            if ( fitted < scaled )
              scale -= scale / 50;  /* scale *= 0.98 */
                ...
                if (dim == AF_DIMENSION_VERT)
                {
                    ...
                    {
                        max_height =
                            FT_MAX(max_height, Axis->blues[nn].ascender);
                        max_height =
                            FT_MAX(max_height, -Axis->blues[nn].descender);
                    }
                    ...
                    if (dist == 0)
                    {
                        FT_TRACE5(
                            ("af_latin_metrics_scale_dim:"
                             ...
                             "                           "
                             " vertical scaling changed from %.5f to %.5f (by "
                             "%d%%)\n"
                             "\n",
                             af_style_names[metrics->root.style_class->style],
        ...
        for (nn = 0; nn < axis->blue_count; nn++)
        {
            ...
            FT_TRACE5(
                ("  reference %d: %d scaled to %.2f%s\n"
                 ...
                 nn, blue->ref.org, blue->ref.fit / 64.0,
                 blue->flags & AF_LATIN_BLUE_ACTIVE ? "" : " (inactive)", nn,
                 blue->shoot.org, blue->shoot.fit / 64.0,
                 blue->flags & AF_LATIN_BLUE_ACTIVE ? "" : " (inactive)"));
        }
#endif
    }
}
...
FT_LOCAL_DEF(void)
af_latin_get_standard_widths(AF_LatinMetrics metrics, FT_Pos* stdHW,
                             FT_Pos* stdVW)
{
    if (stdHW)
    ...
    for (; contour < contour_limit; contour++)
    {
        ...
        for (;;)
        {
            ...
            if (on_edge)
            {
                ...
                {
                    /* check whether the new segment's start point is identical
                     * to */
                    ...
                    if (!prev_segment || segment->first != prev_segment->last)
                    {
                        ...
                        segment->height =
                            segment->max_coord - segment->min_coord;
                    ...
                    {
                        /* points are the same: we don't create a new segment
                         * but */
                        ...
                        if (prev_segment->last->in_dir == point->in_dir)
                        {
                             ...
                             */
                            /* degenerate outlines that move zig-zag along the
                             * main    */
                            /* axis without changing the coordinate value of the
                             * other */
                            /* axis, and where the segments have just been
                             * merged):    */
                            /* unify segments */
                        ...
                        {
                            /* we have different directions; use the properties
                             * of the */
                                ...
                                FT_ABS(max_coord - min_coord))
                            {
                                ...
                                prev_segment->pos = (FT_Short)(
                                    (prev_min_pos + prev_max_pos) >> 1);
                                prev_segment->delta = (FT_Short)(
                                    (prev_max_pos - prev_min_pos) >> 1);
                            }
                            else
                            {
    ...
    /* sense -- this is used to better detect and ignore serifs   */
    {
        ...
        for (segment = segments; segment < segments_end; segment++)
        {
            ...
            if (first_v < last_v)
            {
                ...
                if (p->v < first_v)
                    segment->height =
                        (FT_Short)(segment->height + ((first_v - p->v) >> 1));

                p = last->next;
                if (p->v > last_v)
                    segment->height =
                        (FT_Short)(segment->height + ((p->v - last_v) >> 1));
            }
            else
            {
                AF_Point p;
                ...
                if (p->v > first_v)
                    segment->height =
                        (FT_Short)(segment->height + ((p->v - first_v) >> 1));

                p = last->next;
                if (p->v < last_v)
                    segment->height =
                        (FT_Short)(segment->height + ((last_v - p->v) >> 1));
            }
        }
    }
...
FT_LOCAL_DEF(void)
af_latin_hints_link_segments(AF_GlyphHints hints, FT_UInt width_count,
                             AF_WidthRec* widths, AF_Dimension dim)
{
    AF_AxisHints axis = &hints->axis[dim];
    AF_Segment segments = axis->segments;
        ...
        for (seg2 = segments; seg2 < segment_limit; seg2++)
        {
            ...
            if (seg1->dir + seg2->dir == 0 && pos2 > pos1)
            {
                ...
                if (len >= len_threshold)
                {
                     ...
                     * `badness' of a fit, measured along the segments' main
                     * axis and orthogonal to it, respectively.
                     *
                     * - The less overlapping along the main axis, the worse it
                     *   is, causing a larger demerit.
                     *
                     * - The nearer the orthogonal distance to a stem width, the
                     *   better it is, causing a smaller demerit.  For
                     * simplicity, however, we only increase the demerit for
                     * values that exceed the largest stem width.
                     */
                    ...
                    if (max_width)
                    {
                        /* distance demerits are based on multiples of
                         * `max_width'; */
...
af_latin_hints_compute_edges(AF_GlyphHints hints, AF_Dimension dim)
{
    ...
    for (seg = segments; seg < segment_limit; seg++)
    {
        ...
        if (seg->height < segment_length_threshold ||
            seg->delta > segment_width_threshold || seg->dir == AF_DIR_NONE)
            continue;
        ...
        if (!found)
        {
            ...
            error =
                af_axis_hints_new_edge(axis, seg->pos, (AF_Direction)seg->dir,
                                       top_to_bottom_hinting, memory, &edge);
            if (error)
                goto Exit;
...
FT_LOCAL_DEF(FT_Error)
af_latin_hints_detect_features(AF_GlyphHints hints, FT_UInt width_count,
                               AF_WidthRec* widths, AF_Dimension dim)
{
    FT_Error error;
                                              ...
                                              AF_LatinMetrics metrics)
{
    ...
    for (; edge < edge_limit; edge++)
    {
        ...
        for (bb = 0; bb < latin->blue_count; bb++)
        {
            ...
            is_top_blue =
                (FT_Byte)((blue->flags &
                           (AF_LATIN_BLUE_TOP | AF_LATIN_BLUE_SUB_TOP)) != 0);
            is_neutral_blue =
...
static FT_Pos af_latin_compute_stem_width(AF_GlyphHints hints, AF_Dimension dim,
                                          FT_Pos width, FT_Pos base_delta,
                                          FT_UInt base_flags,
                                          FT_UInt stem_flags)
{
    AF_LatinMetrics metrics = (AF_LatinMetrics)hints->metrics;
    ...
    if (!AF_LATIN_HINTS_DO_STEM_ADJUST(hints) || axis->extra_light)
        return width;
...
static void af_latin_align_linked_edge(AF_GlyphHints hints, AF_Dimension dim,
                                       AF_Edge base_edge, AF_Edge stem_edge)
{
    ...
    fitted_width = af_latin_compute_stem_width(
        hints, dim, dist, base_delta, base_edge->flags, stem_edge->flags);
...
static void af_latin_align_serif_edge(AF_GlyphHints hints, AF_Edge base,
                                      AF_Edge serif)
{
    FT_UNUSED(hints);
...
static void af_latin_hint_edges(AF_GlyphHints hints, AF_Dimension dim)
{
    ...
    for (edge = edges; edge < edge_limit; edge++)
    {
        ...
        if (!anchor)
        {
            ...
            cur_len = af_latin_compute_stem_width(hints, dim, org_len, 0,
            ...
            FT_TRACE5(("  ANCHOR: edge %d (opos=%.2f) and %d (opos=%.2f)"
                       ...
                       edge - edges, edge->opos / 64.0, edge2 - edges,
                       edge2->opos / 64.0, edge->pos / 64.0,
                       edge2->pos / 64.0));
        ...
        else
        {
            ...
            else if (cur_len < 96)
            {
                ...
                FT_TRACE5(
                    ("  STEM: edge %d (opos=%.2f) linked to %d (opos=%.2f)"
                     " snapped to %.2f and %.2f\n",
                     edge - edges, edge->opos / 64.0, edge2 - edges,
                     edge2->opos / 64.0, edge->pos / 64.0, edge2->pos / 64.0));
            }
            ...
            else
            {
                ...
                cur_len = af_latin_compute_stem_width(
                    hints, dim, org_len, 0, edge->flags, edge2->flags);
    ...
    if (has_serifs || !anchor)
    {
        ...
        for (edge = edges; edge < edge_limit; edge++)
        {
            ...
            if (delta < 64 + 16)
            {
                ...
                FT_TRACE5(
                    ("  SERIF: edge %d (opos=%.2f) serif to %d (opos=%.2f)"
                     " aligned to %.2f\n",
                     edge - edges, edge->opos / 64.0, edge->serif - edges,
                     edge->serif->opos / 64.0, edge->pos / 64.0));
            }
            else if (!anchor)
            {
                edge->pos = FT_PIX_ROUND(edge->opos);
            ...
            else
            {
                ...
                if (before >= edges && before < edge && after < edge_limit &&
                    after > edge)
                {
                    if (after->opos == before->opos)
                        ...
                        edge->pos =
                            before->pos + FT_MulDiv(edge->opos - before->opos,
                ...
                {
                    edge->pos =
                        anchor->pos + ((edge->opos - anchor->opos + 16) & ~31);
...
static FT_Error af_latin_hints_apply(FT_UInt glyph_index, AF_GlyphHints hints,
                                     FT_Outline* outline,
                                     AF_LatinMetrics metrics)
{
    FT_Error error;
    ...
    if (AF_HINTS_DO_HORIZONTAL(hints))
    {
        ...
        error = af_latin_hints_detect_features(hints, axis->width_count,
                                               axis->widths, AF_DIMENSION_HORZ);
        if (error)
            goto Exit;
    }
    ...
    for (dim = 0; dim < AF_DIMENSION_MAX; dim++)
    {
            ...
            AF_HINTS_DO_WARP(hints))
        {
            ...
            af_warper_compute(&warper, hints, (AF_Dimension)dim, &scale,
                              &delta);
            af_glyph_hints_scale_dim(hints, (AF_Dimension)dim, scale, delta);
            continue;
        }
...
AF_DEFINE_WRITING_SYSTEM_CLASS(
    ...
    (AF_WritingSystem_ScaleMetricsFunc)
        af_latin_metrics_scale,             /* style_metrics_scale   */
