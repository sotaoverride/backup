                           ...
                           struct dir_list** list)
{
    ...
    if (!dp)
        return;
    dp->name =
        subdir ? blkid_strconcat(dir, "/", subdir) : dir ? strdup(dir) : NULL;
    ...
    while ((dp = readdir(dir)) != NULL)
    {
        ...
        if (S_ISBLK(st.st_mode) && st.st_rdev == devno)
        {
            *devname = blkid_strconcat(dirname, "/", dp->d_name);
            DBG(DEVNO,
                ul_debug("found 0x%llx at %s", (long long)devno, *devname));
            break;
        }
...
#endif
        {
            if (fstatat(dirfd(dir), dp->d_name, &st, 1) || !S_ISDIR(st.st_mode))
...
char* blkid_devno_to_devname(dev_t devno)
{
    ...
    {
        DBG(DEVNO,
            ul_debug("found devno 0x%04llx as %s", (long long)devno, path));
    }
 ...
 */
int blkid_devno_to_wholedisk(dev_t dev, char* diskname, size_t len,
                             dev_t* diskdevno)
{
...
int blkid_driver_has_major(const char* drvname, int drvmaj)
{
    ...
    DBG(DEVNO, ul_debug("major %d %s associated with '%s' driver", drvmaj,
                        match ? "is" : "is NOT", drvname));
    return match;
}
...
int main(int argc, char** argv)
{
    ...
    if (blkid_devno_to_wholedisk(devno, diskname, sizeof(diskname),
                                 &disk_devno) == 0)
        printf("found devno 0x%04llx as /dev/%s\n", (long long)disk_devno,
               diskname);
