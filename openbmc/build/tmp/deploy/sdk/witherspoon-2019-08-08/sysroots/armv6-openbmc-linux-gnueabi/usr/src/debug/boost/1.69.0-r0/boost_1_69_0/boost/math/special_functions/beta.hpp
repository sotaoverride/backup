...
T beta_imp(T a, T b, const Lanczos&, const Policy& pol)
{
    BOOST_MATH_STD_USING // for ADL of std names
        ...
        if (a <= 0) return policies::raise_domain_error<T>(
            "boost::math::beta<%1%>(%1%,%1%)",
            "The arguments to the beta function must be greater than zero (got "
            "a=%1%).",
            a, pol);
    if (b <= 0)
    ...
    */
    ...
    result = Lanczos::lanczos_sum_expG_scaled(a) *
             (Lanczos::lanczos_sum_expG_scaled(b) /
              Lanczos::lanczos_sum_expG_scaled(c));
    T ambh = a - 0.5f - b;
    ...
    if (cgh > 1e10f)
        // this avoids possible overflow, but appears to be marginally less
        // accurate:
           ...
           const Policy& pol)
{
    ...
    sa += detail::upper_gamma_fraction(
        a, la, ::boost::math::policies::get_epsilon<T, Policy>());
    T sb = detail::lower_gamma_series(b, lb, pol) / b;
    sb += detail::upper_gamma_fraction(
        b, lb, ::boost::math::policies::get_epsilon<T, Policy>());
    T sc = detail::lower_gamma_series(c, lc, pol) / c;
    sc += detail::upper_gamma_fraction(
        c, lc, ::boost::math::policies::get_epsilon<T, Policy>());
...
template <class T, class Lanczos, class Policy>
T ibeta_power_terms(
    T a, T b, T x, T y, const Lanczos&, bool normalised, const Policy& pol,
    T prefix = 1,
    const char* function = "boost::math::ibeta<%1%>(%1%, %1%, %1%)")
{
    ...
    if (((std::min)(fabs(l1), fabs(l2)) < 0.2))
    {
        ...
        if ((l1 * l2 > 0) || ((std::min)(a, b) < 1))
        {
            ...
            // since one of the power terms will evaluate to a number close
            // to 1.
            //
            if (fabs(l1) < 0.1)
            {
        ...
        else if ((std::max)(fabs(l1), fabs(l2)) < 0.5)
        {
            ...
            bool small_a = a < b;
            T ratio = b / a;
            if ((small_a && (ratio * l2 < 0.1)) ||
                (!small_a && (l1 / ratio > 0.1)))
            {
                T l3 = boost::math::expm1(ratio * boost::math::log1p(l2, pol),
                                          pol);
                l3 = l1 + l3 + l3 * l1;
            ...
            {
                T l3 = boost::math::expm1(boost::math::log1p(l1, pol) / ratio,
                                          pol);
                l3 = l2 + l3 + l3 * l2;
        ...
        else if (fabs(l1) < fabs(l2))
        {
            ...
            T l = a * boost::math::log1p(l1, pol) + b * log((y * cgh) / bgh);
            if ((l <= tools::log_min_value<T>()) ||
                (l >= tools::log_max_value<T>()))
            {
                l += log(result);
        ...
        else
        {
            ...
            T l = b * boost::math::log1p(l2, pol) + a * log((x * cgh) / agh);
            if ((l <= tools::log_min_value<T>()) ||
                (l >= tools::log_max_value<T>()))
            {
                l += log(result);
            ...
            if (a < b)
            {
                ...
                else
                {
                    ...
                    if (l2 >= tools::log_max_value<T>())
                        return policies::raise_overflow_error<T>(function, 0,
                                                                 pol);
                    result = exp(l2);
                }
            }
...
//
template <class T, class Policy>
T ibeta_power_terms(T a, T b, T x, T y,
                    const boost::math::lanczos::undefined_lanczos&,
                    bool normalised, const Policy& pol, T prefix = 1,
                    const char* = "boost::math::ibeta<%1%>(%1%, %1%, %1%)")
{
    ...
    if ((lb1 >= tools::log_max_value<T>()) ||
        ...
        (e1 >= tools::log_max_value<T>()) || (e1 <= tools::log_min_value<T>()))
    {
...
struct ibeta_series_t
{
    ...
    ibeta_series_t(T a_, T b_, T x_, T mult) :
        result(mult), x(x_), apn(a_), poch(1 - b_), n(1)
    {
    }
    T operator()()
    {
...
template <class T, class Lanczos, class Policy>
T ibeta_series(T a, T b, T x, T s0, const Lanczos&, bool normalised,
               T* p_derivative, T y, const Policy& pol)
{
    BOOST_MATH_STD_USING
    ...
    result = boost::math::tools::sum_series(
        s, boost::math::policies::get_epsilon<T, Policy>(), max_iter, s0);
...
//
template <class T, class Policy>
T ibeta_series(T a, T b, T x, T s0,
               const boost::math::lanczos::undefined_lanczos&, bool normalised,
               T* p_derivative, T y, const Policy& pol)
{
    BOOST_MATH_STD_USING
    ...
    result = boost::math::tools::sum_series(
        s, boost::math::policies::get_epsilon<T, Policy>(), max_iter, s0);
    policies::check_series_iterations<T>(
        "boost::math::ibeta<%1%>(%1%, %1%, %1%) in ibeta_series (without "
        "lanczos)",
        max_iter, pol);
    return result;
}
...
//
template <class T, class Policy>
inline T ibeta_fraction2(T a, T b, T x, T y, const Policy& pol, bool normalised,
                         T* p_derivative)
{
    ...
    T fract = boost::math::tools::continued_fraction_b(
        f, boost::math::policies::get_epsilon<T, Policy>());
...
//
template <class T, class Policy>
T ibeta_a_step(T a, T b, T x, T y, int k, const Policy& pol, bool normalised,
               T* p_derivative)
{
    typedef typename lanczos::lanczos<T, Policy>::type lanczos_type;
...
struct Pn_size<long double>
{
    ...
    BOOST_STATIC_ASSERT(::boost::math::max_factorial<long double>::value >=
                        100);
};

template <class T, class Policy>
T beta_small_b_large_a_series(T a, T b, T x, T y, T s0, T mult,
                              const Policy& pol, bool normalised)
{
    typedef typename lanczos::lanczos<T, Policy>::type lanczos_type;
    BOOST_MATH_STD_USING
    //
    ...
    return sum;
} // template <class T, class Lanczos>T beta_small_b_large_a_series(T a, T b, T
  // x, T y, T s0, T mult, const Lanczos& l, bool normalised)
...
T binomial_ccdf(T n, T k, T x, T y)
{
    ...
    else
    {
        ...
        result = pow(x, start) * pow(y, n - start) *
                 boost::math::binomial_coefficient<T>(itrunc(n), itrunc(start));
        if (result == 0)
        {
            ...
            {
                result +=
                    pow(x, (int)i) * pow(y, n - i) *
                    boost::math::binomial_coefficient<T>(itrunc(n), itrunc(i));
            }
        }
        else
        {
            ...
            T* p_derivative)
{
    ...
    if ((x < 0) || (x > 1))
        return policies::raise_domain_error<T>(
            function,
            "Parameter x outside the range [0,1] in the incomplete beta "
            "function (got x=%1%).",
            x, pol);
    ...
    if (normalised)
    {
        if (a < 0)
            return policies::raise_domain_error<T>(
                function,
                "The argument a to the incomplete beta function must be >= "
                "zero (got a=%1%).",
                a, pol);
        if (b < 0)
            return policies::raise_domain_error<T>(
                function,
                "The argument b to the incomplete beta function must be >= "
                "zero (got b=%1%).",
                b, pol);
        ...
        if (a == 0)
        {
            if (b == 0)
                return policies::raise_domain_error<T>(
                    function,
                    "The arguments a and b to the incomplete beta function "
                    "cannot both be zero, with x=%1%.",
                    x, pol);
            if (b > 0)
    ...
    else
    {
        if (a <= 0)
            return policies::raise_domain_error<T>(
                function,
                "The argument a to the incomplete beta function must be "
                "greater than zero (got a=%1%).",
                a, pol);
        if (b <= 0)
            return policies::raise_domain_error<T>(
                function,
                "The argument b to the incomplete beta function must be "
                "greater than zero (got b=%1%).",
                b, pol);
    }
    ...
    if (x == 0)
    {
        if (p_derivative)
        {
            *p_derivative = (a == 1) ? (T)1
                                     : (a < 1) ? T(tools::max_value<T>() / 2)
                                               : T(tools::min_value<T>() * 2);
        }
        return (invert ? (normalised ? T(1) : boost::math::beta(a, b, pol))
                       : T(0));
    }
    if (x == 1)
    {
        if (p_derivative)
        {
            *p_derivative = (b == 1) ? T(1)
                                     : (b < 1) ? T(tools::max_value<T>() / 2)
                                               : T(tools::min_value<T>() * 2);
        }
        return (invert == 0 ? (normalised ? 1 : boost::math::beta(a, b, pol))
                            : 0);
    }
    if ((a == 0.5f) && (b == 0.5f))
    {
        ...
        T p = invert ? asin(sqrt(y)) / constants::half_pi<T>()
                     : asin(sqrt(x)) / constants::half_pi<T>();
        if (!normalised)
            p *= constants::pi<T>();
        return p;
    }
    ...
    if (b == 1)
    {
        ...
        T p;
        if (y < 0.5)
            p = invert ? T(-boost::math::expm1(a * boost::math::log1p(-y, pol),
                                               pol))
                       : T(exp(a * boost::math::log1p(-y, pol)));
        else
    ...
    if ((std::min)(a, b) <= 1)
    {
        ...
        if ((std::max)(a, b) <= 1)
        {
            ...
            if ((a >= (std::min)(T(0.2), b)) || (pow(x, a) <= 0.9))
            {
                if (!invert)
                {
                    fract = ibeta_series(a, b, x, T(0), lanczos_type(),
                                         normalised, p_derivative, y, pol);
                    BOOST_MATH_INSTRUMENT_VARIABLE(fract);
                }
                else
                {
                    ...
                    invert = false;
                    fract = -ibeta_series(a, b, x, fract, lanczos_type(),
                                          normalised, p_derivative, y, pol);
                    BOOST_MATH_INSTRUMENT_VARIABLE(fract);
                }
            }
            else
            {
                ...
                if (y >= 0.3)
                {
                    ...
                    {
                        fract =
                            -(normalised ? 1 : boost::math::beta(a, b, pol));
                        invert = false;
                        fract = -ibeta_series(a, b, x, fract, lanczos_type(),
                                              normalised, p_derivative, y, pol);
                        BOOST_MATH_INSTRUMENT_VARIABLE(fract);
                ...
                else
                {
                    ...
                    fract = ibeta_a_step(a, b, x, y, 20, pol, normalised,
                                         p_derivative);
                    if (!invert)
                    {
                        fract = beta_small_b_large_a_series(
                            T(a + 20), b, x, y, fract, prefix, pol, normalised);
                        BOOST_MATH_INSTRUMENT_VARIABLE(fract);
                    }
                    else
                    {
        ...
        else
        {
            ...
            else
            {
                ...
                else if (a >= 15)
                {
                    ...
                    {
                        fract = beta_small_b_large_a_series(
                            a, b, x, y, T(0), T(1), pol, normalised);
                        BOOST_MATH_INSTRUMENT_VARIABLE(fract);
                    }
                    else
                    {
                        ...
                        invert = false;
                        fract = -beta_small_b_large_a_series(
                            a, b, x, y, fract, T(1), pol, normalised);
                        BOOST_MATH_INSTRUMENT_VARIABLE(fract);
                    }
                }
                else
    ...
    else
    {
        ...
        if (b < 40)
        {
            if ((floor(a) == a) && (floor(b) == b) &&
                (a < (std::numeric_limits<int>::max)() - 100) && (y != 1))
            {
            ...
            else if (a > 15)
            {
                ...
                fract = ibeta_a_step(bbar, a, y, x, n, pol, normalised,
                                     static_cast<T*>(0));
                fract = beta_small_b_large_a_series(a, bbar, x, y, fract, T(1),
                                                    pol, normalised);
                fract /= prefix;
                BOOST_MATH_INSTRUMENT_VARIABLE(fract);
            }
            ...
            {
                // The formula here for the non-normalised case is tricky to
                // figure out (for me!!), and requires two pochhammer
                // calculations rather than one, so leave it for now and only
                // use this in the normalized case....
                int n = itrunc(T(floor(b)), pol);
                T bbar = b - n;
                if (bbar <= 0)
                {
                ...
                fract += ibeta_a_step(a, bbar, x, y, 20, pol, normalised,
                                      static_cast<T*>(0));
                if (invert)
                    fract -= 1; // Note this line would need changing if we ever
                                // enable this branch in non-normalized case
                fract = beta_small_b_large_a_series(
                    T(a + 20), bbar, x, y, fract, T(1), pol, normalised);
                if (invert)
                {
                    fract = -fract;
                    invert = false;
            ...
            {
                fract =
                    ibeta_fraction2(a, b, x, y, pol, normalised, p_derivative);
                BOOST_MATH_INSTRUMENT_VARIABLE(fract);
            }
        }
        else
    ...
    if (p_derivative)
    {
        ...
        {
            *p_derivative =
                ibeta_power_terms(a, b, x, y, lanczos_type(), true, pol);
        }
        T div = y * x;
    ...
    return invert ? (normalised ? 1 : boost::math::beta(a, b, pol)) - fract
                  : fract;
} // template <class T, class Lanczos>T ibeta_imp(T a, T b, T x, const Lanczos&
  // l, bool inv, bool normalised)
...
T ibeta_derivative_imp(T a, T b, T x, const Policy& pol)
{
    ...
    if (a <= 0)
        return policies::raise_domain_error<T>(
            function,
            "The argument a to the incomplete beta function must be greater "
            "than zero (got a=%1%).",
            a, pol);
    if (b <= 0)
        return policies::raise_domain_error<T>(
            function,
            "The argument b to the incomplete beta function must be greater "
            "than zero (got b=%1%).",
            b, pol);
    if ((x < 0) || (x > 1))
        return policies::raise_domain_error<T>(
    ...
    beta(RT1 a, RT2 b, const Policy&, const mpl::true_*)
{
    ...
    return policies::checked_narrowing_cast<result_type, forwarding_policy>(
        detail::beta_imp(static_cast<value_type>(a), static_cast<value_type>(b),
                         evaluation_type(), forwarding_policy()),
        "boost::math::beta<%1%>(%1%,%1%)");
}
...
inline typename tools::promote_args<RT1, RT2, A>::type beta(RT1 a, RT2 b, A arg)
{
...
inline typename tools::promote_args<RT1, RT2>::type beta(RT1 a, RT2 b)
{
    ...
    beta(RT1 a, RT2 b, RT3 x, const Policy&)
{
    ...
    return policies::checked_narrowing_cast<result_type, forwarding_policy>(
        detail::ibeta_imp(
            static_cast<value_type>(a), static_cast<value_type>(b),
            static_cast<value_type>(x), forwarding_policy(), false, false),
        "boost::math::beta<%1%>(%1%,%1%,%1%)");
}
    ...
    betac(RT1 a, RT2 b, RT3 x, const Policy&)
{
    ...
    return policies::checked_narrowing_cast<result_type, forwarding_policy>(
        detail::ibeta_imp(
            static_cast<value_type>(a), static_cast<value_type>(b),
            static_cast<value_type>(x), forwarding_policy(), true, false),
        "boost::math::betac<%1%>(%1%,%1%,%1%)");
}
template <class RT1, class RT2, class RT3>
inline typename tools::promote_args<RT1, RT2, RT3>::type betac(RT1 a, RT2 b,
                                                               RT3 x)
{
    ...
    ibeta(RT1 a, RT2 b, RT3 x, const Policy&)
{
    ...
    return policies::checked_narrowing_cast<result_type, forwarding_policy>(
        detail::ibeta_imp(
            static_cast<value_type>(a), static_cast<value_type>(b),
            static_cast<value_type>(x), forwarding_policy(), false, true),
        "boost::math::ibeta<%1%>(%1%,%1%,%1%)");
}
template <class RT1, class RT2, class RT3>
inline typename tools::promote_args<RT1, RT2, RT3>::type ibeta(RT1 a, RT2 b,
                                                               RT3 x)
{
    ...
    ibetac(RT1 a, RT2 b, RT3 x, const Policy&)
{
    ...
    return policies::checked_narrowing_cast<result_type, forwarding_policy>(
        detail::ibeta_imp(
            static_cast<value_type>(a), static_cast<value_type>(b),
            static_cast<value_type>(x), forwarding_policy(), true, true),
        "boost::math::ibetac<%1%>(%1%,%1%,%1%)");
}
template <class RT1, class RT2, class RT3>
inline typename tools::promote_args<RT1, RT2, RT3>::type ibetac(RT1 a, RT2 b,
                                                                RT3 x)
{
    ...
    ibeta_derivative(RT1 a, RT2 b, RT3 x, const Policy&)
{
    ...
    return policies::checked_narrowing_cast<result_type, forwarding_policy>(
        detail::ibeta_derivative_imp(
            static_cast<value_type>(a), static_cast<value_type>(b),
            static_cast<value_type>(x), forwarding_policy()),
        "boost::math::ibeta_derivative<%1%>(%1%,%1%,%1%)");
}
template <class RT1, class RT2, class RT3>
inline typename tools::promote_args<RT1, RT2, RT3>::type
