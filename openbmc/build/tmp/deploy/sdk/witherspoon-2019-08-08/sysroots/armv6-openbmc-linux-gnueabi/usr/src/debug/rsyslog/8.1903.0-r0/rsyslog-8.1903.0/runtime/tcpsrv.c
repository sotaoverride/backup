...
/* defines */
#define TCPSESS_MAX_DEFAULT                                                    \
    200 /* default for nbr of tcp sessions if no number is given */
#define TCPLSTN_MAX_DEFAULT 20 /* default for nbr of listeners */

    /* static data */
    DEFobjStaticHelpers DEFobjCurrIf(conf) DEFobjCurrIf(glbl)
        DEFobjCurrIf(ruleset) DEFobjCurrIf(tcps_sess) DEFobjCurrIf(net)
            DEFobjCurrIf(netstrms) DEFobjCurrIf(netstrm) DEFobjCurrIf(nssel)
                DEFobjCurrIf(nspoll) DEFobjCurrIf(prop) DEFobjCurrIf(statsobj)
                   ...
                   const int bSuppOctetFram, const uchar* const pszAddr)
{
    ...
    /* create entry */
    CHKmalloc(pEntry =
                  (tcpLstnPortList_t*)calloc(1, sizeof(tcpLstnPortList_t)));
    ...
    CHKiRet(prop.SetString(pEntry->pInputName, pThis->pszInputName,
                           ustrlen(pThis->pszInputName)));
    CHKiRet(prop.ConstructFinalize(pEntry->pInputName));

    /* support statistics gathering */
    CHKiRet(ratelimitNew(&pEntry->ratelimiter, "tcperver", NULL));
    ratelimitSetLinuxLike(pEntry->ratelimiter, pThis->ratelimitInterval,
                          pThis->ratelimitBurst);
    ratelimitSetThreadSafe(pEntry->ratelimiter);

    CHKiRet(statsobj.Construct(&(pEntry->stats)));
    snprintf((char*)statname, sizeof(statname), "%s(%s)", pThis->pszInputName,
             pszPort);
 ...
 */
static rsRetVal configureTCPListen(tcpsrv_t* pThis, uchar* pszPort,
                                   int bSuppOctetFram, uchar* pszAddr)
{
    int i;
    uchar* pPort = pszPort;
    DEFiRet;
 ...
 */
static rsRetVal TCPSessTblInit(tcpsrv_t* pThis)
{
    DEFiRet;
    ...
    if ((pThis->pSessions = (tcps_sess_t**)calloc(
             pThis->iSessMax, sizeof(tcps_sess_t*))) == NULL)
    {
        DBGPRINTF("Error: TCPSessInit() could not alloc memory for TCP session "
                  "table.\n");
 ...
 * entry (0 or higher).
 */
static int TCPSessTblFindFreeSpot(tcpsrv_t* pThis)
{
    register int i;
 ...
 */
static rsRetVal addTcpLstn(void* pUsr, netstrm_t* pLstn)
{
...
static rsRetVal initTCPListener(tcpsrv_t* pThis, tcpLstnPortList_t* pPortEntry)
{
    ...
    CHKiRet(netstrm.LstnInit(pThis->pNS, (void*)pPortEntry, addTcpLstn,
                             TCPLstnPort, pPortEntry->pszAddr, pThis->iSessMax,
                             pThis->pszLstnPortFileName));
...
static rsRetVal create_tcp_socket(tcpsrv_t* pThis)
{
    DEFiRet;
    rsRetVal localRet;
    tcpLstnPortList_t* pEntry;
    ...
    while (pEntry != NULL)
    {
        ...
        if (localRet != RS_RET_OK)
        {
            LogError(0, localRet,
                     ...
                     "%s bind-address %s.",
                     pEntry->pszPort,
                     (pEntry->pszAddr == NULL) ? "(null)"
                                               : (const char*)pEntry->pszAddr);
        }
        pEntry = pEntry->pNext;
    }
 ...
 */
static rsRetVal SessAccept(tcpsrv_t* pThis, tcpLstnPortList_t* pLstnInfo,
                           tcps_sess_t** ppSess, netstrm_t* pStrm)
{
    DEFiRet;
    ...
    {
        CHKiRet(netstrm.SetGnutlsPriorityString(pNewStrm,
    ...
    /* TODO: check if we need to strip the domain name here -- rgerhards,
     * 2008-04-24 */

    /* Here we check if a host is permitted to send us messages. If it isn't, we
     * do not further process the message but log a warning (if we are
     * configured to do this). rgerhards, 2005-09-26
     */
    if (!pThis->pIsPermittedHost((struct sockaddr*)addr, (char*)fromHostFQDN,
                                 pThis->pUsr, pSess->pUsr))
    {
    ...
    pNewStrm = NULL; /* prevent it from being freed in error handler, now done
                        in tcps_sess! */
...
static void RunCancelCleanup(void* arg)
{
...
/* helper to close a session. Takes status of poll vs. select into
 * consideration. rgerhards, 2009-11-25
 */
static rsRetVal closeSess(tcpsrv_t* pThis, tcps_sess_t** ppSess,
                          nspoll_t* pPoll)
{
    DEFiRet;
    if (pPoll != NULL)
    {
        CHKiRet(nspoll.Ctl(pPoll, (*ppSess)->pStrm, 0, *ppSess, NSDPOLL_IN,
                           NSDPOLL_DEL));
    }
 ...
 */
static rsRetVal doReceive(tcpsrv_t* pThis, tcps_sess_t** ppSess,
                          nspoll_t* pPoll)
{
    ...
    switch (iRet)
    {
        case RS_RET_CLOSED:
            if (pThis->bEmitMsgOnClose)
            {
                ...
                LogError(0, RS_RET_PEER_CLOSED_CONN,
                         "Netstream session %p closed by remote "
                         "peer %s.\n",
            ...
            break;
        case RS_RET_RETRY:
            /* we simply ignore retry - this is not an error, but we also have
        ...
        case RS_RET_OK:
            ...
            if (localRet != RS_RET_OK && localRet != RS_RET_QUEUE_FULL)
            {
                 ...
                 */
                prop.GetString((*ppSess)->fromHostIP, &pszPeer, &lenPeer);
                LogError(oserr, localRet, "Tearing down TCP Session from %s",
                         pszPeer);
                CHKiRet(closeSess(pThis, ppSess, pPoll));
            }
            break;
        default:
            prop.GetString((*ppSess)->fromHostIP, &pszPeer, &lenPeer);
            LogError(oserr, iRet,
                     "netstream session %p from %s will be closed due to error",
                     (*ppSess)->pStrm, pszPeer);
            CHKiRet(closeSess(pThis, ppSess, pPoll));
            break;
    }
...
static rsRetVal ATTR_NONNULL(1)
    processWorksetItem(tcpsrv_t* const pThis, nspoll_t* pPoll, const int idx,
                       void* pUsr)
{
    ...
    if (pUsr == pThis->ppLstn)
    {
        ...
        iRet = SessAccept(pThis, pThis->ppLstnPort[idx], &pNewSess,
                          pThis->ppLstn[idx]);
        if (iRet == RS_RET_OK)
        {
            if (pPoll != NULL)
 ...
 */
static void* ATTR_NONNULL(1) wrkr(void* const myself)
{
 ...
 * as much as possible.
 */
static rsRetVal processWorkset(tcpsrv_t* pThis, nspoll_t* pPoll, int numEntries,
                               nsd_epworkset_t workset[])
{
    ...
    while (numEntries > 0)
    {
        ...
        else
        {
            ...
            for (i = 0; (i < wrkrMax) && ((wrkrInfo[i].pSrv != NULL) ||
            ...
            if (i < wrkrMax)
            {
                ...
                /* Note: we must increment wrkrRunning HERE and not inside the
                 * worker's code. This is because a worker may actually never
                 * start, and thus increment wrkrRunning, before we finish and
                 * check the running worker count. We can only avoid this by
...
PRAGMA_IGNORE_Wempty_body static rsRetVal
    RunSelect(tcpsrv_t* pThis, nsd_epworkset_t workset[], size_t sizeWorkset)
{
    ...
    while (1)
    {
        ...
        while (iTCPSess != -1)
        {
            ...
            CHKiRet(
                nssel.Add(pSel, pThis->pSessions[iTCPSess]->pStrm, NSDSEL_RD));
            /* now get next... */
        ...
        for (i = 0; i < pThis->iLstnCurr; ++i)
        {
            if (glbl.GetGlobalInputTermState() == 1)
                ABORT_FINALIZE(RS_RET_FORCE_TERM);
            CHKiRet(nssel.IsReady(pSel, pThis->ppLstn[i], NSDSEL_RD, &bIsReady,
                                  &nfds));
            if (bIsReady)
            {
        ...
        while (nfds && iTCPSess != -1)
        {
            if (glbl.GetGlobalInputTermState() == 1)
                ABORT_FINALIZE(RS_RET_FORCE_TERM);
            localRet = nssel.IsReady(pSel, pThis->pSessions[iTCPSess]->pStrm,
                                     NSDSEL_RD, &bIsReady, &nfds);
        ...
        nssel.Destruct(
            &pSel); /* no iRet check as it is overriden at start of loop! */
    finalize_it
        : /* this is a very special case - this time only we do not exit the
           * function, because that would not help us either. So we simply retry
           * it. Let's see if that actually is a better idea. Exiting the loop
           * wasn't we always crashed, which made sense (the rest of the engine
           * was not prepared for that) -- rgerhards, 2008-05-19
           */
        if (pSel != NULL)
        { /* cleanup missing? happens during err exit! */
            nssel.Destruct(&pSel);
        }
    }
 ...
 */
static rsRetVal Run(tcpsrv_t* pThis)
{
    DEFiRet;
    int i;
    int bFailed = FALSE; /* If set to TRUE, accept failed already */
    nsd_epworkset_t
        workset[128]; /* 128 is currently fixed num of concurrent requests */
    int numEntries;
    nspoll_t* pPoll = NULL;
    rsRetVal localRet;
    ...
    if (localRet != RS_RET_OK)
    {
        /* fall back to select */
        DBGPRINTF(
            "tcpsrv could not use epoll() interface, iRet=%d, using select()\n",
            localRet);
        iRet = RunSelect(pThis, workset,
                         sizeof(workset) / sizeof(nsd_epworkset_t));
        FINALIZE;
    }
    ...
    for (i = 0; i < pThis->iLstnCurr; ++i)
    {
        ...
        CHKiRet(nspoll.Ctl(pPoll, pThis->ppLstn[i], i, pThis->ppLstn,
                           NSDPOLL_IN, NSDPOLL_ADD));
    ...
    while (1)
    {
        ...
        /* check if we need to ignore the i/o ready state. We do this if we got
         * an invalid return state. Validly, this can happen for RS_RET_EINTR,
         * for other cases it may not be the right thing, but what is the right
         * thing is really hard at this point...
         */
        if (localRet != RS_RET_OK)
            continue;
        ...
        if (localRet != RS_RET_OK)
        {
            ...
            {
                DBGPRINTF("tcpsrv listener (inputname: '%s') still failing to "
                          "process "
...
BEGINobjConstruct(
    tcpsrv) /* be sure to specify the object type also in END macro! */
    ...
    static rsRetVal tcpsrvConstructFinalize(tcpsrv_t* pThis)
{
    ...
    if (pThis->pszDrvrPermitExpiredCerts != NULL)
        CHKiRet(netstrms.SetDrvrPermitExpiredCerts(
            pThis->pNS, pThis->pszDrvrPermitExpiredCerts));
    if (pThis->pPermPeers != NULL)
        CHKiRet(netstrms.SetDrvrPermPeers(pThis->pNS, pThis->pPermPeers));
    if (pThis->gnutlsPriorityString != NULL)
        CHKiRet(netstrms.SetDrvrGnutlsPriorityString(
            pThis->pNS, pThis->gnutlsPriorityString));
    CHKiRet(netstrms.ConstructFinalize(pThis->pNS));

    /* set up listeners */
    CHKmalloc(pThis->ppLstn = calloc(pThis->iLstnMax, sizeof(netstrm_t*)));
    CHKmalloc(pThis->ppLstnPort =
                  calloc(pThis->iLstnMax, sizeof(tcpLstnPortList_t*)));
...
BEGINobjDestruct(tcpsrv) /* be sure to specify the object type also in END and
                            CODESTART macros! */
    CODESTARTobjDestruct(tcpsrv) if (pThis->OnDestruct !=
                                     NULL) pThis->OnDestruct(pThis->pUsr);
...
ENDobjDestruct(tcpsrv)
    ...
    BEGINobjDebugPrint(tcpsrv) /* be sure to specify the object type also in END
                                  and CODESTART macros! */
    CODESTARTobjDebugPrint(tcpsrv) ENDobjDebugPrint(tcpsrv)

    /* set functions */
    static rsRetVal
    SetCBIsPermittedHost(tcpsrv_t* pThis,
                         int (*pCB)(struct sockaddr* addr, char* fromHostFQDN,
                                    void*, void*))
{
    DEFiRet;
...
static rsRetVal SetCBRcvData(tcpsrv_t* pThis,
                             rsRetVal (*pRcvData)(tcps_sess_t*, char*, size_t,
                                                  ssize_t*, int*))
{
    DEFiRet;
...
static rsRetVal SetCBOnSessAccept(tcpsrv_t* pThis,
                                  rsRetVal (*pCB)(tcpsrv_t*, tcps_sess_t*))
{
    DEFiRet;
...
static rsRetVal SetCBOnSessConstructFinalize(tcpsrv_t* pThis,
                                             rsRetVal (*pCB)(void*))
{
    DEFiRet;
...
static rsRetVal SetCBOnRegularClose(tcpsrv_t* pThis,
                                    rsRetVal (*pCB)(tcps_sess_t*))
{
    DEFiRet;
...
static rsRetVal SetLstnPortFileName(tcpsrv_t* pThis, uchar* iVal)
{
    DEFiRet;
...
static rsRetVal SetOnMsgReceive(tcpsrv_t* pThis,
                                rsRetVal (*OnMsgReceive)(tcps_sess_t*, uchar*,
                                                         int))
{
    DEFiRet;
...
static rsRetVal SetInputName(tcpsrv_t* pThis, uchar* name)
{
    uchar* pszName;
    DEFiRet;
    ISOBJ_TYPE_assert(pThis, tcpsrv);
...
static rsRetVal SetLinuxLikeRatelimiters(tcpsrv_t* pThis, int ratelimitInterval,
                                         int ratelimitBurst)
{
    DEFiRet;
...
static rsRetVal SetRuleset(tcpsrv_t* pThis, ruleset_t* pRuleset)
{
    DEFiRet;
...
static rsRetVal SetNotificationOnRemoteClose(tcpsrv_t* pThis, int bNewVal)
{
    DEFiRet;
...
static rsRetVal SetDrvrMode(tcpsrv_t* pThis, int iMode)
{
    DEFiRet;
    ISOBJ_TYPE_assert(pThis, tcpsrv);
...
static rsRetVal SetDrvrName(tcpsrv_t* pThis, uchar* name)
{
    DEFiRet;
    ISOBJ_TYPE_assert(pThis, tcpsrv);
    free(pThis->pszDrvrName);
...
static rsRetVal SetDrvrAuthMode(tcpsrv_t* pThis, uchar* mode)
{
    DEFiRet;
    ISOBJ_TYPE_assert(pThis, tcpsrv);
...
BEGINobjQueryInterface(tcpsrv)
    CODESTARTobjQueryInterface(tcpsrv) if (pIf->ifVersion !=
                                           tcpsrvCURR_IF_VERSION)
...
finalize_it
    : ENDobjQueryInterface(tcpsrv)
       ...
       */
      BEGINObjClassExit(
          tcpsrv, OBJ_IS_LOADABLE_MODULE) /* CHANGE class also in END MACRO! */
...
ENDObjClassExit(tcpsrv)
     ...
     * rgerhards, 2008-02-29
     */
    BEGINObjClassInit(tcpsrv, 1,
                      OBJ_IS_LOADABLE_MODULE) /* class, version - CHANGE class
                                                 also in END MACRO! */
    ...
    static void startWorkerPool(void)
{
    ...
    for (i = 0; i < wrkrMax; ++i)
    {
        ...
        r = pthread_create(&wrkrInfo[i].tid, &sessThrdAttr, wrkr,
                           &(wrkrInfo[i]));
        if (r == 0)
        {
        ...
        else
        {
            ...
            LogError(0, NO_ERRCODE,
                     "tcpsrv error creating thread %d: "
                     "%s",
                     i, errStr);
        }
    }
...
BEGINmodExit CODESTARTmodExit if (bWrkrRunning)
{
    stopWorkerPool();
    bWrkrRunning = 0;
}
...
ENDmodExit
    ...
    BEGINmodInit() CODESTARTmodInit* ipIFVersProvided =
        CURR_MOD_IF_VERSION; /* we only support the current interface
                                specification */
/* we just init the worker mutex, but do not start the workers themselves. This
 * is deferred to the first call of Run(). Reasons for this:
 * 1. depending on load order, tcpsrv gets loaded during rsyslog startup BEFORE
 *    it forks, in which case the workers would be running in the then-killed
 * parent, leading to a defuncnt child (we actually had this bug).
 * 2. depending on circumstances, Run() would possibly never be called, in which
 * case the worker threads would be totally useless. Note that in order to
 * guarantee a non-racy worker start, we need to guard the startup sequence by a
 * mutex, which is why we init it here (no problem with fork() in this case as
 * the mutex is a pure-memory structure). rgerhards, 2012-05-18
 */
...
CHKiRet(
    tcpsrvClassInit(pModInfo)); /* must be done after tcps_sess, as we use it */
ENDmodInit
