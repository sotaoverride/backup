...
static krb5_error_code locate_kpasswd(krb5_context context,
...
static void kpasswd_sendto_msg_cleanup(void* data, krb5_data* message)
{
...
static int kpasswd_sendto_msg_callback(SOCKET fd, void* data,
                                       krb5_data* message)
{
    ...
    else
    {
        ...
        local_kaddr.contents =
            k5memdup(addrs[0]->contents, addrs[0]->length, &code);
    ...
    if (ctx->set_password_for)
        code =
            krb5int_mk_setpw_req(ctx->context, ctx->auth_context, &ctx->ap_req,
                                 ctx->set_password_for, ctx->newpw, &output);
    else
        code = krb5int_mk_chpw_req(ctx->context, ctx->auth_context,
                                   &ctx->ap_req, ctx->newpw, &output);
    if (code)
        goto cleanup;
...
**  otherwise, the password is set for the principal indicated in
*set_password_for
*/
static krb5_error_code change_set_password(krb5_context context,
                                           krb5_creds* creds, const char* newpw,
                                           krb5_principal set_password_for,
                                           int* result_code,
                                           ...
                                           krb5_data* result_string)
{
    ...
    if ((code = krb5_mk_req_extended(
             callback_ctx.context, &callback_ctx.auth_context,
             AP_OPTS_USE_SUBKEY, NULL, creds, &callback_ctx.ap_req)))
        goto cleanup;
    ...
    code = k5_sendto(callback_ctx.context, NULL, &creds->server->realm, &sl,
                     UDP_LAST, &callback_info, &chpw_rep, ss2sa(&remote_addr),
                     &addrlen, NULL, NULL, NULL);
    if (code)
        goto cleanup;

    code = krb5int_rd_chpw_rep(callback_ctx.context, callback_ctx.auth_context,
                               &chpw_rep, &local_result_code, result_string);
...
{
    return change_set_password(context, creds, newpw, NULL, result_code,
    ...
    krb5_data* result_code_string, krb5_data* result_string)
{
    ...
    */
    ...
    if (!code)
    {
        code = krb5_build_principal(
            context, &creds.server, change_password_for->realm.length,
            change_password_for->realm.data, "kadmin", "changepw", NULL);
        if (!code)
        {
            code = krb5_get_credentials(context, 0, ccache, &creds, &credsp);
            if (!code)
            {
                code = krb5_set_password(context, credsp, newpw,
