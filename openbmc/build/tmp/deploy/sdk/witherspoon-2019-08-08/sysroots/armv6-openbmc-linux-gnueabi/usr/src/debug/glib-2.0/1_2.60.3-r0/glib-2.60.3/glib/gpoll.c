...
static int poll_rest(GPollFD* msg_fd, GPollFD* stop_fd, HANDLE* handles,
                     GPollFD* handle_to_fd[], gint nhandles, gint timeout_ms)
{
    DWORD ready;
    GPollFD* f;
    int recursed_result;
    ...
    else if (nhandles == 0)
    {
        ...
        {
            /* Wait for the current process to die, more efficient than
             * SleepEx(). */
    ...
    else
    {
        ...
        ready = WaitForMultipleObjectsEx(nhandles, handles, FALSE, timeout_ms,
                                         TRUE);
        if (ready == WAIT_FAILED)
        {
            gchar* emsg = g_win32_error_message(GetLastError());
    ...
    if (_g_main_poll_debug)
        g_print(
            ...
            (ready == WAIT_FAILED
                 ...
                 : (ready == WAIT_TIMEOUT
                        ? " (WAIT_TIMEOUT)"
                        : (msg_fd != NULL && ready == WAIT_OBJECT_0 + nhandles
                               ? " (msg)"
                               : ""))));
    ...
    else if (msg_fd != NULL && ready == WAIT_OBJECT_0 + nhandles)
    {
         ...
         */
        recursed_result =
            poll_rest(NULL, stop_fd, handles, handle_to_fd, nhandles, 0);
    ...
    else if (ready >= WAIT_OBJECT_0 && ready < WAIT_OBJECT_0 + nhandles)
    {
        ...
        if (timeout_ms == 0 && nhandles > 1)
        {
            ...
            recursed_result =
                poll_rest(NULL, stop_fd, shorter_handles, shorter_handle_to_fd,
                          shorter_nhandles, 0);
...
static gint poll_single_thread(GWin32PollThreadData* data)
{
    int retval;
    ...
    if (data->nhandles > 1 || (data->nhandles > 0 && data->msg_fd != NULL))
    {
         ...
         * available
         */
        retval = poll_rest(data->msg_fd, data->stop_fd, data->handles,
                           data->handle_to_fd, data->nhandles, 0);
         ...
         */
        if (retval == 0 &&
            (data->timeout_ms == INFINITE || data->timeout_ms > 0))
            retval =
                poll_rest(data->msg_fd, data->stop_fd, data->handles,
                          data->handle_to_fd, data->nhandles, data->timeout_ms);
    }
    else
    {
...
static void fill_poll_thread_data(GPollFD* fds, guint nfds, gint timeout_ms,
                                  GPollFD* stop_fd, GWin32PollThreadData* data)
{
    GPollFD* f;
    ...
    for (f = fds; f < &fds[nfds]; ++f)
    {
        if ((data->nhandles == MAXIMUM_WAIT_OBJECTS) ||
            (data->msg_fd != NULL &&
             (data->nhandles == MAXIMUM_WAIT_OBJECTS - 1)))
        {
...
static guint __stdcall poll_thread_run(gpointer user_data)
{
...
gint g_poll(GPollFD* fds, guint nfds, gint timeout)
{
    ...
    for (i = 0; i < nthreads; i++)
    {
        ...
        fill_poll_thread_data(fds + fds_idx, thread_fds, timeout, &stop_event,
                              &threads_data[i]);
        fds_idx += thread_fds;

        /* We must poll for messages from the same thread, so poll it along with
         * the threads */
        ...
        thread_handles[i] = (HANDLE)_beginthreadex(
            NULL, 0, poll_thread_run, &threads_data[i], 0, &ignore);
    }
    ...
    if (msg_fd != NULL)
        ...
        ready = WaitForMultipleObjects(nthreads, thread_handles, timeout > 0,
                                       timeout);
    ...
    /* The return value of all the threads give us all the fds that changed
     * state */
    retval = 0;
    ...
    for (i = 0; i < nthreads; i++)
    {
        if (GetExitCodeThread(thread_handles[i], &thread_retval))
            retval = retval == -1
                         ? -1
                         : thread_retval == -1 ? -1 : retval + thread_retval;
