    ...
    void init_register_nsVacm_context(const char* context)
{
    ...
    reg = netsnmp_create_handler_registration(
        "nsVacmAccessTable", nsVacmAccessTable_handler, nsVacmAccessTable_oid,
        OID_LENGTH(nsVacmAccessTable_oid), HANDLER_CAN_RWRITE);
#else  /* !NETSNMP_NO_WRITE_SUPPORT */
    reg = netsnmp_create_handler_registration(
        "nsVacmAccessTable", nsVacmAccessTable_handler, nsVacmAccessTable_oid,
        OID_LENGTH(nsVacmAccessTable_oid), HANDLER_CAN_RONLY);
#endif /* !NETSNMP_NO_WRITE_SUPPORT */

    table_info = SNMP_MALLOC_TYPEDEF(netsnmp_table_registration_info);
    netsnmp_table_helper_add_indexes(
        table_info, ASN_OCTET_STR, /* index: vacmGroupName */
...
netsnmp_variable_list* nsVacmAccessTable_get_first_data_point(
    ...
    netsnmp_variable_list* put_index_data, netsnmp_iterator_info* mydata)
{
    ...
    *my_loop_context = vacm_scanAccessNext();
    nsViewIdx = 0;
    return nsVacmAccessTable_get_next_data_point(
...
netsnmp_variable_list* nsVacmAccessTable_get_next_data_point(
    ...
    netsnmp_variable_list* put_index_data, netsnmp_iterator_info* mydata)
{
    struct vacm_accessEntry* entry = (struct vacm_accessEntry*)*my_loop_context;
...
int nsVacmAccessTable_handler(netsnmp_mib_handler* handler,
                              ...
                              netsnmp_request_info* requests)
{
    ...
    switch (reqinfo->mode)
    {
        ...
        case MODE_GET:
            ...
            {
                entry =
                    (struct vacm_accessEntry*)netsnmp_extract_iterator_context(
                        request);
                table_info = netsnmp_extract_table_info(request);

                /* Extract the authType token from the list of indexes */
                idx = table_info->indexes->next_variable->next_variable
        ...
        case MODE_SET_RESERVE1:
            for (request = requests; request; request = request->next)
            {
                ...
                {
                    case COLUMN_NSVACMCONTEXTMATCH:
                        ret = netsnmp_check_vb_int_range(request->requestvb, 1,
                                                         2);
                        break;
                    case COLUMN_NSVACMVIEWNAME:
                        ret = netsnmp_check_vb_type_and_max_size(
                            request->requestvb, ASN_OCTET_STR, VACM_MAX_STRING);
                        break;
                    case COLUMN_VACMACCESSSTORAGETYPE:
                        ret = netsnmp_check_vb_storagetype(
                            request->requestvb,
                            (/*entry ? entry->storageType :*/
                             SNMP_STORAGE_NONE));
                        break;
                    case COLUMN_NSVACMACCESSSTATUS:
                        /*
                         * The usual 'check_vb_rowstatus' call is too simplistic
                         *   to be used here.  Because we're implementing a
                         * table within an existing table, it's quite possible
                         * for a the vacmAccessTable entry to exist, even if
                         * this is a "new" nsVacmAccessEntry.
                         *
                         * We can check that the value being assigned is
                         * suitable for a RowStatus syntax object, but the
                         * transition checks need to be done explicitly.
                         */
                        ret = netsnmp_check_vb_rowstatus_value(
                            request->requestvb);
                        ...
                        memcpy(atype, (char*)idx->val.string, idx->val_len);
                        viewIdx =
                            se_find_value_in_slist(VACM_VIEW_ENUM_NAME, atype);
                        if (viewIdx < 0)
                        {
                        ...
                        switch (*request->requestvb->val.integer)
                        {
                            ...
                            case RS_NOTINSERVICE:
                                /* Check that this particular view is already
                                 * set */
                        ...
                        switch (*request->requestvb->val.integer)
                        {
                            ...
                            case RS_CREATEANDWAIT:
                                if (!entry)
                                {
                                    ...
                                    entry = vacm_createAccessEntry(
                                        gName, cPrefix, model, level);
