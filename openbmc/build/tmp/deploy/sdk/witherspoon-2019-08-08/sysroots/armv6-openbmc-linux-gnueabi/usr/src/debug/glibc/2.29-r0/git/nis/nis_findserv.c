                                               ...
                                               const struct timeval* timeout)
{
            ...
            if (strcmp(bind->server_val[i].ep.ep_val[j].family, "inet") == 0)
                    ...
                    (bind->server_val[i].ep.ep_val[j].proto[0] == '\0'))
                {
                    ...
                    if (sin.sin_addr.s_addr == 0)
                        continue;
                    sin.sin_port = htons(__pmap_getnisport(
                        &sin, NIS_PROG, NIS_VERSION, IPPROTO_UDP));
                    if (sin.sin_port == 0)
                        continue;
    ...
    for (i = 0; i < pings_count; ++i)
    {
        ...
        *((uint32_t*)(cu->cu_outbuf)) = pings[i].xid - 1;
        ...
        clnt_call(clnt, NULLPROC, (xdrproc_t)xdr_void, (caddr_t)0,
    ...
    while (found == -1)
    {
        ...
        int rc = clnt_call(clnt, NULLPROC, (xdrproc_t)NULL, (caddr_t)0,
                           (xdrproc_t)xdr_void, (caddr_t)0, *timeout);
        if (RPC_SUCCESS == rc)
        {
            uint32_t val;
