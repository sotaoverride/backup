...
FcBool FcHashStrCopy(const void* src, void** dest)
{
    *dest = FcStrdup(src);
...
FcBool FcHashUuidCopy(const void* src, void** dest)
{
#ifndef _WIN32
...
void FcHashUuidFree(void* data)
{
    free(data);
}

FcHashTable* FcHashTableCreate(FcHashFunc hash_func, FcCompareFunc compare_func,
                               FcCopyFunc key_copy_func,
                               FcCopyFunc value_copy_func,
                               FcDestroyFunc key_destroy_func,
                               FcDestroyFunc value_destroy_func)
...
FcBool FcHashTableFind(FcHashTable* table, const void* key, void** value)
{
    ...
    for (bucket = table->buckets[hash % FC_HASH_SIZE]; bucket;
         bucket = bucket->next)
    {
...
static FcBool FcHashTableAddInternal(FcHashTable* table, void* key, void* value,
                                     FcBool replace)
{
...
FcBool FcHashTableReplace(FcHashTable* table, void* key, void* value)
{
