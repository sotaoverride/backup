...
static void __attribute__((__noreturn__)) usage(void)
{
    ...
    fputs(_(" -d, --dig-holes      detect zeroes and replace with holes\n"),
          out);
    fputs(_(" -i, --insert-range   insert a hole at range, shifting existing "
            "data\n"),
          out);
...
static void xfallocate(int fd, int mode, off_t offset, off_t length)
{
    ...
    if (error < 0)
    {
        if ((mode & FALLOC_FL_KEEP_SIZE) && errno == EOPNOTSUPP)
            errx(EXIT_FAILURE,
                 _("fallocate failed: keep size mode is unsupported"));
...
static void dig_holes(int fd, off_t file_off, off_t len)
{
    ...
    while (file_end == 0 || file_off < file_end)
    {
        ...
        if ((off == -1 && errno == ENXIO) || (file_end && off >= file_end))
            break;
    ...
    {
        char* str =
            size_to_human_string(SIZE_SUFFIX_3LETTER | SIZE_SUFFIX_SPACE, ct);
...
#else
                errx(EXIT_FAILURE,
                     _("posix_fallocate support is not compiled"));
#endif
            case 'v':
                verbose++;
                break;
