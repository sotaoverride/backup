                             ...
                             void* value, size_t max_value_size)
{
             ...
             sizeof(struct nvuint64) <= max_value_size)
    {
        ...
        blkid_probe_sprintf_value(pr, "UUID_SUB", "%" PRIu64, nvu_value);
             ...
             sizeof(struct nvuint64) <= max_value_size)
    {
        ...
        blkid_probe_sprintf_uuid(pr, (unsigned char*)&nvu_value,
                                 sizeof(nvu_value), "%" PRIu64, nvu_value);
        return 1;
    }
    ...
    while (left > sizeof(*nvp) && nvp->nvp_size != 0 && found < 3)
    {
        ...
        DBG(LOWPROBE,
            ul_debug("nvlist: size %u, namelen %u, name %*s\n", nvp_size,
                     nvp_namelen, nvp_namelen, nvp->nvp_name));

        max_value_size = nvp_size - (namesize + sizeof(*nvp));
        value = nvp->nvp_name + namesize;

        found += zfs_process_value(pr, nvp->nvp_name, nvp_namelen, value,
                                   max_value_size);
                     ...
                     const struct blkid_idmag* mag __attribute__((__unused__)))
{
    ...
    blkid_probe_sprintf_version(
        pr, "%" PRIu64, swab_endian ? swab64(ub->ub_version) : ub->ub_version);

    zfs_extract_guid_name(pr, offset);

    if (blkid_probe_set_magic(pr, ub_offset, sizeof(ub->ub_magic),
...
const struct blkid_idinfo zfs_idinfo = {.name = "zfs_member",
