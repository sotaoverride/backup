...
static int match_symbol(const char* name, Lmid_t ns, ElfW(Word) hash,
                        const char* string, struct link_map* map, int verbose,
                        int weak)
{
...
checking for version `%s' in file %s [%lu] required by file %s [%lu]\n",
                         string, DSO_FILENAME(map->l_name), map->l_ns, name,
                         ns);
    ...
    /* XXX We cannot translate the message.  */
    _dl_exception_create_format(&exception, DSO_FILENAME(map->l_name),
...
int _dl_check_map_versions(struct link_map* map, int verbose, int trace_mode)
{
                ...
                !__builtin_expect(needed->l_faked, 0))
            {
               ...
               dependency symbols.  */
                ...
                while (1)
                {
                    /* Match the symbol.  */
                    result |= match_symbol(
                        DSO_FILENAME(map->l_name), map->l_ns, aux->vna_hash,
                        strtab + aux->vna_name, needed->l_real, verbose,
    ...
    if (ndx_high > 0)
    {
       ...
       section.  */
        map->l_versions = (struct r_found_version*)calloc(
            ndx_high + 1, sizeof(*map->l_versions));
        if (__glibc_unlikely(map->l_versions == NULL))
        {
            _dl_exception_create(&exception, DSO_FILENAME(map->l_name),
...
int _dl_check_all_versions(struct link_map* map, int verbose, int trace_mode)
{
    ...
    for (l = map; l != NULL; l = l->l_next)
        result |=
            (!l->l_faked && _dl_check_map_versions(l, verbose, trace_mode));
