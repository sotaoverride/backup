...
pid_t daemon_fork(void)
{
    ...
    else if (pid == 0)
    {
        ...
        if (_daemon_retval_pipe[0] >= 0 &&
            move_fd_up(&_daemon_retval_pipe[0]) < 0)
            goto fail;
        if (_daemon_retval_pipe[1] >= 0 &&
            move_fd_up(&_daemon_retval_pipe[1]) < 0)
            goto fail;

        if (_null_open(O_RDONLY, 0) < 0)
        {
            daemon_log(LOG_ERR, "Failed to open /dev/null for STDIN: %s",
                       strerror(errno));
            goto fail;
        }
        ...
        {
            daemon_log(LOG_ERR, "Failed to open /dev/null for STDOUT: %s",
                       strerror(errno));
            goto fail;
        }
        ...
        {
            daemon_log(LOG_ERR, "Failed to open /dev/null for STDERR: %s",
                       strerror(errno));
            goto fail;
        }
        ...
        else if (pid == 0)
        {
            ...
            {
                daemon_log(LOG_ERR, "sigprocmask() failed: %s",
                           strerror(errno));
                goto fail;
            }
            ...
            {
                daemon_log(LOG_ERR, "signal(SIGTTOU, SIG_IGN) failed: %s",
                           strerror(errno));
                goto fail;
            }
            ...
            {
                daemon_log(LOG_ERR, "signal(SIGTTIN, SIG_IGN) failed: %s",
                           strerror(errno));
                goto fail;
            }
            ...
            {
                daemon_log(LOG_ERR, "signal(SIGTSTP, SIG_IGN) failed: %s",
                           strerror(errno));
                goto fail;
            }
    ...
    fail:
        ...
        if (atomic_write(pipe_fds[1], &dpid, sizeof(dpid)) != sizeof(dpid))
            daemon_log(LOG_ERR, "Failed to write error PID: %s",
                       strerror(errno));
...
int daemon_retval_send(int i)
{
    ...
    if (r != sizeof(i))
    {
        ...
        if (r < 0)
            daemon_log(LOG_ERR,
                       "write() failed while writing return value to pipe: %s",
                       strerror(errno));
        else
        {
            daemon_log(
...
int daemon_retval_wait(int timeout)
{
    ...
    if (timeout > 0)
    {
        ...
        if ((s = select(FD_SETSIZE, &fds, 0, 0, &tv)) != 1)
        {
            ...
            if (s < 0)
                daemon_log(LOG_ERR,
                           "select() failed while waiting for return value: %s",
                           strerror(errno));
            else
            {
                errno = ETIMEDOUT;
    ...
    if ((r = atomic_read(_daemon_retval_pipe[0], &i, sizeof(i))) != sizeof(i))
    {
        ...
        if (r < 0)
            daemon_log(LOG_ERR,
                       "read() failed while reading return value from pipe: %s",
                       strerror(errno));
        else if (r == 0)
        {
            daemon_log(
                LOG_ERR,
                "read() failed with EOF while reading return value from pipe.");
            errno = EINVAL;
        }
        else if (r > 0)
        {
            daemon_log(
                LOG_ERR,
                "read() too short while reading return value from pipe.");
            errno = EINVAL;
        }
