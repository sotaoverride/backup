...
struct info
{
    ...
    typedef boost::variant<nil_, utf8_string, recursive_wrapper<info>,
                           recursive_wrapper<std::pair<info, info>>,
                           recursive_wrapper<std::list<info>>>
        value_type;

    explicit info(utf8_string const& tag_) : tag(tag_), value(nil_())
    {
    }

    template <typename T>
    info(utf8_string const& tag_, T const& value_) : tag(tag_), value(value_)
    {
    }
...
struct basic_info_walker
{
    ...
    basic_info_walker(Callback& callback_, utf8_string const& tag_,
                      int depth_) :
        callback(callback_),
        tag(tag_), depth(depth_)
    {
    }
    ...
    {
        boost::apply_visitor(this_type(callback, what.tag, depth + 1),
                             what.value);
    }
    ...
    {
        callback.element(tag, "", depth);
        boost::apply_visitor(this_type(callback, pair.first.tag, depth + 1),
                             pair.first.value);
        boost::apply_visitor(this_type(callback, pair.second.tag, depth + 1),
                             pair.second.value);
    }
...
struct simple_printer
{
    ...
    simple_printer(Out& out_) : out(out_)
    {
    }
