                                 ...
                                 uint8_t command, struct nl_msg** result)
{
    ...
    msg = nfnlmsg_alloc_simple(NFNL_SUBSYS_ULOG, NFULNL_MSG_CONFIG, 0, family,
                               queuenum);
    if (msg == NULL)
        return -NLE_NOMEM;
                                  ...
                                  struct nl_msg** result)
{
    ...
    msg = nfnlmsg_alloc_simple(NFNL_SUBSYS_ULOG, NFULNL_MSG_CONFIG, 0, 0,
                               nfnl_log_get_group(log));
    if (msg == NULL)
        return -NLE_NOMEM;
