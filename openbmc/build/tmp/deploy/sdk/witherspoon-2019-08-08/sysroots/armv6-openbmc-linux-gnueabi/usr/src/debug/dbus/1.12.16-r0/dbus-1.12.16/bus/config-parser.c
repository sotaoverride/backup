...
struct BusConfigParser
{
    ...
    DBusList* service_dirs; /**< BusConfigServiceDirs to look for session
                               services in */
    ...
    DBusHashTable*
        service_context_table; /**< Map service names to SELinux contexts */

    unsigned int fork : 1; /**< TRUE to fork into daemon mode */

    unsigned int syslog : 1; /**< TRUE to enable syslog */
    unsigned int
        keep_umask : 1; /**< TRUE to keep original umask when forking */

    unsigned int is_toplevel : 1; /**< FALSE if we are a sub-config-file inside
                                     another one */

    unsigned int
        allow_anonymous : 1; /**< TRUE to allow anonymous connections */
};
...
static dbus_bool_t merge_service_context_hash(DBusHashTable* dest,
                                              DBusHashTable* from)
{
    DBusHashIter iter;
    char* service_copy;
...
/*
 * Create a new BusConfigServiceDir. Takes ownership of path if #TRUE is
 * returned.
 *
 * @returns #FALSE on OOM
 */
static BusConfigServiceDir*
    bus_config_service_dir_new_take(char* path, BusServiceDirFlags flags)
{
...
static BusConfigServiceDir* service_dirs_find_dir(DBusList** service_dirs,
                                                  const char* dir)
{
    DBusList* link;
...
static void service_dirs_append_link_unique_or_free(DBusList** service_dirs,
                                                    DBusList* dir_link)
{
    BusConfigServiceDir* dir = dir_link->data;
    BusConfigServiceDir* already =
        service_dirs_find_dir(service_dirs, dir->path);
 ...
 */
static dbus_bool_t service_dirs_absorb_string_list(DBusList** service_dirs,
                                                   DBusList** dirs,
...
static dbus_bool_t merge_included(BusConfigParser* parser,
                                  BusConfigParser* included, DBusError* error)
{
    DBusList* link;
                                       ...
                                       const BusConfigParser* parent)
{
    ...
    if (((parser->policy = bus_policy_new()) == NULL) ||
        ...
        ((parser->service_context_table = _dbus_hash_table_new(
              DBUS_HASH_STRING, dbus_free, dbus_free)) == NULL))
    {
        if (parser->policy)
    ...
    else
    {
        ...
        allocation. */
        parser->limits.max_incoming_unix_fds =
            DBUS_DEFAULT_MESSAGE_UNIX_FDS * 4;
        parser->limits.max_outgoing_unix_fds =
            DBUS_DEFAULT_MESSAGE_UNIX_FDS * 4;
...
void bus_config_parser_unref(BusConfigParser* parser)
{
    ...
    if (parser->refcount == 0)
    {
        ...
        _dbus_list_foreach(&parser->listen_on, (DBusForeachFunction)dbus_free,
                           NULL);
        ...
        _dbus_list_foreach(&parser->conf_dirs, (DBusForeachFunction)dbus_free,
                           NULL);

        _dbus_list_clear(&parser->conf_dirs);

        _dbus_list_foreach(&parser->mechanisms, (DBusForeachFunction)dbus_free,
                           NULL);
...
static dbus_bool_t locate_attributes(BusConfigParser* parser,
    ...
    while (attribute_names[i])
    {
        ...
        while (j < n_attrs)
        {
            if (strcmp(attrs[j].name, attribute_names[i]) == 0)
            {
                ...
                {
                    dbus_set_error(error, DBUS_ERROR_FAILED,
                                   "Attribute \"%s\" repeated twice on the "
                                   "same <%s> element",
...
static dbus_bool_t check_no_attributes(BusConfigParser* parser,
                                       const char* element_name,
                                       const char** attribute_names,
                                       const char** attribute_values,
                                       DBusError* error)
...
static dbus_bool_t start_busconfig_child(BusConfigParser* parser,
                                         const char* element_name,
                                         const char** attribute_names,
                                         const char** attribute_values,
                                         DBusError* error)
{
    ...
    {
        if (!check_no_attributes(parser, "user", attribute_names,
    ...
    {
        if (!check_no_attributes(parser, "type", attribute_names,
                                 attribute_values, error))
            return FALSE;
    ...
    {
        if (!check_no_attributes(parser, "fork", attribute_names,
                                 attribute_values, error))
            return FALSE;
    ...
    {
        if (!check_no_attributes(parser, "syslog", attribute_names,
                                 attribute_values, error))
            return FALSE;
    ...
    {
        if (!check_no_attributes(parser, "keep_umask", attribute_names,
                                 attribute_values, error))
            return FALSE;
    ...
    {
        if (!check_no_attributes(parser, "pidfile", attribute_names,
                                 attribute_values, error))
            return FALSE;
    ...
    {
        if (!check_no_attributes(parser, "listen", attribute_names,
                                 attribute_values, error))
            return FALSE;
    ...
    {
        if (!check_no_attributes(parser, "auth", attribute_names,
                                 attribute_values, error))
            return FALSE;
    ...
    {
        if (!check_no_attributes(parser, "servicehelper", attribute_names,
                                 attribute_values, error))
            return FALSE;
    ...
    {
        if (!check_no_attributes(parser, "includedir", attribute_names,
                                 attribute_values, error))
            return FALSE;
    ...
    else if (element_type == ELEMENT_STANDARD_SESSION_SERVICEDIRS)
    {
        ...
        if (!check_no_attributes(parser, "standard_session_servicedirs",
         ...
         * directories with inotify, and allowed service files whose names do
         * not match the bus name */
    ...
    else if (element_type == ELEMENT_STANDARD_SYSTEM_SERVICEDIRS)
    {
        ...
        if (!check_no_attributes(parser, "standard_system_servicedirs",
                                 attribute_names, attribute_values, error))
            return FALSE;
         ...
         * directories with inotify, and allowed service files whose names do
         * not match the bus name (the servicehelper won't successfully activate
    ...
    {
        if (!check_no_attributes(parser, "allow_anonymous", attribute_names,
                                 attribute_values, error))
            return FALSE;
    ...
    {
        if (!check_no_attributes(parser, "servicedir", attribute_names,
                                 attribute_values, error))
            return FALSE;
    ...
    else if (element_type == ELEMENT_INCLUDE)
    {
        ...
        if (!locate_attributes(parser, "include", attribute_names,
                               attribute_values, error, "ignore_missing",
                               &ignore_missing, "if_selinux_enabled",
                               &if_selinux_enabled, "selinux_root_relative",
                               &selinux_root_relative, NULL))
            return FALSE;
        ...
        if (ignore_missing != NULL)
        {
            ...
            {
                dbus_set_error(error, DBUS_ERROR_FAILED,
                               "ignore_missing attribute must have value "
                               "\"yes\" or \"no\"");
                return FALSE;
            }
        }
    ...
    else if (element_type == ELEMENT_POLICY)
    {
        ...
        if (!locate_attributes(parser, "policy", attribute_names,
                               attribute_values, error, "context", &context,
                               "user", &user, "group", &group, "at_console",
                               &at_console, NULL))
            return FALSE;

        if (((context && user) || (context && group) ||
             (context && at_console)) ||
            ((user && group) || (user && at_console)) ||
            (group && at_console) || !(context || user || group || at_console))
        {
            dbus_set_error(error, DBUS_ERROR_FAILED,
                           "<policy> element must have exactly one of "
                           "(context|user|group|at_console) attributes");
            return FALSE;
        }
        ...
        if (context != NULL)
        {
            ...
            {
                dbus_set_error(error, DBUS_ERROR_FAILED,
                               "context attribute on <policy> must have the "
                               "value \"default\" or \"mandatory\", not \"%s\"",
                               context);
                return FALSE;
            }
        }
        else if (user != NULL)
        {
            ...
            if (_dbus_parse_unix_user_from_config(
        ...
        else if (group != NULL)
        {
            ...
            if (_dbus_parse_unix_group_from_config(
                    &group_name, &e->d.policy.gid_uid_or_at_console))
        ...
        else if (at_console != NULL)
        {
            ...
            {
                dbus_set_error(error, DBUS_ERROR_FAILED,
                               "Unknown value \"%s\" for at_console in message "
        ...
        else
        {
            _dbus_assert_not_reached(
                "all <policy> attributes null and we didn't set error");
        }
    ...
    else if (element_type == ELEMENT_LIMIT)
    {
        ...
        if (!locate_attributes(parser, "limit", attribute_names,
                               attribute_values, error, "name", &name, NULL))
            return FALSE;
    ...
    {
        if (!check_no_attributes(parser, "selinux", attribute_names,
                                 attribute_values, error))
            return FALSE;
    ...
    else if (element_type == ELEMENT_APPARMOR)
    {
        ...
        if (!locate_attributes(parser, "apparmor", attribute_names,
                               attribute_values, error, "mode", &mode, NULL))
            return FALSE;
 ...
 */
static dbus_bool_t parse_int_attribute(const char* name, const char* content,
                                       long min, long max, long def,
                                       long* value, DBusError* error)
{
...
static dbus_bool_t append_rule_from_element(BusConfigParser* parser,
                                            const char* element_name,
                                            const char** attribute_names,
                                            const char** attribute_values,
                                            dbus_bool_t allow, DBusError* error)
{
    ...
    if (!locate_attributes(
            parser, element_name, attribute_names, attribute_values, error,
            "send_interface", &send_interface, "send_member", &send_member,
            "send_error", &send_error, "send_destination", &send_destination,
            "send_path", &send_path, "send_type", &send_type, "send_broadcast",
            &send_broadcast, "receive_interface", &receive_interface,
            "receive_member", &receive_member, "receive_error", &receive_error,
            "receive_sender", &receive_sender, "receive_path", &receive_path,
            "receive_type", &receive_type, "eavesdrop", &eavesdrop, "max_fds",
            &max_fds_attr, "min_fds", &min_fds_attr, "send_requested_reply",
            &send_requested_reply, "receive_requested_reply",
            &receive_requested_reply, "own", &own, "own_prefix", &own_prefix,
            "user", &user, "group", &group, "log", &log, NULL))
        return FALSE;

    any_send_attribute = (send_destination != NULL || send_broadcast != NULL ||
                          send_path != NULL || send_type != NULL ||
                          send_interface != NULL || send_member != NULL ||
                          send_error != NULL || send_requested_reply != NULL);
    any_receive_attribute =
        (receive_sender != NULL || receive_path != NULL ||
         receive_type != NULL || receive_interface != NULL ||
         receive_member != NULL || receive_error != NULL ||
    ...
    any_message_attribute =
        (any_send_attribute || any_receive_attribute || eavesdrop != NULL ||
         max_fds_attr != NULL || min_fds_attr != NULL);

    if (!(any_send_attribute || any_receive_attribute || own || own_prefix ||
          user || group))
    {
        dbus_set_error(error, DBUS_ERROR_FAILED,
    ...
    {
        dbus_set_error(error, DBUS_ERROR_FAILED,
                       "On element <%s>, if you specify a member you must "
                       "specify an interface or a path. Keep in mind that not "
     ...
     * send_requested_reply, send_broadcast, eavesdrop base receive_ with
     * receive_sender, receive_path, receive_type, receive_requested_reply,
     * eavesdrop
     *
     *   user, group, own, own_prefix must occur alone
     */

    if (any_message_attribute + ((own != NULL) + (own_prefix != NULL) +
...
#define IS_WILDCARD(str) ((str) && ((str)[0]) == '*' && ((str)[1]) == '\0')
    ...
    if (any_send_attribute)
    {
        ...
        if (send_type != NULL)
        {
            ...
            {
                dbus_set_error(error, DBUS_ERROR_FAILED,
                               "Bad message type \"%s\"", send_type);
                return FALSE;
            }
        }

        if (eavesdrop && !(strcmp(eavesdrop, "true") == 0 ||
        ...
        if (send_broadcast && !(strcmp(send_broadcast, "true") == 0 ||
        ...
        if (send_destination != NULL && send_broadcast != NULL &&
        ...
        if (!parse_int_attribute(
                "max_fds", max_fds_attr, 0, DBUS_MAXIMUM_MESSAGE_UNIX_FDS,
                DBUS_MAXIMUM_MESSAGE_UNIX_FDS, &max_fds, error) ||
            !parse_int_attribute("min_fds", min_fds_attr, 0,
                                 DBUS_MAXIMUM_MESSAGE_UNIX_FDS, 0, &min_fds,
                                 error))
            return FALSE;
    ...
    else if (any_receive_attribute)
    {
        ...
        if (receive_type != NULL)
        {
            ...
            {
                dbus_set_error(error, DBUS_ERROR_FAILED,
                               "Bad message type \"%s\"", receive_type);
                return FALSE;
            }
        }
        ...
        if (receive_requested_reply)
            rule->d.receive.requested_reply =
                (strcmp(receive_requested_reply, "true") == 0);
    ...
    else if (user)
    {
        ...
        else
        {
            ...
            {
                _dbus_warn("Unknown username \"%s\" on element <%s>", user,
                           element_name);
            }
        }
    }
    else if (group)
    {
        ...
        else
        {
            ...
            {
                _dbus_warn("Unknown group \"%s\" on element <%s>", group,
                           element_name);
            }
        }
    }
    else
        _dbus_assert_not_reached("Did not handle some combination of "
                                 "attributes on <allow> or <deny>");
    ...
    if (rule != NULL)
    {
        ...
        switch (pe->d.policy.type)
        {
            ...
            case POLICY_USER:
                ...
                {
                    dbus_set_error(error, DBUS_ERROR_FAILED,
                                   "<%s> rule cannot be per-user because it "
                                   "has bus-global semantics",
                                   element_name);
                    goto failed;
                }

                if (!bus_policy_append_user_rule(
                        parser->policy, pe->d.policy.gid_uid_or_at_console,
                        rule))
                    goto nomem;
                break;
            case POLICY_GROUP:
                ...
                {
                    dbus_set_error(error, DBUS_ERROR_FAILED,
                                   "<%s> rule cannot be per-group because it "
                                   "has bus-global semantics",
                                   element_name);
                    goto failed;
                }

                if (!bus_policy_append_group_rule(
                        parser->policy, pe->d.policy.gid_uid_or_at_console,
                        rule))
                    goto nomem;
                break;
...
static dbus_bool_t start_policy_child(BusConfigParser* parser,
                                      const char* element_name,
                                      const char** attribute_names,
                                      const char** attribute_values,
                                      DBusError* error)
{
    ...
    {
        if (!append_rule_from_element(parser, element_name, attribute_names,
                                      attribute_values, TRUE, error))
            return FALSE;
...
static dbus_bool_t start_selinux_child(BusConfigParser* parser,
                                       const char* element_name,
                                       const char** attribute_names,
                                       const char** attribute_values,
                                       DBusError* error)
{
    ...
    if (strcmp(element_name, "associate") == 0)
    {
        ...
        if (!locate_attributes(parser, "associate", attribute_names,
                               attribute_values, error, "own", &own, "context",
                               &context, NULL))
            return FALSE;
        ...
        if (own == NULL || context == NULL)
        {
            dbus_set_error(
                error, DBUS_ERROR_FAILED,
                "Element <associate> must have attributes "
                "own=\"<servicename>\" and context=\"<selinux context>\"");
            return FALSE;
        }
                                            ...
                                            DBusError* error)
{
    ...
    if (t == ELEMENT_NONE)
    {
        ...
        {
            if (!check_no_attributes(parser, "busconfig", attribute_names,
                                     attribute_values, error))
                return FALSE;
    ...
    {
        return start_busconfig_child(parser, element_name, attribute_names,
                                     attribute_values, error);
    }
    else if (t == ELEMENT_POLICY)
    {
        return start_policy_child(parser, element_name, attribute_names,
                                  attribute_values, error);
    }
    else if (t == ELEMENT_SELINUX)
    {
        return start_selinux_child(parser, element_name, attribute_names,
                                   attribute_values, error);
    }
    else
    {
...
static dbus_bool_t set_limit(BusConfigParser* parser, const char* name,
                             long value, DBusError* error)
{
...
static dbus_bool_t make_full_path(const DBusString* basedir,
...
static dbus_bool_t include_file(BusConfigParser* parser,
                                const DBusString* filename,
                                dbus_bool_t ignore_missing, DBusError* error)
{
    ...
    {
        dbus_set_error(error, DBUS_ERROR_FAILED,
                       "Circular inclusion of file '%s'", filename_str);
        return FALSE;
    }
...
static dbus_bool_t servicehelper_path(BusConfigParser* parser,
                                      const DBusString* filename,
                                      DBusError* error)
{
    const char* filename_str;
...
static dbus_bool_t include_dir(BusConfigParser* parser,
                               const DBusString* dirname, DBusError* error)
{
    DBusString filename;
    dbus_bool_t retval;
    DBusError tmp_error;
...
#if 0
  {
    ...
    switch (top_element_type(parser))
    {
        ...
        case ELEMENT_APPARMOR:
            ...
            {
                dbus_set_error(error, DBUS_ERROR_FAILED,
                               "No text content expected inside XML element %s "
                               "in configuration file",
                               bus_config_parser_element_type_to_name(
                                   top_element_type(parser)));
                return FALSE;
            }
        ...
        case ELEMENT_INCLUDE:
        {
            ...
            if (e->d.include.if_selinux_enabled && !bus_selinux_enabled())
                break;
            ...
            if (e->d.include.selinux_root_relative)
            {
                ...
                {
                    dbus_set_error(error, DBUS_ERROR_FAILED,
                                   "Could not determine SELinux policy root "
                                   "for relative inclusion");
                    _dbus_string_free(&full_path);
                    return FALSE;
                }
            ...
            if (!include_file(parser, &full_path, e->d.include.ignore_missing,
                              error))
            {
                _dbus_string_free(&full_path);
                return FALSE;
        ...
        case ELEMENT_SERVICEDIR:
        {
             ...
             * directory with inotify, and allow service files whose names do
             * not match the bus name */
            dir =
                bus_config_service_dir_new_take(s, BUS_SERVICE_DIR_FLAGS_NONE);
        ...
        case ELEMENT_LIMIT:
        {
            ...
            {
                dbus_set_error(error, DBUS_ERROR_FAILED,
                               "<limit name=\"%s\"> element has invalid value "
                                       ...
                                       DBusError* error)
{
    ...
    {
        dbus_set_error(error, DBUS_ERROR_FAILED,
                       "Configuration file needs one or more <listen> elements "
                       "giving addresses");
        return FALSE;
    }
    ...
    bus_config_parser_steal_service_context_table(BusConfigParser* parser)
{
    ...
    _dbus_assert(parser->service_context_table !=
                 NULL); /* can only steal once */
...
static dbus_bool_t do_load(const DBusString* full_path, Validity validity,
...
static dbus_bool_t process_test_valid_subdir(const DBusString* test_base_dir,
                                             const char* subdir,
                                             Validity validity)
{
...
next:
    while (_dbus_directory_get_next_file(dir, &filename, &error))
    {
        ...
        d.validity = validity;
        d.check_own_rules =
            _dbus_string_ends_with_c_str(&full_path, "check-own-rules.conf");
         ...
         */
        /* if (!_dbus_test_oom_handling ("config-loader", check_loader_oom_func,
         * &d)) */
    ...
    switch (a->type)
    {
        ...
        case ELEMENT_POLICY:
            if (a->d.policy.type != b->d.policy.type)
                return FALSE;
            if (a->d.policy.gid_uid_or_at_console !=
...
{
    return (a->max_incoming_bytes == b->max_incoming_bytes ||
            ...
            a->max_match_rules_per_connection ==
                b->max_match_rules_per_connection ||
            a->max_replies_per_connection == b->max_replies_per_connection ||
            a->reply_timeout == b->reply_timeout);
}

static dbus_bool_t config_parsers_equal(const BusConfigParser* a,
    ...
    while (_dbus_directory_get_next_file(dir, &filename, &error))
    {
        ...
        if (parser == NULL)
        {
            _dbus_warn("Could not load file %s: %s",
                       _dbus_string_get_const_data(&full_path), error.message);
            _dbus_string_free(&full_path);
            dbus_error_free(&error);
            goto finished;
        }
...
static dbus_bool_t process_test_equiv_subdir(const DBusString* test_base_dir,
                                             const char* subdir)
{
    DBusString test_directory;
    DBusString filename;
    ...
    test_default_session_servicedirs(const DBusString* test_base_dir)
{
     ...
     * not */
    if (!_dbus_string_init(&full_path) || !_dbus_string_init(&progs) ||
    ...
    test_session_service_dir_matches[0] =
    ...
    if (common_progs)
    {
        ...
        test_session_service_dir_matches[1] =
            _dbus_string_get_const_data(&progs);
    }
#else
    ...
    if (!_dbus_string_append(&data_dirs_based, dbus_test_builddir) ||
        !_dbus_string_append(&data_dirs_based,
                             "/XDG_DATA_DIRS/dbus-1/services") ||
        !_dbus_string_append(&data_dirs_based2, dbus_test_builddir) ||
        !_dbus_string_append(&data_dirs_based2,
                             "/XDG_DATA_DIRS2/dbus-1/services") ||
    ...
    if (!_dbus_string_starts_with_c_str(&data_home_based, dbus_test_builddir))
        _dbus_assert_not_reached(
            "$XDG_DATA_HOME should start with $DBUS_TEST_BUILDDIR");

    if (!_dbus_string_starts_with_c_str(&runtime_dir_based, dbus_test_builddir))
        _dbus_assert_not_reached(
            "$XDG_RUNTIME_DIR should start with $DBUS_TEST_BUILDDIR");

    test_session_service_dir_matches[0] =
        _dbus_string_get_const_data(&runtime_dir_based);
    test_session_service_dir_matches[1] =
        _dbus_string_get_const_data(&data_home_based);
    test_session_service_dir_matches[2] =
        _dbus_string_get_const_data(&data_dirs_based);
    test_session_service_dir_matches[3] =
        _dbus_string_get_const_data(&data_dirs_based2);
#endif
...
static const char* test_system_service_dir_matches[] = {
    ...
    "/usr/share/dbus-1/system-services", DBUS_DATADIR "/dbus-1/system-services",
    "/lib/dbus-1/system-services", NULL};
...
static dbus_bool_t test_default_system_servicedirs(void)
{
    ...
    while ((link = _dbus_list_pop_first_link(&dirs)))
    {
        ...
        {
            printf("%s directory does not match %s in the match set\n",
                   (char*)link->data, test_system_service_dir_matches[i]);
            dbus_free(link->data);
            _dbus_list_free_link(link);
            return FALSE;
        }
...
#ifndef DBUS_WIN
    if (!process_test_valid_subdir(test_data_dir, "valid-config-files-system",
                                   VALID))
        return FALSE;
#endif
