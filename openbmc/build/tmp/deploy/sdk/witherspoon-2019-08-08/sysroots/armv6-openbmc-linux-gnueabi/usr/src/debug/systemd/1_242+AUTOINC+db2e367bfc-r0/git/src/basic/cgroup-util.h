...
typedef enum CGroupMask
{
    ...
    CGROUP_MASK_V1 = CGROUP_MASK_CPU | CGROUP_MASK_CPUACCT | CGROUP_MASK_BLKIO |
                     CGROUP_MASK_MEMORY | CGROUP_MASK_DEVICES |
                     CGROUP_MASK_PIDS,

    /* All real cgroup v2 controllers */
    CGROUP_MASK_V2 = CGROUP_MASK_CPU | CGROUP_MASK_IO | CGROUP_MASK_MEMORY |
                     CGROUP_MASK_PIDS,
...
{
    /* We always mount "cpu" and "cpuacct" in the same hierarchy. Hence, when
     * one bit is set also set the other */
...
{
    return x == CGROUP_WEIGHT_INVALID ||
...
{
    return x == CGROUP_CPU_SHARES_INVALID ||
...
{
    return x == CGROUP_BLKIO_WEIGHT_INVALID ||
           (x >= CGROUP_BLKIO_WEIGHT_MIN && x <= CGROUP_BLKIO_WEIGHT_MAX);
}

/* Default resource limits */
#define DEFAULT_TASKS_MAX_PERCENTAGE                                           \
    15U /* 15% of PIDs, 4915 on default settings */
#define DEFAULT_USER_TASKS_MAX_PERCENTAGE                                      \
    33U /* 33% of PIDs, 10813 on default settings */
...
int cg_kill(const char* controller, const char* path, int sig,
            CGroupFlags flags, Set* s, cg_kill_log_func_t kill_log,
            void* userdata);
...
int cg_get_keyed_attribute(const char* controller, const char* path,
                           const char* attribute, char** keys, char** values);
...
int cg_migrate_everywhere(CGroupMask supported, const char* from,
                          const char* to, cg_migrate_callback_t callback,
                          void* userdata);
