...
void* header_hrstoreEntry(struct variable*, oid*, size_t*, int, size_t*,
                          WriteMethod**);
...
struct variable2 hrstore_variables[] = {
    {HRSTORE_INDEX, ASN_INTEGER, NETSNMP_OLDAPI_RONLY, var_hrstore, 1, {1}},
    {HRSTORE_TYPE, ASN_OBJECT_ID, NETSNMP_OLDAPI_RONLY, var_hrstore, 1, {2}},
    {HRSTORE_DESCR, ASN_OCTET_STR, NETSNMP_OLDAPI_RONLY, var_hrstore, 1, {3}},
    {HRSTORE_UNITS, ASN_INTEGER, NETSNMP_OLDAPI_RONLY, var_hrstore, 1, {4}},
    {HRSTORE_SIZE, ASN_INTEGER, NETSNMP_OLDAPI_RONLY, var_hrstore, 1, {5}},
    {HRSTORE_USED, ASN_INTEGER, NETSNMP_OLDAPI_RONLY, var_hrstore, 1, {6}},
    {HRSTORE_FAILS, ASN_COUNTER, NETSNMP_OLDAPI_RONLY, var_hrstore, 1, {7}}};
...
void init_hrh_storage(void)
{
    ...
    netsnmp_register_scalar(netsnmp_create_handler_registration(
        "host/hrMemorySize", handle_memsize, hrMemorySize_oid,
        OID_LENGTH(hrMemorySize_oid), HANDLER_CAN_RONLY));
...
static int storageUseNFS =
    1; /* Default to reporting NFS mounts as NetworkDisk */

static void parse_storage_config(const char* token, char* cptr)
{
    char* val;
    int ival;
    char* st;
...
void* header_hrstoreEntry(struct variable* vp, oid* name, size_t* length,
...
int handle_memsize(netsnmp_mib_handler* handler,
...
u_char* var_hrstore(struct variable* vp, oid* name, size_t* length, int exact,
...
really_try_next:
    ...
    if (HRFS_entry && store_idx > NETSNMP_MEM_TYPE_MAX &&
    ...
    switch (vp->magic)
    {
        ...
        case HRSTORE_TYPE:
            if (store_idx > NETSNMP_MEM_TYPE_MAX)
                if (HRFS_entry->flags & NETSNMP_FS_FLAG_REMOTE && storageUseNFS)
                    storage_type_id[storage_type_len - 1] =
                        10; /* Network Disk */
        ...
        default:
            DEBUGMSGTL(
                ("snmpd", "unknown sub-id %d in var_hrstore\n", vp->magic));
    }
    return NULL;
