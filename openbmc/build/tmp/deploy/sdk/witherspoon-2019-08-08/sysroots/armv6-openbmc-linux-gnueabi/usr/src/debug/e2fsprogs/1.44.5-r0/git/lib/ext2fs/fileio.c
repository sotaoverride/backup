...
errcode_t ext2fs_file_open(ext2_filsys fs, ext2_ino_t ino, int flags,
                           ext2_file_t* ret)
{
...
errcode_t ext2fs_file_flush(ext2_file_t file)
{
    ...
    {
        retval = ext2fs_bmap2(fs, file->ino, &file->inode, BMAP_BUFFER, 0,
                              file->blockno, &ret_flags, &dontcare);
        if (retval)
            return retval;
        if (ret_flags & BMAP_RET_UNINIT)
        {
            retval = ext2fs_bmap2(fs, file->ino, &file->inode, BMAP_BUFFER,
                                  BMAP_SET, file->blockno, 0, &file->physblock);
            if (retval)
                return retval;
        }
    }
    ...
    {
        retval = ext2fs_bmap2(fs, file->ino, &file->inode, BMAP_BUFFER,
                              file->ino ? BMAP_ALLOC : 0, file->blockno, 0,
                              &file->physblock);
        if (retval)
            return retval;
    }
...
static errcode_t load_buffer(ext2_file_t file, int dontfill)
{
    ...
    {
        retval = ext2fs_bmap2(fs, file->ino, &file->inode, BMAP_BUFFER, 0,
                              file->blockno, &ret_flags, &file->physblock);
        if (retval)
            return retval;
        if (!dontfill)
        {
            if (file->physblock && !(ret_flags & BMAP_RET_UNINIT))
            {
                retval = io_channel_read_blk64(fs->io, file->physblock, 1,
                                               file->buf);
                if (retval)
                    return retval;
            }
...
static errcode_t ext2fs_file_read_inline_data(ext2_file_t file, void* buf,
                                              unsigned int wanted,
                                              unsigned int* got)
{
    ext2_filsys fs;
    ...
    fs = file->fs;
    retval =
        ext2fs_inline_data_get(fs, file->ino, &file->inode, file->buf, &size);
    if (retval)
        return retval;
...
errcode_t ext2fs_file_read(ext2_file_t file, void* buf, unsigned int wanted,
                           unsigned int* got)
{
    ext2_filsys fs;
    errcode_t retval = 0;
...
static errcode_t ext2fs_file_write_inline_data(ext2_file_t file,
                                               const void* buf,
                                               unsigned int nbytes,
                                               unsigned int* written)
{
    ...
    if (file->pos < size)
    {
        ...
        retval = ext2fs_inline_data_set(fs, file->ino, &file->inode, file->buf,
                                        count);
                            ...
                            unsigned int nbytes, unsigned int* written)
{
    ...
    if (file->inode.i_flags & EXT4_INLINE_DATA_FL)
    {
        retval = ext2fs_file_write_inline_data(file, buf, nbytes, written);
        if (retval != EXT2_ET_INLINE_DATA_NO_SPACE)
            return retval;
    ...
    while (nbytes > 0)
    {
        ...
        if (!file->physblock)
        {
            ...
            if (fs->flags & EXT2_FLAG_SHARE_DUP)
            {
                ...
                ext2fs_sha512((const unsigned char*)file->buf, fs->blocksize,
                              new_block->sha);
                old_block = ext2fs_hashmap_lookup(
                    fs->block_sha_map, new_block->sha, sizeof(new_block->sha));
            }
            ...
            retval =
                ext2fs_bmap2(fs, file->ino, &file->inode, BMAP_BUFFER,
                             bmap_flags, file->blockno, 0, &file->physblock);
            if (retval)
            {
                free(new_block);
                new_block = NULL;
            ...
            if (new_block)
            {
                ...
                ext2fs_hashmap_add(fs->block_sha_map, new_block, new_block->sha,
...
errcode_t ext2fs_file_llseek(ext2_file_t file, __u64 offset, int whence,
                             __u64* ret_pos)
{
    EXT2_CHECK_MAGIC(file, EXT2_ET_MAGIC_EXT2_FILE);
...
errcode_t ext2fs_file_lseek(ext2_file_t file, ext2_off_t offset, int whence,
                            ext2_off_t* ret_pos)
{
                                              ...
                                              ext2_off64_t offset)
{
    ...
    retval = ext2fs_bmap2(fs, file->ino, NULL, NULL, 0, offset / fs->blocksize,
                          &ret_flags, &blk);
    if (retval)
        return retval;
...
errcode_t ext2fs_file_set_size2(ext2_file_t file, ext2_off64_t size)
{
    ...
    if (size && ext2fs_file_block_offset_too_big(
                    file->fs, &file->inode, (size - 1) / file->fs->blocksize))
    ...
    return ext2fs_punch(file->fs, file->ino, &file->inode, 0, truncate_block,
                        ~0ULL);
}
