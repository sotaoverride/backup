...
#define trace(X)                                                               \
    do                                                                         \
    {                                                                          \
        _gpgrt_internal_trace_begin(ENABLE_TRACING, __func__, __LINE__, 0);    \
        _gpgrt_internal_trace X;                                               \
        _gpgrt_internal_trace_end();                                           \
    } while (0)
#define trace_errno(C, X)                                                      \
    do                                                                         \
    {                                                                          \
        _gpgrt_internal_trace_begin(ENABLE_TRACING, __func__, __LINE__, (C));  \
        _gpgrt_internal_trace X;                                               \
        _gpgrt_internal_trace_end();                                           \
    } while (0)
...
#define trace_append(X)                                                        \
    do                                                                         \
    {                                                                          \
        _gpgrt_internal_trace_printf X;                                        \
    } while (0)
#define trace_finish(X)                                                        \
    do                                                                         \
    {                                                                          \
        _gpgrt_internal_trace_printf X;                                        \
        _gpgrt_internal_trace_end();                                           \
...
void _gpgrt_internal_trace_begin(const char* mod, const char* file, int line,
                                 int with_errno);
void _gpgrt_internal_trace(const char* format, ...) GPGRT_ATTR_PRINTF(1, 2);
...
gpg_err_code_t _gpgrt_b64enc_write(gpgrt_b64state_t state, const void* buffer,
                                   size_t nbytes);
...
#define gpgrt_assert(expr)                                                     \
    ((expr) ? (void)0 : _gpgrt__log_assert(#expr, __FILE__, __LINE__, NULL))
 ...
 * should then be 0.   */
gpg_err_code_t _gpgrt_make_pipe(int filedes[2], estream_t* r_fp, int direction,
                                int nonblock);
 ...
 */
gpg_err_code_t _gpgrt_spawn_process(const char* pgmname, const char* argv[],
 ...
 * error code. */
gpg_err_code_t _gpgrt_spawn_process_fd(const char* pgmname, const char* argv[],
...
gpg_err_code_t _gpgrt_setenv(const char* name, const char* value,
                             int overwrite);
...
char* _gpgrt_w32_reg_query_string(const char* root, const char* dir,
                                  const char* name);
...
#ifndef HAVE_STPCPY
static GPG_ERR_INLINE char* _gpgrt_stpcpy(char* a, const char* b)
{
    while (*b)
        *a++ = *b++;
    *a = 0;
