 ...
 *  The table iterator helper is designed to simplify the task of writing a
 table handler for the net-snmp agent when the data being accessed is not in an
 oid sorted form and must be accessed externally.
 *  @ingroup table
...
netsnmp_feature_child_of(table_iterator_all, mib_helpers)
    ...
    netsnmp_feature_child_of(table_iterator_insert_context, table_iterator_all)
        netsnmp_feature_child_of(table_iterator_create_table,
                                 table_iterator_all)
            netsnmp_feature_child_of(table_iterator_row_first,
                                     table_iterator_all)
                netsnmp_feature_child_of(table_iterator_row_count,
                                         table_iterator_all)
...
static netsnmp_iterator_info* netsnmp_iterator_ref(netsnmp_iterator_info* iinfo)
{
    iinfo->refcnt++;
    return iinfo;
}

static void netsnmp_iterator_deref(netsnmp_iterator_info* iinfo)
{
    if (--iinfo->refcnt == 0)
    ...
    for (; req; req = req->next)
    {
            ...
            0)
        {
            netsnmp_request_add_list_data(
                req, netsnmp_create_data_list(TABLE_ITERATOR_NAME, data, NULL));
        }
    }
}
...
static void netsnmp_free_ti_cache(void* it)
{
...
static ti_cache_info* netsnmp_iterator_remember(netsnmp_request_info* request,
                                                oid* oid_to_save,
                                                ...
                                                netsnmp_iterator_info* iinfo)
{
    ...
    ti_info = (ti_cache_info*)netsnmp_request_get_list_data(request,
                                                            TI_REQUEST_CACHE);
    ...
    if (!ti_info)
    {
        ...
        if (ti_info == NULL)
            return NULL;
        netsnmp_request_add_list_data(
            request, netsnmp_create_data_list(TI_REQUEST_CACHE, ti_info,
                                          ...
                                          netsnmp_request_info* requests)
{
    ...
    switch (reqinfo->mode)
    {
        ...
        case MODE_GET_STASH:
            ...
            netsnmp_request_add_list_data(
                reqtmp, netsnmp_create_data_list(TABLE_HANDLER_NAME,
        ...
        case MODE_GETNEXT:
            for (request = requests; request; request = request->next)
            {
                ...
                ti_info = (ti_cache_info*)netsnmp_request_get_list_data(
                    request, TI_REQUEST_CACHE);
                if (!ti_info)
                {
                    ti_info = SNMP_MALLOC_TYPEDEF(ti_cache_info);
                    if (ti_info == NULL)
                    ...
                    netsnmp_request_add_list_data(
                        request,
                        netsnmp_create_data_list(TI_REQUEST_CACHE, ti_info,
                                                 netsnmp_free_ti_cache));
                }
    ...
    )
    {
        ...
        while (notdone)
        {
            ...
            if (!index_search)
            {
                ...
                else
                {
                    ...
                    if (!index_search)
                    {
                         ...
                         */
                        snmp_log(LOG_WARNING,
                                 "invalid index list or failed malloc for "
                                 "table %s\n",
                                 reginfo->handlerName);
                        netsnmp_free_request_data_sets(reqtmp);
                        SNMP_FREE(reqtmp);
            ...
            while (index_search)
            {
                    ...
                    switch (reqinfo->mode)
                    {
                        ...
                        case MODE_SET_RESERVE1:
                                    ...
                                    request->requestvb->name_length) == 0)
                            {
                                 ...
                                 * keep this
                                 */
                                if (netsnmp_iterator_remember(
                                        request, myname, myname_len,
                                        callback_data_context,
                                        callback_loop_context, iinfo) == NULL)
                                {
                                    /*
                                     * Cleanup
                                     */
                            ...
                            {
                                if (iinfo->free_data_context &&
                                    callback_data_context)
                                {
                                    (iinfo->free_data_context)(
                                        callback_data_context, iinfo);
                                }
                            }
                            break;
                        ...
                        case MODE_GET_STASH:
                            ...
                            if (reqtmp)
                                ldata = netsnmp_get_list_node(
                                    reqtmp->parent_data, TABLE_ITERATOR_NAME);
                            if (!ldata)
                            {
                                netsnmp_request_add_list_data(
                                    reqtmp, netsnmp_create_data_list(
                                                TABLE_ITERATOR_NAME,
                        ...
                        default:
                            snmp_log(LOG_ERR, "table_iterator called with "
                                              "unsupported mode\n");
                            break; /* XXX return */
                    }
                }
                ...
                index_search = (iinfo->get_next_data_point)(
                    &callback_loop_context, &callback_data_context,
                    index_search, iinfo);
                                     ...
                                     netsnmp_variable_list* indexes)
{
    ...
    build_oid_noalloc(instance, MAX_OID_LEN, &len, dummy, 2, indexes);
                                     ...
                                     oid* instance, size_t len)
{
    ...
    if (!iinfo || !iinfo->get_first_data_point || !iinfo->get_next_data_point)
        return NULL;
...
int netsnmp_iterator_row_count(netsnmp_iterator_info* iinfo)
{
    ...
    DEBUGMSGTL(
        ("table:iterator:count", "first DP: %p %p %p\n", ctx1, ctx2, vp2));
