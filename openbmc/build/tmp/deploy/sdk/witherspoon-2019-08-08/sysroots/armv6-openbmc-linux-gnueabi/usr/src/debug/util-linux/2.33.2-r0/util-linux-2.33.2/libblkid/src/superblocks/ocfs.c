...
#define ocfsmajor(o)                                                           \
    ((uint32_t)o.major_version[0] + (((uint32_t)o.major_version[1]) << 8) +    \
     (((uint32_t)o.major_version[2]) << 16) +                                  \
     (((uint32_t)o.major_version[3]) << 24))

#define ocfsminor(o)                                                           \
    ((uint32_t)o.minor_version[0] + (((uint32_t)o.minor_version[1]) << 8) +    \
     (((uint32_t)o.minor_version[2]) << 16) +                                  \
     (((uint32_t)o.minor_version[3]) << 24))

#define ocfslabellen(o)                                                        \
    ((uint32_t)o.label_len[0] + (((uint32_t)o.label_len[1]) << 8))
#define ocfsmountlen(o)                                                        \
    ((uint32_t)o.mount_len[0] + (((uint32_t)o.mount_len[1]) << 8))
...
static int probe_ocfs(blkid_probe pr, const struct blkid_idmag* mag)
{
    ...
    if (maj == 1)
        blkid_probe_set_value(pr, "SEC_TYPE", (unsigned char*)"ocfs1",
                              sizeof("ocfs1"));
    else if (maj >= 9)
        blkid_probe_set_value(pr, "SEC_TYPE", (unsigned char*)"ntocfs",
                              sizeof("ntocfs"));

    blkid_probe_set_label(pr, (unsigned char*)ovl.label, ocfslabellen(ovl));
...
static int probe_ocfs2(blkid_probe pr, const struct blkid_idmag* mag)
{
    ...
    blkid_probe_set_label(pr, (unsigned char*)osb->s_label,
                          sizeof(osb->s_label));
...
const struct blkid_idinfo ocfs_idinfo = {
    ...
    .minsz = 14000 * 1024,
    .magics = {{.magic = "OracleCFS", .len = 9, .kboff = 8}, {NULL}}};
...
const struct blkid_idinfo ocfs2_idinfo = {
    ...
    .minsz = 14000 * 1024,
    .magics = {{.magic = "OCFSV2", .len = 6, .kboff = 1},
...
const struct blkid_idinfo oracleasm_idinfo = {
    ...
    .magics = {{.magic = "ORCLDISK", .len = 8, .sboff = 32}, {NULL}}};
