...
#define AFM_GETC()                                                             \
    (((stream)->cursor < (stream)->limit) ? *(stream)->cursor++ : EOF)
...
static const char* const afm_key_table[N_AFM_TOKENS] = {"Ascender",
                                                        "AxisLabel",
                                                        "AxisType",
                                                        "B",
                                                        "BlendAxisTypes",
    ...
    for (i = 0; i < n; i++)
    {
        ...
        switch (val->type)
        {
            ...
            case AFM_VALUE_TYPE_INTEGER:
                val->u.i =
                    PS_Conv_ToInt((FT_Byte**)(void*)&str, (FT_Byte*)str + len);
                break;

            case AFM_VALUE_TYPE_BOOL:
                val->u.b = FT_BOOL(len == 4 && !ft_strncmp(str, "true", 4));
                break;
...
afm_parser_next_key(AFM_Parser parser, FT_Bool line, FT_Offset* len)
{
    ...
    if (line)
    {
        while (1)
        {
            ...
            /* skip empty line */
            if (!key && !AFM_STATUS_EOF(stream) && AFM_STATUS_EOL(stream))
                continue;
    ...
    else
    {
        while (1)
        {
            ...
            /* skip empty column */
            if (!key && !AFM_STATUS_EOF(stream) && AFM_STATUS_EOC(stream))
                continue;
    ...
    if (len)
        *len = (key) ? (FT_Offset)AFM_STREAM_KEY_LEN(stream, key) : 0;
...
afm_parser_init(AFM_Parser parser, FT_Memory memory, FT_Byte* base,
                FT_Byte* limit)
{
        ...
        switch (token)
        {
            ...
            case AFM_TOKEN_ENDFONTMETRICS:
                ...
                ft_qsort(fi->KernPairs, fi->NumKernPair,
                         sizeof(AFM_KernPairRec), afm_compare_kern_pairs);
                return FT_Err_Ok;
...
static FT_Error afm_parser_skip_section(AFM_Parser parser, FT_Int n,
...
afm_parser_parse(AFM_Parser parser)
{
    ...
    if (!key || len != 16 || ft_strncmp(key, "StartFontMetrics", 16) != 0)
