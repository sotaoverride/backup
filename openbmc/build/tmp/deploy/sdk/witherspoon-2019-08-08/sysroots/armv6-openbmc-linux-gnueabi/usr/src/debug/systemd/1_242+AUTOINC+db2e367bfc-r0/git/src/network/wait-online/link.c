...
int link_update_monitor(Link* l)
{
    ...
    if (r < 0)
        ret = log_link_debug_errno(l, r,
                                   "Failed to determine whether the link is "
                                   "required for online or not, "
                                   "ignoring: %m");
    else
        l->required_for_online = r > 0;

    r = sd_network_link_get_required_operstate_for_online(l->ifindex,
                                                          &required_operstate);
    if (r < 0)
        ret = log_link_debug_errno(
            l, r, "Failed to get required operational state, ignoring: %m");
    else
    {
    ...
    if (r < 0)
        ret = log_link_debug_errno(
            l, r, "Failed to get operational state, ignoring: %m");
    else
    {
