...
{
    return genl_send_simple(h, GENL_ID_CTRL, CTRL_CMD_GETFAMILY, CTRL_VERSION,
                            NLM_F_DUMP);
}
                           ...
                           struct genl_info* info, void* arg)
{
    ...
    genl_family_set_id(family, nla_get_u16(info->attrs[CTRL_ATTR_FAMILY_ID]));
    ...
    if (info->attrs[CTRL_ATTR_OPS])
    {
        ...
        nla_for_each_nested(nla, nla_ops, remaining)
        {
            ...
            err = nla_parse_nested(tb, CTRL_ATTR_OP_MAX, nla, family_op_policy);
            if (err < 0)
                goto errout;
                                                   ...
                                                   const char* name)
{
    ...
    if (!genlmsg_put(msg, NL_AUTO_PORT, NL_AUTO_SEQ, GENL_ID_CTRL, 0, 0,
