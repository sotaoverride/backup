...
static gboolean g_io_stream_real_close(GIOStream* stream,
                                       ...
                                       GError** error);
static void g_io_stream_real_close_async(GIOStream* stream, int io_priority,
                                         GCancellable* cancellable,
...
static void g_io_stream_get_property(GObject* object, guint prop_id,
...
static void g_io_stream_class_init(GIOStreamClass* klass)
{
    ...
    g_object_class_install_property(
        ...
        g_param_spec_boolean("closed", P_("Closed"), P_("Is the stream closed"),
                             FALSE, G_PARAM_READABLE | G_PARAM_STATIC_STRINGS));

    g_object_class_install_property(
        gobject_class, PROP_INPUT_STREAM,
        g_param_spec_object("input-stream", P_("Input stream"),
                            P_("The GInputStream to read from"),
                            G_TYPE_INPUT_STREAM,
                            G_PARAM_READABLE | G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(
        gobject_class, PROP_OUTPUT_STREAM,
        g_param_spec_object("output-stream", P_("Output stream"),
 ...
 * Since: 2.22
 */
gboolean g_io_stream_close(GIOStream* stream, GCancellable* cancellable,
                           GError** error)
{
 ...
 * Since: 2.22
 */
void g_io_stream_close_async(GIOStream* stream, int io_priority,
                             GCancellable* cancellable,
 ...
 * Since: 2.22
 */
gboolean g_io_stream_close_finish(GIOStream* stream, GAsyncResult* result,
                                  GError** error)
{
    g_return_val_if_fail(G_IS_IO_STREAM(stream), FALSE);
...
static void stream_close_complete(GObject* source, GAsyncResult* result,
                                  gpointer user_data)
{
    GTask* task = user_data;
    CloseAsyncData* data;
    ...
    if (G_IS_OUTPUT_STREAM(source))
    {
        ...
        g_input_stream_close_finish(G_INPUT_STREAM(source), result,
                                         ...
                                         gpointer user_data)
{
    ...
    if (g_input_stream_async_close_is_via_threads(input) &&
    ...
    else
    {
        ...
        g_input_stream_close_async(input, io_priority, cancellable,
                                   stream_close_complete, task);
        g_output_stream_close_async(output, io_priority, cancellable,
                                    stream_close_complete, task);
    }
}
...
static void splice_complete(GTask* task, SpliceContext* ctx)
{
...
static void splice_close_cb(GObject* iostream, GAsyncResult* res,
                            gpointer user_data)
{
    GTask* task = user_data;
...
static void splice_cb(GObject* ostream, GAsyncResult* res, gpointer user_data)
{
    ...
    else if (ctx->completed == 2)
    {
        ...
        {
            g_io_stream_close_async(ctx->stream1, g_task_get_priority(task),
                                    ctx->op1_cancellable, splice_close_cb,
                                    g_object_ref(task));
        }
        else
            ctx->completed++;
        ...
        {
            g_io_stream_close_async(ctx->stream2, g_task_get_priority(task),
                                    ctx->op2_cancellable, splice_close_cb,
                                    g_object_ref(task));
        }
        else
            ctx->completed++;
...
static void splice_cancelled_cb(GCancellable* cancellable, GTask* task)
{
    SpliceContext* ctx;
 ...
 * Since: 2.28
 **/
void g_io_stream_splice_async(GIOStream* stream1, GIOStream* stream2,
                              GIOStreamSpliceFlags flags, gint io_priority,
                              GCancellable* cancellable,
                              GAsyncReadyCallback callback, gpointer user_data)
{
    GTask* task;
    ...
    if (cancellable != NULL && g_cancellable_is_cancelled(cancellable))
    {
        g_task_report_new_error(
            NULL, callback, user_data, g_io_stream_splice_async, G_IO_ERROR,
            G_IO_ERROR_CANCELLED, "Operation has been cancelled");
        return;
    }
    ...
    if (cancellable != NULL)
    {
        ...
        ctx->cancelled_id =
            g_cancellable_connect(cancellable, G_CALLBACK(splice_cancelled_cb),
