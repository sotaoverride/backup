...
static const struct trie_child_entry_f*
    trie_node_child(sd_hwdb* hwdb, const struct trie_node_f* node, size_t idx)
{
...
static const struct trie_value_entry_f*
    trie_node_value(sd_hwdb* hwdb, const struct trie_node_f* node, size_t idx)
{
    const char* base = (const char*)node;
    ...
    node_lookup_f(sd_hwdb* hwdb, const struct trie_node_f* node, uint8_t c)
{
    ...
    search.c = c;
    child = bsearch(&search, (const char*)node + le64toh(hwdb->head->node_size),
...
static int hwdb_add_property(sd_hwdb* hwdb,
                             const struct trie_value_entry_f* entry)
{
    ...
    if (le64toh(hwdb->head->value_entry_size) >=
...
static int trie_fnmatch_f(sd_hwdb* hwdb, const struct trie_node_f* node,
                          size_t p, struct linebuf* buf, const char* search)
{
    ...
    for (i = 0; i < node->children_count; i++)
    {
        ...
        err = trie_fnmatch_f(hwdb, trie_node_from_off(hwdb, child->child_off),
                             0, buf, search);
        if (err < 0)
            return err;
...
static const char hwdb_bin_paths[] = "/etc/systemd/hwdb/hwdb.bin\0"
                                     "/etc/udev/hwdb.bin\0"
...
_public_ int sd_hwdb_new(sd_hwdb** ret)
{
    ...
    NULSTR_FOREACH(hwdb_bin_path, hwdb_bin_paths)
    {
        ...
        if (hwdb->f)
            break;
        if (errno != ENOENT)
            return log_debug_errno(errno, "Failed to open %s: %m",
                                   hwdb_bin_path);
    }
    ...
    if (hwdb->st.st_size <
        (off_t)offsetof(struct trie_header_f, strings_len) + 8)
        return log_debug_errno(SYNTHETIC_ERRNO(EIO), "File %s is too short: %m",
                               hwdb_bin_path);
...
_public_ int sd_hwdb_get(sd_hwdb* hwdb, const char* modalias, const char* key,
                         const char** _value)
{
    const struct trie_value_entry_f* entry;
    int r;
...
_public_ int sd_hwdb_enumerate(sd_hwdb* hwdb, const char** key,
                               const char** value)
{
    const struct trie_value_entry_f* entry;
    const void* k;
    ...
    ordered_hashmap_iterate(hwdb->properties, &hwdb->properties_iterator,
                            (void**)&entry, &k);
    if (!k)
        return 0;
