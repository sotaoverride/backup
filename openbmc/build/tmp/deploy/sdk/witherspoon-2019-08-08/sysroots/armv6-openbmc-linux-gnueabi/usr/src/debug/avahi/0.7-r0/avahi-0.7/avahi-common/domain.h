...
/** Return some kind of hash value for the domain, useful for using domains as
 * hash table keys. */
...
char* avahi_escape_label(const char* src, size_t src_length, char** ret_name,
                         size_t* ret_size);
...
/** Return 1 when the specified string contains a valid DNS-SD service name, 0
 * otherwise */
 ...
 * with more than one label and non-numerical), 0 otherwise. \since 0.6.9 */
...
/** Construct a valid complete DNS-SD service name from a name, a type and a
 * domain */
int avahi_service_name_join(char* p, size_t size, const char* name,
                            const char* type, const char* domain);

/** Split a full service name into name, type and domain */
int avahi_service_name_split(const char* p, char* name, size_t name_size,
                             char* type, size_t type_size, char* domain,
                             size_t domain_size);
...
/** Return a pointer to the type section of a subtype i.e. _foo._sub._bar._tcp
 * => _bar._tcp */
