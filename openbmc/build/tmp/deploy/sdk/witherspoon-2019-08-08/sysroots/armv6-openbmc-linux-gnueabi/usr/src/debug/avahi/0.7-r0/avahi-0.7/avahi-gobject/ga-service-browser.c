...
#define GA_SERVICE_BROWSER_GET_PRIVATE(o)                                      \
    (G_TYPE_INSTANCE_GET_PRIVATE((o), GA_TYPE_SERVICE_BROWSER,                 \
                                 GaServiceBrowserPrivate))
...
static void ga_service_browser_set_property(GObject* object, guint property_id,
                                            const GValue* value,
                                            GParamSpec* pspec)
{
...
static void ga_service_browser_get_property(GObject* object, guint property_id,
...
static void ga_service_browser_class_init(
    GaServiceBrowserClass* ga_service_browser_class)
{
    ...
    signals[NEW] = g_signal_new(
        "new-service", G_OBJECT_CLASS_TYPE(ga_service_browser_class),
        G_SIGNAL_RUN_LAST, 0, NULL, NULL,
        _ga_signals_marshal_VOID__INT_ENUM_STRING_STRING_STRING_UINT,
        G_TYPE_NONE, 6, G_TYPE_INT, GA_TYPE_PROTOCOL, G_TYPE_STRING,
        G_TYPE_STRING, G_TYPE_STRING, GA_TYPE_LOOKUP_RESULT_FLAGS);

    signals[REMOVED] = g_signal_new(
        "removed-service", G_OBJECT_CLASS_TYPE(ga_service_browser_class),
        G_SIGNAL_RUN_LAST, 0, NULL, NULL,
        _ga_signals_marshal_VOID__INT_ENUM_STRING_STRING_STRING_UINT,
        G_TYPE_NONE, 6, G_TYPE_INT, GA_TYPE_PROTOCOL, G_TYPE_STRING,
        G_TYPE_STRING, G_TYPE_STRING, GA_TYPE_LOOKUP_RESULT_FLAGS);
    ...
    signals[ALL_FOR_NOW] = g_signal_new(
        "all-for-now", G_OBJECT_CLASS_TYPE(ga_service_browser_class),
        G_SIGNAL_RUN_LAST, 0, NULL, NULL, g_cclosure_marshal_VOID__VOID,
        G_TYPE_NONE, 0);

    signals[CACHE_EXHAUSTED] = g_signal_new(
        "cache-exhausted", G_OBJECT_CLASS_TYPE(ga_service_browser_class),
        G_SIGNAL_RUN_LAST, 0, NULL, NULL, g_cclosure_marshal_VOID__VOID,
        G_TYPE_NONE, 0);

    signals[FAILURE] = g_signal_new(
        "failure", G_OBJECT_CLASS_TYPE(ga_service_browser_class),
    ...
    param_spec = g_param_spec_string(
        "type", "service type", "Service type to browse for", NULL,
        G_PARAM_READWRITE | G_PARAM_STATIC_NAME | G_PARAM_STATIC_BLURB);
    g_object_class_install_property(object_class, PROP_TYPE, param_spec);

    param_spec = g_param_spec_string(
        "domain", "service domain", "Domain to browse in", NULL,
        G_PARAM_READWRITE | G_PARAM_STATIC_NAME | G_PARAM_STATIC_BLURB);
...
static void _avahi_service_browser_cb(
...
{
    return ga_service_browser_new_full(AVAHI_IF_UNSPEC, AVAHI_PROTO_UNSPEC,
                                       type, NULL, 0);
}
...
GaServiceBrowser* ga_service_browser_new_full(AvahiIfIndex interface,
                                              ...
                                              GaLookupFlags flags)
{
    return g_object_new(GA_TYPE_SERVICE_BROWSER, "interface", interface,
                        "protocol", protocol, "type", type, "domain", domain,
                        "flags", flags, NULL);
}

gboolean ga_service_browser_attach(GaServiceBrowser* browser, GaClient* client,
                                   GError** error)
{
    GaServiceBrowserPrivate* priv = GA_SERVICE_BROWSER_GET_PRIVATE(browser);
    ...
    priv->browser = avahi_service_browser_new(
        client->avahi_client, priv->interface, priv->protocol, priv->type,
        priv->domain, priv->flags, _avahi_service_browser_cb, browser);
    if (priv->browser == NULL)
    {
        if (error != NULL)
        {
