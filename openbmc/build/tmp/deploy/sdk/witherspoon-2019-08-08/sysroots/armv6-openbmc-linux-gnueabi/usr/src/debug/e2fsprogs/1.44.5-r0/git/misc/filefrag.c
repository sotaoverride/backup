...
static void print_extent_header(void)
{
    printf(" ext: %*s %*s length: %*s flags:\n", logical_width * 2 + 3,
           "logical_offset:", physical_width * 2 + 3,
           "physical_offset:", physical_width + 1, "expected:");
}
                              ...
                              ext2fs_struct_stat* st)
{
    ...
    if (expected)
        sprintf(flags,
                ext_fmt == hex_fmt ? "%*llx: " : "%*llu: ", physical_width,
                expected >> blk_shift);
    else
    ...
    printf(ext_fmt, cur_ex, logical_width, logical_blk, logical_width,
           logical_blk + ext_blks, physical_width, physical_blk, physical_width,
           physical_blk + ext_blks, ext_len, flags);
}
                           ...
                           ext2fs_struct_stat* st)
{
    ...
    do
    {
        ...
        for (i = 0; i < fiemap->fm_mapped_extents; i++)
        {
            ...
            expected =
                fm_last.fe_physical + fm_ext[i].fe_logical - fm_last.fe_logical;
            ...
            if (verbose)
                print_extent_info(&fm_ext[i], n, expected, blk_shift, st);
...
static int filefrag_fibmap(int fd, int blk_shift, int* num_extents,
                           ext2fs_struct_stat* st, unsigned long numblocks,
                           int is_ext2)
{
    ...
    for (i = 0, logical = 0, *num_extents = 0, count = last_block = 0;
         i < numblocks; i++, logical += st->st_blksize)
    {
    ...
    if (force_extent && *num_extents != 0)
    {
        ...
        {
            expected =
                fm_last.fe_physical + (fm_ext.fe_logical - fm_last.fe_logical);
            if (expected == fm_ext.fe_physical)
                expected = 0;
        }
        print_extent_info(&fm_ext, *num_extents - 1, expected, blk_shift, st);
    }
...
static int frag_report(const char* filename)
{
    ...
    if (is_ext2)
    {
        ...
        data_blocks_per_cyl =
            blksize * 8 - (fsinfo.f_files / 8 / cylgroups) - 3;
    }
    ...
    if (verbose)
        printf("File size of %s is %llu (%llu block%s of %d bytes)\n", filename,
               (unsigned long long)st.st_size, numblocks * blksize >> blk_shift,
    ...
    if (force_bmap || rc < 0)
    { /* FIEMAP failed, try FIBMAP instead */
        expected = filefrag_fibmap(fd, blk_shift, &num_extents, &st, numblocks,
                                   is_ext2);
        if (expected < 0)
        {
            ...
            {
                fprintf(stderr, "%s: FIBMAP requires root privileges\n",
                        filename);
            }
            else
            {
                fprintf(stderr, "%s: FIBMAP error: %s", filename,
                        strerror(expected));
            }
            rc = expected;
            goto out_close;
...
#pragma GCC diagnostic pop
                ...
                { /* Allow -b without argument for compat. Remove
