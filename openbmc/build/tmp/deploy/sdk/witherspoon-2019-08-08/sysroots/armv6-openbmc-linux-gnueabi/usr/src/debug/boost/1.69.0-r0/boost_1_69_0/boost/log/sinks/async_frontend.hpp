...
#define BOOST_LOG_SINK_CTOR_FORWARD_INTERNAL_1(n, data)                        \
    ...
    explicit asynchronous_sink(                                                \
        T0 const& arg0, typename boost::log::aux::enable_if_named_parameters<  \
                            T0, boost::log::aux::sfinae_dummy>::type =         \
                            boost::log::aux::sfinae_dummy()) :                 \
        base_type(true),                                                       \
        queue_base_type(arg0),                                                 \
        m_pBackend(boost::make_shared<sink_backend_type>(arg0)),               \
        m_StopRequested(false), m_FlushRequested(false)                        \
    {                                                                          \
    ...
    explicit asynchronous_sink(shared_ptr<sink_backend_type> const& backend,   \
                               T0 const& arg0) :                               \
        base_type(true),                                                       \
        queue_base_type(arg0), m_pBackend(backend), m_StopRequested(false),    \
...
#define BOOST_LOG_SINK_CTOR_FORWARD_INTERNAL_N(n, data)                        \
    ...
    template <BOOST_PP_ENUM_PARAMS(n, typename T)>                             \
    explicit asynchronous_sink(                                                \
        shared_ptr<sink_backend_type> const& backend,                          \
        BOOST_PP_ENUM_BINARY_PARAMS(n, T, const& arg)) :                       \
        base_type(true),                                                       \
        queue_base_type((BOOST_PP_ENUM_PARAMS(n, arg))), m_pBackend(backend),  \
        m_StopRequested(false), m_FlushRequested(false)                        \
    {                                                                          \
        if ((BOOST_PP_ENUM_PARAMS(n, arg))[keywords::start_thread | true])     \
            start_feeding_thread();                                            \
...
#define BOOST_LOG_SINK_CTOR_FORWARD_INTERNAL(z, n, data)                       \
    BOOST_PP_IF(BOOST_PP_EQUAL(n, 1), BOOST_LOG_SINK_CTOR_FORWARD_INTERNAL_1,  \
                BOOST_LOG_SINK_CTOR_FORWARD_INTERNAL_N)                        \
    (n, data)
 ...
 *
 * The frontend starts a separate thread on construction. All logging records
 * are passed to the backend in this dedicated thread only.
 */
      ...
      public QueueingStrategyT
{
  ...
  private:
    ...
    class scoped_thread_id
    {
      ...
      public:
        ...
        scoped_thread_id(frontend_mutex_type& mut, condition_variable_any& cond,
            ...
            m_Mutex(mut),
            m_Cond(cond), m_ThreadID(tid), m_StopRequested(sr)
        {
            lock_guard<frontend_mutex_type> lock(m_Mutex);
            if (m_ThreadID != thread::id())
                BOOST_LOG_THROW_DESCR(unexpected_call,
                                      "Asynchronous sink frontend already runs "
                                      "a record feeding thread");
    ...
    class scoped_flag
    {
      ...
      public:
        explicit scoped_flag(frontend_mutex_type& mut,
                             condition_variable_any& cond,
                             boost::atomic<bool>& f) :
            m_Mutex(mut),
            m_Cond(cond), m_Flag(f)
        {
        }
        ~scoped_flag()
        {
  ...
  public:
    ...
    BOOST_STATIC_ASSERT_MSG(
        (has_requirement<typename sink_backend_type::frontend_requirements,
                         synchronized_feeding>::value),
        "Asynchronous sink frontend is incompatible with the specified "
        "backend: thread synchronization requirements are not met");
    //! \endcond

#ifndef BOOST_LOG_DOXYGEN_PASS

    //! A pointer type that locks the backend until it's destroyed
    typedef boost::log::aux::locking_ptr<sink_backend_type, backend_mutex_type>
        locked_backend_ptr;
  ...
  public:
    ...
    asynchronous_sink(bool start_thread = true) :
        base_type(true), m_pBackend(boost::make_shared<sink_backend_type>()),
        m_StopRequested(false), m_FlushRequested(false)
    {
        if (start_thread)
            start_feeding_thread();
    ...
    explicit asynchronous_sink(shared_ptr<sink_backend_type> const& backend,
        ...
        base_type(true),
        m_pBackend(backend), m_StopRequested(false), m_FlushRequested(false)
    {
        if (start_thread)
            start_feeding_thread();
    }
    ...
    /*!
     * Constructor that passes arbitrary named parameters to the interprocess
     * sink backend constructor. Refer to the backend documentation for the list
     ...
     */
#ifndef BOOST_LOG_DOXYGEN_PASS
    BOOST_LOG_PARAMETRIZED_CONSTRUCTORS_GEN(
        BOOST_LOG_SINK_CTOR_FORWARD_INTERNAL, ~)
#else
    ...
    /*!
     * Destructor. Implicitly stops the dedicated feeding thread, if one is
     * running.
     */
    ...
    /*!
     * The method starts record feeding loop and effectively blocks until either
     * of this happens:
     *
     * \li the thread is interrupted due to either standard thread interruption
     * or a call to \c stop \li an exception is thrown while processing a log
     * record in the backend, and the exception is not terminated by the
     * exception handler, if one is installed
     *
     * \pre The sink frontend must be constructed without spawning a dedicated
     * thread
     */
    void run()
    {
        // First check that no other thread is running
        scoped_thread_id guard(base_type::frontend_mutex(), m_BlockCond,
                               m_FeedingThreadID, m_StopRequested);
    ...
    /*!
     * The method softly interrupts record feeding loop. This method must be
     * called when the \c run method execution has to be interrupted. Unlike
     * regular thread interruption, calling \c stop will not interrupt the
     * record processing in the middle. Instead, the sink frontend will attempt
     * to finish its business with the record in progress and return afterwards.
     * This method can be called either if the sink was created with a dedicated
     * thread, or if the feeding loop was initiated by user.
     *
     * \note Returning from this method does not guarantee that there are no
     * records left buffered in the sink frontend. It is possible that log
     * records keep coming during and after this method is called. At some point
     * of execution of this method log records stop being processed, and all
     * records that come after this point are put into the queue. These records
     * will be processed upon further calls to \c run or \c feed_records.
     */
    void stop()
    {
        unique_lock<frontend_mutex_type> lock(base_type::frontend_mutex());
        if (m_FeedingThreadID != thread::id() ||
            m_DedicatedFeedingThread.joinable())
        {
            try
            {
    ...
    /*!
     * The method feeds log records that may have been buffered to the backend
     * and returns
     *
     * \pre The sink frontend must be constructed without spawning a dedicated
     * thread
    ...
    /*!
     * The method feeds all log records that may have been buffered to the
     * backend and returns. Unlike \c feed_records, in case of ordering queueing
     * the method also feeds records that were enqueued during the ordering
     * window, attempting to empty the queue completely.
     */
    void flush()
    {
        unique_lock<frontend_mutex_type> lock(base_type::frontend_mutex());
            ...
            m_DedicatedFeedingThread.joinable())
        {
            ...
            queue_base_type::interrupt_dequeue();
            while (!m_StopRequested.load(boost::memory_order_acquire) &&
                   m_FlushRequested.load(boost::memory_order_acquire))
                m_BlockCond.wait(lock);

            // The condition may have been signalled when the feeding thread was
            // finishing. In that case records may not have been flushed, and we
            // do the flush ourselves.
            if (m_FeedingThreadID != thread::id())
                return;
        }
        ...
        scoped_thread_id guard(lock, m_BlockCond, m_FeedingThreadID,
                               m_StopRequested);
  ...
  private:
    ...
    {
        boost::thread(boost::bind(&asynchronous_sink::run, this))
            .swap(m_DedicatedFeedingThread);
    }
    ...
    void do_feed_records()
    {
        ...
        if (BOOST_UNLIKELY(m_FlushRequested.load(boost::memory_order_acquire)))
        {
            scoped_flag guard(base_type::frontend_mutex(), m_BlockCond,
                              m_FlushRequested);
