...
void init_tcpTable(void)
{
    ...
    netsnmp_table_helper_add_indexes(table_info, ASN_IPADDRESS, ASN_INTEGER,
                                     ASN_IPADDRESS, ASN_INTEGER, 0);
    ...
    reginfo = netsnmp_create_handler_registration(
        "tcpTable", tcpTable_handler, tcpTable_oid, OID_LENGTH(tcpTable_oid),
        HANDLER_CAN_RONLY),
    ...
    netsnmp_inject_handler(reginfo, netsnmp_get_cache_handler(
                                        TCP_STATS_CACHE_TIMEOUT, tcpTable_load,
                                        tcpTable_free, tcpTable_oid,
                                        OID_LENGTH(tcpTable_oid)));
}

int tcpTable_handler(netsnmp_mib_handler* handler,
                     netsnmp_handler_registration* reginfo,
    ...
    switch (reqinfo->mode)
    {
        case MODE_GET:
            for (request = requests; request; request = request->next)
            {
                ...
                entry = (TCPTABLE_ENTRY_TYPE*)netsnmp_extract_iterator_context(
                    request);
                if (!entry)
                    continue;
                ...
                switch (subid)
                {
                    ...
                    case TCPCONNLOCALPORT:
                        port = TCP_PORT_TO_HOST_ORDER(
                            (u_short)entry->TCPTABLE_LOCALPORT);
                        snmp_set_var_typed_value(requestvb, ASN_INTEGER,
                    ...
                    case TCPCONNREMOTEPORT:
                        port = TCP_PORT_TO_HOST_ORDER(
                            (u_short)entry->TCPTABLE_REMOTEPORT);
                        snmp_set_var_typed_value(requestvb, ASN_INTEGER,
                                                 (u_char*)&port, sizeof(port));
                        break;
                }
...
netsnmp_variable_list* tcpTable_first_entry(void** loop_context,
                                            void** data_context,
...
netsnmp_variable_list* tcpTable_next_entry(void** loop_context,
                                           void** data_context,
                                           netsnmp_variable_list* index,
                                           netsnmp_iterator_info* data)
{
...
void tcpTable_free(netsnmp_cache* cache, void* magic)
{
#if defined(WIN32) || defined(cygwin)
    if (tcp_head)
    {
...
#ifdef hpux11
int tcpTable_load(netsnmp_cache* cache, void* vmagic)
{
    int fd;
    struct nmparms p;
    int val = 0;
...
static int tcpTable_load_netlink(void)
{
    ...
    nm = nlmsg_alloc_simple(TCPDIAG_GETSOCK,
    ...
    while (running)
    {
        ...
        while (nlmsg_ok(h, len))
        {
            ...
            memcpy(&pcb.inp_laddr.s_addr, r->id.idiag_src,
                   r->idiag_family == AF_INET ? 4 : 6);
            memcpy(&pcb.inp_faddr.s_addr, r->id.idiag_dst,
                   r->idiag_family == AF_INET ? 4 : 6);

            pcb.inp_lport = r->id.idiag_sport;
            pcb.inp_fport = r->id.idiag_dport;

            pcb.inp_state = (r->idiag_state & 0xf) < 12
                                ? linux_states[r->idiag_state & 0xf]
                                : 2;
...
int tcpTable_load(netsnmp_cache* cache, void* vmagic)
{
    ...
    while (line == fgets(line, sizeof(line), in))
    {
        ...
        memset(&pcb, 0, sizeof(pcb));
        if (6 != sscanf(line, "%*d: %x:%x %x:%x %x %*X:%*X %*X:%*X %*X %d",
                        &pcb.inp_laddr.s_addr, &lp, &pcb.inp_faddr.s_addr, &fp,
                        &state, &uid))
            continue;
...
int tcpTable_load(netsnmp_cache* cache, void* vmagic)
{
    ...
    if (getMibstat(MIB_TCP_CONN, &entry, sizeof(mib2_tcpConnEntry_t), GET_FIRST,
                   &TCP_Cmp, &entry) != 0)
    {
...
int tcpTable_load(netsnmp_cache* cache, void* vmagic)
{
    ...
    tcp_head = kvm_getfiles(kd, KERN_FILE_BYFILE, DTYPE_SOCKET,
                            sizeof(struct kinfo_file), &count);
    tcp_size = count;
    for (i = 0; i < tcp_size; i++)
    {
        if (tcp_head[i].so_protocol != IPPROTO_TCP ||
            tcp_head[i].so_family != AF_INET)
            continue;
...
int tcpTable_load(netsnmp_cache* cache, void* vmagic)
{
    ...
    if (status == NO_ERROR)
    {
        ...
        tcp_size = pTcpTable->dwNumEntries -
                   1; /* entries are counted starting with 0 */
...
int tcpTable_load(netsnmp_cache* cache, void* vmagic)
{
...
#endif
    {
        ...
        if (nnew->state == 5 /* established */ ||
            ...
            tcp_estab++;
        memcpy(&(nnew->pcb), &(((NS_ELEM*)xig)->xt_inp), sizeof(struct inpcb));
...
int tcpTable_load(netsnmp_cache* cache, void* vmagic)
{
    ...
    while (entry)
    {
        ...
        if (!NETSNMP_KLOOKUP(nnew->pcb.inp_ppcb, (char*)&tcpcb,
                             sizeof(struct tcpcb)))
        {
            DEBUGMSGTL(("mibII/tcpTable:TcpTable_load", "klookup failed\n"));
            break;
