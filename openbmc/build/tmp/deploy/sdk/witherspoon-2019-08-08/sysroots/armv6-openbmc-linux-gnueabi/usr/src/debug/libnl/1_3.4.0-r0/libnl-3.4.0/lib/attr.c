...
{
    return remaining >= sizeof(*nla) && nla->nla_len >= sizeof(*nla) &&
...
struct nlattr* nla_find(const struct nlattr* head, int len, int attrtype)
{
    ...
    nla_for_each_attr(nla, head, len,
                      rem) if (nla_type(nla) ==
                               attrtype) return (struct nlattr*)nla;
...
struct nlattr* nla_reserve(struct nl_msg* msg, int attrtype, int attrlen)
{
    ...
    NL_DBG(2,
           ...
           "nlmsg_len=%d\n",
           msg, nla, nla->nla_type, nla_total_size(attrlen), attrlen,
           (void*)nla - nlmsg_data(msg->nm_nlh), msg->nm_nlh->nlmsg_len);
...
{
    return nla_put(msg, attrtype, nl_data_get_size(data), nl_data_get(data));
}
...
int nla_nest_end(struct nl_msg* msg, struct nlattr* start)
{
    ...
    if (pad > 0)
    {
        ...
        NL_DBG(2, "msg %p: attr <%p> %d: added %zu bytes of padding\n", msg,
               start, start->nla_type, pad);
    }

    NL_DBG(2, "msg %p: attr <%p> %d: closing nesting, len=%u\n", msg, start,
           start->nla_type, start->nla_len);
