...
DEFobjCurrIf(glbl) DEFobjCurrIf(net) DEFobjCurrIf(netstrms)
    DEFobjCurrIf(netstrm) DEFobjCurrIf(tcpclt)
...
#define FORW_TCP 1
    ...
    int bResendLastOnRecon; /* should the last message be re-sent on a
...
#define COMPRESS_STREAM_ALWAYS 2
    ...
    sbool
        strmCompFlushOnTxEnd; /* flush stream compression on transaction end? */
} instanceData;
...
typedef struct wrkrInstanceData
{
    ...
    int bIsConnected;  /* are we connected to remote host? 0 - no, 1 - yes, UDP
                          means addr resolved */
    int nXmit;         /* number of transmissions since last (re-)bind */
    tcpclt_t* pTCPClt; /* our tcpclt object */
    sbool bzInitDone;  /* did we do an init of zstrm already? */
    z_stream zstrm;    /* zip stream to use for tcp compression */
    uchar sndBuf[16 * 1024]; /* this is intensionally fixed -- see no good
...
typedef struct configSettings_s
{
    ...
    int iStrmDrvrMode;  /* mode for stream driver, driver-dependent (0 mostly
                           means plain tcp) */
    int bResendLastOnRecon;     /* should the last message be re-sent on a
                                   successful reconnect? */
    uchar* pszStrmDrvrAuthMode; /* authentication mode to use */
    uchar* pszStrmDrvrPermitExpiredCerts; /* control how to handly expired
                                             certificates */
    int iTCPRebindInterval; /* support for automatic re-binding (load
                               balancers!). 0 - no rebind */
    int iUDPRebindInterval; /* support for automatic re-binding (load
                               balancers!). 0 - no rebind */
    int bKeepAlive;
    int iKeepAliveIntvl;
    int iKeepAliveProbes;
...
BEGINinitConfVars /* (re)set config variables to default values */
    CODESTARTinitConfVars cs.pszTplName =
    NULL;              /* name of the default template to use */
cs.pszStrmDrvr = NULL; /* name of the stream driver to use */
cs.iStrmDrvrMode =
    0; /* mode for stream driver, driver-dependent (0 mostly means plain tcp) */
cs.bResendLastOnRecon =
    0; /* should the last message be re-sent on a successful reconnect? */
cs.pszStrmDrvrAuthMode = NULL; /* authentication mode to use */
cs.iUDPRebindInterval =
    0; /* support for automatic re-binding (load balancers!). 0 - no rebind */
cs.iTCPRebindInterval =
    0; /* support for automatic re-binding (load balancers!). 0 - no rebind */
cs.pPermPeers = NULL;
ENDinitConfVars

    static rsRetVal
    doTryResume(wrkrInstanceData_t*);
...
ENDbeginCnfLoad
    ...
    BEGINsetModCnf int i;
CODESTARTsetModCnf const struct cnfparamvals* const __restrict__ pvals =
    nvlstGetParams(lst, &modpblk, NULL);
if (pvals == NULL)
{
    ...
    if (!strcmp(modpblk.descr[i].name, "template"))
    {
        ...
        if (cs.pszTplName != NULL)
        {
            LogError(0, RS_RET_DUP_PARAM,
                     ...
                     "was already set via legacy directive - may lead to "
                     "inconsistent "
                     "results.");
        }
    }
...
ENDfreeCnf
    ...
    BEGINcreateInstance CODESTARTcreateInstance if (cs.pszStrmDrvr != NULL)
        CHKmalloc(pData->pszStrmDrvr = (uchar*)strdup((char*)cs.pszStrmDrvr));
if (cs.pszStrmDrvrAuthMode != NULL)
    CHKmalloc(pData->pszStrmDrvrAuthMode = (uchar*)
                  strdup((char*)cs.pszStrmDrvrAuthMode));
finalize_it
    : ENDcreateInstance

          BEGINcreateWrkrInstance CODESTARTcreateWrkrInstance
...
ENDisCompatibleWithFeature
    ...
    BEGINfreeInstance CODESTARTfreeInstance free(pData->pszStrmDrvr);
...
ENDfreeInstance
    ...
    BEGINfreeWrkrInstance CODESTARTfreeWrkrInstance
...
ENDfreeWrkrInstance
    ...
    BEGINdbgPrintInstInfo CODESTARTdbgPrintInstInfo dbgprintf("%s",
                                                              pData->target);
ENDdbgPrintInstInfo
...
#define UDP_MAX_MSGSIZE 65507 /* limit per RFC definition */
    static rsRetVal
    UDPSend(wrkrInstanceData_t* __restrict__ const pWrkrData,
            uchar* __restrict__ const msg, size_t len)
{
    DEFiRet;
    struct addrinfo* r;
    int i;
    ...
    if (pWrkrData->pData->iRebindInterval &&
        (pWrkrData->nXmit++ % pWrkrData->pData->iRebindInterval == 0))
    {
    ...
    for (r = pWrkrData->f_addr; r; r = r->ai_next)
    {
        ...
        for (i = 0; runSockArrayLoop && (i < *pWrkrData->pSockArray); i++)
        {
            ...
            while (try_send)
            {
                ...
                {
                    const size_t newlen =
                        (lenThisTry > 1024) ? lenThisTry - 1024 : 512;
                    LogError(
                        0, RS_RET_UDP_MSGSIZE_TOO_LARGE,
    ...
    {
        LogError(lasterrno, RS_RET_ERR_UDPSEND,
                 "omfwd: socket %d: error %d sending via udp", lasterr_sock,
                 lasterrno);
 ...
 */
static rsRetVal setPermittedPeer(void __attribute__((unused)) * pVal,
                                 uchar* pszID)
{
    DEFiRet;
...
static rsRetVal TCPSendBufUncompressed(wrkrInstanceData_t* pWrkrData,
                                       uchar* buf, unsigned len)
{
    DEFiRet;
...
static rsRetVal TCPSendBufCompressed(wrkrInstanceData_t* pWrkrData, uchar* buf,
                                     unsigned len, sbool bIsFlush)
{
    ...
    if (!pWrkrData->bzInitDone)
    {
        ...
        zRet =
            deflateInit(&pWrkrData->zstrm, pWrkrData->pData->compressionLevel);
        if (zRet != Z_OK)
        {
    ...
    {
        DBGPRINTF(
            "omfwd: in deflate() loop, avail_in %d, total_in %ld, isFlush %d\n",
 ...
 */
static rsRetVal doZipFinish(wrkrInstanceData_t* pWrkrData)
{
    int zRet; /* zlib return state */
    DEFiRet;
    unsigned outavail;
...
static rsRetVal TCPSendFrame(void* pvData, char* msg, size_t len)
{
    ...
    if (pWrkrData->offsSndBuf != 0 &&
        pWrkrData->offsSndBuf + len >= sizeof(pWrkrData->sndBuf))
    {
        ...
        CHKiRet(TCPSendBuf(pWrkrData, pWrkrData->sndBuf, pWrkrData->offsSndBuf,
                           NO_FLUSH));
        pWrkrData->offsSndBuf = 0;
    }
...
static rsRetVal TCPSendInit(void* pvData)
{
    ...
    if (pWrkrData->pNetstrm == NULL)
    {
        ...
        {
            CHKiRet(netstrm.SetDrvrAuthMode(pWrkrData->pNetstrm,
                                            pData->pszStrmDrvrAuthMode));
        }
        if (pData->pszStrmDrvrPermitExpiredCerts != NULL)
        {
            CHKiRet(netstrm.SetDrvrPermitExpiredCerts(
                pWrkrData->pNetstrm, pData->pszStrmDrvrPermitExpiredCerts));
        }

        if (pData->pPermPeers != NULL)
        {
            CHKiRet(netstrm.SetDrvrPermPeers(pWrkrData->pNetstrm,
                                             pData->pPermPeers));
        }
        /* params set, now connect */
        if (pData->gnutlsPriorityString != NULL)
        {
            CHKiRet(netstrm.SetGnutlsPriorityString(
                pWrkrData->pNetstrm, pData->gnutlsPriorityString));
        }
        ...
        if (pData->bKeepAlive)
        {
            CHKiRet(netstrm.SetKeepAliveProbes(pWrkrData->pNetstrm,
                                               pData->iKeepAliveProbes));
            CHKiRet(netstrm.SetKeepAliveIntvl(pWrkrData->pNetstrm,
                                              pData->iKeepAliveIntvl));
            CHKiRet(netstrm.SetKeepAliveTime(pWrkrData->pNetstrm,
                                             pData->iKeepAliveTime));
    ...
    if (pData->networkNamespace)
    {
        ...
        if (pData->originalNamespace < 0)
        {
            LogError(0, RS_RET_IO_ERROR,
                     "omfwd: could not read /proc/self/ns/net");
            ABORT_FINALIZE(RS_RET_IO_ERROR);
        }

        /* build network namespace path */
        if (asprintf(&nsPath, "/var/run/netns/%s", pData->networkNamespace) ==
            -1)
        {
        ...
        {
            LogError(0, RS_RET_IO_ERROR,
                     "omfwd: could not change to namespace '%s'",
        ...
        {
            LogError(0, RS_RET_IO_ERROR,
                     "could not change to namespace '%s': %s",
 ...
 * changeToNs().
 */
static rsRetVal returnToOriginalNs(instanceData* const pData
                                   __attribute__((unused)))
{
    ...
    if (pData->networkNamespace && pData->originalNamespace >= 0)
    {
        ...
        {
            LogError(0, RS_RET_IO_ERROR,
                     "could not return to original namespace: %s",
                     gai_strerror(iErr));
            ABORT_FINALIZE(RS_RET_IO_ERROR);
        }
...
static rsRetVal doTryResume(wrkrInstanceData_t* pWrkrData)
{
    ...
    if (pData->protocol == FORW_UDP)
    {
        ...
        if ((iErr = (getaddrinfo(pData->target, pData->port, &hints, &res))) !=
            0)
        {
        ...
        if (pWrkrData->pSockArray == NULL)
        {
            ...
            pWrkrData->pSockArray = net.create_udp_socket(
                (uchar*)address, NULL, bBindRequired, 0, pData->UDPSendBuf,
...
BEGINtryResume CODESTARTtryResume dbgprintf("omfwd: tryResume: pWrkrData %p\n",
                                            pWrkrData);
               ...
               actWrkrIParams_t* __restrict__ const iparam)
{
     ...
     */
    if (pData->compressionMode == COMPRESS_SINGLE_MSG &&
        (l > CONF_MIN_SIZE_FOR_COMPRESS))
    {
        uLongf destLen = iMaxLine + iMaxLine / 100 +
                         12; /* recommended value from zlib doc */
        uLong srcLen = l;
        int ret;
        CHKmalloc(out = (Bytef*)malloc(destLen));
        out[0] = 'z';
        out[1] = '\0';
        ret = compress2((Bytef*)out + 1, &destLen, (Bytef*)psz, srcLen,
                        pData->compressionLevel);
        dbgprintf(
            "Compressing message, length was %d now %d, return state  %d.\n", l,
            (int)destLen, ret);
        if (ret != Z_OK)
        {
    ...
    else
    {
            ...
            iRet != RS_RET_PREVIOUS_COMMITTED)
        {
            ...
            LogError(
                0, iRet,
                "omfwd: error forwarding via tcp to %s:%s, suspending action",
...
BEGINcommitTransaction unsigned i;
CODESTARTcommitTransaction CHKiRet(doTryResume(pWrkrData));
...
if (pWrkrData->offsSndBuf != 0)
{
    iRet = TCPSendBuf(pWrkrData, pWrkrData->sndBuf, pWrkrData->offsSndBuf,
                      IS_FLUSH);
    pWrkrData->offsSndBuf = 0;
}
finalize_it
    : ENDcommitTransaction
      ...
      /* This function loads TCP support, if not already loaded. It will be
       * called during config processing. To server ressources, TCP support will
       * only be loaded if it actually is used. -- rgerhard, 2008-04-17
       */
      static rsRetVal
      loadTCPSupport(void)
{
...
static rsRetVal initTCP(wrkrInstanceData_t* pWrkrData)
{
    ...
    if (pData->protocol == FORW_TCP)
    {
        ...
        CHKiRet(tcpclt.SetResendLastOnRecon(pWrkrData->pTCPClt,
                                            pData->bResendLastOnRecon));
        /* and set callbacks */
        ...
        CHKiRet(tcpclt.SetFramingDelimiter(pWrkrData->pTCPClt,
                                           pData->tcp_framingDelimiter));
        CHKiRet(tcpclt.SetRebindInterval(pWrkrData->pTCPClt,
                                         pData->iRebindInterval));
    }
finalize_it:
    RETiRet;
}
...
rsRetVal localRet;
int complevel = -1;
CODESTARTnewActInst DBGPRINTF("newActInst (omfwd)\n");
...
for (i = 0; i < actpblk.nParams; ++i)
{
    ...
    else if (!strcmp(actpblk.descr[i].name, "protocol"))
    {
        ...
        else
        {
            ...
            LogError(0, RS_RET_INVLD_PROTOCOL, "omfwd: invalid protocol \"%s\"",
                     str);
            free(str);
    ...
    else if (!strcmp(actpblk.descr[i].name, "tcp_framing"))
    {
        ...
        else if (!es_strcasebufcmp(pvals[i].val.d.estr, (uchar*)"octet-counted",
                                   13))
        {
    ...
    else if (!strcmp(actpblk.descr[i].name, "streamdriverpermittedpeers"))
    {
        ...
        start = str;
        lenStr =
            ustrlen(start); /* we need length after '\0' has been dropped... */
        while (lenStr > 0)
        {
            p = start;
            while (*p && *p != ',' && lenStr--)
...
CODE_STD_STRING_REQUESTnewActInst(1)
    ...
    tplToUse = ustrdup((pData->tplName == NULL) ? getDfltTpl()
                                                : pData->tplName);
...
ENDnewActInst
    ...
    BEGINparseSelectorAct uchar* q;
int i;
rsRetVal localRet;
struct addrinfo;
TCPFRAMINGMODE tcp_framing = TCP_FRAMING_OCTET_STUFFING;
CODESTARTparseSelectorAct CODE_STD_STRING_REQUESTparseSelectorAct(1) if (*p !=
                                                                         '@')
...
if (*p == '(')
{
    ...
    do
    {
        ...
        {        /* octet-couting based TCP framing? */
            ++p; /* eat */
            /* no further options settable */
            tcp_framing = TCP_FRAMING_OCTET_COUNTING;
        }
        else
        { /* invalid option! Just skip it... */
            LogError(0, NO_ERRCODE,
                     "Invalid option %c in forwarding action - ignoring.", *p);
    ...
    if (*p == ')')
         ...
         */
        LogError(0, NO_ERRCODE,
                 "Option block not terminated in forwarding action.");
}

/* extract the host first (we do a trick - we replace the ';' or ':' with a
 * '\0') now skip to port and then template name. rgerhards 2005-07-06
 */
if (*p == '[')
{        /* everything is hostname upto ']' */
    ++p; /* skip '[' */
    for (q = p; *p && *p != ']'; ++p)
        /* JUST SKIP */;
    if (*p == ']')
...
pData->iRebindInterval = (pData->protocol == FORW_TCP) ? cs.iTCPRebindInterval
                                                       : cs.iUDPRebindInterval;
...
BEGINmodInit(Fwd) CODESTARTmodInit INITLegCnfVars* ipIFVersProvided =
...
CHKiRet(regCfSysLineHdlr((uchar*)"actionforwarddefaulttemplate", 0,
...
CHKiRet(regCfSysLineHdlr((uchar*)"actionsendtcpkeepalive_probes", 0,
                         eCmdHdlrInt, NULL, &cs.iKeepAliveProbes, NULL));
...
CHKiRet(regCfSysLineHdlr((uchar*)"actionsendstreamdriverauthmode", 0,
                         eCmdHdlrGetWord, NULL, &cs.pszStrmDrvrAuthMode, NULL));
CHKiRet(regCfSysLineHdlr((uchar*)"actionsendstreamdriverpermittedpeer", 0,
                         eCmdHdlrGetWord, setPermittedPeer, NULL, NULL));
CHKiRet(regCfSysLineHdlr((uchar*)"actionsendresendlastmsgonreconnect", 0,
                         eCmdHdlrBinary, NULL, &cs.bResendLastOnRecon, NULL));
