...
G_DEFINE_TYPE_WITH_CODE(GFileIOStream, g_file_io_stream, G_TYPE_IO_STREAM,
                        G_ADD_PRIVATE(GFileIOStream) G_IMPLEMENT_INTERFACE(
                            G_TYPE_SEEKABLE,
 ...
 * the stream's interface (with @error being set to %G_IO_ERROR_NOT_SUPPORTED).
                                       ...
                                       gpointer user_data)
{
    ...
    if (!g_io_stream_set_pending(io_stream, &error))
    {
        g_task_report_error(stream, callback, user_data,
                            g_file_io_stream_query_info_async, error);
        return;
    }
...
static gboolean g_file_io_stream_seek(GFileIOStream* stream, goffset offset,
...
static gboolean g_file_io_stream_seekable_seek(GSeekable* seekable,
                                               ...
                                               GError** error)
{
    return g_file_io_stream_seek(G_FILE_IO_STREAM(seekable), offset, type,
                                 cancellable, error);
}
...
static gboolean g_file_io_stream_truncate(GFileIOStream* stream, goffset size,
                                          GCancellable* cancellable,
                                          GError** error)
{
    GFileIOStreamClass* class;
...
static gboolean g_file_io_stream_seekable_truncate(GSeekable* seekable,
                                                   ...
                                                   GError** error)
{
    return g_file_io_stream_truncate(G_FILE_IO_STREAM(seekable), size,
                                     cancellable, error);
}
/*****************************************************
...
static gboolean g_file_io_stream_real_seek(GFileIOStream* stream,
                                           goffset offset, GSeekType type,
                                           GCancellable* cancellable,
                                           GError** error)
{
...
static gboolean g_file_io_stream_real_truncate_fn(GFileIOStream* stream,
                                                  goffset size,
                                                  GCancellable* cancellable,
                                                  GError** error)
{
...
static GFileInfo* g_file_io_stream_real_query_info(GFileIOStream* stream,
                                                   const char* attributes,
                                                   GCancellable* cancellable,
                                                   GError** error)
{
    ...
    return g_file_output_stream_query_info(file_out, attributes, cancellable,
                                           error);
}
...
static AsyncOpWrapper* async_op_wrapper_new(gpointer object,
                                            GAsyncReadyCallback callback,
                                            gpointer user_data)
{
    AsyncOpWrapper* data;
...
static void async_op_wrapper_callback(GObject* source_object, GAsyncResult* res,
                                      gpointer user_data)
{
...
static void g_file_io_stream_real_query_info_async(
    GFileIOStream* stream, const char* attributes, int io_priority,
    GCancellable* cancellable, GAsyncReadyCallback callback, gpointer user_data)
{
    ...
    g_file_output_stream_query_info_async(file_out, attributes, io_priority,
                                          cancellable,
                                          async_op_wrapper_callback, data);
}
