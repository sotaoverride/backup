...
static const GOptionEntry entries[] = {
    {"mountable", 'm', 0, G_OPTION_ARG_NONE, &mount_mountable,
     N_("Mount as mountable"), NULL},
    {"device", 'd', 0, G_OPTION_ARG_STRING, &mount_device_file,
    ...
    {"stop", 't', 0, G_OPTION_ARG_STRING, &stop_device_file,
     N_("Stop drive with device file"), N_("DEVICE")},
    {"unmount-scheme", 's', 0, G_OPTION_ARG_STRING, &unmount_scheme,
     N_("Unmount all mounts with the given scheme"), N_("SCHEME")},
    {"force", 'f', 0, G_OPTION_ARG_NONE, &force,
     N_("Ignore outstanding file operations when unmounting or ejecting"),
     NULL},
    {"anonymous", 'a', 0, G_OPTION_ARG_NONE, &anonymous,
     N_("Use an anonymous user when authenticating"), NULL},
    /* Translator: List here is a verb as in 'List all mounts' */
    {"list", 'l', 0, G_OPTION_ARG_NONE, &mount_list, N_("List"), NULL},
    {"monitor", 'o', 0, G_OPTION_ARG_NONE, &mount_monitor, N_("Monitor events"),
     NULL},
    {"detail", 'i', 0, G_OPTION_ARG_NONE, &extra_detail,
     N_("Show extra information"), NULL},
    {"tcrypt-pim", 0, 0, G_OPTION_ARG_INT, &tcrypt_pim,
     N_("The numeric PIM when unlocking a VeraCrypt volume"), N_("PIM")},
    {"tcrypt-hidden", 0, 0, G_OPTION_ARG_NONE, &tcrypt_hidden,
     N_("Mount a TCRYPT hidden volume"), NULL},
    {"tcrypt-system", 0, 0, G_OPTION_ARG_NONE, &tcrypt_system,
     N_("Mount a TCRYPT system volume"), NULL},
    {NULL}};

static char* prompt_for(const char* prompt, const char* default_value,
                        gboolean echo)
{
#ifdef HAVE_TERMIOS_H
...
static void ask_password_cb(GMountOperation* op, const char* message,
                            ...
                            const char* default_domain, GAskPasswordFlags flags)
{
    ...
    if (anonymous && GPOINTER_TO_INT(g_object_get_data(
                         G_OBJECT(op), "state")) == MOUNT_OP_ASKED)
    {
        g_object_set_data(G_OBJECT(op), "state",
                          GINT_TO_POINTER(MOUNT_OP_ABORTED));
        g_mount_operation_reply(op, G_MOUNT_OPERATION_ABORTED);
    }
    else
    {
        g_object_set_data(G_OBJECT(op), "state",
                          GINT_TO_POINTER(MOUNT_OP_ASKED));
...
static void ask_question_cb(GMountOperation* op, char* message, char** choices,
                            gpointer user_data)
{
    char** ptr = choices;
    char* s;
...
static void mount_mountable_done_cb(GObject* object, GAsyncResult* res,
                                    gpointer user_data)
{
    GFile* target;
    GError* error = NULL;
    ...
    {
        success = FALSE;
        if (GPOINTER_TO_INT(g_object_get_data(G_OBJECT(op), "state")) ==
            MOUNT_OP_ABORTED)
...
static void mount_done_cb(GObject* object, GAsyncResult* res,
                          gpointer user_data)
{
    gboolean succeeded;
    GError* error = NULL;
...
static void unmount_done_cb(GObject* object, GAsyncResult* res,
                            gpointer user_data)
{
    gboolean succeeded;
    GError* error = NULL;
    ...
    succeeded =
        g_mount_unmount_with_operation_finish(G_MOUNT(object), res, &error);
...
static void unmount(GFile* file)
{
    GMount* mount;
    GError* error = NULL;
    ...
    g_mount_unmount_with_operation(mount, flags, mount_op, NULL,
                                   unmount_done_cb, g_object_ref(file));
    g_object_unref(mount_op);

    outstanding_mounts++;
}

static void eject_done_cb(GObject* object, GAsyncResult* res,
                          gpointer user_data)
{
    gboolean succeeded;
    GError* error = NULL;
    ...
    succeeded =
        g_mount_eject_with_operation_finish(G_MOUNT(object), res, &error);
...
static void eject(GFile* file)
{
    ...
    mount_op = new_mount_op();
    flags = force ? G_MOUNT_UNMOUNT_FORCE : G_MOUNT_UNMOUNT_NONE;
    g_mount_eject_with_operation(mount, flags, mount_op, NULL, eject_done_cb,
                                 g_object_ref(file));
    g_object_unref(mount_op);
...
static void stop_with_device_file_cb(GObject* object, GAsyncResult* res,
                                     gpointer user_data)
{
    GError* error = NULL;
...
static void stop_with_device_file(const char* device_file)
{
    GList* drives;
    GList* l;
    ...
    for (l = drives; l != NULL; l = l->next)
    {
        ...
        if (g_strcmp0(id, device_file) == 0)
        {
            ...
            op = new_mount_op();
            flags = force ? G_MOUNT_UNMOUNT_FORCE : G_MOUNT_UNMOUNT_NONE;
            g_drive_stop(drive, flags, op, NULL, stop_with_device_file_cb,
                         g_steal_pointer(&id));
            g_object_unref(op);
...
static void iterate_gmain(void)
{
    g_timeout_add(500, iterate_gmain_timeout_function, NULL);
    g_main_loop_run(main_loop);
}

static void show_themed_icon_names(GThemedIcon* icon, gboolean symbolic,
                                   int indent)
{
    char** names;
    char** iter;
...
static char* get_type_name(gpointer object)
{
    ...
    if (strcmp("GProxyDrive", type_name) == 0)
    {
        ret = g_strdup_printf(
            ...
            (const char*)g_object_get_data(
                G_OBJECT(object), "g-proxy-drive-volume-monitor-name"));
    }
    else if (strcmp("GProxyVolume", type_name) == 0)
    {
        ret = g_strdup_printf(
            "%s (%s)", type_name,
            (const char*)g_object_get_data(
                G_OBJECT(object), "g-proxy-volume-volume-monitor-name"));
    }
    else if (strcmp("GProxyMount", type_name) == 0)
    {
        ret = g_strdup_printf(
            "%s (%s)", type_name,
            (const char*)g_object_get_data(
                G_OBJECT(object), "g-proxy-mount-volume-monitor-name"));
    }
    else if (strcmp("GProxyShadowMount", type_name) == 0)
    {
        ret = g_strdup_printf(
            "%s (%s)", type_name,
            (const char*)g_object_get_data(
                G_OBJECT(object), "g-proxy-shadow-mount-volume-monitor-name"));
    }
    else
    {
...
static void list_mounts(GList* mounts, int indent, gboolean only_with_no_volume)
{
    GList* l;
    int c;
    GMount* mount;
        ...
        if (extra_detail)
        {
            ...
            if (icon)
            {
                if (G_IS_THEMED_ICON(icon))
                    show_themed_icon_names(G_THEMED_ICON(icon), FALSE,
                                           indent + 2);
            ...
            {
                if (G_IS_THEMED_ICON(icon))
                    show_themed_icon_names(G_THEMED_ICON(icon), TRUE,
                                           indent + 2);
            ...
            x_content_types =
                g_mount_guess_content_type_sync(mount, FALSE, NULL, NULL);
...
static void list_volumes(GList* volumes, int indent,
...
static void list_drives(GList* drives, int indent)
{
    GList *volumes, *l;
    int c, i;
    GDrive* drive;
    ...
    for (c = 0, l = drives; l != NULL; l = l->next, c++)
    {
        ...
        if (extra_detail)
        {
            ...
            if (ids && ids[0] != NULL)
            {
                ...
                for (i = 0; ids[i] != NULL; i++)
                {
                    char* id = g_drive_get_identifier(drive, ids[i]);
                    g_print("%*s %s: '%s'\n", indent + 2, "", ids[i], id);
                    g_free(id);
                }
            }
            ...
            {
                enum_value =
                    g_enum_get_value(klass, g_drive_get_start_stop_type(drive));
                g_print("%*sstart_stop_type=%s\n", indent + 2, "",
                        enum_value != NULL ? enum_value->value_nick
                                           : "UNKNOWN");
...
static void list_monitor_items(void)
{
...
static void unmount_all_with_scheme(const char* scheme)
{
    GList* mounts;
    GList* l;
...
static void mount_with_device_file_cb(GObject* object, GAsyncResult* res,
                                      gpointer user_data)
{
    GVolume* volume;
    gboolean succeeded;
...
static void mount_with_device_file(const char* device_file)
{
    GList* volumes;
    GList* l;
    ...
    for (l = volumes; l != NULL; l = l->next)
    {
        ...
        id = g_volume_get_identifier(volume,
        ...
        if (g_strcmp0(id, device_file) == 0)
        {
            ...
            g_volume_mount(volume, G_MOUNT_MOUNT_NONE, op, NULL,
                           mount_with_device_file_cb, id);
...
static void monitor_print_mount(GMount* mount)
{
    if (extra_detail)
    {
        GList* l;
...
static void monitor_print_volume(GVolume* volume)
{
    if (extra_detail)
    {
        GList* l;
...
static void monitor_print_drive(GDrive* drive)
{
    if (extra_detail)
    {
        GList* l;
...
static void monitor_mount_pre_unmount(GVolumeMonitor* volume_monitor,
                                      GMount* mount)
{
    char* name;
    name = g_mount_get_name(mount);
...
static void monitor_volume_added(GVolumeMonitor* volume_monitor,
                                 GVolume* volume)
{
    char* name;
    name = g_volume_get_name(volume);
...
static void monitor_volume_removed(GVolumeMonitor* volume_monitor,
                                   GVolume* volume)
{
    char* name;
    name = g_volume_get_name(volume);
...
static void monitor_volume_changed(GVolumeMonitor* volume_monitor,
                                   GVolume* volume)
{
    char* name;
    name = g_volume_get_name(volume);
...
static void monitor_drive_connected(GVolumeMonitor* volume_monitor,
                                    GDrive* drive)
{
    char* name;
    name = g_drive_get_name(drive);
...
static void monitor_drive_disconnected(GVolumeMonitor* volume_monitor,
                                       GDrive* drive)
{
    char* name;
    name = g_drive_get_name(drive);
...
static void monitor_drive_changed(GVolumeMonitor* volume_monitor, GDrive* drive)
{
    char* name;
    name = g_drive_get_name(drive);
...
static void monitor_drive_eject_button(GVolumeMonitor* volume_monitor,
                                       GDrive* drive)
{
    char* name;
    name = g_drive_get_name(drive);
