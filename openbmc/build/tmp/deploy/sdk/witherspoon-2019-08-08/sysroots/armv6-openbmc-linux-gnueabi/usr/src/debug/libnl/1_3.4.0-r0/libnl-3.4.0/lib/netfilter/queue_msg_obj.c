...
static void nfnl_queue_msg_dump(struct nl_object* a, struct nl_dump_params* p)
{
    ...
    if (msg->ce_mask & QUEUE_MSG_ATTR_INDEV)
    {
        if (link_cache)
            nl_dump(p, "IN=%s ",
                    rtnl_link_i2name(link_cache, msg->queue_msg_indev, buf,
                                     sizeof(buf)));
        else
    ...
    if (msg->ce_mask & QUEUE_MSG_ATTR_PHYSINDEV)
    {
        if (link_cache)
            nl_dump(p, "PHYSIN=%s ",
                    rtnl_link_i2name(link_cache, msg->queue_msg_physindev, buf,
                                     sizeof(buf)));
        else
    ...
    if (msg->ce_mask & QUEUE_MSG_ATTR_OUTDEV)
    {
        if (link_cache)
            nl_dump(p, "OUT=%s ",
                    rtnl_link_i2name(link_cache, msg->queue_msg_outdev, buf,
                                     sizeof(buf)));
        else
    ...
    if (msg->ce_mask & QUEUE_MSG_ATTR_PHYSOUTDEV)
    {
        if (link_cache)
            nl_dump(p, "PHYSOUT=%s ",
                    rtnl_link_i2name(link_cache, msg->queue_msg_physoutdev, buf,
                                     sizeof(buf)));
        else
    ...
    if (msg->ce_mask & QUEUE_MSG_ATTR_HWADDR)
    {
        ...
        for (i = 0; i < msg->queue_msg_hwaddr_len; i++)
            nl_dump(p, "%c%02x", i ? ':' : '=', msg->queue_msg_hwaddr[i]);
        nl_dump(p, " ");
    }
    ...
    if (msg->ce_mask & QUEUE_MSG_ATTR_HWPROTO)
        nl_dump(p, "HWPROTO=%s ",
                nl_ether_proto2str(ntohs(msg->queue_msg_hwproto), buf,
                                   sizeof(buf)));

    if (msg->ce_mask & QUEUE_MSG_ATTR_HOOK)
        nl_dump(p, "HOOK=%s ",
                nfnl_inet_hook2str(msg->queue_msg_hook, buf, sizeof(buf)));
    ...
    if (msg->ce_mask & QUEUE_MSG_ATTR_VERDICT)
        nl_dump(p, "VERDICT=%s ",
                nfnl_verdict2str(msg->queue_msg_verdict, buf, sizeof(buf)));
...
const struct timeval*
    nfnl_queue_msg_get_timestamp(const struct nfnl_queue_msg* msg)
{
