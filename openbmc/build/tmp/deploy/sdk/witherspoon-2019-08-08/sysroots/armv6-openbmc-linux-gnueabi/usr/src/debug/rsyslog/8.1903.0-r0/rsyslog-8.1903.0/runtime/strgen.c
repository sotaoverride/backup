 ...
 * for their existance is performance -- they do the same as template strings,
 * but potentially faster (if well implmented).
 *
...
DEFobjStaticHelpers DEFobjCurrIf(glbl) DEFobjCurrIf(ruleset)
...
/* destruct a strgen list. The list elements are destroyed, but the strgen
 ...
 * skip to the tail when adding elements...
 */
...
BEGINobjConstruct(
    strgen) /* be sure to specify the object type also in END macro! */
    ENDobjConstruct(strgen)

    /* ConstructionFinalizer. The most important chore is to add the strgen
     * object to our global list of available strgens. rgerhards, 2009-11-03
     */
...
BEGINobjDestruct(strgen) /* be sure to specify the object type also in END and
                            CODESTART macros! */
...
BEGINobjQueryInterface(strgen)
    CODESTARTobjQueryInterface(strgen) if (pIf->ifVersion !=
                                           strgenCURR_IF_VERSION)
...
finalize_it : ENDobjQueryInterface(strgen)

              /* This destroys the master strgenlist and all of its strgen
               * entries. MUST only be done when the module is shut down. Strgen
