...
netsnmp_feature_require(iquery) netsnmp_feature_require(string_time_to_secs)
...
void parse_sched_periodic(const char* token, char* line)
{
    ...
    entry->flags =
        SCHEDULE_FLAG_ENABLED | SCHEDULE_FLAG_ACTIVE | SCHEDULE_FLAG_VALID;
 ...
 */
void _sched_convert_bits(char* cron_spec, char* bit_buf, int bit_buf_len,
                         int max_val, int startAt1)
{
    char* cp = cron_spec;
...
void parse_schedTable(const char* token, char* line)
{
    ...
    line = read_config_read_data(ASN_OCTET_STR, line, &vp, &len);
    line =
        read_config_read_data(ASN_UNSIGNED, line, &entry->schedInterval, NULL);
    ...
    line = read_config_read_data(ASN_INTEGER, line, &entry->schedValue, NULL);
    line = read_config_read_data(ASN_UNSIGNED, line, &entry->schedType, NULL);
...
int store_schedTable(int majorID, int minorID, void* serverarg, void* clientarg)
{
    ...
    for (row = netsnmp_tdata_row_first(schedule_table); row;
