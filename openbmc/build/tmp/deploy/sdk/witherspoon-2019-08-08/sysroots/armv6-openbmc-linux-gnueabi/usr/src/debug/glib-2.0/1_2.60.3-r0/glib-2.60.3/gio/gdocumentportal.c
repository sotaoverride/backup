...
static gboolean init_document_portal(void)
{
    ...
    if (g_once_init_enter(&documents_inited))
    {
        ...
        if (connection != NULL)
        {
            documents = gxdp_documents_proxy_new_sync(
                connection, 0, "org.freedesktop.portal.Documents",
                "/org/freedesktop/portal/documents", NULL, &error);
            if (documents != NULL)
            {
                gxdp_documents_call_get_mount_point_sync(
                    documents, &documents_mountpoint, NULL, &error);

                if (error != NULL)
                {
                    g_warning("Cannot get document portal mount point: %s",
                              error->message);
                    g_error_free(error);
                }
            }
...
GList* g_document_portal_add_documents(GList* uris, const char* app_id,
                                       GError** error)
{
    int length;
    GList* ruris = NULL;
    ...
    if (g_unix_fd_list_get_length(fd_list) > 0)
    {
        if (!gxdp_documents_call_add_full_sync(
                documents, g_variant_builder_end(&builder),
                XDP_ADD_FLAGS_AS_NEEDED_BY_APP, app_id, permissions, fd_list,
                &doc_ids, &extra_out, NULL, NULL, error))
            goto out;
        ...
        for (l = uris, i = 0, j = 0; l; l = l->next, i++)
        {
            ...
            else
            {
                ...
                char* doc_path = g_build_filename(documents_mountpoint,
                                                  doc_ids[j], basename, NULL);
