...
static int mkfilter_rpc_byname(const char* name, char* buffer, size_t buflen)
{
    ...
    /* build filter */
    return mysnprintf(buffer, buflen, "(&%s(%s=%s))", rpc_filter, attmap_rpc_cn,
                      safename);
}

static int mkfilter_rpc_bynumber(int number, char* buffer, size_t buflen)
{
    return mysnprintf(buffer, buflen, "(&%s(%s=%d))", rpc_filter,
...
NSLCD_HANDLE(rpc, byname, NSLCD_ACTION_RPC_BYNAME, char name[BUFLEN_NAME];
...
NSLCD_HANDLE(rpc, bynumber, NSLCD_ACTION_RPC_BYNUMBER, int number;
             char filter[BUFLEN_FILTER]; READ_INT32(fp, number);
             log_setrequest("rpc=%lu", (unsigned long int)number);
             , mkfilter_rpc_bynumber(number, filter, sizeof(filter)),
             write_rpc(fp, entry, NULL))

NSLCD_HANDLE(rpc, all, NSLCD_ACTION_RPC_ALL, const char* filter;
             log_setrequest("rpc(all)");
             , (filter = rpc_filter, 0), write_rpc(fp, entry, NULL))
