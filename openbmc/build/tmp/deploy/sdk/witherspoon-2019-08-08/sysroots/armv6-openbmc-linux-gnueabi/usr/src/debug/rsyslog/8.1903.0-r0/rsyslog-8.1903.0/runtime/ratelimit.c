    ...
    static smsg_t* ratelimitGenRepMsg(ratelimit_t* ratelimit)
{
    ...
    if (ratelimit->nsupp == 1)
    { /* we simply use the original message! */
...
static rsRetVal doLastMessageRepeatedNTimes(ratelimit_t* ratelimit,
                                            smsg_t* pMsg, smsg_t** ppRepMsg)
{
    ...
    if (ratelimit->pMsg != NULL &&
        ...
        !strcmp(getPROCID(pMsg, LOCK_MUTEX),
                getPROCID(ratelimit->pMsg, LOCK_MUTEX)) &&
        !strcmp(getAPPNAME(pMsg, LOCK_MUTEX),
                getAPPNAME(ratelimit->pMsg, LOCK_MUTEX)))
    {
        ratelimit->nsupp++;
    ...
    { /* new message, do "repeat processing" & save it */
        if (ratelimit->pMsg != NULL)
        {
...
static void tellLostCnt(ratelimit_t* ratelimit)
{
    ...
    if (ratelimit->missed)
    {
        snprintf((char*)msgbuf, sizeof(msgbuf),
                 "%s: %u messages lost due to rate-limiting", ratelimit->name,
                 ratelimit->missed);
        ratelimit->missed = 0;
...
static int ATTR_NONNULL()
    withinRatelimit(ratelimit_t* __restrict__ const ratelimit, time_t tt,
                    const char* const appname)
{
    ...
    else
    {
        ...
        {
            snprintf(
                (char*)msgbuf, sizeof(msgbuf),
                "%s from <%s>: begin to drop messages due to rate-limiting",
                ratelimit->name, appname);
            logmsgInternal(RS_RET_RATE_LIMITED, LOG_SYSLOG | LOG_INFO, msgbuf,
                           0);
        }
        ret = 0;
    }
 ...
 */
rsRetVal ratelimitMsg(ratelimit_t* __restrict__ const ratelimit, smsg_t* pMsg,
                      smsg_t** ppRepMsg)
{
    DEFiRet;
    rsRetVal localRet;
 ...
 * settings.
 * if pMultiSub == NULL, a single-message enqueue happens (under
 * reconsideration)
 */
rsRetVal ratelimitAddMsg(ratelimit_t* ratelimit, multi_submit_t* pMultiSub,
                         smsg_t* pMsg)
{
    rsRetVal localRet;
    smsg_t* repMsg;
 ...
 */
rsRetVal ratelimitNew(ratelimit_t** ppThis, const char* modname,
                      const char* dynname)
{
    ratelimit_t* pThis;
    char namebuf[256];
...
void ratelimitSetLinuxLike(ratelimit_t* ratelimit, unsigned short interval,
                           unsigned burst)
{
