...
static void g_mount_operation_set_property(GObject* object, guint prop_id,
                                           const GValue* value,
                                           GParamSpec* pspec)
{
    ...
    switch (prop_id)
    {
        ...
        case PROP_DOMAIN:
            g_mount_operation_set_domain(operation, g_value_get_string(value));
            break;
        ...
        case PROP_CHOICE:
            g_mount_operation_set_choice(operation, g_value_get_int(value));
            break;

        case PROP_IS_TCRYPT_HIDDEN_VOLUME:
            g_mount_operation_set_is_tcrypt_hidden_volume(
                operation, g_value_get_boolean(value));
            break;
        ...
        case PROP_PIM:
            g_mount_operation_set_pim(operation, g_value_get_uint(value));
            break;
...
static void g_mount_operation_get_property(GObject* object, guint prop_id,
...
static void ask_password(GMountOperation* op, const char* message,
                         const char* default_user, const char* default_domain,
                         GAskPasswordFlags flags)
{
    g_idle_add_full(G_PRIORITY_DEFAULT_IDLE, reply_non_handled_in_idle,
                    g_object_ref(op), g_object_unref);
}

static void ask_question(GMountOperation* op, const char* message,
                         const char* choices[])
{
    g_idle_add_full(G_PRIORITY_DEFAULT_IDLE, reply_non_handled_in_idle,
                    g_object_ref(op), g_object_unref);
...
static void show_processes(GMountOperation* op, const gchar* message,
                           GArray* processes, const gchar* choices[])
{
    g_idle_add_full(G_PRIORITY_DEFAULT_IDLE, reply_non_handled_in_idle,
                    g_object_ref(op), g_object_unref);
}

static void show_unmount_progress(GMountOperation* op, const gchar* message,
...
static void g_mount_operation_class_init(GMountOperationClass* klass)
{
     ...
     */
    signals[ASK_PASSWORD] = g_signal_new(
        I_("ask-password"), G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
        G_STRUCT_OFFSET(GMountOperationClass, ask_password), NULL, NULL, NULL,
     ...
     * primary text in a #GtkMessageDialog.
     */
    signals[ASK_QUESTION] = g_signal_new(
        I_("ask-question"), G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
        G_STRUCT_OFFSET(GMountOperationClass, ask_question), NULL, NULL, NULL,
     ...
     */
    signals[REPLY] = g_signal_new(
        I_("reply"), G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
        G_STRUCT_OFFSET(GMountOperationClass, reply), NULL, NULL,
     ...
     * Since: 2.20
     */
    signals[ABORTED] = g_signal_new(
        I_("aborted"), G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
        G_STRUCT_OFFSET(GMountOperationClass, aborted), NULL, NULL,
     ...
     * Since: 2.22
     */
    signals[SHOW_PROCESSES] = g_signal_new(
        I_("show-processes"), G_TYPE_FROM_CLASS(object_class),
        G_SIGNAL_RUN_LAST,
        G_STRUCT_OFFSET(GMountOperationClass, show_processes), NULL, NULL, NULL,
        G_TYPE_NONE, 3, G_TYPE_STRING, G_TYPE_ARRAY, G_TYPE_STRV);
     ...
     *     completes (or -1 if such amount is not known), or zero if the
     ...
     * and then one last time when the operation is completed, with @bytes_left
     * set to zero.
     *
     * Implementations of GMountOperation should handle this signal by
     * showing an UI notification, and then dismiss it, or show another
     * notification of completion, when @bytes_left reaches zero.
     *
     * If the message contains a line break, the first line should be
     * presented as a heading. For example, it may be used as the
     * primary text in a #GtkMessageDialog.
     ...
     * Since: 2.34
     */
    signals[SHOW_UNMOUNT_PROGRESS] = g_signal_new(
        I_("show-unmount-progress"), G_TYPE_FROM_CLASS(object_class),
        G_SIGNAL_RUN_LAST,
        G_STRUCT_OFFSET(GMountOperationClass, show_unmount_progress), NULL,
        NULL, NULL, G_TYPE_NONE, 3, G_TYPE_STRING, G_TYPE_INT64, G_TYPE_INT64);
    ...
    g_object_class_install_property(
        ...
        g_param_spec_string("username", P_("Username"), P_("The user name"),
                            NULL,
    ...
    g_object_class_install_property(
        ...
        g_param_spec_string("password", P_("Password"), P_("The password"),
                            NULL,
                            G_PARAM_READWRITE | G_PARAM_STATIC_NAME |
                                G_PARAM_STATIC_NICK | G_PARAM_STATIC_BLURB));
    ...
    g_object_class_install_property(
        ...
        g_param_spec_boolean("anonymous", P_("Anonymous"),
                             P_("Whether to use an anonymous user"), FALSE,
                             G_PARAM_READWRITE | G_PARAM_STATIC_NAME |
                                 G_PARAM_STATIC_NICK | G_PARAM_STATIC_BLURB));
    ...
    g_object_class_install_property(
        ...
        g_param_spec_string("domain", P_("Domain"),
                            P_("The domain of the mount operation"), NULL,
                            G_PARAM_READWRITE | G_PARAM_STATIC_NAME |
                                G_PARAM_STATIC_NICK | G_PARAM_STATIC_BLURB));
     ...
     */
    g_object_class_install_property(
        object_class, PROP_PASSWORD_SAVE,
        g_param_spec_enum("password-save", P_("Password save"),
                          P_("How passwords should be saved"),
                          G_TYPE_PASSWORD_SAVE, G_PASSWORD_SAVE_NEVER,
                          G_PARAM_READWRITE | G_PARAM_STATIC_NAME |
                              G_PARAM_STATIC_NICK | G_PARAM_STATIC_BLURB));
     ...
     */
    g_object_class_install_property(
        object_class, PROP_CHOICE,
        g_param_spec_int("choice", P_("Choice"), P_("The users choice"), 0,
                         G_MAXINT, 0,
                         G_PARAM_READWRITE | G_PARAM_STATIC_NAME |
                             G_PARAM_STATIC_NICK | G_PARAM_STATIC_BLURB));
     ...
     * See [the VeraCrypt
     * documentation](https://www.veracrypt.fr/en/Hidden%20Volume.html).
     *
     * Since: 2.58
     */
    g_object_class_install_property(
        object_class, PROP_IS_TCRYPT_HIDDEN_VOLUME,
        g_param_spec_boolean(
            "is-tcrypt-hidden-volume", P_("TCRYPT Hidden Volume"),
     ...
     * [the VeraCrypt
     * documentation](https://www.veracrypt.fr/en/System%20Encryption.html).
     *
     * Since: 2.58
     */
    g_object_class_install_property(
        object_class, PROP_IS_TCRYPT_SYSTEM_VOLUME,
        g_param_spec_boolean(
            "is-tcrypt-system-volume", P_("TCRYPT System Volume"),
            P_("Whether to unlock a TCRYPT system volume. Only supported for "
               "unlocking Windows system volumes. See "
     ...
     */
    g_object_class_install_property(
        object_class, PROP_PIM,
        g_param_spec_uint("pim", P_("PIM"), P_("The VeraCrypt PIM value"), 0,
                          G_MAXUINT, 0,
                          G_PARAM_READWRITE | G_PARAM_STATIC_NAME |
                              G_PARAM_STATIC_NICK | G_PARAM_STATIC_BLURB));
}
