...
BOOST_LOG_ANONYMOUS_NAMESPACE
{
    ...
    //! A possible Boost.Filesystem extension - renames or moves the file to the
    //! target storage
    inline void move_file(filesystem::path const& from,
    ...
    //! An auxiliary traits that contain various constants and functions
    //! regarding string and character operations
    ...
    const file_char_traits<char>::char_type
        file_char_traits<char>::number_placeholder;
    const file_char_traits<char>::char_type
        file_char_traits<char>::day_placeholder;
    const file_char_traits<char>::char_type
        file_char_traits<char>::month_placeholder;
    const file_char_traits<char>::char_type
        file_char_traits<char>::year_placeholder;
    const file_char_traits<char>::char_type
        file_char_traits<char>::full_year_placeholder;
    const file_char_traits<char>::char_type
        file_char_traits<char>::frac_sec_placeholder;
    const file_char_traits<char>::char_type
        file_char_traits<char>::seconds_placeholder;
    const file_char_traits<char>::char_type
        file_char_traits<char>::minutes_placeholder;
    const file_char_traits<char>::char_type
        file_char_traits<char>::hours_placeholder;
    ...
    struct file_char_traits<wchar_t>
    {
        ...
        static std::wstring default_file_name_pattern()
        {
            return L"%5N.log";
        }
    };
...
#ifndef BOOST_LOG_BROKEN_STATIC_CONSTANTS_LINKAGE
    const file_char_traits<wchar_t>::char_type
        file_char_traits<wchar_t>::percent;
    const file_char_traits<wchar_t>::char_type
        file_char_traits<wchar_t>::number_placeholder;
    const file_char_traits<wchar_t>::char_type
        file_char_traits<wchar_t>::day_placeholder;
    const file_char_traits<wchar_t>::char_type
        file_char_traits<wchar_t>::month_placeholder;
    const file_char_traits<wchar_t>::char_type
        file_char_traits<wchar_t>::year_placeholder;
    const file_char_traits<wchar_t>::char_type
        file_char_traits<wchar_t>::full_year_placeholder;
    const file_char_traits<wchar_t>::char_type
        file_char_traits<wchar_t>::frac_sec_placeholder;
    const file_char_traits<wchar_t>::char_type
        file_char_traits<wchar_t>::seconds_placeholder;
    const file_char_traits<wchar_t>::char_type
        file_char_traits<wchar_t>::minutes_placeholder;
    const file_char_traits<wchar_t>::char_type
        file_char_traits<wchar_t>::hours_placeholder;
    ...
    const file_char_traits<wchar_t>::char_type
        file_char_traits<wchar_t>::newline;
#endif // BOOST_LOG_BROKEN_STATIC_CONSTANTS_LINKAGE
    ...
    class date_and_time_formatter
    {
      ...
      private:
        typedef date_time::time_facet<posix_time::ptime, path_char_type>
            time_facet_type;
      ...
      public:
        ...
        //! The method formats the current date and time according to the format
        //! string str and writes the result into it
        path_string_type operator()(path_string_type const& pattern,
                                    unsigned int counter) const
        {
            m_Facet.format(pattern.c_str());
            m_Stream.str(path_string_type());
            // Note: the regular operator<< fails because std::use_facet fails
            // to find the facet in the locale because the facet type in
            // Boost.DateTime has hidden visibility. See this ticket:
            // https://svn.boost.org/trac/boost/ticket/11707
            std::ostreambuf_iterator<path_char_type> sbuf_it(m_Stream);
            m_Facet.put(sbuf_it, m_Stream, m_Stream.fill(),
                        boost::log::attributes::local_time_traits::get_clock());
            if (m_Stream.good())
            {
                return m_Stream.str();
            }
        ...
        BOOST_DELETED_FUNCTION(
            date_and_time_formatter& operator=(date_and_time_formatter const&))
    };
    ...
    class file_counter_formatter
    {
      ...
      public:
        ...
        file_counter_formatter(path_string_type::size_type pos,
                               unsigned int width) :
        ...
        BOOST_DELETED_FUNCTION(
            file_counter_formatter& operator=(file_counter_formatter const&))
    };
    ...
    class empty_formatter
    {
      ...
      public:
        //! Initializing constructor
        explicit empty_formatter(path_string_type const& pattern) :
            m_Pattern(pattern)
        {
        }
        //! Copy constructor
        ...
        BOOST_DELETED_FUNCTION(
            empty_formatter& operator=(empty_formatter const&))
    };
                                   ...
                                   unsigned int& width)
    {
        ...
        path_char_type c = *it;
        if (c == traits_t::zero || c == traits_t::space ||
            c == traits_t::plus || c == traits_t::minus)
        {
                       ...
                       unsigned int& file_counter, bool& file_counter_parsed)
    {
        ...
        while (f_it != f_end && p_it != p_end)
        {
            ...
            else
            {
                switch (p_c)
                {
                    ...
                    case traits_t::seconds_placeholder: // Date/time components
                                                        // with 2-digits width
                    ...
                    case traits_t::full_year_placeholder: // Date/time
                                                          // components with
                                                          // 4-digits width
                    ...
                    case traits_t::
                        frac_sec_placeholder: // Fraction seconds width is
                                              // configuration-dependent
                        typedef posix_time::time_res_traits
                            posix_resolution_traits;
                    ...
                    default: // This should be the file counter placeholder or
                             // some unsupported placeholder
                    {
                        ...
                        if (!parse_counter_placeholder(p, p_end, width))
                        {
                            BOOST_THROW_EXCEPTION(std::invalid_argument(
                                "Unsupported placeholder used in pattern for "
                                "file scanning"));
                        }
        ...
        if (p_it == p_end)
        {
            if (f_it != f_end)
            {
                ...
                return local::scan_digits(f_it, f_end,
                                          std::distance(f_it, f_end));
            }
            else
                return true;
        }
    ...
    class file_collector : public file::collector,
                           ...
                           public enable_shared_from_this<file_collector>
    {
      ...
      public:
        //! Constructor
        file_collector(shared_ptr<file_collector_repository> const& repo,
                       filesystem::path const& target_dir, uintmax_t max_size,
                       uintmax_t min_free_space, uintmax_t max_files);
        ...
        //! Scans the target directory for the files that have already been
        //! stored
        uintmax_t scan_for_files(file::scan_method method,
    ...
    class file_collector_repository
        : public log::aux::lazy_singleton<file_collector_repository,
                                          shared_ptr<file_collector_repository>>
    {
      private:
        //! Base type
        typedef log::aux::lazy_singleton<file_collector_repository,
                                         shared_ptr<file_collector_repository>>
            base_type;

#if !defined(BOOST_LOG_BROKEN_FRIEND_TEMPLATE_SPECIALIZATIONS)
        friend class log::aux::lazy_singleton<
            file_collector_repository, shared_ptr<file_collector_repository>>;
#else
        friend class base_type;
#endif

        //! The type of the list of collectors
        typedef intrusive::list<file_collector,
      ...
      public:
        ...
        shared_ptr<file::collector>
            get_collector(filesystem::path const& target_dir,
                          uintmax_t max_size, uintmax_t min_free_space,
                          uintmax_t max_files);
      ...
      private:
        ...
        static void init_instance()
        {
            base_type::get_instance() =
                boost::make_shared<file_collector_repository>();
        }
    };
    ...
    file_collector::file_collector(
        ...
        uintmax_t min_free_space, uintmax_t max_files) :
        m_pRepository(repo),
        m_MaxSize(max_size), m_MinFreeSpace(min_free_space),
        m_MaxFiles(max_files), m_BasePath(filesystem::current_path()),
        m_TotalSize(0)
    {
    ...
    void file_collector::store_file(filesystem::path const& src_path)
    {
        ...
        // Avoid using Boost.Filesystem functions that would call
        // path::codecvt(). store_file() can be called at process termination,
        // and the global codecvt facet can already be destroyed at this point.
        ...
        filesystem::path src_dir =
            src_path.has_parent_path()
                ...
                : m_BasePath;
        const bool is_in_target_dir =
            filesystem::equivalent(src_dir, m_StorageDir);
        if (!is_in_target_dir)
        {
            if (filesystem::exists(info.m_Path))
            {
                // If the file already exists, try to mangle the file name
                // to ensure there's no conflict. I'll need to make this
                ...
                while (true)
                {
                    ...
                    info.m_Path =
                        m_StorageDir / filesystem::path(alt_file_name);
                    if (!filesystem::exists(info.m_Path))
                        break;
                    ...
                    {
                        BOOST_THROW_EXCEPTION(filesystem_error(
                            "Target file exists and an unused fallback file "
                            "name could not be found",
                            info.m_Path,
                            system::error_code(system::errc::io_error,
            ...
            // The directory should have been created in constructor, but just
            // in case it got deleted since then...
            filesystem::create_directories(m_StorageDir);
        }
        ...
        if (is_in_target_dir)
        {
            // If the sink writes log file into the target dir (is_in_target_dir
            // == true), it is possible that after scanning an old file entry
            // refers to the file that is picked up by the sink for writing.
            // Later on, the sink attempts to store the file in the storage. At
            // best, this would result in duplicate file entries. At worst, if
            // the storage limits trigger a deletion and this file get deleted,
            // we may have an entry that refers to no actual file. In any case,
            // the total size of files in the storage will be incorrect. Here we
            // work around this problem and simply remove the old file entry
            // without removing the file. The entry will be re-added to the list
            // later.
            while (it != end)
            {
                system::error_code ec;
        ...
        uintmax_t free_space = m_MinFreeSpace
                                   ? filesystem::space(m_StorageDir).available
                                   : static_cast<uintmax_t>(0);
        while (it != end && (m_TotalSize + info.m_Size > m_MaxSize ||
                             (m_MinFreeSpace && m_MinFreeSpace > free_space) ||
                             m_MaxFiles <= m_Files.size()))
        {
            file_info& old_info = *it;
            system::error_code ec;
            filesystem::file_status status =
                filesystem::status(old_info.m_Path, ec);
            ...
            if (status.type() == filesystem::regular_file)
            {
                try
                {
                    ...
                    // Free space has to be queried as it may not increase
                    // equally to the erased file size on compressed filesystems
                    if (m_MinFreeSpace)
    ...
    uintmax_t file_collector::scan_for_files(file::scan_method method,
                                             filesystem::path const& pattern,
                                             unsigned int* counter)
    {
        ...
        if (method != file::no_scan)
        {
            ...
            if (status.type() == filesystem::directory_file)
            {
                ...
                for (; it != end; ++it)
                {
                    ...
                    if (status.type() == filesystem::regular_file)
                    {
                        // Check that there are no duplicates in the resulting
                        // list
                        struct local
                        {
                            static bool equivalent(filesystem::path const& left,
                                                   file_info const& right)
                            {
                                return filesystem::equivalent(left,
                                                              right.m_Path);
                            }
                        };
                        if (std::find_if(m_Files.begin(), m_Files.end(),
                                         boost::bind(&local::equivalent,
                                                     boost::cref(info.m_Path),
                                                     _1)) == m_Files.end())
                        {
                            ...
                            if (method != file::scan_matching ||
                                match_pattern(filename_string(info.m_Path),
                                              mask, file_number,
                                              file_number_parsed))
                            {
                                info.m_Size =
                                    filesystem::file_size(info.m_Path);
                                total_size += info.m_Size;
                                info.m_TimeStamp =
                                    filesystem::last_write_time(info.m_Path);
                                files.push_back(info);
                                ++file_count;

                                // Test that the file_number >= *counter
                                // accounting for the integer overflow
                                if (file_number_parsed && counter != NULL &&
                                    (file_number - *counter) <
                                        ((~0u) ^ ((~0u) >> 1)))
                ...
                m_Files.sort(boost::bind(&file_info::m_TimeStamp, _1) <
                             boost::bind(&file_info::m_TimeStamp, _2));
            }
        }
        ...
        uintmax_t min_free_space, uintmax_t max_files)
    {
        ...
        file_collectors::iterator it =
            std::find_if(m_Collectors.begin(), m_Collectors.end(),
                         boost::bind(&file_collector::is_governed, _1,
        ...
        if (!p)
        {
            p = boost::make_shared<file_collector>(
                file_collector_repository::get(), target_dir, max_size,
                min_free_space, max_files);
    ...
    void check_time_point_validity(unsigned char hour, unsigned char minute,
                                   unsigned char second)
    {
        if (hour >= 24)
        {
            std::ostringstream strm;
            strm << "Time point hours value is out of range: "
        ...
        {
            std::ostringstream strm;
            strm << "Time point minutes value is out of range: "
        ...
        {
            std::ostringstream strm;
            strm << "Time point seconds value is out of range: "
...
BOOST_LOG_API shared_ptr<collector>
    make_collector(filesystem::path const& target_dir, uintmax_t max_size,
                   uintmax_t min_free_space, uintmax_t max_files)
{
...
BOOST_LOG_API rotation_at_time_point::rotation_at_time_point(
    ...
    m_Day(0), m_Hour(hour), m_Minute(minute), m_Second(second),
    m_Previous(date_time::not_a_date_time)
{
    check_time_point_validity(hour, minute, second);
}

//! Creates a rotation time point of each specified weekday at the specified
//! time
BOOST_LOG_API rotation_at_time_point::rotation_at_time_point(
    date_time::weekdays wday, unsigned char hour, unsigned char minute,
    unsigned char second) :
    m_DayKind(weekday),
    m_Day(static_cast<unsigned char>(wday)), m_Hour(hour), m_Minute(minute),
    m_Second(second), m_Previous(date_time::not_a_date_time)
{
    check_time_point_validity(hour, minute, second);
}

//! Creates a rotation time point of each specified day of month at the
//! specified time
BOOST_LOG_API rotation_at_time_point::rotation_at_time_point(
    gregorian::greg_day mday, unsigned char hour, unsigned char minute,
    unsigned char second) :
    m_DayKind(monthday),
    m_Day(static_cast<unsigned char>(mday.as_number())), m_Hour(hour),
    m_Minute(minute), m_Second(second), m_Previous(date_time::not_a_date_time)
{
    check_time_point_validity(hour, minute, second);
}
...
BOOST_LOG_API bool rotation_at_time_point::operator()() const
{
    ...
    const bool time_of_day_passed = rotation_time.total_seconds() <=
                                    m_Previous.time_of_day().total_seconds();
    switch (m_DayKind)
    {
        case not_specified:
        {
        ...
        case weekday:
        {
            // The rotation takes place on the specified week day at the
            // specified time
            gregorian::date previous_date = m_Previous.date(),
                            next_date = previous_date;
            int weekday = m_Day,
                previous_weekday =
                    static_cast<int>(previous_date.day_of_week().as_number());
            next_date += gregorian::days(weekday - previous_weekday);
            if (weekday < previous_weekday ||
                (weekday == previous_weekday && time_of_day_passed))
            {
        ...
        case monthday:
        {
            // The rotation takes place on the specified day of month at the
            // specified time
            gregorian::date previous_date = m_Previous.date();
            gregorian::date::day_type
                monthday = static_cast<gregorian::date::day_type>(m_Day),
                previous_monthday = previous_date.day();
            gregorian::date next_date(previous_date.year(),
                                      previous_date.month(), monthday);
            if (monthday < previous_monthday ||
                (monthday == previous_monthday && time_of_day_passed))
            {
...
struct text_file_backend::implementation
{
    ...
    boost::log::aux::light_function<path_string_type(unsigned int)>
        m_FileNameGenerator;
    ...
    implementation(uintmax_t rotation_size, bool auto_flush,
        ...
        m_FileCounter(0), m_CharactersWritten(0),
        m_FileRotationSize(rotation_size), m_AutoFlush(auto_flush),
        m_FinalRotationEnabled(enable_final_rotation)
    {
    }
};

//! Constructor. No streams attached to the constructed backend, auto flush
//! feature disabled.
...
BOOST_LOG_API text_file_backend::~text_file_backend()
{
    try
    {
        ...
        if (m_pImpl->m_FinalRotationEnabled && m_pImpl->m_File.is_open() &&
...
BOOST_LOG_API void text_file_backend::construct(
    filesystem::path const& pattern, std::ios_base::openmode mode,
    uintmax_t rotation_size,
    time_based_rotation_predicate const& time_based_rotation, bool auto_flush,
    bool enable_final_rotation)
{
    m_pImpl =
        new implementation(rotation_size, auto_flush, enable_final_rotation);
...
BOOST_LOG_API void text_file_backend::set_time_based_rotation(
    time_based_rotation_predicate const& predicate)
{
    m_pImpl->m_TimeBasedRotation = predicate;
}

//! The method allows to enable or disable log file rotation on sink
//! destruction.
...
//! Sets the flag to automatically flush write buffers of the file being written
...
BOOST_LOG_API void
    text_file_backend::consume(record_view const& rec,
                               string_type const& formatted_message)
{
    ...
    {
        // The file stream is not operational. One possible reason is that there
        // is no more free space on the file system. In this case it is possible
        // that this log record will fail to be written as well, leaving the
        // newly creted file empty. Eventually this results in lots of empty log
        // files. We should take precautions to avoid this.
        // https://svn.boost.org/trac/boost/ticket/11016
        ...
        if (!!ec || size == 0)
        {
            // To reuse the empty file avoid re-generating the new file name
            // later
    ...
    else if (m_pImpl->m_File.is_open() &&
             (m_pImpl->m_CharactersWritten + formatted_message.size() >=
                  m_pImpl->m_FileRotationSize ||
              (!m_pImpl->m_TimeBasedRotation.empty() &&
               m_pImpl->m_TimeBasedRotation())))
    {
        rotate_file();
    }
    ...
    if (!m_pImpl->m_File.is_open())
    {
        ...
        if (!use_prev_file_name)
            new_file_name =
                ...
                m_pImpl->m_FileNameGenerator(m_pImpl->m_FileCounter++);
        else
        ...
        {
            BOOST_THROW_EXCEPTION(filesystem_error(
                "Failed to open file for writing", new_file_name,
                system::error_code(system::errc::io_error,
                                   system::generic_category())));
        }
        ...
        m_pImpl->m_CharactersWritten =
            static_cast<std::streamoff>(m_pImpl->m_File.tellp());
    }
...
BOOST_LOG_API void text_file_backend::set_file_name_pattern_internal(
    filesystem::path const& pattern)
{
    // Note: avoid calling Boost.Filesystem functions that involve
    // path::codecvt() https://svn.boost.org/trac/boost/ticket/9119
    ...
    if (placeholder_count > 0)
    {
        if (counter_found)
        {
            ...
            m_pImpl->m_FileNameGenerator = boost::bind(
                date_and_time_formatter(),
                boost::bind(file_counter_formatter(counter_pos, width),
                            name_pattern, _1),
                _1);
        }
        else
    ...
    else if (counter_found)
    {
        ...
        m_pImpl->m_FileNameGenerator = boost::bind(
            file_counter_formatter(counter_pos, width), name_pattern, _1);
    }
    else
    {
...
void text_file_backend::close_file()
{
    if (m_pImpl->m_File.is_open())
    {
        ...
        {
            // Rationale: We should call the close handler even if the stream is
            // !good() because writing the footer may not be the only thing the
            // handler does. However, there is a chance that the file had become
            // writable since the last failure (e.g. there was no space left to
            // write the last record, but it got freed since then), so if the
            // handler attempts to write a footer it may succeed now. For this
            // reason we clear the stream state and let the handler have a try.
...
BOOST_LOG_API void
    text_file_backend::set_open_mode(std::ios_base::openmode mode)
{
...
BOOST_LOG_API void text_file_backend::set_file_collector(
    shared_ptr<file::collector> const& collector)
{
...
BOOST_LOG_API void
    text_file_backend::set_open_handler(open_handler_type const& handler)
{
...
BOOST_LOG_API void
    text_file_backend::set_close_handler(close_handler_type const& handler)
{
    m_pImpl->m_CloseHandler = handler;
}

//! The method returns name of the currently open log file. If no file is open,
...
BOOST_LOG_API uintmax_t text_file_backend::scan_for_files(
    file::scan_method method, bool update_counter)
{
    if (m_pImpl->m_pFileCollector)
    {
        unsigned int* counter = update_counter
                                    ? &m_pImpl->m_FileCounter
                                    : static_cast<unsigned int*>(NULL);
        return m_pImpl->m_pFileCollector->scan_for_files(
            method, m_pImpl->m_FileNamePattern, counter);
    }
    else
    {
