...
static void display_help(int status)
{
    fprintf(status == EXIT_SUCCESS ? stdout : stderr,
            ...
            "Adds a partition to an MTD device, or remove an existing "
            "partition from it.\n"
            "\n"
            "  -h, --help    Display this help and exit\n"
            "  -V, --version Output version information and exit\n"
            "\n"
            "START location and SIZE of the partition are in bytes. They "
            "should align on\n"
            "eraseblock size.\n",
            PROGRAM_NAME);
    exit(status);
...
static void process_options(int argc, char* const argv[])
{
    ...
    if (strcmp(s_command, "del") == 0 && (argc - optind) == 1)
    {
        ...
        if (tmp < 0)
            errmsg_die("Can't specify negative partition number: %ld", tmp);
        if (tmp > INT_MAX)
            errmsg_die("Partition number exceeds INT_MAX: %ld", tmp);
    ...
    else if (strcmp(s_command, "add") == 0 && (argc - optind) == 3)
    {
        ...
        if (start_addr < 0)
            errmsg_die("Can't specify negative start offset: %lld", start_addr);

        length = simple_strtoll(s_length, &error);
        if (length < 0)
            errmsg_die("Can't specify negative length: %lld", length);
