...
bool is_locale_utf8(void)
{
     ...
     * explicitly. */
    cached_answer = STR_IN_SET(set, "C", "POSIX") && !getenv("LC_ALL") &&
...
const char* special_glyph(SpecialGlyph code)
{
    ...
    /* A list of a number of interesting unicode glyphs we can use to decorate
     * our output. It's probably wise to be conservative here, and primarily
     * stick to the glyphs defined in the eurlatgr font, so that display still
    ...
    return draw_table[code >= _SPECIAL_GLYPH_FIRST_SMILEY
                          ? emoji_enabled()
                          : is_locale_utf8()][code];
}
