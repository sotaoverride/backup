    ...
    while (p != end)
    {
        switch (*p)
        {
            ...
            case DBUS_DICT_ENTRY_END_CHAR:
                ...
                element_count = _DBUS_POINTER_TO_INT(
                    _dbus_list_pop_last(&element_count_stack));
        ...
        if (*p != DBUS_TYPE_ARRAY && *p != DBUS_DICT_ENTRY_BEGIN_CHAR &&
 ...
 */
static DBusValidity validate_body_helper(DBusTypeReader* reader, int byte_order,
      ...
      _dbus_verbose ("   validating value of type %s type reader %p type_pos %d p %p end %p %d remain\n",
        ...
        switch (current_type)
        {
            ...
            case DBUS_TYPE_OBJECT_PATH:
            {
                ...
                {
                    DBusString str;
                    _dbus_string_init_const_len(&str, (const char*)p,
                                                claimed_len);
                ...
                else if (current_type == DBUS_TYPE_ARRAY && claimed_len > 0)
                {
                    ...
                    /* avoid recursive call to validate_body_helper if this is
                     * an array of fixed-size elements
                     */
                    if (dbus_type_is_fixed(array_elem_type))
                    {
                        /* bools need to be handled differently, because they
                        ...
                        if (array_elem_type == DBUS_TYPE_BOOLEAN)
                        {
                            dbus_uint32_t v;
                            alignment =
                                _dbus_type_get_alignment(array_elem_type);
                    ...
                    else
                    {
                        while (p < array_end)
                        {
                            validity = validate_body_helper(
                                &sub, byte_order, FALSE, total_depth + 1, p,
                                end, &p);
            ...
            case DBUS_TYPE_SIGNATURE:
            {
                ...
                validity = _dbus_validate_signature_with_reason(
                    &str, 0, _dbus_string_get_length(&str));
            ...
            case DBUS_TYPE_VARIANT:
            {
                ...
                /* In addition to normal signature validation, we need to be
                 * sure the signature contains only a single (possibly
                 * container) type.
                 */
                dbus_uint32_t claimed_len;
                DBusString sig;
                ...
                reason = _dbus_validate_signature_with_reason(
                    &sig, 0, _dbus_string_get_length(&sig));
...
DBusValidity _dbus_validate_body_with_reason(
    const DBusString* expected_signature, int expected_signature_start,
    int byte_order, int* bytes_remaining, const DBusString* value_str,
    int value_pos, int len)
{
    ...
    _dbus_verbose("validating body from pos %d len %d sig '%s'\n", value_pos,
                  len,
                  _dbus_string_get_const_data_len(expected_signature,
                                                  expected_signature_start, 0));

    _dbus_type_reader_init_types_only(&reader, expected_signature,
...
const char* _dbus_validity_to_error_message(DBusValidity validity)
{
    switch (validity)
    {
        ...
        case DBUS_INVALID_NESTED_TOO_DEEPLY:
            return "Variants cannot be used to create a hugely recursive tree "
                   "of values";
...
static dbus_bool_t _dbus_validate_bus_name_full(const DBusString* str,
                                                int start, int len,
        ...
        if (*s == '.')
        {
            if (_DBUS_UNLIKELY((s + 1) == end))
                return FALSE;
                         ...
                         !VALID_INITIAL_BUS_NAME_CHARACTER(*(s + 1))))
                return FALSE;
            last_dot = s;
            ++s; /* we just validated the next char, so skip two */
        }
 ...
 * @returns #TRUE if the byte range exists and is a valid name
 */
dbus_bool_t _dbus_validate_bus_namespace(const DBusString* str, int start,
                                         int len)
{
