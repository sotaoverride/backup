...
static void _g_dbus_auth_finalize(GObject* object)
{
    ...
    g_list_free_full(auth->priv->available_mechanisms,
                     (GDestroyNotify)mechanism_free);

    if (G_OBJECT_CLASS(_g_dbus_auth_parent_class)->finalize != NULL)
        G_OBJECT_CLASS(_g_dbus_auth_parent_class)->finalize(object);
}

static void _g_dbus_auth_get_property(GObject* object, guint prop_id,
...
static void _g_dbus_auth_set_property(GObject* object, guint prop_id,
...
static void add_mechanism(GDBusAuth* auth, GDBusAuthObserver* observer,
                          GType mechanism_type)
{
        ...
        g_dbus_auth_observer_allow_mechanism(observer, name))
    {
        ...
        auth->priv->available_mechanisms =
            g_list_prepend(auth->priv->available_mechanisms, m);
    }
}
...
static void _g_dbus_auth_add_mechs(GDBusAuth* auth, GDBusAuthObserver* observer)
{
    ...
    auth->priv->available_mechanisms = g_list_sort(
        auth->priv->available_mechanisms, (GCompareFunc)mech_compare_func);
}
...
/* ----------------------------------------------------------------------------------------------------
 */
/* like g_data_input_stream_read_line() but sets error if there's no content to
 * read */
static gchar* _my_g_data_input_stream_read_line(GDataInputStream* dis,
                                                ...
                                                GError** error)
{
    ...
    ret =
        g_data_input_stream_read_line(dis, out_line_length, cancellable, error);
 ...
 * this)
 */
static gchar* _my_g_input_stream_read_line_safe(GInputStream* i,
                                                gsize* out_line_length,
                                                GCancellable* cancellable,
                                                GError** error)
{
        ...
        if (num_read == 0)
        {
            if (error != NULL && *error == NULL)
            {
                g_set_error_literal(error, G_IO_ERROR, G_IO_ERROR_FAILED,
                                    _("Unexpected lack of content trying to "
                                      "(safely) read a line"));
            }
            goto fail;
        }
    ...
    for (n = 0; str[n] != '\0'; n += 2)
    {
        ...
        if (upper_nibble == -1 || lower_nibble == -1)
        {
            g_set_error(error, G_IO_ERROR, G_IO_ERROR_FAILED,
                        "Error hexdecoding string '%s' around position %d", str,
                        n);
            goto out;
        }
...
static GDBusAuthMechanism* client_choose_mech_and_send_initial_response(
    GDBusAuth* auth, GCredentials* credentials_that_were_sent,
...
again:
    ...
    for (n = 0; supported_auth_mechs[n] != NULL; n++)
    {
        ...
        {
            if (g_strcmp0(supported_auth_mechs[n],
                          attempted_auth_mechs->pdata[m]) == 0)
            {
        ...
        if (!attempted_already)
        {
            auth_mech_to_use_gtype =
                find_mech_by_name(auth, supported_auth_mechs[n]);
    ...
    if (auth_mech_to_use_gtype == (GType)0)
    {
        ...
        g_set_error(error, G_IO_ERROR, G_IO_ERROR_FAILED,
                    _("Exhausted all available authentication mechanisms "
                      "(tried: %s) (available: %s)"),
    ...
    mech = g_object_new(auth_mech_to_use_gtype, "stream", auth->priv->stream,
                        "credentials", credentials_that_were_sent, NULL);
    debug_print("CLIENT: Trying mechanism '%s'",
                _g_dbus_auth_mechanism_get_name(auth_mech_to_use_gtype));
    g_ptr_array_add(
        attempted_auth_mechs,
        (gpointer)_g_dbus_auth_mechanism_get_name(auth_mech_to_use_gtype));
    ...
    {
        debug_print("CLIENT: Mechanism '%s' says it is not supported",
                    _g_dbus_auth_mechanism_get_name(auth_mech_to_use_gtype));
        g_object_unref(mech);
        mech = NULL;
        goto again;
    ...
    initial_response =
        _g_dbus_auth_mechanism_client_initiate(mech, &initial_response_len);
#if 0
...
gchar*
    _g_dbus_auth_run_client(GDBusAuth* auth, GDBusAuthObserver* observer,
                            ...
                            GCancellable* cancellable, GError** error)
{
    ...
    dis = G_DATA_INPUT_STREAM(g_data_input_stream_new(
        g_io_stream_get_input_stream(auth->priv->stream)));
    dos = G_DATA_OUTPUT_STREAM(g_data_output_stream_new(
        g_io_stream_get_output_stream(auth->priv->stream)));
    g_filter_input_stream_set_close_base_stream(G_FILTER_INPUT_STREAM(dis),
                                                FALSE);
    g_filter_output_stream_set_close_base_stream(G_FILTER_OUTPUT_STREAM(dos),
                                                 FALSE);
    ...
    if (G_IS_UNIX_CONNECTION(auth->priv->stream))
    {
        ...
        if (!g_unix_connection_send_credentials(
                G_UNIX_CONNECTION(auth->priv->stream), cancellable, error))
            goto out;
    }
    else
    {
    ...
    while (TRUE)
    {
        switch (state)
        {
            case CLIENT_STATE_WAITING_FOR_REJECT:
                ...
                line = _my_g_data_input_stream_read_line(dis, &line_length,
                                                         cancellable, error);
                if (line == NULL)
                    goto out;
            ...
            choose_mechanism:
                ...
                {
                    g_set_error(error, G_IO_ERROR, G_IO_ERROR_FAILED,
                                "In WaitingForReject: Expected 'REJECTED am1 "
                                "am2 ... amN', got '%s'",
                                line);
                    g_free(line);
                    goto out;
                ...
                if (supported_auth_mechs == NULL)
                {
                    supported_auth_mechs =
                        g_strsplit(line + sizeof("REJECTED ") - 1, " ", 0);
#if 0
              for (n = 0; supported_auth_mechs != NULL && supported_auth_mechs[n] != NULL; n++)
                ...
                g_free(line);
                mech = client_choose_mech_and_send_initial_response(
                    auth, credentials,
                    (const gchar* const*)supported_auth_mechs,
                    attempted_auth_mechs, dos, cancellable, error);
                if (mech == NULL)
                    goto out;
            ...
            case CLIENT_STATE_WAITING_FOR_AGREE_UNIX_FD:
                ...
                if (g_strcmp0(line, "AGREE_UNIX_FD") == 0)
                {
                    ...
                    s = "BEGIN\r\n";
                    debug_print("CLIENT: writing '%s'", s);
                    if (!g_data_output_stream_put_string(dos, s, cancellable,
                                                         error))
                        goto out;
                    /* and we're done! */
                    goto out;
                         ...
                         (line[5] == 0 || g_ascii_isspace(line[5])))
                {
                    // g_strstrip (line + 5); g_debug ("bah, no unix_fd: '%s'",
                    // line + 5);
                    g_free(line);
                    s = "BEGIN\r\n";
                    debug_print("CLIENT: writing '%s'", s);
            ...
            case CLIENT_STATE_WAITING_FOR_DATA:
                ...
                if (g_str_has_prefix(line, "DATA "))
                {
                    ...
                    _g_dbus_auth_mechanism_client_data_receive(
                        mech, decoded_data, decoded_data_len);
                    g_free(decoded_data);
                ...
                {
                    /* could be the chosen authentication method just doesn't
...
static gchar* get_auth_mechanisms(GDBusAuth* auth, gboolean allow_anonymous,
...
gboolean
    _g_dbus_auth_run_server(GDBusAuth* auth, GDBusAuthObserver* observer,
                            const gchar* guid, gboolean allow_anonymous,
                            GDBusCapabilityFlags offered_capabilities,
                            GDBusCapabilityFlags* out_negotiated_capabilities,
                            ...
                            GCancellable* cancellable, GError** error)
{
    ...
    {
        g_set_error(error, G_IO_ERROR, G_IO_ERROR_FAILED,
                    "The given guid '%s' is not valid", guid);
        goto out;
    }
    ...
    if (G_IS_UNIX_CONNECTION(auth->priv->stream))
    {
        ...
        credentials = g_unix_connection_receive_credentials(
            G_UNIX_CONNECTION(auth->priv->stream), cancellable, &local_error);
        if (credentials == NULL &&
        ...
        switch (state)
        {
            case SERVER_STATE_WAITING_FOR_AUTH:
                ...
                else if (g_str_has_prefix(line, "AUTH "))
                {
                    ...
                    /* TODO: record that the client has attempted to use this
                     * mechanism */
                    ...
                    if ((auth_mech_to_use_gtype == (GType)0) ||
                        (!allow_anonymous &&
                         g_strcmp0(mech_name, "ANONYMOUS") == 0))
                    {
                        /* We don't support this auth mechanism */
                        g_strfreev(tokens);
                        s = get_auth_mechanisms(auth, allow_anonymous,
                                                "REJECTED ", "\r\n", " ");
                        debug_print("SERVER: writing '%s'", s);
                        if (!g_data_output_stream_put_string(
                                dos, s, cancellable, error))
                    ...
                    else
                    {
                        ...
                        g_clear_object(&mech);
                        mech = g_object_new(auth_mech_to_use_gtype, "stream",
                                            auth->priv->stream, "credentials",
                                            credentials, NULL);
                        ...
                        if (encoded != NULL)
                        {
                            initial_response = hexdecode(
                                encoded, &initial_response_len, error);
                            if (initial_response == NULL)
                            {
                                g_prefix_error(
                                    error, "Initial response is malformed: ");
                                /* invalid encoding, disconnect! */
                                g_strfreev(tokens);
                                goto out;
                            }
                        ...
                        _g_dbus_auth_mechanism_server_initiate(
                            mech, initial_response, initial_response_len);
                        g_free(initial_response);
                        g_strfreev(tokens);
            ...
            case SERVER_STATE_WAITING_FOR_DATA:
                ...
                if (g_str_has_prefix(line, "DATA "))
                {
                    ...
                    _g_dbus_auth_mechanism_server_data_receive(
                        mech, decoded_data, decoded_data_len);
                    g_free(decoded_data);
                    /* oh man, this goto-crap is so ugly.. really need to
            ...
            case SERVER_STATE_WAITING_FOR_BEGIN:
                ...
                /* Use extremely slow (but reliable) line reader - this
                 * basically does a recvfrom() system call per character
                 *
                 * (the problem with using GDataInputStream's read_line is that
                 * because of buffering it might start reading into the first
                 * D-Bus message that appears after "BEGIN\r\n"....)
                 */
                line = _my_g_input_stream_read_line_safe(
                    g_io_stream_get_input_stream(auth->priv->stream),
                    &line_length, cancellable, error);
                if (line == NULL)
                    goto out;
    ...
    if (ret)
    {
        ...
        if (out_received_credentials != NULL)
            *out_received_credentials =
                credentials != NULL ? g_object_ref(credentials) : NULL;
    }
