...
static OM_uint32 val_wrap_args(OM_uint32* minor_status,
                                 ...
                                 gss_buffer_t output_message_buffer)
{
    ...
    status =
        val_wrap_args(minor_status, context_handle, conf_req_flag, qop_req,
    ...
    if (mech)
    {
        if (mech->gss_wrap)
        {
            status = mech->gss_wrap(
                minor_status, ctx->internal_ctx_id, conf_req_flag, qop_req,
                input_message_buffer, conf_state, output_message_buffer);
            if (status != GSS_S_COMPLETE)
        ...
        {
            status = gssint_wrap_aead(mech, minor_status, ctx, conf_req_flag,
                                      (gss_qop_t)qop_req, GSS_C_NO_BUFFER,
 ...
 */
static OM_uint32 gssint_wrap_size_limit_iov_shim(
    ...
    OM_uint32* max_input_size)
{
    ...
    status = mech->gss_wrap_iov_length(minor_status, context_handle,
                                       conf_req_flag, qop_req, NULL, iov,
