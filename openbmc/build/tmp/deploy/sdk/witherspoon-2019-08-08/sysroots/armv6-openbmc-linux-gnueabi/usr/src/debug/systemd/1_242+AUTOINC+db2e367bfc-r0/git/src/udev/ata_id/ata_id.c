...
static int disk_scsi_inquiry_command(int fd, void* buf, size_t buf_len)
{
    ...
    /* even if the ioctl succeeds, we need to check the return value */
    if (!(io_v4.device_status == 0 && io_v4.transport_status == 0 &&
...
static int disk_identify_packet_device_command(int fd, void* buf,
                                               size_t buf_len)
{
...
static void disk_identify_get_string(uint8_t identify[512],
                                     unsigned offset_words, char* dest,
                                     size_t dest_len)
{
...
static void disk_identify_fixup_string(uint8_t identify[512],
                                       unsigned offset_words, size_t len)
{
    disk_identify_get_string(identify, offset_words,
                             (char*)identify + offset_words * 2, len);
}

static void disk_identify_fixup_uint16(uint8_t identify[512],
 ...
 * @out_is_packet_device: Return location for whether returned data is from a
 ...
 */
static int disk_identify(int fd, uint8_t out_identify[512],
                         int* out_is_packet_device)
{
     ...
     * the original bug-fix and see
     * http://bugs.debian.org/cgi-bin/bugreport.cgi?bug=556635 for the original
     * bug-report.)
     */
...
int main(int argc, char* argv[])
{
    ...
    if (disk_identify(fd, identify.byte, &is_packet_device) == 0)
    {
        ...
        disk_identify_fixup_uint16(
            identify.byte, 89); /* time required for SECURITY ERASE UNIT */
        disk_identify_fixup_uint16(
            identify.byte,
    ...
    else
    {
        ...
        {
            log_debug_errno(errno, "HDIO_GET_IDENTITY failed for '%s': %m",
                            node);
            return 2;
        }
    }
    ...
    if (export)
    {
        ...
        if (id.command_set_1 & (1 << 10))
        {
            ...
            /*
             * TODO: use the READ NATIVE MAX ADDRESS command to get the native
             * max address so it is easy to check whether the protected area is
             * in use.
             */
        }
        ...
        if (id.command_set_1 & (1 << 1))
        {
            ...
            if (id.dlf & (1 << 5))
                printf(
                    "ID_ATA_FEATURE_SET_SECURITY_ENHANCED_ERASE_UNIT_MIN=%d\n",
                    id.trsEuc * 2);
            if (id.dlf & (1 << 4))
        ...
        /*
         * Word 76 indicates the capabilities of a SATA device. A PATA device
         * shall set word 76 to 0000h or FFFFh. If word 76 is set to 0000h or
         * FFFFh, then the device does not claim compliance with the Serial ATA
         * specification and words 76 through 79 are not valid and shall be
         * ignored.
         */
        ...
        if (!IN_SET(word, 0x0000, 0xffff))
        {
            ...
            /*
             * If bit 2 of word 76 is set to one, then the device supports the
             * Gen2 signaling rate of 3.0 Gb/s (see SATA 2.6).
             *
             * If bit 1 of word 76 is set to one, then the device supports the
             * Gen1 signaling rate of 1.5 Gb/s (see SATA 2.6).
             */
            if (word & (1 << 2))
        ...
        /*
         * Words 108-111 contain a mandatory World Wide Name (WWN) in the NAA
         * IEEE Registered identifier format. Word 108 bits (15:12) shall
         * contain 5h, indicating that the naming authority is IEEE. All other
         * values are reserved.
         */
