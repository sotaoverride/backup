...
/* An abstract parser for simple, line based, shallow configuration files
 * consisting of variable assignments only. */
...
/* A macro declaring the a function prototype, following the typedef above,
 * simply because it's so cumbersomely long otherwise. (And current emacs gets
 * irritatingly slow when editing files that contain lots of very long function
...
typedef struct ConfigPerfItem
{
    ...
    int ltype; /* Distinguish different variables passed to the same callback */
    size_t
        offset; /* Offset where to store data, from the beginning of userdata */
} ConfigPerfItem;

/* Prototype for a low-level gperf lookup function */
typedef const ConfigPerfItem* (*ConfigPerfItemLookup)(
    const char* section_and_lvalue, unsigned length);
...
#define DEFINE_CONFIG_PARSE(function, parser, msg)                             \
    ...
    {                                                                          \
        ...
        if (r < 0)                                                             \
        {                                                                      \
            log_syntax(unit, LOG_ERR, filename, line, r, msg ", ignoring: %s", \
                       rvalue);                                                \
            return 0;                                                          \
        }                                                                      \
                                                                               \
...
#define DEFINE_CONFIG_PARSE_ENUM(function, name, type, msg)                    \
    ...
    {                                                                          \
        ...
        if (x < 0)                                                             \
        {                                                                      \
            log_syntax(unit, LOG_ERR, filename, line, 0, msg ", ignoring: %s", \
                       rvalue);                                                \
            return 0;                                                          \
        }                                                                      \
                                                                               \
...
#define DEFINE_CONFIG_PARSE_ENUM_WITH_DEFAULT(function, name, type,            \
                                              default_value, msg)              \
    CONFIG_PARSER_PROTOTYPE(function)                                          \
    {                                                                          \
        type *i = data, x;                                                     \
