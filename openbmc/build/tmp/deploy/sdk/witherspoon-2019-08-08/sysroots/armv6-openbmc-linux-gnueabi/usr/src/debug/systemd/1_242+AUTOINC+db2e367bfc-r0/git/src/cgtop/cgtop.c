...
static const char* maybe_format_bytes(char* buf, size_t l, bool is_valid,
                                      uint64_t t)
{
    if (!is_valid)
        return "-";
...
static bool is_root_cgroup(const char* path)
{
    ...
    /* Returns true if the specified path belongs to the root cgroup. The root
     * cgroup is special on cgroup v2 as it carries only very few attributes in
     * order not to export multiple truth about system state as most information
     * is available elsewhere in /proc anyway. We need to be able to deal with
     * that, and need to get our data from different sources in that case.
     *
     * There's one extra complication in all of this, though 😣: if the path to
     * the cgroup indicates we are in the root cgroup this might actually not be
     * the case, because cgroup namespacing might be in effect
     * (CLONE_NEWCGROUP). Since there's no nice way to distuingish a real cgroup
     * root from a fake namespaced one we do an explicit container check here,
     * under the assumption that CLONE_NEWCGROUP is generally used when
     * container managers are used too.
     *
     * Note that checking for a container environment is kinda ugly, since in
     * theory people could use cgtop from inside a container where cgroup
     * namespacing is turned off to watch the host system. However, that's
     * mostly a theoretic usecase, and if people actually try all they'll lose
     * is accounting for the top-level cgroup. Which isn't too bad. */
...
static int process(const char* controller, const char* path, Hashmap* a,
                   Hashmap* b, unsigned iteration, Group** ret)
{
    ...
    if (!g)
    {
        ...
        else
        {
            ...
            g->cpu_valid = g->memory_valid = g->io_valid = g->n_tasks_valid =
                false;
        }
    }
        ...
        IN_SET(arg_count, COUNT_ALL_PROCESSES, COUNT_USERSPACE_PROCESSES))
    {
        ...
        while (cg_read_pid(f, &pid) > 0)
        {
            ...
            if (arg_count == COUNT_USERSPACE_PROCESSES &&
             ...
             (streq(controller, "blkio") && !all_unified))
    {
        ...
        r = cg_get_path(controller, path,
                        all_unified ? "io.stat" : "blkio.io_service_bytes", &p);
        if (r < 0)
            return r;
             ...
             cpu_accounting_is_cheap())
    {
        ...
        else if (all_unified)
        {
            ...
            r = cg_get_keyed_attribute("cpu", path, "cpu.stat",
...
static int refresh_one(const char* controller, const char* path, Hashmap* a,
                       Hashmap* b, unsigned iteration, unsigned depth,
                       Group** ret)
{
...
static int refresh(const char* root, Hashmap* a, Hashmap* b, unsigned iteration)
{
    ...
    FOREACH_STRING(c, SYSTEMD_CGROUP_CONTROLLER, "cpu", "cpuacct", "memory",
                   "io", "blkio", "pids")
    {
...
static int group_compare(Group* const* a, Group* const* b)
{
    ...
    switch (arg_order)
    {
        ...
        case ORDER_IO:
            ...
            {
                r = CMP(y->io_input_bps + y->io_output_bps,
                        x->io_input_bps + x->io_output_bps);
                if (r != 0)
                    return r;
            }
            else if (x->io_valid)
...
static void display(Hashmap* a)
{
    ...
    Group** array;
    signed path_columns;
    unsigned rows, n = 0, j, maxtcpu = 0,
                   maxtpath = 3; /* 3 for ellipsize() to work properly */
    ...
    for (j = 0; j < n; j++)
    {
        ...
        format_timespan(buffer, sizeof(buffer),
                        (usec_t)(array[j]->cpu_usage / NSEC_PER_USEC), 0);
    ...
    if (on_tty())
    {
        ...
        printf(
            ...
            ansi_underline(), arg_order == ORDER_PATH ? on : "", path_columns,
            "Control Group", arg_order == ORDER_PATH ? off : "",
            ...
            arg_order == ORDER_IO ? off : "", arg_order == ORDER_IO ? on : "",
            "Output/s", arg_order == ORDER_IO ? off : "", ansi_normal());
    }
    else
    ...
    for (j = 0; j < n; j++)
    {
        ...
        if (arg_cpu_type == CPU_PERCENT)
        {
            ...
            printf(" %*s", maxtcpu,
                   format_timespan(buffer, sizeof(buffer),
                                   (usec_t)(g->cpu_usage / NSEC_PER_USEC), 0));

        printf(" %8s", maybe_format_bytes(buffer, sizeof(buffer),
                                          g->memory_valid, g->memory));
        printf(" %8s", maybe_format_bytes(buffer, sizeof(buffer), g->io_valid,
                                          g->io_input_bps));
        printf(" %8s", maybe_format_bytes(buffer, sizeof(buffer), g->io_valid,
                                          g->io_output_bps));
...
static int parse_argv(int argc, char* argv[])
{
    ...
    while ((c = getopt_long(argc, argv, "hptcmin:brd:kPM:1", options, NULL)) >=
           0)
        ...
        switch (c)
        {
            ...
            case ARG_DEPTH:
                ...
                if (r < 0)
                    return log_error_errno(
                        r, "Failed to parse depth parameter '%s': %m", optarg);
            ...
            case 'd':
                ...
                if (r < 0)
                    return log_error_errno(
                        r, "Failed to parse delay parameter '%s': %m", optarg);
                if (arg_delay <= 0)
                    return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
            ...
            case 'n':
                ...
                if (r < 0)
                    return log_error_errno(
                        r, "Failed to parse iterations parameter '%s': %m",
                        optarg);
            ...
            case ARG_RECURSIVE:
                ...
                if (r < 0)
                    return log_error_errno(
                        r, "Failed to parse --recursive= argument '%s': %m",
                        optarg);
...
DEFINE_PRIVATE_HASH_OPS_WITH_VALUE_DESTRUCTOR(group_hash_ops, char,
                                              path_hash_func, path_compare_func,
                                              Group, group_free);
...
static int run(int argc, char* argv[])
{
    ...
    arg_count =
        (mask & CGROUP_MASK_PIDS) ? COUNT_PIDS : COUNT_USERSPACE_PROCESSES;

    if (arg_recursive_unset && arg_count == COUNT_PIDS)
        return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                               "Non-recursive counting is only supported when "
                               "counting processes, not tasks. Use -P or -k.");
    ...
    while (!quit)
    {
        ...
        switch (key)
        {
            ...
            case '%':
                arg_cpu_type =
                    arg_cpu_type == CPU_TIME ? CPU_PERCENT : CPU_TIME;
                break;

            case 'k':
                arg_count = arg_count != COUNT_ALL_PROCESSES
                                ? COUNT_ALL_PROCESSES
                                : COUNT_PIDS;
            ...
            case 'P':
                arg_count = arg_count != COUNT_USERSPACE_PROCESSES
                                ? COUNT_USERSPACE_PROCESSES
                                : COUNT_PIDS;
                fprintf(stdout, "\nCounting: %s.", counting_what());
                fflush(stdout);
            ...
            case 'r':
                if (arg_count == COUNT_PIDS)
                    fprintf(stdout, "\n\aCannot toggle recursive counting, not "
                                    "available in task counting mode.");
                else
                {
                    arg_recursive = !arg_recursive;
                    fprintf(stdout, "\nRecursive process counting: %s",
                            yes_no(arg_recursive));
                }
                fflush(stdout);
                sleep(1);
                break;
...
#define OFF ANSI_NORMAL
                ...
                fprintf(stdout,
                        "\t<" ON "p" OFF "> By path; <" ON "t" OFF
                        "> By tasks/procs; <" ON "c" OFF "> By CPU; <" ON
                        "m" OFF "> By memory; <" ON "i" OFF "> By I/O\n"
                        "\t<" ON "+" OFF "> Inc. delay; <" ON "-" OFF
                        "> Dec. delay; <" ON "%%" OFF "> Toggle time; <" ON
                        "SPACE" OFF "> Refresh\n"
                        "\t<" ON "P" OFF
                        "> Toggle count userspace processes; <" ON "k" OFF
                        "> Toggle count all processes\n"
                        "\t<" ON "r" OFF "> Count processes recursively; <" ON
                        "q" OFF "> Quit");
                fflush(stdout);
                sleep(3);
                break;
