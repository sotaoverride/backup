...
static void event_print_custom(unsigned int offset, const struct timespec* ts,
                         ...
                         const struct timespec* timestamp)
{
    ...
    {
        if (ctx->fmt)
            event_print_custom(line_offset, timestamp, event_type, ctx);
        else
...
int main(int argc, char** argv)
{
    ...
    rv = gpiod_ctxless_event_monitor_multiple(
        argv[0], event_type, offsets, num_lines, active_low, "gpiomon",
        &timeout, poll_callback, event_callback, &ctx);
    if (rv)
