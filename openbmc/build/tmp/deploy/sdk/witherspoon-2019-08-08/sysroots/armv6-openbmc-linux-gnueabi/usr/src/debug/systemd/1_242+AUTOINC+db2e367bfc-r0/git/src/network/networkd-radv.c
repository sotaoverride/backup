...
static const char* const
    radv_prefix_delegation_table[_RADV_PREFIX_DELEGATION_MAX] = {
    ...
    void* data, void* userdata)
{
    ...
    {
        log_syntax(
            unit, LOG_ERR, filename, line, -EINVAL,
...
int config_parse_router_preference(const char* unit, const char* filename,
                             ...
                             unsigned section_line, Prefix** ret)
{
    ...
    if (filename)
    {
        ...
        r = hashmap_ensure_allocated(&network->prefixes_by_section,
...
int config_parse_prefix(const char* unit, const char* filename, unsigned line,
...
int config_parse_prefix_flags(const char* unit, const char* filename,
                              unsigned line, const char* section,
                              unsigned section_line, const char* lvalue,
                              int ltype, const char* rvalue, void* data,
                              void* userdata)
...
int config_parse_prefix_lifetime(const char* unit, const char* filename,
                                 unsigned line, const char* section,
                                 unsigned section_line, const char* lvalue,
                                 int ltype, const char* rvalue, void* data,
                                 void* userdata)
{
    ...
    if (streq(lvalue, "PreferredLifetimeSec"))
        r = sd_radv_prefix_set_preferred_lifetime(
            p->radv_prefix, DIV_ROUND_UP(usec, USEC_PER_SEC));
...
static int radv_set_dns(Link* link, Link* uplink)
{
    ...
    if (uplink)
    {
        ...
        {
            log_link_debug(uplink, "Cannot fetch DNS servers as uplink "
                                   "interface is not managed by us");
            return 0;
        }
...
set_dns:
    return sd_radv_set_rdnss(
        link->radv, DIV_ROUND_UP(lifetime_usec, USEC_PER_SEC), dns, n_dns);
}
...
static int radv_set_domains(Link* link, Link* uplink)
{
    ...
    _cleanup_free_ char** s =
        NULL; /* just free() because the strings are owned by the set */
    ...
    if (uplink)
    {
        if (uplink->network == NULL)
        {
            log_link_debug(uplink, "Cannot fetch DNS search domains as uplink "
                                   "interface is not managed by us");
            return 0;
        }
...
set_domains:
    ...
    return sd_radv_set_dnssl(link->radv,
                             DIV_ROUND_UP(lifetime_usec, USEC_PER_SEC), s);
}
...
int radv_configure(Link* link)
{
    ...
    r = sd_radv_set_managed_information(link->radv,
                                        link->network->router_managed);
    if (r < 0)
        return r;

    r = sd_radv_set_other_information(link->radv,
    ...
    if (IN_SET(link->network->router_prefix_delegation,
               RADV_PREFIX_DELEGATION_STATIC, RADV_PREFIX_DELEGATION_BOTH))
    {
        ...
        LIST_FOREACH(prefixes, p, link->network->static_prefixes)
        {
            ...
            if (r == -ENOEXEC)
            {
                log_link_warning_errno(
                    link, r,
                    "[IPv6Prefix] section configured without Prefix= setting, "
                    "ignoring section.");
                continue;
            }
            if (r < 0)
