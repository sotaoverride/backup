 ...
 * - conditional/filtering/deletion on binding (using '(w)'/'(s)'/'(=)'
 * notation)
 */
...
#define FCMATCH_FORMAT                                                         \
    "%{file:-<unknown filename>|basename}: \"%{family[0]:-<unknown "           \
    "family>}\" \"%{style[0]:-<unknown style>}\""
#define FCLIST_FORMAT "%{?file{%{file}: }}%{-file{%{=unparse}}}"
#define PKGKIT_FORMAT                                                          \
    "%{[]family{font(%{family|downcase|delete( "                               \
...
static FcBool FcFormatContextInit(FcFormatContext* c, const FcChar8* format,
                                  FcChar8* scratch, int scratch_len)
{
...
static void FcFormatContextDone(FcFormatContext* c)
{
...
static FcBool consume_char(FcFormatContext* c, FcChar8 term)
{
    if (*c->format != term)
        return FcFalse;
...
static FcBool expect_char(FcFormatContext* c, FcChar8 term)
{
    FcBool res = consume_char(c, term);
    if (!res)
    {
        if (c->format == c->format_orig + c->format_len)
            message("format ended while expecting '%c'", term);
        else
            message("expected '%c' at %d", term,
...
static FcBool read_word(FcFormatContext* c)
{
    FcChar8* p;
    ...
    *p = '\0';
    ...
    if (p == c->word)
    {
        message("expected identifier at %d", c->format - c->format_orig + 1);
        return FcFalse;
    }
...
static FcBool read_chars(FcFormatContext* c, FcChar8 term)
{
    FcChar8* p;
...
static FcBool FcPatternFormatToBuf(FcPattern* pat, const FcChar8* format,
                                   FcStrBuf* buf);

static FcBool interpret_builtin(FcFormatContext* c, FcPattern* pat,
                                FcStrBuf* buf)
{
    FcChar8* new_str;
    FcBool ret;

    if (!expect_char(c, '=') || !read_word(c))
        return FcFalse;
...
#define BUILTIN(name, format)                                                  \
    else if (0 == strcmp((const char*)c->word, name)) ret =                    \
        FcPatternFormatToBuf(pat, (const FcChar8*)format, buf)
...
#undef BUILTIN
    ...
    if (!ret)
        message("unknown builtin \"%s\"", c->word);

    return ret;
}

static FcBool interpret_expr(FcFormatContext* c, FcPattern* pat, FcStrBuf* buf,
                             FcChar8 term);

static FcBool interpret_subexpr(FcFormatContext* c, FcPattern* pat,
                                FcStrBuf* buf)
{
    return expect_char(c, '{') && interpret_expr(c, pat, buf, '}') &&
           expect_char(c, '}');
}

static FcBool maybe_interpret_subexpr(FcFormatContext* c, FcPattern* pat,
                                      FcStrBuf* buf)
{
    return (*c->format == '{') ? interpret_subexpr(c, pat, buf) : FcTrue;
}

static FcBool skip_subexpr(FcFormatContext* c);

static FcBool skip_percent(FcFormatContext* c)
{
    if (!expect_char(c, '%'))
        return FcFalse;
...
static FcBool skip_expr(FcFormatContext* c)
{
    while (*c->format && *c->format != '}')
    {
        switch (*c->format)
...
static FcBool skip_subexpr(FcFormatContext* c)
{
    return expect_char(c, '{') && skip_expr(c) && expect_char(c, '}');
}

static FcBool maybe_skip_subexpr(FcFormatContext* c)
{
    return (*c->format == '{') ? skip_subexpr(c) : FcTrue;
}

static FcBool interpret_filter_in(FcFormatContext* c, FcPattern* pat,
                                  FcStrBuf* buf)
{
    FcObjectSet* os;
    FcPattern* subpat;
    ...
    {
        /* XXX binding */
        if (!read_word(c) || !FcObjectSetAdd(os, (const char*)c->word))
        {
    ...
    if (!subpat || !interpret_subexpr(c, subpat, buf))
        return FcFalse;
...
static FcBool interpret_filter_out(FcFormatContext* c, FcPattern* pat,
                                   FcStrBuf* buf)
{
    FcPattern* subpat;
...
static FcBool interpret_cond(FcFormatContext* c, FcPattern* pat, FcStrBuf* buf)
{
    FcBool pass;
    ...
    do
    {
        ...
        pass =
            pass && (negate ^ (FcResultMatch ==
                               FcPatternGet(pat, (const char*)c->word, 0, &v)));
    } while (consume_char(c, ','));

    if (pass)
    {
        if (!interpret_subexpr(c, pat, buf) || !maybe_skip_subexpr(c))
            return FcFalse;
    }
    else
    {
        if (!skip_subexpr(c) || !maybe_interpret_subexpr(c, pat, buf))
            return FcFalse;
    }
...
static FcBool interpret_count(FcFormatContext* c, FcPattern* pat, FcStrBuf* buf)
{
    int count;
    FcPatternIter iter;
...
static FcBool interpret_enumerate(FcFormatContext* c, FcPattern* pat,
                                  FcStrBuf* buf)
{
    FcObjectSet* os;
    FcPattern* subpat;
    ...
    if (!expect_char(c, '[') || !expect_char(c, ']'))
        return FcFalse;
    ...
    do
    {
        ...
        else
        {
            for (i = 0; i < os->nobject; i++)
            {
                 ...
                 * directly and remembering where we were.  Most (all) value
                 * lists in normal uses are pretty short though (language tags
                 * are stored as a LangSet, not separate values.). */
                FcPatternDel(subpat, os->objects[i]);
                if (FcResultMatch == FcPatternGet(pat, os->objects[i], idx, &v))
                {
                    /* XXX binding */
...
static FcBool interpret_simple(FcFormatContext* c, FcPattern* pat,
                               FcStrBuf* buf)
{
    FcPatternIter iter;
...
static FcBool cescape(FcFormatContext* c FC_UNUSED, const FcChar8* str,
                      FcStrBuf* buf)
{
    /* XXX escape \n etc? */
...
static FcBool shescape(FcFormatContext* c FC_UNUSED, const FcChar8* str,
                       FcStrBuf* buf)
{
    FcStrBufChar(buf, '\'');
    while (*str)
...
static FcBool xmlescape(FcFormatContext* c FC_UNUSED, const FcChar8* str,
                        FcStrBuf* buf)
{
    /* XXX escape \n etc? */
...
static FcBool delete_chars(FcFormatContext* c, const FcChar8* str,
                           FcStrBuf* buf)
{
    /* XXX not UTF-8 aware */

    if (!expect_char(c, '(') || !read_chars(c, ')') || !expect_char(c, ')'))
        return FcFalse;
...
static FcBool escape_chars(FcFormatContext* c, const FcChar8* str,
                           FcStrBuf* buf)
{
    /* XXX not UTF-8 aware */
...
static FcBool translate_chars(FcFormatContext* c, const FcChar8* str,
                              FcStrBuf* buf)
{
    char *from, *to, repeat;
    int from_len, to_len;
...
static FcBool interpret_convert(FcFormatContext* c, FcStrBuf* buf, int start)
{
    const FcChar8* str;
    FcChar8* new_str;
    FcStrBuf new_buf;
...
#undef CONVERTER
    ...
    if (ret)
    {
        ...
        message("unknown converter \"%s\"", c->word);
...
static FcBool maybe_interpret_converts(FcFormatContext* c, FcStrBuf* buf,
                                       int start)
{
    while (*c->format == '|')
...
static FcBool align_to_width(FcStrBuf* buf, int start, int width)
{
    int len;
    ...
    else if (len < width)
    {
        ...
        if (buf->failed)
            return FcFalse;
        len = old_len;
        memmove(buf->buf + buf->len - len, buf->buf + buf->len - width, len);
        memset(buf->buf + buf->len - width, ' ', width - len);
    }

    return !buf->failed;
}
static FcBool interpret_percent(FcFormatContext* c, FcPattern* pat,
                                FcStrBuf* buf)
{
    int width, start;
    FcBool ret;
    ...
    {
        case '=':
            ret = interpret_builtin(c, pat, buf);
            break;
        case '{':
            ret = interpret_subexpr(c, pat, buf);
            break;
        case '+':
            ret = interpret_filter_in(c, pat, buf);
            break;
        case '-':
            ret = interpret_filter_out(c, pat, buf);
            break;
        case '?':
            ret = interpret_cond(c, pat, buf);
            break;
        case '#':
            ret = interpret_count(c, pat, buf);
            break;
        case '[':
            ret = interpret_enumerate(c, pat, buf);
            break;
        default:
            ret = interpret_simple(c, pat, buf);
            break;
    }

    return ret && maybe_interpret_converts(c, buf, start) &&
           align_to_width(buf, start, width) && expect_char(c, '}');
}
