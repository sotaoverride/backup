...
#define alt_month_name (&_nl_C_LC_TIME.values[_NL_ITEM_INDEX(ALTMON_1)].string)
...
static char const weekday_name[][10] = {"Sunday",    "Monday",   "Tuesday",
                                        ...
                                        "Saturday"};
static char const ab_weekday_name[][4] = {"Sun", "Mon", "Tue", "Wed",
                                          "Thu", "Fri", "Sat"};
static char const month_name[][10] = {
    "January", "February", "March",     "April",   "May",      "June",
    "July",    "August",   "September", "October", "November", "December"};
static char const ab_month_name[][4] = {"Jan", "Feb", "Mar", "Apr",
                                        "May", "Jun", "Jul", "Aug",
                                        "Sep", "Oct", "Nov", "Dec"};
		     ...
		     void *statep LOCALE_PARAM)
{
        ...
        switch (*fmt++)
        {
            ...
            case 'A':
                ...
                for (cnt = 0; cnt < 7; ++cnt)
                {
                    ...
                    {
                        trp = rp;
                        if (match_string(_NL_CURRENT(LC_TIME, DAY_1 + cnt),
                                         trp) &&
                            trp > rp_longest)
                        {
                            rp_longest = trp;
                            cnt_longest = cnt;
                            if (s.decided ==
                                not&&strcmp(_NL_CURRENT(LC_TIME, DAY_1 + cnt),
                                            weekday_name[cnt]))
                                decided_longest = loc;
                        }
                        trp = rp;
                        if (match_string(_NL_CURRENT(LC_TIME, ABDAY_1 + cnt),
                                         trp) &&
                            trp > rp_longest)
                        {
                            rp_longest = trp;
                            cnt_longest = cnt;
                            if (s.decided ==
                                not&&strcmp(_NL_CURRENT(LC_TIME, ABDAY_1 + cnt),
                                            ab_weekday_name[cnt]))
                                decided_longest = loc;
                        }
                    }
            ...
            case 'h':
                ...
                for (cnt = 0; cnt < 12; ++cnt)
                {
                    ...
                    {
                        trp = rp;
                        if (match_string(_NL_CURRENT(LC_TIME, MON_1 + cnt),
                                         trp) &&
                            trp > rp_longest)
                        {
                            rp_longest = trp;
                            cnt_longest = cnt;
                            if (s.decided ==
                                not&&strcmp(_NL_CURRENT(LC_TIME, MON_1 + cnt),
                                            month_name[cnt]))
                                decided_longest = loc;
                        }
                        trp = rp;
                        if (match_string(_NL_CURRENT(LC_TIME, ABMON_1 + cnt),
                                         trp) &&
                            trp > rp_longest)
                        {
                            rp_longest = trp;
                            cnt_longest = cnt;
                            if (s.decided ==
                                not&&strcmp(_NL_CURRENT(LC_TIME, ABMON_1 + cnt),
                                            ab_month_name[cnt]))
                                decided_longest = loc;
                        }
#ifdef _LIBC
                        ...
                        trp = rp;
                        if (match_string(_NL_CURRENT(LC_TIME, ALTMON_1 + cnt),
                                         trp) &&
                            trp > rp_longest)
                        {
                            rp_longest = trp;
                        ...
                        trp = rp;
                        if (match_string(
                                _NL_CURRENT(LC_TIME, _NL_ABALTMON_1 + cnt),
                                trp) &&
                            trp > rp_longest)
                        {
                            rp_longest = trp;
                    ...
                    if (s.decided != loc &&
                        (((trp = rp, match_string(month_name[cnt], trp)) &&
                         ...
                         ((trp = rp, match_string(alt_month_name[cnt], trp)) &&
                          trp > rp_longest) ||
                         ((trp = rp,
                           match_string(ab_alt_month_name[cnt], trp)) &&
                          trp > rp_longest)
#endif
                             ))
                    {
            ...
            case 'c':
                ...
                if (s.decided != raw)
                {
                    ...
                    {
                        if (s.decided ==
                            not&&strcmp(_NL_CURRENT(LC_TIME, D_T_FMT),
                                        HERE_D_T_FMT))
                            s.decided = loc;
                        s.want_xday = 1;
                        break;
            ...
            case 'x':
                ...
                if (s.decided != raw)
                {
                    ...
                    {
                        if (s.decided ==
                            not&&strcmp(_NL_CURRENT(LC_TIME, D_FMT),
                                        HERE_D_FMT))
                            s.decided = loc;
                        s.want_xday = 1;
                        break;
            ...
            case 'r':
                ...
                if (s.decided != raw)
                {
                    ...
                    {
                        if (s.decided ==
                            not&&strcmp(_NL_CURRENT(LC_TIME, T_FMT_AMPM),
                                        HERE_T_FMT_AMPM))
                            s.decided = loc;
                        break;
                    }
                ...
                switch (*fmt++)
                {
                    ...
                    case 'C':
                        if (s.decided != raw)
                        {
                            if (s.era_cnt >= 0)
                            {
                                era = _nl_select_era_entry(
                                    s.era_cnt HELPER_LOCALE_ARG);
                                if (era != NULL &&
                                    match_string(era->era_name, rp))
                                {
                                    s.decided = loc;
                                    break;
                                }
                            ...
                            num_eras = _NL_CURRENT_WORD(
                                LC_TIME, _NL_TIME_ERA_NUM_ENTRIES);
                ...
                switch (*fmt++)
                {
                    ...
                    case 'V':
                       ...
                       further information.  */
                        break;
                    case 'w':
                        /* Match number of weekday using alternate numeric
                         * symbols.  */
                        ...
                        break;
                    case 'y':
                        /* Match year within century using alternate numeric
                         * symbols.  */
    ...
    if (s.era_cnt != -1)
    {
        ...
        if (era == NULL)
            return NULL;
        if (s.want_era)
            tm->tm_year = (era->start_date[0] + ((tm->tm_year - era->offset) *
                                                 era->absolute_direction));
        else
    ...
    if (s.want_xday && !s.have_yday &&
        (s.have_mon || (unsigned)tm->tm_mon <= 11))
        day_of_the_year(tm);
    ...
    if ((s.have_uweek || s.have_wweek) && s.have_wday)
    {
        ...
        if (!s.have_yday)
            tm->tm_yday =
                ((7 - (tm->tm_wday - w_offset)) % 7 + (s.week_no - 1) * 7 +
