...
void _gcry_initialize_fips_mode(int force)
{
    ...
    /* Checking based on /proc file properties.  */
    {
        ...
        else if ((saved_errno = errno) != ENOENT && saved_errno != EACCES &&
...
static const char* state2str(enum module_states state)
{
    const char* s;
...
static void reporter(const char* domain, int algo, const char* what,
                     const char* errtxt)
{
    ...
    log_info("libgcrypt selftest: %s %s%s (%d): %s%s%s%s\n",
             ...
             !strcmp(domain, "cipher")
                 ? _gcry_cipher_algo_name(algo)
                 : !strcmp(domain, "digest")
                       ? _gcry_md_algo_name(algo)
                       : !strcmp(domain, "hmac")
                             ? _gcry_md_algo_name(algo)
                             : !strcmp(domain, "pubkey")
                                   ? _gcry_pk_algo_name(algo)
                                   : "",
             algo, errtxt ? errtxt : "Okay", what ? " (" : "", what ? what : "",
             what ? ")" : "");
}

/* Run self-tests for all required cipher algorithms.  Return 0 on
   success. */
static int run_cipher_selftests(int extended)
{
    static int algos[] = {GCRY_CIPHER_3DES, GCRY_CIPHER_AES128,
                          GCRY_CIPHER_AES192, GCRY_CIPHER_AES256, 0};
    int idx;
    gpg_error_t err;
    int anyerr = 0;
...
/* Run self-tests for all required hash algorithms.  Return 0 on
   success. */
static int run_digest_selftests(int extended)
{
    static int algos[] = {GCRY_MD_SHA1,   GCRY_MD_SHA224, GCRY_MD_SHA256,
                          GCRY_MD_SHA384, GCRY_MD_SHA512, 0};
    int idx;
    gpg_error_t err;
    int anyerr = 0;
...
static int run_hmac_selftests(int extended)
{
    static int algos[] = {GCRY_MD_SHA1,     GCRY_MD_SHA224,
                          GCRY_MD_SHA256,   GCRY_MD_SHA384,
                          GCRY_MD_SHA512,   GCRY_MD_SHA3_224,
                          GCRY_MD_SHA3_256, GCRY_MD_SHA3_384,
                          GCRY_MD_SHA3_512, 0};
    int idx;
    gpg_error_t err;
    int anyerr = 0;
...
/* Run self-tests for all required public key algorithms.  Return 0 on
   success. */
static int run_pubkey_selftests(int extended)
{
    static int algos[] = {GCRY_PK_RSA, GCRY_PK_DSA, GCRY_PK_ECC, 0};
    int idx;
    gpg_error_t err;
    int anyerr = 0;
...
static int check_binary_integrity(void)
{
    ...
    {
        dlen = _gcry_hmac256_file(digest, sizeof digest, info.dli_fname, key,
                                  strlen(key));
        if (dlen < 0)
            err = gpg_error_from_syserror();
        else if (dlen != 32)
                             ...
                             const char* description)
{
    ...
    log_info("%serror in libgcrypt, file %s, line %d%s%s: %s\n",
             is_fatal ? "fatal " : "", srcfile, srcline,
...
static void fips_new_state(enum module_states new_state)
{
    ...
    switch (current_state)
    {
        ...
        case STATE_INIT:
            if (new_state == STATE_SELFTEST || new_state == STATE_ERROR ||
                new_state == STATE_FATALERROR)
                ok = 1;
            break;

        case STATE_SELFTEST:
            if (new_state == STATE_OPERATIONAL || new_state == STATE_ERROR ||
                new_state == STATE_FATALERROR)
                ok = 1;
            break;

        case STATE_OPERATIONAL:
            if (new_state == STATE_SHUTDOWN || new_state == STATE_SELFTEST ||
                new_state == STATE_ERROR || new_state == STATE_FATALERROR)
                ok = 1;
            break;

        case STATE_ERROR:
            if (new_state == STATE_SHUTDOWN || new_state == STATE_ERROR ||
