...
#define G_RESOURCE_FILE_ENUMERATOR(o)                                          \
    (G_TYPE_CHECK_INSTANCE_CAST((o), G_TYPE_RESOURCE_FILE_ENUMERATOR,          \
                                GResourceFileEnumerator))
#define G_RESOURCE_FILE_ENUMERATOR_CLASS(k)                                    \
    (G_TYPE_CHECK_CLASS_CAST((k), G_TYPE_RESOURCE_FILE_ENUMERATOR,             \
                             GResourceFileEnumeratorClass))
#define G_IS_RESOURCE_FILE_ENUMERATOR(o)                                       \
    (G_TYPE_CHECK_INSTANCE_TYPE((o), G_TYPE_RESOURCE_FILE_ENUMERATOR))
#define G_IS_RESOURCE_FILE_ENUMERATOR_CLASS(k)                                 \
    (G_TYPE_CHECK_CLASS_TYPE((k), G_TYPE_RESOURCE_FILE_ENUMERATOR))
#define G_RESOURCE_FILE_ENUMERATOR_GET_CLASS(o)                                \
    (G_TYPE_INSTANCE_GET_CLASS((o), G_TYPE_RESOURCE_FILE_ENUMERATOR,           \
                               GResourceFileEnumeratorClass))

#define G_TYPE_RESOURCE_FILE_INPUT_STREAM                                      \
    (_g_resource_file_input_stream_get_type())
#define G_RESOURCE_FILE_INPUT_STREAM(o)                                        \
    (G_TYPE_CHECK_INSTANCE_CAST((o), G_TYPE_RESOURCE_FILE_INPUT_STREAM,        \
                                GResourceFileInputStream))
#define G_RESOURCE_FILE_INPUT_STREAM_CLASS(k)                                  \
    (G_TYPE_CHECK_CLASS_CAST((k), G_TYPE_RESOURCE_FILE_INPUT_STREAM,           \
                             GResourceFileInputStreamClass))
#define G_IS_RESOURCE_FILE_INPUT_STREAM(o)                                     \
    (G_TYPE_CHECK_INSTANCE_TYPE((o), G_TYPE_RESOURCE_FILE_INPUT_STREAM))
#define G_IS_RESOURCE_FILE_INPUT_STREAM_CLASS(k)                               \
    (G_TYPE_CHECK_CLASS_TYPE((k), G_TYPE_RESOURCE_FILE_INPUT_STREAM))
#define G_RESOURCE_FILE_INPUT_STREAM_GET_CLASS(o)                              \
    (G_TYPE_INSTANCE_GET_CLASS((o), G_TYPE_RESOURCE_FILE_INPUT_STREAM,         \
                               GResourceFileInputStreamClass))
...
G_DEFINE_TYPE(GResourceFileEnumerator, g_resource_file_enumerator,
              G_TYPE_FILE_ENUMERATOR)

static GFileEnumerator*
    _g_resource_file_enumerator_new(GResourceFile* file, const char* attributes,
                                    GFileQueryInfoFlags flags,
                                    GCancellable* cancellable, GError** error);

static GType _g_resource_file_input_stream_get_type(void) G_GNUC_CONST;

static GFileInputStream* _g_resource_file_input_stream_new(GInputStream* stream,
                                                           GFile* file);

static void g_resource_file_finalize(GObject* object)
{
...
static void g_resource_file_class_init(GResourceFileClass* klass)
{
...
static void g_resource_file_init(GResourceFile* resource)
{
}

static inline gchar* scan_backwards(const gchar* begin, const gchar* end,
                                    gchar c)
{
    while (end >= begin)
    {
...
static inline void pop_to_previous_part(const gchar* begin, gchar** out)
{
    if (*out > begin)
 ...
 */
static char* canonicalize_filename(const char* in)
{
    gchar* bptr;
    char* out;
...
static GFile* g_resource_file_new_for_path(const char* path)
{
...
static gboolean g_resource_file_is_native(GFile* file)
{
    return FALSE;
}

static gboolean g_resource_file_has_uri_scheme(GFile* file,
                                               const char* uri_scheme)
{
    return g_ascii_strcasecmp(uri_scheme, "resource") == 0;
}

static char* g_resource_file_get_uri_scheme(GFile* file)
{
    return g_strdup("resource");
}

static char* g_resource_file_get_basename(GFile* file)
{
    gchar* base;
...
static char* g_resource_file_get_path(GFile* file)
{
    return NULL;
}

static char* g_resource_file_get_uri(GFile* file)
{
    char *escaped, *res;
    escaped = g_uri_escape_string(G_RESOURCE_FILE(file)->path,
...
static char* g_resource_file_get_parse_name(GFile* file)
{
    return g_resource_file_get_uri(file);
}

static GFile* g_resource_file_get_parent(GFile* file)
{
    ...
    parent->path = g_strndup(resource->path, end - resource->path);

    return G_FILE(parent);
}

static GFile* g_resource_file_dup(GFile* file)
{
    GResourceFile* resource = G_RESOURCE_FILE(file);
...
static guint g_resource_file_hash(GFile* file)
{
    GResourceFile* resource = G_RESOURCE_FILE(file);
...
static gboolean g_resource_file_equal(GFile* file1, GFile* file2)
{
...
static gboolean g_resource_file_prefix_matches(GFile* parent, GFile* descendant)
{
...
static char* g_resource_file_get_relative_path(GFile* parent, GFile* descendant)
{
    GResourceFile* parent_resource = G_RESOURCE_FILE(parent);
    GResourceFile* descendant_resource = G_RESOURCE_FILE(descendant);
    const char* remainder;
...
static GFile* g_resource_file_resolve_relative_path(GFile* file,
...
static GFileEnumerator* g_resource_file_enumerate_children(
    GFile* file, const char* attributes, GFileQueryInfoFlags flags,
    GCancellable* cancellable, GError** error)
{
    GResourceFile* resource = G_RESOURCE_FILE(file);
    return _g_resource_file_enumerator_new(resource, attributes, flags,
                                           cancellable, error);
}

static GFile* g_resource_file_get_child_for_display_name(
...
static GFileInfo* g_resource_file_query_info(GFile* file,
                                             ...
                                             GError** error)
{
    ...
    if (!is_dir)
    {
        res = g_resources_get_info(resource->path, 0, &size, &resource_flags,
                                   &my_error);
        if (!res)
        {
            if (g_error_matches(my_error, G_RESOURCE_ERROR,
    ...
    _g_file_info_set_attribute_boolean_by_id(
        info, G_FILE_ATTRIBUTE_ID_ACCESS_CAN_READ, TRUE);
    _g_file_info_set_attribute_boolean_by_id(
        info, G_FILE_ATTRIBUTE_ID_ACCESS_CAN_WRITE, FALSE);
    _g_file_info_set_attribute_boolean_by_id(
        info, G_FILE_ATTRIBUTE_ID_ACCESS_CAN_EXECUTE, FALSE);
    _g_file_info_set_attribute_boolean_by_id(
        info, G_FILE_ATTRIBUTE_ID_ACCESS_CAN_RENAME, FALSE);
    _g_file_info_set_attribute_boolean_by_id(
        info, G_FILE_ATTRIBUTE_ID_ACCESS_CAN_DELETE, FALSE);
    _g_file_info_set_attribute_boolean_by_id(
        info, G_FILE_ATTRIBUTE_ID_ACCESS_CAN_TRASH, FALSE);
    ...
    else
    {
        ...
        if ((_g_file_attribute_matcher_matches_id(
                 matcher, G_FILE_ATTRIBUTE_ID_STANDARD_CONTENT_TYPE) ||
             ((~resource_flags & G_RESOURCE_FLAGS_COMPRESSED) &&
              _g_file_attribute_matcher_matches_id(
                  matcher, G_FILE_ATTRIBUTE_ID_STANDARD_FAST_CONTENT_TYPE))) &&
        ...
        if (content_type)
        {
            _g_file_info_set_attribute_string_by_id(
                info, G_FILE_ATTRIBUTE_ID_STANDARD_CONTENT_TYPE, content_type);
            _g_file_info_set_attribute_string_by_id(
...
static GFileInfo*
    g_resource_file_query_filesystem_info(GFile* file, const char* attributes,
                                          GCancellable* cancellable,
                                          GError** error)
{
    GFileInfo* info;
...
static GFileAttributeInfoList* g_resource_file_query_settable_attributes(
...
static GFileInputStream*
    g_resource_file_read(GFile* file, GCancellable* cancellable, GError** error)
{
    GResourceFile* resource = G_RESOURCE_FILE(file);
    GError* my_error = NULL;
    GInputStream* stream;
...
G_DEFINE_TYPE(GResourceFileMonitor, g_resource_file_monitor,
              G_TYPE_FILE_MONITOR)

static gboolean g_resource_file_monitor_cancel(GFileMonitor* monitor)
{
    return TRUE;
}

static void g_resource_file_monitor_init(GResourceFileMonitor* monitor)
{
}

static void g_resource_file_monitor_class_init(GResourceFileMonitorClass* class)
{
    class->cancel = g_resource_file_monitor_cancel;
}

static GFileMonitor* g_resource_file_monitor_file(GFile* file,
...
static void g_resource_file_file_iface_init(GFileIface* iface)
{
    ...
    iface->query_settable_attributes =
        g_resource_file_query_settable_attributes;
    iface->query_writable_namespaces =
...
static GFileInfo* g_resource_file_enumerator_next_file(
...
static void g_resource_file_enumerator_finalize(GObject* object)
{
...
static void g_resource_file_enumerator_init(GResourceFileEnumerator* resource)
{
}
                                    ...
                                    GCancellable* cancellable, GError** error)
{
    ...
    if (children == NULL && strcmp("/", file->path) != 0)
    {
        ...
        if (res)
            ...
            g_set_error(error, G_IO_ERROR, G_IO_ERROR_NOT_FOUND,
                        _("The resource at “%s” does not exist"), file->path);
        return NULL;
    }

    resource =
        g_object_new(G_TYPE_RESOURCE_FILE_ENUMERATOR, "container", file, NULL);
    ...
    GFileEnumerator* enumerator, GCancellable* cancellable, GError** error)
{
    ...
    path = g_build_path("/", resource->path,
                        resource->children[resource->index++], NULL);
    file = g_resource_file_new_for_path(path);
    g_free(path);

    info = g_file_query_info(file, resource->attributes, resource->flags,
                             cancellable, error);
...
#define g_resource_file_input_stream_get_type                                  \
    _g_resource_file_input_stream_get_type
G_DEFINE_TYPE(GResourceFileInputStream, g_resource_file_input_stream,
...
static gboolean g_resource_file_input_stream_seek(GFileInputStream* stream,
                                                  ...
                                                  GCancellable* cancellable,
                                                  GError** error);
static GFileInfo* g_resource_file_input_stream_query_info(
    GFileInputStream* stream, const char* attributes, GCancellable* cancellable,
    GError** error);

static void g_resource_file_input_stream_finalize(GObject* object)
{
...
static void g_resource_file_input_stream_class_init(
    GResourceFileInputStreamClass* klass)
{
    GObjectClass* gobject_class = G_OBJECT_CLASS(klass);
...
static void g_resource_file_input_stream_init(GResourceFileInputStream* info)
{
}
...
static gboolean g_resource_file_input_stream_can_seek(GFileInputStream* stream)
{
    ...
    return G_IS_SEEKABLE(file->stream) &&
           g_seekable_can_seek(G_SEEKABLE(file->stream));
}
                                                  ...
                                                  GError** error)
{
    ...
    return g_seekable_seek(G_SEEKABLE(file->stream), offset, type, cancellable,
                           error);
}
