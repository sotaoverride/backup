...
static BOOL copy_valid_path(const char* fname)
{
    ...
    if (stringjoin(rel, backup_dir_remainder, fname, backup_suffix, NULL) >=
        ...
        *b = '\0';
        ...
        while (do_mkdir(backup_dir_buf, ACCESSPERMS) < 0)
        {
            if (errno == EEXIST)
            {
                ...
                rsyserr(FERROR, errno, "backup mkdir %s failed",
                        backup_dir_buf);
            *name = '\0';
            ret = False;
            goto cleanup;
...
char* get_backup_name(const char* fname)
{
    ...
    if (stringjoin(backup_dir_buf, MAXPATHLEN, fname, backup_suffix, NULL) <
        MAXPATHLEN)
        return backup_dir_buf;
...
int make_backup(const char* fname, BOOL prefer_rename)
{
    ...
    if (errno == EEXIST || errno == EISDIR)
    {
        ...
        {
            int flags =
                get_del_for_flag(bakst.st_mode) | DEL_FOR_BACKUP | DEL_RECURSE;
    ...
    if (!ret && preserve_links && S_ISLNK(file->mode))
    {
        ...
        else
        {
            if (do_symlink(sl, buf) < 0)
                rsyserr(FERROR, errno, "link %s -> \"%s\"", full_fname(buf),
                        sl);
            else if (DEBUG_GTE(BACKUP, 1))
