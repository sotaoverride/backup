...
long auto_nlist_value(const char* string)
{
...
int auto_nlist(const char* string, char* var, size_t size)
{
    long result;
    int ret;
...
static void init_nlist(struct nlist nl[])
{
    int ret;
    ...
    if ((kernel = kvm_openfiles(KERNEL_LOC, NULL, NULL, O_RDONLY,
                                kvm_errbuf)) == NULL)
    {
        if (netsnmp_ds_get_boolean(NETSNMP_DS_APPLICATION_ID,
                                   NETSNMP_DS_AGENT_NO_ROOT_ACCESS))
    ...
    for (ret = 0; nl[ret].n_name != NULL; ret++)
    {
        ...
        if (nl[ret].n_type == 0)
        {
            if (!netsnmp_ds_get_boolean(NETSNMP_DS_APPLICATION_ID,
                                        NETSNMP_DS_AGENT_NO_ROOT_ACCESS))
            {
                DEBUGMSGTL(("auto_nlist:init_nlist",
                            "nlist err:  %s not found\n", nl[ret].n_name));
            }
        }
        else
        {
            DEBUGMSGTL(("auto_nlist:init_nlist", "nlist: %s 0x%X\n",
                        nl[ret].n_name, (unsigned int)nl[ret].n_value));
        }
    }
}

int KNLookup(struct nlist nl[], int nl_which, char* buf, size_t s)
{
...
#ifdef TESTING
void auto_nlist_print_tree(int indent, struct autonlist* ptr)
{
    char buf[1024];
    if (indent == -2)
    {
