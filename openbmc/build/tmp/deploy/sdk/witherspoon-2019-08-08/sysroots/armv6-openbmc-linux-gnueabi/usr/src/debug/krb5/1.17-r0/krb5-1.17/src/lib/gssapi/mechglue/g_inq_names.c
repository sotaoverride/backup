...
OM_uint32 KRB5_CALLCONV gss_inquire_names_for_mech(minor_status, mechanism,
                                                   name_types)
...

{
    ...
    status = gssint_select_mech_type(minor_status, mechanism, &selected_mech);
    ...
    status =
        mech->gss_inquire_names_for_mech(minor_status, public_mech, name_types);
    if (status != GSS_S_COMPLETE)
...
static OM_uint32 val_inq_mechs4name_args(OM_uint32* minor_status,
...
static int mech_supports_nametype(gss_OID mech_oid, gss_OID name_type)
{
...
OM_uint32 KRB5_CALLCONV gss_inquire_mechs_for_name(OM_uint32* minor_status,
                                                   const gss_name_t input_name,
                                                   gss_OID_set* mech_set)
{
    ...
    status =
        gss_display_name(minor_status, input_name, &name_buffer, &name_type);
    if (status != GSS_S_COMPLETE)
        goto cleanup;
