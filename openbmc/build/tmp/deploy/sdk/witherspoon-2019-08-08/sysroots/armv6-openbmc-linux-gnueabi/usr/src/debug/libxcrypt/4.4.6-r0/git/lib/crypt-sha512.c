...
/* Subroutine of _xcrypt_crypt_sha512crypt_rn: Feed CTX with LEN bytes of a
   ...
   indefinitely.  */
static void sha512_process_recycled_bytes(unsigned char block[64], size_t len,
                                          SHA512_CTX* ctx)
{
    size_t cnt;
                          ...
                          size_t scr_size)
{
        ...
        0)
    {
        ...
        if (endp == num || *endp != '$' || rounds < ROUNDS_MIN ||
            rounds > ROUNDS_MAX || errno)
        {
            errno = EINVAL;
            return;
        }
    ...
    {
        int n =
            snprintf(cp, SHA512_HASH_LENGTH - (sizeof(sha512_salt_prefix) - 1),
...
void gensalt_sha512crypt_rn(unsigned long count, const uint8_t* rbytes,
