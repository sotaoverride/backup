...
#define NL_CAPABILITY_ROUTE_BUILD_MSG_SET_SCOPE                                \
...
#define NL_CAPABILITY_ROUTE_LINK_VETH_GET_PEER_OWN_REFERENCE                   \
    NL_CAPABILITY_ROUTE_LINK_VETH_GET_PEER_OWN_REFERENCE

    /**
     * rtnl_u32_add_action() and rtnl_basic_add_action() now grab a reference to
     * act caller are free to release its own
     */
    NL_CAPABILITY_ROUTE_LINK_CLS_ADD_ACT_OWN_REFERENCE = 3,
#define NL_CAPABILITY_ROUTE_LINK_CLS_ADD_ACT_OWN_REFERENCE                     \
    NL_CAPABILITY_ROUTE_LINK_CLS_ADD_ACT_OWN_REFERENCE

    /**
     * Indicate that the local port is unspecified until the user accesses
     * it (via nl_socket_get_local_port()) or until nl_connect(). More
     * importantly, if the port is left unspecified, nl_connect() will retry
     * generating another port when bind() fails with ADDRINUSE.
     */
    NL_CAPABILITY_NL_CONNECT_RETRY_GENERATE_PORT_ON_ADDRINUSE = 4,
#define NL_CAPABILITY_NL_CONNECT_RETRY_GENERATE_PORT_ON_ADDRINUSE              \
...
#define NL_CAPABILITY_ROUTE_LINK_GET_KERNEL_FAIL_OPNOTSUPP                     \
    NL_CAPABILITY_ROUTE_LINK_GET_KERNEL_FAIL_OPNOTSUPP

    /**
     * Also consider the a_cacheinfo field (ADDR_ATTR_CACHEINFO) that contains
     * the address timestamps and expiry when comparing struct rtnl_addr objects
     * with nl_object_diff().
     */
    NL_CAPABILITY_ROUTE_ADDR_COMPARE_CACHEINFO = 6,
#define NL_CAPABILITY_ROUTE_ADDR_COMPARE_CACHEINFO                             \
    NL_CAPABILITY_ROUTE_ADDR_COMPARE_CACHEINFO

    /**
     * The library version is libnl3 3.2.26 or newer. This capability should
     * never be backported.
     */
...
#define NL_CAPABILITY_NL_RECV_FAIL_TRUNC_NO_PEEK                               \
    NL_CAPABILITY_NL_RECV_FAIL_TRUNC_NO_PEEK

    /**
     * rtnl_link_build_change_request() and rtnl_link_change() would set
     * ifi.ifi_flags but leave ifi.ifi_change at zero. This was later fixed to
     * set ifi.ifi_change to the flags that are actually set in changes.
     */
    NL_CAPABILITY_LINK_BUILD_CHANGE_REQUEST_SET_CHANGE = 9,
#define NL_CAPABILITY_LINK_BUILD_CHANGE_REQUEST_SET_CHANGE                     \
    NL_CAPABILITY_LINK_BUILD_CHANGE_REQUEST_SET_CHANGE

    /**
     * Between 3.2.14 (64fcb47a36ec12d7e7f00605f6a8952ce985dd08) and 3.2.22
     * (8571f58f23763d8db7365d02c9b27832ad3d7005), rtnl_neigh_get() behaved
     * differently and only returned objects with family AF_UNSPEC. This
     * capability indicates, that the function was fixed. The absense of the
     * capability, doesn't indicate however which behavior the function will
     * have. So beware. */
    NL_CAPABILITY_RTNL_NEIGH_GET_FILTER_AF_UNSPEC_FIX = 10,
#define NL_CAPABILITY_RTNL_NEIGH_GET_FILTER_AF_UNSPEC_FIX                      \
    NL_CAPABILITY_RTNL_NEIGH_GET_FILTER_AF_UNSPEC_FIX

    /**
     * The library version is libnl3 3.2.27 or newer. This capability should
     * never be backported.
     */
...
#define NL_CAPABILITY_RTNL_LINK_VLAN_PROTOCOL_SERIALZE                         \
...
#define NL_CAPABILITY_RTNL_LINK_PARSE_GRE_REMOTE                               \
    NL_CAPABILITY_RTNL_LINK_PARSE_GRE_REMOTE

    /**
     * Don't skip over vlan ingress-map entries with "to" field zero when
     * serializing a netlink message. Previously such entires would be ignored
     * which inhibits the user from clearing ingress map entries.
     */
    NL_CAPABILITY_RTNL_LINK_VLAN_INGRESS_MAP_CLEAR = 14,
#define NL_CAPABILITY_RTNL_LINK_VLAN_INGRESS_MAP_CLEAR                         \
...
#define NL_CAPABILITY_RTNL_LINK_VXLAN_IO_COMPARE                               \
...
#define NL_CAPABILITY_RTNL_ADDR_PEER_FIX NL_CAPABILITY_RTNL_ADDR_PEER_FIX
    ...
    /**
     * The library version is libnl3 3.2.28 or newer. This capability should
     * never be backported.
     */
...
#define NL_CAPABILITY_XFRM_SEC_CTX_LEN NL_CAPABILITY_XFRM_SEC_CTX_LEN
    ...
    /**
     * rtnl_link_build_add_request() would set ifi.ifi_flags but leave
     * ifi.ifi_change at zero. This was later fixed to set ifi.ifi_change to the
     * flags that are actually set
     */
    NL_CAPABILITY_LINK_BUILD_ADD_REQUEST_SET_CHANGE = 23,
#define NL_CAPABILITY_LINK_BUILD_ADD_REQUEST_SET_CHANGE                        \
    NL_CAPABILITY_LINK_BUILD_ADD_REQUEST_SET_CHANGE

    /* Older versions of libnl3 would not use MSG_PEEK for nl_recvmsgs() unless
     * calling nl_socket_enable_msg_peek(). Instead, the user had to specify the
     * buffer size via nl_socket_set_msg_buf_size(), which in turn would default
     * to 4*getpagesize().
     *
     * The default value might not be large enough, so users who were not aware
     * of the problem easily ended up using a too small receive buffer. Usually,
     * one wants to avoid MSG_PEEK for recvmsg() because it requires an
     * additional syscall.
     *
     * Now, as indicated by this capability, nl_recvmsgs() would use MSG_PEEK by
     * default. The user still can explicitly disable MSG_PEEK by calling
     * nl_socket_disable_msg_peek() or by setting the
     * nl_socket_set_msg_buf_size() to a non-zero value.
     */
    NL_CAPABILITY_NL_RECVMSGS_PEEK_BY_DEFAULT = 24,
#define NL_CAPABILITY_NL_RECVMSGS_PEEK_BY_DEFAULT                              \
    NL_CAPABILITY_NL_RECVMSGS_PEEK_BY_DEFAULT

    /**
     * The library version is libnl3 3.2.29 or newer. This capability should
     * never be backported.
     */
...
#define NL_CAPABILITY_XFRM_SP_SEC_CTX_LEN NL_CAPABILITY_XFRM_SP_SEC_CTX_LEN
    ...
    /**
     * The library version is libnl3 3.3.0 or newer. This capability should
     * never be backported.
     */
...
#define NL_CAPABILITY_VERSION_3_3_0 NL_CAPABILITY_VERSION_3_3_0
    ...
    /**
     * The library version is libnl3 3.4.0 or newer. This capability should
     * never be backported.
     */
...
/**
 * The range 0x7000 to 0x7FFF is reserved for private capabilities. Upstream
 * libnl3 will not register capabilities in this range. However, instead of
 * adding private capabilities, better register their number with upstream
 * libnl3. */
