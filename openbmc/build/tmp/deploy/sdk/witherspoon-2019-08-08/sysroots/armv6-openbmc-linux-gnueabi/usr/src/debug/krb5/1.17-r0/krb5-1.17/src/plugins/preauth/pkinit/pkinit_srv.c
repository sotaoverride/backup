...
static void pkinit_fini_kdc_req_context(krb5_context context, void* blob);

static void pkinit_server_plugin_fini_realm(krb5_context context,
                                            pkinit_kdc_context plgctx);

static void pkinit_server_plugin_fini(krb5_context context,
...
static void free_realm_contexts(krb5_context context,
                                pkinit_kdc_context* realm_contexts)
{
    int i;
...
static void free_certauth_handles(krb5_context context, certauth_handle* list)
{
    int i;
...
static krb5_error_code pkinit_create_edata(
    ...
    krb5_error_code err_code, krb5_pa_data*** e_data_out)
{
    ...
    switch (err_code)
    {
        case KRB5KDC_ERR_CANT_VERIFY_CERTIFICATE:
            retval = pkinit_create_td_trusted_certifiers(
        ...
        case KRB5KDC_ERR_REVOKED_CERTIFICATE:
            retval = pkinit_create_td_invalid_certificate(
                context, plg_cryptoctx, req_cryptoctx, id_cryptoctx,
                e_data_out);
            break;
        default:
...
static void pkinit_server_get_edata(krb5_context context, krb5_kdc_req* request,
                                    krb5_kdcpreauth_callbacks cb,
                                    krb5_kdcpreauth_rock rock,
                                    krb5_kdcpreauth_moddata moddata,
...
static krb5_error_code
    verify_client_san(krb5_context context, pkinit_kdc_context plgctx,
                      pkinit_kdc_req_context reqctx,
                      krb5_kdcpreauth_callbacks cb, krb5_kdcpreauth_rock rock,
                      krb5_const_principal client, int* valid_san)
{
    ...
    *valid_san = 0;
    retval = crypto_retrieve_cert_sans(
        context, plgctx->cryptoctx, reqctx->cryptoctx, plgctx->idctx, &princs,
        plgctx->opts->allow_upn ? &upns : NULL, NULL);
    if (retval)
    {
                                         ...
                                         int* eku_accepted)
{
    ...
    *eku_accepted = 0;
    ...
    retval =
        crypto_check_cert_eku(context, plgctx->cryptoctx, reqctx->cryptoctx,
                              plgctx->idctx, 0, /* kdc cert */
                              plgctx->opts->accept_secondary_eku, eku_accepted);
    if (retval)
    {
...
static krb5_error_code check_log_freshness(krb5_context context,
                                           pkinit_kdc_context plgctx,
                                           krb5_kdc_req* request,
                                           krb5_boolean valid_freshness_token)
{
    krb5_error_code ret;
    char* name = NULL;
...
static void pkinit_server_verify_padata(
    krb5_context context, krb5_data* req_pkt, krb5_kdc_req* request,
    krb5_enc_tkt_part* enc_tkt_reply, krb5_pa_data* data,
    krb5_kdcpreauth_callbacks cb, krb5_kdcpreauth_rock rock,
    krb5_kdcpreauth_moddata moddata, krb5_kdcpreauth_verify_respond_fn respond,
    void* arg)
{
    krb5_error_code retval = 0;
    ...
    switch ((int)data->pa_type)
    {
        case KRB5_PADATA_PK_AS_REQ:
            ...
            print_buffer_bin(reqp->signedAuthPack.data,
                             ...
                             "/tmp/kdc_signed_data");
#endif
            retval = cms_signeddata_verify(
                context, plgctx->cryptoctx, reqctx->cryptoctx, plgctx->idctx,
                CMS_SIGN_CLIENT, plgctx->opts->require_crl_checking,
                (unsigned char*)reqp->signedAuthPack.data,
                reqp->signedAuthPack.length, (unsigned char**)&authp_data.data,
                &authp_data.length, (unsigned char**)&krb5_authz.data,
        ...
        case KRB5_PADATA_PK_AS_REQ_OLD:
            ...
            retval = cms_signeddata_verify(
                context, plgctx->cryptoctx, reqctx->cryptoctx, plgctx->idctx,
                CMS_SIGN_DRAFT9, plgctx->opts->require_crl_checking,
                (unsigned char*)reqp9->signedAuthPack.data,
                reqp9->signedAuthPack.length, (unsigned char**)&authp_data.data,
                &authp_data.length, (unsigned char**)&krb5_authz.data,
    ...
    switch ((int)data->pa_type)
    {
        case KRB5_PADATA_PK_AS_REQ:
            ...
            retval =
                krb5_check_clockskew(context, auth_pack->pkAuthenticator.ctime);
            if (retval)
                goto cleanup;
            ...
            if (auth_pack->clientPublicValue != NULL)
            {
                retval = server_check_dh(
                    context, plgctx->cryptoctx, reqctx->cryptoctx,
                    plgctx->idctx,
            ...
            {
                int valid_kdcPkId = 0;
                retval = pkinit_check_kdc_pkid(
                    context, plgctx->cryptoctx, reqctx->cryptoctx,
                    plgctx->idctx, (unsigned char*)reqp->kdcPkId.data,
...
static krb5_error_code return_pkinit_kx(krb5_context context,
                                        krb5_kdc_req* request,
                                        krb5_kdc_rep* reply,
                                        ...
                                        krb5_pa_data** out_padata)
{
    ...
    *out_padata = NULL;
     ...
     */
    ret = krb5_c_fx_cf2_simple(context, session, "PKINIT", encrypting_key,
                               "KEYEXCHANGE", &new_session);
    if (ret)
        goto cleanup;
...
static krb5_error_code pkinit_pick_kdf_alg(krb5_context context,
                                           krb5_data** kdf_list,
                                           krb5_data** alg_oid)
{
    krb5_error_code retval = 0;
        ...
        for (j = 0; NULL != (req_oid = kdf_list[j]); j++)
        {
                ...
                (0 == memcmp(req_oid->data, supp_oid->data, req_oid->length)))
            {
                ...
                if (retval)
                    goto cleanup;
                tmp_oid->data =
                    k5memdup(supp_oid->data, supp_oid->length, &retval);
                if (retval)
                    goto cleanup;
...
static krb5_error_code pkinit_server_return_padata(
    krb5_context context, krb5_pa_data* padata, krb5_data* req_pkt,
    krb5_kdc_req* request, krb5_kdc_rep* reply, krb5_keyblock* encrypting_key,
    ...
    krb5_kdcpreauth_modreq modreq)
{
    ...
    *send_pa = NULL;
    ...
    {
        subjectPublicKey = (unsigned char*)reqctx->rcv_auth_pack9
                               ->clientPublicValue->subjectPublicKey.data;
        subjectPublicKey_len =
                        ...
                        rep->choice == choice_pa_pk_as_rep_draft9_dhSignedData))
    {
        ...
        retval = server_process_dh(
            context, plgctx->cryptoctx, reqctx->cryptoctx, plgctx->idctx,
            subjectPublicKey, subjectPublicKey_len, &dh_pubkey, &dh_pubkey_len,
        ...
        (rep != NULL && rep->choice == choice_pa_pk_as_rep_dhInfo))
    {
        ...
        retval =
            k5int_encode_krb5_kdc_dh_key_info(&dhkey_info, &encoded_dhkey_info);
        if (retval)
        {
        ...
        switch ((int)padata->pa_type)
        {
            case KRB5_PADATA_PK_AS_REQ:
                retval = cms_signeddata_create(
                    context, plgctx->cryptoctx, reqctx->cryptoctx,
                    plgctx->idctx, CMS_SIGN_SERVER, 1,
                    (unsigned char*)encoded_dhkey_info->data,
                    encoded_dhkey_info->length,
                    (unsigned char**)&rep->u.dh_Info.dhSignedData.data,
            ...
            case KRB5_PADATA_PK_AS_REQ_OLD:
                retval = cms_signeddata_create(
                    context, plgctx->cryptoctx, reqctx->cryptoctx,
                    plgctx->idctx, CMS_SIGN_DRAFT9, 1,
                    (unsigned char*)encoded_dhkey_info->data,
                    encoded_dhkey_info->length,
                    (unsigned char**)&rep9->u.dhSignedData.data,
    ...
    else
    {
        ...
        if ((int)padata->pa_type == KRB5_PADATA_PK_AS_REQ || fixed_keypack)
        {
            ...
            retval =
                k5int_encode_krb5_reply_key_pack(key_pack, &encoded_key_pack);
            if (retval)
            {
        ...
        switch ((int)padata->pa_type)
        {
            case KRB5_PADATA_PK_AS_REQ:
                rep->choice = choice_pa_pk_as_rep_encKeyPack;
                retval = cms_envelopeddata_create(
                    context, plgctx->cryptoctx, reqctx->cryptoctx,
                    plgctx->idctx, padata->pa_type, 1,
                    (unsigned char*)encoded_key_pack->data,
                    encoded_key_pack->length,
                    (unsigned char**)&rep->u.encKeyPack.data,
            ...
            case KRB5_PADATA_PK_AS_REQ_OLD:
                ...
                if (!fixed_keypack)
                {
                    ...
                    key_pack9->nonce =
                        reqctx->rcv_auth_pack9->pkAuthenticator.nonce;
                    krb5_copy_keyblock_contents(context, encrypting_key,
                                                &key_pack9->replyKey);

                    retval = k5int_encode_krb5_reply_key_pack_draft9(
                        key_pack9, &encoded_key_pack);
                    if (retval)
                    {
                        pkiDebug("failed to encode reply_key_pack\n");
                        goto cleanup;
                ...
                retval = cms_envelopeddata_create(
                    ...
                    (unsigned char*)encoded_key_pack->data,
                    encoded_key_pack->length,
                    (unsigned char**)&rep9->u.encKeyPack.data,
        ...
        print_buffer_bin((unsigned char*)encoded_key_pack->data,
                         encoded_key_pack->length, "/tmp/kdc_key_pack");
        switch ((int)padata->pa_type)
        {
            case KRB5_PADATA_PK_AS_REQ:
          ...
          reqctx->rcv_auth_pack->supportedKDFs != NULL)))
    {
        ...
        if (reqctx->rcv_auth_pack != NULL &&
            reqctx->rcv_auth_pack->supportedKDFs != NULL)
        {
            retval = pkinit_pick_kdf_alg(context,
        ...
        (rep != NULL && rep->choice == choice_pa_pk_as_rep_dhInfo))
    {
        ...
        if (rep != NULL && rep->u.dh_Info.kdfID)
        {
            ...
            retval = pkinit_alg_agility_kdf(
                context, &secret, rep->u.dh_Info.kdfID, request->client,
                request->server, enctype, req_pkt, out_data, encrypting_key);
            if (retval)
            {
...
static int pkinit_server_get_flags(krb5_context kcontext,
...
static krb5_preauthtype supported_server_pa_types[] = {
    KRB5_PADATA_PK_AS_REQ, KRB5_PADATA_PK_AS_REQ_OLD, KRB5_PADATA_PK_AS_REP_OLD,
    KRB5_PADATA_PKINIT_KX, 0};

static void pkinit_fini_kdc_profile(krb5_context context,
                                    pkinit_kdc_context plgctx)
{
    /*
...
static krb5_error_code pkinit_init_kdc_profile(krb5_context context,
                                               pkinit_kdc_context plgctx)
{
    krb5_error_code retval;
    ...
    pkinit_kdcdefault_strings(context, plgctx->realmname, KRB5_CONF_PKINIT_POOL,
                              &plgctx->idopts->intermediates);

    pkinit_kdcdefault_strings(context, plgctx->realmname,
                              KRB5_CONF_PKINIT_REVOKE, &plgctx->idopts->crls);

    pkinit_kdcdefault_string(context, plgctx->realmname,
                             KRB5_CONF_PKINIT_KDC_OCSP, &ocsp_check);
    if (ocsp_check != NULL)
    {
        free(ocsp_check);
        retval = ENOTSUP;
    ...
    pkinit_kdcdefault_integer(
        context, plgctx->realmname, KRB5_CONF_PKINIT_DH_MIN_BITS,
        PKINIT_DEFAULT_DH_MIN_BITS, &plgctx->opts->dh_min_bits);
    if (plgctx->opts->dh_min_bits < PKINIT_DH_MIN_CONFIG_BITS)
    {
        pkiDebug("%s: invalid value (%d < %d) for pkinit_dh_min_bits, "
                 "using default value (%d) instead\n",
                 __FUNCTION__, plgctx->opts->dh_min_bits,
                 PKINIT_DH_MIN_CONFIG_BITS, PKINIT_DEFAULT_DH_MIN_BITS);
        plgctx->opts->dh_min_bits = PKINIT_DEFAULT_DH_MIN_BITS;
    }

    pkinit_kdcdefault_boolean(context, plgctx->realmname,
                              KRB5_CONF_PKINIT_ALLOW_UPN, 0,
                              &plgctx->opts->allow_upn);

    pkinit_kdcdefault_boolean(context, plgctx->realmname,
                              KRB5_CONF_PKINIT_REQUIRE_CRL_CHECKING, 0,
                              &plgctx->opts->require_crl_checking);

    pkinit_kdcdefault_boolean(context, plgctx->realmname,
                              KRB5_CONF_PKINIT_REQUIRE_FRESHNESS, 0,
                              &plgctx->opts->require_freshness);

    pkinit_kdcdefault_string(context, plgctx->realmname,
                             KRB5_CONF_PKINIT_EKU_CHECKING, &eku_string);
    if (eku_string != NULL)
    {
...
static pkinit_kdc_context pkinit_find_realm_context(
    krb5_context context, krb5_kdcpreauth_moddata moddata, krb5_principal princ)
{
    int i;
    pkinit_kdc_context* realm_contexts;
...
static int pkinit_server_plugin_init_realm(krb5_context context,
                                           ...
                                           pkinit_kdc_context* pplgctx)
{
    ...
    *pplgctx = NULL;
    ...
    retval = pkinit_identity_initialize(context, plgctx->cryptoctx, NULL,
                                        plgctx->idopts, plgctx->idctx, NULL,
                                        NULL, NULL);
    if (retval)
        goto errout;
    retval =
        pkinit_identity_prompt(context, plgctx->cryptoctx, NULL, plgctx->idopts,
                               plgctx->idctx, NULL, NULL, 0, NULL);
    if (retval)
        goto errout;
...
static krb5_error_code pkinit_san_authorize(
    krb5_context context, krb5_certauth_moddata moddata, const uint8_t* cert,
    size_t cert_len, krb5_const_principal princ, const void* opts,
    const struct _krb5_db_entry_new* db_entry, char*** authinds_out)
{
    krb5_error_code ret;
    int valid_san;
...
static krb5_error_code pkinit_eku_authorize(
    krb5_context context, krb5_certauth_moddata moddata, const uint8_t* cert,
    size_t cert_len, krb5_const_principal princ, const void* opts,
    const struct _krb5_db_entry_new* db_entry, char*** authinds_out)
{
...
static krb5_error_code certauth_pkinit_san_initvt(krb5_context context,
...
static krb5_error_code certauth_pkinit_eku_initvt(krb5_context context,
                                                  int maj_ver, int min_ver,
                                                  krb5_plugin_vtable vtable)
{
    krb5_certauth_vtable vt;
 ...
 */
static krb5_error_code dbmatch_authorize(
    krb5_context context, krb5_certauth_moddata moddata, const uint8_t* cert,
    size_t cert_len, krb5_const_principal princ, const void* opts,
    const struct _krb5_db_entry_new* db_entry, char*** authinds_out)
{
    ...
    *authinds_out = NULL;
    ...
    ret = req_opts->cb->get_string(context, req_opts->rock, "pkinit_cert_match",
                                   &pattern);
    if (ret)
        return ret;
    if (pattern == NULL)
...
static krb5_error_code certauth_dbmatch_initvt(krb5_context context,
                                               int maj_ver, int min_ver,
                                               krb5_plugin_vtable vtable)
{
    krb5_certauth_vtable vt;
...
static krb5_error_code load_certauth_plugins(krb5_context context,
                                             certauth_handle** handle_out)
{
    ...
    ret = k5_plugin_register(context, PLUGIN_INTERFACE_CERTAUTH, "pkinit_san",
                             certauth_pkinit_san_initvt);
    if (ret)
        goto cleanup;

    ret = k5_plugin_register(context, PLUGIN_INTERFACE_CERTAUTH, "pkinit_eku",
                             certauth_pkinit_eku_initvt);
    if (ret)
        goto cleanup;
...
static int pkinit_server_plugin_init(krb5_context context,
                                     ...
                                     const char** realmnames)
{
    ...
    for (i = 0, j = 0; i < numrealms; i++)
    {
        ...
        retval =
            pkinit_server_plugin_init_realm(context, realmnames[i], &plgctx);
        if (retval)
...
krb5_error_code kdcpreauth_pkinit_initvt(krb5_context context, int maj_ver,
                                         int min_ver,
                                         krb5_plugin_vtable vtable);
