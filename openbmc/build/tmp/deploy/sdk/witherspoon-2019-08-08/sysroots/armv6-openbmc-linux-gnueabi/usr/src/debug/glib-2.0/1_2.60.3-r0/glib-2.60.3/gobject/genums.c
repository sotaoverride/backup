...
/* --- prototypes --- */
static void g_enum_class_init(GEnumClass* class, gpointer class_data);
static void g_flags_class_init(GFlagsClass* class, gpointer class_data);
...
void _g_enum_types_init(void)
{
    ...
    type = g_type_register_fundamental(
        G_TYPE_ENUM, g_intern_static_string("GEnum"), &info, &finfo,
    ...
    type = g_type_register_fundamental(
        G_TYPE_FLAGS, g_intern_static_string("GFlags"), &info, &finfo,
        G_TYPE_FLAG_ABSTRACT | G_TYPE_FLAG_VALUE_ABSTRACT);
...
GEnumValue* g_enum_get_value_by_name(GEnumClass* enum_class, const gchar* name)
{
    ...
    if (enum_class->n_values)
    {
        ...
        for (enum_value = enum_class->values; enum_value->value_name;
             enum_value++)
    ...
    if (enum_class->n_values)
    {
        ...
        for (enum_value = enum_class->values; enum_value->value_name;
            ...
            if (enum_value->value_nick &&
                strcmp(nick, enum_value->value_nick) == 0)
                return enum_value;
    }
                                       ...
                                       const gchar* nick)
{
    ...
    if (flags_class->n_values)
    {
        ...
        for (flags_value = flags_class->values; flags_value->value_nick;
             flags_value++)
            if (flags_value->value_nick &&
                strcmp(nick, flags_value->value_nick) == 0)
                return flags_value;
    }
 ...
 * Since: 2.54
 */
static gchar* g_flags_get_value_string(GFlagsClass* flags_class, guint value)
{
    GString* str;
    GFlagsValue* flags_value;
