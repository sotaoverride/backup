                     ...
                     const char* domain)
{
    ...
    if ((strlen(domainname) + strlen(hostname) + OPSYS_LEN + 3) > MAXNETNAMELEN)
        return 0;
...
int netname2user(const char netname[MAXNETNAMELEN + 1], uid_t* uidp,
...
libc_hidden_nolink_sunrpc(netname2user, GLIBC_2_1)
    ...
    int netname2host(const char netname[MAXNETNAMELEN + 1], char* hostname,
                     const int hostlen)
{
    char *p1, *p2;
