...
static void do_encrypt_block(CAST5_context* context, byte* outbuf,
                             const byte* inbuf)
{
#ifdef HAVE_COMPATIBLE_GCC_WIN64_PLATFORM_AS
...
static void do_decrypt_block(CAST5_context* context, byte* outbuf,
                             const byte* inbuf)
{
#ifdef HAVE_COMPATIBLE_GCC_WIN64_PLATFORM_AS
...
static void cast5_amd64_ctr_enc(CAST5_context* ctx, byte* out, const byte* in,
                                byte* ctr)
{
#ifdef HAVE_COMPATIBLE_GCC_WIN64_PLATFORM_AS
...
static void cast5_amd64_cbc_dec(CAST5_context* ctx, byte* out, const byte* in,
                                byte* iv)
{
#ifdef HAVE_COMPATIBLE_GCC_WIN64_PLATFORM_AS
...
static void cast5_amd64_cfb_dec(CAST5_context* ctx, byte* out, const byte* in,
                                byte* iv)
{
#ifdef HAVE_COMPATIBLE_GCC_WIN64_PLATFORM_AS
...
static void do_encrypt_block(CAST5_context* c, byte* outbuf, const byte* inbuf)
{
    u32 l, r, t;
...
static void do_decrypt_block(CAST5_context* c, byte* outbuf, const byte* inbuf)
{
    u32 l, r, t;
    u32 I;
    u32* Km;
...
static const char* selftest_ctr(void)
{
    ...
    return _gcry_selftest_helper_ctr("CAST5", &cast_setkey, &encrypt_block,
                                     &_gcry_cast5_ctr_enc, nblocks, blocksize,
                                     context_size);
}
...
static const char* selftest_cbc(void)
{
    ...
    return _gcry_selftest_helper_cbc("CAST5", &cast_setkey, &encrypt_block,
                                     &_gcry_cast5_cbc_dec, nblocks, blocksize,
                                     context_size);
}
...
static const char* selftest_cfb(void)
{
    ...
    return _gcry_selftest_helper_cfb("CAST5", &cast_setkey, &encrypt_block,
                                     &_gcry_cast5_cfb_dec, nblocks, blocksize,
                                     context_size);
}
...
{
    CAST5_context c;
    static const byte key[16] = {0x01, 0x23, 0x45, 0x67, 0x12, 0x34,
                                 0x56, 0x78, 0x23, 0x45, 0x67, 0x89,
                                 0x34, 0x56, 0x78, 0x9A};
    static const byte plain[8] = {0x01, 0x23, 0x45, 0x67,
                                  0x89, 0xAB, 0xCD, 0xEF};
    static const byte cipher[8] = {0x23, 0x8B, 0x4F, 0xE5,
                                   0x84, 0x7E, 0x44, 0xB2};
    byte buffer[8];
    const char* r;
...
static gcry_err_code_t do_cast_setkey(CAST5_context* c, const byte* key,
                                      unsigned keylen)
{
    ...
    for (i = 0; i < 4; i++)
    {
        ...
        c->Kr_arm_enc[i] = Kr_arm[0] | (Kr_arm[1] << 8) | (Kr_arm[2] << 16) |
                           (Kr_arm[3] << 24);
        c->Kr_arm_dec[i] = Kr_arm[3] | (Kr_arm[2] << 8) | (Kr_arm[1] << 16) |
                           (Kr_arm[0] << 24);
...
static gcry_err_code_t cast_setkey(void* context, const byte* key,
                                   unsigned keylen)
{
    CAST5_context* c = (CAST5_context*)context;
...
gcry_cipher_spec_t _gcry_cipher_spec_cast5 = {GCRY_CIPHER_CAST5,
                                              {0, 0},
                                              "CAST5",
                                              NULL,
                                              NULL,
