...
/* static data */
DEFobjStaticHelpers DEFobjCurrIf(glbl) DEFobjCurrIf(net) DEFobjCurrIf(netstrms)
    DEFobjCurrIf(netstrm) DEFobjCurrIf(prop)
 ...
 */
BEGINobjConstruct(
    nsd_ptcp) /* be sure to specify the object type also in END macro! */
    pThis->sock = -1;
ENDobjConstruct(nsd_ptcp)

    /* destructor for the nsd_ptcp object */
    BEGINobjDestruct(nsd_ptcp) /* be sure to specify the object type also in END
                                  and CODESTART macros! */
    CODESTARTobjDestruct(nsd_ptcp) sockClose(&pThis->sock);
 ...
 * alorbach, 2018-12-20
 */
static rsRetVal SetPermitExpiredCerts(nsd_t __attribute__((unused)) * pNsd,
                                      uchar* mode)
{
    DEFiRet;
    if (mode != NULL)
    {
        LogError(0, RS_RET_VALUE_NOT_SUPPORTED,
                 "error: permitexpiredcerts settingnot supported by "
                 "ptcp netstream driver");
        ABORT_FINALIZE(RS_RET_VALUE_NOT_SUPPORTED);
    }
...
static rsRetVal SetGnutlsPriorityString(nsd_t __attribute__((unused)) * pNsd,
                                        uchar* iVal)
{
    DEFiRet;
    if (iVal != NULL)
...
static rsRetVal SetPermPeers(nsd_t __attribute__((unused)) * pNsd,
                             permittedPeers_t __attribute__((unused)) *
                                 pPermPeers)
{
    DEFiRet;
...
static rsRetVal Abort(nsd_t* pNsd)
{
    ...
    if ((pThis)->sock != -1)
    {
        ling.l_onoff = 1;
        ling.l_linger = 0;
        if (setsockopt((pThis)->sock, SOL_SOCKET, SO_LINGER, &ling,
                       sizeof(ling)) < 0)
        {
...
/* Set pRemHost based on the address provided. This is to be called upon
 * accept()ing a connection request. It must be provided by the socket we
 * received the message on as well as a NI_MAXHOST size large character buffer
 * for the FQDN. Please see http://www.hmug.org/man/3/getnameinfo.php (under
 * Caveats) for some explanation of the code found below. If we detect a
 * malicious hostname, we return RS_RET_MALICIOUS_HNAME and let the caller
 * decide on how to deal with that. rgerhards, 2008-03-31
 */
static rsRetVal FillRemHost(nsd_ptcp_t* pThis, struct sockaddr_storage* pAddr)
{
    ...
    /* We now have the names, so now let's allocate memory and store them
     * permanently. (side note: we may hold on to these values for quite a
     * while, thus we trim their memory consumption)
     */
    if ((pThis->pRemHostName = malloc(prop.GetStringLen(fqdn) + 1)) == NULL)
        ABORT_FINALIZE(RS_RET_OUT_OF_MEMORY);
    memcpy(pThis->pRemHostName, propGetSzStr(fqdn),
           prop.GetStringLen(fqdn) + 1);
    prop.Destruct(&fqdn);
 ...
 */
static rsRetVal AcceptConnReq(nsd_t* pNsd, nsd_t** ppNew)
{
    int sockflags;
    nsd_ptcp_t* pThis = (nsd_ptcp_t*)pNsd;
    ...
    if (iNewSock < 0)
    {
        if (Debug)
        {
            ...
            dbgprintf("nds_ptcp: error accepting connection on socket %d, "
                      "errno %d: %s\n",
     ...
     * begin perfect (from an abstract design perspective), we need this to
     * prevent breaking everything. TODO: we need to implement a new ACL module
     * to get rid of this function. -- rgerhards, 2008-12-01
     */
    memcpy(&pNew->remAddr, &addr, sizeof(struct sockaddr_storage));
    CHKiRet(FillRemHost(pNew, &addr));

    /* set the new socket to non-blocking IO -TODO:do we really need to do this
     * here? Do we always want it? */
    ...
    if (sockflags == -1)
    {
        dbgprintf("error %d setting fcntl(O_NONBLOCK) on tcp socket %d", errno,
                  iNewSock);
...
/* initialize tcp sockets for a listner. The initialized sockets are passed to
 * the app-level caller via a callback. pLstnPort must point to a port name or
 * number. NULL is NOT permitted. pLstnIP points to the port to listen to (NULL
 * means "all"), iMaxSess has the maximum number of sessions permitted.
 * rgerhards, 2008-04-22
 */
PRAGMA_DIAGNOSTIC_PUSH
PRAGMA_IGNORE_Wcast_align static rsRetVal
    LstnInit(netstrms_t* pNS, void* pUsr,
             rsRetVal (*fAddLstn)(void*, netstrm_t*), uchar* pLstnPort,
             uchar* pLstnIP, int iSessMax, uchar* pszLstnPortFileName)
{
    DEFiRet;
    ...
    int sockflags;
    int port_override =
        0; /* if dyn port (0): use this for actually bound port */
    ...
    for (r = res; r != NULL; r = r->ai_next)
    {
        ...
        if (r->ai_family == AF_INET6)
        {
            isIPv6 = 1;
            int iOn = 1;
            if (setsockopt(sock, IPPROTO_IPV6, IPV6_V6ONLY, (char*)&iOn,
                           sizeof(iOn)) < 0)
            {
                close(sock);
                sock = -1;
        ...
        if (sockflags == -1)
        {
            dbgprintf("error %d setting fcntl(O_NONBLOCK) on tcp socket",
                      errno);
            close(sock);
            sock = -1;
            continue;
 ...
 */
        ...
        {
            if (setsockopt(sock, SOL_SOCKET, SO_BSDCOMPAT, (char*)&on,
                           sizeof(on)) < 0)
            {
        ...
        /* if we bind to dynamic port (port 0 given), we will do so
         * consistently. Thus once we got a dynamic port, we will keep it and
         * use it for other protocols as well. As of my understanding, this
         * should always work as the OS does not pick a port that is used by
         * some protocol (well, at least this looks very unlikely...). If our
         * asusmption is wrong, we should iterate until we find a combination
         * that works - it is very unusual to have the same service listen on
         * differnt ports on IPv4 and IPv6.
         */
        const int currport =
            (isIPv6) ? (((struct sockaddr_in6*)r->ai_addr)->sin6_port)
        ...
        if (currport == 0)
        {
            ...
            {
                FILE* fp;
                if ((fp = fopen((const char*)pszLstnPortFileName, "w+")) ==
                    NULL)
                {
                ...
                if (isIPv6)
                {
                    fprintf(
                        fp, "%d",
                        ntohs((((struct sockaddr_in6*)r->ai_addr)->sin6_port)));
                }
                else
                {
                    fprintf(
        ...
        if (listen(sock, iSessMax / 10 + 5) < 0)
        {
             ...
             */
            dbgprintf("listen with a backlog of %d failed - retrying with "
                      "default of 32.\n",
                      iSessMax / 10 + 5);
            if (listen(sock, 32) < 0)
            {
        ...
        /* if we reach this point, we were able to obtain a valid socket, so we
         * can construct a new netstrm obj and hand it over to the upper layers
         * for inclusion into their socket array. -- rgerhards, 2008-04-23
         */
        ...
        CHKiRet(pNS->Drvr.SetPermitExpiredCerts(
            pNewNsd, netstrms.GetDrvrPermitExpiredCerts(pNS)));
        CHKiRet(
            pNS->Drvr.SetPermPeers(pNewNsd, netstrms.GetDrvrPermPeers(pNS)));
        CHKiRet(pNS->Drvr.SetGnutlsPriorityString(
            pNewNsd, netstrms.GetDrvrGnutlsPriorityString(pNS)));
 ...
 */
static rsRetVal Rcv(nsd_t* pNsd, uchar* pRcvBuf, ssize_t* pLenBuf,
                    int* const oserr)
{
    char errStr[1024];
    DEFiRet;
...
static rsRetVal EnableKeepAlive(nsd_t* pNsd)
{
    ...
    if (pThis->iKeepAliveProbes > 0)
    {
        ...
        optlen = sizeof(optval);
        ret =
            setsockopt(pThis->sock, IPPROTO_TCP, TCP_KEEPCNT, &optval, optlen);
    }
    else
    {
        ret = 0;
    ...
    if (ret < 0)
    {
        LogError(ret, NO_ERRCODE,
                 "imptcp cannot set keepalive probes - ignored");
    }
    ...
    if (pThis->iKeepAliveTime > 0)
    {
        ...
        optlen = sizeof(optval);
        ret =
            setsockopt(pThis->sock, IPPROTO_TCP, TCP_KEEPIDLE, &optval, optlen);
    }
    else
    {
        ret = 0;
    ...
    if (pThis->iKeepAliveIntvl > 0)
    {
        ...
        optlen = sizeof(optval);
        ret = setsockopt(pThis->sock, IPPROTO_TCP, TCP_KEEPINTVL, &optval,
                         optlen);
    }
    else
    {
    ...
    {
        LogError(errno, NO_ERRCODE,
                 "imptcp cannot set keepalive intvl - ignored");
    }
 ...
 * rgerhards, 2008-03-19
 */
static rsRetVal Connect(nsd_t* pNsd, int family, uchar* port, uchar* host,
                        char* device)
{
    nsd_ptcp_t* pThis = (nsd_ptcp_t*)pNsd;
    ...
    {
        LogError(errno, RS_RET_IO_ERROR, "cannot bind socket for %s:%s", host,
                 port);
        ABORT_FINALIZE(RS_RET_IO_ERROR);
    }
    ...
    if (device)
    {
        ...
        if (setsockopt(pThis->sock, SOL_SOCKET, SO_BINDTODEVICE, device,
                       strlen(device) + 1) < 0)
#endif
        {
    ...
    {
        LogError(errno, RS_RET_IO_ERROR, "cannot connect to %s:%s", host, port);
        ABORT_FINALIZE(RS_RET_IO_ERROR);
    }
...
static rsRetVal GetRemoteHName(nsd_t* pNsd, uchar** ppszHName)
{
    ...
    CHKmalloc(*ppszHName = (uchar*)strdup(pThis->pRemHostName == NULL
                                              ? ""
                                              : (char*)pThis->pRemHostName));
...
BEGINobjQueryInterface(nsd_ptcp)
    CODESTARTobjQueryInterface(nsd_ptcp) if (pIf->ifVersion !=
                                             nsdCURR_IF_VERSION)
...
CHKiRet(nsd_ptcpClassInit(
    pModInfo)); /* must be done after tcps_sess, as we use it */
CHKiRet(nsdsel_ptcpClassInit(
    pModInfo));          /* must be done after tcps_sess, as we use it */
#ifdef HAVE_EPOLL_CREATE /* module only available if epoll() is supported! */
CHKiRet(nsdpoll_ptcpClassInit(
    pModInfo)); /* must be done after tcps_sess, as we use it */
#endif
ENDmodInit
    /* vi:set ai:
