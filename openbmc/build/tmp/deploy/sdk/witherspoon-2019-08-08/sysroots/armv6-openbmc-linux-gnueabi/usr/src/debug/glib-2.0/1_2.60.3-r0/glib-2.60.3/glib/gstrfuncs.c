...
const guint16* const g_ascii_table = ascii_table_data;
    ...
    defined(HAVE_STRTOD_L) && defined(HAVE_STRTOULL_L) &&                      \
...
static guint64 g_parse_long_long(const gchar* nptr, const gchar** endptr,
                                 guint base, gboolean* negative)
{
...
static void index_add_folded(GPtrArray* array, const gchar* start,
                             const gchar* end)
{
    gchar* normal;
...
static gchar** split_words(const gchar* value)
{
        ...
        *ascii_alternates = g_new(gchar*, n + 1);
        ...
        for (i = 0; i < n; i++)
        {
            if (!g_str_is_ascii(result[i]))
            {
                ...
                composed =
                    g_utf8_normalize(result[i], -1, G_NORMALIZE_ALL_COMPOSE);
                            ...
                            gboolean accept_alternates)
{
    ...
    hit_tokens = g_str_tokenize_and_fold(
        potential_hit, NULL, accept_alternates ? &alternates : NULL);
                                  ...
                                  gint64 max, gint64* out_num, GError** error)
{
    ...
    if (str[0] == '\0')
    {
        g_set_error_literal(error, G_NUMBER_PARSER_ERROR,
    ...
    if (/* We do not allow leading whitespace, but g_ascii_strtoll
         ...
         * 0X.
         */
        (base == 16 && (str_has_sign(str) ? str_has_hex_prefix(str + 1)
                                          : str_has_hex_prefix(str))) ||
        (saved_errno != 0 && saved_errno != ERANGE) || end_ptr == NULL ||
        *end_ptr != '\0')
    {
    ...
    if (saved_errno == ERANGE || number < min || number > max)
    {
        ...
        g_set_error(
            error, G_NUMBER_PARSER_ERROR, G_NUMBER_PARSER_ERROR_OUT_OF_BOUNDS,
