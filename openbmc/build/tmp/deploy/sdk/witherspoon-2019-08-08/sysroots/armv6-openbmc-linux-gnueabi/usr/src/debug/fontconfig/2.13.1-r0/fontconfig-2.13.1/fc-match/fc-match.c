...
static const struct option longopts[] = {
    {"sort", 0, 0, 's'},  {"all", 0, 0, 'a'},    {"verbose", 0, 0, 'v'},
    {"brief", 0, 0, 'b'}, {"format", 1, 0, 'f'}, {"version", 0, 0, 'V'},
...
static void usage(char* program, int error)
{
...
#if HAVE_GETOPT_LONG
    fprintf(file,
            _("usage: %s [-savbVh] [-f FORMAT] [--sort] [--all] [--verbose] "
              "[--brief] [--format=FORMAT] [--version] [--help] [pattern] "
              "{element...}\n"),
            program);
#else
    ...
    fprintf(
        file,
        _("  -a, --all            display unpruned sorted list of matches\n"));
    fprintf(
        file,
