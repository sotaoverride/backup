 ...
 * [Desktop Entry
 * Specification](http://freedesktop.org/Standards/desktop-entry-spec), here is
 * a quick summary: Key files consists of groups of key-value pairs,
 ...
 * [Desktop Entry
 * Specification](http://freedesktop.org/Standards/desktop-entry-spec), groups
 * in key files may contain the same key multiple times; the last entry wins.
 * Key files may also contain multiple groups with the same name; they are
 * merged together. Another difference is that keys and group names in key files
 * are not restricted to ASCII characters.
 *
 ...
 *   }
 *
 * g_autofree gchar *val = g_key_file_get_string (key_file, "Group Name",
 * "SomeKey", &error); if (val == NULL && !g_error_matches (error,
 ...
 * gsize data_len;
 * g_autofree guint8 *data = (guint8 *) g_key_file_to_data (key_file, &data_len,
 * &error); if (data == NULL)
 *   {
 *     g_warning ("Error saving key file: %s", error->message);
 *     return;
 *   }
 * g_autoptr(GBytes) bytes = g_bytes_new_take (g_steal_pointer (&data),
 * data_len);
 * ]|
 */
 ...
 * [Desktop Entry
 * Specification](http://freedesktop.org/Standards/desktop-entry-spec). Consult
 * the specification for more details about the meanings of the keys below.
 *
 * Since: 2.14
 */
...
struct _GKeyFileGroup
{
    ...
    GKeyFileKeyValuePair*
        comment; /* Special comment that is stuck to the top of a group */
...
static gint find_file_in_data_dirs(const gchar* file, const gchar** data_dirs,
                                   gchar** output_file, GError** error);
static gboolean g_key_file_load_from_fd(GKeyFile* key_file, gint fd,
...
static GKeyFileKeyValuePair*
    g_key_file_lookup_key_value_pair(GKeyFile* key_file, GKeyFileGroup* group,
                                     const gchar* key);

static void g_key_file_remove_group_node(GKeyFile* key_file, GList* group_node);
...
static void g_key_file_add_key(GKeyFile* key_file, GKeyFileGroup* group,
                               const gchar* key, const gchar* value);
static void g_key_file_add_group(GKeyFile* key_file, const gchar* group_name);
...
static gint g_key_file_parse_value_as_integer(GKeyFile* key_file,
                                              const gchar* value,
                                              GError** error);
static gchar* g_key_file_parse_integer_as_value(GKeyFile* key_file, gint value);
...
static void g_key_file_parse_key_value_pair(GKeyFile* key_file,
                                            const gchar* line, gsize length,
                                            GError** error);
static void g_key_file_parse_comment(GKeyFile* key_file, const gchar* line,
                                     gsize length, GError** error);
static void g_key_file_parse_group(GKeyFile* key_file, const gchar* line,
                                   gsize length, GError** error);
static gchar* key_get_locale(const gchar* key);
static void g_key_file_parse_data(GKeyFile* key_file, const gchar* data,
                                  gsize length, GError** error);
static void g_key_file_flush_parse_buffer(GKeyFile* key_file, GError** error);

G_DEFINE_QUARK(g - key - file - error - quark, g_key_file_error)

static void g_key_file_init(GKeyFile* key_file)
{
...
static void g_key_file_clear(GKeyFile* key_file)
{
 ...
 */
static gint find_file_in_data_dirs(const gchar* file, const gchar** dirs,
                                   gchar** output_file, GError** error)
{
    ...
    if (fd == -1)
    {
        g_set_error_literal(error, G_KEY_FILE_ERROR, G_KEY_FILE_ERROR_NOT_FOUND,
 ...
 * @file: (type filename): the path of a filename to load, in the GLib filename
 *encoding
 ...
 * @length: the length of @data in bytes (or (gsize)-1 if data is
 *nul-terminated)
 * @flags: flags from #GKeyFileFlags
 * @error: return location for a #GError, or %NULL
 *
 ...
 * Since: 2.50
 **/
gboolean g_key_file_load_from_bytes(GKeyFile* key_file, GBytes* bytes,
                                    GKeyFileFlags flags, GError** error)
{
    const guchar* data;
    gsize size;
    ...
    return g_key_file_load_from_data(key_file, (const gchar*)data, size, flags,
                                     error);
}
 ...
 * Since: 2.14
 **/
gboolean g_key_file_load_from_dirs(GKeyFile* key_file, const gchar* file,
                                   const gchar** search_dirs, gchar** full_path,
                                   GKeyFileFlags flags, GError** error)
{
    GError* key_file_error = NULL;
 ...
 * Since: 2.6
 **/
gboolean g_key_file_load_from_data_dirs(GKeyFile* key_file, const gchar* file,
                                        ...
                                        GError** error)
{
    ...
    found_file = g_key_file_load_from_dirs(
        key_file, file, (const gchar**)all_data_dirs, full_path, flags, error);
 ...
 */
static gboolean g_key_file_locale_is_interesting(GKeyFile* key_file,
                                                 const gchar* locale)
{
    gsize i;
...
static void g_key_file_parse_line(GKeyFile* key_file, const gchar* line,
                                  gsize length, GError** error)
{
    ...
    if (g_key_file_line_is_comment(line_start))
        ...
        g_key_file_parse_group(key_file, line_start,
                               length - (line_start - line), &parse_error);
    else if (g_key_file_line_is_key_value_pair(line_start))
        g_key_file_parse_key_value_pair(
            key_file, line_start, length - (line_start - line), &parse_error);
    else
    {
                                            ...
                                            GError** error)
{
    ...
    if (key_file->current_group == NULL ||
        key_file->current_group->name == NULL)
    {
    ...
    if (key_file->current_group && key_file->current_group->name &&
        strcmp(key_file->start_group->name, key_file->current_group->name) ==
            0 &&
                                  ...
                                  gsize length, GError** error)
{
    ...
    while (i < length)
    {
        ...
        else
        {
            ...
            g_string_append_len(key_file->parse_buffer, start_of_line,
                                line_length);
            i += line_length;
        }
    }
...
static void g_key_file_flush_parse_buffer(GKeyFile* key_file, GError** error)
{
    ...
    if (key_file->parse_buffer->len > 0)
    {
        g_key_file_parse_line(key_file, key_file->parse_buffer->str,
                              key_file->parse_buffer->len, &file_error);
...
gchar* g_key_file_to_data(GKeyFile* key_file, gsize* length, GError** error)
{
    ...
    for (group_node = g_list_last(key_file->groups); group_node != NULL;
         group_node = group_node->prev)
    {
        ...
        for (key_file_node = g_list_last(group->key_value_pairs);
             key_file_node != NULL; key_file_node = key_file_node->prev)
        {
            GKeyFileKeyValuePair* pair;
                            ...
                            gsize* length, GError** error)
{
    ...
    if (!group)
    {
        g_set_error(error, G_KEY_FILE_ERROR, G_KEY_FILE_ERROR_GROUP_NOT_FOUND,
                    _("Key file does not have group “%s”"), group_name);
        return NULL;
    }
 ...
 *
 * Returns: (array zero-terminated=1) (transfer full): a newly-allocated
 *%NULL-terminated array of strings. Use g_strfreev() to free it. Since: 2.6
 **/
gchar** g_key_file_get_groups(GKeyFile* key_file, gsize* length)
{
    ...
    g_return_val_if_fail(((GKeyFileGroup*)group_node->data)->name == NULL,
                         NULL);
    ...
    i = 0;
    for (group_node = group_node->prev; group_node != NULL;
         group_node = group_node->prev)
    {
        GKeyFileGroup* group;
...
static void set_not_found_key_error(const char* group_name, const char* key,
                                    GError** error)
{
    g_set_error(error, G_KEY_FILE_ERROR, G_KEY_FILE_ERROR_KEY_NOT_FOUND,
                _("Key file does not have key “%s” in group “%s”"), key,
                group_name);
}
 ...
 * Since: 2.6
 **/
gchar* g_key_file_get_value(GKeyFile* key_file, const gchar* group_name,
 ...
 * Since: 2.6
 **/
void g_key_file_set_value(GKeyFile* key_file, const gchar* group_name,
                          const gchar* key, const gchar* value)
{
    GKeyFileGroup* group;
    GKeyFileKeyValuePair* pair;
                             ...
                             const gchar* key, GError** error)
{
    ...
    if (key_file_error)
    {
        if (g_error_matches(key_file_error, G_KEY_FILE_ERROR,
 ...
 * Since: 2.6
 **/
void g_key_file_set_string(GKeyFile* key_file, const gchar* group_name,
 ...
 *(transfer full): a %NULL-terminated string array or %NULL if the specified key
                                   ...
                                   GError** error)
{
    ...
    string_value = g_key_file_parse_value_as_string(key_file, value, &pieces,
                                                    &key_file_error);
    g_free(value);
    g_free(string_value);
 ...
 * Since: 2.6
 **/
void g_key_file_set_string_list(GKeyFile* key_file, const gchar* group_name,
                                const gchar* key, const gchar* const list[],
                                gsize length)
{
    GString* value_list;
    gsize i;
 ...
 * Since: 2.6
 **/
void g_key_file_set_locale_string(GKeyFile* key_file, const gchar* group_name,
 ...
 * Since: 2.6
 **/
gchar* g_key_file_get_locale_string(GKeyFile* key_file, const gchar* group_name,
                                    const gchar* key, const gchar* locale,
                                    GError** error)
{
    ...
    for (i = 0; languages[i]; i++)
    {
        ...
        translated_value =
            g_key_file_get_string(key_file, group_name, candidate_key, NULL);
    ...
    {
        translated_value =
            g_key_file_get_string(key_file, group_name, key, &key_file_error);
 ...
 *
 * Returns: (array zero-terminated=1 length=length) (element-type utf8)
 *(transfer full): a newly allocated %NULL-terminated string array or %NULL if
 *the key isn't found. The string array should be freed with g_strfreev().
 *
 * Since: 2.6
 **/
                                          ...
                                          gsize* length, GError** error)
{
    ...
    value = g_key_file_get_locale_string(key_file, group_name, key, locale,
                                         &key_file_error);
                                ...
                                const gchar* key, GError** error)
{
    ...
    bool_value =
        g_key_file_parse_value_as_boolean(key_file, value, &key_file_error);
    g_free(value);
 ...
 * Since: 2.6
 **/
void g_key_file_set_boolean(GKeyFile* key_file, const gchar* group_name,
                                      ...
                                      gsize* length, GError** error)
{
    ...
    values = g_key_file_get_string_list(key_file, group_name, key, &num_bools,
                                        &key_file_error);
    ...
    {
        bool_values[i] = g_key_file_parse_value_as_boolean(key_file, values[i],
                                                           &key_file_error);
 ...
 * Since: 2.6
 **/
void g_key_file_set_boolean_list(GKeyFile* key_file, const gchar* group_name,
                            ...
                            const gchar* key, GError** error)
{
    ...
    int_value =
        g_key_file_parse_value_as_integer(key_file, value, &key_file_error);
    g_free(value);
 ...
 * Since: 2.6
 **/
void g_key_file_set_integer(GKeyFile* key_file, const gchar* group_name,
 ...
 * Since: 2.26
 */
gint64 g_key_file_get_int64(GKeyFile* key_file, const gchar* group_name,
                            const gchar* key, GError** error)
{
    gchar *s, *end;
    gint64 v;
 ...
 * Since: 2.26
 **/
void g_key_file_set_int64(GKeyFile* key_file, const gchar* group_name,
 ...
 * Since: 2.26
 */
guint64 g_key_file_get_uint64(GKeyFile* key_file, const gchar* group_name,
                              const gchar* key, GError** error)
{
    gchar *s, *end;
    guint64 v;
 ...
 * Since: 2.26
 **/
void g_key_file_set_uint64(GKeyFile* key_file, const gchar* group_name,
 ...
 * Since: 2.6
 **/
gint* g_key_file_get_integer_list(GKeyFile* key_file, const gchar* group_name,
                                  const gchar* key, gsize* length,
                                  GError** error)
{
    GError* key_file_error = NULL;
    ...
    values = g_key_file_get_string_list(key_file, group_name, key, &num_ints,
                                        &key_file_error);
    ...
    {
        int_values[i] = g_key_file_parse_value_as_integer(key_file, values[i],
                                                          &key_file_error);
 ...
 * Since: 2.6
 **/
void g_key_file_set_integer_list(GKeyFile* key_file, const gchar* group_name,
 ...
 * Since: 2.12
 **/
gdouble g_key_file_get_double(GKeyFile* key_file, const gchar* group_name,
                              const gchar* key, GError** error)
{
    GError* key_file_error;
    gchar* value;
    ...
    double_value =
        g_key_file_parse_value_as_double(key_file, value, &key_file_error);
    g_free(value);
 ...
 * Since: 2.12
 **/
void g_key_file_set_double(GKeyFile* key_file, const gchar* group_name,
 ...
 * Since: 2.12
 **/
gdouble* g_key_file_get_double_list(GKeyFile* key_file, const gchar* group_name,
                                    const gchar* key, gsize* length,
                                    GError** error)
{
    GError* key_file_error = NULL;
    ...
    values = g_key_file_get_string_list(key_file, group_name, key, &num_doubles,
                                        &key_file_error);
    ...
    {
        double_values[i] = g_key_file_parse_value_as_double(key_file, values[i],
                                                            &key_file_error);
 ...
 * Since: 2.12
 **/
void g_key_file_set_double_list(GKeyFile* key_file, const gchar* group_name,
...
static gboolean g_key_file_set_key_comment(GKeyFile* key_file,
                                           const gchar* group_name,
                                           const gchar* key,
...
static gboolean g_key_file_set_group_comment(GKeyFile* key_file,
                                             const gchar* group_name,
                                             const gchar* comment,
                                             GError** error)
{
    ...
    group->comment->value =
        g_key_file_parse_comment_as_value(key_file, comment);

    return TRUE;
}

static gboolean g_key_file_set_top_comment(GKeyFile* key_file,
                                           const gchar* comment, GError** error)
{
    GList* group_node;
    GKeyFileGroup* group;
     ...
     */
    g_list_free_full(group->key_value_pairs,
                     (GDestroyNotify)g_key_file_key_value_pair_free);
 ...
 * Since: 2.6
 **/
gboolean g_key_file_set_comment(GKeyFile* key_file, const gchar* group_name,
                                ...
                                GError** error)
{
    ...
    {
        if (!g_key_file_set_key_comment(key_file, group_name, key, comment,
                                        error))
            return FALSE;
    }
...
static gchar* g_key_file_get_key_comment(GKeyFile* key_file,
                                         const gchar* group_name,
                                         const gchar* key, GError** error)
{
    GKeyFileGroup* group;
...
static gchar* get_group_comment(GKeyFile* key_file, GKeyFileGroup* group,
                                GError** error)
{
    GString* string;
    GList* tmp;
...
static gchar* g_key_file_get_group_comment(GKeyFile* key_file,
                                           const gchar* group_name,
                                           GError** error)
{
    GList* group_node;
...
static gchar* g_key_file_get_top_comment(GKeyFile* key_file, GError** error)
{
    GList* group_node;
    GKeyFileGroup* group;
 ...
 * Since: 2.6
 **/
gchar* g_key_file_get_comment(GKeyFile* key_file, const gchar* group_name,
                              const gchar* key, GError** error)
{
    g_return_val_if_fail(key_file != NULL, NULL);
...
gboolean g_key_file_remove_comment(GKeyFile* key_file, const gchar* group_name,
                                   const gchar* key, GError** error)
{
    g_return_val_if_fail(key_file != NULL, FALSE);

    if (group_name != NULL && key != NULL)
        return g_key_file_set_key_comment(key_file, group_name, key, NULL,
                                          error);
    else if (group_name != NULL)
 ...
 */
static gboolean g_key_file_has_key_full(GKeyFile* key_file,
                                        const gchar* group_name,
                                        const gchar* key, gboolean* has_key,
                                        GError** error)
{
    GKeyFileKeyValuePair* pair;
    GKeyFileGroup* group;
                            ...
                            const gchar* key, GError** error)
{
    ...
    if (g_key_file_has_key_full(key_file, group_name, key, &has_key,
                                &temp_error))
    {
        return has_key;
    }
                                                  ...
                                                  GList* pair_node)
{
    ...
    group->key_value_pairs =
        g_list_remove_link(group->key_value_pairs, pair_node);
...
static void g_key_file_remove_group_node(GKeyFile* key_file, GList* group_node)
{
    ...
    if (key_file->start_group == group)
    {
        ...
        while (tmp != NULL)
        {
            if (tmp != group_node && ((GKeyFileGroup*)tmp->data)->name != NULL)
                break;
 ...
 * Since: 2.6
 **/
gboolean g_key_file_remove_group(GKeyFile* key_file, const gchar* group_name,
                                 GError** error)
{
    GList* group_node;
 ...
 * Since: 2.6
 **/
gboolean g_key_file_remove_key(GKeyFile* key_file, const gchar* group_name,
                               const gchar* key, GError** error)
{
    GKeyFileGroup* group;
    GKeyFileKeyValuePair* pair;
...
static GKeyFileGroup* g_key_file_lookup_group(GKeyFile* key_file,
                                              const gchar* group_name)
{
    return (GKeyFileGroup*)g_hash_table_lookup(key_file->group_hash,
                                               group_name);
}
                                                    ...
                                                    const gchar* key)
{
    ...
    for (key_node = group->key_value_pairs; key_node != NULL;
...
static gboolean g_key_file_is_key_name(const gchar* name)
{
    ...
    {
        q++;
        while (*q && (g_unichar_isalnum(g_utf8_get_char_validated(q, -1)) ||
                      *q == '-' || *q == '_' || *q == '.' || *q == '@'))
            q = g_utf8_find_next_char(q, NULL);
                                              ...
                                              GError** error)
{
    ...
    if (int_value != long_value || errsv == ERANGE)
    {
        gchar* value_utf8 = g_utf8_make_valid(value, -1);
        g_set_error(error, G_KEY_FILE_ERROR, G_KEY_FILE_ERROR_INVALID_VALUE,
                    _("Integer value “%s” out of range"), value_utf8);
        g_free(value_utf8);
...
static gint strcmp_sized(const gchar* s1, size_t len1, const gchar* s2)
{
 ...
 * Since: 2.40
 */
gboolean g_key_file_save_to_file(GKeyFile* key_file, const gchar* filename,
                                 GError** error)
{
    gchar* contents;
    gboolean success;
