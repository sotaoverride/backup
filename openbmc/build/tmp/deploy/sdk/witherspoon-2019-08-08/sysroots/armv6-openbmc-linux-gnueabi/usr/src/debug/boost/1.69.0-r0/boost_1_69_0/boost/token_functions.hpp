...
template <class Char, class Traits = BOOST_DEDUCED_TYPENAME
                          std::basic_string<Char>::traits_type>
    ...
    void do_escape(iterator& next, iterator end, Token& tok)
    {
        if (++next == end)
            BOOST_THROW_EXCEPTION(
                escaped_list_error(std::string("cannot end with escape")));
        if (Traits::eq(*next, 'n'))
        {
            tok += '\n';
            return;
        ...
        else if (is_escape(*next))
        {
            ...
            BOOST_THROW_EXCEPTION(
                escaped_list_error(std::string("unknown escape sequence")));
    }
...
{
//===========================================================================
// Tokenizer was broken for wide character separators, at least on Windows,
// since CRT functions isspace etc only expect values in [0, 0xFF]. Debug build
// asserts if higher values are passed in. The traits extension class should
// take care of this. Assuming that the conditional will always get optimized
// out in the function implementations, argument types are not a problem since
// both forms of character classifiers expect an int.
...
class offset_separator
{
  ...
  public:
    ...
    offset_separator(Iter begin, Iter end, bool wrap_offsets = true,
        ...
        offsets_(begin, end),
        current_offset_(0), wrap_offsets_(wrap_offsets),
        return_partial_last_(return_partial_last)
    {
    }

    offset_separator() :
        offsets_(1, 1), current_offset_(), wrap_offsets_(true),
...
// The out of the box GCC 2.95 on cygwin does not have a char_traits class.
template <typename Char, typename Tr = BOOST_DEDUCED_TYPENAME
                             std::basic_string<Char>::traits_type>
class char_separator
{
  ...
  public:
    explicit char_separator(
        const Char* dropped_delims, const Char* kept_delims = 0,
        empty_token_policy empty_tokens = drop_empty_tokens) :
        m_dropped_delims(dropped_delims),
        m_use_ispunct(false), m_use_isspace(false),
        m_empty_tokens(empty_tokens), m_output_done(false)
    {
        // Borland workaround
        if (kept_delims)
    ...
    explicit char_separator() :
        m_use_ispunct(true), m_use_isspace(true),
        m_empty_tokens(drop_empty_tokens), m_output_done(false)
    {
    }
...
// The out of the box GCC 2.95 on cygwin does not have a char_traits class.
template <class Char, class Tr = BOOST_DEDUCED_TYPENAME
                          std::basic_string<Char>::traits_type>
