...
static const UnitActiveState state_translation_table[_SLICE_STATE_MAX] = {
    [SLICE_DEAD] = UNIT_INACTIVE, [SLICE_ACTIVE] = UNIT_ACTIVE};
...
static int slice_verify(Slice* s)
{
    ...
    {
        log_unit_error(UNIT(s), "Slice name %s is not valid. Refusing.",
                       UNIT(s)->id);
        return -ENOEXEC;
    }
    ...
    if (parent ? !unit_has_name(UNIT_DEREF(UNIT(s)->slice), parent)
               : UNIT_ISSET(UNIT(s)->slice))
    {
...
static int slice_load_root_slice(Unit* u)
{
    ...
    /* The root slice is a bit special. For example it is always running and
     * cannot be terminated. Because of its special semantics we synthesize it
     * here, instead of relying on the unit file on disk. */
...
static int slice_load_system_slice(Unit* u)
{
    ...
    /* The system slice is a bit special. For example it is always running and
     * cannot be terminated. Because of its special semantics we synthesize it
     * here, instead of relying on the unit file on disk. */
...
static int slice_deserialize_item(Unit* u, const char* key, const char* value,
                                  FDSet* fds)
{
    Slice* s = SLICE(u);
    ...
    if (!u)
    {
        ...
        if (r < 0)
            return log_error_errno(
                r, "Failed to allocate the special %s unit: %m", name);
    }
...
static void slice_enumerate_perpetual(Manager* m)
{
    ...
    if (r >= 0 && manager_owns_host_root_cgroup(m))
    {
        ...
        /* If we are managing the root cgroup then this means our root slice
         * covers the whole system, which means the kernel will track
         * CPU/tasks/memory for us anyway, and it is all available in /proc.
         * Let's hence turn accounting on here, so that our APIs to query this
         * data are available. */
