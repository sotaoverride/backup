    ...
    if (num)
    {
        for (i = 1; i <= num; i++)
        {
            ...
            else if (fingerprint == i)
            {
                ...
                BIO_printf(out,
                           "%s Fingerprint=", OBJ_nid2sn(EVP_MD_type(fdig)));
                for (j = 0; j < (int)n; j++)
                {
            ...
            else if ((sign_flag == i) && (x509req == 0))
            {
                ...
                if (Upkey == NULL)
                {
                    Upkey = load_key(keyfile, keyformat, 0, passin, e,
                                     "Private key");
                    if (Upkey == NULL)
                        goto end;
                }
            ...
            else if (CA_flag == i)
            {
                ...
                if (!x509_certify(ctx, CAfile, digest, x, xca, CApkey, sigopts,
            ...
            else if (x509req == i)
            {
                ...
                {
                    pk = load_key(keyfile, keyformat, 0, passin, e,
                                  "request key");
                    if (pk == NULL)
                        goto end;
                }
