...
G_DEFINE_TYPE_WITH_CODE(
    GNetworkMonitorNM, g_network_monitor_nm, G_TYPE_NETWORK_MONITOR_NETLINK,
    ...
    g_io_extension_point_implement(G_NETWORK_MONITOR_EXTENSION_POINT_NAME,
                                   g_define_type_id, "networkmanager", 30))

static void g_network_monitor_nm_init(GNetworkMonitorNM* nm)
{
    nm->priv = g_network_monitor_nm_get_instance_private(nm);
}

static void g_network_monitor_nm_get_property(GObject* object, guint prop_id,
...
static GNetworkConnectivity nm_conn_to_g_conn(int nm_state)
{
    switch (nm_state)
    {
...
static gboolean nm_metered_to_bool(guint nm_metered)
{
    switch (nm_metered)
    {
        case 1: /* yes */
...
static void sync_properties(GNetworkMonitorNM* nm, gboolean emit_signals)
{
    GVariant* v;
    NMState nm_state;
...
static void update_cached_property(GDBusProxy* proxy, const char* property_name,
                                   GVariantDict* dict)
{
    GVariant* v;
...
static void proxy_signal_cb(GDBusProxy* proxy, gchar* sender_name,
                            ...
                            GNetworkMonitorNM* nm)
{
    ...
    {
        g_warning(
            "Failed to handle PropertiesChanged signal from NetworkManager");
        return;
    }
...
static gboolean has_property(GDBusProxy* proxy, const char* property_name)
{
    char** props;
...
static gboolean g_network_monitor_nm_initable_init(GInitable* initable,
                                                   ...
                                                   GError** error)
{
    ...
    parent_iface = g_type_interface_peek_parent(
        G_NETWORK_MONITOR_NM_GET_INITABLE_IFACE(initable));
    ...
    g_signal_connect(G_OBJECT(proxy), "g-signal", G_CALLBACK(proxy_signal_cb),
                     nm);
    nm->priv->proxy = proxy;
...
static void g_network_monitor_nm_finalize(GObject* object)
{
    GNetworkMonitorNM* nm = G_NETWORK_MONITOR_NM(object);
...
static void g_network_monitor_nm_class_init(GNetworkMonitorNMClass* nl_class)
{
