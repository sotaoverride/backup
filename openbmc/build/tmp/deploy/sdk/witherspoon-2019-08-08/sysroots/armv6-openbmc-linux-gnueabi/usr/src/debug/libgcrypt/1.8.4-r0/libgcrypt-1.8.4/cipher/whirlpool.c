...
static unsigned int whirlpool_transform(void* ctx, const unsigned char* data,
                                        size_t nblks);

static void whirlpool_init(void* ctx, unsigned int flags)
{
...
extern unsigned int _gcry_whirlpool_transform_amd64(
    u64* state, const unsigned char* data, size_t nblks,
    const struct whirlpool_tables_s* tables) ASM_FUNC_ABI;
                                        ...
                                        size_t nblks)
{
    ...
    return _gcry_whirlpool_transform_amd64(context->hash_state, data, nblks,
                                           &tab) +
           ASM_EXTRA_STACK;
}
 ...
 * Transform block.
 */
static unsigned int whirlpool_transform_blk(void* ctx,
                                            const unsigned char* data)
{
    ...
    for (r = 0; r < R; r++)
    {
        ...
        block[0] =
            (C0[(key[0] >> 56) & 0xFF] ^ C1[(key[7] >> 48) & 0xFF] ^
...
static unsigned int whirlpool_transform(void* c, const unsigned char* data,
                                        size_t nblks)
{
    unsigned int burn;
...
static void whirlpool_add_bugemu(whirlpool_context_t* context,
...
/* Bug compatibility Whirlpool version.  */
static void whirlpool_final_bugemu(void* ctx)
{
    whirlpool_context_t* context = ctx;
    unsigned int i;
    ...
    memcpy(context->bctx.buf + context->bugemu.count, context->bugemu.length,
           32);
...
static void whirlpool_write(void* ctx, const void* buffer, size_t buffer_n)
{
    whirlpool_context_t* context = ctx;
...
static void whirlpool_final(void* ctx)
{
    whirlpool_context_t* context = ctx;
    unsigned int i;
    u64 t, th, lsb, msb;
...
static byte* whirlpool_read(void* ctx)
{
    whirlpool_context_t* context = ctx;
...
gcry_md_spec_t _gcry_digest_spec_whirlpool = {GCRY_MD_WHIRLPOOL,
                                              {0, 0},
                                              "WHIRLPOOL",
                                              NULL,
                                              0,
