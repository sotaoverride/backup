...
#define INIT_BLKPG_PARTITION(_partno, _start, _size)                           \
    {                                                                          \
        .pno = (_partno), .start = (_start) << 9, .length = (_size) << 9,      \
        .devname[0] = 0, .volname[0] = 0                                       \
    }

#define INIT_BLKPG_ARG(_action, _part)                                         \
    {                                                                          \
        .op = (_action), .flags = 0, .datalen = sizeof(*(_part)),              \
        .data = (_part)                                                        \
    }
...
static inline int partx_add_partition(int fd, int partno, uint64_t start,
                                      uint64_t size)
{
...
static inline int partx_resize_partition(int fd, int partno, uint64_t start,
                                         uint64_t size)
{
    struct blkpg_partition p = INIT_BLKPG_PARTITION(partno, start, size);
