...
void construct_path(char* name, int lenmax, off_t size, off_t pos, off_t* ppos)
{
    ...
    if (size < TREEDIRSIZE * TREEPAGESIZE)
    {
        ...
        snprintf(name, lenmax, "/FILE%04" PRIX64,
                 (pos / TREEPAGESIZE) % TREEDIRSIZE);
    ...
    else
    {
        ...
        char buffer[10];
        snprintf(buffer, sizeof(buffer), "/TREE%04jX",
                 (intmax_t)(*ppos % TREEDIRSIZE));
...
void mkdir_path(char* path)
{
    ...
    {
        *subpath = '\0'; // path is modified in place with terminating null char
                         // instead of slash
                  ...
                  pthread_mutex_t* mutex)
{
    ...
    DEBUG("Accessing treefile %s ( offset %llu of %llu)", filename,
