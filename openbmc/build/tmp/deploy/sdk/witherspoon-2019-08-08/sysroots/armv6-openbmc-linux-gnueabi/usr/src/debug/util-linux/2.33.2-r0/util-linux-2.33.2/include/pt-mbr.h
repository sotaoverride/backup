...
{
    return (struct dos_partition*)(mbr + MBR_PT_OFFSET +
                                   (i * sizeof(struct dos_partition)));
}
...
static inline void dos_partition_set_start(struct dos_partition* p,
                                           unsigned int n)
{
...
static inline void dos_partition_set_size(struct dos_partition* p,
                                          unsigned int n)
{
...
enum
{
    ...
    MBR_OS2_HIDDEN_DRIVE_PARTITION =
        0x84, /* also hibernation MS APM, Intel Rapid Start */
    ...
    MBR_ACRONIS_FAT32LBA_PARTITION =
        0xbc, /* Acronis Secure Zone with ipl for loader F11.SYS */
    ...
    MBR_LINUX_RAID_PARTITION = 0xfd,     /* Linux raid partition with autodetect
