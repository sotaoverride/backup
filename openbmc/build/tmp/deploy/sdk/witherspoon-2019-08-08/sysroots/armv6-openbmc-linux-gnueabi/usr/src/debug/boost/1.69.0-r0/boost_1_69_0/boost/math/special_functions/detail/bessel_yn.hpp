...
// Y_n(z) is the dominant solution, forward recurrence always OK (though
// unstable)
...
T bessel_yn(int n, T x, const Policy& pol)
{
    ...
    {
        factor =
            static_cast<T>((n & 0x1) ? -1 : 1); // Y_{-n}(z) = (-1)^n Y_n(z)
        n = -n;
    }
    else
    {
    ...
    if (x < policies::get_epsilon<T, Policy>())
    {
        ...
        if (tools::max_value<T>() * fabs(scale) < fabs(value))
            return boost::math::sign(scale) * boost::math::sign(value) *
    ...
    {
        value =
            factor * asymptotic_bessel_y_large_x_2(static_cast<T>(abs(n)), x);
    }
    else if (n == 0)
    {
    ...
    else
    {
        ...
        policies::check_series_iterations<T>(
            "boost::math::bessel_y_n<%1%>(%1%,%1%)", n, pol);
        T mult = 2 * k / x;
