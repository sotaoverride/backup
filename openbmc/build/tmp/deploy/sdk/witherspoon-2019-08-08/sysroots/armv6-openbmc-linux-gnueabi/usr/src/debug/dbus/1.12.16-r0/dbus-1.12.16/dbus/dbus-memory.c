    ...
    if (free_block != NULL)
    {
      ...
      _dbus_verbose ("Checking %d bytes request from source %s\n",
        ...
        while (i < GUARD_START_OFFSET)
        {
            ...
            {
                _dbus_warn("Block of %lu bytes from %s had start guard value "
                           "0x%ux at %d expected 0x%x",
                           (long)requested_bytes, source_string(source), value,
                           i, GUARD_VALUE);
                failed = TRUE;
            }
        ...
        while (i < (GUARD_START_OFFSET + requested_bytes + GUARD_END_PAD))
        {
            ...
            if (value != GUARD_VALUE)
            {
                _dbus_warn("Block of %lu bytes from %s had end guard value "
                           "0x%ux at %d expected 0x%x",
                           (long)requested_bytes, source_string(source), value,
                           i, GUARD_VALUE);
                failed = TRUE;
...
static void* set_guards(void* real_block, size_t requested_bytes,
                        BlockSource source)
{
...
void* dbus_malloc(size_t bytes)
{
    ...
    else if (guards)
    {
        ...
        {
            _dbus_warn("out of memory: malloc (%ld + %ld)", (long)bytes,
...
void* dbus_malloc0(size_t bytes)
{
    ...
    else if (guards)
    {
        ...
        {
            _dbus_warn("out of memory: calloc (%ld + %ld, 1)", (long)bytes,
                       (long)GUARD_EXTRA_SIZE);
            _dbus_abort();
        }
    ...
    else if (guards)
    {
        if (memory)
        {
            ...
            if (block == NULL)
            {
                ...
                {
                    _dbus_warn("out of memory: realloc (%p, %ld + %ld)", memory,
