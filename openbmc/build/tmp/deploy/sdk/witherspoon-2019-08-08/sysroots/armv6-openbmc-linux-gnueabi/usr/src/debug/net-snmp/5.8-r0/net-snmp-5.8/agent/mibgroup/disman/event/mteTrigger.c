...
void _mteTrigger_dump(void)
{
         ...
         row = netsnmp_tdata_row_next(trigger_table_data, row))
    {
        ...
        DEBUGMSGOID(
            ("disman:event:dump", row->oid_index.oids, row->oid_index.len));
        DEBUGMSG(("disman:event:dump", "(%s, %s)", row->indexes->val.string,
 ...
 */
netsnmp_tdata_row* mteTrigger_createEntry(const char* mteOwner, char* mteTName,
                                          int fixed)
{
    struct mteTrigger* entry;
    netsnmp_tdata_row* row;
    ...
    netsnmp_table_row_add_index(row, ASN_OCTET_STR, entry->mteOwner,
                                mteOwner_len);
    if (mteTName)
...
void mteTrigger_removeEntry(netsnmp_tdata_row* row)
{
    ...
    entry = (struct mteTrigger*)netsnmp_tdata_remove_and_delete_row(
 ...
 *
 * =================================================== */
const char* _ops[] = {"",   "!=", /* MTE_BOOL_UNEQUAL      */
                      "==",       /* MTE_BOOL_EQUAL        */
                      "<",        /* MTE_BOOL_LESS         */
                      "<=",       /* MTE_BOOL_LESSEQUAL    */
                      ">",        /* MTE_BOOL_GREATER      */
...
void mteTrigger_run(unsigned int reg, void* clientarg)
{
    ...
    {
        DEBUGMSGTL(
            ("disman:event:trigger:monitor", "Trigger query (%s) failed: %d\n",
    ...
    while (vp1)
    {
        ...
        if (vp2)
        {
            cmp = snmp_oid_compare(vp1->name, vp1->name_length, vp2->name,
                                   vp2->name_length);
            if (cmp < 0)
            {
                /*
        ...
        (entry->mteTExEvent[0] != '\0'))
    {
        ...
        if (!entry->old_results)
        {
            ...
            if (entry->mteTExTest & entry->mteTExStartup & MTE_EXIST_PRESENT)
            {
                for (vp1 = var; vp1; vp1 = vp1->next_variable)
                {
                    ...
                    DEBUGMSGOID(("disman:event:trigger:fire", vp1->name,
                                 vp1->name_length));
                    DEBUGMSG(("disman:event:trigger:fire", " (present)\n"));
            ...
            if (entry->mteTExTest & entry->mteTExStartup & MTE_EXIST_ABSENT)
            {
                    ...
                    var->type == ASN_NULL)
                {
                    DEBUGMSGTL(("disman:event:trigger:fire",
                                "Firing initial existence test: "));
                    DEBUGMSGOID(("disman:event:trigger:fire", var->name,
                                 var->name_length));
                    DEBUGMSG(("disman:event:trigger:fire", " (absent)\n"));
                    entry->mteTriggerXOwner = entry->mteTExObjOwner;
                    entry->mteTriggerXObjects = entry->mteTExObjects;
                    /*
        ...
        {
            for (vp1 = var, vp2 = entry->old_results; vp1;
                 vp1 = vp1->next_variable, vp2 = vp2->next_variable)
            {
                ...
                if ((entry->mteTExTest & MTE_EXIST_PRESENT) &&
                    (vp1->type != ASN_NULL) && (vp2->type == ASN_NULL))
                {
        ...
        (entry->mteTriggerTest & MTE_TRIGGER_THRESHOLD))
    {
        ...
        switch (var->type)
        {
            ...
            default:
                ...
                DEBUGMSGOID(
                    ("disman:event:trigger:fire", var->name, var->name_length));
        ...
        if (entry->flags & MTE_TRIGGER_FLAG_DELTA)
        {
            ...
            if (!(entry->flags & MTE_TRIGGER_FLAG_SYSUPT))
            {
                ...
                DEBUGMSG(
                    ("disman:event:delta", " %s\n",
                     (entry->flags & MTE_TRIGGER_FLAG_DWILD ? " (wild)" : "")));

                dvar = (netsnmp_variable_list*)SNMP_MALLOC_TYPEDEF(
            ...
            {
                DEBUGMSGTL(
                    ("disman:event:delta", "single discontinuity: (sysUT)\n"));
                snmp_free_varbind(entry->old_results);
            ...
            if (entry->flags & MTE_TRIGGER_FLAG_DWILD)
            {
                ...
                while (vp1)
                {
                     ...
                     */
                    cmp =
                        snmp_oid_compare(vp1->name + n, vp1->name_length - n,
                    ...
                    if (cmp < 0)
                    {
                         ...
                         */
                        vtmp = (netsnmp_variable_list*)SNMP_MALLOC_TYPEDEF(
                            netsnmp_variable_list);
                        if (!vtmp)
                        {
                            _mteTrigger_failure("failed to create mteTrigger "
                                                "discontinuity varbind");
                            snmp_free_varbind(dvar);
                            return;
                        }
        ...
        for (vp1 = var; vp1; vp1 = vp1->next_variable)
        {
            ...
            if (entry->flags & MTE_TRIGGER_FLAG_DELTA)
            {
                if (entry->flags & MTE_TRIGGER_FLAG_DWILD)
                {
                     ...
                     */
                    if ((dv1->type == ASN_NULL) || (dv1->type != dv2->type) ||
                        (*dv1->val.integer != *dv2->val.integer))
                    {
                         ...
                         */
                        DEBUGMSGTL(
                            ("disman:event:delta", "discontinuity occurred: "));
                ...
                if (vp2->type == ASN_NULL)
                {
                    ...
                    DEBUGMSGOID(
                        ("disman:event:delta", vp1->name, vp1->name_length));
                    DEBUGMSG(("disman:event:delta", " \n"));
                    vp2 = vp2->next_variable;
                    continue;
                }
            ...
            DEBUGMSGTL(("disman:event:delta",
                        "Bool comparison: (%ld %s %ld) %d\n", value,
                        _ops[entry->mteTBoolComparison], entry->mteTBoolValue,
                        cmp));
            ...
            if (cmp)
            {
                if (vp1->index & MTE_ARMED_BOOLEAN)
                {
                        ...
                        (entry->flags & MTE_TRIGGER_FLAG_BSTART))
                    {
                        ...
                        entry->mteTriggerFired = vp1;
                        n = entry->mteTriggerValueID_len;
                        mteEvent_fire(entry->mteTBoolEvOwner,
                                      entry->mteTBoolEvent, entry,
                                      vp1->name + n, vp1->name_length - n);
                    }
                }
            }
            else
         ...
         (entry->mteTThDFallEvent[0] != '\0')))
    {
        ...
        {
            DEBUGMSGTL(
                ("disman:event:trigger", "Delta-threshold on delta-sample\n"));
        }
        else if (vp2 != NULL)
        {
            for (vp1 = var; vp1; vp1 = vp1->next_variable)
            {
                ...
                if ((!vp1->val.integer) || (vp2->type == ASN_NULL))
                {
                    vp2 = vp2->next_variable;
                    continue;
                }
...
void mteTrigger_enable(struct mteTrigger* entry)
{
    ...
    if (entry->mteTriggerFrequency)
    {
        ...
        entry->alarm = snmp_alarm_register(entry->mteTriggerFrequency,
                                           SA_REPEAT, mteTrigger_run, entry);
    }
}
