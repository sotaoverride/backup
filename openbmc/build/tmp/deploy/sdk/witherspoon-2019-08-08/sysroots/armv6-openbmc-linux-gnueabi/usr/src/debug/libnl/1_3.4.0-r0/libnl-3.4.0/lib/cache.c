...
struct nl_object* nl_cache_get_prev(struct nl_object* obj)
{
    if (nl_list_at_head(obj, &obj->ce_cache->c_items, ce_list))
        ...
        return nl_list_entry(obj->ce_list.prev, struct nl_object, ce_list);
}
                                 ...
                                 struct nl_object* filter)
{
    ...
    NL_DBG(2, "Filling subset of cache %p <%s> with filter %p into %p\n", orig,
           nl_cache_name(orig), filter, cache);
...
void nl_cache_get(struct nl_cache* cache)
{
    ...
    NL_DBG(3, "Incremented cache %p <%s> reference count to %d\n", cache,
           nl_cache_name(cache), cache->c_refcnt);
}
...
static int __cache_add(struct nl_cache* cache, struct nl_object* obj)
{
    ...
    NL_DBG(3, "Added object %p to cache %p <%s>, nitems %d\n", obj, cache,
           nl_cache_name(cache), cache->c_nitems);
        ...
        case NL_ACT_DEL:
            ...
            if (old)
            {
                ...
                if (nl_object_update(old, obj) == 0)
                {
                    if (cb_v2)
                    {
                        cb_v2(cache, clone, obj, diff, NL_ACT_CHANGE, data);
                        nl_object_put(clone);
                    }
                    else if (cb)
                ...
                {
                    if (cb_v2)
                        cb_v2(cache, old, NULL, 0, NL_ACT_DEL, data);
                    else if (cb)
            ...
            if (type->mt_act == NL_ACT_NEW)
            {
                ...
                if (old == NULL)
                {
                    if (cb_v2)
                    {
                        cb_v2(cache, NULL, obj, 0, NL_ACT_NEW, data);
                    }
                    else if (cb)
                ...
                else if (old)
                {
                    ...
                    if (diff && cb_v2)
                    {
                        cb_v2(cache, old, obj, diff, NL_ACT_CHANGE, data);
                    }
                    else if (diff && cb)
                     ...
                     change_func_t change_cb, void* data)
{
    ...
    for (i = 0; ops->co_msgtypes[i].mt_id >= 0; i++)
        if (ops->co_msgtypes[i].mt_id == obj->ce_msgtype)
            return cache_include(cache, obj, &ops->co_msgtypes[i], change_cb,
                                 NULL, data);
                        ...
                        change_func_v2_t change_cb, void* data)
{
    ...
    for (i = 0; ops->co_msgtypes[i].mt_id >= 0; i++)
        if (ops->co_msgtypes[i].mt_id == obj->ce_msgtype)
            return cache_include(cache, obj, &ops->co_msgtypes[i], NULL,
                                 change_cb, data);
                    ...
                    change_func_t change_cb, void* data)
{
    ...
    do
    {
        if (grp && grp->ag_group && (cache->c_flags & NL_CACHE_AF_ITER))
    ...
    restart:
        ...
        if (grp)
            grp++;
    } while (grp && grp->ag_group && (cache->c_flags & NL_CACHE_AF_ITER));
...
int nl_cache_refill(struct nl_sock* sk, struct nl_cache* cache)
{
    ...
    restart:
        ...
        NL_DBG(2,
               "Updating cache %p <%s> for family %u, request sent, waiting "
               "for reply\n",
                                ...
                                struct nl_object* filter)
{
    ...
    if ((nl_object_get_id_attrs(filter) == filter->ce_mask) && cache->hashtable)
...
void nl_cache_mark_all(struct nl_cache* cache)
{
    ...
    nl_list_for_each_entry(obj, &cache->c_items, ce_list) nl_object_mark(obj);
}
 ...
 */
void nl_cache_dump_filter(struct nl_cache* cache, struct nl_dump_params* params,
                          struct nl_object* filter)
{
