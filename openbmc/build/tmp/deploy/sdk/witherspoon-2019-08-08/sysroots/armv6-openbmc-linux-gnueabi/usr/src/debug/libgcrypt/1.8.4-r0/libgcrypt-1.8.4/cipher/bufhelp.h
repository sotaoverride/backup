...
static inline void buf_cpy(void* _dst, const void* _src, size_t len)
{
...
static inline void buf_xor(void* _dst, const void* _src1, const void* _src2,
                           size_t len)
{
    byte* dst = _dst;
    ...
    /* Skip fast processing if buffers are unaligned.  */
    if (UNLIKELY(((uintptr_t)dst | (uintptr_t)src1 | (uintptr_t)src2) &
                 longmask))
        goto do_bytes;
#endif
...
static inline void buf_xor_1(void* _dst, const void* _src, size_t len)
{
    byte* dst = _dst;
    const byte* src = _src;
    bufhelp_int_t* ldst;
...
static inline void buf_xor_2dst(void* _dst1, void* _dst2, const void* _src,
                                size_t len)
{
    byte* dst1 = _dst1;
    byte* dst2 = _dst2;
    ...
    /* Skip fast processing if buffers are unaligned.  */
    if (UNLIKELY(((uintptr_t)src | (uintptr_t)dst1 | (uintptr_t)dst2) &
                 longmask))
        goto do_bytes;
#endif
...
/* Optimized function for combined buffer xoring and copying.  Used by mainly
   CFB mode decryption.  */
static inline void buf_xor_n_copy(void* _dst_xor, void* _srcdst_cpy,
...
static inline int buf_eq_const(const void* _a, const void* _b, size_t len)
{
    const byte* a = _a;
    const byte* b = _b;
    int ab, ba;
...
{
    const byte* in = _buf;
    return ((u32)in[0] << 24) | ((u32)in[1] << 16) | ((u32)in[2] << 8) |
           (u32)in[3];
}
...
{
    const byte* in = _buf;
    return ((u32)in[3] << 24) | ((u32)in[2] << 16) | ((u32)in[1] << 8) |
           (u32)in[0];
}
...
{
    const byte* in = _buf;
    return ((u64)in[0] << 56) | ((u64)in[1] << 48) | ((u64)in[2] << 40) |
           ((u64)in[3] << 32) | ((u64)in[4] << 24) | ((u64)in[5] << 16) |
           ((u64)in[6] << 8) | (u64)in[7];
}
...
{
    const byte* in = _buf;
    return ((u64)in[7] << 56) | ((u64)in[6] << 48) | ((u64)in[5] << 40) |
           ((u64)in[4] << 32) | ((u64)in[3] << 24) | ((u64)in[2] << 16) |
           ((u64)in[1] << 8) | (u64)in[0];
}
