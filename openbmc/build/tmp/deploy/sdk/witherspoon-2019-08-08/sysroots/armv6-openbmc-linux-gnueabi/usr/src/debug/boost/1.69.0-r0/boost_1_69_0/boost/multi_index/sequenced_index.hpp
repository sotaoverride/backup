...
#pragma parse_mfunc_templ off
  ...
  protected:
    ...
    typedef tuples::cons<ctor_args, typename super::ctor_args_list>
        ctor_args_list;
    typedef typename mpl::push_front<typename super::index_type_list,
  ...
  private:
#if defined(BOOST_MULTI_INDEX_ENABLE_SAFE_MODE)
    typedef safe_mode::safe_container<sequenced_index> safe_super;
#endif
  ...
  public:
    ...
    sequenced_index<SuperMeta, TagList>&
        operator=(const sequenced_index<SuperMeta, TagList>& x)
    {
        this->final() = x.final();
        return *this;
    }
    ...
    BOOST_MULTI_INDEX_OVERLOADS_TO_VARTEMPL(emplace_return_type, emplace_front,
                                            emplace_front_impl)
    ...
    BOOST_MULTI_INDEX_OVERLOADS_TO_VARTEMPL(emplace_return_type, emplace_back,
                                            emplace_back_impl)
    ...
    BOOST_MULTI_INDEX_OVERLOADS_TO_VARTEMPL_EXTRA_ARG(emplace_return_type,
                                                      emplace, emplace_impl,
                                                      iterator, position)
    ...
    std::pair<iterator, bool> insert(iterator position,
    ...
    void insert(iterator position, InputIterator first, InputIterator last)
    {
        insert_iter(position, first, last,
                    mpl::not_<is_integral<InputIterator>>());
    }
                ...
                iterator i)
    {
        ...
        else
        {
            if (insert(position, *i).second)
            {
                 ...
                 * following workaround is needed. Left it for all compilers as
                 * it does no harm.
                 */
                i.detach();
                ...
                iterator first, iterator last)
    {
        ...
        if (&x == this)
        {
            ...
            if (position != last)
                relink(position.get_node(), first.get_node(), last.get_node());
        }
        else
        {
            while (first != last)
    ...
    {
        sequenced_index_remove(*this, ::boost::bind(std::equal_to<value_type>(),
                                                    ::boost::arg<1>(), value));
    }
    ...
    BOOST_MULTI_INDEX_PROTECTED_IF_MEMBER_TEMPLATE_FRIENDS
        : sequenced_index(const ctor_args_list& args_list,
    ...
    void copy_(const sequenced_index<SuperMeta, TagList>& x,
               const copy_map_type& map)
    {
        ...
        do
        {
            ...
            node_type* next_cpy =
                map.find(static_cast<final_node_type*>(next_org));
    ...
    {
        for (node_type* x = node_type::from_impl(header()->next());
             x != header();)
        {
    ...
    {
        lm.load(::boost::bind(&sequenced_index::rearranger, this,
  ...
  private:
    ...
    std::pair<iterator, bool>
        emplace_front_impl(BOOST_MULTI_INDEX_FUNCTION_PARAM_PACK)
    {
    ...
    template <BOOST_MULTI_INDEX_TEMPLATE_PARAM_PACK>
    std::pair<iterator, bool>
        emplace_back_impl(BOOST_MULTI_INDEX_FUNCTION_PARAM_PACK)
    {
    ...
    template <BOOST_MULTI_INDEX_TEMPLATE_PARAM_PACK>
    std::pair<iterator, bool>
        emplace_impl(iterator position, BOOST_MULTI_INDEX_FUNCTION_PARAM_PACK)
    {
        BOOST_MULTI_INDEX_CHECK_VALID_ITERATOR(position);
        BOOST_MULTI_INDEX_CHECK_IS_OWNER(position, *this);
        BOOST_MULTI_INDEX_SEQ_INDEX_CHECK_INVARIANT;
...
template <typename SuperMeta1, typename TagList1, typename SuperMeta2,
          typename TagList2>
