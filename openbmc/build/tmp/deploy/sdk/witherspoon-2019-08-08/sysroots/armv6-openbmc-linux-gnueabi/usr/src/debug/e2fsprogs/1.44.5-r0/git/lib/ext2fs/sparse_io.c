                                          ...
                                          struct sparse_map* sm, io_channel io)
{
    ...
    sm->blocks_count =
        (sparse_file_len(sparse_file, 0, 0) - 1) / sm->block_size + 1;
                                      ...
                                      int flags, io_channel io)
{
    ...
    if (flags & IO_FLAG_RW)
    {
        ...
        {
            sm->fd =
                open(params->file, O_CREAT | O_RDWR | O_TRUNC | O_BINARY, 0644);
            if (sm->fd < 0)
            {
                retval = errno;
                goto err_open;
...
static errcode_t sparse_read_blk64(io_channel channel, blk64_t block, int count,
                                   void* buf)
{
    int i;
    char* out = buf;
    ...
    else
    {
        for (i = 0; i < count; ++i)
        {
            cur_block = block_to_sparse_block(block + i, &offset, channel, sm);
            if (sm->blocks[cur_block])
                memcpy(out + (i * channel->block_size),
                       sm->blocks[cur_block] + offset, channel->block_size);
            else if (sm->blocks)
                memset(out + (i * channel->block_size), 0, channel->block_size);
        }
    }
    return 0;
}
    ...
    else
    {
        for (i = 0; i < count; ++i)
        {
            ...
            memcpy(sm->blocks[cur_block] + offset,
                   in + (i * channel->block_size), channel->block_size);
        }
    }
    return 0;
}
                                ...
                                blk64_t blk, unsigned long long count)
{
    ...
    for (unsigned long long i = 0; i < count; ++i)
    {
        if (blk + i >= sm->blocks_count)
            return 0;
        cur_block = block_to_sparse_block(blk + i, &offset, channel, sm);
        if (!sm->blocks[cur_block])
            continue;
...
static errcode_t sparse_cache_readahead(io_channel channel
