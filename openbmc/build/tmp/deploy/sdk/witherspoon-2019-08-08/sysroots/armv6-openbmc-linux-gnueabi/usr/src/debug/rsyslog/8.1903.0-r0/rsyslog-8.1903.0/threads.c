...
static rsRetVal thrdTerminateNonCancel(thrdInfo_t* pThis)
{
    ...
    while (was_active)
    {
        ...
        ret = d_pthread_cond_timedwait(&pThis->condThrdTerm, &pThis->mutThrd,
                                       &tTimeout);
        if (ret == ETIMEDOUT)
        {
        ...
        else if (ret != 0)
        {
            ...
            DBGPRINTF(
                "input thread term: cond_wait returned with error %d: %s\n",
                err, errStr);
        }
        was_active = pThis->bIsActive;
    }
    ...
    {
        DBGPRINTF(
            "non-cancel input thread termination FAILED for thread %s %p\n",
            pThis->name, (void*)pThis->thrdID);
    }
    else
    {
        DBGPRINTF(
            "non-cancel input thread termination succeeded for thread %s %p\n",
            pThis->name, (void*)pThis->thrdID);
    }
...
static ATTR_NORETURN void* thrdStarter(void* const arg)
{
     ...
     * regain control until the user code is finished, in which case we
    ...
    if (iRet == RS_RET_OK)
    {
        dbgprintf("thrdStarter: usrThrdMain %s - 0x%lx returned with iRet %d, "
                  "exiting now.\n",
                  pThis->name, (unsigned long)pThis->thrdID, iRet);
    }
    else
    {
        LogError(0, iRet, "main thread of %s terminated abnormally",
                 pThis->name);
    }
...
rsRetVal thrdCreate(rsRetVal (*thrdMain)(thrdInfo_t*),
                    rsRetVal (*afterRun)(thrdInfo_t*), sbool bNeedsCancel,
                    uchar* name)
{
    DEFiRet;
    thrdInfo_t* pThis;
