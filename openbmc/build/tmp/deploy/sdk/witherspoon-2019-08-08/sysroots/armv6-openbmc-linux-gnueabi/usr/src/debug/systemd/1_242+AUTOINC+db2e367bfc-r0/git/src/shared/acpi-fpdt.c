...
int acpi_get_boot_usec(usec_t* loader_start, usec_t* loader_exit)
{
    ...
    for (rec =
             (struct acpi_fpdt_header*)(buf + sizeof(struct acpi_table_header));
         (char*)rec < buf + l;
    ...
    if (hbrec.length <
        sizeof(struct acpi_fpdt_boot_header) + sizeof(struct acpi_fpdt_boot))
        return -EINVAL;

    l = pread(fd, &brec, sizeof(struct acpi_fpdt_boot),
              ptr + sizeof(struct acpi_fpdt_boot_header));
