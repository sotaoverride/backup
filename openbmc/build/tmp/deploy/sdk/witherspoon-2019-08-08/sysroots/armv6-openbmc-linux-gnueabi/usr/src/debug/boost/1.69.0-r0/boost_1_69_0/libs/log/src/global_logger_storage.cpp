...
BOOST_LOG_API shared_ptr<logger_holder_base>
    global_storage::get_or_init(typeindex::type_index key,
                                initializer_t initializer)
{
    ...
    BOOST_LOG_EXPR_IF_MT(
        log::aux::exclusive_lock_guard<mutex> lock(repo.m_Mutex);)
                        ...
                        logger_holder_base const& registered)
{
    ...
    if (log::aux::snprintf(buf, sizeof(buf), "%u",
                           registered.m_RegistrationLine) < 0)
        buf[0] = '\0';
    std::string str =
        std::string("Could not initialize global logger with tag \"") +
        tag_type.pretty_name() + "\" and type \"" + logger_type.pretty_name() +
        "\". A logger of type \"" + registered.m_LoggerType.pretty_name() +
