...
static int word_compare(const void* s1, const void* s2)
{
...
static int init_dict(dict_moddata dict, const char* dict_file)
{
    int fd;
    size_t len, i;
    char *p, *t;
...
static void destroy_dict(dict_moddata dict)
{
    if (dict == NULL)
        return;
    free(dict->word_list);
...
static krb5_error_code dict_open(krb5_context context, const char* dict_file,
...
static krb5_error_code dict_check(krb5_context context,
                                  krb5_pwqual_moddata data,
...
static void dict_close(krb5_context context, krb5_pwqual_moddata data)
{
