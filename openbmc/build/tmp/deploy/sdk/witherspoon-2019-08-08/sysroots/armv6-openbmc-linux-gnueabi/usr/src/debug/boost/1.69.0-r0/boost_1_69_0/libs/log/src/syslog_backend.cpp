...
struct syslog_backend::implementation
{
    ...
    //! Logging facility (portable or native, depending on the backend
    //! implementation)
    const int m_Facility;

    //! Constructor
    explicit implementation(int facility) : m_Facility(facility)
    {
    }
    //! Virtual destructor
    ...
    virtual void send(syslog::level lev,
                      string_type const& formatted_message) = 0;
};
...
BOOST_LOG_ANONYMOUS_NAMESPACE
{
...
#endif
    {
        ...
        typedef log::aux::lazy_singleton<native_syslog_initializer, mutex>
            mutex_holder;
#endif
      ...
      public:
        ...
        {
            ::openlog((ident.empty() ? static_cast<const char*>(NULL)
                                     : ident.c_str()),
                      0, facility);
        }
        ...
        static shared_ptr<native_syslog_initializer>
            get_instance(std::string const& ident, int facility)
        {
#if !defined(BOOST_LOG_NO_THREADS)
...
struct syslog_backend::implementation::native : public implementation
{
    ...
    native(syslog::facility const& fac, std::string const& ident) :
        ...
        m_pSyslogInitializer(
            native_syslog_initializer::get_instance(ident, this->m_Facility))
    {
    }
    ...
    void send(syslog::level lev, string_type const& formatted_message)
    {
        ...
        switch (lev)
        {
            case syslog::emergency:
                native_level = LOG_EMERG;
                break;
            case syslog::alert:
                native_level = LOG_ALERT;
                break;
            case syslog::critical:
                native_level = LOG_CRIT;
                break;
            case syslog::error:
                native_level = LOG_ERR;
                break;
            case syslog::warning:
                native_level = LOG_WARNING;
                break;
            case syslog::notice:
                native_level = LOG_NOTICE;
                break;
            case syslog::debug:
                native_level = LOG_DEBUG;
                break;
            default:
                native_level = LOG_INFO;
                break;
        }

        ::syslog(this->m_Facility | native_level, "%s",
...
#define LOG_FTP LOG_DAEMON
        ...
        BOOST_ASSERT(n <
                     sizeof(native_facilities) / sizeof(*native_facilities));
...
BOOST_LOG_ANONYMOUS_NAMESPACE
{
    ...
    struct syslog_udp_socket
    {
      ...
      public:
        //! The constructor creates a socket bound to the specified local
        //! address and port
        explicit syslog_udp_socket(
            asio::io_context& io_ctx, asio::ip::udp const& protocol,
        ...
        void send_message(int pri, const char* local_host_name,
                          asio::ip::udp::endpoint const& target,
                          const char* message);
    ...
    class syslog_udp_service
        : public log::aux::lazy_singleton<syslog_udp_service,
                                          shared_ptr<syslog_udp_service>>
    {
        friend class log::aux::lazy_singleton<syslog_udp_service,
                                              shared_ptr<syslog_udp_service>>;
        typedef log::aux::lazy_singleton<syslog_udp_service,
                                         shared_ptr<syslog_udp_service>>
            base_type;
    ...
    //! The method sends the syslog message to the specified endpoint
    void syslog_udp_socket::send_message(int pri, const char* local_host_name,
                                         asio::ip::udp::endpoint const& target,
                                         const char* message)
    {
        ...
        // Month will have to be injected separately, as involving locale won't
        // do here
        static const char months[12][4] = {"Jan", "Feb", "Mar", "Apr",
                                           "May", "Jun", "Jul", "Aug",
                                           "Sep", "Oct", "Nov", "Dec"};

        // The packet size is mandated in RFC3164, plus one for the terminating
        // zero
        char packet[1025];
        int n = boost::log::aux::snprintf(
            packet, sizeof(packet), "<%d>%s %2d %02d:%02d:%02d %s %s", pri,
            months[time_stamp->tm_mon], time_stamp->tm_mday,
            time_stamp->tm_hour, time_stamp->tm_min, time_stamp->tm_sec,
            local_host_name, message);
        if (BOOST_LIKELY(n > 0))
        {
            std::size_t packet_size =
                static_cast<std::size_t>(n) >= sizeof(packet)
                    ? sizeof(packet) - 1u
...
struct syslog_backend::implementation::udp_socket_based : public implementation
{
    //! Protocol to be used
    ...
    //! Constructor
    explicit udp_socket_based(syslog::facility const& fac,
                              asio::ip::udp const& protocol) :
        implementation(fac),
        m_Protocol(protocol), m_pService(syslog_udp_service::get())
    {
        if (m_Protocol == asio::ip::udp::v4())
        {
            m_TargetHost = asio::ip::udp::endpoint(
                asio::ip::address_v4(0x7F000001), 514); // 127.0.0.1:514
        }
        else
        {
            // ::1, port 514
            asio::ip::address_v6::bytes_type addr;
            std::fill_n(addr.data(), addr.size() - 1u,
                        static_cast<unsigned char>(0u));
            addr[addr.size() - 1u] = 1u;
            m_TargetHost =
                asio::ip::udp::endpoint(asio::ip::address_v6(addr), 514);
        }
    }
    ...
    void send(syslog::level lev, string_type const& formatted_message)
    {
        if (!m_pSocket.get())
        {
            ...
            m_pSocket.reset(new syslog_udp_socket(
                m_pService->m_IOContext, m_Protocol, any_local_address));
        }

        m_pSocket->send_message(this->m_Facility | static_cast<int>(lev),
                                m_pService->m_LocalHostName.c_str(),
                                m_TargetHost, formatted_message.c_str());
    }
};
...
//! The method installs the function object that maps application severity
//! levels to Syslog levels
BOOST_LOG_API void
    syslog_backend::set_severity_mapper(severity_mapper_type const& mapper)
{
    m_pImpl->m_LevelMapper = mapper;
}

//! The method writes the message to the sink
BOOST_LOG_API void syslog_backend::consume(record_view const& rec,
                                           string_type const& formatted_message)
{
    m_pImpl->send(m_pImpl->m_LevelMapper.empty() ? syslog::info
                                                 : m_pImpl->m_LevelMapper(rec),
                  formatted_message);
}

//! The method creates the backend implementation
BOOST_LOG_API void syslog_backend::construct(syslog::facility fac,
...
BOOST_LOG_API void syslog_backend::set_local_address(std::string const& addr,
                                                     unsigned short port)
{
#if !defined(BOOST_LOG_NO_THREADS)
    typedef implementation::udp_socket_based udp_socket_based_impl;
    if (udp_socket_based_impl* impl =
            dynamic_cast<udp_socket_based_impl*>(m_pImpl))
    {
        char service_name[std::numeric_limits<int>::digits10 + 3];
        boost::log::aux::snprintf(service_name, sizeof(service_name), "%d",
        ...
        asio::ip::udp::endpoint local_address;
        {
            ...
            asio::ip::udp::resolver::results_type results =
                impl->m_pService->m_HostNameResolver.resolve(
                    addr, service_name,
        ...
        impl->m_pSocket.reset(new syslog_udp_socket(
            impl->m_pService->m_IOContext, impl->m_Protocol, local_address));
    }
#else
    // Boost.ASIO requires threads for the host name resolver,
    // so without threads we simply assume the string already contains IP
    // address
...
//! The method sets the local address which log records will be sent from.
BOOST_LOG_API void
    syslog_backend::set_local_address(boost::asio::ip::address const& addr,
                                      unsigned short port)
{
    typedef implementation::udp_socket_based udp_socket_based_impl;
    if (udp_socket_based_impl* impl =
    ...
    {
        impl->m_pSocket.reset(new syslog_udp_socket(
            impl->m_pService->m_IOContext, impl->m_Protocol,
            asio::ip::udp::endpoint(addr, port)));
    }
}

//! The method sets the address of the remote host where log records will be
//! sent to.
BOOST_LOG_API void syslog_backend::set_target_address(std::string const& addr,
                                                      unsigned short port)
{
#if !defined(BOOST_LOG_NO_THREADS)
    typedef implementation::udp_socket_based udp_socket_based_impl;
...
//! sent to.
BOOST_LOG_API void
    syslog_backend::set_target_address(boost::asio::ip::address const& addr,
                                       unsigned short port)
{
    typedef implementation::udp_socket_based udp_socket_based_impl;
    if (udp_socket_based_impl* impl =
