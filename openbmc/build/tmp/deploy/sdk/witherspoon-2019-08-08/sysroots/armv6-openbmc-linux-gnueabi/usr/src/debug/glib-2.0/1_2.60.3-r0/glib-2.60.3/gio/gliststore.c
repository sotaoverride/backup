...
static void g_list_store_items_changed(GListStore* store, guint position,
...
static void g_list_store_dispose(GObject* object)
{
...
static void g_list_store_get_property(GObject* object, guint property_id,
...
static void g_list_store_set_property(GObject* object, guint property_id,
...
static void g_list_store_class_init(GListStoreClass* klass)
{
    ...
    g_object_class_install_property(
        object_class, PROP_ITEM_TYPE,
        g_param_spec_gtype("item-type", "", "", G_TYPE_OBJECT,
                           G_PARAM_CONSTRUCT_ONLY | G_PARAM_READWRITE |
                               G_PARAM_STATIC_STRINGS));
}

static GType g_list_store_get_item_type(GListModel* list)
{
...
static guint g_list_store_get_n_items(GListModel* list)
{
    GListStore* store = G_LIST_STORE(list);
...
static gpointer g_list_store_get_item(GListModel* list, guint position)
{
    GListStore* store = G_LIST_STORE(list);
...
static void g_list_store_init(GListStore* store)
{
...
GListStore* g_list_store_new(GType item_type)
{
    ...
    return g_object_new(G_TYPE_LIST_STORE, "item-type", item_type, NULL);
}
 ...
 * Since: 2.44
 */
guint g_list_store_insert_sorted(GListStore* store, gpointer item,
                                 ...
                                 gpointer user_data)
{
    ...
    it = g_sequence_insert_sorted(store->items, g_object_ref(item),
 ...
 * Since: 2.46
 */
void g_list_store_sort(GListStore* store, GCompareDataFunc compare_func,
                       gpointer user_data)
{
    gint n_items;
 ...
 * @additions: (array length=n_additions) (element-type GObject): the items to
 * add
 ...
 * Since: 2.44
 */
void g_list_store_splice(GListStore* store, guint position, guint n_removals,
                         gpointer* additions, guint n_additions)
{
    ...
    if (n_additions)
    {
        ...
        for (i = 0; i < n_additions; i++)
        {
            if
                G_UNLIKELY(
                    !g_type_is_a(G_OBJECT_TYPE(additions[i]), store->item_type))
                {
                    g_critical("%s: item %d is a %s instead of a %s.  "
                               "GListStore is now in an undefined state.",
                               G_STRFUNC, i, G_OBJECT_TYPE_NAME(additions[i]),
