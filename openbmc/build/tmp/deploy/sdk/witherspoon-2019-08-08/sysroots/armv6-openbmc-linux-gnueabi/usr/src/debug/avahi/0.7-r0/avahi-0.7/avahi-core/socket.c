...
static void ipv4_address_to_sockaddr(struct sockaddr_in* ret_sa,
                                     const AvahiIPv4Address* a, uint16_t port)
{
    assert(ret_sa);
    assert(a);
    assert(port > 0);
...
static void ipv6_address_to_sockaddr(struct sockaddr_in6* ret_sa,
                                     const AvahiIPv6Address* a, uint16_t port)
{
    assert(ret_sa);
    assert(a);
    assert(port > 0);
...
int avahi_mdns_mcast_join_ipv4(int fd, const AvahiIPv4Address* a, int idx,
                               int join)
{
...
int avahi_mdns_mcast_join_ipv6(int fd, const AvahiIPv6Address* a, int idx,
                               int join)
{
    ...
    if (join)
        setsockopt(fd, IPPROTO_IPV6, IPV6_DROP_MEMBERSHIP, &mreq6,
                   sizeof(mreq6));

    if (setsockopt(fd, IPPROTO_IPV6,
                   join ? IPV6_ADD_MEMBERSHIP : IPV6_DROP_MEMBERSHIP, &mreq6,
                   sizeof(mreq6)) < 0)
    {
        avahi_log_warn("%s failed: %s",
...
int avahi_open_socket_ipv6(int no_reuse)
{
    ...
    ttl = 255;
    if (setsockopt(fd, IPPROTO_IPV6, IPV6_MULTICAST_HOPS, &ttl, sizeof(ttl)) <
        0)
    {
    ...
    yes = 1;
    if (setsockopt(fd, IPPROTO_IPV6, IPV6_MULTICAST_LOOP, &yes, sizeof(yes)) <
        0)
    {
...
static int sendmsg_loop(int fd, struct msghdr* msg, int flags)
{
    ...
    for (;;)
    {
        ...
        if (errno != EAGAIN)
        {
            ...
            avahi_log_debug("sendmsg() to %s failed: %s", where,
                            strerror(errno));
            return -1;
        }
...
int avahi_send_dns_packet_ipv4(int fd, AvahiIfIndex interface,
                               AvahiDnsPacket* p,
                               ...
                               uint16_t dst_port)
{
    ...
    struct cmsghdr* cmsg;
    size_t
        cmsg_data[(CMSG_SPACE(sizeof(struct in_pktinfo)) / sizeof(size_t)) + 1];
    ...
    if (src_address)
    {
        ...
        if (setsockopt(fd, IPPROTO_IP, IP_MULTICAST_IF,
                       src_address ? &src_address->address : &any,
...
int avahi_send_dns_packet_ipv6(int fd, AvahiIfIndex interface,
                               AvahiDnsPacket* p,
                               ...
                               uint16_t dst_port)
{
    ...
    struct iovec io;
    struct cmsghdr* cmsg;
    size_t cmsg_data[(CMSG_SPACE(sizeof(struct in6_pktinfo)) / sizeof(size_t)) +
                     1];
    ...
    if (interface > 0 || src_address)
    {
        ...
        if (src_address)
            memcpy(&pkti->ipi6_addr, src_address->address,
                   sizeof(src_address->address));
    }
    else
    {
        msg.msg_control = NULL;
...
AvahiDnsPacket* avahi_recv_dns_packet_ipv4(int fd,
...
AvahiDnsPacket* avahi_recv_dns_packet_ipv6(int fd,
        ...
        if (cmsg->cmsg_level == IPPROTO_IPV6)
        {
            ...
            switch (cmsg->cmsg_type)
            {
                ...
                case IPV6_PKTINFO:
                {
                    ...
                    if (ret_dst_address)
                        memcpy(ret_dst_address->address, i->ipi6_addr.s6_addr,
                               16);
