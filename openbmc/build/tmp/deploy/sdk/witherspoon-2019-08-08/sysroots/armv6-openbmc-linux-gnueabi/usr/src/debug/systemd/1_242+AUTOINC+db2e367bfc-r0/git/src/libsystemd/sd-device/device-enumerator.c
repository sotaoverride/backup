...
static sd_device_enumerator*
    device_enumerator_free(sd_device_enumerator* enumerator)
{
    size_t i;
...
DEFINE_PUBLIC_TRIVIAL_REF_UNREF_FUNC(sd_device_enumerator, sd_device_enumerator,
...
static void
    device_enumerator_clear_match_parent(sd_device_enumerator* enumerator)
{
    if (!enumerator)
        return;
...
int device_enumerator_add_match_parent_incremental(
    sd_device_enumerator* enumerator, sd_device* parent)
{
    const char* path;
    int r;
...
int device_enumerator_add_device(sd_device_enumerator* enumerator,
                                 sd_device* device)
{
    assert_return(enumerator, -EINVAL);
    assert_return(device, -EINVAL);

    if (!GREEDY_REALLOC(enumerator->devices, enumerator->n_allocated,
                        enumerator->n_devices + 1))
        return -ENOMEM;
...
static bool match_sysattr_value(sd_device* device, const char* sysattr,
...
static bool match_subsystem(sd_device_enumerator* enumerator,
                               ...
                               const char* subsystem)
{
    ...
    FOREACH_DIRENT_ALL(dent, dir, return -errno)
    {
        ...
        k = enumerator_scan_dir_and_add_devices(enumerator, basedir,
                                                dent->d_name, subdir);
        if (k < 0)
            r = k;
    }
...
static int enumerator_scan_devices_tag(sd_device_enumerator* enumerator,
                                       const char* tag)
{
    _cleanup_closedir_ DIR* dir = NULL;
    char* path;
    ...
    if (!dir)
    {
        if (errno != ENOENT)
            return log_debug_errno(
                errno,
                "sd-device-enumerator: Failed to open tags directory %s: %m",
                path);
        return 0;
    }
...
static int parent_crawl_children(sd_device_enumerator* enumerator,
                                 const char* path, unsigned maxdepth)
{
    _cleanup_closedir_ DIR* dir = NULL;
    struct dirent* dent;
    int r = 0;
    ...
    if (!dir)
        return log_debug_errno(
            errno,
            "sd-device-enumerator: Failed to open parent directory %s: %m",
            path);
    ...
    FOREACH_DIRENT_ALL(dent, dir, return -errno)
    {
        ...
        if (maxdepth > 0)
            ...
            log_debug(
                "sd-device-enumerator: Max depth reached, %s: ignoring devices",
                child);
    }
...
static int enumerator_scan_devices_all(sd_device_enumerator* enumerator)
{
    ...
    if (access("/sys/subsystem", F_OK) >= 0)
    {
        ...
        if (r < 0)
            return log_debug_errno(
                r, "sd-device-enumerator: Failed to scan /sys/subsystem: %m");
    }
    else
    {
        int k;
        ...
        {
            log_debug_errno(
                k, "sd-device-enumerator: Failed to scan /sys/bus: %m");
            r = k;
        }
        ...
        {
            log_debug_errno(
                k, "sd-device-enumerator: Failed to scan /sys/class: %m");
            r = k;
        }
    }
...
int device_enumerator_scan_subsystems(sd_device_enumerator* enumerator)
{
    ...
    {
        k = enumerator_scan_dir_and_add_devices(enumerator, "module", NULL,
                                                NULL);
        if (k < 0)
        {
            log_debug_errno(k,
    ...
    if (match_subsystem(enumerator, "subsystem"))
    {
        k = enumerator_scan_dir_and_add_devices(enumerator, subsysdir, NULL,
                                                NULL);
        if (k < 0)
        {
            log_debug_errno(
                k, "sd-device-enumerator: Failed to scan subsystems: %m");
            r = k;
        }
    }
...
sd_device** device_enumerator_get_devices(sd_device_enumerator* enumerator,
