...
int bus_send_pending_reload_message(Manager* m)
{
    ...
    /* If we cannot get rid of this message we won't dispatch any D-Bus
     * messages, so that we won't end up wanting to queue another message. */
...
int bus_forward_agent_released(Manager* m, const char* path)
{
    ...
    /* If we are running a system instance we forward the agent message on the
     * system bus, so that the user instances get notified about this, too */

    r = sd_bus_emit_signal(m->system_bus, "/org/freedesktop/systemd1/agent",
                           "org.freedesktop.systemd1.Agent", "Released", "s",
                           path);
    if (r < 0)
...
static int signal_agent_released(sd_bus_message* message, void* userdata,
                                 sd_bus_error* error)
{
...
static int signal_disconnected(sd_bus_message* message, void* userdata,
                               sd_bus_error* error)
{
    Manager* m = userdata;
    sd_bus* bus;
...
static int signal_activation_request(sd_bus_message* message, void* userdata,
                                     sd_bus_error* ret_error)
{
    ...
    {
        r = sd_bus_error_setf(&error, BUS_ERROR_SHUTTING_DOWN,
    ...
    if (u->refuse_manual_start)
    {
        r = sd_bus_error_setf(
            ...
            "Operation refused, %s may be requested by dependency only (it is "
...
failed:
    ...
    log_debug("D-Bus activation failed for %s: %s", name,
              bus_error_message(&error, r));

    r = sd_bus_message_new_signal(
        sd_bus_message_get_bus(message), &reply, "/org/freedesktop/systemd1",
    ...
    if (r < 0)
        return log_error_errno(
            r, "Failed to respond with to bus activation request: %m");

    return 0;
}

#if HAVE_SELINUX
static int mac_selinux_filter(sd_bus_message* message, void* userdata,
                              sd_bus_error* error)
{
    Manager* m = userdata;
    ...
    if (sd_bus_message_is_method_call(message,
        ...
        verb = "reload";
                 ...
                 message, "org.freedesktop.DBus.Introspectable", NULL) ||
             sd_bus_message_is_method_call(
                 message, "org.freedesktop.DBus.Properties", NULL) ||
             sd_bus_message_is_method_call(
                 message, "org.freedesktop.DBus.ObjectManager", NULL) ||
             sd_bus_message_is_method_call(message, "org.freedesktop.DBus.Peer",
                                           NULL))
        verb = "status";
    else
        return 0;
...
static int bus_job_find(sd_bus* bus, const char* path, const char* interface,
...
static int find_unit(Manager* m, sd_bus* bus, const char* path, Unit** unit,
                     sd_bus_error* error)
{
    Unit* u =
...
static int bus_unit_find(sd_bus* bus, const char* path, const char* interface,
                         void* userdata, void** found, sd_bus_error* error)
{
    Manager* m = userdata;
...
static int bus_unit_interface_find(sd_bus* bus, const char* path,
...
static int bus_unit_cgroup_find(sd_bus* bus, const char* path,
                                const char* interface, void* userdata,
                                void** found, sd_bus_error* error)
{
    Manager* m = userdata;
...
static int bus_cgroup_context_find(sd_bus* bus, const char* path,
                                   const char* interface, void* userdata,
                                   void** found, sd_bus_error* error)
{
    Manager* m = userdata;
...
static int bus_exec_context_find(sd_bus* bus, const char* path,
                                 const char* interface, void* userdata,
                                 void** found, sd_bus_error* error)
{
    Manager* m = userdata;
...
static int bus_kill_context_find(sd_bus* bus, const char* path,
                                 const char* interface, void* userdata,
                                 void** found, sd_bus_error* error)
{
    Manager* m = userdata;
...
static int bus_job_enumerate(sd_bus* bus, const char* path, void* userdata,
...
static int bus_unit_enumerate(sd_bus* bus, const char* path, void* userdata,
                              char*** nodes, sd_bus_error* error)
{
    _cleanup_strv_free_ char** l = NULL;
    Manager* m = userdata;
...
static int bus_setup_api_vtables(Manager* m, sd_bus* bus)
{
    ...
    r = sd_bus_add_fallback_vtable(bus, NULL, "/org/freedesktop/systemd1/job",
                                   "org.freedesktop.systemd1.Job",
                                   bus_job_vtable, bus_job_find, m);
    if (r < 0)
        return log_error_errno(r, "Failed to register Job vtable: %m");

    r = sd_bus_add_node_enumerator(bus, NULL, "/org/freedesktop/systemd1/job",
                                   bus_job_enumerate, m);
    if (r < 0)
        return log_error_errno(r, "Failed to add job enumerator: %m");

    r = sd_bus_add_fallback_vtable(bus, NULL, "/org/freedesktop/systemd1/unit",
                                   "org.freedesktop.systemd1.Unit",
                                   bus_unit_vtable, bus_unit_find, m);
    if (r < 0)
        return log_error_errno(r, "Failed to register Unit vtable: %m");

    r = sd_bus_add_node_enumerator(bus, NULL, "/org/freedesktop/systemd1/unit",
                                   bus_unit_enumerate, m);
    if (r < 0)
        return log_error_errno(r, "Failed to add job enumerator: %m");
    ...
    for (t = 0; t < _UNIT_TYPE_MAX; t++)
    {
        ...
        r = sd_bus_add_fallback_vtable(
            bus, NULL, "/org/freedesktop/systemd1/unit", interface,
            unit_vtable[t]->bus_vtable, bus_unit_interface_find, m);
        if (r < 0)
            return log_error_errno(
                r, "Failed to register type specific vtable for %s: %m",
                interface);
        ...
        if (unit_vtable[t]->cgroup_context_offset > 0)
        {
            r = sd_bus_add_fallback_vtable(
                bus, NULL, "/org/freedesktop/systemd1/unit", interface,
                bus_unit_cgroup_vtable, bus_unit_cgroup_find, m);
            if (r < 0)
                return log_error_errno(
                    r,
            ...
            if (r < 0)
                return log_error_errno(
                    r, "Failed to register control group vtable for %s: %m",
                    interface);
        }
        ...
        if (unit_vtable[t]->exec_context_offset > 0)
        {
            r = sd_bus_add_fallback_vtable(
                bus, NULL, "/org/freedesktop/systemd1/unit", interface,
                bus_exec_vtable, bus_exec_context_find, m);
            if (r < 0)
                return log_error_errno(
                    r, "Failed to register execute vtable for %s: %m",
                    interface);
        }
        ...
        if (unit_vtable[t]->kill_context_offset > 0)
        {
            r = sd_bus_add_fallback_vtable(
                bus, NULL, "/org/freedesktop/systemd1/unit", interface,
                bus_kill_vtable, bus_kill_context_find, m);
            if (r < 0)
                return log_error_errno(
                    r, "Failed to register kill vtable for %s: %m", interface);
        }
    }
...
static int bus_setup_disconnected_match(Manager* m, sd_bus* bus)
{
    ...
    if (r < 0)
        return log_error_errno(
            r, "Failed to request match for Disconnected message: %m");

    return 0;
}

static int bus_on_connection(sd_event_source* s, int fd, uint32_t revents,
                             void* userdata)
{
    ...
    if (r < 0)
    {
        log_warning_errno(r, "Incoming private connection from unprivileged "
                             "client, refusing: %m");
        return 0;
    }
    ...
    {
        log_warning_errno(
            r, "Failed to enable server support for new connection bus: %m");
        return 0;
    }
    ...
    {
        log_warning_errno(
            r, "Failed to enable credentials for new connection: %m");
        return 0;
    }
    ...
    {
        log_warning_errno(
            r, "Failed to attach new connection bus to event loop: %m");
        return 0;
    }
    ...
    {
        log_warning_errno(
            r, "Failed to set up API vtables on new connection bus: %m");
        return 0;
    }
...
static int manager_dispatch_sync_bus_names(sd_event_source* es, void* userdata)
{
    ...
    /* First things first, destroy the defer event so that we aren't triggered
     * again */
    m->sync_bus_names_event_source =
    ...
    /* OK, let's sync up the names. Let's see which names are currently on the
     * bus. */
    ...
    HASHMAP_FOREACH_KEY(u, name, m->watch_bus, i)
    {
        ...
        {
            log_unit_warning(u, "Bus name has changed from %s → %s, ignoring.",
                             s->bus_name, name);
            continue;
        }
        ...
        if (strv_contains(names, name))
        {
            ...
            r = sd_bus_get_name_creds(m->api_bus, name,
                                      SD_BUS_CREDS_UNIQUE_NAME, &creds);
            if (r < 0)
            {
                log_full_errno(r == -ENXIO ? LOG_DEBUG : LOG_ERR, r,
            ...
            if (!streq_ptr(unique, s->bus_name_owner))
                UNIT_VTABLE(u)->bus_name_owner_change(
                    u, name, s->bus_name_owner, unique);
        }
        else
        {
...
int manager_enqueue_sync_bus_names(Manager* m)
{
    ...
    /* Enqueues a request to synchronize the bus names in a later event loop
     * iteration. The callers generally don't want us to invoke
     * ->bus_name_owner_change() unit calls from their stack frames as this
     * might result in event dispatching on its own creating loops, hence we
     * simply create a defer event for the event loop and exit. */

    if (m->sync_bus_names_event_source)
        return 0;

    r = sd_event_add_defer(m->event, &m->sync_bus_names_event_source,
                           manager_dispatch_sync_bus_names, m);
    if (r < 0)
        return log_error_errno(
            r, "Failed to create bus name synchronization event: %m");

    r = sd_event_source_set_priority(m->sync_bus_names_event_source,
                                     SD_EVENT_PRIORITY_IDLE);
    if (r < 0)
        return log_error_errno(r, "Failed to set event priority: %m");

    r = sd_event_source_set_enabled(m->sync_bus_names_event_source,
                                    SD_EVENT_ONESHOT);
    if (r < 0)
        return log_error_errno(r, "Failed to set even to oneshot: %m");

    (void)sd_event_source_set_description(m->sync_bus_names_event_source,
...
static int bus_setup_api(Manager* m, sd_bus* bus)
{
    ...
    /* Let's make sure we have enough credential bits so that we can make
     * security and selinux decisions */
    r = sd_bus_negotiate_creds(
        bus, 1,
        SD_BUS_CREDS_PID | SD_BUS_CREDS_UID | SD_BUS_CREDS_EUID |
            SD_BUS_CREDS_EFFECTIVE_CAPS | SD_BUS_CREDS_SELINUX_CONTEXT);
    ...
    /* Allow replacing of our name, to ease implementation of reexecution, where
     * we keep the old connection open until after the new connection is set up
     * and the name installed to allow clients to synchronously wait for
     * reexecution to finish */
    r = sd_bus_request_name_async(bus, NULL, "org.freedesktop.systemd1",
...
int bus_init_api(Manager* m)
{
    ...
    else
    {
        ...
        if (r < 0)
            return log_error_errno(
                r, "Failed to attach API bus to event loop: %m");
...
static int bus_setup_system(Manager* m, sd_bus* bus)
{
    ...
    {
        r = sd_bus_match_signal_async(
            bus, NULL, NULL, "/org/freedesktop/systemd1/agent",
            "org.freedesktop.systemd1.Agent", "Released", signal_agent_released,
            NULL, m);
        if (r < 0)
            log_warning_errno(
                r, "Failed to request Released match on system bus: %m");
    }
...
int bus_init_system(Manager* m)
{
    ...
    else
    {
        ...
        if (r < 0)
            return log_error_errno(
                r, "Failed to attach system bus to event loop: %m");
...
int bus_init_private(Manager* m)
{
    ...
    if (salen < 0)
        return log_error_errno(
            salen, "Can't set path for AF_UNIX socket to bind to: %m");
    ...
    /* Generate an inotify event in case somebody waits for this socket to
     * appear using inotify() */
...
static void destroy_bus(Manager* m, sd_bus** bus)
{
    ...
    if (m->pending_reload_message &&
        sd_bus_message_get_bus(m->pending_reload_message) == *bus)
        m->pending_reload_message =
            sd_bus_message_unref(m->pending_reload_message);
                    ...
                    void* userdata)
{
    ...
    SET_FOREACH(b, m->private_buses, i)
    {
        ...
        /* Don't bother with enqueing these messages to clients that haven't
         * started yet */
    ...
    if (m->api_bus && (sd_bus_track_count(m->subscribed) > 0 ||
...
int bus_verify_manage_units_async(Manager* m, sd_bus_message* call,
                                  sd_bus_error* error)
{
    return bus_verify_polkit_async(
        call, CAP_SYS_ADMIN, "org.freedesktop.systemd1.manage-units", NULL,
        false, UID_INVALID, &m->polkit_registry, error);
}
...
{
    return bus_verify_polkit_async(
        call, CAP_SYS_ADMIN, "org.freedesktop.systemd1.manage-unit-files", NULL,
        false, UID_INVALID, &m->polkit_registry, error);
}
...
{
    return bus_verify_polkit_async(
        call, CAP_SYS_ADMIN, "org.freedesktop.systemd1.reload-daemon", NULL,
        false, UID_INVALID, &m->polkit_registry, error);
}

int bus_verify_set_environment_async(Manager* m, sd_bus_message* call,
                                     sd_bus_error* error)
{
    return bus_verify_polkit_async(
        call, CAP_SYS_ADMIN, "org.freedesktop.systemd1.set-environment", NULL,
        false, UID_INVALID, &m->polkit_registry, error);
}
...
uint64_t manager_bus_n_queued_write(Manager* m)
{
    ...
    /* Returns the total number of messages queued for writing on all our direct
     * and API busses. */
    ...
    SET_FOREACH(b, m->private_buses, i)
    {
        ...
        if (r < 0)
            log_debug_errno(
                r, "Failed to query queued messages for private bus: %m");
        else
            c += k;
    }
...
static void vtable_dump_bus_properties(FILE* f, const sd_bus_vtable* table)
{
    ...
    {
        if (!IN_SET(i->type, _SD_BUS_VTABLE_PROPERTY,
