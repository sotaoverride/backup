...
#ifndef _WIN32
static void FcDirCacheReadUUID(FcChar8* dir, FcConfig* config)
{
    void* u;
    uuid_t uuid;
    const FcChar8* sysroot = FcConfigGetSysRoot(config);
...
#define CACHEBASE_LEN                                                          \
    (1 + 36 + 1 + sizeof(FC_ARCHITECTURE) + sizeof(FC_CACHE_SUFFIX))
...
static FcChar8* FcDirCacheBasenameMD5(const FcChar8* dir,
...
#ifndef _WIN32
static FcChar8* FcDirCacheBasenameUUID(const FcChar8* dir,
                                       FcChar8 cache_base[CACHEBASE_LEN],
                                       FcConfig* config)
{
    void* u;
...
FcBool FcDirCacheUnlink(const FcChar8* dir, FcConfig* config)
{
    ...
    while ((cache_dir = FcStrListNext(list)))
    {
        if (sysroot)
            cache_hashed =
                FcStrBuildFilename(sysroot, cache_dir, cache_base, NULL);
        else
 ...
 * indicates success
 */
static FcBool FcDirCacheProcess(FcConfig* config, const FcChar8* dir,
                                FcBool (*callback)(FcConfig* config, int fd,
                                                   struct stat* fd_stat,
...
static FcCacheSkip* FcCacheFindByAddrUnlocked(void* object)
{
    ...
    for (i = fcCacheMaxLevel; --i >= 0;)
        while (next[i] &&
               (char*)object >= ((char*)next[i]->cache + next[i]->size))
            next = next[i]->next;
    /*
     * Here we are
     */
...
static FcBool FcCacheTimeValid(FcConfig* config, FcCache* cache,
                               struct stat* dir_stat)
{
    ...
    if (FcDebug() & FC_DBG_CACHE)
        printf("FcCacheTimeValid dir \"%s\" cache checksum %d.%ld dir checksum "
               "%d.%ld\n",
               FcCacheDir(cache), cache->checksum, (long)cache->checksum_nano,
               (int)dir_stat->st_mtime, dir_stat->st_mtim.tv_nsec);
#else
    if (FcDebug() & FC_DBG_CACHE)
        printf(
...
static FcBool FcCacheOffsetsValid(FcCache* cache)
{
    ...
    if (dirs)
    {
        for (i = 0; i < cache->dirs_count; i++)
        {
            ...
            if (dirs[i] < 0 || dirs[i] > end - (char*)dirs - sizeof(intptr_t))
                return FcFalse;
    ...
    if (fs)
    {
        ...
        for (i = 0; i < fs->nfont; i++)
        {
            ...
            if ((char*)font < base || (char*)font > end - sizeof(FcFontSet) ||
                font->elts_offset < 0 ||
                font->elts_offset > end - (char*)font ||
                font->num > (end - (char*)font - font->elts_offset) /
                                sizeof(FcPatternElt) ||
 ...
 */
static FcCache* FcDirCacheMapFd(FcConfig* config, int fd, struct stat* fd_stat,
                                struct stat* dir_stat)
{
    FcCache* cache;
...
static FcBool FcDirCacheMapHelper(FcConfig* config, int fd,
                                  struct stat* fd_stat, struct stat* dir_stat,
                                  void* closure)
{
...
#endif
    if (!FcDirCacheProcess(config, dir, FcDirCacheMapHelper, &cache,
                           cache_file))
        return NULL;
...
static int FcDirChecksum(struct stat* statb)
{
    ...
    if (source_date_epoch)
    {
        ...
        if (endptr == source_date_epoch)
            fprintf(stderr, "Fontconfig: SOURCE_DATE_EPOCH invalid\n");
 ...
 */
static FcBool FcDirCacheValidateHelper(FcConfig* config, int fd,
                                       struct stat* fd_stat,
                                       struct stat* dir_stat,
                                       void* closure FC_UNUSED)
{
    FcBool ret = FcTrue;
    FcCache c;
...
{
    return FcDirCacheProcess(config, dir, FcDirCacheValidateHelper, NULL, NULL);
}
...
FcCache* FcDirCacheRebuild(FcCache* cache, struct stat* dir_stat,
                           FcStrSet* dirs)
{
    FcCache* new;
...
FcBool FcDirCacheWrite(FcCache* cache, FcConfig* config)
{
    ...
    fd = FcOpen((char*)FcAtomicNewFile(atomic), O_RDWR | O_CREAT | O_BINARY,
                0666);
    if (fd == -1)
        goto bail4;
...
FcBool FcDirCacheClean(const FcChar8* cache_dir, FcBool verbose)
{
    ...
    while ((ent = readdir(d)))
    {
        ...
        if (strlen(ent->d_name) !=
                32 + strlen("-" FC_ARCHITECTURE FC_CACHE_SUFFIX) ||
...
const FcChar8* FcCacheDir args1(const FcCache* c)
{
    return FcCacheDir(c);
}

FcFontSet* FcCacheCopySet args1(const FcCache* c)
{
...
const FcChar8* FcCacheSubdir args2(const FcCache* c, int i)
{
    return FcCacheSubdir(c, i);
}

int FcCacheNumSubdir args1(const FcCache* c)
{
    return c->dirs_count;
}

int FcCacheNumFont args1(const FcCache* c)
{
...
FcBool FcDirCacheCreateTagFile(const FcChar8* cache_dir)
{
    ...
    if (access((char*)cache_dir, W_OK) == 0)
    {
        ...
        fwrite(cache_tag_contents, cache_tag_contents_size, sizeof(FcChar8),
               fp);
        fclose(fp);
