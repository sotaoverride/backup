...
osa_pw_hist_ent* hist;
{
    ...
    hist->key_data =
        (krb5_key_data*)malloc(hist->n_key_data * sizeof(krb5_key_data));
 ...
 *
 */
int process_ov_principal(kcontext, fname, filep, verbose,
                         linenop) krb5_context kcontext;
const char* fname;
FILE* filep;
...
int* linenop;
{
    ...
    {
        if (!(rec->old_keys = (osa_pw_hist_ent*)malloc(sizeof(osa_pw_hist_ent) *
                                                       rec->old_key_len)))
        {
            ret = ENOMEM;
            goto done;
        }
        memset(rec->old_keys, 0, sizeof(osa_pw_hist_ent) * rec->old_key_len);
