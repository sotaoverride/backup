 ...
 */
static int addr_strings(const BIO_ADDR* ap, int numeric, char** hostname,
                        char** service)
{
    if (BIO_sock_init() != 1)
        return 0;
    ...
    if (1)
    {
        ...
        if ((ret = getnameinfo(BIO_ADDR_sockaddr(ap),
                               BIO_ADDR_sockaddr_size(ap), host, sizeof(host),
                       ...
                       enum BIO_hostserv_priorities hostserv_prio)
{
    ...
    else
    {
         ...
         *    IPv6 address and port number if hostserv_prio ==
         * BIO_PARSE_PRIO_SERV Because of this ambiguity, we currently choose to
         * make it an error.
         */
        if (p != p2)
            goto amb_err;
 ...
 */
static int addrinfo_wrap(int family, int socktype, const void* where,
                         size_t wherelen, unsigned short port,
                         BIO_ADDRINFO** bai)
{
...
#pragma pointer_size restore
            ...
            if (endp != service && *endp == '\0' && portnum > 0 &&
                portnum < 65536)
            {
...
#pragma pointer_size restore
            ...
            {
                if (!addrinfo_wrap(he->h_addrtype, socktype, *addrlistp,
