...
#define seq_len(seq)                                                           \
    seq##_CHOICE, seq##_LOW0, seq##_LOW1, seq##_LOW2, seq##_CHOICE2,           \
        seq##_MID0, seq##_MID1, seq##_MID2, seq##_HIGH0, seq##_HIGH1,          \
        seq##_HIGH2, seq##_HIGH3, seq##_HIGH4, seq##_HIGH5, seq##_HIGH6,       \
        seq##_HIGH7
            ...
            {                                                                  \
                ...
                case seq##_CHOICE2:                                            \
                    ...
                    {                                                          \
                        ...
                        rc_bit_case(ld.mid[pos_state][symbol], , ,             \
                                    seq##_MID2);                               \
                        target = symbol - LEN_MID_SYMBOLS + MATCH_LEN_MIN +    \
                                 LEN_LOW_SYMBOLS;                              \
                    }                                                          \
                    else                                                       \
                    {                                                          \
                        ...
                        target = symbol - LEN_HIGH_SYMBOLS + MATCH_LEN_MIN +   \
...
static lzma_ret lzma_decode(void* coder_ptr, lzma_dict* restrict dictptr,
                            ...
                            size_t in_size)
{
    ...
    {
        const lzma_ret ret = rc_read_init(&coder->rc, in, in_pos, in_size);
    ...
    // marker.
    const bool no_eopm = coder->uncompressed_size != LZMA_VLI_UNKNOWN;
        ...
        while (true)
        {
            ...
            case SEQ_IS_MATCH:
                ...
                rc_if_0(coder->is_match[state][pos_state], SEQ_IS_MATCH)
                {
                    ...
                    probs = literal_subcoder(
                        coder->literal, literal_context_bits, literal_pos_mask,
                    ...
                    else
                    {
                        ...
                        case SEQ_LITERAL_MATCHED:
                            do
                            {
                                const uint32_t match_bit = len & offset;
                                const uint32_t subcoder_index =
                                    offset + match_bit + symbol;

                                rc_bit(
                                    probs[subcoder_index], offset &= ~match_bit,
                                    offset &= match_bit, SEQ_LITERAL_MATCHED);
...
#define d(seq)                                                                 \
    case seq:                                                                  \
        ...
        rc_bit(probs[subcoder_index], offset &= ~match_bit,                    \
               offset &= match_bit, seq)
...
#undef d
                    ...
                    static const lzma_lzma_state next_state[] = {
                        ...
                        STATE_LIT_LIT,          STATE_LIT_LIT,
                        STATE_MATCH_LIT_LIT,    STATE_REP_LIT_LIT,
                        STATE_SHORTREP_LIT_LIT, STATE_MATCH_LIT,
                        STATE_REP_LIT,          STATE_SHORTREP_LIT,
                        STATE_MATCH_LIT,        STATE_REP_LIT};
            ...
            case SEQ_IS_REP:
                rc_if_0(coder->is_rep[state], SEQ_IS_REP)
                {
                    ...
                    len_decode(len, coder->match_len_decoder, pos_state,
                               SEQ_MATCH_LEN);
                    ...
                    rc_bit_case(probs[symbol], , , SEQ_DIST_SLOT5);
                        ...
                        else
                        {
                            ...
                            if (symbol < DIST_MODEL_END)
                            {
                                ...
                                // the array.
                                assert((int32_t)(rep0 - symbol - 1) >= -1);
                                assert((int32_t)(rep0 - symbol - 1) <= 82);
                                probs = coder->pos_special + rep0 - symbol - 1;
                                symbol = 1;
                                offset = 0;
                                case SEQ_DIST_MODEL:
#ifdef HAVE_SMALL
                                ...
                                case SEQ_ALIGN:
                                    do
                                    {
                                        rc_bit(coder->pos_align[symbol], ,
                                               rep0 += 1 << offset, SEQ_ALIGN);
                                    } while (++offset < ALIGN_BITS);
#else
                            case SEQ_ALIGN0:
                                rc_bit(coder->pos_align[symbol], , rep0 += 1,
                                       SEQ_ALIGN0);
                            case SEQ_ALIGN1:
                                rc_bit(coder->pos_align[symbol], , rep0 += 2,
                                       SEQ_ALIGN1);
                            case SEQ_ALIGN2:
                                rc_bit(coder->pos_align[symbol], , rep0 += 4,
                                       SEQ_ALIGN2);
                            case SEQ_ALIGN3:
                        ...
                        rc_if_0(coder->is_rep0[state], SEQ_IS_REP0)
                        {
                            ...
                            case SEQ_IS_REP0_LONG:
                                        ...
                                        SEQ_IS_REP0_LONG)
                                {
                                    ...
                                    case SEQ_SHORTREP:
                                        if (unlikely(dict_put(
                                                &dict, dict_get(&dict, rep0))))
                                        {
                                ...
                                else
                                {
                                    ...
                                    case SEQ_IS_REP2:
                                        rc_if_0(coder->is_rep2[state],
                                                SEQ_IS_REP2)
                                        {
                                            rc_update_0(coder->is_rep2[state]);
                                        ...
                                        {
                                            rc_update_1(coder->is_rep2[state]);
...
out:
    ...
    if (coder->uncompressed_size != LZMA_VLI_UNKNOWN)
    {
        ...
        if (coder->uncompressed_size == 0 && ret == LZMA_OK &&
            ...
            ret = coder->sequence == SEQ_IS_MATCH ? LZMA_STREAM_END
                                                  : LZMA_DATA_ERROR;
    }
...
static void lzma_decoder_uncompressed(void* coder_ptr,
                                      lzma_vli uncompressed_size)
{
...
extern lzma_ret lzma_lzma_decoder_create(lzma_lz_decoder* lz,
                                  ...
                                  lzma_lz_options* lz_options)
{
    ...
    return_if_error(
        lzma_lzma_decoder_create(lz, allocator, options, lz_options));
                                       ...
                                       const lzma_filter_info* filters)
{
    ...
    return lzma_lz_decoder_init(next, allocator, filters, &lzma_decoder_init);
}
...
extern lzma_ret lzma_lzma_props_decode(void** options,
                                       const lzma_allocator* allocator,
                                       const uint8_t* props, size_t props_size)
{
    ...
    lzma_options_lzma* opt = lzma_alloc(sizeof(lzma_options_lzma), allocator);
    if (opt == NULL)
        return LZMA_MEM_ERROR;
