 ...
 */
static FcBool FcListValueListMatchAny(FcValueListPtr patOrig, /* pattern */
...
static FcBool FcListValueListEqual(FcValueListPtr v1orig, FcValueListPtr v2orig)
{
...
static FcBool FcListPatternEqual(FcPattern* p1, FcPattern* p2, FcObjectSet* os)
{
    int i;
    FcPatternElt *e1, *e2;
...
static FcChar32 FcListValueListHash(FcValueListPtr list)
{
    FcChar32 h = 0;
...
static FcChar32 FcListPatternHash(FcPattern* font, FcObjectSet* os)
{
    int n;
    FcPatternElt* e;
    FcChar32 h = 0;
...
static void FcListHashTableInit(FcListHashTable* table)
{
    table->entries = 0;
    memset(table->buckets, '\0', sizeof(table->buckets));
}

static void FcListHashTableCleanup(FcListHashTable* table)
{
    int i;
...
static int FcGetDefaultObjectLangIndex(FcPattern* font, FcObject object,
                                       const FcChar8* lang)
{
...
static FcBool FcListAppend(FcListHashTable* table, FcPattern* font,
                           FcObjectSet* os, const FcChar8* lang)
{
    int o;
    FcPatternElt* e;
    FcValueListPtr v;
    ...
    for (prev = &table->buckets[hash % FC_LIST_HASH_SIZE]; (bucket = *prev);
         prev = &(bucket->next))
    {
        if (bucket->hash == hash &&
    ...
    {
        if (!strcmp(os->objects[o], FC_FAMILY) ||
            !strcmp(os->objects[o], FC_FAMILYLANG))
        {
            if (familyidx < 0)
                familyidx = FcGetDefaultObjectLangIndex(
                    font, FC_FAMILYLANG_OBJECT, lang);
            defidx = familyidx;
        }
        else if (!strcmp(os->objects[o], FC_FULLNAME) ||
                 !strcmp(os->objects[o], FC_FULLNAMELANG))
        {
            if (fullnameidx < 0)
                fullnameidx = FcGetDefaultObjectLangIndex(
                    font, FC_FULLNAMELANG_OBJECT, lang);
            defidx = fullnameidx;
        }
        else if (!strcmp(os->objects[o], FC_STYLE) ||
                 !strcmp(os->objects[o], FC_STYLELANG))
        {
            if (styleidx < 0)
                styleidx = FcGetDefaultObjectLangIndex(
                    font, FC_STYLELANG_OBJECT, lang);
            defidx = styleidx;
        }
        else
            defidx = 0;
        ...
        if (e)
        {
            ...
            {
                if (!FcPatternAdd(bucket->pattern, os->objects[o],
    ...
    for (set = 0; set < nsets; set++)
    {
        ...
        for (f = 0; f < s->nfont; f++)
                                      ...
                                      s->fonts[f])) /* font */
            {
                ...
                if (FcPatternObjectGetString(p, FC_NAMELANG_OBJECT, 0, &lang) !=
                    FcResultMatch)
                {
