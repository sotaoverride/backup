...
#define L_PRINTF(l, args)                                                      \
    if (svc_debug_gssapi >= l)                                                 \
    gssrpcint_printf args
#define PRINTF(args) L_PRINTF(99, args)
#define AUTH_GSSAPI_DISPLAY_STATUS(args)                                       \
    if (svc_debug_gssapi)                                                      \
    auth_gssapi_display_status args
#else
#define PRINTF(args)
#define L_PRINTF(l, args)
#define AUTH_GSSAPI_DISPLAY_STATUS(args)
...
#define SVCAUTH_PRIVATE(auth) ((svc_auth_gssapi_data*)(auth)->svc_ah_private)
...
static void client_expire(svc_auth_gssapi_data* client_data, uint32_t exp);
static void dump_db(char* msg);

struct svc_auth_ops svc_auth_gssapi_ops = {
    svc_auth_gssapi_wrap, svc_auth_gssapi_unwrap, svc_auth_gssapi_destroy};
...
static void badauth(OM_uint32 maj, OM_uint32 minor, SVCXPRT* xprt)
{
    if (log_badauth != NULL)
...
enum auth_stat gssrpc__svcauth_gssapi(struct svc_req* rqst, struct rpc_msg* msg,
                                      bool_t* no_dispatch)
{
    XDR xdrs;
    ...
    PRINTF(
        ("svcauth_gssapi: got credentials, version %d, client_handle len %d\n",
    ...
    if (client_data->established == FALSE)
    {
         ...
         */
        if (rqst->rq_proc != AUTH_GSSAPI_INIT &&
            rqst->rq_proc != AUTH_GSSAPI_CONTINUE_INIT)
        {
            PRINTF(
        ...
        if (!svc_getargs(rqst->rq_xprt, xdr_authgssapi_init_arg, &call_arg))
        {
        ...
        for (i = 0; i < server_creds_count; i++)
        {
            ...
            call_res.gss_major = gss_accept_sec_context(
                &call_res.gss_minor, &client_data->context, server_creds,
                &call_arg.token, bindp, &client_data->client_name, NULL,
                &output_token, &ret_flags, &time_rec, NULL);
            ...
            call_res.gss_major != GSS_S_CONTINUE_NEEDED)
        {
            AUTH_GSSAPI_DISPLAY_STATUS(
                ("accepting context", call_res.gss_major, call_res.gss_minor));
        ...
        if (gssstat == GSS_S_COMPLETE)
        {
            ...
            client_expire(
                client_data,
                (time_rec == GSS_C_INDEFINITE ? INDEF_EXPIRE : time_rec) +
                    time(0));

            PRINTF(("svcauth_gssapi: context established, isn %d\n",
                    client_data->seq_num));

            if (auth_gssapi_seal_seq(client_data->context, client_data->seq_num,
                                     &call_res.signed_isn) == FALSE)
            {
                ret = AUTH_FAILED;
    ...
    else
    {
        ...
        if (auth_gssapi_unseal_seq(client_data->context, &in_buf, &seq_num) ==
            FALSE)
        {
            ret = AUTH_BADVERF;
        ...
        if (seq_num != client_data->seq_num + 1)
        {
            ...
            if (log_badverf != NULL)
                (*log_badverf)(client_data->client_name,
                               client_data->server_name, rqst, msg,
                               log_badverf_data);
        ...
        if (auth_gssapi_seal_seq(client_data->context, seq_num, &out_buf) ==
            FALSE)
        {
            ret = AUTH_FAILED;
            LOG_MISCERR("internal error sealing sequence number");
            ...
            switch (rqst->rq_proc)
            {
                case AUTH_GSSAPI_MSG:
                    ...
                    gssstat = gss_process_context_token(
                        &minor_stat, client_data->context, &call_arg.token);
                    ...
                    {
                        AUTH_GSSAPI_DISPLAY_STATUS(
                            ("processing token", gssstat, minor_stat));
                        ret = AUTH_FAILED;
                        goto error;
                    }
...
static void destroy_client(svc_auth_gssapi_data* client_data)
{
    ...
    gssstat =
        gss_delete_sec_context(&minor_stat, &client_data->context, &out_buf);
    if (gssstat != GSS_S_COMPLETE)
        AUTH_GSSAPI_DISPLAY_STATUS(("deleting context", gssstat, minor_stat));
 ...
 */
bool_t svcauth_gssapi_set_names(auth_gssapi_name* names, int num)
{
    OM_uint32 gssstat, minor_stat;
    ...
    for (i = 0; i < num; i++)
    {
        ...
        if (gssstat != GSS_S_COMPLETE)
        {
            AUTH_GSSAPI_DISPLAY_STATUS(("importing name", gssstat, minor_stat));
            goto fail;
        }
        ...
        {
            AUTH_GSSAPI_DISPLAY_STATUS(
                ("acquiring credentials", gssstat, minor_stat));
            goto fail;
        }
    }
 ...
 * See functional specifications.
 */
void svcauth_gssapi_set_log_badauth_func(auth_gssapi_log_badauth_func func,
                                         caddr_t data)
{
    log_badauth = func;
...
void svcauth_gssapi_set_log_badauth2_func(auth_gssapi_log_badauth2_func func,
                                          caddr_t data)
{
    log_badauth2 = func;
 ...
 * See functional specifications.
 */
void svcauth_gssapi_set_log_badverf_func(auth_gssapi_log_badverf_func func,
                                         caddr_t data)
{
    log_badverf = func;
 ...
 * See functional specifications.
 */
void svcauth_gssapi_set_log_miscerr_func(auth_gssapi_log_miscerr_func func,
                                         caddr_t data)
{
    log_miscerr = func;
 ...
 */
static bool_t svc_auth_gssapi_wrap(SVCAUTH* auth, XDR* out_xdrs,
                                   bool_t (*xdr_func)(), caddr_t xdr_ptr)
{
    ...
    else if (!auth_gssapi_wrap_data(
                 &gssstat, &minor_stat, SVCAUTH_PRIVATE(auth)->context,
                 SVCAUTH_PRIVATE(auth)->seq_num, out_xdrs, xdr_func, xdr_ptr))
    {
        if (gssstat != GSS_S_COMPLETE)
            AUTH_GSSAPI_DISPLAY_STATUS(
                ("encrypting function arguments", gssstat, minor_stat));
        return FALSE;
    }
    else
        return TRUE;
...
static bool_t svc_auth_gssapi_unwrap(SVCAUTH* auth, XDR* in_xdrs,
                                     bool_t (*xdr_func)(), caddr_t xdr_ptr)
{
    ...
    else if (!auth_gssapi_unwrap_data(
                 &gssstat, &minor_stat, client_data->context,
                 client_data->seq_num - 1, in_xdrs, xdr_func, xdr_ptr))
    {
        if (gssstat != GSS_S_COMPLETE)
            AUTH_GSSAPI_DISPLAY_STATUS(
                ("decrypting function arguments", gssstat, minor_stat));
        return FALSE;
    }
    else
        return TRUE;
