...
#define LOG_NFACILITIES                                                        \
    24 + 1 /* we copy&paste this as including rsyslog.h gets us in off64_t     \
              trouble... :-( */
...
struct cnfstmt
{
    ...
    union
    {
        ...
        struct
        {
            ...
            struct cnfstmt* stmt;
            ruleset_t*
                ruleset; /* non-NULL if the ruleset has a queue assigned */
        } s_call;
        struct
        {
            struct cnfexpr* expr;
...
{
    CNFFUNC_INVALID =
        0, /**< defunct entry, do not use (should normally not be present) */
...
struct cnffunc
{
    ...
    rscriptFuncPtr fPtr;
    void* funcdata; /* global data for function-specific use (e.g. compiled
                       regex) */
 ...
 * to care.
 */
struct cnfparamdescr
{                     /* first the param description */
...
struct cnfparamblk
{ /* now the actual param block use in API calls */
 ...
 */
struct cnfparamvals
{ /* the values we obtained for param descr. */
    struct svar val;
    unsigned char bUsed;
};
...
struct json_object* cnfexprEvalCollection(struct cnfexpr* const expr,
                                          void* const usrptr, wti_t* pWti);
