...
static bool is_gpiochip_cdev(const char* path)
{
     ...
     */
    snprintf(devstr, sizeof(devstr), "%u:%u", major(statbuf.st_rdev),
                                       ...
                                       unsigned int offset)
{
    ...
    if (!chip->lines)
    {
        chip->lines = calloc(chip->num_lines, sizeof(struct gpiod_line*));
        if (!chip->lines)
            return NULL;
    }
...
static int
    line_request_event_single(struct gpiod_line* line,
                              const struct gpiod_line_request_config* config)
{
    struct line_fd_handle* line_fd;
