...
errcode_t ext2fs_copy_dblist(ext2_dblist src, ext2_dblist* dest)
{
    ...
    retval = make_dblist(src->fs, src->size, src->count, src->list, &dblist);
    if (retval)
        return retval;
 ...
 */
errcode_t ext2fs_add_dir_block2(ext2_dblist dblist, ext2_ino_t ino, blk64_t blk,
                                e2_blkcnt_t blockcnt)
{
    ...
    if (dblist->count >= dblist->size)
    {
        ...
        retval = ext2fs_resize_mem(
            old_size, (size_t)dblist->size * sizeof(struct ext2_db_entry2),
            &dblist->list);
        if (retval)
        {
 ...
 */
errcode_t ext2fs_set_dir_block2(ext2_dblist dblist, ext2_ino_t ino, blk64_t blk,
                                e2_blkcnt_t blockcnt)
{
    dgrp_t i;
...
void ext2fs_dblist_sort2(ext2_dblist dblist,
                         EXT2_QSORT_TYPE (*sortfunc)(const void*, const void*))
{
    if (!sortfunc)
        sortfunc = dir_block_cmp2;
    qsort(dblist->list, (size_t)dblist->count, sizeof(struct ext2_db_entry2),
          sortfunc);
    dblist->sorted = 1;
}
...
errcode_t ext2fs_dblist_iterate3(ext2_dblist dblist,
                                 ...
                                 unsigned long long count, void* priv_data)
{
...
errcode_t ext2fs_dblist_iterate2(ext2_dblist dblist,
                                 ...
                                 void* priv_data)
{
    return ext2fs_dblist_iterate3(dblist, func, 0, dblist->count, priv_data);
}
...
static int passthrough_func(ext2_filsys fs, struct ext2_db_entry2* db_info,
                            void* priv_data)
{
