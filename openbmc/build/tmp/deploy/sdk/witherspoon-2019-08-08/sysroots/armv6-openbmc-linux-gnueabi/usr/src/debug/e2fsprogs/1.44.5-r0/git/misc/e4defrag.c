...
#define FREE(tmp)                                                              \
    ...
    {                                                                          \
        if ((tmp) != NULL)                                                     \
            free(tmp);                                                         \
    } while (0) /* Insert list2 after list1 */
                        ...
                        unsigned char** vec, unsigned int* page_num)
{
    ...
    *vec = (unsigned char*)calloc(*page_num, 1);
    ...
    if (mincore(page, (size_t)length, *vec) == -1 || munmap(page, length) == -1)
    {
        FREE(*vec);
        return -1;
    }
...
static int check_free_size(int fd, const char* file, ext4_fsblk_t blk_count)
{
    ...
    if (fstatfs64(fd, &fsbuf) < 0)
    {
        ...
        {
            PRINT_FILE_NAME(file);
            PRINT_ERR_MSG_WITH_ERRNO("Failed to get filesystem information");
        }
        return -1;
    }
                      ...
                      int extents, ext4_fsblk_t blk_count)
{
    ...
    if (ret < 0)
    {
        if ((mode_flag & DETAIL) && ret == -ENOSPC)
        {
            printf("\033[79;0H\033[K[%u/%u] \"%s\"\t\t"
                   "  extents: %d -> %d\n",
                   defraged_file_count, total_count, file, extents, extents);
    ...
    /* Access authority */
    if (current_uid != ROOT_UID && buf->st_uid != current_uid)
    {
        if (mode_flag & DETAIL)
        {
            printf("\033[79;0H\033[K[%u/%u] \"%s\"\t\t"
                   "  extents: %d -> %d\n",
                   defraged_file_count, total_count, file, extents, extents);
            IN_FTW_PRINT_ERR_MSG("File is not current user's file"
    ...
    if (fcntl(fd, F_GETLK, &lock) < 0)
    {
        ...
        {
            PRINT_FILE_NAME(file);
            PRINT_ERR_MSG_WITH_ERRNO("Failed to get lock information");
        }
        return -1;
    }
                                    ...
                                    struct fiemap_extent_list* ext)
{
    ...
    {
        /* Insert before head */
        if (ext_list_tmp->data.logical < ext->data.logical + ext->data.len)
            /* Overlap */
            goto out;
        /* Adjust head */
        *ext_list_head = ext;
    ...
    else
    {
        ...
        if (ext_list_tmp != *ext_list_head &&
            ext_list_tmp->data.logical < ext->data.logical + ext->data.len)
            /* Overlap */
            goto out;
    }
                                     ...
                                     struct fiemap_extent_list* ext)
{
    ...
    {
        /* Insert before head */
        if (ext_list_tmp->data.physical < ext->data.physical + ext->data.len)
            /* Overlap */
            goto out;
        /* Adjust head */
        *ext_list_head = ext;
    ...
    else
    {
        ...
        if (ext->data.physical <
            ext_list_tmp->prev->data.physical + ext_list_tmp->prev->data.len)
            /* Overlap */
            goto out;

        if (ext_list_tmp != *ext_list_head &&
            ext_list_tmp->data.physical < ext->data.physical + ext->data.len)
            /* Overlap */
            goto out;
    }
    ext_list_tmp = ext_list_tmp->prev;
                        ...
                        struct fiemap_extent_group** ext_group_head)
{
    ...
    do
    {
         ...
         */
        if ((ext_list_tmp->prev->data.logical + ext_list_tmp->prev->data.len) !=
            ext_list_tmp->data.logical)
        {
            ext_group_tmp = malloc(sizeof(struct fiemap_extent_group));
            if (ext_group_tmp == NULL)
                return -1;

            memset(ext_group_tmp, 0, sizeof(struct fiemap_extent_group));
            ext_group_tmp->len = len;
            ext_group_tmp->start = ext_list_start;
            ext_group_tmp->end = ext_list_tmp->prev;

            if (insert_exts_group(ext_group_head, ext_group_tmp) < 0)
            {
                FREE(ext_group_tmp);
                return -1;
            }
...
static int get_file_extents(int fd, struct fiemap_extent_list** ext_list_head)
{
    ...
    do
    {
        ...
        for (i = 0; i < fiemap_buf->fm_mapped_extents; i++)
        {
            ...
            ext_list->data.physical = ext_buf[i].fe_physical / block_size;
            ext_list->data.logical = ext_buf[i].fe_logical / block_size;
            ext_list->data.len = ext_buf[i].fe_length / block_size;

            ret = insert_extent_by_physical(ext_list_head, ext_list);
            if (ret < 0)
            {
                FREE(ext_list);
                goto out;
 ...
 */
static int
    change_physical_to_logical(struct fiemap_extent_list** physical_list_head,
                               struct fiemap_extent_list** logical_list_head)
{
    ...
    while (1)
    {
        ...
        {
            ret = insert_extent_by_logical(logical_list_head, ext_list_tmp);
            if (ret < 0)
                return -1;
...
static int get_best_count(ext4_fsblk_t block_count)
{
    ...
    if (feature_incompat & EXT4_FEATURE_INCOMPAT_FLEX_BG)
    {
        ...
        ret = ((block_count - 1) /
               ((ext4_fsblk_t)blocks_per_group * flex_bg_num)) +
              1;
    }
    else
                          ...
                          struct FTW* ftwbuf EXT2FS_ATTR((unused)))
{
    ...
    if (current_uid != ROOT_UID && buf->st_uid != current_uid)
    {
        ...
        {
            PRINT_FILE_NAME(file);
            STATISTIC_ERR_MSG("File is not current user's file"
                              " or current user is not root");
        }
        return 0;
    }
    ...
    if (current_uid == ROOT_UID)
    {
        ...
        size_per_ext = blk_count * (buf->st_blksize / 1024) / now_ext_count;

        ratio = (float)(physical_ext_count - best_ext_count) * 100 / blk_count;
        ...
        if (mode_flag & DETAIL)
        {
            ...
            do
            {
                ...
                printf("[ext %d]:\tstart %llu:\tlogical "
                       "%llu:\tlen %llu\n",
                       count, ext_list_tmp->data.physical,
                       ext_list_tmp->data.logical, ext_list_tmp->data.len);
                ext_list_tmp = ext_list_tmp->next;
            } while (ext_list_tmp != logical_list_head);
        }
    ...
    if (mode_flag & DETAIL)
    {
        ...
        sprintf(msg_buffer, "[%u/%u]%s", defraged_file_count, total_count,
                file);
        if (current_uid == ROOT_UID)
        {
            if (strlen(msg_buffer) > 40)
                printf("\033[79;0H\033[K%s\n"
                       "%50d/%-10d%6llu KB\n",
                       msg_buffer, now_ext_count, best_ext_count, size_per_ext);
            else
    ...
    for (i = 0; i < SHOW_FRAG_FILES; i++)
    {
        if (ratio >= frag_rank[i].ratio)
        {
            ...
            {
                memset(&frag_rank[j], 0, sizeof(struct frag_statistic_ino));
                strncpy(frag_rank[j].msg_buffer, frag_rank[j - 1].msg_buffer,
                        strnlen(frag_rank[j - 1].msg_buffer, PATH_MAX));
                frag_rank[j].now_count = frag_rank[j - 1].now_count;
                frag_rank[j].best_count = frag_rank[j - 1].best_count;
                frag_rank[j].size_per_ext = frag_rank[j - 1].size_per_ext;
                frag_rank[j].ratio = frag_rank[j - 1].ratio;
            }
            memset(&frag_rank[i], 0, sizeof(struct frag_statistic_ino));
            strncpy(frag_rank[i].msg_buffer, file, strnlen(file, PATH_MAX));
...
static void print_progress(const char* file, loff_t start, loff_t file_size)
{
    ...
    printf("\033[79;0H\033[K[%u/%u]%s:\t%3d%%", defraged_file_count,
           total_count, file, min(percent, 100));
    fflush(stdout);
                       ...
                       struct fiemap_extent_list* ext_list_head)
{
    ...
    do
    {
        ...
        if (ret < 0)
        {
            ...
            {
                printf("\n");
                PRINT_ERR_MSG_WITH_ERRNO("Failed to get file map");
            }
            else
            {
                printf("\t[ NG ]\n");
        ...
        if (ret < 0)
        {
            ...
            {
                printf("\n");
                PRINT_ERR_MSG_WITH_ERRNO("Failed to free page");
            }
            else
            {
                printf("\t[ NG ]\n");
        ...
        if (defraged_ret < 0)
        {
            ...
            {
                printf("\n");
                PRINT_ERR_MSG_WITH_ERRNO("Failed to defrag with "
                       ...
                       struct FTW* ftwbuf EXT2FS_ATTR((unused)))
{
    ...
    /* Change list from physical to logical */
    ret = change_physical_to_logical(&orig_list_physical, &orig_list_logical);
    if (ret < 0)
    {
        if (mode_flag & DETAIL)
        {
    ...
    sprintf(tmp_inode_name, "%.*s.defrag", (int)strnlen(file, PATH_MAX), file);
    ...
    ret = change_physical_to_logical(&donor_list_physical, &donor_list_logical);
    if (ret < 0)
    {
        if (mode_flag & DETAIL)
        {
...
check_improvement:
    ...
    if (file_frags_start <= best || orig_physical_cnt <= donor_physical_cnt)
    {
        printf("\033[79;0H\033[K[%u/%u]%s:\t%3d%%", defraged_file_count,
               total_count, file, 100);
        if (mode_flag & DETAIL)
            printf("  extents: %d -> %d", file_frags_start, file_frags_start);
    ...
    if (mode_flag & DETAIL)
    {
        ...
        printf("  extents: %d -> %d", file_frags_start, file_frags_end);
        fflush(stdout);
    }
...
int main(int argc, char* argv[])
{
    ...
    for (i = optind; i < argc; i++)
    {
        ...
        if (S_ISLNK(buf.st_mode))
        {
            ...
            if (stat64(argv[i], &buf2) == 0 && S_ISBLK(buf2.st_mode))
                buf = buf2;
        }
        ...
        if (current_uid == ROOT_UID)
        {
            ...
            ret = ext2fs_open(dev_name, EXT2_FLAG_64BITS, 0, block_size,
                              unix_io_manager, &fs);
            if (ret)
            {
                if (mode_flag & DETAIL)
        ...
        switch (arg_type)
        {
            ...
            case DIRNAME:
                ...
                if (dir_name[mount_dir_len] != '\0')
                {
                    ...
                    if (strncmp(lost_found_dir, dir_name,
                                strnlen(lost_found_dir, PATH_MAX)) == 0 &&
                        (dir_name[strnlen(lost_found_dir, PATH_MAX)] == '\0' ||
                         dir_name[strnlen(lost_found_dir, PATH_MAX)] == '/'))
                    {
            ...
            case DEVNAME:
                ...
                if (mode_flag & STATISTIC)
                {
                    ...
                    if (!(mode_flag & DETAIL) && current_uid != ROOT_UID)
                    {
                        printf(" Done.\n");
                        success_flag = 1;
                        continue;
                    ...
                    nftw64(dir_name, file_statistic, FTW_OPEN_FD, flags);

                    if (succeed_cnt != 0 && current_uid == ROOT_UID)
                    {
                        if (mode_flag & DETAIL)
                            printf("\n");
                        printf("%-40s%10s/%-10s%9s\n", "<Fragmented files>",
                        ...
                        {
                            if (strlen(frag_rank[j].msg_buffer) > 37)
                            {
                                printf("%d. %s\n%50d/"
                                       "%-10d%6llu KB\n",
                                       j + 1, frag_rank[j].msg_buffer,
                                       frag_rank[j].now_count,
                                       frag_rank[j].best_count,
                                       frag_rank[j].size_per_ext);
                            }
                            else if (strlen(frag_rank[j].msg_buffer) > 0)
                            {
                                printf("%d. %-37s%10d/"
                                       "%-10d%6llu KB\n",
                                       j + 1, frag_rank[j].msg_buffer,
        ...
        if (mode_flag & STATISTIC)
        {
            ...
            else
            {
                ...
                files_ratio =
                    (float)(extents_before_defrag - extents_after_defrag) *
