...
struct gcry_mpi
{
    ...
    unsigned int
        flags;     /* Bit 0: Array to be allocated in secure memory space.*/
...
#define mpi_alloc_secure(n)                                                    \
    _gcry_mpi_debug_alloc_secure((n), M_DBGINFO(__LINE__))
...
byte* _gcry_mpi_get_buffer_extra(gcry_mpi_t a, unsigned int fill_le,
                                 int extraalloc, unsigned int* r_nbytes,
                                 int* sign);
byte* _gcry_mpi_get_secure_buffer(gcry_mpi_t a, unsigned int fill_le,
                                  unsigned* r_nbytes, int* sign);
void _gcry_mpi_set_buffer(gcry_mpi_t a, const void* buffer, unsigned int nbytes,
                          int sign);
gpg_err_code_t _gcry_mpi_to_octet_string(unsigned char** r_frame, void* space,
...
void _gcry_mpi_mulpowm(gcry_mpi_t res, gcry_mpi_t* basearray,
                       gcry_mpi_t* exparray, gcry_mpi_t mod);
...
void _gcry_mpi_ec_dup_point(mpi_point_t result, mpi_point_t point,
                            mpi_ec_t ctx);
