...
#define GA_RECORD_BROWSER_GET_PRIVATE(o)                                       \
    (G_TYPE_INSTANCE_GET_PRIVATE((o), GA_TYPE_RECORD_BROWSER,                  \
                                 GaRecordBrowserPrivate))
...
static void ga_record_browser_set_property(GObject* object, guint property_id,
                                           const GValue* value,
                                           GParamSpec* pspec)
{
...
static void ga_record_browser_get_property(GObject* object, guint property_id,
...
static void
    ga_record_browser_class_init(GaRecordBrowserClass* ga_record_browser_class)
{
    ...
    signals[NEW] = g_signal_new(
        "new-record", G_OBJECT_CLASS_TYPE(ga_record_browser_class),
        G_SIGNAL_RUN_LAST, 0, NULL, NULL,
        _ga_signals_marshal_VOID__INT_ENUM_STRING_UINT_UINT_POINTER_INT_INT,
        G_TYPE_NONE, 8, G_TYPE_INT, GA_TYPE_PROTOCOL, G_TYPE_STRING,
        G_TYPE_UINT, G_TYPE_UINT, G_TYPE_POINTER, G_TYPE_INT,
        GA_TYPE_LOOKUP_RESULT_FLAGS);

    signals[REMOVED] = g_signal_new(
        "removed-record", G_OBJECT_CLASS_TYPE(ga_record_browser_class),
        G_SIGNAL_RUN_LAST, 0, NULL, NULL,
        _ga_signals_marshal_VOID__INT_ENUM_STRING_UINT_UINT_POINTER_INT_INT,
        G_TYPE_NONE, 8, G_TYPE_INT, GA_TYPE_PROTOCOL, G_TYPE_STRING,
        G_TYPE_UINT, G_TYPE_UINT, G_TYPE_POINTER, G_TYPE_INT,
    ...
    signals[ALL_FOR_NOW] = g_signal_new(
        "all-for-now", G_OBJECT_CLASS_TYPE(ga_record_browser_class),
        G_SIGNAL_RUN_LAST, 0, NULL, NULL, g_cclosure_marshal_VOID__VOID,
        G_TYPE_NONE, 0);

    signals[CACHE_EXHAUSTED] = g_signal_new(
        "cache-exhausted", G_OBJECT_CLASS_TYPE(ga_record_browser_class),
        G_SIGNAL_RUN_LAST, 0, NULL, NULL, g_cclosure_marshal_VOID__VOID,
        G_TYPE_NONE, 0);

    signals[FAILURE] = g_signal_new(
        "failure", G_OBJECT_CLASS_TYPE(ga_record_browser_class),
        G_SIGNAL_RUN_LAST, 0, NULL, NULL, g_cclosure_marshal_VOID__POINTER,
        G_TYPE_NONE, 1, G_TYPE_POINTER);

    param_spec = g_param_spec_enum(
        "protocol", "Avahi protocol to browse", "Avahi protocol to browse",
        GA_TYPE_PROTOCOL, GA_PROTOCOL_UNSPEC,
        G_PARAM_READWRITE | G_PARAM_STATIC_NAME | G_PARAM_STATIC_BLURB);
    g_object_class_install_property(object_class, PROP_PROTOCOL, param_spec);

    param_spec = g_param_spec_int(
        "interface", "interface index", "Interface use for browser",
        AVAHI_IF_UNSPEC, G_MAXINT, AVAHI_IF_UNSPEC,
        G_PARAM_READWRITE | G_PARAM_STATIC_NAME | G_PARAM_STATIC_BLURB);
    g_object_class_install_property(object_class, PROP_IFINDEX, param_spec);

    param_spec = g_param_spec_string(
        "name", "record name", "Record name to browse for", NULL,
        G_PARAM_READWRITE | G_PARAM_STATIC_NAME | G_PARAM_STATIC_BLURB);
    g_object_class_install_property(object_class, PROP_NAME, param_spec);

    param_spec = g_param_spec_uint(
        "type", "record type", "Record type to browse for", 0, G_MAXUINT16, 0,
        G_PARAM_READWRITE | G_PARAM_STATIC_NAME | G_PARAM_STATIC_BLURB);
    g_object_class_install_property(object_class, PROP_TYPE, param_spec);

    param_spec = g_param_spec_uint(
        "class", "record class", "Record class to browse for", 0, G_MAXUINT16,
        0, G_PARAM_READWRITE | G_PARAM_STATIC_NAME | G_PARAM_STATIC_BLURB);
    g_object_class_install_property(object_class, PROP_CLASS, param_spec);

    param_spec = g_param_spec_enum(
        "flags", "Lookup flags for the browser", "Browser lookup flags",
        GA_TYPE_LOOKUP_FLAGS, GA_LOOKUP_NO_FLAGS,
        G_PARAM_READWRITE | G_PARAM_STATIC_NAME | G_PARAM_STATIC_BLURB);
...
{
    return ga_record_browser_new_full(AVAHI_IF_UNSPEC, AVAHI_PROTO_UNSPEC, name,
                                      AVAHI_DNS_CLASS_IN, type, 0);
}

GaRecordBrowser* ga_record_browser_new_full(AvahiIfIndex interface,
                                            AvahiProtocol protocol,
                                            const gchar* name, guint16 clazz,
                                            guint16 type, GaLookupFlags flags)
{
    return g_object_new(GA_TYPE_RECORD_BROWSER, "interface", interface,
                        "protocol", protocol, "name", name, "class", clazz,
                        "type", type, "flags", flags, NULL);
}

static void _avahi_record_browser_cb(
    AVAHI_GCC_UNUSED AvahiRecordBrowser* r, AvahiIfIndex interface,
    AvahiProtocol protocol, AvahiBrowserEvent event, const char* name,
    uint16_t clazz, uint16_t type, const void* rdata, size_t rdata_size,
    AvahiLookupResultFlags flags, void* userdata)
{
    ...
    switch (event)
    {
        ...
        case AVAHI_BROWSER_REMOVE:
        {
            ...
            g_signal_emit(self, signals[signalid], 0, interface, protocol, name,
                          clazz, type, rdata, rdata_size, flags);
            break;
        }
        ...
        case AVAHI_BROWSER_FAILURE:
        {
            ...
            error = g_error_new(GA_ERROR, aerrno, "Browsing failed: %s",
...
gboolean ga_record_browser_attach(GaRecordBrowser* browser, GaClient* client,
                                  GError** error)
{
    GaRecordBrowserPrivate* priv = GA_RECORD_BROWSER_GET_PRIVATE(browser);

    priv->client = g_object_ref(client);
    priv->browser = avahi_record_browser_new(
        client->avahi_client, priv->interface, priv->protocol, priv->name,
        priv->class, priv->type, priv->flags, _avahi_record_browser_cb,
        browser);
