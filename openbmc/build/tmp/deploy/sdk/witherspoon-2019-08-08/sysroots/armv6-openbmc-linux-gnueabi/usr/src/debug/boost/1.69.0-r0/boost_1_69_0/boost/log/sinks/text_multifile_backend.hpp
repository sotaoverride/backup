...
/*!
 * An adapter class that allows to use regular formatters as file name
 * generators.
 */
...
class file_name_composer_adapter
{
  ...
  public:
     ...
     */
    explicit file_name_composer_adapter(
    ...
    file_name_composer_adapter(file_name_composer_adapter const& that) :
        m_Formatter(that.m_Formatter), m_FormattingStream(m_FileName)
    {
        m_FormattingStream.exceptions(std::ios_base::badbit |
                                      std::ios_base::failbit);
    ...
    {
        boost::log::aux::cleanup_guard<stream_type> cleanup1(
            m_FormattingStream);
        boost::log::aux::cleanup_guard<result_type::string_type> cleanup2(
            m_FileName);
 ...
 */
template <typename FormatterT>
inline file_name_composer_adapter<FormatterT>
 ...
 * The particular file is chosen upon each record's attribute values, which
 * allows to distribute records into individual files or to group records
 * related to some entity or process in a separate file.
 */
class text_multifile_backend : public basic_formatted_sink_backend<char>
{
  ...
  public:
    ...
    typedef boost::log::aux::light_function<filesystem::path(
        record_view const&)>
  ...
  public:
    /*!
     * Default constructor. The constructed sink backend has no file name
     * composer and thus will not write any files.
     */
    ...
    /*!
     * The method sets file name composer functional object. Log record
     * formatters are accepted, too.
     *
  ...
  private:
    ...
    BOOST_LOG_API void set_file_name_composer_internal(
