...
int main(int argc, char* argv[])
{
    ...
    while (1)
    {
        ...
        if ((list = dlist_new_with_delete(sizeof(Complex), complex_del)) ==
            NULL)
        {
            fprintf(stderr, "ERR dlist_new fail\n");
            return (2);
        ...
        if ((cfound = (Complex*)dlist_find_custom(list, ctemp,
                                                  complex_equal)) != NULL)
...
void complex_out(void* data)
{
    ...
    printf("cnumber %d label %s number %d \n", thisone->cnumber,
           thisone->sthing->label, thisone->sthing->number);
}
...
int complex_equal(void* a, void* b)
{
    if ((((Complex*)a)->cnumber == ((Complex*)b)->cnumber) &&
        (((Complex*)a)->sthing->number == ((Complex*)b)->sthing->number) &&
        strcmp(((Complex*)a)->sthing->label, ((Complex*)b)->sthing->label) == 0)
        return (1);
    return (0);
}
