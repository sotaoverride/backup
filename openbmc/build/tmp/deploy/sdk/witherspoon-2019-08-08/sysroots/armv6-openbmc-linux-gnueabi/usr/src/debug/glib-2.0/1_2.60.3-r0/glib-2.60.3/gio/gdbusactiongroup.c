...
static void action_info_free(gpointer user_data)
{
...
static ActionInfo* action_info_new_from_iter(GVariantIter* iter)
{
    ...
    if (!g_variant_iter_next(iter, "{s(b&g@av)}", &name, &enabled, &param_str,
                             &state))
        return NULL;
                             ...
                             G_VARIANT_TYPE("(asa{sb}a{sv}a{s(bgav)})")))
    {
        ...
        /* Enable changes */
        {
            ...
            while (g_variant_iter_next(iter, "{&sb}", &name, &enabled))
            {
                ...
                {
                    info->enabled = enabled;
                    g_action_group_action_enabled_changed(g_group, name,
                                                          enabled);
                }
            }
            g_variant_iter_free(iter);
...
static void g_dbus_action_group_describe_all_done(GObject* source,
                                                  GAsyncResult* result,
                                                  gpointer user_data)
{
    GDBusActionGroup* group = user_data;
    ...
    group->actions =
        g_hash_table_new_full(g_str_hash, g_str_equal, NULL, action_info_free);
...
static void g_dbus_action_group_async_init(GDBusActionGroup* group)
{
    if (group->subscription_id != 0)
        return;

    group->subscription_id = g_dbus_connection_signal_subscribe(
        group->connection, group->bus_name, "org.gtk.Actions", "Changed",
        group->object_path, NULL, G_DBUS_SIGNAL_FLAGS_NONE,
        g_dbus_action_group_changed, group, NULL);

    g_dbus_connection_call(
        group->connection, group->bus_name, group->object_path,
        "org.gtk.Actions", "DescribeAll", NULL, G_VARIANT_TYPE("(a{s(bgav)})"),
        G_DBUS_CALL_FLAGS_NONE, -1, NULL, g_dbus_action_group_describe_all_done,
        g_object_ref(group));
}

static gchar** g_dbus_action_group_list_actions(GActionGroup* g_group)
{
...
static gboolean g_dbus_action_group_query_action(
    GActionGroup* g_group, const gchar* action_name, gboolean* enabled,
...
static void g_dbus_action_group_activate_action_full(GRemoteActionGroup* remote,
...
static void g_dbus_action_group_change_action_state_full(
    ...
    GVariant* platform_data)
{
    ...
    g_dbus_connection_call(
        ...
        g_variant_new("(sv@a{sv})", action_name, value, platform_data), NULL,
        G_DBUS_CALL_FLAGS_NONE, -1, NULL, NULL, NULL);
}

static void g_dbus_action_group_change_state(GActionGroup* group,
                                             const gchar* action_name,
                                             GVariant* value)
{
...
static void g_dbus_action_group_activate(GActionGroup* group,
                                         const gchar* action_name,
                                         GVariant* parameter)
{
...
static void g_dbus_action_group_finalize(GObject* object)
{
    GDBusActionGroup* group = G_DBUS_ACTION_GROUP(object);

    if (group->subscription_id)
        g_dbus_connection_signal_unsubscribe(group->connection,
    ...
    G_OBJECT_CLASS(g_dbus_action_group_parent_class)->finalize(object);
}

static void g_dbus_action_group_init(GDBusActionGroup* group)
{
}

static void g_dbus_action_group_class_init(GDBusActionGroupClass* class)
{
...
gboolean g_dbus_action_group_sync(GDBusActionGroup* group,
                                  GCancellable* cancellable, GError** error)
{
    ...
    if (reply != NULL)
    {
        ...
        g_assert(group->actions == NULL);
        group->actions = g_hash_table_new_full(g_str_hash, g_str_equal, NULL,
                                               action_info_free);
