...
struct link_map* _dl_new_object(char* realname, const char* libname, int type,
                                struct link_map* loader, int mode, Lmid_t nsid)
{
    ...
    unsigned int naudit =
        GLRO(dl_naudit) ?: ((mode & __RTLD_OPENEXEC) ? DL_NNS : 0);
...
#define audit_space 0
    ...
    new->l_symbolic_searchlist.r_list =
        (struct link_map**)((char*)(new + 1) + audit_space);
