...
static NORETURN void whine_about_eof(BOOL allow_kluge)
{
    ...
    rprintf(FERROR,
            RSYNC_NAME ": connection unexpectedly closed "
        ...
        if (FD_ISSET(fd, &r_fds))
        {
            ...
            if (n < 0)
            {
                ...
                rsyserr(FERROR, errno,
                        "safe_read failed to read %ld bytes [%s]", (long)len,
                        who_am_i());
                exit_cleanup(RERR_STREAMIO);
            }
            if ((got += (size_t)n) == len)
...
static void forward_filesfrom_data(void)
{
    ...
    if (filesfrom_convert && len)
    {
        ...
        if (ff_lastchar == '\0')
            flags |= ICB_INIT;
        /* Convert/send each null-terminated string separately, skipping
         * empties. */
        while (s != eob)
        {
            if (*s++ == '\0')
...
void reduce_iobuf_size(xbuf* out, size_t new_size)
{
    if (new_size < out->size)
    {
        ...
        {
            const char* name = out == &iobuf.out
                                   ? "iobuf.out"
                                   : out == &iobuf.msg ? "iobuf.msg" : NULL;
            if (name)
            {
                rprintf(FINFO, "[%s] reduced size of %s (-%d)\n", who_am_i(),
                        name, (int)(out->size - new_size));
            }
        }
        out->size = new_size;
    }
...
static char* perform_io(size_t needed, int flags)
{
    ...
    switch (flags & PIO_NEED_FLAGS)
    {
        case PIO_NEED_INPUT:
            ...
            if (msgs2stderr && DEBUG_GTE(IO, 3))
            {
                rprintf(FINFO, "[%s] perform_io(%ld, %sinput)\n", who_am_i(),
                        (long)needed,
                        flags & PIO_CONSUME_INPUT ? "consume&" : "");
            }
            break;
        ...
        case PIO_NEED_OUTROOM:
            ...
            {
                fprintf(stderr,
                        "need to write %ld bytes, iobuf.out.buf is only %ld "
                        "bytes.\n",
                        (long)needed,
                        (long)(iobuf.out.size - iobuf.out_empty_len));
                exit_cleanup(RERR_PROTOCOL);
            }
            ...
            if (msgs2stderr && DEBUG_GTE(IO, 3))
            {
                rprintf(FINFO,
                        ...
                        iobuf.out.len + needed > iobuf.out.size
                            ? (long)(iobuf.out.len + needed - iobuf.out.size)
                            : 0L);
            }
            break;
        ...
        case PIO_NEED_MSGROOM:
            ...
            {
                fprintf(stderr,
                        "need to write %ld bytes, iobuf.msg.buf is only %ld "
                        "bytes.\n",
            ...
            if (msgs2stderr && DEBUG_GTE(IO, 3))
            {
                rprintf(FINFO,
                        ...
                        iobuf.msg.len + needed > iobuf.msg.size
                            ? (long)(iobuf.msg.len + needed - iobuf.msg.size)
                            : 0L);
            }
            break;
    ...
    while (1)
    {
        ...
        /* Only do more filesfrom processing if there is enough room in the out
         * buffer. */
        if (ff_forward_fd >= 0 &&
            iobuf.out.size - iobuf.out.len > FILESFROM_BUFLEN * 2)
        {
        ...
        if (iobuf.out_fd >= 0)
        {
            if (iobuf.raw_flushing_ends_before ||
                (!iobuf.msg.len && iobuf.out.len > iobuf.out_empty_len &&
                 !(flags & PIO_NEED_MSGROOM)))
            {
                if (OUT_MULTIPLEXED && !iobuf.raw_flushing_ends_before)
                {
                    /* The iobuf.raw_flushing_ends_before value can point off
                     * the end of the iobuf.out buffer for a while, for easier
                     * subtracting. */
                    iobuf.raw_flushing_ends_before =
                        iobuf.out.pos + iobuf.out.len;

                    SIVAL(iobuf.out.buf + iobuf.raw_data_header_pos, 0,
                          ((MPLEX_BASE + (int)MSG_DATA) << 24) + iobuf.out.len -
                              4);

                    if (msgs2stderr && DEBUG_GTE(IO, 1))
                    {
                        rprintf(FINFO, "[%s] send_msg(%d, %ld)\n", who_am_i(),
                                (int)MSG_DATA, (long)iobuf.out.len - 4);
                    }
                    ...
                    else if (iobuf.raw_data_header_pos + 4 > iobuf.out.size)
                    {
                        ...
                        reduce_iobuf_size(&iobuf.out,
                                          iobuf.raw_data_header_pos);
                        iobuf.raw_data_header_pos = 0;
                    }
                    /* Yes, it is possible for this to make len > size for a
                     * while. */
                    iobuf.out.len += 4;
                }
        ...
        {
            size_t len = iobuf.raw_flushing_ends_before
                             ? iobuf.raw_flushing_ends_before - out->pos
                             : out->len;
            int n;
            ...
            if ((n = write(iobuf.out_fd, out->buf + out->pos, len)) <= 0)
            {
                ...
                else
                {
                    ...
                    msgs2stderr = 1;
                    iobuf.out_fd = -2;
                    iobuf.out.len = iobuf.msg.len =
                        iobuf.raw_flushing_ends_before = 0;
                    rsyserr(FERROR_SOCKET, errno, "[%s] write error",
                            who_am_i());
                    drain_multiplex_messages();
                    exit_cleanup(RERR_SOCKETIO);
                }
...
void noop_io_until_death(void)
{
    ...
    if (!iobuf.in.buf || !iobuf.out.buf || iobuf.in_fd < 0 ||
        iobuf.out_fd < 0 || kluge_around_eof)
        return;
...
/* Buffer a message for the multiplexed output stream.  Is not used for (normal)
 * MSG_DATA. */
int send_msg(enum msgcode code, const char* buf, size_t len, int convert)
{
    ...
    size_t needed, pos;
    BOOL want_debug =
        DEBUG_GTE(IO, 1) && convert >= 0 && (msgs2stderr || code != MSG_INFO);
    ...
    if (convert > 0)
    {
        /* Ensuring double-size room leaves space for maximal conversion
         * expansion. */
        needed = len * 2 + 4 + 3;
    }
    else
...
static void check_for_d_option_error(const char* msg)
{
    ...
    if (*msg == '-' || (colon = strchr(msg, ':')) == NULL ||
        strncmp(colon, REMOTE_OPTION_ERROR2, sizeof REMOTE_OPTION_ERROR2 - 1) !=
            0)
        return;
    ...
    while (1)
    {
        ...
        if (dot_pos)
        {
            if (request_p && request_len < 1024)
            {
                ...
                if (!(*request_p = realloc_array(*request_p, char,
                                                 request_len + len + 1)))
                    out_of_memory("read_args");
...
void maybe_send_keepalive(time_t now, int flags)
{
    if (flags & MSK_ACTIVE_RECEIVER)
        last_io_in =
            now; /* Fudge things when we're working hard on the files. */

    /* Early in the transfer (before the receiver forks) the receiving side
     * doesn't care if it hasn't sent data in a while as long as it is receiving
     * data (in fact, a pre-3.1.0 rsync would die if we tried to send it a keep
     * alive during this time).  So, if we're an early-receiving proc, just
     * return and let the incoming data determine if we timeout. */
    ...
    switch (tag)
    {
        ...
        case MSG_DELETED:
            ...
            if (ic_recv != (iconv_t)-1)
            {
                ...
                while (msg_bytes)
                {
                    size_t len = msg_bytes > sizeof ibuf - inbuf.len
                                     ? sizeof ibuf - inbuf.len
                                     : msg_bytes;
        ...
        case MSG_SUCCESS:
            ...
            invalid_msg:
                rprintf(FERROR, "invalid multi-message %d:%lu [%s%s]\n", tag,
                        (unsigned long)msg_bytes, who_am_i(),
                        inc_recurse ? "/inc" : "");
                exit_cleanup(RERR_STREAMIO);
            }
            val = raw_read_int();
        ...
        case MSG_WARNING:
            ...
            {
            overflow:
                rprintf(FERROR, "multiplexing overflow %d:%lu [%s%s]\n", tag,
                        (unsigned long)msg_bytes, who_am_i(),
                        inc_recurse ? "/inc" : "");
                exit_cleanup(RERR_STREAMIO);
            }
            raw_read_buf(data, msg_bytes);
            /* We don't set in_multiplexed value back to 1 before writing this
             * message because the write might loop back and read yet another
             * message, over and over again, while waiting for room to put the
...
void write_vstring(int f, const char* str, int len)
{
    ...
    if (len > 0x7F)
    {
        if (len > 0x7FFF)
        {
            rprintf(FERROR, "attempting to send over-long vstring (%d > %d)\n",
                    len, 0x7FFF);
            exit_cleanup(RERR_PROTOCOL);
        }
        *lb++ = len / 0x100 + 0x80;
