...
tt_face_load_kern(TT_Face face, FT_Stream stream)
{
    ...
    for (nn = 0; nn < num_tables; nn++)
    {
        ...
        if ((coverage & 3U) != 0x0001 || p + 8 > p_next)
            goto NextTable;
...
tt_face_get_kerning(TT_Face face, FT_UInt left_glyph, FT_UInt right_glyph)
{
    ...
    for (count = face->num_kern_tables; count > 0 && p + 6 <= p_limit;
         count--, mask <<= 1)
    {
        FT_Byte* base = p;
        FT_Byte* next;
        ...
        switch (coverage >> 8)
        {
            ...
            break;
                ...
                /*
                 * We don't support format 2 because we haven't seen a single
                 * font using it in real life...
                 */
