 ...
 */
int _netsnmp_ioctl_ipaddress_container_load_v4(netsnmp_container* container,
                                               int idx_offset)
{
    ...
    {
        snmp_log_perror("_netsnmp_ioctl_ipaddress_container_load_v4: could not "
                        "create socket");
        return -1;
    }
    ...
    for (i = 0; i < interfaces; ++i, ++ifrp)
    {
        ...
        DEBUGMSGTL(("access:ipaddress:container", " interface %d, %s\n", i,
                    ifrp->ifr_name));
        ...
        memcpy(entry->ia_address, &si->sin_addr.s_addr, entry->ia_address_len);
        ...
        {
            snmp_log(LOG_ERR, "error getting netmask for interface %d\n", i);
            netsnmp_access_ipaddress_entry_free(entry);
            continue;
        }
        ...
        {
            snmp_log(LOG_ERR, "error getting if_flags for interface %d\n", i);
            netsnmp_access_ipaddress_entry_free(entry);
            continue;
        }
        ...
        DEBUGIF("access:ipaddress:container")
        {
            ...
            if (4 == entry->ia_address_len)
                DEBUGMSGT_NC(("access:ipaddress:container",
                              " address %d.%d.%d.%d\n", entry->ia_address[0],
                              entry->ia_address[1], entry->ia_address[2],
                              entry->ia_address[3]));
            DEBUGMSGT_NC(
                ("access:ipaddress:container", "flags 0x%x\n", extras->flags));
        ...
        if (bcastentry)
        {
            ...
            {
                DEBUGMSGTL(("access:ipaddress:container",
                            "error with ipaddress_entry: insert broadcast "
                            "entry into container failed.\n"));
        ...
        {
            DEBUGMSGTL((
                "access:ipaddress:container",
                "error with ipaddress_entry: insert into container failed.\n"));
            NETSNMP_LOGONCE(
                (LOG_ERR, "Duplicate IPv4 address detected, some interfaces "
 ...
 */
static int _next_alias(const char* if_name)
{
...
int _netsnmp_ioctl_ipaddress_set_v4(netsnmp_ipaddress_entry* entry)
{
    ...
    {
        snmp_log_perror(
            "_netsnmp_ioctl_ipaddress_set_v4: couldn't create socket");
        return -2;
    }
    ...
    if ('\0' == extras->name[0])
    {
        ...
        snprintf(ifrq.ifr_name, sizeof(ifrq.ifr_name), "%s:%d", name,
                 alias_idx);
    ...
    memcpy(&sin->sin_addr.s_addr, entry->ia_address, entry->ia_address_len);
...
int _netsnmp_ioctl_ipaddress_delete_v4(netsnmp_ipaddress_entry* entry)
{
    ...
    {
        snmp_log_perror(
            "_netsnmp_ioctl_ipaddress_delete_v4: couldn't create socket");
        return -2;
    }
...
int _netsnmp_ioctl_ipaddress_v6(netsnmp_ipaddress_entry* entry, int operation)
{
    ...
    DEBUGMSGT(
        ("access:ipaddress:set", "_netsnmp_ioctl_ipaddress_set_v6 started\n"));
    ...
    memcpy(&ifrq.ifr6_addr, entry->ia_address, entry->ia_address_len);
    ...
    DEBUGMSGT(
        ("access:ipaddress:set", "_netsnmp_ioctl_ipaddress_set_v6 finished\n"));
    return 0;
#else
 ...
 */
int netsnmp_access_ipaddress_ioctl_get_interface_count(int sd,
                                                       struct ifconf* ifc)
{
    int lastlen = 0, i;
...
static void _print_flags(short flags)
{
    ...
    if (unknown)
        DEBUGMSGT_NC(
            ("access:ipaddress:container", "  unknown 0x%x\n", unknown));
}
