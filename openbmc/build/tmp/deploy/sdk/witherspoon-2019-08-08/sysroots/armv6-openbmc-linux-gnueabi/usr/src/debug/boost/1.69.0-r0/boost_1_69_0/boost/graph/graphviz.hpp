...
inline std::string escape_dot_string(const T& obj)
{
    ...
    static sregex valid_unquoted_id =
        (((alpha | '_') >> *_w) |
         (!as_xpr('-') >> (('.' >> *_d) | (+_d >> !('.' >> *_d)))));
...
template <class Name>
inline label_writer<Name> make_label_writer(Name n)
{
...
inline void write_all_attributes(Attributes attributes, const std::string& name,
                                 std::ostream& out)
{
...
{
    graph_attributes_writer(GraphGraphAttributes gg, GraphNodeAttributes gn,
                            GraphEdgeAttributes ge) :
        g_attributes(gg),
        n_attributes(gn), e_attributes(ge)
    {
    }
...
{
    return graph_attributes_writer<GAttrMap, NAttrMap, EAttrMap>(g_attr, n_attr,
                                                                 e_attr);
}
...
{
    typedef
        typename graph_property<Graph, graph_graph_attribute_t>::type GAttrMap;
    typedef
        typename graph_property<Graph, graph_vertex_attribute_t>::type NAttrMap;
    typedef
...
template <typename Graph>
attributes_writer<typename property_map<Graph, edge_attribute_t>::const_type>
...
template <typename Graph>
attributes_writer<typename property_map<Graph, vertex_attribute_t>::const_type>
...
{
  public:
    dynamic_graph_properties_writer(const dynamic_properties& dp,
                                    const Graph& g) :
        g(&g),
        dp(&dp)
    {
...
template <typename Graph>
inline void write_graphviz_dp(
    std::ostream& out, const Graph& g, const dynamic_properties& dp,
    const std::string& node_id = "node_id" BOOST_GRAPH_ENABLE_IF_MODELS_PARM(
        Graph, vertex_list_graph_tag))
{
    typedef typename graph_traits<Graph>::vertex_descriptor Vertex;
...
class mutate_graph
{
  public:
    ...
    virtual void do_add_edge(const edge_t& edge, const node_t& source,
                             const node_t& target) = 0;

    virtual void set_node_property(const id_t& key, const node_t& node,
                                   const id_t& value) = 0;

    virtual void set_edge_property(const id_t& key, const edge_t& edge,
                                   const id_t& value) = 0;
...
class mutate_graph_impl : public mutate_graph
{
  ...
  public:
    mutate_graph_impl(MutableGraph& graph, dynamic_properties& dp,
        ...
        graph_(graph),
        dp_(dp), node_id_prop_(node_id_prop)
    {
    }
...
class mutate_graph_impl<compressed_sparse_row_graph<
    Directed, VertexProperty, EdgeProperty, GraphProperty, Vertex, EdgeIndex>>
    : public mutate_graph
{
  ...
  public:
    mutate_graph_impl(CSRGraph& graph, dynamic_properties& dp,
                      std::string node_id_prop) :
        graph_(graph),
        dp_(dp), vertex_count(0), node_id_prop_(node_id_prop)
    {
    }
    ...
    void finish_building_graph()
    {
        typedef compressed_sparse_row_graph<directedS, no_property, bgl_edge_t,
                                            GraphProperty, Vertex, EdgeIndex>
            TempCSRGraph;
        TempCSRGraph temp(edges_are_unsorted_multi_pass, edges_to_add.begin(),
                          edges_to_add.end(), counting_iterator<bgl_edge_t>(0),
                          vertex_count);
...
#else // Non-Spirit parser
    return read_graphviz_new(std::string(user_first, user_last), graph, dp,
                             node_id);
#endif
}
