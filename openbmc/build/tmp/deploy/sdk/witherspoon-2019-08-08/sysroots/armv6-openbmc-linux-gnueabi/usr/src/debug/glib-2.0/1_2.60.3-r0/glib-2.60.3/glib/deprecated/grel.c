...
static gboolean tuple_equal_2(gconstpointer v_a, gconstpointer v_b)
{
...
static guint tuple_hash_2(gconstpointer v_a)
{
...
static GHashFunc tuple_hash(gint fields)
{
    switch (fields)
    {
        case 2:
...
static GEqualFunc tuple_equal(gint fields)
{
    switch (fields)
    {
        case 2:
...
static void relation_delete_value_tuple(gpointer tuple_key,
...
static void g_relation_free_array(gpointer key, gpointer value,
                                  gpointer user_data)
{
        ...
        for (i = 0; i < relation->fields; i += 1)
        {
            if (relation->hashed_tuple_tables[i])
            {
                g_hash_table_foreach(relation->hashed_tuple_tables[i],
                                     g_relation_free_array, NULL);
                g_hash_table_destroy(relation->hashed_tuple_tables[i]);
            }
        }

        g_hash_table_foreach(relation->all_tuples, relation_delete_value_tuple,
                             relation);
 ...
 * Deprecated: 2.26: Rarely used API
 **/
void g_relation_index(GRelation* relation, gint field, GHashFunc hash_func,
                      GEqualFunc key_equal_func)
{
    g_return_if_fail(relation != NULL);

    g_return_if_fail(relation->count == 0 &&
                     relation->hashed_tuple_tables[field] == NULL);

    relation->hashed_tuple_tables[field] =
...
void g_relation_insert(GRelation* relation, ...)
{
    ...
    for (i = 0; i < relation->fields; i += 1)
    {
        ...
        {
            per_key_table = g_hash_table_new(tuple_hash(relation->fields),
                                             tuple_equal(relation->fields));
...
static void g_relation_delete_tuple(gpointer tuple_key, gpointer tuple_value,
                                    gpointer user_data)
{
...
static void g_relation_select_tuple(gpointer tuple_key, gpointer tuple_value,
                                    gpointer user_data)
{
    gpointer* tuple = (gpointer*)tuple_value;
 ...
 * Deprecated: 2.26: Rarely used API
 **/
void g_tuples_destroy(GTuples* tuples0)
{
 ...
 * Deprecated: 2.26: Rarely used API
 **/
gpointer g_tuples_index(GTuples* tuples0, gint index, gint field)
{
    GRealTuples* tuples = (GRealTuples*)tuples0;
...
static void g_relation_print_one(gpointer tuple_key, gpointer tuple_value,
                                 gpointer user_data)
{
    gint i;
    GString* gstring;
...
static void g_relation_print_index(gpointer tuple_key, gpointer tuple_value,
                                   gpointer user_data)
{
    GRelation* rel = (GRelation*)user_data;
    ...
    g_hash_table_foreach(table, g_relation_print_one, rel);
}
...
void g_relation_print(GRelation* relation)
{
    ...
    g_log(G_LOG_DOMAIN, G_LOG_LEVEL_INFO, "*** all tuples (%d)",
          relation->count);

    g_hash_table_foreach(relation->all_tuples, g_relation_print_one, relation);
    ...
    for (i = 0; i < relation->fields; i += 1)
    {
        ...
        g_hash_table_foreach(relation->hashed_tuple_tables[i],
                             g_relation_print_index, relation);
    }
}
