...
triple_timestamp* triple_timestamp_get(triple_timestamp* ts)
{
    ...
    ts->monotonic = now(CLOCK_MONOTONIC);
    ts->boottime =
        clock_boottime_supported() ? now(CLOCK_BOOTTIME) : USEC_INFINITY;
...
triple_timestamp* triple_timestamp_from_realtime(triple_timestamp* ts, usec_t u)
{
    ...
    ts->boottime = clock_boottime_supported()
                       ? usec_sub_signed(now(CLOCK_BOOTTIME), delta)
                       : USEC_INFINITY;
...
dual_timestamp* dual_timestamp_from_boottime_or_monotonic(dual_timestamp* ts,
                                                          usec_t u)
{
    int64_t delta;
                                       ...
                                       bool us)
{
    ...
    /* The weekdays in non-localized (English) form. We use this instead of the
     * localized form, so that our generated timestamps may be parsed with
     * parse_timestamp(), and always read the same. */
    ...
    if (utc)
    {
        /* If this is UTC then let's explicitly use the "UTC" string here,
         * because gmtime_r() normally uses the obsolete "GMT" instead. */
        if (n + 5 > l)
    ...
    else if (!isempty(tm.tm_zone))
    {
        ...
        if (n + 1 + tn + 1 > l)
        {
            ...
            if (n + 1 + _POSIX_TZNAME_MAX + 1 > l)
                return NULL; /* Not even enough space for the POSIX minimum (of
                                6)? In that case, complain that it doesn't fit
                              */

            /* So the time zone doesn't fit in fully, but the caller passed
             * enough space for the POSIX minimum time zone length. In this case
             * suppress the timezone entirely, in order not to dump an overly
             * long, hard to read string on the user. This should be safe,
             * because the user will
             * assume the local timezone anyway if none is shown. And so does
             * parse_timestamp(). */
        }
        else
        {
...
char* format_timestamp_relative(char* buf, size_t l, usec_t t)
{
    ...
    if (d >= USEC_PER_YEAR)
        snprintf(buf, l, USEC_FMT " years " USEC_FMT " months %s",
                 d / USEC_PER_YEAR, (d % USEC_PER_YEAR) / USEC_PER_MONTH, s);
    else if (d >= USEC_PER_MONTH)
        snprintf(buf, l, USEC_FMT " months " USEC_FMT " days %s",
                 d / USEC_PER_MONTH, (d % USEC_PER_MONTH) / USEC_PER_DAY, s);
    else if (d >= USEC_PER_WEEK)
        snprintf(buf, l, USEC_FMT " weeks " USEC_FMT " days %s",
                 d / USEC_PER_WEEK, (d % USEC_PER_WEEK) / USEC_PER_DAY, s);
        ...
        snprintf(buf, l, USEC_FMT "h " USEC_FMT "min %s", d / USEC_PER_HOUR,
                 (d % USEC_PER_HOUR) / USEC_PER_MINUTE, s);
    else if (d >= 5 * USEC_PER_MINUTE)
        snprintf(buf, l, USEC_FMT "min %s", d / USEC_PER_MINUTE, s);
    else if (d >= USEC_PER_MINUTE)
        snprintf(buf, l, USEC_FMT "min " USEC_FMT "s %s", d / USEC_PER_MINUTE,
...
char* format_timespan(char* buf, size_t l, usec_t t, usec_t accuracy)
{
    ...
    for (i = 0; i < ELEMENTSOF(table); i++)
    {
        ...
        if (t < USEC_PER_MINUTE && b > 0)
        {
            ...
            if (j > 0)
            {
                k = snprintf(p, l, "%s" USEC_FMT ".%0*" PRI_USEC "%s",
                             p > buf ? " " : "", a, j, b, table[i].suffix);
        ...
        if (!done)
        {
            k = snprintf(p, l, "%s" USEC_FMT "%s", p > buf ? " " : "", a,
                         table[i].suffix);
...
static int parse_timestamp_impl(const char* t, usec_t* usec, bool with_tz)
{
    ...
    if (!with_tz)
    {
        ...
        else
        {
            ...
            /* See if the timestamp is suffixed by either the DST or non-DST
             * local timezone. Note that we only support the local timezones
             * here, nothing else. Not because we wouldn't want to, but simply
             * because there are no nice APIs available to cover this. By
             * accepting the local time zone strings, we make sure that all
             * timestamps written by format_timestamp() can be parsed correctly,
             * even though we don't support arbitrary timezone specifications.
             */
...
int parse_timestamp(const char* t, usec_t* usec)
{
    ...
    if (r == 0)
    {
        ...
        /* If there is a timezone that matches the tzname fields, leave the
         * parsing to the implementation. Otherwise just cut it off. */
...
bool clock_boottime_supported(void)
{
    ...
    /* Note that this checks whether CLOCK_BOOTTIME is available in general as
     * well as available for timerfds()! */
...
int get_timezone(char** tz)
{
    ...
    e = PATH_STARTSWITH_SET(t, "/usr/share/zoneinfo/",
                            "../usr/share/zoneinfo/");
    if (!e)
        return -EINVAL;
...
int time_change_fd(void)
{
    ...
    /* We only care for the cancellation event, hence we set the timeout to the
     * latest possible value. */
    ...
    /* Uses TFD_TIMER_CANCEL_ON_SET to get notifications whenever CLOCK_REALTIME
     * makes a jump relative to CLOCK_MONOTONIC. */

    fd = timerfd_create(CLOCK_REALTIME, TFD_NONBLOCK | TFD_CLOEXEC);
    if (fd < 0)
        return -errno;

    if (timerfd_settime(fd, TFD_TIMER_ABSTIME | TFD_TIMER_CANCEL_ON_SET, &its,
                        NULL) < 0)
        return -errno;
