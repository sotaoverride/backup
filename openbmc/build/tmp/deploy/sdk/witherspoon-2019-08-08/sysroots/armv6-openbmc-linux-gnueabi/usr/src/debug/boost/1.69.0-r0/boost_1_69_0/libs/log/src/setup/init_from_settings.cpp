...
// This warning is caused by a compiler bug which is exposed when
// boost::optional is used: https://gcc.gnu.org/bugzilla/show_bug.cgi?id=47679
// It has to be disabled here, before any code is included, since otherwise it
// doesn't help and the warning is still emitted.
...
BOOST_LOG_ANONYMOUS_NAMESPACE
{
    ...
    {
        std::string descr =
            std::string("Invalid parameter \"") + param_name + "\" value";
        BOOST_LOG_THROW_DESCR(invalid_value, descr);
    }

    //! Extracts an integral value from parameter value
    template <typename IntT, typename CharT>
    inline IntT param_cast_to_int(const char* param_name,
                                  std::basic_string<CharT> const& value)
    {
        IntT res = 0;
        typedef
            typename mpl::if_<is_unsigned<IntT>,
                              qi::extract_uint<IntT, 10, 1, -1>,
                              qi::extract_int<IntT, 10, 1, -1>>::type extract;
    ...
    template <typename CharT>
    inline bool param_cast_to_bool(const char* param_name,
                                   std::basic_string<CharT> const& value)
    {
        ...
        if (keyword.size() == len && std::equal(begin, end, keyword.c_str(),
        ...
        else
        {
            ...
            if (keyword.size() == len &&
                std::equal(begin, end, keyword.c_str(),
                           is_case_insensitive_equal()))
            {
                return false;
            }
    ...
    template <typename CharT>
    inline std::string param_cast_to_address(
        const char* param_name, std::basic_string<CharT> const& value)
    {
    ...
    template <typename CharT>
    inline bool is_weekday(
        const CharT* str, std::size_t len,
        boost::log::basic_string_literal<CharT> const& weekday,
        boost::log::basic_string_literal<CharT> const& short_weekday)
    {
        return (len == weekday.size() &&
                std::equal(weekday.begin(), weekday.end(), str)) ||
               (len == short_weekday.size() &&
                std::equal(short_weekday.begin(), short_weekday.end(), str));
    }

    //! The function extracts the file rotation time point predicate from the
    //! parameter
    template <typename CharT>
    sinks::file::rotation_at_time_point param_cast_to_rotation_time_point(
        const char* param_name, std::basic_string<CharT> const& value)
    {
        typedef CharT char_type;
        typedef boost::log::aux::char_constants<char_type> constants;
        ...
        typedef qi::extract_uint<unsigned char, 10, 2, 2>
            time_component_extract;
        ...
        if (!encoding::isalnum(
                *begin)) // begin is null-terminated, so we also check that the
                         // string is not empty here
            throw_invalid_value(param_name);
        ...
        if (encoding::isalpha(*begin))
        {
            ...
            if (is_weekday(begin, len, constants::monday_keyword(),
                           constants::short_monday_keyword()))
                weekday = date_time::Monday;
            else if (is_weekday(begin, len, constants::tuesday_keyword(),
                                constants::short_tuesday_keyword()))
                weekday = date_time::Tuesday;
            else if (is_weekday(begin, len, constants::wednesday_keyword(),
                                constants::short_wednesday_keyword()))
                weekday = date_time::Wednesday;
            else if (is_weekday(begin, len, constants::thursday_keyword(),
                                constants::short_thursday_keyword()))
                weekday = date_time::Thursday;
            else if (is_weekday(begin, len, constants::friday_keyword(),
                                constants::short_friday_keyword()))
                weekday = date_time::Friday;
            else if (is_weekday(begin, len, constants::saturday_keyword(),
                                constants::short_saturday_keyword()))
                weekday = date_time::Saturday;
            else if (is_weekday(begin, len, constants::sunday_keyword(),
                                constants::short_sunday_keyword()))
        ...
        if (weekday)
            ...
            return sinks::file::rotation_at_time_point(
                gregorian::greg_day(day.get()), hour, minute, second);
        else
    ...
    template <typename CharT>
    class basic_default_sink_factory : public sink_factory<CharT>
    {
      public:
      ...
      protected:
        ...
        static shared_ptr<sinks::sink>
            select_backend_character_type(settings_section const& params,
        ...
        //! The function initializes common parameters of a formatting sink and
        //! returns the constructed sink
        template <typename BackendT>
        static shared_ptr<sinks::sink>
            init_sink(shared_ptr<BackendT> const& backend,
                      settings_section const& params)
        {
            ...
            typedef typename sinks::has_requirement<
                ...
                sinks::formatted_records>::type is_formatting_t;
            ...
            {
                p = init_formatter(
                    boost::make_shared<sinks::synchronous_sink<backend_t>>(
                        backend),
                    params, is_formatting_t());
            }
            else
            {
                p = init_formatter(
                    boost::make_shared<sinks::asynchronous_sink<backend_t>>(
                        backend),
                    params, is_formatting_t());

                // https://svn.boost.org/trac/boost/ticket/10638
                // The user doesn't have a way to process excaptions from the
                // dedicated thread anyway, so just suppress them instead of
                // terminating the application.
                p->set_exception_handler(nop());
            }
#else
            // When multithreading is disabled we always use the unlocked sink
            // frontend
            p = init_formatter(
                boost::make_shared<sinks::unlocked_sink<backend_t>>(backend),
                params, is_formatting_t());
#endif
      ...
      private:
        //! The function initializes formatter for the sinks that support
        //! formatting
        template <typename SinkT>
        static shared_ptr<SinkT> init_formatter(shared_ptr<SinkT> const& sink,
                                                settings_section const& params,
                                                mpl::true_)
        {
            // Formatter
        ...
        : public basic_default_sink_factory<CharT>
    {
      ...
      private:
        ...
        struct impl
        {
            ...
            result_type operator()(settings_section const& params,
                                   type<BackendCharT>) const
            {
                // Construct the backend
                typedef boost::log::aux::char_constants<BackendCharT> constants;
                typedef sinks::basic_text_ostream_backend<BackendCharT>
                    backend_t;
                shared_ptr<backend_t> backend = boost::make_shared<backend_t>();
                backend->add_stream(shared_ptr<typename backend_t::stream_type>(
                    &constants::get_console_log_stream(),
                    boost::null_deleter()));

                // Auto flush
                if (optional<string_type> auto_flush_param =
                        params["AutoFlush"])
                {
                    backend->auto_flush(param_cast_to_bool(
                        "AutoFlush", auto_flush_param.get()));
                }
      ...
      public:
        //! The function constructs a sink that writes log records to the
        //! console
        ...
        : public basic_default_sink_factory<CharT>
    {
      ...
      public:
        //! The function constructs a sink that writes log records to a text
        //! file
        shared_ptr<sinks::sink> create_sink(settings_section const& params)
        {
            ...
            if (optional<string_type> file_name_param = params["FileName"])
            {
                backend->set_file_name_pattern(
                    filesystem::path(file_name_param.get()));
            }
            else
                BOOST_LOG_THROW_DESCR(missing_value,
                                      "File name is not specified");

            // File rotation size
            if (optional<string_type> rotation_size_param =
                    params["RotationSize"])
            {
                backend->set_rotation_size(param_cast_to_int<uintmax_t>(
                    "RotationSize", rotation_size_param.get()));
            }

            // File rotation interval
            if (optional<string_type> rotation_interval_param =
                    params["RotationInterval"])
            {
                backend->set_time_based_rotation(
                    sinks::file::rotation_at_time_interval(
                        posix_time::seconds(param_cast_to_int<unsigned int>(
                            "RotationInterval",
                            rotation_interval_param.get()))));
            }
            else if (optional<string_type> rotation_time_point_param =
                         params["RotationTimePoint"])
            {
                // File rotation time point
                backend->set_time_based_rotation(
                    param_cast_to_rotation_time_point(
                        "RotationTimePoint", rotation_time_point_param.get()));
            }

            // Final rotation
            if (optional<string_type> enable_final_rotation_param =
                    params["EnableFinalRotation"])
            {
                backend->enable_final_rotation(param_cast_to_bool(
                    "EnableFinalRotation", enable_final_rotation_param.get()));
            }
            ...
            {
                backend->auto_flush(
                    param_cast_to_bool("AutoFlush", auto_flush_param.get()));
            }
            ...
            if (optional<string_type> append_param = params["Append"])
            {
                if (param_cast_to_bool("Append", append_param.get()))
                    backend->set_open_mode(std::ios_base::out |
                                           std::ios_base::app);
            }
            ...
            if (optional<string_type> target_param = params["Target"])
            {
                ...
                if (optional<string_type> max_size_param = params["MaxSize"])
                    max_size = param_cast_to_int<uintmax_t>(
                        "MaxSize", max_size_param.get());

                // Min free space
                uintmax_t space = 0;
                if (optional<string_type> min_space_param =
                        params["MinFreeSpace"])
                    space = param_cast_to_int<uintmax_t>("MinFreeSpace",
                                                         min_space_param.get());

                // Max number of files
                uintmax_t max_files = (std::numeric_limits<uintmax_t>::max)();
                if (optional<string_type> max_files_param = params["MaxFiles"])
                    max_files = param_cast_to_int<uintmax_t>(
                        "MaxFiles", max_files_param.get());
                ...
                if (optional<string_type> scan_param = params["ScanForFiles"])
                {
                    ...
                    {
                        BOOST_LOG_THROW_DESCR(
                            invalid_value,
                            "File scan method \"" +
                                boost::log::aux::to_narrow(value) +
                                "\" is not supported");
                    }
                }
            }
    ...
    template <typename CharT>
    class default_syslog_sink_factory : public basic_default_sink_factory<CharT>
    {
      public:
        typedef basic_default_sink_factory<CharT> base_type;
        typedef typename base_type::char_type char_type;
      ...
      public:
        ...
        shared_ptr<sinks::sink> create_sink(settings_section const& params)
        {
            ...
            backend->set_severity_mapper(
                sinks::syslog::direct_severity_mapping<>(
                    log::aux::default_attribute_names::severity()));

#if !defined(BOOST_LOG_NO_ASIO)
            // Setup local and remote addresses
            if (optional<string_type> local_address_param =
                    params["LocalAddress"])
                backend->set_local_address(param_cast_to_address(
                    "LocalAddress", local_address_param.get()));

            if (optional<string_type> target_address_param =
                    params["TargetAddress"])
                backend->set_target_address(param_cast_to_address(
                    "TargetAddress", target_address_param.get()));
#endif // !defined(BOOST_LOG_NO_ASIO)
        ...
        : public basic_default_sink_factory<CharT>
    {
      ...
      private:
        ...
        struct impl
        {
            ...
            {
                // Construct the backend
                typedef sinks::basic_debug_output_backend<BackendCharT>
                    backend_t;
                shared_ptr<backend_t> backend = boost::make_shared<backend_t>();
      ...
      private:
        ...
        struct impl
        {
            ...
            result_type operator()(settings_section const& params,
                                   type<BackendCharT>) const
            {
                typedef sinks::basic_simple_event_log_backend<BackendCharT>
                    backend_t;
                ...
                if (optional<string_type> log_source_param =
                        params["LogSource"])
                ...
                sinks::event_log::registration_mode reg_mode =
                    sinks::event_log::on_demand;
                if (optional<string_type> registration_param =
                        params["Registration"])
                {
                ...
                // For now we use only the default event type mapping. Will add
                // support for configuration later.
                backend->set_event_type_mapper(
                    sinks::event_log::direct_event_type_mapping<>(
                        log::aux::default_attribute_names::severity()));
        ...
        : public log::aux::lazy_singleton<sinks_repository<CharT>>
    {
        ...
        shared_ptr<sinks::sink>
            construct_sink_from_settings(settings_section const& params)
        {
            typedef typename settings_section::const_reference
                param_const_reference;
            if (param_const_reference dest_node = params["Destination"])
            {
                ...
                BOOST_LOG_EXPR_IF_MT(
                    log::aux::shared_lock_guard<log::aux::light_rw_mutex> lock(
                        m_Mutex);)
                typename sink_factories::const_iterator it =
                    m_Factories.find(dest);
        ...
        static void init_instance()
        {
            ...
            instance.m_Factories["TextFile"] =
                boost::make_shared<default_text_file_sink_factory<char_type>>();
            instance.m_Factories["Console"] =
                boost::make_shared<default_console_sink_factory<char_type>>();
#ifndef BOOST_LOG_WITHOUT_SYSLOG
            instance.m_Factories["Syslog"] =
                boost::make_shared<default_syslog_sink_factory<char_type>>();
#endif
#ifndef BOOST_LOG_WITHOUT_DEBUG_OUTPUT
            instance.m_Factories["Debugger"] =
                boost::make_shared<default_debugger_sink_factory<char_type>>();
#endif
#ifndef BOOST_LOG_WITHOUT_EVENT_LOG
            instance.m_Factories["SimpleEventLog"] = boost::make_shared<
                default_simple_event_log_sink_factory<char_type>>();
#endif
        }
    ...
    void apply_core_settings(basic_settings_section<CharT> const& params)
    {
        ...
        // DisableLogging
        if (optional<string_type> disable_logging_param =
                params["DisableLogging"])
            core->set_logging_enabled(!param_cast_to_bool(
                "DisableLogging", disable_logging_param.get()));
        else
    ...
    init_from_settings(basic_settings_section<CharT> const& setts)
{
    ...
    if (section sink_params = setts["Sinks"])
    {
        ...
        for (typename section::const_iterator it = sink_params.begin(),
                                              end = sink_params.end();
             it != end; ++it)
        {
            section sink_params = *it;

            // Ignore empty sections as they are most likely individual
            // parameters (which should not be here anyway)
            if (!sink_params.empty())
            {
                new_sinks.push_back(
                    sinks_repo.construct_sink_from_settings(sink_params));
            }
        }

        std::for_each(new_sinks.begin(), new_sinks.end(),
                      boost::bind(&core::add_sink, core::get(), _1));
    }
}
                          ...
                          shared_ptr<sink_factory<CharT>> const& factory)
{
    ...
    BOOST_LOG_EXPR_IF_MT(
        lock_guard<log::aux::light_rw_mutex> lock(repo.m_Mutex);)
    repo.m_Factories[sink_name] = factory;
}

#ifdef BOOST_LOG_USE_CHAR
template BOOST_LOG_SETUP_API void
    register_sink_factory<char>(const char* sink_name,
                                shared_ptr<sink_factory<char>> const& factory);
template BOOST_LOG_SETUP_API void
    init_from_settings<char>(basic_settings_section<char> const& setts);
#endif

#ifdef BOOST_LOG_USE_WCHAR_T
template BOOST_LOG_SETUP_API void register_sink_factory<wchar_t>(
    const char* sink_name, shared_ptr<sink_factory<wchar_t>> const& factory);
template BOOST_LOG_SETUP_API void
    init_from_settings<wchar_t>(basic_settings_section<wchar_t> const& setts);
#endif
