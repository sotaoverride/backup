...
rsRetVal ATTR_NONNULL()
    multiSubmitMsg2(multi_submit_t* const pMultiSub); /* friends only! */
...
rsRetVal logmsgInternal(const int iErr, const syslog_pri_t pri,
                        const uchar* const msg, int flags);
rsRetVal __attribute__((deprecated))
parseAndSubmitMessage(const uchar* hname, const uchar* hnameIP,
                      const uchar* msg, const int len, const int flags,
                      const flowControl_t flowCtlType, prop_t* pInputName,
                      const struct syslogTime* stTime, const time_t ttGenTime,
                      ruleset_t* pRuleset);
...
#define CONF_VERIFY_PARTIAL_CONF                                               \
    0x02                  /* bit: partial configuration to be checked */
