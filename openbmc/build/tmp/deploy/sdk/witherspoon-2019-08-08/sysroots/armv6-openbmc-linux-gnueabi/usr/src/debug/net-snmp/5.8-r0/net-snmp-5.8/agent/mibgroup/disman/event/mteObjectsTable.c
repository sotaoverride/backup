...

{
    ...
    reg = netsnmp_create_handler_registration(
        "mteObjectsTable", mteObjectsTable_handler, mteObjectsTable_oid,
        mteObjectsTable_oid_len, HANDLER_CAN_RWRITE);
#else  /* !NETSNMP_NO_WRITE_SUPPORT */
    reg = netsnmp_create_handler_registration(
        "mteObjectsTable", mteObjectsTable_handler, mteObjectsTable_oid,
        mteObjectsTable_oid_len, HANDLER_CAN_RONLY);
#endif /* !NETSNMP_NO_WRITE_SUPPORT */
...
int mteObjectsTable_handler(netsnmp_mib_handler* handler,
    ...
    switch (reqinfo->mode)
    {
            ...
            for (request = requests; request; request = request->next)
            {
                ...
                switch (tinfo->colnum)
                {
                    ...
                    case COLUMN_MTEOBJECTSIDWILDCARD:
                        ret = (entry->flags & MTE_OBJECT_FLAG_WILD) ? TV_TRUE
                                                                    : TV_FALSE;
                    ...
                    case COLUMN_MTEOBJECTSENTRYSTATUS:
                        switch (*request->requestvb->val.integer)
                        {
                            ...
                            case RS_CREATEANDWAIT:
                                ...
                                ret = *tinfo->indexes->next_variable
                                           ->next_variable->val.integer;

                                row = mteObjects_createEntry(mteOwner, mteOName,
                                                             ret, 0);
                                if (!row)
                                {
                                    netsnmp_set_request_error(
        ...
        case MODE_SET_COMMIT:
            ...
            for (request = requests; request; request = request->next)
            {
                ...
                {
                    case COLUMN_MTEOBJECTSID:
                        memset(entry->mteObjectID, 0,
                               sizeof(entry->mteObjectID));
                        memcpy(entry->mteObjectID,
