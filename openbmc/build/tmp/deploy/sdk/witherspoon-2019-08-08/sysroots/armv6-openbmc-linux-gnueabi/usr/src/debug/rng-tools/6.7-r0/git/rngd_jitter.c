...
static int init_gcrypt(const void* key)
{
    ...
    if (gcry_error)
    {
        message(LOG_DAEMON | LOG_ERR, "could not set key or IV: %s\n",
...
static inline int gcrypt_mangle(unsigned char* tmp, size_t size)
{
    ...
    for (i = 0; i < (size - stride) && !gcry_error; i += stride)
    {
        gcry_error = gcry_cipher_encrypt(gcry_cipher_hd, &tmp[i],
                                         AES_BLOCK * RDRAND_ROUNDS, NULL, 0);
    }

    if (gcry_error)
    {
        message(LOG_DAEMON | LOG_ERR, "gcry_cipher_encrypt error: %s\n",
                gcry_strerror(gcry_error));
        return -1;
    }
    return 0;
...
try_again:
    while (need)
    {
        message(LOG_DAEMON | LOG_DEBUG,
                "xread_jitter requests %lu bytes from pipe\n", need);
        ...
        else if (request < need)
        {
            if (request == -1)
            {
                ...
                message(
                    ...
                    "request of random data returns %ld less than need %ld\n",
                    request, need);
            if (retry_count <
                ent_src->rng_options[JITTER_OPT_RETRY_COUNT].int_val)
            {
                retry_count++;
...
static void* thread_entropy_task(void* data)
{
    ...
    {
        message(LOG_DAEMON | LOG_DEBUG,
                "Unable to allocte temp buffer on cpu %d\n", me->core_id);
        goto out;
    }
    ...
    {
        message(LOG_DAEMON | LOG_DEBUG,
                "JITTER thread on cpu %d wakes up for refill\n", me->core_id);

        /* We are awake because we need to refil the buffer */
        clock_gettime(CLOCK_REALTIME, &start);
        ret = jent_read_entropy(me->ec, tmpbuf, me->buf_sz);
        clock_gettime(CLOCK_REALTIME, &end);
        message(LOG_DEBUG | LOG_ERR,
                "jent_read_entropy time on cpu %d is %.12e sec\n", me->core_id,
                elapsed_time(&start, &end));
        if (ret < 0)
            message(LOG_DAEMON | LOG_DEBUG,
                    "JITTER THREAD_FAILS TO GATHER ENTROPY\n");
        ...
        while (me->active && written != me->buf_sz)
        {
            ...
            message(LOG_DAEMON | LOG_DEBUG,
                    "DONE Writing to pipe with return %ld\n", ret);
            if (first)
                me->active = 1;
            if (ret < 0)
...
int validate_jitter_options(struct rng* ent_src)
{
    ...
    if (!threads)
    {
        message(
            LOG_DAEMON | LOG_ERR,
            "JITTER Requires a minimum of 1 thread, setting threads to 1\n");
    ...
    {
        message(LOG_DAEMON | LOG_ERR,
                "JITTER buffer size must be larger than refill threshold\n");
        return 1;
    }

    if (rcount < 0)
    {
        message(LOG_DAEMON | LOG_ERR, "JITTER retry delay and count must be "
    ...
    {
        message(LOG_DAEMON | LOG_ERR,
                "JITTER retry delay must be -1 or larger than 0\n");
        return 1;
    }
...
int init_jitter_entropy_source(struct rng* ent_src)
{
    ...
    if (sched_getaffinity(0, cpusize, cpus) < 0)
    {
        message(
            LOG_DAEMON | LOG_DEBUG,
            "Can not determine affinity of process, defaulting to 1 thread\n");
        CPU_SET(0, cpus);
    }
    ...
    if (ent_src->rng_options[JITTER_OPT_USE_AES].int_val)
    {
        ...
        if (xread_jitter(key, AES_BLOCK, ent_src))
        {
            message(
                LOG_CONS | LOG_INFO,
                "Unable to obtain AES key, disabling AES in JITTER source\n");
        }
        else if (xread_jitter(iv_buf, CHUNK_SIZE, ent_src))
        {
            message(
                LOG_CONS | LOG_INFO,
                "Unable to obtain iv_buffer, disabling AES in JITTER source\n");
        }
        else if (init_gcrypt(key))
        {
            message(
                LOG_CONS | LOG_INFO,
                "Unable to inity gcrypt lib, disabling AES in JITTER source\n");
        }
        else
        {
            /* re-enable AES */
