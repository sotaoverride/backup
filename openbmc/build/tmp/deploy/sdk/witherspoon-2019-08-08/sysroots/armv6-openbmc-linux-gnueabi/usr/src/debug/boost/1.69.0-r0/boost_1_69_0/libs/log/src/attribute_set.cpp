...
BOOST_LOG_API void attribute::impl::
    operator delete(void* p, std::size_t size)BOOST_NOEXCEPT
{
    aux::stateless_allocator<unsigned char>().deallocate(
        static_cast<unsigned char*>(p), size);
}

inline attribute_set::node_base::node_base() : m_pPrev(NULL), m_pNext(NULL)
{
}

inline attribute_set::node::node(key_type const& key, mapped_type const& data) :
    node_base(), m_Value(key, data)
{
}

//! Default constructor
BOOST_LOG_API attribute_set::attribute_set() : m_pImpl(new implementation())
{
}
...
BOOST_LOG_API attribute_set::const_iterator
    attribute_set::begin() const BOOST_NOEXCEPT
{
    return const_iterator(m_pImpl->begin());
}
BOOST_LOG_API attribute_set::const_iterator
    attribute_set::end() const BOOST_NOEXCEPT
{
    return const_iterator(m_pImpl->end());
}

//! The method returns number of elements in the container
BOOST_LOG_API attribute_set::size_type
    attribute_set::size() const BOOST_NOEXCEPT
{
    return m_pImpl->size();
}
...
BOOST_LOG_API attribute_set::size_type
    attribute_set::erase(key_type key) BOOST_NOEXCEPT
{
...
BOOST_LOG_API void attribute_set::erase(iterator begin,
...
BOOST_LOG_API attribute_set::iterator
    attribute_set::find(key_type key) BOOST_NOEXCEPT
{
