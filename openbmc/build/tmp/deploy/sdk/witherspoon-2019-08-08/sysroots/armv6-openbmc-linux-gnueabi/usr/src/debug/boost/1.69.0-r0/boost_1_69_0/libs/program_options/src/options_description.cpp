...
std::basic_string<charT> tolower_(const std::basic_string<charT>& str)
{
    ...
    for (typename std::basic_string<charT>::size_type i = 0; i < str.size();
         ++i)
    {
...
option_description::option_description(const char* names,
...
option_description::match_result
    option_description::match(const std::string& option, bool approx,
                              ...
                              bool short_ignore_case) const
{
    ...
    for (std::vector<std::string>::const_iterator it(m_long_names.begin());
         it != m_long_names.end(); it++)
    {
    ...
    {
        std::string local_short_name(short_ignore_case ? tolower_(m_short_name)
                                                       : m_short_name);
    ...
    option_description::long_names() const
{
    ...
    return (m_long_names.empty())
               ...
               : std::pair<const std::string*, size_t>(&(*m_long_names.begin()),
                                                       m_long_names.size());
}
...
options_description::options_description(const std::string& caption,
                                         ...
                                         unsigned min_description_length) :
    m_caption(caption),
    m_line_length(line_length), m_min_description_length(min_description_length)
{
    // we require a space between the option and description parts, so add 1.
    assert(m_min_description_length < m_line_length - 1);
}
...
const option_description&
    options_description::find(const std::string& name, bool approx,
                              bool long_ignore_case,
                              bool short_ignore_case) const
{
...
const option_description*
    options_description::find_nothrow(const std::string& name, bool approx,
                                      bool long_ignore_case,
                                      bool short_ignore_case) const
{
    ...
    {
        option_description::match_result r = m_options[i]->match(
...
*/
void format_paragraph(std::ostream& os, std::string par, unsigned indent,
                      unsigned line_length)
{
    ...
    else
    {
        ...
        while (line_begin < par_end) // paragraph lines
        {
            ...
            unsigned remaining =
                static_cast<unsigned>(std::distance(line_begin, par_end));
                ...
                unsigned first_column_width, unsigned line_length)
{
    ...
    if (!opt.description().empty())
    {
        ...
        {
            os.put('\n'); // first column is too long, lets put description in
                          // new line
            for (unsigned pad = first_column_width; pad > 0; --pad)
            {
                os.put(' ');
        ...
        format_description(os, opt.description(), first_column_width,
                           line_length);
    }
}
} // namespace
