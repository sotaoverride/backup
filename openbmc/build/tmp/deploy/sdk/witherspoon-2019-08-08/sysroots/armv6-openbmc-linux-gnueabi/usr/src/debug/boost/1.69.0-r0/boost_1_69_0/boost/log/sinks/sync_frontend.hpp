...
#define BOOST_LOG_SINK_CTOR_FORWARD_INTERNAL_N(n, data)                        \
    ...
    explicit synchronous_sink(BOOST_PP_ENUM_BINARY_PARAMS(n, T, const& arg)) : \
        base_type(false), m_pBackend(boost::make_shared<sink_backend_type>(    \
 ...
 *
 * The sink frontend serializes threads before passing logging records to the
 * backend
 */
template <typename SinkBackendT>
class synchronous_sink : public aux::make_sink_frontend_base<SinkBackendT>::type
{
  ...
  public:
    ...
    BOOST_STATIC_ASSERT_MSG(
        ...
        "Synchronous sink frontend is incompatible with the specified backend: "
        "thread synchronization requirements are not met");
    //! \endcond
  ...
  public:
    ...
    explicit synchronous_sink(shared_ptr<sink_backend_type> const& backend) :
        base_type(false), m_pBackend(backend)
    {
    }
    ...
    /*!
     * The method performs flushing of any internal buffers that may hold log
     * records. The method may take considerable time to complete and may block
     * both the calling thread and threads attempting to put new records into
     * the sink while this call is in progress.
     */
    void flush()
    {
