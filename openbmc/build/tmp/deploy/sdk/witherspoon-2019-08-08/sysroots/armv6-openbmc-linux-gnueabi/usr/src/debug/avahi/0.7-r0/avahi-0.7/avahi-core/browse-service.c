                            ...
                            void* userdata)
{
    ...
    if (record)
    {
        char service[AVAHI_LABEL_MAX], type[AVAHI_DOMAIN_NAME_MAX],
            domain[AVAHI_DOMAIN_NAME_MAX];

        assert(record->key->type == AVAHI_DNS_TYPE_PTR);

        if (event == AVAHI_BROWSER_NEW &&
            avahi_server_is_service_local(b->server, interface, protocol,
                                          record->data.ptr.name))
            flags |= AVAHI_LOOKUP_RESULT_LOCAL;

        if (avahi_service_name_split(record->data.ptr.name, service,
                                     sizeof(service), type, sizeof(type),
        ...
        b->callback(b, interface, protocol, event, service, type, domain, flags,
                    b->userdata);
    }
    else
        b->callback(b, interface, protocol, event, NULL, b->service_type,
                    b->domain_name, flags, b->userdata);
}

AvahiSServiceBrowser* avahi_s_service_browser_new(
    AvahiServer* server, AvahiIfIndex interface, AvahiProtocol protocol,
    const char* service_type, const char* domain, AvahiLookupFlags flags,
    AvahiSServiceBrowserCallback callback, void* userdata)
{
    ...
    AVAHI_CHECK_VALIDITY_RETURN_NULL(
        server, avahi_is_valid_service_type_generic(service_type),
        AVAHI_ERR_INVALID_SERVICE_TYPE);

    if (!domain)
        domain = server->domain_name;

    if ((r = avahi_service_name_join(n, sizeof(n), NULL, service_type,
                                     domain)) < 0)
    {
    ...
    AVAHI_LLIST_PREPEND(AvahiSServiceBrowser, browser, server->service_browsers,
                        b);
...
void avahi_s_service_browser_free(AvahiSServiceBrowser* b)
{
    ...
    AVAHI_LLIST_REMOVE(AvahiSServiceBrowser, browser,
                       b->server->service_browsers, b);
