                           ...
                           int dx, int dy)
{
    ...
    while ((cl = rfbClientIteratorNext(iterator)))
    {
        ...
        if (cl->useCopyRect)
        {
             ...
             * mark them as modified, because the source of these can be
             * overlapped either by new modified or now copied regions. */
            ...
            if (!cl->enableCursorShapeUpdates)
            {
                ...
                {
                    /*
                     * current cursor rect overlaps with the copy region
                     * *source*, mark the *displaced* cursorRegion as modified
                     * since we won't copyrect stuff to it.
                     */
                    sraRgnOr(cl->modifiedRegion, cursorRegion);
                }
                sraRgnDestroy(cursorRegion);
...
void rfbDoCopyRegion(rfbScreenInfoPtr screen, sraRegionPtr copyRegion, int dx,
                     int dy)
{
    ...
    while (sraRgnIteratorNext(i, &rect))
    {
        ...
        in = screen->frameBuffer + (rect.x1 - dx) * bpp +
             (rect.y1 - dy) * rowstride;
        if (dy < 0)
            for (j = rect.y1; j < rect.y2;
                 j++, out += rowstride, in += rowstride)
                memmove(out, in, widthInBytes);
        else
        {
            out += rowstride * (rect.y2 - rect.y1 - 1);
            in += rowstride * (rect.y2 - rect.y1 - 1);
            for (j = rect.y2 - 1; j >= rect.y1;
                 j--, out -= rowstride, in -= rowstride)
                memmove(out, in, widthInBytes);
        }
    }
...
void rfbDoCopyRect(rfbScreenInfoPtr screen, int x1, int y1, int x2, int y2,
                   int dx, int dy)
{
...
void rfbScheduleCopyRect(rfbScreenInfoPtr screen, int x1, int y1, int x2,
...
void rfbScaledScreenUpdate(rfbScreenInfoPtr screen, int x1, int y1, int x2,
                           int y2);
...
static void* clientInput(void* data)
{
    ...
    while (1)
    {
        ...
        int nfds = cl->pipe_notify_client_thread[0] > cl->sock
                       ? cl->pipe_notify_client_thread[0]
                       : cl->sock;
        ...
        if (FD_ISSET(cl->pipe_notify_client_thread[0], &rfds))
        {
            /* Reset the pipe */
            char buf;
            while (read(cl->pipe_notify_client_thread[0], &buf, sizeof(buf)) ==
                   sizeof(buf))
                ;
        }
...
static void* listenerRun(void* data)
{
    ...
    while (1)
    {
        ...
        if (FD_ISSET(screen->listenSock, &listen_fds))
            client_fd =
                accept(screen->listenSock, (struct sockaddr*)&peer, &len);
        else if (FD_ISSET(screen->listen6Sock, &listen_fds))
            client_fd =
                accept(screen->listen6Sock, (struct sockaddr*)&peer, &len);
...
static void rfbDefaultKbdAddEvent(rfbBool down, rfbKeySym keySym,
                                  rfbClientPtr cl)
{
}
...
static rfbCursor myCursor = {FALSE,
                             FALSE,
                             FALSE,
                             FALSE,
...
static rfbBool rfbDefaultPasswordCheck(rfbClientPtr cl, const char* response,
                                       int len)
{
    int i;
                              ...
                              int bytesPerPixel)
{
    ...
    if (width & 3)
        rfbErr("WARNING: Width (%d) is not a multiple of 4. VncViewer has "
               "problems with that.\n",
               width);
...
void rfbNewFramebuffer(rfbScreenInfoPtr screen, char* framebuffer, int width,
                       int height, int bitsPerSample, int samplesPerPixel,
                       int bytesPerPixel)
{
    ...
    if (memcmp(&screen->serverFormat, &old_format, sizeof(rfbPixelFormat)) != 0)
    {
        format_changed = TRUE;
    }
...
#ifdef LIBVNCSERVER_HAVE_LIBPTHREAD
            /*
          Notify the thread. This simply writes a NULL byte to the notify pipe
          in order to get past the select() in clientInput(), the loop in there
          will then break because the rfbCloseClient() above has set
          currentCl->sock to -1.
            */
    ...
    if (!cl->viewOnly && cl->lastPtrX >= 0)
    {
        ...
        else
        {
                       ...
                       cl->screen->deferPtrUpdateTime)
            {
                ...
                cl->screen->ptrAddEvent(cl->lastPtrButtons, cl->lastPtrX,
                                        cl->lastPtrY, cl);
                cl->lastPtrX = -1;
            }
        }
...
{
    return screenInfo->socketState != RFB_SOCKET_SHUTDOWN ||
           screenInfo->clientHead != NULL;
}

void rfbRunEventLoop(rfbScreenInfoPtr screen, long usec,
