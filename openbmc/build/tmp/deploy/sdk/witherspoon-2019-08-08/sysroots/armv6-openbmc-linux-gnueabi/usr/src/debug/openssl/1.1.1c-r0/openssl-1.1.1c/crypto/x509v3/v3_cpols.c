...
static STACK_OF(POLICYINFO) *
    r2i_certpol(X509V3_EXT_METHOD* method, X509V3_CTX* ctx, const char* value);
...
const X509V3_EXT_METHOD v3_cpols = {NID_certificate_policies,
                                    0,
...
ASN1_ITEM_TEMPLATE(CERTIFICATEPOLICIES) = ASN1_EX_TEMPLATE_TYPE(
    ASN1_TFLG_SEQUENCE_OF, 0, CERTIFICATEPOLICIES,
    POLICYINFO) ASN1_ITEM_TEMPLATE_END(CERTIFICATEPOLICIES)

    IMPLEMENT_ASN1_FUNCTIONS(CERTIFICATEPOLICIES)

        ASN1_SEQUENCE(POLICYINFO) =
            {ASN1_SIMPLE(POLICYINFO, policyid, ASN1_OBJECT),
             ASN1_SEQUENCE_OF_OPT(POLICYINFO, qualifiers,
                                  POLICYQUALINFO)} ASN1_SEQUENCE_END(POLICYINFO)
...
ASN1_ADB(POLICYQUALINFO) =
    {ADB_ENTRY(NID_id_qt_cps,
     ...
     ADB_ENTRY(NID_id_qt_unotice,
               ASN1_SIMPLE(POLICYQUALINFO, d.usernotice,
                           USERNOTICE))} ASN1_ADB_END(POLICYQUALINFO, 0,
                                                      pqualid, 0,
                                                      &policydefault_tt, NULL);
        ...
        IMPLEMENT_ASN1_FUNCTIONS(POLICYQUALINFO)
            ...
            ASN1_SEQUENCE(USERNOTICE) =
                {ASN1_OPT(USERNOTICE, noticeref, NOTICEREF),
                 ASN1_OPT(USERNOTICE, exptext,
                          DISPLAYTEXT)} ASN1_SEQUENCE_END(USERNOTICE)
                    ...
                    IMPLEMENT_ASN1_FUNCTIONS(USERNOTICE)
                        ...
                        ASN1_SEQUENCE(NOTICEREF) =
                            {ASN1_SIMPLE(NOTICEREF, organization, DISPLAYTEXT),
                             ASN1_SEQUENCE_OF(
                                 NOTICEREF, noticenos,
                                 ASN1_INTEGER)} ASN1_SEQUENCE_END(NOTICEREF)
...
static int displaytext_str2tag(const char* tagstr, unsigned int* tag_len)
{
    ...
    *tag_len = len;
    ...
    if (len == sizeof("UTF8String") - 1 &&
        strncmp(tagstr, "UTF8String", len) == 0)
        return V_ASN1_UTF8STRING;
    if (len == sizeof("BMP") - 1 && strncmp(tagstr, "BMP", len) == 0)
        return V_ASN1_BMPSTRING;
    if (len == sizeof("BMPSTRING") - 1 &&
        strncmp(tagstr, "BMPSTRING", len) == 0)
        return V_ASN1_BMPSTRING;
    if (len == sizeof("VISIBLE") - 1 && strncmp(tagstr, "VISIBLE", len) == 0)
        return V_ASN1_VISIBLESTRING;
    if (len == sizeof("VISIBLESTRING") - 1 &&
        strncmp(tagstr, "VISIBLESTRING", len) == 0)
        return V_ASN1_VISIBLESTRING;
    *tag_len = 0;
    return V_ASN1_VISIBLESTRING;
}
