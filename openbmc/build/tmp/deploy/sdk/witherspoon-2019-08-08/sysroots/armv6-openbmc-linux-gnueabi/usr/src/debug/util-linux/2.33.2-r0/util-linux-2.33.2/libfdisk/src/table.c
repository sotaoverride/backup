...
int fdisk_reset_table(struct fdisk_table* tb)
{
    ...
    while (!list_empty(&tb->parts))
    {
        struct fdisk_partition* pa =
            list_entry(tb->parts.next, struct fdisk_partition, parts);
 ...
 * @pa: new entry
 *
 * Adds a new entry to table and increment @pa reference counter. Don't forget
 * to use fdisk_unref_partition() after fdisk_table_add_partition() if you want
 * to keep the @pa referenced by the table only.
 *
 * Returns: 0 on success or negative number in case of error.
 */
                              ...
                              struct fdisk_partition* pa)
{
    ...
    DBG(TAB, ul_debugobj(
                 tb, "add entry %p [start=%ju, end=%ju, size=%ju, %s %s %s]",
                 pa, (uintmax_t)fdisk_partition_get_start(pa),
...
static int table_insert_partition(struct fdisk_table* tb,
                                  ...
                                  struct fdisk_partition* pa)
{
    ...
    DBG(TAB,
        ul_debugobj(
            ...
            pa, poz ? poz : NULL, (uintmax_t)fdisk_partition_get_start(pa),
            (uintmax_t)fdisk_partition_get_end(pa),
            (uintmax_t)fdisk_partition_get_size(pa),
            fdisk_partition_is_freespace(pa) ? "freespace" : "",
            fdisk_partition_is_nested(pa) ? "nested" : "",
 ...
 * to use fdisk_ref_partition() before call fdisk_table_remove_partition() if
 * you want to use @pa later.
 *
 * Returns: 0 on success or negative number in case of error.
 */
...
void fdisk_debug_print_table(struct fdisk_table* tb)
{
    ...
    while (fdisk_table_next_partition(tb, &itr, &pa) == 0)
        ul_debugobj(
            ...
            pa, pa->partno, (uintmax_t)fdisk_partition_get_start(pa),
            (uintmax_t)fdisk_partition_get_end(pa),
            (uintmax_t)fdisk_partition_get_size(pa),
            fdisk_partition_is_nested(pa) ? " nested" : "",
            fdisk_partition_is_freespace(pa) ? " freespace" : "",
...
typedef int (*fdisk_partcmp_t)(struct fdisk_partition*,
                               struct fdisk_partition*);

static int cmp_parts_wrapper(struct list_head* a, struct list_head* b,
                             void* data)
{
...
static int new_freespace(struct fdisk_context* cxt, fdisk_sector_t start,
                         fdisk_sector_t end, struct fdisk_partition* parent,
                         struct fdisk_partition** pa)
{
...
static int table_add_freespace(struct fdisk_context* cxt,
                               struct fdisk_table* tb, fdisk_sector_t start,
                               fdisk_sector_t end,
                                     ...
                                     struct fdisk_partition* cont)
{
    ...
    while (fdisk_table_next_partition(parts, &itr, &pa) == 0)
    {
        ...
        DBG(CXT, ul_debugobj(cxt, "partno=%zu, start=%ju", pa->partno,
                             (uintmax_t)pa->start));
        ...
        DBG(CXT,
            ul_debugobj(
                ...
                pa->partno, (uintmax_t)fdisk_partition_get_start(pa),
                (uintmax_t)fdisk_partition_get_end(pa)));
    ...
    {
        DBG(TAB,
            ul_debugobj(tb, "add remaining space in container 0x%p", cont));
...
int fdisk_get_freespaces(struct fdisk_context* cxt, struct fdisk_table** tb)
{
    ...
    while (rc == 0 && fdisk_table_next_partition(parts, &itr, &pa) == 0)
    {
        ...
        if (!pa->used || pa->wholedisk || fdisk_partition_is_nested(pa) ||
            ...
            continue;
        DBG(CXT, ul_debugobj(
                     cxt, "freespace analyze: partno=%zu, start=%ju, end=%ju",
                     pa->partno, (uintmax_t)fdisk_partition_get_start(pa),
                     (uintmax_t)fdisk_partition_get_end(pa)));
        ...
        if (last + grain < pa->start || (last < pa->start && nparts == 0))
        {
            rc = table_add_freespace(cxt, *tb, last + (nparts == 0 ? 0 : 1),
                                     pa->start - 1, NULL);
        }
    ...
    if (rc == 0 && last + grain < cxt->total_sectors - 1)
    {
        ...
        rc =
            new_freespace(cxt, last + (last > cxt->first_lba || nparts ? 1 : 0),
...
int fdisk_diff_tables(struct fdisk_table* a, struct fdisk_table* b,
                      struct fdisk_iter* itr, struct fdisk_partition** res,
                      int* change)
{
