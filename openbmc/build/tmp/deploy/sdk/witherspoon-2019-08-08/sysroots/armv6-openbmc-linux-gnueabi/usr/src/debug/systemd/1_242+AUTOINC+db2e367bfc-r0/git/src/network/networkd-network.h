...
typedef enum DHCPClientIdentifier
{
    DHCP_CLIENT_ID_MAC,
    DHCP_CLIENT_ID_DUID,
    /* The following option may not be good for RFC regarding DHCP (3315 and
     * 4361). But some setups require this. E.g., Sky Broadband, the second
     * largest provider in the UK requires the client id to be set to a custom
     * string, reported at https://github.com/systemd/systemd/issues/7828 */
...
{
    /* The values map to the kernel's /proc/sys/net/ipv6/conf/xxx/use_tempaddr
       values */
...
struct Network
{
    ...
    bool required_for_online; /* Is this network required to be considered
                                 online? */
...
int network_get(Manager* manager, sd_device* device, const char* ifname,
                const struct ether_addr* mac, Network** ret);
...
const struct ConfigPerfItem*
    network_network_gperf_lookup(const char* key, GPERF_LEN_TYPE length);
