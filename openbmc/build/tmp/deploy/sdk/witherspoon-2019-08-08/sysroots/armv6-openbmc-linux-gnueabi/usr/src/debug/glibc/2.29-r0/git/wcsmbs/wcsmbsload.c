...
attribute_hidden struct __gconv_step*
...
__libc_rwlock_define(extern, __libc_setlocale_lock attribute_hidden)
    ...
    void __wcsmbs_load_conv(struct __locale_data* new_category)
{
    /* Acquire the lock.  */
    ...
    if (__glibc_likely(new_category->private.ctype == NULL))
    {
       ...
       complete lookup.  */
        complete_name =
            norm_add_slashes(charset_name, use_translit ? "TRANSLIT" : "");
       ...
       represent all others.  */
        new_fcts->towc =
            __wcsmbs_getfct("INTERNAL", complete_name, &new_fcts->towc_nsteps);
