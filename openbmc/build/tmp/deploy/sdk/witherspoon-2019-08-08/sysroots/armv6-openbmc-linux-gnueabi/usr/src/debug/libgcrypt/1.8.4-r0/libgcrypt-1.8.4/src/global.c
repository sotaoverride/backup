...
static void print_config(const char* what, gpgrt_stream_t fp)
{
    ...
    if (!what || !strcmp(what, "version"))
    {
        gpgrt_fprintf(fp, "version:%s:%x:%s:%x:\n", VERSION,
                      GCRYPT_VERSION_NUMBER, GPGRT_VERSION,
...
gcry_err_code_t _gcry_vcontrol(enum gcry_ctl_cmds cmd, va_list arg_ptr)
{
    ...
    switch (cmd)
    {
        ...
        case GCRYCTL_USE_SECURE_RNDPOOL:
            global_init();
            _gcry_secure_random_alloc(); /* Put random number into secure
                                            memory. */
            break;
...
void _gcry_set_allocation_handler(
    ...
    gcry_handler_realloc_t new_realloc_func, gcry_handler_free_t new_free_func)
{
    global_init();
...
static gcry_err_code_t do_malloc(size_t n, unsigned int flags, void** mem)
{
 ...
 * returned.  */
char* _gcry_strdup(const char* string)
{
