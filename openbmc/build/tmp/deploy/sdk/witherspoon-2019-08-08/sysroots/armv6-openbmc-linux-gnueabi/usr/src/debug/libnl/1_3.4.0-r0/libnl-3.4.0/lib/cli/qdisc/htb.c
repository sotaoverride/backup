...
static void print_qdisc_usage(void)
{
    printf(
        ...
        "\n"
        "EXAMPLE"
        "    # Create htb root qdisc 1: and direct unclassified traffic to "
        "class 1:10\n"
        "    nl-qdisc-add --dev=eth1 --parent=root --handle=1: htb "
        "--default=10\n");
}
...
static void htb_parse_qdisc_argv(struct rtnl_tc* tc, int argc, char** argv)
{
    ...
    for (;;)
    {
        ...
        static struct option long_opts[] = {{"help", 0, 0, 'h'},
                                            {"r2q", 1, 0, ARG_R2Q},
...
static void print_class_usage(void)
{
    printf(
        ...
        "     --ceil=RATE           Rate limit while borrowing (default: equal "
        "to --rate).\n"
        "     --prio=PRIO           Priority, lower is served first (default: "
        "0).\n"
        "     --quantum=SIZE        Amount of bytes to serve at once (default: "
        "rate/r2q).\n"
        "     --burst=SIZE          Max charge size of rate burst buffer "
        "(default: auto).\n"
        "     --cburst=SIZE         Max charge size of ceil rate burst buffer "
        "(default: auto)\n"
        "\n"
        "EXAMPLE"
        "    # Attach class 1:1 to htb qdisc 1: and rate limit it to 20mbit\n"
        "    nl-class-add --dev=eth1 --parent=1: --classid=1:1 htb "
        "--rate=20mbit\n");
}
