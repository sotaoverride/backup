...
#ifdef G_OS_UNIX
static void g_socket_input_stream_file_descriptor_based_iface_init(
...
G_DEFINE_TYPE_WITH_CODE(
    GSocketInputStream, g_socket_input_stream, G_TYPE_INPUT_STREAM,
...
static void g_socket_input_stream_get_property(GObject* object, guint prop_id,
...
static void g_socket_input_stream_set_property(GObject* object, guint prop_id,
                                               const GValue* value,
                                               GParamSpec* pspec)
{
    GSocketInputStream* stream = G_SOCKET_INPUT_STREAM(object);
...
static void g_socket_input_stream_finalize(GObject* object)
{
    GSocketInputStream* stream = G_SOCKET_INPUT_STREAM(object);
...
static gssize g_socket_input_stream_read(GInputStream* stream, void* buffer,
                                         gsize count, GCancellable* cancellable,
                                         GError** error)
{
    GSocketInputStream* input_stream = G_SOCKET_INPUT_STREAM(stream);

    return g_socket_receive_with_blocking(input_stream->priv->socket, buffer,
                                                 ...
                                                 GCancellable* cancellable)
{
    ...
    socket_source = g_socket_create_source(input_stream->priv->socket, G_IO_IN,
                                           cancellable);
...
static gssize g_socket_input_stream_pollable_read_nonblocking(
    GPollableInputStream* pollable, void* buffer, gsize size, GError** error)
{
    GSocketInputStream* input_stream = G_SOCKET_INPUT_STREAM(pollable);
...
#ifdef G_OS_UNIX
static int g_socket_input_stream_get_fd(GFileDescriptorBased* fd_based)
{
...
static void g_socket_input_stream_class_init(GSocketInputStreamClass* klass)
{
...
static void g_socket_input_stream_init(GSocketInputStream* stream)
{
