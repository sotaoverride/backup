                                    ...
                                    dbus_int32_t* slot_id_p)
{
    ...
    _dbus_verbose(
        ...
        *slot_id_p, allocator, allocator->n_allocated_slots,
...
dbus_bool_t _dbus_data_slot_list_set(DBusDataSlotAllocator* allocator,
                                     DBusDataSlotList* list, int slot,
                                     void* data,
...
#define N_SLOTS 100
    ...
    while (i < N_SLOTS)
    {
        if (!_dbus_data_slot_list_set(
                &allocator, &list, i, _DBUS_INT_TO_POINTER(i),
                test_free_slot_data_func, &old_free_func, &old_data))
