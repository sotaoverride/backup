...
void usage(void)
{
    fprintf(stderr, "Usage: snmpdelta [-Cf] [-CF commandFile] [-Cl] [-CL "
                    "SumFileName]\n\t[-Cs] [-Ck] [-Ct] [-CS] [-Cv vars/pkt] "
                    "[-Cp period]\n\t[-CP peaks] ");
...
void processFileArgs(char* fileName)
{
    ...
    while (fgets(buf, sizeof(buf), fp))
    {
        ...
        {
            fprintf(stderr,
                    "Too many variables read at line %d of %s (max %d)\n",
...
int main(int argc, char* argv[])
{
    ...
    while (1)
    {
        ...
        begin = count;
        for (;
             count < current_name && count < begin + varbindsPerPacket - deltat;
             count++)
        {
            ...
            if (response->errstat == SNMP_ERR_NOERROR)
            {
                if (timestamp)
                {
                    ...
                    if (((period % 60) &&
                        ...
                        sprintf(timestring, " [%02d:%02d %d/%d]", tm.tm_hour,
                                tm.tm_min, tm.tm_mon + 1, tm.tm_mday);
                }
                ...
                for (count = begin; count < end; count++)
                {
                    ...
                    if (vip->oidlen)
                    {
                        ...
                        {
                            u64Subtract(vars->val.counter64, &vip->c64value,
                                        &c64value);
                    ...
                    if (deltat || tableForm)
                    {
                        ...
                        {
                            fprintf(stderr, "time delta and table form not "
                                            "supported for counter64s\n");
                            goto close_session;
                        }
                        else
                        {
