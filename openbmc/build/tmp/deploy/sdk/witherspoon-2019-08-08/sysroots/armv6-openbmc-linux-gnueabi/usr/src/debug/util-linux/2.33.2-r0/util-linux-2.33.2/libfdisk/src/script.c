 ...
 * SECTION: script
 * @title: Script
 * @short_description: text based sfdisk compatible description of partition
 * table
 *
 ...
 *
 * Returns: new script instance or NULL in case of error (check errno for more
 * details).
 */
...
static void fdisk_reset_script(struct fdisk_script* dp)
{
    ...
    while (!list_empty(&dp->headers))
    {
        struct fdisk_scriptheader* fi =
            list_entry(dp->headers.next, struct fdisk_scriptheader, headers);
                                                    ...
                                                    const char* name)
{
    ...
    {
        struct fdisk_scriptheader* fi =
            list_entry(p, struct fdisk_scriptheader, headers);
                            ...
                            const char* data)
{
    ...
    else
    {
        ...
        DBG(SCRIPT, ul_debugobj(dp, "update '%s' header '%s' -> '%s'", name,
                                fi->data, data));
...
static struct fdisk_label* script_get_label(struct fdisk_script* dp)
{
    ...
    {
        dp->label =
            fdisk_get_label(dp->cxt, fdisk_script_get_header(dp, "label"));
        DBG(SCRIPT,
            ul_debugobj(dp, "label '%s'", dp->label ? dp->label->name : ""));
    }
    return dp->label;
}
 ...
 * If the context is no specified than defaults to context used for
 * fdisk_new_script().
 *
...
static int write_file_json(struct fdisk_script* dp, FILE* f)
{
    ...
    {
        struct fdisk_scriptheader* fi =
            list_entry(h, struct fdisk_scriptheader, headers);
        ...
        if (!dp->table &&
            fi == list_last_entry(&dp->headers, struct fdisk_scriptheader,
                                  headers))
            fputc('\n', f);
        else
            fputs(",\n", f);
    ...
    DBG(SCRIPT,
        ul_debugobj(dp, "%zu entries", fdisk_table_get_nents(dp->table)));
    ...
    while (fdisk_table_next_partition(dp->table, &itr, &pa) == 0)
    {
        ...
        {
            fput_var_separator(&nvars, f);
            fputs("\"name\":", f), fputs_quoted_json(pa->name, f);
        }
...
static int parse_line_header(struct fdisk_script* dp, char* s)
{
    ...
    *value = '\0';
    ...
    else if (strcmp(name, "label-id") == 0 || strcmp(name, "device") == 0 ||
             strcmp(name, "grain") == 0 || strcmp(name, "first-lba") == 0 ||
             strcmp(name, "last-lba") == 0 || strcmp(name, "table-length") == 0)
    {
...
{
    char *tk_begin = NULL, *tk_end = NULL, *end = NULL, *p;
...
static int parse_line_nameval(struct fdisk_script* dp, char* s)
{
    ...
    while (rc == 0 && p && *p)
    {
        ...
        else if (!strncasecmp(p, "type=", 5) || !strncasecmp(p, "Id=", 3))
        { /* backward compatibility */
            ...
            if (rc)
                break;
            pa->type = fdisk_label_parse_parttype(script_get_label(dp), type);
            free(type);
        ...
        {
            DBG(SCRIPT,
                ul_debugobj(dp, "script parse error: unknown field '%s'", p));
            rc = -EINVAL;
            break;
        }
    }
 ...
 */
static struct fdisk_parttype* translate_type_shortcuts(struct fdisk_script* dp,
                                                       char* str)
{
    struct fdisk_label* lb;
        ...
        switch (item)
        {
            ...
            case ITEM_SIZE:
                if (*p == ',' || *p == ';' || alone_sign(sign, p))
                {
                    fdisk_partition_end_follow_default(pa, 1);
                    if (sign == TK_PLUS)
                        /* '+' alone means use all possible space, '-' alone
                         * means nothing */
            ...
            case ITEM_TYPE:
                ...
                if (!pa->type)
                    pa->type =
                        fdisk_label_parse_parttype(script_get_label(dp), str);
                free(str);
...
static int fdisk_script_read_buffer(struct fdisk_script* dp, char* s)
{
    ...
    if (rc)
        DBG(SCRIPT,
            ul_debugobj(dp, "%zu: parse error [rc=%d]", dp->nlines, rc));
    return rc;
}
                           ...
                           size_t bufsz)
{
    ...
    do
    {
        ...
        if (!s)
        {
            ...
            {
                DBG(SCRIPT, ul_debugobj(dp, "%zu: missing newline at line",
                                        dp->nlines));
                return -EINVAL;
            }
        }
...
int fdisk_set_script(struct fdisk_context* cxt, struct fdisk_script* dp)
{
    ...
    {
        DBG(CXT,
            ul_debugobj(cxt, "setting reference to script %p", cxt->script));
 ...
 * The script may be later unreference by fdisk_set_script() with NULL as
 * script.
 *
 * Returns: 0 on success, <0 on error.
 */
