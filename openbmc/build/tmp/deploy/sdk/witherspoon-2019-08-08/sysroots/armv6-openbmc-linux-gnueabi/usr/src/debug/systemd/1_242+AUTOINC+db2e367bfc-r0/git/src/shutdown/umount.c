...
int mount_points_list_get(const char* mountinfo, MountPoint** head)
{
    ...
    for (;;)
    {
        ...
        if (r == 1)
            break;
        if (r < 0)
            return log_error_errno(r, "Failed to get next entry from %s: %m",
                                   mountinfo);
        ...
        if (mount_point_is_api(path) || mount_point_ignore(path) ||
        ...
        if (try_remount_ro)
        {
            /* mount(2) states that mount flags and options need to be exactly
             * the same as they were when the filesystem was mounted, except for
             * the desired changes. So we reconstruct both here and adjust them
             * for the later remount call too. */

            r = mnt_fs_get_propagation(fs, &remount_flags);
            if (r < 0)
            {
                log_warning_errno(
                    r, "mnt_fs_get_propagation() failed for %s, ignoring: %m",
                    path);
                continue;
            }

            r = mount_option_mangle(options, remount_flags, &remount_flags,
                                    &remount_options);
            if (r < 0)
            {
                log_warning_errno(
                    r, "mount_option_mangle failed for %s, ignoring: %m", path);
                continue;
            }

            /* MS_BIND is special. If it is provided it will only make the
             * mount-point read-only. If left out, the super block itself is
             * remounted, which we want. */
...
static int loopback_list_get(MountPoint** head)
{
    ...
    r = sd_device_enumerator_add_match_sysattr(e, "loop/backing_file", NULL,
                                               true);
    if (r < 0)
        return r;
...
static int remount_with_timeout(MountPoint* m, int umount_log_level)
{
     ...
     * failed. */
    r = safe_fork("(sd-remount)",
                  FORK_RESET_SIGNALS | FORK_CLOSE_ALL_FDS | FORK_LOG |
                      FORK_REOPEN_LOG,
                  &pid);
    if (r < 0)
        return r;
    if (r == 0)
    {
        log_info("Remounting '%s' read-only in with options '%s'.", m->path,
                 m->remount_options);

        /* Start the mount operation here in the child */
        r = mount(NULL, m->path, NULL, m->remount_flags, m->remount_options);
        if (r < 0)
            log_full_errno(umount_log_level, errno,
                           "Failed to remount '%s' read-only: %m", m->path);
    ...
    {
        log_error_errno(
            r, "Remounting '%s' timed out, issuing SIGKILL to PID " PID_FMT ".",
            m->path, pid);
        ...
        log_error_errno(r,
                        "Remounting '%s' failed unexpectedly, couldn't wait "
                        "for child process " PID_FMT ": %m",
                        m->path, pid);
...
static int umount_with_timeout(MountPoint* m, int umount_log_level)
{
    ...
    if (r == 0)
    {
        ...
        if (r < 0)
            log_full_errno(umount_log_level, errno, "Failed to unmount %s: %m",
                           m->path);
    ...
    {
        log_error_errno(
            r, "Unmounting '%s' timed out, issuing SIGKILL to PID " PID_FMT ".",
            m->path, pid);
        (void)kill(pid, SIGKILL);
    }
    else if (r == -EPROTO)
        ...
        log_error_errno(r,
                        "Unmounting '%s' failed unexpectedly, couldn't wait "
                        "for child process " PID_FMT ": %m",
                        m->path, pid);
 ...
 * Therefore the list passed to this function is invalidated, and should not be
 * reused. */
...
static int swap_points_list_off(MountPoint** head, bool* changed)
{
    ...
    LIST_FOREACH_SAFE(mount_point, m, n, *head)
    {
        ...
        {
            log_warning_errno(errno, "Could not deactivate swap %s: %m",
                              m->path);
            n_failed++;
        }
    }
                                 ...
                                 int umount_log_level)
{
    ...
    LIST_FOREACH_SAFE(mount_point, m, n, *head)
    {
        ...
        {
            log_full_errno(umount_log_level, errno,
                           "Could not detach DM %s: %m", m->path);
            n_failed++;
        }
    }
...
int loopback_detach_all(bool* changed, int umount_log_level)
{
    ...
    return loopback_points_list_detach(&loopback_list_head, changed,
                                       umount_log_level);
}
