 ...
 */
/*@observer@*/ /*@unchecked@*/
struct poptOption poptAliasOptions[] = {POPT_TABLEEND};
 ...
 * @return
 */
static /*@only@*/ /*@null@*/ char* singleOptionDefaultValue(
    size_t lineLength, const struct poptOption* opt,
    /*@-paramuse@*/ /* FIX: i18n macros disabled with lclint */
    /*@null@*/ const char* translation_domain)
/*@=paramuse@*/
/*@*/
...
#define prtlong (opt->longName != NULL) /* XXX splint needs a clue */
    ...
    else if (prtlong)
    {
        ...
        if (F_ISSET(opt, TOGGLE))
        {
            ...
            toggle = "";
        (void)stpcpy(stpcpy(stpcpy(stpcpy(left, "    "), dash), toggle),
                     longName);
    }
#undef prtlong
    ...
    if (argDescrip)
    {
        ...
        if (F_ISSET(opt, SHOW_DEFAULT))
        {
            ...
            if (defs)
            {
                char* t = malloc((help ? strlen(help) : 0) + strlen(defs) +
                                 sizeof(" "));
                if (t)
                {
                    char* te = t;
    ...
    if (opt != NULL)
        while (opt->longName || opt->shortName || opt->arg)
        {
            ...
            else if (!F_ISSET(opt, DOC_HIDDEN))
            {
                ...
                {
                    len +=
                        (F_ISSET(opt, ONEDASH) ? sizeof("-") : sizeof("--")) -
                        1;
...
static void itemHelp(FILE* fp,
                     /*@null@*/ poptItem items, int nitems, columns_t columns,
                     /*@null@*/ const char* translation_domain)
/*@globals fileSystem @*/
/*@modifies fp, fileSystem @*/
{
...
/*@modifies fp, columns->cur, fileSystem @*/
{
    ...
    if (table != NULL)
        for (opt = table; opt->longName || opt->shortName || opt->arg; opt++)
        {
            ...
            if (opt->arg == poptAliasOptions &&
                !(con->numAliases || con->numExecs))
                continue;
            if (opt->descrip)
...
#define prtlong (opt->longName != NULL) /* XXX splint needs a clue */
    ...
    if (prtlong)
        fprintf(fp, "%s%s%s", (prtshort ? "|" : ""),
                (F_ISSET(opt, ONEDASH) ? "-" : "--"), opt->longName);
#undef prtlong
...
/*@modifies fp, columns->cur, fileSystem @*/
{
    ...
    if (item != NULL)
        for (i = 0; i < nitems; i++, item++)
        {
            ...
            {
                columns->cur =
                    singleOptionUsage(fp, columns, opt, translation_domain);
            }
        }
...
/*@requires maxRead(str) >= 0 @*/
{
    ...
    if (opt != NULL)
        for (; (opt->longName || opt->shortName || opt->arg); opt++)
        {
            if (!F_ISSET(opt, DOC_HIDDEN) && opt->shortName &&
                !poptArgType(opt))
            {
                /* Display shortName iff unique printable non-space. */
                if (!strchr(s, opt->shortName) &&
                    isprint((int)opt->shortName) && opt->shortName != ' ')
...
void poptPrintUsage(poptContext con, FILE* fp, /*@unused@*/ UNUSED(int flags))
{
    ...
    if (columns)
    {
        ...
        columns->cur =
            singleTableUsage(con, fp, columns, con->options, NULL, done);
        columns->cur =
            itemUsage(fp, columns, con->aliases, con->numAliases, NULL);
        columns->cur = itemUsage(fp, columns, con->execs, con->numExecs, NULL);

        if (con->otherHelp)
        {
            columns->cur += strlen(con->otherHelp) + 1;
            if (columns->cur > columns->max)
                fprintf(fp, "\n       ");
