...
void _gcry_salsa20_amd64_keysetup(u32* ctxinput, const void* key,
                                  int keybits) ASM_FUNC_ABI;
void _gcry_salsa20_amd64_ivsetup(u32* ctxinput, const void* iv) ASM_FUNC_ABI;
unsigned int _gcry_salsa20_amd64_encrypt_blocks(u32* ctxinput, const void* src,
                                                void* dst, size_t len,
                                                int rounds) ASM_FUNC_ABI;

static void salsa20_keysetup(SALSA20_context_t* ctx, const byte* key,
                             int keylen)
{
...
static unsigned int salsa20_core(u32* dst, SALSA20_context_t* ctx,
                                 unsigned int rounds)
{
                                 ...
                                 unsigned rounds)
{
    ...
    /* burn_stack */
    return (3 * sizeof(void*) + 2 * sizeof(void*) + 64 + sizeof(unsigned int) +
            sizeof(u32));
}
#undef QROUND
...
unsigned int _gcry_arm_neon_salsa20_encrypt(void* c, const void* m,
                                            unsigned int nblks, void* k,
                                            unsigned int rounds);

static unsigned int salsa20_core_neon(u32* dst, SALSA20_context_t* ctx,
                                      unsigned int rounds)
{
...
static void salsa20_keysetup_neon(SALSA20_context_t* ctx, const byte* key,
                                  int klen)
{
...
static gcry_err_code_t salsa20_do_setkey(SALSA20_context_t* ctx,
...
static gcry_err_code_t salsa20_setkey(void* context, const byte* key,
                                      unsigned int keylen)
{
...
static void salsa20_do_encrypt_stream(SALSA20_context_t* ctx, byte* outbuf,
...
static void salsa20_encrypt_stream(void* context, byte* outbuf,
                                   const byte* inbuf, size_t length)
{
    SALSA20_context_t* ctx = (SALSA20_context_t*)context;
...
static void salsa20r12_encrypt_stream(void* context, byte* outbuf,
                                      const byte* inbuf, size_t length)
{
    SALSA20_context_t* ctx = (SALSA20_context_t*)context;
...
static const char* selftest(void)
{
    ...
    static byte key_1[] = {0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                           0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                           0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                           0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
    static const byte nonce_1[] = {0x00, 0x00, 0x00, 0x00,
                                   0x00, 0x00, 0x00, 0x00};
    static const byte plaintext_1[] = {0x00, 0x00, 0x00, 0x00,
                                       0x00, 0x00, 0x00, 0x00};
    static const byte ciphertext_1[] = {0xE3, 0xBE, 0x8F, 0xDD,
                                        0x8B, 0xEC, 0xA2, 0xE3};
    ...
    salsa20_encrypt_stream(ctx, buf + (sizeof buf) - 1, buf + (sizeof buf) - 1,
                           1);
    for (i = 0; i < sizeof buf; i++)
        if (buf[i] != (byte)i)
