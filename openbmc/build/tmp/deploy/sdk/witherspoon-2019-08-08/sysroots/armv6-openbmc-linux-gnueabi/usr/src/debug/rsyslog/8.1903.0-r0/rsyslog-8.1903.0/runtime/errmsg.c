 ...
 * to take further case, as the code now boils to be either my own or, a few
 * lines, of the original BSD-licenses sysklogd code. rgerhards, 2012-01-16
 *
...
static int
    bHadErrMsgs; /* indicates if we had error messages since reset of this flag
 ...
 */
static void doLogMsg(const int iErrno, const int iErrCode, const int severity,
                     const char* msg)
{
    char buf[2048];
    char errStr[1024];
    ...
    if (iErrno != 0)
    {
        ...
        else
        {
            snprintf(buf, sizeof(buf),
                     "%s: %s [v%s try https://www.rsyslog.com/e/%d ]", msg,
                     errStr, VERSION, iErrCode * -1);
        }
    }
    else
    {
        ...
        {
            snprintf(buf, sizeof(buf),
                     "%s [v%s try https://www.rsyslog.com/e/%d ]", msg, VERSION,
                     iErrCode * -1);
        }
    }
    ...
    {
        /* in extreme cases, our error messages may be longer than the
         * configured max message size. If so, we just truncate without further
         * indication, as anything else would probably lead to a death loop on
         * error messages. Note that we do not split, as we really do not
         * anticipate there is much value in supporting extremely short max
         * message sizes - we assume it's just a testbench thing. -- rgerhards,
         * 2018-05-11
         */
...
LogError(const int iErrno, const int iErrCode, const char* fmt, ...)
{
    ...
    if (lenBuf < 0)
    {
        strncpy(buf, "error message lost due to problem with vsnprintf",
                sizeof(buf));
    }
    va_end(ap);
    buf[sizeof(buf) - 1] = '\0'; /* just to be on the safe side... */
...
LogMsg(const int iErrno, const int iErrCode, const int severity,
       const char* fmt, ...)
{
    va_list ap;
    char buf[2048];
...
rsRetVal ATTR_NONNULL() writeOversizeMessageLog(const smsg_t* const pMsg)
{
    ...
    {
        fdOversizeMsgLog =
            open((char*)glblGetOversizeMsgErrorFile(),
                 O_WRONLY | O_CREAT | O_APPEND | O_LARGEFILE | O_CLOEXEC,
                 S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP);
        if (fdOversizeMsgLog == -1)
        {
            LogError(errno, RS_RET_ERR,
                     "error opening oversize message log file %s",
