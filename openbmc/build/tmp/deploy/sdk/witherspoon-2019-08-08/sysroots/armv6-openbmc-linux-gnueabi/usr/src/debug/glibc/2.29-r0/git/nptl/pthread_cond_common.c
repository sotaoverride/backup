...
static uint64_t __condvar_fetch_add_64_relaxed(_condvar_lohi* lh,
                                               unsigned int op)
{
...
static uint64_t __condvar_load_64_relaxed(_condvar_lohi* lh)
{
    unsigned int h, l, h2;
    do
    {
...
__condvar_fetch_xor_wseq_release(pthread_cond_t* cond, unsigned int val)
{
    ...
    unsigned int l2 =
        atomic_fetch_xor_release(&lh->low, val) & ~((unsigned int)1 << 31);
    if (l2 < l)
        /* The lower-order half overflowed in the meantime.  This happened
           exactly once due to the limit on concurrent waiters (see above).  */
        h++;
...
/* Returns FUTEX_SHARED or FUTEX_PRIVATE based on the provided __wrefs
   value.  */
static int __attribute__((unused)) __condvar_get_private(int flags)
{
    ...
    while ((r >> 1) > 0)
    {
        ...
        if ((r >> 1) > 0)
        {
            /* There is still a waiter after spinning.  Set the wake-request
               ...
               Update r to include the set wake-request flag so that the
               upcoming futex_wait only blocks if the flag is still set
               (otherwise, we'd violate the basic client-side futex protocol).
             */
