...
{
    if (common_args_process(argc, argv, min_argc, max_argc, cmd, usage, flags))
        return 1;
...
void do_extent_open(int argc, char* argv[])
{
    ...
    if (cp)
        *cp = 0;
    sprintf(extent_prompt + strlen(extent_prompt),
            " (extent ino %d): ", current_ino);
...
void do_extent_close(int argc, char* argv[])
{
    ...
    if (common_args_process(argc, argv, 1, 1, "extent_close", "", 0))
        return;
...
static void generic_goto_node(const char* my_name, int argc, char** argv,
                              int op)
{
    struct ext2fs_extent extent;
    errcode_t retval;

    if (my_name && common_args_process(argc, argv, 1, 1, my_name, "", 0))
        return;
...
void do_delete_node(int argc, char* argv[])
{
    ...
    retval = ext2fs_extent_get(current_handle, EXT2_EXTENT_CURRENT, &extent);
    if (retval)
        return;
    dbg_print_extent(0, &extent);
}
...
void do_replace_node(int argc, char* argv[])
{
    ...
    if (common_extent_args_process(argc, argv, 3, 5, "replace_node", usage,
                                   CHECK_FS_RW | CHECK_FS_BITMAPS))
        return;
...
void do_insert_node(int argc, char* argv[])
{
    ...
    if (common_extent_args_process(argc, argv, 3, 6, "insert_node", usage,
                                   CHECK_FS_RW | CHECK_FS_BITMAPS))
        return;
...
void do_set_bmap(int argc, char** argv)
{
    ...
    if (common_extent_args_process(argc, argv, 3, 5, "set_bmap", usage,
                                   CHECK_FS_RW | CHECK_FS_BITMAPS))
        return;
    ...
    retval = ext2fs_extent_set_bmap(current_handle, logical, physical, flags);
    if (retval)
    {
        com_err(cmd, retval, 0);
        return;
...
void do_print_all(int argc, char** argv)
{
    ...
    if (common_extent_args_process(argc, argv, 1, 2, "print_all", usage, 0))
        return;
...
void do_goto_block(int argc, char** argv)
{
    ...
    if (retval)
    {
        com_err(argv[0], retval, "while trying to go to block %llu, level %d",
