...
{
    struct sigaction oldact, act = {.sa_handler = reply_sighandler};
    struct pollfd fds[] = {{.fd = fileno(stdin), .events = POLLIN}};
    size_t sz;
    int ret = 0;

    DBG(ASK, ul_debug("asking for user reply %s",
...
static int ask_menu(struct fdisk_context* cxt, struct fdisk_ask* ask, char* buf,
                    size_t bufsz)
...
static int ask_number(struct fdisk_context* cxt, struct fdisk_ask* ask,
                      char* buf, size_t bufsz)
{
    ...
    {
        if (inchar)
            snprintf(prompt, sizeof(prompt), _("%s (%s, default %c): "), q,
                     range, tochar(dflt));
        else
            snprintf(prompt, sizeof(prompt),
                     _("%s (%s, default %" PRIu64 "): "), q, range, dflt);
    }
    else if (dflt >= low && dflt <= high)
    {
        if (inchar)
            snprintf(prompt, sizeof(prompt), _("%s (%c-%c, default %c): "), q,
                     tochar(low), tochar(high), tochar(dflt));
        else
            snprintf(prompt, sizeof(prompt),
        ...
        snprintf(prompt, sizeof(prompt), _("%s (%c-%c): "), q, tochar(low),
                 tochar(high));
    else
        snprintf(prompt, sizeof(prompt), _("%s (%" PRIu64 "-%" PRIu64 "): "), q,
                 low, high);
...
static int ask_offset(struct fdisk_context* cxt, struct fdisk_ask* ask,
                      char* buf, size_t bufsz)
{
    char prompt[128] = {'\0'};
    const char* q = fdisk_ask_get_query(ask);
    ...
    DBG(ASK, ul_debug("asking for offset ['%s', <%" PRIu64 ",%" PRIu64
                      ">, base=%" PRIu64 ", default=%" PRIu64 ", range: %s]",
                      q, low, high, base, dflt, range));

    if (range && dflt >= low && dflt <= high)
        snprintf(prompt, sizeof(prompt), _("%s (%s, default %" PRIu64 "): "), q,
                 range, dflt);
    else if (dflt >= low && dflt <= high)
        snprintf(prompt, sizeof(prompt),
                 _("%s (%" PRIu64 "-%" PRIu64 ", default %" PRIu64 "): "), q,
                 low, high, dflt);
    else
        snprintf(prompt, sizeof(prompt), _("%s (%" PRIu64 "-%" PRIu64 "): "), q,
                 low, high);
    ...
    do
    {
        ...
        DBG(ASK, ul_debug("final offset: %ju [sig: %c, power: %d, %s]", num,
                          sig, pwr, sig ? "relative" : "absolute"));
        if (num >= low && num <= high)
        {
            if (sig && pwr)
                 ...
                 void* data __attribute__((__unused__)))
{
    ...
    switch (fdisk_ask_get_type(ask))
    {
        ...
        default:
            warnx(_("internal error: unsupported dialog type %d"),
...
#define NAME_WIDTH 15
            ...
            if (fdisk_parttype_get_name(t))
            {
                ...
                ret = mbsalign(_(fdisk_parttype_get_name(t)), name,
                               sizeof(name), &width, MBS_ALIGN_LEFT, 0);

                if (ret == (size_t)-1 || ret >= sizeof(name))
                    printf("%-15.15s", _(fdisk_parttype_get_name(t)));
                else
                    fputs(name, stdout);
            }
...
void toggle_dos_compatibility_flag(struct fdisk_context* cxt)
{
    ...
    fdisk_info(cxt, flag ? _("DOS Compatibility flag is set (DEPRECATED!)")
...
void change_partition_type(struct fdisk_context* cxt)
{
    ...
    if (fdisk_set_partition_type(cxt, i, t) == 0)
        fdisk_info(cxt, _("Changed type of partition '%s' to '%s'."), old,
                   t ? fdisk_parttype_get_name(t) : _("Unknown"));
    else
        fdisk_info(cxt, _("Type of partition %zu is unchanged: %s."), i + 1,
                   old);
...
void follow_wipe_mode(struct fdisk_context* cxt)
{
    ...
    if (dowipe)
        fdisk_warnx(
            cxt, _("The old %s signature will be removed by a write command."),
...
static void __attribute__((__noreturn__)) usage(void)
{
    ...
    fputs(
        _(" -b, --sector-size <size>      physical and logical sector size\n"),
        out);
    fputs(_(" -B, --protect-boot            don't erase bootbits when creating "
            "a new label\n"),
          out);
    fputs(_(" -c, --compatibility[=<mode>]  mode is 'dos' or 'nondos' "
            "(default)\n"),
          out);
    fputs(_(" -L, --color[=<when>]          colorize output (auto, always or "
            "never)\n"),
          out);
    ...
    fputs(_(" -t, --type <type>             recognize specified partition "
            "table type only\n"),
          out);
    fputs(_(" -u, --units[=<unit>]          display units: 'cylinders' or "
            "'sectors' (default)\n"),
          out);
    fputs(_(" -s, --getsz                   display device size in 512-byte "
            "sectors [DEPRECATED]\n"),
          out);
    ...
    fputs(_(" -H, --heads <number>          specify the number of heads\n"),
          out);
    fputs(_(" -S, --sectors <number>        specify the number of sectors per "
            "track\n"),
          out);
...
int main(int argc, char** argv)
{
                            ...
                            longopts, NULL)) != -1)
    {
        switch (c)
        {
            ...
            {
                size_t sz =
                    strtou32_or_err(optarg, _("invalid sector size argument"));
            ...
            case 'C':
                fdisk_save_user_geometry(
                    cxt,
                    strtou32_or_err(optarg, _("invalid cylinders argument")), 0,
                    0);
                break;
            case 'c':
                if (optarg)
                {
                    ...
                    if (strcmp(p, "dos") == 0)
                        ...
                        errx(EXIT_FAILURE, _("unknown compatibility mode '%s'"),
                             p);
                }
            ...
            case 'H':
                fdisk_save_user_geometry(
                    cxt, 0,
                    strtou32_or_err(optarg, _("invalid heads argument")), 0);
                break;
            case 'S':
                fdisk_save_user_geometry(
                    cxt, 0, 0,
                    strtou32_or_err(optarg, _("invalid sectors argument")));
                break;
            case 'l':
                act = ACT_LIST;
                break;
    ...
    switch (act)
    {
        ...
        case ACT_FDISK:
            ...
            color_disable();
            fdisk_info(cxt, _("Changes will remain in memory only, until you "
            ...
            if (!fdisk_has_label(cxt))
            {
                ...
                fdisk_warnx(cxt,
                            _("A hybrid GPT was detected. You have to sync "
