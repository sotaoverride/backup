...
const char* const catalog_file_dirs[] = {"/usr/local/lib/systemd/catalog/",
                                         "/usr/lib/systemd/catalog/", NULL};
...
{
    sd_id128_t id;
    char language[32]; /* One byte is used for termination, so the maximum
                        * allowed length of the string is actually 31 bytes. */
    le64_t offset;
} CatalogItem;
...
DEFINE_HASH_OPS(catalog_hash_ops, CatalogItem, catalog_hash_func,
...
static int finish_item(Hashmap* h, sd_id128_t id, const char* language,
...
static int catalog_entry_lang(const char* filename, unsigned line,
                              const char* t, const char* deflang, char** ret)
{
    ...
    if (deflang)
    {
        if (streq(t, deflang))
        {
            log_warning("[%s:%u] language specified unnecessarily", filename,
                        line);
            return 0;
        }

        log_warning("[%s:%u] language differs from default for file", filename,
                    line);
    }
...
int catalog_import_file(Hashmap* h, const char* path)
{
    ...
    if (r < 0)
        log_error_errno(r, "Failed to determine language for file %s: %m",
                        path);
    if (r == 1)
    ...
    for (;;)
    {
        ...
        if (empty_line && strlen(line) >= 2 + 1 + 32 && line[0] == '-' &&
            line[1] == '-' && line[2] == ' ' &&
        ...
        if (!got_id)
            return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                                   "[%s:%u] Got payload before ID.", path, n);

        line_len = strlen(line);
        if (!GREEDY_REALLOC(payload, payload_allocated,
                            payload_size + (empty_line ? 1 : 0) + line_len + 1 +
                                1))
            return log_oom();
                             ...
                             CatalogItem* items, size_t n)
{
    ...
    if (r < 0)
        return log_error_errno(
            r, "Failed to create parent directories of %s: %m", database);
                   ...
                   const char* const* dirs)
{
    ...
    log_debug("%s: wrote %u items, with %zu bytes of strings, %" PRIi64
              " total size.",
              database, n, sb->len, sz);
    return 0;
}

static int open_mmap(const char* database, int* _fd, struct stat* _st,
                     void** _p)
{
    ...
    h = p;
    if (memcmp(h->signature, (const uint8_t[])CATALOG_SIGNATURE,
               sizeof(h->signature)) != 0 ||
        le64toh(h->header_size) < sizeof(CatalogHeader) ||
        le64toh(h->catalog_item_size) < sizeof(CatalogItem) ||
        h->incompatible_flags != 0 || le64toh(h->n_items) <= 0 ||
        st.st_size <
            (off_t)(le64toh(h->header_size) +
...
static const char* find_id(void* p, sd_id128_t id)
{
    ...
    if (!isempty(loc) && !STR_IN_SET(loc, "C", "POSIX"))
    {
        ...
        if (len > sizeof(key.language) - 1)
            log_debug("LC_MESSAGES value too long, ignoring: \"%.*s\"",
                      (int)len, loc);
        else
        {
            ...
            if (!f)
            {
                ...
                if (e)
                {
                    *e = 0;
                    f = bsearch(
                        &key, (const uint8_t*)p + le64toh(h->header_size),
                        le64toh(h->n_items), le64toh(h->catalog_item_size),
                        (comparison_fn_t)catalog_compare_func);
                }
            }
...
static void dump_catalog_entry(FILE* f, sd_id128_t id, const char* s,
                               bool oneline)
{
    if (oneline)
    {
...
int catalog_list_items(FILE* f, const char* database, bool oneline,
                       char** items)
{
    char** item;
    int r = 0;
