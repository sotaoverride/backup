...
#define G_NULL_SETTINGS_BACKEND(inst)                                          \
    (G_TYPE_CHECK_INSTANCE_CAST((inst), G_TYPE_NULL_SETTINGS_BACKEND,          \
...
G_DEFINE_TYPE_WITH_CODE(
    GNullSettingsBackend, g_null_settings_backend, G_TYPE_SETTINGS_BACKEND,
    g_io_extension_point_implement(G_SETTINGS_BACKEND_EXTENSION_POINT_NAME,
                                   g_define_type_id, "null", 10))

static GVariant* g_null_settings_backend_read(GSettingsBackend* backend,
                                              const gchar* key,
...
static gboolean g_null_settings_backend_write(GSettingsBackend* backend,
...
static gboolean g_null_settings_backend_write_one(gpointer key, gpointer value,
                                                  gpointer data)
{
    if (value)
        g_variant_unref(g_variant_ref_sink(value));
...
static gboolean g_null_settings_backend_write_tree(GSettingsBackend* backend,
                                                   GTree* tree,
                                                   gpointer origin_tag)
{
...
static void g_null_settings_backend_reset(GSettingsBackend* backend,
                                          const gchar* key, gpointer origin_tag)
{
}

static gboolean g_null_settings_backend_get_writable(GSettingsBackend* backend,
                                                     const gchar* name)
{
    return FALSE;
}
...
static void g_null_settings_backend_init(GNullSettingsBackend* memory)
{
}

static void g_null_settings_backend_class_init(GNullSettingsBackendClass* class)
{
