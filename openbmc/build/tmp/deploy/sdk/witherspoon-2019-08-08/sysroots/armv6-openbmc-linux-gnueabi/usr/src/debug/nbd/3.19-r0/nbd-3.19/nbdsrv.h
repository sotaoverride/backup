...
{
    gchar*
        exportname; /**< (unprocessed) filename of the file we're exporting */
...
typedef struct _client
{
    ...
    struct sockaddr_storage
        clientaddr;   /**< peer, in binary format, network byte order */
...
typedef enum
{
    ...
    NBDS_ERR_CFILE_MISSING_GENERIC, /**< The (required) group "generic" is
                                       missing */
    NBDS_ERR_CFILE_KEY_MISSING,     /**< A (required) key is missing */
    NBDS_ERR_CFILE_VALUE_INVALID,   /**< A value is syntactically invalid */
    NBDS_ERR_CFILE_VALUE_UNSUPPORTED, /**< A value is not supported in this
                                         build */
    ...
    NBDS_ERR_CFILE_INVALID_SPLICE, /**< We can't use splice with the other
                                      options specified for the export. */
...
#define F_COPYONWRITE                                                          \
    4                    /**< flag to tell us a file is exported using         \
               copyonwrite */
#define F_AUTOREADONLY 8 /**< flag to tell us a file is set to autoreadonly */
#define F_SPARSE                                                               \
    16 /**< flag to tell us copyronwrite should use a sparse file */
#define F_SDP                                                                  \
    32 /**< flag to tell us the export should be done using the Socket Direct  \
          Protocol for RDMA */
...
#define F_ROTATIONAL                                                           \
    512 /**< Whether server wants the client to implement the elevator         \
           algorithm */
#define F_TEMPORARY                                                            \
    1024 /**< Whether the backing file is temporary and should be created then \
            unlinked */
#define F_TRIM                                                                 \
    2048 /**< Whether server wants TRIM (discard) to be sent by the client */
#define F_FIXED                                                                \
    4096 /**< Client supports fixed new-style protocol (and can thus send us   \
            extra options */
#define F_TREEFILES 8192 /**< flag to tell us a file is exported using -t */
#define F_FORCEDTLS                                                            \
    16384 /**< TLS is required, either for the server as a whole or for a      \
             given export */
#define F_SPLICE                                                               \
    32768 /**< flag to tell us to use splice for read/write operations */
 ...
 */
bool address_matches(const char* mask, const struct sockaddr* addr,
                     GError** err);
