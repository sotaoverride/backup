...
#define g_array_elt_pos(array, i)                                              \
    ((array)->data + g_array_elt_len((array), (i)))
...
GArray* g_array_sized_new(gboolean zero_terminated, gboolean clear,
                          guint elt_size, guint reserved_size)
{
    GRealArray* array;
...
gchar* g_array_free(GArray* farray, gboolean free_segment)
{
    ...
    /* if others are holding a reference, preserve the wrapper but do
     * free/return the data */
 ...
 * g_array_append_vals:
 * @array: a #GArray
 * @data: (not nullable): a pointer to the elements to append to the end of the
 * array
 ...
 * g_array_prepend_vals:
 * @array: a #GArray
 * @data: (nullable): a pointer to the elements to prepend to the start of the
 * array
 ...
 * Returns: the #GArray
 */
GArray* g_array_insert_vals(GArray* farray, guint index_, gconstpointer data,
                            guint len)
{
    GRealArray* array = (GRealArray*)farray;
 ...
 * Returns: the #GArray
 */
GArray* g_array_remove_index(GArray* farray, guint index_)
{
    GRealArray* array = (GRealArray*)farray;
 ...
 * Returns: the #GArray
 */
GArray* g_array_remove_index_fast(GArray* farray, guint index_)
{
    GRealArray* array = (GRealArray*)farray;
...
void g_array_sort(GArray* farray, GCompareFunc compare_func)
{
    ...
    g_qsort_with_data(array->data, array->len, array->elt_size,
 ...
 */
void g_array_sort_with_data(GArray* farray, GCompareDataFunc compare_func,
                            gpointer user_data)
{
    GRealArray* array = (GRealArray*)farray;
    ...
    g_qsort_with_data(array->data, array->len, array->elt_size, compare_func,
                      user_data);
}
...
static gpointer* ptr_array_free(GPtrArray* array, ArrayFreeFlags flags)
{
    GRealPtrArray* rarray = (GRealPtrArray*)array;
    gpointer* segment;
...
void g_ptr_array_set_size(GPtrArray* array, gint length)
{
    ...
    g_return_if_fail(rarray->len == 0 ||
                     (rarray->len != 0 && rarray->pdata != NULL));
    ...
    if (length_unsigned > rarray->len)
    {
        ...
        g_ptr_array_remove_range(array, length_unsigned,
                                 rarray->len - length_unsigned);

    rarray->len = length_unsigned;
}

static gpointer ptr_array_remove_index(GPtrArray* array, guint index_,
                                       gboolean fast, gboolean free_element)
{
    GRealPtrArray* rarray = (GRealPtrArray*)array;
    gpointer result;
    ...
    g_return_val_if_fail(
        rarray->len == 0 || (rarray->len != 0 && rarray->pdata != NULL), NULL);
                                    ...
                                    guint length)
{
    ...
    {
        memmove(&rarray->pdata[index_], &rarray->pdata[index_ + length],
...
gboolean g_ptr_array_remove(GPtrArray* array, gpointer data)
{
    ...
    g_return_val_if_fail(
        array->len == 0 || (array->len != 0 && array->pdata != NULL), FALSE);
...
gboolean g_ptr_array_remove_fast(GPtrArray* array, gpointer data)
{
    ...
    g_return_val_if_fail(
        rarray->len == 0 || (rarray->len != 0 && rarray->pdata != NULL), FALSE);
...
void g_ptr_array_insert(GPtrArray* array, gint index_, gpointer data)
{
    ...
    if ((guint)index_ < rarray->len)
        memmove(&(rarray->pdata[index_ + 1]), &(rarray->pdata[index_]),
...
void g_ptr_array_sort(GPtrArray* array, GCompareFunc compare_func)
{
    ...
    /* Don't use qsort as we want a guaranteed stable sort */
    g_qsort_with_data(array->pdata, array->len, sizeof(gpointer),
                      (GCompareDataFunc)compare_func, NULL);
}
 ...
 * This is guaranteed to be a stable sort since version 2.32.
 */
void g_ptr_array_sort_with_data(GPtrArray* array, GCompareDataFunc compare_func,
                                gpointer user_data)
{
    g_return_if_fail(array != NULL);

    g_qsort_with_data(array->pdata, array->len, sizeof(gpointer), compare_func,
                      user_data);
}
