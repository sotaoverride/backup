...
static gboolean unquote_string_inplace(gchar* str, gchar** end, GError** err)
{
    ...
    *dest = '\0';
    ...
    g_set_error_literal(err, G_SHELL_ERROR, G_SHELL_ERROR_BAD_QUOTING,
                        _("Unmatched quotation mark in command line or other "
                          "shell-quoted text"));
    *end = s;
    return FALSE;
}
        ...
        else
        {
            switch (*p)
            {
                ...
                default:
                     ...
                     * it, otherwise create a new token.
                     */
                    ensure_token(&current_token);
                    g_string_append_c(current_token, *p);
                    break;
 ...
 * @argvp: (out) (optional) (array length=argcp zero-terminated=1) (element-type
 *filename): return location for array of args
