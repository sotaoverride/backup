...
static const struct option longopts[] = {
    {"index", 1, 0, 'i'},   {"brief", 0, 0, 'b'}, {"format", 1, 0, 'f'},
...
static void usage(char* program, int error)
{
    ...
    fprintf(
        file,
        _("usage: %s [-bVh] [-i index] [-f FORMAT] [--index index] [--brief] "
    ...
    fprintf(file, _("  -i INDEX   (index)         display the INDEX face of "
                    "each font file only\n"));
    fprintf(file,
            _("  -b         (brief)         display font pattern briefly\n"));
    fprintf(file,
            _("  -f FORMAT  (format)        use the given output format\n"));
...
int main(int argc, char** argv)
{
...
#endif
    {
        ...
        {
            case 'i':
                id = (unsigned int)strtol(optarg, NULL,
                                          0); /* strtol() To handle -1. */
                break;
            case 'b':
                brief = 1;
                break;
