...
struct vertex_from_name
{
  private:
    typedef
        typename internal_vertex_name<VertexProperty>::type extract_name_type;

    typedef typename remove_cv<typename remove_reference<
        typename extract_name_type::result_type>::type>::type vertex_name_type;
...
class named_graph
{
  ...
  private:
    ...
    struct extract_name_from_vertex
    {
        ...
        extract_name_from_vertex(Graph& graph,
                                 const extract_name_type& extract) :
            graph(graph),
            extract(extract)
        {
        }
  ...
  public:
    ...
    typedef multi_index::multi_index_container<
        Vertex, multi_index::indexed_by<multi_index::hashed_unique<
                    multi_index::tag<vertex_name_t>, extract_name_from_vertex>>>
        named_vertices_type;
...
BGL_NAMED_GRAPH::named_graph(
    const extract_name_type& extract,
    const vertex_constructor_type& vertex_constructor) :
    named_vertices(
        typename named_vertices_type::ctor_args_list(boost::make_tuple(
            boost::make_tuple(0, // initial number of buckets
...
inline void BGL_NAMED_GRAPH::removing_vertex(Vertex vertex, VertexIterStability)
{
    BOOST_STATIC_ASSERT_MSG(
        ...
        "Named graphs cannot use vecS as vertex container and remove vertices; "
        "the lack of vertex descriptor stability (which iterator stability is "
        "a proxy for) means that the name -> vertex mapping would need to be "
        "completely rebuilt after each deletion.  See "
        "https://svn.boost.org/trac/boost/ticket/7863 for more information and "
        "a test case.");
...
{
    typedef
        typename BGL_NAMED_GRAPH::vertices_by_name_type vertices_by_name_type;
