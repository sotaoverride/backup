                           ...
                           struct __gconv_step* handle, size_t nsteps)
{
    ...
    new_deriv = (struct known_derivation*)malloc(
        sizeof(struct known_derivation) + fromset_len + toset_len);
    if (new_deriv != NULL)
    {
...
static void __libc_freeres_fn_section free_derivation(void* p)
{
...
static int gen_steps(struct derivation_step* best, const char* toset,
                     const char* fromset, struct __gconv_step** handle,
                     size_t* nsteps)
{
    size_t step_cnt = 0;
    ...
    result =
        (struct __gconv_step*)malloc(sizeof(struct __gconv_step) * step_cnt);
    if (result != NULL)
    {
        int failed = 0;
                           ...
                           struct __gconv_step** handle, size_t* nsteps)
{
                ...
                do
                {
                         ...
                         strcmp(result_set, toset_expand) == 0))
                    {
                       ...
                       is no entry with this name.  */
                        ...
                        if (step == NULL)
                        {
                            step = NEW_STEP(result_set, cost_hi, cost_lo, runp,
                                            current);
                            step->next = solution;
                            solution = step;
                        }
                              ...
                              cost_lo < best_cost_lo))
                    {
                       ...
                       is no entry with this name.  */
                        ...
                        if (step == NULL)
                        {
                            *lastp = NEW_STEP(result_set, cost_hi, cost_lo,
                                              runp, current);
                            lastp = &(*lastp)->next;
                        }
                        else if (step->cost_hi > cost_hi ||
                                  ...
                                  step->cost_lo > cost_lo))
                        {
                            ...
                            {
                                step->cost_hi =
                                    (step->code->cost_hi + step->last->cost_hi);
                                step->cost_lo =
                                    (step->code->cost_lo + step->last->cost_lo);
    ...
    add_derivation(fromset_expand ?: fromset, toset_expand ?: toset, *handle,
                   *nsteps);
...
static void __libc_freeres_fn_section free_modules_db(struct gconv_module* node)
{
    if (node->left != NULL)
