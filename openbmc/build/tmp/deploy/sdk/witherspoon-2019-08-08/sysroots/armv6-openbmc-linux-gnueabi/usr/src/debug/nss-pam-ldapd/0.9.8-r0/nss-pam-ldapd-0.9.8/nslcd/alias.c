...
/* create a search filter for searching an alias by name,
   return -1 on errors */
static int mkfilter_alias_byname(const char* name, char* buffer, size_t buflen)
{
    ...
    /* build filter */
    return mysnprintf(buffer, buflen, "(&%s(%s=%s))", alias_filter,
...
NSLCD_HANDLE(alias, byname, NSLCD_ACTION_ALIAS_BYNAME, char name[BUFLEN_NAME];
...
NSLCD_HANDLE(alias, all, NSLCD_ACTION_ALIAS_ALL, const char* filter;
             log_setrequest("alias(all)");
             , (filter = alias_filter, 0), write_alias(fp, entry, NULL))
