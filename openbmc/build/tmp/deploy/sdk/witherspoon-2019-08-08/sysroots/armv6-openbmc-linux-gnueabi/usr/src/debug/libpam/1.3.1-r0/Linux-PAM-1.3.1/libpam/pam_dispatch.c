        ...
        switch (action)
        {
            ...
            default:
                if (_PAM_ACTION_IS_JUMP(action))
                {
                     ...
                     */
                    while (h->next != NULL &&
                           h->next->stack_level >= stack_level && action > 0)
                    {
                        do
                        {
                            h = h->next;
                            ++depth;
                                 ...
                                 h->next->stack_level > stack_level);
                        --action;
                    }
...
int _pam_dispatch(pam_handle_t* pamh, int flags, int choice)
{
    ...
    if (h == NULL)
    { /* there was no handlers.conf... entry; will use
       * handlers.other... */
        switch (choice)
        {
            case PAM_AUTHENTICATE:
...
#ifdef HAVE_LIBAUDIT
    if (choice != PAM_CHAUTHTOK || flags & PAM_UPDATE_AUTHTOK ||
        retval != PAM_SUCCESS)
    {
