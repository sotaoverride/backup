...
static int socket_buffer_new_internal(SocketBuffer** bufferp, size_t n_vecs,
                                      size_t n_line)
{
    SocketBuffer* buffer;
...
static int socket_buffer_new_line(SocketBuffer** bufferp, Socket* socket,
                                  User* user, size_t n)
{
    ...
    r = user_charge(socket->user, &buffer->charges[0], user, USER_SLOT_BYTES,
...
static int socket_buffer_new_message(SocketBuffer** bufferp, Socket* socket,
                                     User* user, Message* message)
{
    ...
    r = user_charge(socket->user, &buffer->charges[1], user, USER_SLOT_FDS,
...
static size_t socket_buffer_get_line_space(SocketBuffer* buffer)
{
    ...
    n_remaining -=
        (char*)buffer->vecs[0].iov_base - socket_buffer_get_base(buffer);
    n_remaining -= buffer->vecs[0].iov_len;

    return n_remaining;
}

static void socket_buffer_get_line_cursor(SocketBuffer* buffer, char** datap,
                                          size_t** posp)
{
    assert(!buffer->message);
...
static void socket_discard_output(Socket* socket)
{
    ...
    while (
        (buffer = c_list_first_entry(&socket->out.queue, SocketBuffer, link)))
...
void socket_deinit(Socket* socket)
{
    ...
    while (
        (buffer = c_list_first_entry(&socket->out.pending, SocketBuffer, link)))
        socket_buffer_free(buffer);
...
{
    if (_c_unlikely_(!socket->reset && socket->hup_in && socket->hup_out &&
...
int socket_dequeue(Socket* socket, Message** messagep)
{
    ...
    {
        r = iqueue_set_target(&socket->in.queue, &socket->in.header,
    ...
    if (!socket->in.message)
    {
        ...
        if (r == MESSAGE_E_CORRUPT_HEADER || r == MESSAGE_E_TOO_LARGE)
        {
            socket_close(socket);
            return SOCKET_E_EOF;
        }
...
static int socket_recvmsg(Socket* socket, void* buffer, size_t* from, size_t to,
                          FDList** fdsp, UserCharge* charge_fds)
{
    union
    {
        struct cmsghdr cmsg;
    ...
    if (_c_unlikely_(n_fds))
    {
        ...
        r = user_charge(socket->user, charge_fds, NULL, USER_SLOT_FDS, n_fds);
        if (r == USER_E_QUOTA)
        {
            /*
...
static int socket_dispatch_read(Socket* socket)
{
    ...
    r = iqueue_get_cursor(&socket->in.queue, &buffer, &from, &to, &fds,
                          &charge_fds);
    if (r == IQUEUE_E_PENDING)
    {
        return 0;
    }
    else if (r == IQUEUE_E_QUOTA || r == IQUEUE_E_VIOLATION)
    {
        socket_close(socket);
        return SOCKET_E_LOST_INTEREST;
    }
    ...
    return socket_recvmsg(socket, buffer, from, to, fds, charge_fds);
}
