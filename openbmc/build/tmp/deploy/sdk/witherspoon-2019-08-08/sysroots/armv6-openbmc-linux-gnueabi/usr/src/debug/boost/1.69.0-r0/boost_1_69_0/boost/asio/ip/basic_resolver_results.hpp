...
#endif // !defined(BOOST_ASIO_NO_DEPRECATED)
{
  public:
    ...
    static basic_resolver_results
        create(boost::asio::detail::addrinfo_type* address_info,
               const std::string& host_name, const std::string& service_name)
    {
        ...
        while (address_info)
        {
                ...
                address_info->ai_family == BOOST_ASIO_OS_DEF(AF_INET6))
            {
                ...
                endpoint.resize(
                    static_cast<std::size_t>(address_info->ai_addrlen));
               ...
               const std::string& host_name, const std::string& service_name)
    {
        ...
        if (begin != end)
        {
            ...
            {
                results.values_->push_back(
                    basic_resolver_entry<InternetProtocol>(*ep_iter, host_name,
                                                           service_name));
            }
        }
        return results;
    ...
    {
        return this->values_ ? this->values_->max_size()
                             : values_type().max_size();
    }
