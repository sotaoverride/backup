...
#define pdbg_for_each_compatible(parent, target, compat)                       \
    for (target = NULL; (target = __pdbg_next_compatible_node(                 \
                             parent, target, compat)) != NULL;)
...
#define pdbg_for_each_child_target(parent, target)                             \
    for (target = __pdbg_next_child_target(parent, NULL); target;              \
...
struct pdbg_target* pdbg_target_parent(const char* klass,
...
int pdbg_target_u32_property(struct pdbg_target* target, const char* name,
                             uint32_t* val);
int pdbg_target_u32_index(struct pdbg_target* target, const char* name,
...
struct pdbg_target* pdbg_target_from_path(struct pdbg_target* target,
                                          const char* path);
...
enum pdbg_sleep_state
{
    ...
    PDBG_THREAD_STATE_DOZE,
    PDBG_THREAD_STATE_NAP,
    PDBG_THREAD_STATE_SLEEP,
...
int adu_getmem(struct pdbg_target* target, uint64_t addr, uint8_t* ouput,
               uint64_t size);
int adu_putmem(struct pdbg_target* target, uint64_t addr, uint8_t* input,
               uint64_t size);
int adu_getmem_ci(struct pdbg_target* target, uint64_t addr, uint8_t* ouput,
                  uint64_t size);
int adu_putmem_ci(struct pdbg_target* target, uint64_t addr, uint8_t* input,
                  uint64_t size);
...
int sbe_chipop(struct pdbg_target* target, uint32_t* msg, uint32_t msg_len,
