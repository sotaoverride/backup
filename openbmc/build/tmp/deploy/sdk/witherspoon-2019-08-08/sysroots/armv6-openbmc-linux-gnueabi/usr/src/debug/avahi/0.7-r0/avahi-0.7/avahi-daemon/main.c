...
typedef struct
{
    ...
    int rlimit_as_set, rlimit_core_set, rlimit_data_set, rlimit_fsize_set,
        rlimit_nofile_set, rlimit_stack_set;
    rlim_t rlimit_as, rlimit_core, rlimit_data, rlimit_fsize, rlimit_nofile,
        rlimit_stack;
...
static int load_resolv_conf(void)
{
    ...
    resolv_conf_name_servers =
        avahi_new0(char*, AVAHI_WIDE_AREA_SERVERS_MAX + 1);
    ...
    while (!feof(f))
    {
        ...
        if ((has_prefix(p, "nameserver ") || has_prefix(p, "nameserver\t")) &&
            i < AVAHI_WIDE_AREA_SERVERS_MAX)
        {
            p += 10;
            p += strspn(p, "\t ");
            p[strcspn(p, "\t ")] = 0;
...
static AvahiSEntryGroup* add_dns_servers(AvahiServer* s, AvahiSEntryGroup* g,
                                         char** l)
{
    char** p;
    ...
    for (p = l; *p; p++)
    {
        ...
        else if (avahi_server_add_dns_server_address(
                     s, g, AVAHI_IF_UNSPEC, AVAHI_PROTO_UNSPEC, 0, NULL,
                     AVAHI_DNS_SERVER_RESOLVE, &a, 53) < 0)
        {
            avahi_s_entry_group_free(g);
            avahi_log_error("Failed to add DNS server address: %s",
...
static void update_wide_area_servers(void)
{
    ...
    for (p = resolv_conf_name_servers; *p && n < AVAHI_WIDE_AREA_SERVERS_MAX;
         p++)
    {
...
static void update_browse_domains(void)
{
    ...
    for (p = resolv_conf_search_domains, n = 0; *p && n < BROWSE_DOMAINS_MAX;
         p++, n++)
    {
...
static void server_callback(AvahiServer* s, AvahiServerState state,
                            void* userdata)
{
...
#ifdef HAVE_DBUS
    if (c->enable_dbus && state != AVAHI_SERVER_INVALID &&
        state != AVAHI_SERVER_FAILURE)
    ...
    switch (state)
    {
        case AVAHI_SERVER_RUNNING:
            avahi_log_info("Server startup complete. Host name is %s. Local "
                           "service cookie is %u.",
                           avahi_server_get_host_name_fqdn(s),
                           avahi_server_get_local_service_cookie(s));
            sd_notifyf(0,
                       "STATUS=Server startup complete. Host name is %s. Local "
                       "service cookie is %u.",
                       avahi_server_get_host_name_fqdn(s),
                       avahi_server_get_local_service_cookie(s));
            avahi_set_proc_title(argv0, "%s: running [%s]", argv0,
                                 avahi_server_get_host_name_fqdn(s));
            ...
            if (c->publish_resolv_conf && resolv_conf_name_servers &&
                resolv_conf_name_servers[0])
                resolv_conf_entry_group = add_dns_servers(
                    s, resolv_conf_entry_group, resolv_conf_name_servers);

            if (c->publish_dns_servers && c->publish_dns_servers[0])
                dns_servers_entry_group = add_dns_servers(
                    s, dns_servers_entry_group, c->publish_dns_servers);
        ...
        case AVAHI_SERVER_FAILURE:
            ...
            avahi_log_error("Server error: %s",
                            avahi_strerror(avahi_server_errno(s)));
            sd_notifyf(0, "STATUS=Server error: %s",
                       avahi_strerror(avahi_server_errno(s)));
        ...
        case AVAHI_SERVER_REGISTERING:
            ...
            sd_notifyf(0, "STATUS=Registering host name %s",
                       avahi_server_get_host_name_fqdn(s));
            avahi_set_proc_title(argv0, "%s: registering [%s]", argv0,
                                 avahi_server_get_host_name_fqdn(s));
...
static void help(FILE* f)
{
    fprintf(f,
            ...
            "    -s --syslog        Write log messages to syslog(3) instead of "
            "STDERR\n"
            "    -k --kill          Kill a running daemon\n"
            "    -r --reload        Request a running daemon to reload static "
            "services\n"
            "    -c --check         Return 0 if a daemon is already running\n"
            "    -V --version       Show version\n"
            "    -f --file=FILE     Load the specified configuration file "
            "instead of\n"
            "                       " AVAHI_CONFIG_FILE "\n"
...
static int load_config_file(DaemonConfig* c)
{
    ...
    if (!(f = avahi_ini_file_load(c->config_file ? c->config_file
                                                 : AVAHI_CONFIG_FILE)))
        goto finish;
            ...
            for (p = g->pairs; p; p = p->pairs_next)
            {
                ...
                else if (strcasecmp(p->key, "browse-domains") == 0)
                {
                    ...
                    for (t = e; *t; t++)
                    {
                        ...
                        {
                            avahi_log_error("Invalid domain name \"%s\" for "
                                            "key \"%s\" in group \"%s\"\n",
                                            *t, p->key, g->name);
                            avahi_strfreev(e);
                            goto finish;
                        ...
                        c->server_config.browse_domains = avahi_string_list_add(
                            c->server_config.browse_domains, cleaned);
                    }

                    avahi_strfreev(e);

                    c->server_config.browse_domains = filter_duplicate_domains(
                        c->server_config.browse_domains);
                }
                    ...
                    for (t = e; *t; t++)
                        c->server_config.allow_interfaces =
                            avahi_string_list_add(
                                c->server_config.allow_interfaces, *t);
                ...
                else if (strcasecmp(p->key, "deny-interfaces") == 0)
                {
                    ...
                    for (t = e; *t; t++)
                        c->server_config.deny_interfaces =
                            avahi_string_list_add(
                                c->server_config.deny_interfaces, *t);
                ...
                else if (strcasecmp(p->key, "ratelimit-interval-usec") == 0)
                {
                    ...
                    {
                        avahi_log_error(
                            "Invalid ratelimit-interval-usec setting %s",
                            p->value);
                        goto finish;
                    }
                ...
                else if (strcasecmp(p->key, "ratelimit-burst") == 0)
                {
                    ...
                    {
                        avahi_log_error("Invalid ratelimit-burst setting %s",
                                        p->value);
                        goto finish;
                    }
                ...
                else if (strcasecmp(p->key, "cache-entries-max") == 0)
                {
                    ...
                    if (parse_unsigned(p->value, &k) < 0)
                    {
                        avahi_log_error("Invalid cache-entries-max setting %s",
                                        p->value);
                        goto finish;
                    }
                ...
                else if (strcasecmp(p->key, "clients-max") == 0)
                {
                    ...
                    if (parse_unsigned(p->value, &k) < 0)
                    {
                        avahi_log_error("Invalid clients-max setting %s",
                                        p->value);
                        goto finish;
                    }
                ...
                else if (strcasecmp(p->key, "objects-per-client-max") == 0)
                {
                    ...
                    {
                        avahi_log_error(
                            "Invalid objects-per-client-max setting %s",
                            p->value);
                        goto finish;
                    }
                ...
                else if (strcasecmp(p->key, "entries-per-entry-group-max") == 0)
                {
                    ...
                    {
                        avahi_log_error(
                            "Invalid entries-per-entry-group-max setting %s",
                            p->value);
                        goto finish;
                    }
            ...
            for (p = g->pairs; p; p = p->pairs_next)
            {
                ...
                if (strcasecmp(p->key, "publish-addresses") == 0)
                    ...
                    c->server_config.publish_domain = is_yes(p->value);
                                    ...
                                    "publish-resolv-conf-dns-servers") == 0)
                    ...
                    c->server_config.disable_publishing = is_yes(p->value);
                                    ...
                                    "disable-user-service-publishing") == 0)
...
#else
                    avahi_log_error(
                        "Ignoring configuration key \"%s\" in group \"%s\"\n",
                        p->key, g->name);
#endif
                }
                else
...
static void log_function(AvahiLogLevel level, const char* txt)
{
    ...
    static const int log_level_map[] = {LOG_ERR, LOG_WARNING, LOG_NOTICE,
                                        LOG_INFO, LOG_DEBUG};
...
static void add_inotify_watches(void)
{
    ...
    inotify_add_watch(inotify_fd, c ? "/services" : AVAHI_SERVICE_DIR,
    ...
    inotify_add_watch(inotify_fd, c ? "/" : AVAHI_CONFIG_DIR,
                      IN_CLOSE_WRITE | IN_DELETE | IN_DELETE_SELF |
                          IN_MOVED_FROM | IN_MOVED_TO | IN_MOVE_SELF
#ifdef IN_ONLYDIR
                          | IN_ONLYDIR
...
static void reload_config(void)
{
    ...
    if (config.publish_resolv_conf && resolv_conf_name_servers &&
        resolv_conf_name_servers[0])
        resolv_conf_entry_group = add_dns_servers(
            avahi_server, resolv_conf_entry_group, resolv_conf_name_servers);
}

#ifdef HAVE_INOTIFY

static void inotify_callback(AvahiWatch* watch, int fd,
                             AVAHI_GCC_UNUSED AvahiWatchEvent event,
                             AVAHI_GCC_UNUSED void* userdata)
{
    ...
    {
        avahi_free(buffer);
        avahi_log_error("Failed to read inotify event: %s",
...
static void kqueue_callback(AvahiWatch* watch, int fd,
                            AVAHI_GCC_UNUSED AvahiWatchEvent event,
                            AVAHI_GCC_UNUSED void* userdata)
{
    struct kevent ev;
    ...
    {
        avahi_log_error("Failed to read kqueue event: %s",
                        avahi_strerror(errno));
    }
}
...
static void signal_callback(AvahiWatch* watch, AVAHI_GCC_UNUSED int fd,
                            AVAHI_GCC_UNUSED AvahiWatchEvent event,
                            AVAHI_GCC_UNUSED void* userdata)
{
    int sig;
...
static int run_server(DaemonConfig* c)
{
    ...
    if (!(nss_support = avahi_nss_support()))
        avahi_log_warn("WARNING: No NSS support for mDNS detected, consider "
                       "installing nss-mdns!");
    ...
    {
        avahi_log_error("Could not register signal handlers (%s).",
                        strerror(errno));
        goto finish;
    }

    if (!(sig_watch =
              poll_api->watch_new(poll_api, daemon_signal_fd(), AVAHI_WATCH_IN,
                                  signal_callback, simple_poll_api)))
    {
    ...
    if (c->enable_dbus)
    {
        if (dbus_protocol_setup(
                poll_api, config.disable_user_service_publishing,
                config.n_clients_max, config.n_objects_per_client_max,
    ...
    else
    {
        ...
        if (!(inotify_watch =
                  poll_api->watch_new(poll_api, inotify_fd, AVAHI_WATCH_IN,
                                      inotify_callback, NULL)))
        {
    ...
    else
    {
        ...
        if (!(kqueue_watch = poll_api->watch_new(poll_api, kq, AVAHI_WATCH_IN,
                                                 kqueue_callback, NULL)))
        {
    ...
    if (!(avahi_server = avahi_server_new(poll_api, &c->server_config,
                                          server_callback, c, &error)))
    {
...
static int drop_root(void)
{
    ...
    avahi_log_info("Found user '" AVAHI_USER
                   "' (UID %lu) and group '" AVAHI_GROUP "' (GID %lu).",
                   (unsigned long)pw->pw_uid, (unsigned long)gr->gr_gid);
...
static int make_runtime_dir(void)
{
    ...
    {
        avahi_log_error(
            "Failed to create runtime directory " AVAHI_DAEMON_RUNTIME_DIR ".");
        goto fail;
    }
...
static void set_one_rlimit(int resource, rlim_t limit, const char* name)
{
    ...
    if (setrlimit(resource, &rl) < 0)
        avahi_log_warn("setrlimit(%s, {%u, %u}) failed: %s", name,
                       (unsigned)limit, (unsigned)limit, strerror(errno));
}
...
static void init_rand_seed(void)
{
    ...
    /* If the initialization failed by some reason, we add the time to the
     * seed*/
    ...
    else if (config.command == DAEMON_RUN)
    {
        ...
        if (sd_listen_fds(0) <= 0)
            if (daemon_close_all(-1) < 0)
                avahi_log_warn(
                    "Failed to close all remaining file descriptors: %s",
                    strerror(errno));
