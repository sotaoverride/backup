...
static inline void double_block(unsigned char* b)
{
#if OCB_BLOCK_LEN != 16
...
static void double_block_cpy(unsigned char* d, const unsigned char* s)
{
    if (d != s)
...
static void bit_copy(unsigned char* d, const unsigned char* s,
...
static void ocb_get_L_big(gcry_cipher_hd_t c, u64 n, unsigned char* l_buf)
{
...
/* Set the nonce for OCB.  This requires that the key has been set.
   ...
   key.  */
gcry_err_code_t _gcry_cipher_ocb_set_nonce(gcry_cipher_hd_t c,
                                           ...
                                           size_t noncelen)
{
    ...
    /* 120 bit is the allowed maximum.  In addition we impose a minimum
       of 64 bit.  */
    if (noncelen > (120 / 8) || noncelen < (64 / 8) ||
        noncelen >= OCB_BLOCK_LEN)
...
/* Process additional authentication data.  This implementation allows
   ...
   gcry_cipher_gettag.  */
gcry_err_code_t _gcry_cipher_ocb_authenticate(gcry_cipher_hd_t c,
                                              ...
                                              size_t abuflen)
{
    ...
    if (c->u_mode.ocb.aad_nleftover)
    {
        ...
        if (c->u_mode.ocb.aad_nleftover == OCB_BLOCK_LEN)
        {
            ...
            buf_xor(l_tmp, c->u_mode.ocb.aad_offset, c->u_mode.ocb.aad_leftover,
                    OCB_BLOCK_LEN);
    ...
    while (abuflen >= OCB_BLOCK_LEN)
    {
        ...
        while (nblks)
        {
            ...
            buf_xor_1(c->u_mode.ocb.aad_offset,
                      ocb_get_l(c, c->u_mode.ocb.aad_nblocks), OCB_BLOCK_LEN);
            /* Sum_i = Sum_{i-1} xor ENCIPHER(K, A_i xor Offset_i)  */
            buf_xor(l_tmp, c->u_mode.ocb.aad_offset, abuf, OCB_BLOCK_LEN);
            nburn = c->spec->encrypt(&c->context.c, l_tmp, l_tmp);
            burn = nburn > burn ? nburn : burn;
...
static void ocb_aad_finalize(gcry_cipher_hd_t c)
{
    unsigned char l_tmp[OCB_BLOCK_LEN];
    unsigned int burn = 0;
    unsigned int nburn;
    ...
    if (c->u_mode.ocb.aad_nleftover)
    {
        ...
        buf_xor_1(c->u_mode.ocb.aad_offset, c->u_mode.ocb.L_star,
                  OCB_BLOCK_LEN);
...
static gcry_err_code_t ocb_crypt(gcry_cipher_hd_t c, int encrypt,
                                 ...
                                 const unsigned char* inbuf, size_t inbuflen)
{
    ...
    while (inbuflen >= OCB_BLOCK_LEN)
    {
        ...
        if (nblks)
        {
            ...
            while (nblks)
            {
                ...
                /* Offset_i = Offset_{i-1} xor L_{ntz(i)} */
                buf_xor_1(c->u_iv.iv, ocb_get_l(c, c->u_mode.ocb.data_nblocks),
                          OCB_BLOCK_LEN);
                /* C_i = Offset_i xor ENCIPHER(K, P_i xor Offset_i)  */
                buf_xor(outbuf, c->u_iv.iv, inbuf, OCB_BLOCK_LEN);
                nburn = crypt_fn(&c->context.c, outbuf, outbuf);
    ...
    if (c->marks.finalize)
    {
        ...
        nburn = c->spec->encrypt(&c->context.c, c->u_mode.ocb.tag,
                                 c->u_mode.ocb.tag);
        burn = nburn > burn ? nburn : burn;
...
gcry_err_code_t _gcry_cipher_ocb_encrypt(gcry_cipher_hd_t c,
...
/* Decrypt (INBUF,INBUFLEN) in OCB mode to OUTBUF.  OUTBUFLEN gives
   ...
   of a full block unless gcry_cipher_final has been called in which
   case the next block may have any length.  */
gcry_err_code_t _gcry_cipher_ocb_decrypt(gcry_cipher_hd_t c,
                                         unsigned char* outbuf,
                                         size_t outbuflen,
                                         const unsigned char* inbuf,
                                         size_t inbuflen)
...
static void compute_tag_if_needed(gcry_cipher_hd_t c)
{
    if (!c->marks.tag)
    {
        ocb_aad_finalize(c);
...
gcry_err_code_t _gcry_cipher_ocb_get_tag(gcry_cipher_hd_t c,
...
/* Check that the tag (INTAG,TAGLEN) matches the computed tag for the
   handle C.  */
gcry_err_code_t _gcry_cipher_ocb_check_tag(gcry_cipher_hd_t c,
