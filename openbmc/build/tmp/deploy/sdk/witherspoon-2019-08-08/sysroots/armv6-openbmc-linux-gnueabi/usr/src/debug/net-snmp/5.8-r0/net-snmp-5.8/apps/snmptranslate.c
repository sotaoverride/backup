...
void usage(void)
{
    ...
    fprintf(stderr, "  -m MIB[" ENV_SEPARATOR
                    "...]\t\tload given list of MIBs (ALL loads everything)\n");
    fprintf(stderr, "  -M DIR[" ENV_SEPARATOR
    ...
    fprintf(stderr, "  -O OUTOPTS\t\tToggle various defaults controlling "
                    "output display:\n");
...
void process_stdin(void)
{
    ...
    while (NULL != fgets(buf, sizeof(buf), stdin))
    {
         ...
         * a hex dump.
         */
        if (!(rest && strlen(buf) <= 5) && read_objid(buf, name, &name_length))
        {
...
int main(int argc, char* argv[])
{
    ...
    do
    {
        ...
        {
            if (0 == show_all_matched_objects(stdout, current_name, name,
                                              &name_length, description, width))
            {
                fprintf(
                    stderr,
...
int show_all_matched_objects(
    FILE* fp, const char* patmatch, oid* name, size_t* name_length,
