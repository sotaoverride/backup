...
static void g_local_file_io_stream_finalize(GObject* object)
{
    ...
    _g_local_file_io_stream_new(GLocalFileOutputStream* output_stream)
{
    ...
    _g_local_file_input_stream_set_do_close(
        G_LOCAL_FILE_INPUT_STREAM(stream->input_stream), FALSE);

    return G_FILE_IO_STREAM(stream);
}

static GInputStream* g_local_file_io_stream_get_input_stream(GIOStream* stream)
{
...
static gboolean g_local_file_io_stream_close(GIOStream* stream,
                                             ...
                                             GError** error)
{
    ...
    return _g_local_file_output_stream_really_close(
        G_LOCAL_FILE_OUTPUT_STREAM(file->output_stream), cancellable, error);
}

static void g_local_file_io_stream_class_init(GLocalFileIOStreamClass* klass)
{
...
static void g_local_file_io_stream_init(GLocalFileIOStream* stream)
{
}
