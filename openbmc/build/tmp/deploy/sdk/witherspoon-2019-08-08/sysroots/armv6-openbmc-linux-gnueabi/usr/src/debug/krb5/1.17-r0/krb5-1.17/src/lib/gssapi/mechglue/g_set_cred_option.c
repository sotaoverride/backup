...
static OM_uint32 alloc_union_cred(OM_uint32* minor_status, gss_mechanism mech,
                                  ...
                                  gss_union_cred_t* pcred)
{
    ...
    *pcred = NULL;
    ...
    status = generic_gss_copy_oid(minor_status, &mech->mech_type,
                                  &cred->mechs_array);
                                            ...
                                            const gss_buffer_t value)
{
    ...
    *minor_status = 0;
    ...
    if (*cred_handle == GSS_C_NO_CREDENTIAL)
    {
        ...
        status = mech->gssspi_set_cred_option(minor_status, &mech_cred,
        ...
        {
            status =
                alloc_union_cred(minor_status, mech, mech_cred, &union_cred);
            if (status != GSS_S_COMPLETE)
                return status;
    ...
    else
    {
        ...
        for (i = 0; i < union_cred->count; i++)
        {
            ...
            mech_status = mech->gssspi_set_cred_option(
                &mech_minor_status, &union_cred->cred_array[i], desired_object,
                value);
 ...
 * header.
 */
OM_uint32 KRB5_CALLCONV gssspi_set_cred_option(OM_uint32* minor_status,
                                               gss_cred_id_t cred,
                                               const gss_OID desired_object,
                                               const gss_buffer_t value);
...
OM_uint32 KRB5_CALLCONV gssspi_set_cred_option(OM_uint32* minor_status,
                                               ...
                                               const gss_buffer_t value)
{
    return gss_set_cred_option(minor_status, &cred, desired_object, value);
}
