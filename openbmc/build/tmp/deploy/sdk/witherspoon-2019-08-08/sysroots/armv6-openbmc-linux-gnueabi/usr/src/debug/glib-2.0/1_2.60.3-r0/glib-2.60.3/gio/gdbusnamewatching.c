        ...
        if (client->connection != NULL)
        {
            ...
            if (client->disconnected_signal_handler_id > 0)
                g_signal_handler_disconnect(
                    client->connection, client->disconnected_signal_handler_id);
...
static void actually_do_call(Client* client, GDBusConnection* connection,
                             const gchar* name_owner, CallType call_type)
{
    switch (call_type)
    {
        case CALL_TYPE_NAME_APPEARED:
            ...
            {
                client->name_appeared_handler(connection, client->name,
                                              name_owner, client->user_data);
            }
            break;
        ...
        case CALL_TYPE_NAME_VANISHED:
            ...
            {
                client->name_vanished_handler(connection, client->name,
                                              client->user_data);
            }
            break;
...
static gboolean call_in_idle_cb(gpointer _data)
{
    ...
    actually_do_call(data->client, data->connection, data->name_owner,
                     data->call_type);
    return FALSE;
}
...
static void schedule_call_in_idle(Client* client, CallType call_type)
{
    ...
    data->connection =
        client->connection != NULL ? g_object_ref(client->connection) : NULL;
    data->name_owner = g_strdup(client->name_owner);
    data->call_type = call_type;

    idle_source = g_idle_source_new();
    g_source_set_priority(idle_source, G_PRIORITY_HIGH);
    g_source_set_callback(idle_source, call_in_idle_cb, data,
...
static void do_call(Client* client, CallType call_type)
{
    ...
    if (current_context != client->main_context)
        ...
        actually_do_call(client, client->connection, client->name_owner,
                         call_type);
...
static void call_vanished_handler(Client* client, gboolean ignore_cancelled)
{
    if (client->previous_call != PREVIOUS_CALL_VANISHED)
    {
        ...
        if (((!client->cancelled) || ignore_cancelled) &&
            client->name_vanished_handler != NULL)
        {
...
static Client* dup_client(guint watcher_id)
{
    Client* client;
    ...
    client =
        g_hash_table_lookup(map_id_to_client, GUINT_TO_POINTER(watcher_id));
 ...
 * atomically. */
static void on_connection_disconnected(GDBusConnection* connection,
                                       ...
                                       GError* error, gpointer user_data)
{
    ...
    if (client->name_owner_changed_subscription_id > 0)
        g_dbus_connection_signal_unsubscribe(
            client->connection, client->name_owner_changed_subscription_id);
    if (client->disconnected_signal_handler_id > 0)
        g_signal_handler_disconnect(client->connection,
...
static void get_name_owner_cb(GObject* source_object, GAsyncResult* res,
                              gpointer user_data)
{
...
static void invoke_get_name_owner(Client* client)
{
    g_dbus_connection_call(
        client->connection, "org.freedesktop.DBus", /* bus name */
        "/org/freedesktop/DBus",                    /* object path */
        "org.freedesktop.DBus",                     /* interface name */
        "GetNameOwner",                             /* method name */
        g_variant_new("(s)", client->name), G_VARIANT_TYPE("(s)"),
        G_DBUS_CALL_FLAGS_NONE, -1, NULL,
        (GAsyncReadyCallback)get_name_owner_cb, client_ref(client));
}

/* ----------------------------------------------------------------------------------------------------
 */

static void start_service_by_name_cb(GObject* source_object, GAsyncResult* res,
                                     gpointer user_data)
{
    Client* client = user_data;
    GVariant* result;
    ...
    if (result != NULL)
    {
        ...
        {
            g_warning("Unexpected reply %d from StartServiceByName() method",
...
static void has_connection(Client* client)
{
    ...
    client->disconnected_signal_handler_id = g_signal_connect(
        client->connection, "closed", G_CALLBACK(on_connection_disconnected),
        GUINT_TO_POINTER(client->id));

    /* start listening to NameOwnerChanged messages immediately */
    client->name_owner_changed_subscription_id =
        g_dbus_connection_signal_subscribe(
            client->connection, "org.freedesktop.DBus", /* name */
            "org.freedesktop.DBus",                     /* if */
            "NameOwnerChanged",                         /* signal */
            "/org/freedesktop/DBus",                    /* path */
            client->name, G_DBUS_SIGNAL_FLAGS_NONE, on_name_owner_changed,
            GUINT_TO_POINTER(client->id), NULL);
    ...
    if (client->flags & G_BUS_NAME_WATCHER_FLAGS_AUTO_START)
    {
        g_dbus_connection_call(
            ...
            g_variant_new("(su)", client->name, 0), G_VARIANT_TYPE("(u)"),
            G_DBUS_CALL_FLAGS_NONE, -1, NULL,
            (GAsyncReadyCallback)start_service_by_name_cb, client_ref(client));
    }
    else
    {
        /* check owner */
...
static void connection_get_cb(GObject* source_object, GAsyncResult* res,
                              gpointer user_data)
{
    Client* client = user_data;
 ...
 * @name_appeared_handler: (nullable): Handler to invoke when @name is known to
 * exist or %NULL.
 * @name_vanished_handler: (nullable): Handler to invoke when @name is known to
 * not exist or %NULL.
                       ...
                       gpointer user_data, GDestroyNotify user_data_free_func)
{
    ...
    client->ref_count = 1;
    client->id =
        g_atomic_int_add(&next_global_id, 1); /* TODO: uh oh, handle overflow */
...
guint g_bus_watch_name_on_connection(
    GDBusConnection* connection, const gchar* name, GBusNameWatcherFlags flags,
    GBusNameAppearedCallback name_appeared_handler,
    GBusNameVanishedCallback name_vanished_handler, gpointer user_data,
    GDestroyNotify user_data_free_func)
{
    Client* client;
...
static WatchNameData* watch_name_data_new(GClosure* name_appeared_closure,
                                          GClosure* name_vanished_closure)
{
    WatchNameData* data;
    ...
    if (name_appeared_closure != NULL)
    {
        ...
        if (G_CLOSURE_NEEDS_MARSHAL(name_appeared_closure))
            g_closure_set_marshal(name_appeared_closure,
    ...
    if (name_vanished_closure != NULL)
    {
        ...
        if (G_CLOSURE_NEEDS_MARSHAL(name_vanished_closure))
            g_closure_set_marshal(name_vanished_closure,
                                  g_cclosure_marshal_generic);
    }
...
static void watch_with_closures_on_name_appeared(GDBusConnection* connection,
                                                 const gchar* name,
...
static void watch_with_closures_on_name_vanished(GDBusConnection* connection,
                                                 const gchar* name,
                                                 gpointer user_data)
{
    WatchNameData* data = user_data;
 ...
 * Since: 2.26
 */
guint g_bus_watch_name_with_closures(GBusType bus_type, const gchar* name,
                                     GBusNameWatcherFlags flags,
                                     GClosure* name_appeared_closure,
                                     GClosure* name_vanished_closure)
{
    return g_bus_watch_name(
        bus_type, name, flags,
        name_appeared_closure != NULL ? watch_with_closures_on_name_appeared
                                      : NULL,
        name_vanished_closure != NULL ? watch_with_closures_on_name_vanished
                                      : NULL,
...
/**
 * g_bus_watch_name_on_connection_with_closures: (rename-to
 * g_bus_watch_name_on_connection)
 * @connection: A #GDBusConnection.
 * @name: The name (well-known or unique) to watch.
 * @flags: Flags from the #GBusNameWatcherFlags enumeration.
...
void g_bus_unwatch_name(guint watcher_id)
{
    ...
    G_LOCK(lock);
    if (watcher_id == 0 || map_id_to_client == NULL ||
        (client = g_hash_table_lookup(map_id_to_client,
                                      GUINT_TO_POINTER(watcher_id))) == NULL)
    {
    ...
    g_warn_if_fail(
        g_hash_table_remove(map_id_to_client, GUINT_TO_POINTER(watcher_id)));
