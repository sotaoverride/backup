...
extern int __sgetspent_r(const char* __string, struct spwd* __result_buf,
                         char* __buffer, size_t __buflen,
                         struct spwd** __result) attribute_hidden;
...
#define DECLARE_NSS_PROTOTYPES(service)                                        \
    ...
    extern enum nss_status _nss_##service##_getspent_r(                        \
        struct spwd* pwd, char* buffer, size_t buflen, int* errnop);           \
    extern enum nss_status _nss_##service##_getspnam_r(                        \
        const char* name, struct spwd* pwd, char* buffer, size_t buflen,       \
        int* errnop);

    DECLARE_NSS_PROTOTYPES(compat) DECLARE_NSS_PROTOTYPES(files)
        DECLARE_NSS_PROTOTYPES(hesiod) DECLARE_NSS_PROTOTYPES(nis)
