...
static void g_action_map_default_init(GActionMapInterface* iface)
{
}
                                     ...
                                     gint n_entries, gpointer user_data)
{
    ...
    for (i = 0; n_entries == -1 ? entries[i].name != NULL : i < n_entries; i++)
    {
        ...
        if (entry->state)
        {
            ...
            action = g_simple_action_new_stateful(entry->name, parameter_type,
                                                  state);
        ...
        if (entry->activate != NULL)
            g_signal_connect(action, "activate", G_CALLBACK(entry->activate),
                             user_data);
