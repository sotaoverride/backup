...
int btrfs_subvol_make_fd(int fd, const char* subvolume)
{
    ...
    {
        /* An O_PATH fd was specified, let's convert here to a proper one, as
         * btrfs ioctl's can't deal with O_PATH. */
...
int btrfs_reflink(int infd, int outfd)
{
    ...
    /* Make sure we invoke the ioctl on a regular file, so that no device driver
...
int btrfs_clone_range(int infd, uint64_t in_offset, int outfd,
...
{
    struct btrfs_ioctl_ino_lookup_args args = {.objectid =
...
int btrfs_subvol_get_id(int fd, const char* subvol, uint64_t* ret)
{
    ...
    subvol_fd =
        openat(fd, subvol, O_RDONLY | O_CLOEXEC | O_NOCTTY | O_NOFOLLOW);
    if (subvol_fd < 0)
        return -errno;
...
static void
    btrfs_ioctl_search_args_set(struct btrfs_ioctl_search_args* args,
...
static int
    btrfs_ioctl_search_args_compare(const struct btrfs_ioctl_search_args* args)
{
    int r;
...
#define FOREACH_BTRFS_IOCTL_SEARCH_HEADER(i, sh, args)                         \
    for ((i) = 0, (sh) = (const struct btrfs_ioctl_search_header*)(args).buf;  \
         (i) < (args).key.nr_items;                                            \
         (i)++,                                                                \
        (sh) =                                                                 \
             (const struct                                                     \
              btrfs_ioctl_search_header*)((uint8_t*)(sh) +                     \
                                          sizeof(struct                        \
...
int btrfs_subvol_get_info_fd(int fd, uint64_t subvol_id, BtrfsSubvolInfo* ret)
{
    ...
    while (btrfs_ioctl_search_args_compare(&args) <= 0)
    {
        ...
        FOREACH_BTRFS_IOCTL_SEARCH_HEADER(i, sh, args)
        {
            ...
            if (sh->len < offsetof(struct btrfs_root_item, otime) +
            ...
            memcpy(&ret->parent_uuid, ri->parent_uuid,
                   sizeof(ret->parent_uuid));
...
int btrfs_qgroup_get_quota_fd(int fd, uint64_t qgroupid, BtrfsQuotaInfo* ret)
{
    ...
    while (btrfs_ioctl_search_args_compare(&args) <= 0)
    {
        ...
        FOREACH_BTRFS_IOCTL_SEARCH_HEADER(i, sh, args)
        {
            ...
            {
                const struct btrfs_qgroup_info_item* qii =
            ...
            {
                const struct btrfs_qgroup_limit_item* qli =
                    BTRFS_IOCTL_SEARCH_HEADER_BODY(sh);
...
int btrfs_qgroup_get_quota(const char* path, uint64_t qgroupid,
                           BtrfsQuotaInfo* ret)
{
    _cleanup_close_ int fd = -1;
...
int btrfs_subvol_get_subtree_quota_fd(int fd, uint64_t subvol_id,
                                      BtrfsQuotaInfo* ret)
{
    uint64_t qgroupid;
    int r;
...
int btrfs_subvol_get_subtree_quota(const char* path, uint64_t subvol_id,
                                   BtrfsQuotaInfo* ret)
{
    _cleanup_close_ int fd = -1;
...
int btrfs_qgroup_set_limit_fd(int fd, uint64_t qgroupid,
...
int btrfs_qgroup_set_limit(const char* path, uint64_t qgroupid,
                           uint64_t referenced_max)
{
    _cleanup_close_ int fd = -1;
...
int btrfs_subvol_set_subtree_quota_limit_fd(int fd, uint64_t subvol_id,
                                            uint64_t referenced_max)
{
    uint64_t qgroupid;
    int r;
...
int btrfs_subvol_set_subtree_quota_limit(const char* path, uint64_t subvol_id,
                                         uint64_t referenced_max)
{
    _cleanup_close_ int fd = -1;
    ...
    return btrfs_subvol_set_subtree_quota_limit_fd(fd, subvol_id,
                                                   referenced_max);
}
...
static int qgroup_create_or_destroy(int fd, bool b, uint64_t qgroupid)
{
    ...
    for (c = 0;; c++)
    {
        if (ioctl(fd, BTRFS_IOC_QGROUP_CREATE, &args) < 0)
        {
            ...
            /* If quota is not enabled, we get EINVAL. Turn this into a
             * recognizable error */
            if (errno == EINVAL)
                return -ENOPROTOOPT;
...
static int qgroup_assign_or_unassign(int fd, bool b, uint64_t child,
                                     uint64_t parent)
{
...
static int subvol_remove_children(int fd, const char* subvolume,
                                  uint64_t subvol_id, BtrfsRemoveFlags flags)
{
    struct btrfs_ioctl_search_args args = {
        .key.tree_id = BTRFS_ROOT_TREE_OBJECTID,
    ...
    /* Let's check if this is actually a subvolume. Note that this is mostly
     * redundant, as BTRFS_IOC_SNAP_DESTROY would fail anyway if it is not.
     * However, it's a good thing to check this ahead of time so that we can
     * return ENOTTY unconditionally in this case. This is different from the
     * ioctl() which will return EPERM/EACCES if we don't have the privileges to
     * remove subvolumes, regardless if the specified directory is actually a
     * subvolume or not. In order to make it easy for callers to cover the "this
     * is not a btrfs subvolume" case let's prefer ENOTTY over EPERM/EACCES
     * though. */
    ...
    {
        (void)btrfs_qgroup_destroy_recursive(
            fd, subvol_id); /* for the leaf subvolumes, the qgroup id is
                               identical to the subvol id */
        return 0;
    }
    ...
    while (btrfs_ioctl_search_args_compare(&args) <= 0)
    {
        ...
        FOREACH_BTRFS_IOCTL_SEARCH_HEADER(i, sh, args)
        {
            ...
            p = strndup((char*)ref + sizeof(struct btrfs_root_ref),
                        le64toh(ref->name_len));
            if (!p)
                return -ENOMEM;
            ...
            else
            {
                ...
                child_fd = openat(subvol_fd, ino_args.name,
...
int btrfs_subvol_remove_fd(int fd, const char* subvolume,
                             ...
                             uint64_t new_qgroupid)
{
    ...
    while (btrfs_ioctl_search_args_compare(&args) <= 0)
    {
        ...
        if (ioctl(fd, BTRFS_IOC_TREE_SEARCH, &args) < 0)
        {
            if (errno == ENOENT) /* quota tree missing: quota is not enabled,
                                    hence nothing to copy */
                break;
        ...
        FOREACH_BTRFS_IOCTL_SEARCH_HEADER(i, sh, args)
        {
            ...
            qargs = (struct btrfs_ioctl_qgroup_limit_args){
                ...
                .lim.flags =
                    le64toh(qli->flags) &
                    (BTRFS_QGROUP_LIMIT_MAX_RFER | BTRFS_QGROUP_LIMIT_MAX_EXCL |
...
static int copy_quota_hierarchy(int fd, uint64_t old_subvol_id,
                                uint64_t new_subvol_id)
{
    ...
    else
    {
        n_old_parent_qgroups =
            btrfs_qgroup_find_parents(fd, old_parent_id, &old_parent_qgroups);
    ...
    return btrfs_subvol_auto_qgroup_fd(fd, new_subvol_id,
                                       insert_intermediary_qgroup);
}

static int copy_subtree_quota_limits(int fd, uint64_t old_subvol,
                                     uint64_t new_subvol)
{
...
static int subvol_snapshot_children(int old_fd, int new_fd,
                                    const char* subvolume,
                                    ...
                                    BtrfsSnapshotFlags flags)
{
    ...
    if (!(flags & BTRFS_SNAPSHOT_RECURSIVE) && !(flags & BTRFS_SNAPSHOT_QUOTA))
        return 0;
    ...
    if (!(flags & BTRFS_SNAPSHOT_RECURSIVE))
    {
        ...
        if (flags & BTRFS_SNAPSHOT_QUOTA)
            (void)copy_subtree_quota_limits(new_fd, old_subvol_id,
                                            new_subvol_id);
    ...
    while (btrfs_ioctl_search_args_compare(&args) <= 0)
    {
        ...
        FOREACH_BTRFS_IOCTL_SEARCH_HEADER(i, sh, args)
        {
            ...
            if (flags & BTRFS_SNAPSHOT_READ_ONLY)
            {
                ...
                if (subvolume_fd < 0)
                {
                    subvolume_fd = openat(new_fd, subvolume,
                                          O_RDONLY | O_NOCTTY | O_CLOEXEC |
                                              O_DIRECTORY | O_NOFOLLOW);
                    if (subvolume_fd < 0)
                        return -errno;
            ...
            r = subvol_snapshot_children(old_child_fd, new_child_fd, p,
                                         sh->objectid,
                                         flags & ~BTRFS_SNAPSHOT_FALLBACK_COPY);
...
int btrfs_subvol_snapshot_fd_full(int old_fd, const char* new_path,
                                  BtrfsSnapshotFlags flags,
                                  ...
                                  void* userdata)
{
    ...
    if (r == 0)
    {
        ...
        /* If the source isn't a proper subvolume, fail unless fallback is
         * requested */
        ...
        {
            /* If the destination doesn't support subvolumes, then use a plain
             * directory, if that's requested. */
        ...
        r = copy_directory_fd_full(old_fd, new_path,
                                   COPY_MERGE | COPY_REFLINK | COPY_SAME_MOUNT,
        ...
        if (flags & BTRFS_SNAPSHOT_READ_ONLY)
        {
            ...
            if (plain_directory)
            {
                /* Plain directories have no recursive read-only flag, but
                 * something pretty close to it: the IMMUTABLE bit. Let's use
                 * this here, if this is requested. */

                if (flags & BTRFS_SNAPSHOT_FALLBACK_IMMUTABLE)
                    (void)chattr_path(new_path, FS_IMMUTABLE_FL,
                                      FS_IMMUTABLE_FL, NULL);
            }
            else
            {
...
int btrfs_subvol_snapshot_full(const char* old_path, const char* new_path,
                               BtrfsSnapshotFlags flags,
                               copy_progress_path_t progress_path,
                               copy_progress_bytes_t progress_bytes,
                               void* userdata)
{
    ...
    return btrfs_subvol_snapshot_fd_full(old_fd, new_path, flags, progress_path,
...
int btrfs_subvol_auto_qgroup_fd(int fd, uint64_t subvol_id,
                                bool insert_intermediary_qgroup)
{
    ...
    if (insert_intermediary_qgroup)
    {
        ...
        if (lowest <= 1) /* There are no levels left we could use insert an
                            intermediary qgroup at */
            return -EBUSY;
...
int btrfs_subvol_auto_qgroup(const char* path, uint64_t subvol_id,
                             bool create_intermediary_qgroup)
{
    ...
    return btrfs_subvol_auto_qgroup_fd(fd, subvol_id,
