...
int set_modtime(const char* fname, time_t modtime, uint32 mod_nsec, mode_t mode)
{
    ...
    if (DEBUG_GTE(TIME, 1))
    {
        rprintf(FINFO, "set modtime of %s to (%ld) %s", fname, (long)modtime,
...
int make_path(char* fname, int flags)
{
    ...
    for (p = end;;)
    {
        ...
        if (errno != ENOENT)
        {
            STRUCT_STAT st;
            if (errno != EEXIST ||
                (do_stat(fname, &st) == 0 && !S_ISDIR(st.st_mode)))
                ret = -ret - 1;
            break;
        }
        while (1)
...
int copy_file(const char* source, const char* dest, int ofd, mode_t mode)
{
    ...
    if (ofd < 0)
    {
        ...
        if ((ofd = do_open(dest, O_WRONLY | O_CREAT | O_TRUNC | O_EXCL, mode)) <
            0)
        {
            int save_errno = errno;
...
#define ENSURE_MEMSPACE(buf, type, sz, req)                                    \
    if ((req) > sz &&                                                          \
        !(buf = realloc_array(buf, type, sz = MAX(sz * 2, req))))              \
    ...
    if (strpbrk(arg, "*?["))
    {
        ...
        while ((di = readdir(d)) != NULL)
        {
            ...
            call_glob_match(dname, strlen(dname), 1,
                            slash ? arg + len + 1 : NULL, abpos, fbpos);
        }
        closedir(d);
    }
    else
    {
        call_glob_match(arg, len, 0, slash ? arg + len + 1 : NULL, abpos,
                        fbpos);
    }
    if (slash)
        *slash = '/';
...
int glob_expand(const char* arg, char*** argv_p, int* argc_p, int* maxargs_p)
{
    ...
    else
    {
        ...
        if (!s)
            out_of_memory("glob_expand");
        clean_fname(s, CFN_KEEP_DOT_DIRS | CFN_KEEP_TRAILING_SLASH |
...
void glob_expand_module(char* base1, char* arg, char*** argv_p, int* argc_p,
                        int* maxargs_p)
{
    char *p, *s;
    char* base = base1;
...
#define DOT_IS_DOT_DOT_DIR(bp) (bp[1] == '.' && (bp[2] == '/' || !bp[2]))
    ...
    else if (flags & CFN_REFUSE_DOT_DOT_DIRS && *f == '.' &&
                    ...
                    int flags)
{
    char *start, *sanp;
    int rlen = 0,
        drop_dot_dirs = !relative_paths || !(flags & SP_KEEP_DOT_DIRS);
...
int unsafe_symlink(const char* dest, const char* src)
{
    ...
    for (name = src; (slash = strchr(name, '/')) != 0; name = slash + 1)
    {
            ...
            (name[1] == '/' || (name[1] == '.' && name[2] == '/')))
        {
            ...
            depth++;
        while (slash[1] == '/')
            slash++; /* just in case src isn't clean */
    }
 ...
 **/
int cmp_time(time_t f1_sec, unsigned long f1_nsec, time_t f2_sec,
             unsigned long f2_nsec)
{
    if (f2_sec > f1_sec)
    {
         ...
         */
        if (modify_window <= 0 || (f2_sec > f1_sec + modify_window &&
                                   f1_sec + modify_window > f1_sec))
            return -1;
    }
    else if (f1_sec > f2_sec)
    {
        if (modify_window <= 0 || (f1_sec > f2_sec + modify_window &&
                                   f2_sec + modify_window > f2_sec))
            return 1;
    }
...
int _Insure_trap_error(int a1, int a2, int a3, int a4, int a5, int a6)
{
    ...
    if (asprintf(&cmd,
                 "/usr/X11R6/bin/xterm -display :0 -T Panic -n Panic -e "
                 "/bin/sh -c 'cat /tmp/ierrs.*.%d ; "
                 "gdb /proc/%d/exe %d'",
                 pid_int, pid_int, pid_int) < 0)
        return -1;

    if (!fn)
    {
        static void* h;
        h = dlopen("/usr/local/parasoft/insure++lite/lib.linux2/libinsure.so",
                   RTLD_LAZY);
...
uint32 fuzzy_distance(const char* s1, unsigned len1, const char* s2,
                      unsigned len2)
{
...
#define LOG2(n)                                                                \
    case 1u << n:                                                              \
        return slot * BB_PER_SLOT_BITS + i * 32 + n
                LOG2(0);
                LOG2(1);
                LOG2(2);
                LOG2(3);
 ...
 */
void* expand_item_list(item_list* lp, size_t item_size, const char* desc,
                       int incr)
{
