...
weak_alias(__tzname, tzname) weak_alias(__daylight, daylight)
...
static unsigned int compute_offset(unsigned int ss, unsigned int mm,
                                   unsigned int hh)
{
    if (ss > 59)
        ss = 59;
...
static bool parse_offset(const char** tzp, int whichrule)
{
    ...
    *tzp = tz;
    ...
    unsigned short ss = 0;
    int consumed = 0;
    if (sscanf(tz, "%hu%n:%hu%n:%hu%n", &hh, &consumed, &mm, &consumed, &ss,
               &consumed) > 0)
...
static bool parse_rule(const char** tzp, int whichrule)
{
    ...
    else if (*tz == 'M')
    {
        tzr->type = M;
        int consumed;
        if (sscanf(tz, "M%hu.%hu.%hu%n", &tzr->m, &tzr->n, &tzr->d,
                   &consumed) != 3 ||
...
static void compute_change(tz_rule* rule, int year)
{
    ...
    if (year > 1970)
        t = ((year - 1970) * 365 + /* Compute the number of leapdays between
                                      1970 and YEAR (exclusive).  There is a
                                      leapday every 4th year ...  */
             +((year - 1) / 4 - 1970 / 4)
    ...
    switch (rule->type)
    {
        case J1:
           ...
           years. In non-leap years, or if the day number is 59 or less, just
