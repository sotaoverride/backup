...
struct msg
{
    BEGINobjInstance; /* Data to implement generic object - MUST be the first
                         data element! */
    flowControl_t flowCtlType;
    /**< type of flow control we can apply, for enqueueing, needs not to be
       persisted because once data has entered the queue, this property is no
       longer needed. */
    pthread_mutex_t mut;
    int iRefCount;       /* reference counter (0 = unused) */
    sbool bParseSuccess; /* set to reflect state of last executed higher level
                            parser */
    unsigned short iSeverity; /* the severity  */
    unsigned short iFacility; /* Facility code */
    uint32_t offAfterPRI;     /* offset, at which raw message WITHOUT PRI part
                                 starts in pszRawMsg */
    uint32_t offMSG; /* offset at which the MSG part starts in pszRawMsg */
    short iProtocolVersion; /* protocol version of message received 0 - legacy,
                               1 syslog-protocol) */
    ...
    uchar*
        pszRawMsg; /* message as it was received on the wire. This is important
                    * in case we need to preserve cryptographic verifiers.  */
    uchar* pszHOSTNAME;     /* HOSTNAME from syslog message */
    char* pszRcvdAt3164;    /* time as RFC3164 formatted string (always 15
                               charcters) */
    char* pszRcvdAt3339;    /* time as RFC3164 formatted string (32 charcters at
                               most) */
    ...
    char* pszRcvdAt_PgSQL;  /* rcvdAt as PgSQL formatted string (always 21
                               characters) */
    char* pszTIMESTAMP3164; /* TIMESTAMP as RFC3164 formatted string (always 15
                               charcters) */
    char* pszTIMESTAMP3339; /* TIMESTAMP as RFC3339 formatted string (32
                               charcters at most) */
    char* pszTIMESTAMP_MySQL; /* TIMESTAMP as MySQL formatted string (always 14
                                 charcters) */
    char* pszTIMESTAMP_PgSQL; /* TIMESTAMP as PgSQL formatted string (always 21
                                 characters) */
    ...
    time_t
        ttGenTime; /* time msg object was generated, same as tRcvdAt, but a Unix
              timestamp. While this field looks redundant, it is required
              because a Unix timestamp is used at later processing stages
              (namely in the output arena). Thanks to the subleties of how time
              is defined, there is no reliable way to reconstruct the Unix
              timestamp from the syslogTime fields (in practice, we may be close
              enough to reliable, but I prefer to leave the subtle things to the
              OS, where it obviously is solved in way or another...). */
    struct syslogTime tRcvdAt;    /* time the message entered this program */
    struct syslogTime tTIMESTAMP; /* (parsed) value of the timestamp */
    struct json_object* json;
    struct json_object* localvars;
    /* some fixed-size buffers to save malloc()/free() for frequently used
     * fields (from the default templates) */
    ...
    /* Note: a pointer is 64 bits/8 char, so this is actually fewer than a
     * pointer! */
...
#define NOFLAG 0x000
/* no flag is set (to be used when a flag must be specified and none is
 * required) */
#define INTERNAL_MSG 0x001
/* msg generated by logmsgInternal() --> special handling */
/* 0x002 not used because it was previously a known value - rgerhards,
 * 2008-10-09 */
...
/* fromhost address is unresolved and must be locked up via DNS reverse lookup
 * first */
...
/* rawmsg does not include a PRI (Solaris!), but PRI is already set correctly in
 * the msg object */
...
void MsgSetRcvFromStr(smsg_t* const pMsg, const uchar* pszRcvFrom, const int,
                      prop_t**);
...
void MsgSetHOSTNAME(smsg_t* pMsg, const uchar* pszHOSTNAME,
...
void ATTR_NONNULL()
    MsgSetRawMsg(smsg_t* const pThis, const char* const pszRawMsg,
                 const size_t lenMsg);
rsRetVal MsgReplaceMSG(smsg_t* pThis, const uchar* pszMSG, int lenMSG);
uchar* MsgGetProp(smsg_t* pMsg, struct templateEntry* pTpe,
...
rsRetVal msgAddJSON(smsg_t* pM, uchar* name, struct json_object* json,
                    int force_reset, int sharedReference);
rsRetVal msgAddMetadata(smsg_t* msg, uchar* metaname, uchar* metaval);
rsRetVal msgAddMultiMetadata(smsg_t* msg, const uchar** metaname,
                             const uchar** metaval, const int count);
...
rsRetVal MsgSetPropsViaJSON(smsg_t* __restrict__ const pMsg,
                            const uchar* __restrict__ const json);
...
rsRetVal msgGetJSONPropJSON(smsg_t* pMsg, msgPropDescr_t* pProp,
                            struct json_object** pjson);
rsRetVal msgGetJSONPropJSONorString(smsg_t* const pMsg, msgPropDescr_t* pProp,
                                    struct json_object** pjson, uchar** pcstr);
rsRetVal getJSONPropVal(smsg_t* pMsg, msgPropDescr_t* pProp, uchar** pRes,
                        rs_size_t* buflen, unsigned short* pbMustBeFreed);
rsRetVal msgSetJSONFromVar(smsg_t* pMsg, uchar* varname, struct svar* var,
                           int force_reset);
 ...
 */
/* set raw message size. This is needed in some cases where a trunctation is
 * necessary but the raw message must not be newly set. The most important (and
 * currently only) use case is if we remove trailing LF or NUL characters. Note
 * that the size can NOT be extended, only shrunk! rgerhards, 2009-08-26
 */
