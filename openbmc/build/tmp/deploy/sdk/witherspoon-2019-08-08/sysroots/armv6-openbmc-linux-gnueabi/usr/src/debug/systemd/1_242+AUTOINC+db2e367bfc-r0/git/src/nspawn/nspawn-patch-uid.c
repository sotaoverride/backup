...
static int shift_acl(acl_t acl, uid_t shift, acl_t* ret)
{
    ...
    while (r > 0)
    {
        ...
        if (IN_SET(tag, ACL_USER, ACL_GROUP))
        {
            ...
            /* We don't distuingish here between uid_t and gid_t, let's make
             * sure the compiler checks that this is actually OK */
            ...
            if (modify && !copy)
            {
                ...
                /* There's no copy of the ACL yet? if so, let's create one, and
                 * start the loop from the beginning, so that we copy all
                 * entries, starting from the first, this time. */
...
static int patch_acls(int fd, const char* name, const struct stat* st,
                      uid_t shift)
{
...
static int patch_fd(int fd, const char* name, const struct stat* st,
                    uid_t shift)
{
    uid_t new_uid;
    gid_t new_gid;
    ...
    if (st->st_uid != new_uid || st->st_gid != new_gid)
    {
        ...
        /* The Linux kernel alters the mode in some cases of chown(). Let's undo
         * this. */
        if (name)
        {
            if (!S_ISLNK(st->st_mode))
...
static int recurse_fd(int fd, bool donate_fd, const struct stat* st,
                      uid_t shift, bool is_toplevel)
{
    ...
    /* We generally want to permit crossing of mount boundaries when patching
     * the UIDs/GIDs. However, we probably shouldn't do this for /proc and /sys
     * if that is already mounted into place. Hence, let's stop the recursion
    ...
    /* Also, if we hit a read-only file system, then don't bother, skip the
     * whole subtree */
    ...
    if (S_ISDIR(st->st_mode))
    {
        ...
        FOREACH_DIRENT_ALL(de, d, r = -errno; goto finish)
        {
            ...
            if (S_ISDIR(fst.st_mode))
            {
                ...
                subdir_fd = openat(dirfd(d), de->d_name,
    ...
    /* After we descended, also patch the directory itself. It's key to do this
     * in this order so that the top-level directory is patched as very last
     * object in the tree, so that we can use it as quick indicator whether the
...
static int fd_patch_uid_internal(int fd, bool donate_fd, uid_t shift,
                                 uid_t range)
{
    struct stat st;
    int r;
    ...
    /* Recursively adjusts the UID/GIDs of all files of a directory tree. This
     * is used to automatically fix up an OS tree to the used user namespace UID
     * range. Note that this automatic adjustment only works for UID ranges
     * following the concept that the upper 16bit of a UID identify the
     * container, and the lower 16bit are the actual UID within the container.
     */
    ...
    if (range != 0x10000)
    {
        /* We only support containers with 16bit UID ranges for the patching
         * logic */
        r = -EOPNOTSUPP;
        goto finish;
    }
    ...
    /* Try to detect if the range is already right. Of course, this a pretty
     * drastic optimization, as we assume that if the top-level dir has the
     * right upper 16bit assigned, then everything below will have too... */
    if (((uint32_t)(st.st_uid ^ shift) >> 16) == 0)
        return 0;

    /* Before we start recursively chowning, mark the top-level dir as "busy" by
     * chowning it to the "busy" range. Should we be interrupted in the middle
     * of our work, we'll see it owned by this user and will start
     * chown()ing it again, unconditionally, as the busy UID is not a valid UID
     * we'd everpick for ourselves. */

    if ((st.st_uid & UID_BUSY_MASK) != UID_BUSY_BASE)
    {
        if (fchown(fd, UID_BUSY_BASE | (st.st_uid & ~UID_BUSY_MASK),
                   (gid_t)UID_BUSY_BASE | (st.st_gid & ~(gid_t)UID_BUSY_MASK)) <
            0)
        {
            r = -errno;
            goto finish;
