...
static int IheaderHostToBigEndian(char* pHeader, int nBitType)
{
...
static int IheaderBigEndianToHost(char* pHeader, int nBitType)
{
    if (nBitType == en_is64 && __BYTE_ORDER == __LITTLE_ENDIAN)
    {
        struct pi_header64* pHeader64 = (struct pi_header64*)pHeader;
...
static int HwmsHostToBigEndian(char* pHwms, int nLen, int nBitType)
{
    int i = 0;
    ...
    for (i = 0; i < nLen; i += 8)
    {
        printf("hwms%s: %02X %02X %02X %02X %02X %02X %02X %02X\n",
               ...
               pHwms[i + 1] & 0xFF, pHwms[i + 2] & 0xFF, pHwms[i + 3] & 0xFF,
               pHwms[i + 4] & 0xFF, pHwms[i + 5] & 0xFF, pHwms[i + 6] & 0xFF,
               pHwms[i + 7] & 0xFF);
    }
#endif
...
static int HwmsBigEndianToHost(char* pHwms, int nLen, int nBitType)
{
    int i = 0;
    ...
    else
    {
        ...
        if (pdesc.flags & PFOR_USEHWMS)
        {
            ...
            if (use64)
            {
                ...
                HwmsBigEndianToHost((char*)pdesc64.hwms, sizeof(pdesc64.hwms),
                                    en_is64);
...
char* string;
{
    ...
    if ((pwp->flags & PFOR_FLUSH) || !(pwp->count % NUMWORDS))
    {
        ...
        uint32_t tmpdatum =
            (__BYTE_ORDER == __LITTLE_ENDIAN) ? bswap_32(datum) : datum;
...
uint32_t number;
{
    ...
    if (_PWIsBroken64(pwp->ifp))
    {
        uint64_t datum64;
        if (fseek(pwp->ifp,
                  sizeof(struct pi_header64) + (thisblock * sizeof(uint64_t)),
                  0))
        {
        ...
        datum64 =
            (__BYTE_ORDER == __LITTLE_ENDIAN) ? bswap_64(datum64) : datum64;
        datum = datum64;
    }
    else
    {
        if (fseek(pwp->ifp,
                  sizeof(struct pi_header) + (thisblock * sizeof(uint32_t)), 0))
        {
            perror("(index fseek failed)");
            return NULL;
        }
