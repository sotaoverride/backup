                        ...
                        const char** argv)
{
    ...
    for (i = 0; i < argc; i++)
    {
        ...
        else if (!strcmp(mybuf, "file"))
        {
            ...
            { /* These items are related to the user, but are not
    ...
    if (!citem)
    {
        pam_syslog(pamh, LOG_ERR, "Unknown item or item not specified");
        free(ifname);
        return onerr;
    }
    else if (!ifname)
    ...
    {
        pam_syslog(pamh, LOG_ERR, "Unknown sense or sense not specified");
        free(ifname);
        return onerr;
    }
    else if ((apply_type == APPLY_TYPE_NONE) ||
             ((apply_type != APPLY_TYPE_NULL) && (*apply_val == '\0')))
    {
        pam_syslog(pamh, LOG_ERR, "Invalid usage for apply= parameter");
        free(ifname);
        return onerr;
    }
    ...
    if (apply_type != APPLY_TYPE_NULL)
    {
        ...
        {
            pam_syslog(pamh, LOG_WARNING, "Non-sense use for apply= parameter");
            apply_type = APPLY_TYPE_NULL;
        }
    ...
    /* Short-circuit - test if this session apply for this user */
    {
        ...
        if ((rval == PAM_SUCCESS) && user_name && user_name[0])
        {
            ...
            if (apply_type == APPLY_TYPE_USER)
            {
                if (strcmp(user_name, apply_val))
                {
                    ...
                    pam_syslog(pamh, LOG_DEBUG,
                               "don't apply: apply=%s, user=%s", apply_val,
                               user_name);
#endif /* PAM_DEBUG */
                    free(ifname);
                    return PAM_IGNORE;
            ...
            {
                if (!pam_modutil_user_in_group_nam_nam(pamh, user_name,
                                                       apply_val))
                {
    ...
    if (extitem)
    {
        switch (extitem)
        {
            ...
            case EI_SHELL:
                ...
                if (userinfo == NULL)
                {
                    pam_syslog(pamh, LOG_ERR, "getpwnam(%s) failed", citemp);
                    free(ifname);
                    return onerr;
                }
    ...
    pam_syslog(pamh, LOG_INFO,
               ...
               "Got file = %s, item = %d, value = %s, sense = %d", ifname,
               citem, citemp, sense);
#endif
    ...
    if ((fileinfo.st_mode & S_IWOTH) || !S_ISREG(fileinfo.st_mode))
    {
        ...
        pam_syslog(pamh, LOG_ERR,
                   "%s is either world writable or not a normal file", ifname);
        free(ifname);
        return PAM_AUTH_ERR;
    }
...
#endif
    while ((fgets(aline, sizeof(aline), inf) != NULL) && retval)
    {
        char* a = aline;
    ...
    {
#ifdef PAM_DEBUG
        pam_syslog(pamh, LOG_INFO, "Returning PAM_SUCCESS, retval = %d",
                   retval);
#endif
        return PAM_SUCCESS;
    }
    else
    {
        ...
        const char* user_name;
#ifdef PAM_DEBUG
        pam_syslog(pamh, LOG_INFO, "Returning PAM_AUTH_ERR, retval = %d",
                   retval);
#endif
