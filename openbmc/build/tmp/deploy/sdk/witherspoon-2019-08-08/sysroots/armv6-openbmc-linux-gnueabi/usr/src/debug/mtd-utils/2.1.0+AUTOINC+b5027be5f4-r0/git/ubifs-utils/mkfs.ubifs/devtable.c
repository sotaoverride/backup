...
static int separate_last(const char* buf, int len, char** path, char** name)
{
    ...
    *path = malloc(path_len + 1);
    if (!*path)
        return err_msg("cannot allocate %d bytes of memory", path_len + 1);
    ...
    *name = malloc(name_len + 1);
    ...
    {
        free(*path);
        return err_msg("cannot allocate %d bytes of memory", name_len + 1);
    }
...
static int interpret_table_entry(const char* line)
{
    ...
    if (sscanf(line, "%1023s %c %o %u %u %u %u %u %u %u", buf, &type, &mode,
               &uid, &gid, &major, &minor, &start, &increment, &count) < 0)
        return sys_err_msg("sscanf failed");

    dbg_msg(3,
            "name %s, type %c, mode %o, uid %u, gid %u, major %u, "
            "minor %u, start %u, inc %u, cnt %u",
            buf, type, mode, uid, gid, major, minor, start, increment, count);
    ...
    if (!ph_elt)
    {
        ...
        ph_elt->path = path;
        path = NULL;
        ph_elt->name_htbl = create_hashtable(128, &r5_hash, &is_equivalent);
        if (!ph_elt->name_htbl)
        {
    ...
    else
    {
        ...
        for (i = start; i < num; i++)
        {
            ...
            dbg_msg(3,
                    "inserting '%s' into name hash table (major %d, minor %d)",
