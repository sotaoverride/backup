    ...
    if (len > 0)
    {
        ...
        if (verbose)
            BIO_printf(bio_err,
                       "Creating\n user=\"%s\"\n g=\"%s\"\n N=\"%s\"\n", user,
                       g, N);
        if ((gNid = SRP_create_verifier(user, password, &salt, srp_verifier, N,
                                        g)) == NULL)
        {
...
int srp_main(int argc, char** argv)
{
    ...
    if (mode == OPT_ERR)
    {
        BIO_printf(bio_err, "Exactly one of the options -add, -delete, -modify "
    ...
    if (srpvfile == NULL)
    {
        ...
        {
            if (verbose)
                BIO_printf(bio_err, "trying to read " ENV_DEFAULT_SRP
                                    " in " BASE_SECTION "\n");
    ...
    while (mode == OPT_LIST || user != NULL)
    {
        ...
        else if (mode == OPT_ADD)
        {
            ...
            else
            {
                ...
                row[DB_srpsalt] = NULL;
                row[DB_srpinfo] = NULL;
                if (!(gNid = srp_create_user(
                          user, &(row[DB_srpverifier]), &(row[DB_srpsalt]),
                          gNrow ? gNrow[DB_srpsalt] : gN,
                          gNrow ? gNrow[DB_srpverifier] : NULL, passout,
                          verbose)))
                {
                    BIO_printf(bio_err,
                ...
                if ((row[DB_srpid] == NULL) || (row[DB_srpgN] == NULL) ||
                    (row[DB_srptype] == NULL) ||
                    (row[DB_srpverifier] == NULL) ||
                    (row[DB_srpsalt] == NULL) ||
                    (userinfo &&
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (row[DB_srptype][0] == 'V')
                    {
                        ...
                        if (verbose)
                            BIO_printf(bio_err,
                                       ...
                                       user);
                        if ((user_gN = get_index(db, row[DB_srpgN],
                                                 DB_SRP_INDEX)) >= 0)
                    ...
                    if (row[DB_srpid] == NULL || row[DB_srpgN] == NULL ||
                        ...
                        row[DB_srpverifier] == NULL ||
                        row[DB_srpsalt] == NULL ||
                        (userinfo && ((row[DB_srpinfo] =
                                           OPENSSL_strdup(userinfo)) == NULL)))
                        goto end;
