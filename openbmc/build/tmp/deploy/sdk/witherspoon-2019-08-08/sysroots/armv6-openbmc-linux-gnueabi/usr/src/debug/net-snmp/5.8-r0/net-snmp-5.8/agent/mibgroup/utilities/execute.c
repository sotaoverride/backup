...
int run_shell_command(char* command, char* input, char* output,
                      int* out_len) /* Or realloc style ? */
{
#if HAVE_SYSTEM
 ...
 */
char** tokenize_exec_command(char* command, int* argc)
{
    char ctmp[STRMAX];
    char* cp;
    char** argv;
...
int run_exec_command(char* command, char* input, char* output,
                     int* out_len) /* Or realloc style ? */
{
#if HAVE_EXECV
    int ipipe[2];
    ...
    else if (pid > 0)
    {
        ...
        {
            DEBUGMSGTL(
                ("run:exec", "invalid params; no output will be returned\n"));
            output = NULL;
        }
        if (output)
        {
        ...
        for (; i; --i)
        {
            ...
            DEBUGMSGTL(("verbose:run:exec", "    read %d bytes\n", (int)count));
            if (0 == count)
            {
                int rc;
                /*
                ...
                if ((rc = waitpid(pid, &result, WNOHANG)) <= 0)
                {
                    if (rc < 0)
                    {
                        ...
                        DEBUGMSGTL(
                            ("verbose:run:exec", "      child not done!?!\n"));
                }
                else
                {
            ...
            else if (count > 0)
            {
                ...
                if (output)
                {
                    ...
                    if (cache_size <= 0)
                    {
                        DEBUGMSGTL(("verbose:run:exec", "      output full\n"));
                        break;
                    }
                    DEBUGMSGTL(("verbose:run:exec", "    %d left in buffer\n",
                                (int)cache_size));
                }
            }
        ...
        DEBUGMSGTL(
            ("run:exec", "  child %d finished. result=%d\n", pid, result));
