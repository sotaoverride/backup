...
{
    if (group >= fs->group_desc_count || !ext2fs_has_group_desc_csum(fs) ||
                           ...
                           ext2fs_inode_bitmap map, ext2_ino_t* ret)
{
    ...
    do
    {
        ...
        retval =
            ext2fs_find_first_zero_inode_bitmap2(map, i, upto, &first_zero);
        if (retval == 0)
        {
            i = first_zero;
            break;
                            ...
                            struct blk_alloc_ctx* ctx)
{
    ...
    errcode_t (*gab2)(ext2_filsys, blk64_t, blk64_t*, struct blk_alloc_ctx*);
    ...
    retval = ext2fs_find_first_zero_block_bitmap2(
        map, goal, ext2fs_blocks_count(fs->super) - 1, &b);
    if ((retval == ENOENT) && (goal != fs->super->s_first_data_block))
        retval = ext2fs_find_first_zero_block_bitmap2(
            map, fs->super->s_first_data_block, goal - 1, &b);
allocated:
    if (retval == ENOENT)
...
errcode_t ext2fs_new_block(ext2_filsys fs, blk_t goal, ext2fs_block_bitmap map,
                           blk_t* ret)
{
    errcode_t retval;
    blk64_t val;
...
errcode_t ext2fs_alloc_block2(ext2_filsys fs, blk64_t goal, char* block_buf,
                              blk64_t* ret)
{
    return ext2fs_alloc_block3(fs, goal, block_buf, ret, NULL);
}

errcode_t ext2fs_alloc_block(ext2_filsys fs, blk_t goal, char* block_buf,
                             blk_t* ret)
{
    errcode_t retval;
...
errcode_t ext2fs_get_free_blocks2(ext2_filsys fs, blk64_t start, blk64_t finish,
                                  int num, ext2fs_block_bitmap map,
                                  blk64_t* ret)
{
    blk64_t b = start;
    int c_ratio;
...
void ext2fs_set_alloc_block_callback(
    ext2_filsys fs,
                           ...
                           blk64_t* plen)
{
    ...
    errcode_t (*nrf)(ext2_filsys fs, int flags, blk64_t goal, blk64_t len,
    ...
    while (!looped || start <= goal)
    {
        ...
        retval = ext2fs_find_first_set_block_bitmap2(map, start, b, &end);
        if (retval == ENOENT)
            end = b + 1;
        else if (retval)
            goto errout;

        if (!(flags & EXT2_NEWRANGE_MIN_LENGTH) || (end - start) >= len)
        {
            /* Success! */
            *pblk = start;
            *plen = end - start;
            dbg_printf("%s: new_range goal=%llu--%llu "
                       "blk=%llu--%llu %llu\n",
                       __func__, goal, goal + len - 1, *pblk, *pblk + *plen - 1,
                       *plen);
        allocated:
            for (b = start; b < end; b += fs->super->s_blocks_per_group)
                ext2fs_clear_block_uninit(fs, ext2fs_group_of_blk2(fs, b));
            return 0;
        }
