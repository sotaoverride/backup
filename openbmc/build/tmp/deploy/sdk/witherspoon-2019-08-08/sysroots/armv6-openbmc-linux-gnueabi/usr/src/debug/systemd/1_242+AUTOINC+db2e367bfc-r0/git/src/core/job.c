...
void job_uninstall(Job* j)
{
    ...
    *pj = NULL;
    ...
    unit_add_to_dbus_queue(
        j->unit); /* The Job property of the unit has changed now */
...
static void job_merge_into_installed(Job* j, Job* other)
{
    ...
    if (j->type != JOB_NOP)
        assert_se(job_type_merge_and_collapse(&j->type, other->type, j->unit) ==
                  0);
    else
...
Job* job_install(Job* j)
{
    ...
    if (uj)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                /* XXX It should be safer to queue j to run after uj finishes,
                 * but it is not currently possible to have more than one
                 * installed job per unit. */
                job_merge_into_installed(uj, j);
                log_unit_debug(
                    uj->unit,
    ...
    *pj = j;
    ...
    log_unit_debug(j->unit, "Installed new job %s/%s as %u", j->unit->id,
                   job_type_to_string(j->type), (unsigned)j->id);
...
int job_install_deserialized(Job* j)
{
    ...
    if (*pj)
        return log_unit_debug_errno(j->unit, SYNTHETIC_ERRNO(EEXIST),
                                    "Unit already has a job installed. Not "
                                    "installing deserialized job.");

    r = hashmap_put(j->manager->jobs, UINT32_TO_PTR(j->id), j);
    if (r == -EEXIST)
        return log_unit_debug_errno(
            j->unit, r,
            "Job ID %" PRIu32 " already used, cannot deserialize job.", j->id);
    if (r < 0)
        return log_unit_debug_errno(
            j->unit, r, "Failed to insert job into jobs hash table: %m");
    ...
    *pj = j;
    ...
    log_unit_debug(j->unit, "Reinstalled deserialized job %s/%s as %u",
...
void job_dump(Job* j, FILE* f, const char* prefix)
{
    ...
    fprintf(f,
            ...
            yes_no(j->irreversible), prefix, yes_no(job_may_gc(j)));
}
...
static const JobType job_merging_table[] = {
    /* What \ With       *  JOB_START         JOB_VERIFY_ACTIVE  JOB_STOP
       JOB_RELOAD */
    /*********************************************************************************/
    /*JOB_START          */
...
static void job_change_type(Job* j, JobType newtype)
{
    ...
    log_unit_debug(j->unit, "Converting job %s/%s -> %s/%s", j->unit->id,
                   job_type_to_string(j->type), j->unit->id,
                   job_type_to_string(newtype));

    j->type = newtype;
}

_pure_ static const char* job_get_begin_status_message_format(Unit* u,
                                                              JobType t)
{
    const char* format;
    ...
    format =
        UNIT_VTABLE(u)->status_message_formats.starting_stopping[t == JOB_STOP];
    if (format)
        return format;
...
static void job_log_begin_status_message(Unit* u, uint32_t job_id, JobType t)
{
     ...
     * name. */
    log_struct(LOG_INFO, LOG_MESSAGE("%s", buf), "JOB_ID=%" PRIu32, job_id,
               "JOB_TYPE=%s", job_type_to_string(t), LOG_UNIT_ID(u),
...
static int job_perform_on_unit(Job** j)
{
     ...
     * did something. Note that this means for units for which there's no
     * 'activating' phase (i.e. because we transition directly from 'inactive'
     * to 'active') we'll possibly skip the "Starting..." message. */
...
int job_run_and_invalidate(Job* j)
{
    ...
    switch (j->type)
    {
        ...
        case JOB_RESTART:
            ...
            /* If the unit type does not support starting/stopping, then simply
             * wait. */
            if (r == -EBADR)
                r = 0;
            break;
    ...
    {
        if (r == -EAGAIN)
            job_set_state(j, JOB_WAITING); /* Hmm, not ready after all, let's
                                              return to JOB_WAITING state */
...
_pure_ static const char* job_get_done_status_message_format(Unit* u, JobType t,
                                                             JobResult result)
{
    ...
    static const char* const
        generic_finished_verify_active_job[_JOB_RESULT_MAX] = {
    ...
    if (IN_SET(t, JOB_START, JOB_STOP, JOB_RESTART))
    {
        format = t == JOB_START
                     ? UNIT_VTABLE(u)
                           ->status_message_formats.finished_start_job[result]
                     : UNIT_VTABLE(u)
                           ->status_message_formats.finished_stop_job[result];
        if (format)
            return format;
    }
...
static void job_print_done_status_message(Unit* u, JobType t, JobResult result)
{
    ...
    /* No message if the job did not actually do anything due to failed
     * condition. */
    ...
    if (log_get_show_color())
        status =
            strjoina(job_print_done_status_messages[result].color,
                     job_print_done_status_messages[result].word, ANSI_NORMAL);
    else
...
static void job_log_done_status_message(Unit* u, uint32_t job_id, JobType t,
                                        JobResult result)
{
    const char *format, *mid;
    char buf[LINE_MAX];
    ...
    /* Skip printing if output goes to the console, and
    ...
    /* Show condition check message if the job did not actually do anything due
     * to failed condition. */
    if (t == JOB_START && result == JOB_DONE && !u->condition_result)
    {
        log_struct(LOG_INFO,
                   "MESSAGE=Condition check resulted in %s being skipped.",
                   unit_description(u), "JOB_ID=%" PRIu32, job_id,
                   "JOB_TYPE=%s", job_type_to_string(t), "JOB_RESULT=%s",
                   job_result_to_string(result), LOG_UNIT_ID(u),
    ...
    switch (t)
    {
        ...
        default:
            log_struct(job_result_log_level[result], LOG_MESSAGE("%s", buf),
                       "JOB_ID=%" PRIu32, job_id, "JOB_TYPE=%s",
    ...
    log_struct(job_result_log_level[result], LOG_MESSAGE("%s", buf),
               "JOB_ID=%" PRIu32, job_id, "JOB_TYPE=%s", job_type_to_string(t),
               "JOB_RESULT=%s", job_result_to_string(result), LOG_UNIT_ID(u),
               LOG_UNIT_INVOCATION_ID(u), mid);
}

static void job_emit_done_status_message(Unit* u, uint32_t job_id, JobType t,
                                         JobResult result)
{
    assert(u);
                              ...
                              bool already)
{
    ...
    log_unit_debug(u, "Job %" PRIu32 " %s/%s finished, result=%s", j->id, u->id,
                   job_type_to_string(t), job_result_to_string(result));

    /* If this job did nothing to respective unit we don't log the status
     * message */
    if (!already)
    ...
    {
        log_struct(LOG_NOTICE, "JOB_TYPE=%s", job_type_to_string(t),
...
static int job_dispatch_timer(sd_event_source* s, uint64_t monotonic,
                              void* userdata)
{
    Job* j = userdata;
    Unit* u;
    ...
    log_unit_warning(j->unit, "Job %s/%s timed out.", j->unit->id,
...
int job_start_timer(Job* j, bool job_running)
{
    ...
    if (job_running)
    {
        ...
        if (j->timer_event_source)
        {
             ...
             */
            r = sd_event_source_get_time(j->timer_event_source,
                                         &old_timeout_time);
            if (r < 0)
                return r;
            ...
            return sd_event_source_set_time(j->timer_event_source,
                                            timeout_time);
        }
    }
    else
    ...
    r = sd_event_add_time(j->manager->event, &j->timer_event_source,
                          CLOCK_MONOTONIC, timeout_time, 0, job_dispatch_timer,
                          j);
    if (r < 0)
        return r;
...
void job_add_to_run_queue(Job* j)
{
    ...
    {
        r = sd_event_source_set_enabled(j->manager->run_queue_event_source,
                                        SD_EVENT_ONESHOT);
        if (r < 0)
            log_warning_errno(
                r, "Failed to enable job run queue event source, ignoring: %m");
    }
...
int job_serialize(Job* j, FILE* f)
{
    ...
    (void)serialize_bool(f, "job-sent-dbus-new-signal",
...
int job_deserialize(Job* j, FILE* f)
{
    ...
    for (;;)
    {
        ...
        else if (streq(l, "job-sent-dbus-new-signal"))
        {
            ...
            b = parse_boolean(v);
            if (b < 0)
                log_debug("Failed to parse job sent_dbus_new_signal flag: %s",
                          v);
            else
...
int job_coldplug(Job* j)
{
    ...
    /* Create timer only when job began or began running and the respective
     * timeout is finite.
    ...
    if (j->begin_running_usec > 0 &&
        j->unit->job_running_timeout != USEC_INFINITY)
        timeout_time =
            MIN(timeout_time,
                usec_add(j->begin_running_usec, j->unit->job_running_timeout));
...
bool job_may_gc(Job* j)
{
    ...
    /* Checks whether this job should be GC'ed away. We only do this for jobs of
     * units that have no effect on their own and just track external state. For
     * now the only unit type that qualifies for this are .device units.
    ...
    /* FIXME: So this is a bit ugly: for now we don't properly track references
     * made via private bus connections (because it's nasty, as sd_bus_track
     * doesn't apply to it). We simply remember that the job was once referenced
     * by one, and reset this whenever we notice that no private bus connections
     * are around. This means
     * the GC is a bit too conservative when it comes to jobs created by private
     * bus connections. */
    ...
    /* If a job is ordered after ours, and is to be started, then it needs to
     * wait for us, regardless if we stop or start, hence let's not GC in that
     * case. */
    HASHMAP_FOREACH_KEY(v, other, j->unit->dependencies[UNIT_BEFORE], i)
    {
        if (!other->job)
    ...
    /* If we are going down, but something else is ordered After= us, then it
     * needs to wait for us */
    ...
    /* The logic above is kinda the inverse of the job_is_runnable() logic.
     * Specifically, if the job "we" is ordered before the job "other":
     *
...
int job_get_before(Job* j, Job*** ret)
{
    ...
    /* Returns a list of all pending jobs that need to finish before this job
     * may be started. */
...
int job_get_after(Job* j, Job*** ret)
{
    ...
    /* Returns a list of all pending jobs that are waiting for this job to
     * finish. */
