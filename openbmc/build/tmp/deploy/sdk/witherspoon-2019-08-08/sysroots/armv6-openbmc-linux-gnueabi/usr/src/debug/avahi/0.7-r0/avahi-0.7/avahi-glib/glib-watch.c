...
{
    return (events & AVAHI_WATCH_IN ? G_IO_IN : 0) |
...
static gboolean dispatch_func(GSource* source,
                              AVAHI_GCC_UNUSED GSourceFunc callback,
                              AVAHI_GCC_UNUSED gpointer userdata)
{
    AvahiGLibPoll* g = (AvahiGLibPoll*)source;
    AvahiWatch* w;
    AvahiTimeout* next_timeout;
    ...
    for (w = g->watches; w; w = w->watches_next)
        ...
        {
            assert(w->callback);
            w->callback(w, w->pollfd.fd,
                        map_events_from_glib(w->pollfd.revents), w->userdata);
            w->pollfd.revents = 0;
            return TRUE;
        }
...
AvahiGLibPoll* avahi_glib_poll_new(GMainContext* context, gint priority)
{
    ...
    static GSourceFuncs source_funcs = {prepare_func, check_func, dispatch_func,
                                        NULL,         NULL,       NULL};

    g = (AvahiGLibPoll*)g_source_new(&source_funcs, sizeof(AvahiGLibPoll));
    g_main_context_ref(g->context =
                           context ? context : g_main_context_default());
