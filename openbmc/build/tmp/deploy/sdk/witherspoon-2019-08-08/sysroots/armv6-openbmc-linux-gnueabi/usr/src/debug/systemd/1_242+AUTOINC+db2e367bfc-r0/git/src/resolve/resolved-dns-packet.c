...
#define INIT_REWINDER(rewinder, p)                                             \
    do                                                                         \
    {                                                                          \
        rewinder.packet = p;                                                   \
        rewinder.saved_rindex = p->rindex;                                     \
    } while (0)
#define CANCEL_REWINDER(rewinder)                                              \
    do                                                                         \
    {                                                                          \
        rewinder.packet = NULL;                                                \
    } while (0)

int dns_packet_new(DnsPacket** ret, DnsProtocol protocol,
                   size_t min_alloc_dsize, size_t max_size)
{
    ...
    /* The caller may not check what is going to be truly allocated, so do not
     * allow to allocate a DNS packet bigger than DNS_PACKET_SIZE_MAX.
     */
    ...
    /* When dns_packet_new() is called with min_alloc_dsize == 0, allocate more
     * than the absolute minimum (which is the dns packet header size), to avoid
...
void dns_packet_set_flags(DnsPacket* p, bool dnssec_checking_disabled,
                          bool truncated)
{
    ...
    switch (p->protocol)
    {
        case DNS_PROTOCOL_LLMNR:
            ...
            h->flags = htobe16(DNS_PACKET_MAKE_FLAGS(
                0 /* qr */, 0 /* opcode */, 0 /* c */, 0 /* tc */, 0 /* t */,
                0 /* ra */, 0 /* ad */, 0 /* cd */, 0 /* rcode */));
            break;

        case DNS_PROTOCOL_MDNS:
            h->flags = htobe16(DNS_PACKET_MAKE_FLAGS(
                0 /* qr */, 0 /* opcode */, 0 /* aa */, truncated /* tc */,
                0 /* rd (ask for recursion) */, 0 /* ra */, 0 /* ad */,
                0 /* cd */, 0 /* rcode */));
            break;
        ...
        default:
            ...
            h->flags = htobe16(DNS_PACKET_MAKE_FLAGS(
                0 /* qr */, 0 /* opcode */, 0 /* aa */, 0 /* tc */,
                1 /* rd (ask for recursion) */, 0 /* ra */, 0 /* ad */,
                dnssec_checking_disabled /* cd */, 0 /* rcode */));
    }
}

int dns_packet_new_query(DnsPacket** ret, DnsProtocol protocol,
                         size_t min_alloc_dsize, bool dnssec_checking_disabled)
{
    DnsPacket* p;
    int r;
     ...
     * If there are multiple packets later, we'll update the bit shortly before
     * sending.
     */
...
int dns_packet_validate_reply(DnsPacket* p)
{
    ...
    switch (p->protocol)
    {
        ...
        case DNS_PROTOCOL_LLMNR:
            /* RFC 4795, Section 2.1.1. says to discard all replies with QDCOUNT
             * != 1 */
...
int dns_packet_validate_query(DnsPacket* p)
{
    ...
    switch (p->protocol)
    {
        ...
        case DNS_PROTOCOL_DNS:
            /* RFC 4795, Section 2.1.1. says to discard all queries with QDCOUNT
             * != 1 */
            if (DNS_PACKET_QDCOUNT(p) != 1)
                return -EBADMSG;

            /* RFC 4795, Section 2.1.1. says to discard all queries with ANCOUNT
             * != 0 */
            if (DNS_PACKET_ANCOUNT(p) > 0)
                return -EBADMSG;

            /* RFC 4795, Section 2.1.1. says to discard all queries with NSCOUNT
             * != 0 */
        ...
        case DNS_PROTOCOL_MDNS:
            /* RFC 6762, Section 18 */
            if (DNS_PACKET_AA(p) != 0 || DNS_PACKET_RD(p) != 0 ||
                DNS_PACKET_RA(p) != 0 || DNS_PACKET_AD(p) != 0 ||
                DNS_PACKET_CD(p) != 0 || DNS_PACKET_RCODE(p) != 0)
                return -EBADMSG;
...
static int dns_packet_extend(DnsPacket* p, size_t add, void** ret,
                             size_t* start)
{
    assert(p);
...
int dns_packet_append_raw_string(DnsPacket* p, const void* s, size_t size,
                                 size_t* start)
{
    void* d;
    int r;
...
int dns_packet_append_label(DnsPacket* p, const char* d, size_t l,
                            bool canonical_candidate, size_t* start)
{
    uint8_t* w;
    int r;
...
int dns_packet_append_name(DnsPacket* p, const char* name,
                           bool allow_compression, bool canonical_candidate,
                           size_t* start)
{
...
int dns_packet_append_key(DnsPacket* p, const DnsResourceKey* k,
                          const DnsAnswerFlags flags, size_t* start)
{
    size_t saved_size;
    uint16_t class;
    int r;
    ...
    class = flags & DNS_ANSWER_CACHE_FLUSH ? k->class | MDNS_RR_CACHE_FLUSH
                                           : k->class;
...
static int dns_packet_append_type_window(DnsPacket* p, uint8_t window,
                                         uint8_t length, const uint8_t* types,
                                         size_t* start)
{
    size_t saved_size;
    int r;
...
static int dns_packet_append_types(DnsPacket* p, Bitmap* types, size_t* start)
{
    ...
    BITMAP_FOREACH(n, types, i)
    {
        ...
        {
            r = dns_packet_append_type_window(p, window, entry / 8 + 1, bitmaps,
                                              NULL);
            if (r < 0)
                goto fail;
...
int dns_packet_append_opt(DnsPacket* p, uint16_t max_udp_size, bool edns0_do,
                          int rcode, size_t* start)
{
    size_t saved_size;
    int r;
    ...
    {
        /* If DO is on and this is not a reply, also append RFC6975 Algorithm
         * data */
...
int dns_packet_append_rr(DnsPacket* p, const DnsResourceRecord* rr,
                         const DnsAnswerFlags flags, size_t* start,
                         size_t* rdata_start)
{
    ...
    switch (rr->unparseable ? _DNS_TYPE_INVALID : rr->key->type)
    {
        ...
        case DNS_TYPE_TXT:
            ...
            else
            {
                ...
                {
                    r = dns_packet_append_raw_string(p, i->data, i->length,
                                                     NULL);
                    if (r < 0)
                        goto fail;
                }
        ...
        case DNS_TYPE_A:
            r = dns_packet_append_blob(p, &rr->a.in_addr,
                                       sizeof(struct in_addr), NULL);
            break;

        case DNS_TYPE_AAAA:
            r = dns_packet_append_blob(p, &rr->aaaa.in6_addr,
        ...
        case DNS_TYPE_DS:
            ...
            r = dns_packet_append_blob(p, rr->ds.digest, rr->ds.digest_size,
                                       NULL);
            break;
        ...
        case DNS_TYPE_SSHFP:
            ...
            r = dns_packet_append_blob(p, rr->sshfp.fingerprint,
                                       rr->sshfp.fingerprint_size, NULL);
            break;
        ...
        case DNS_TYPE_DNSKEY:
            ...
            r = dns_packet_append_blob(p, rr->dnskey.key, rr->dnskey.key_size,
                                       NULL);
            break;
        ...
        case DNS_TYPE_RRSIG:
            ...
            r = dns_packet_append_blob(p, rr->rrsig.signature,
                                       rr->rrsig.signature_size, NULL);
            break;

        case DNS_TYPE_NSEC:
            r = dns_packet_append_name(p, rr->nsec.next_domain_name, false,
                                       false, NULL);
            if (r < 0)
                goto fail;
        ...
        case DNS_TYPE_NSEC3:
            ...
            r = dns_packet_append_blob(p, rr->nsec3.salt, rr->nsec3.salt_size,
                                       NULL);
            if (r < 0)
                goto fail;

            r = dns_packet_append_uint8(p, rr->nsec3.next_hashed_name_size,
                                        NULL);
            if (r < 0)
                goto fail;

            r = dns_packet_append_blob(p, rr->nsec3.next_hashed_name,
                                       rr->nsec3.next_hashed_name_size, NULL);
            if (r < 0)
                goto fail;
        ...
        case DNS_TYPE_TLSA:
            ...
            r = dns_packet_append_blob(p, rr->tlsa.data, rr->tlsa.data_size,
                                       NULL);
            break;
        ...
        case DNS_TYPE_CAA:
            ...
            r = dns_packet_append_blob(p, rr->caa.value, rr->caa.value_size,
                                       NULL);
            break;
        ...
        default:
            ...
            r = dns_packet_append_blob(p, rr->generic.data,
                                       rr->generic.data_size, NULL);
            break;
    }
    if (r < 0)
        goto fail;
...
static int dns_packet_read_memdup(DnsPacket* p, size_t size, void** ret,
                                  size_t* ret_size, size_t* ret_start)
{
...
int dns_packet_read_raw_string(DnsPacket* p, const void** ret, size_t* size,
                               size_t* start)
{
    _cleanup_(rewind_dns_packet) DnsPacketRewinder rewinder;
    uint8_t c;
                         ...
                         size_t* start)
{
    ...
    for (;;)
    {
        ...
        else if (c <= 63)
        {
            ...
            if (!GREEDY_REALLOC(ret, allocated,
                                n + !first + DNS_LABEL_ESCAPED_MAX))
                return -ENOMEM;
...
static int dns_packet_read_type_window(DnsPacket* p, Bitmap** types,
                                       size_t* start)
{
    uint8_t window;
    uint8_t length;
...
static int dns_packet_read_type_windows(DnsPacket* p, Bitmap** types,
...
int dns_packet_read_key(DnsPacket* p, DnsResourceKey** ret,
                        bool* ret_cache_flush, size_t* start)
{
    _cleanup_(rewind_dns_packet) DnsPacketRewinder rewinder;
...
int dns_packet_read_rr(DnsPacket* p, DnsResourceRecord** ret,
                       bool* ret_cache_flush, size_t* start)
{
    ...
    switch (rr->key->type)
    {
        ...
        case DNS_TYPE_TXT:
            ...
            else
            {
                ...
                while (p->rindex < offset + rdlength)
                {
                    ...
                    i = malloc0(offsetof(DnsTxtItem, data) + sz +
        ...
        case DNS_TYPE_A:
            r = dns_packet_read_blob(p, &rr->a.in_addr, sizeof(struct in_addr),
                                     NULL);
            break;

        case DNS_TYPE_AAAA:
            r = dns_packet_read_blob(p, &rr->aaaa.in6_addr,
                                     sizeof(struct in6_addr), NULL);
            break;
        ...
        case DNS_TYPE_DS:
            ...
            r = dns_packet_read_memdup(p, rdlength - 4, &rr->ds.digest,
                                       &rr->ds.digest_size, NULL);
            if (r < 0)
                return r;
        ...
        case DNS_TYPE_SSHFP:
            ...
            r = dns_packet_read_memdup(p, rdlength - 2, &rr->sshfp.fingerprint,
                                       &rr->sshfp.fingerprint_size, NULL);
        ...
        case DNS_TYPE_DNSKEY:
            ...
            r = dns_packet_read_memdup(p, rdlength - 4, &rr->dnskey.key,
                                       &rr->dnskey.key_size, NULL);
        ...
        case DNS_TYPE_NSEC:
        {
            ...
            /*
             * RFC6762, section 18.14 explictly states mDNS should use name
             * compression. This contradicts RFC3845, section 2.1.1
             */

            bool allow_compressed = p->protocol == DNS_PROTOCOL_MDNS;

            r = dns_packet_read_name(p, &rr->nsec.next_domain_name,
                                     allow_compressed, NULL);
            if (r < 0)
                return r;

            r = dns_packet_read_type_windows(
                p, &rr->nsec.types, offset + rdlength - p->rindex, NULL);

            /* We accept empty NSEC bitmaps. The bit indicating the presence of
             * the NSEC record itself is redundant and in e.g., RFC4956 this
             * fact is used to define a use for NSEC records without the NSEC
             * bit set. */
        ...
        case DNS_TYPE_NSEC3:
        {
            ...
            r = dns_packet_read_memdup(p, size, &rr->nsec3.salt,
                                       &rr->nsec3.salt_size, NULL);
            if (r < 0)
                return r;
            ...
            r = dns_packet_read_memdup(p, size, &rr->nsec3.next_hashed_name,
                                       &rr->nsec3.next_hashed_name_size, NULL);
            if (r < 0)
                return r;

            r = dns_packet_read_type_windows(
                p, &rr->nsec3.types, offset + rdlength - p->rindex, NULL);

            /* empty non-terminals can have NSEC3 records, so empty bitmaps are
             * allowed */
        ...
        case DNS_TYPE_TLSA:
            ...
            r = dns_packet_read_memdup(p, rdlength - 3, &rr->tlsa.data,
                                       &rr->tlsa.data_size, NULL);
        ...
        case DNS_TYPE_CAA:
            ...
            r = dns_packet_read_memdup(p, rdlength + offset - p->rindex,
                                       &rr->caa.value, &rr->caa.value_size,
                                       NULL);
        ...
        default:
        unparseable:
            r = dns_packet_read_memdup(p, rdlength, &rr->generic.data,
                                       &rr->generic.data_size, NULL);
...
static bool opt_is_good(DnsResourceRecord* rr, bool* rfc6975)
{
    ...
    /* Checks whether the specified OPT RR is well-formed and whether it
     * contains RFC6975 data (which is not OK in a reply). */
    ...
    {
        *rfc6975 = false;
        return true; /* if it's not version 0, it's OK, but we will ignore the
                        OPT field contents */
    }
...
static int dns_packet_extract_question(DnsPacket* p, DnsQuestion** ret_question)
{
    ...
    if (n > 0)
    {
        ...
        _cleanup_set_free_ Set* keys =
            NULL; /* references to keys are kept by Question */
        ...
        r = set_reserve(keys,
                        n * 2); /* Higher multipliers give slightly higher
                                 * efficiency through hash collisions, but the
                                 * gains quickly drop of after 2. */
        if (r < 0)
            return r;
    ...
    for (i = 0; i < n; i++)
    {
        ...
        if (rr->key->type == DNS_TYPE_OPT)
        {
            ...
            if (p->opt || bad_opt)
            {
                 ...
                 * something wrong with the server, and if one is valid we
            ...
            {
                /* If the OPT RR is not owned by the root domain, then it is
                 * bad, let's ignore it. */
            ...
            {
                /* OPT RR is in the wrong section? Some Belkin routers do this.
                 * This is a hint the EDNS implementation is borked, like the
                 * Belkin one is, hence ignore it. */
        ...
        {
            /* According to RFC 4795, section 2.9. only the RRs from the Answer
             * section shall be cached. Hence mark only those RRs as cacheable
             * by default, but not the ones from the Additional or Authority
             * sections. */
            DnsAnswerFlags flags =
                (i < DNS_PACKET_ANCOUNT(p) ? DNS_ANSWER_CACHEABLE : 0) |
                (p->protocol == DNS_PROTOCOL_MDNS && !cache_flush
        ...
        /* Remember this RR, so that we potentically can merge it's ->key object
         * with the next RR. Note that we only do this if we actually decided to
         * keep the RR around.
         */
...
static int dns_packet_compare_func(const DnsPacket* x, const DnsPacket* y)
{
    ...
    return memcmp(DNS_PACKET_DATA((DnsPacket*)x),
                  DNS_PACKET_DATA((DnsPacket*)y), x->size);
}

DEFINE_HASH_OPS(dns_packet_hash_ops, DnsPacket, dns_packet_hash_func,
                dns_packet_compare_func);

static const char* const dns_rcode_table[_DNS_RCODE_MAX_DEFINED] = {
    [DNS_RCODE_SUCCESS] = "SUCCESS",     [DNS_RCODE_FORMERR] = "FORMERR",
    [DNS_RCODE_SERVFAIL] = "SERVFAIL",   [DNS_RCODE_NXDOMAIN] = "NXDOMAIN",
    [DNS_RCODE_NOTIMP] = "NOTIMP",       [DNS_RCODE_REFUSED] = "REFUSED",
    [DNS_RCODE_YXDOMAIN] = "YXDOMAIN",   [DNS_RCODE_YXRRSET] = "YRRSET",
    [DNS_RCODE_NXRRSET] = "NXRRSET",     [DNS_RCODE_NOTAUTH] = "NOTAUTH",
    [DNS_RCODE_NOTZONE] = "NOTZONE",     [DNS_RCODE_BADVERS] = "BADVERS",
    [DNS_RCODE_BADKEY] = "BADKEY",       [DNS_RCODE_BADTIME] = "BADTIME",
    [DNS_RCODE_BADMODE] = "BADMODE",     [DNS_RCODE_BADNAME] = "BADNAME",
    [DNS_RCODE_BADALG] = "BADALG",       [DNS_RCODE_BADTRUNC] = "BADTRUNC",
