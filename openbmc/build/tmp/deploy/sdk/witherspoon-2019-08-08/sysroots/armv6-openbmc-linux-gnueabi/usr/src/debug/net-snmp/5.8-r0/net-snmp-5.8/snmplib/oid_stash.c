    ...
    netsnmp_oid_stash_node* netsnmp_oid_stash_create_sized_node(size_t mysize)
{
    ...
    if (!ret)
        return NULL;
    ret->children = (netsnmp_oid_stash_node**)calloc(
        mysize, sizeof(netsnmp_oid_stash_node*));
    if (!ret->children)
    {
        free(ret);
        return NULL;
...
netsnmp_feature_child_of(oid_stash_iterate, oid_stash_all)
    ...
    netsnmp_oid_stash_node* netsnmp_oid_stash_getnext_node(
        netsnmp_oid_stash_node* root, oid* lookup, size_t lookup_len)
{
    netsnmp_oid_stash_node *curnode, *tmpp, *loopp;
    ...
    do
    {
        ...
        {
            for (loopp = curnode->children[j]; loopp;
                 loopp = loopp->next_sibling)
            {
...
netsnmp_feature_child_of(oid_stash_store_all, oid_stash_all)
     ...
     */
    int netsnmp_oid_stash_store_all(int majorID, int minorID, void* serverarg,
                                    void* clientarg)
{
        ...
        if (root->children[i])
        {
            for (tmpp = root->children[i]; tmpp; tmpp = tmpp->next_sibling)
            {
                ...
                if (tmpp->thedata)
                {
                    ...
                    *cp++ = ' ';
                    *cp = '\0';
                    if ((*dumpfn)(cp, sizeof(buf) - strlen(buf), tmpp->thedata,
                                  tmpp))
                        read_config_store(appname, buf);
                }
                netsnmp_oid_stash_store(tmpp, tokenname, dumpfn, curoid,
                                        curoid_len + 1);
            }
        }
    }
...
/** For debugging: dump the netsnmp_oid_stash tree to stdout
    ...
    @param prefix a character string prefix printed to the beginning of each
   line.
*/
