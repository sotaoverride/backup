...
static int append_vlan_info_data(Link* const link, sd_netlink_message* req,
                                 uint16_t pvid, const uint32_t* br_vid_bitmap,
                ...
                if (begin == end)
                {
                    ...
                    if (r < 0)
                        return log_link_error_errno(
                            link, r,
                            "Could not append IFLA_BRIDGE_VLAN_INFO attribute: "
                            "%m");
                }
                else
                {
...
static int set_brvlan_handler(sd_netlink* rtnl, sd_netlink_message* m,
                              Link* link)
{
    int r;
...
int br_vlan_configure(Link* link, uint16_t pvid, uint32_t* br_vid_bitmap,
                      uint32_t* br_untagged_bitmap)
{
    ...
    if (!link->network->bridge)
    {
        ...
        sd_netlink_message_append_data(req, IFLA_BRIDGE_FLAGS, &flags,
                                       sizeof(uint16_t));
    }

    /* add vlan info */
    r = append_vlan_info_data(link, req, pvid, br_vid_bitmap,
                              br_untagged_bitmap);
    if (r < 0)
    ...
    else
    {
        if (vid >= vid_end)
        {
            log_syntax(unit, LOG_ERR, filename, line, 0,
                       "Invalid VLAN range, ignoring %s", rvalue);
            return 0;
        }
        for (; vid <= vid_end; vid++)
            set_bit(vid, network->br_vid_bitmap);
                                 ...
                                 void* userdata)
{
    ...
    if (r < 0)
    {
        log_syntax(unit, LOG_ERR, filename, line, r, "Could not parse VLAN: %s",
                   rvalue);
        return 0;
    }
