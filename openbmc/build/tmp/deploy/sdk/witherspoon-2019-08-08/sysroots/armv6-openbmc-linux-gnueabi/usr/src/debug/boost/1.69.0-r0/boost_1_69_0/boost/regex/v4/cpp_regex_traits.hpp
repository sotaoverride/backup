...
template <class charT, class traits>
typename parser_buf<charT, traits>::pos_type
    parser_buf<charT, traits>::seekpos(pos_type sp,
                                       ::std::ios_base::openmode which)
{
    if (which & ::std::ios_base::out)
        return pos_type(off_type(-1));
...
#ifndef BOOST_NO_STD_MESSAGES
    m_pmessages = BOOST_HAS_FACET(std::messages<charT>, l)
                      ? &BOOST_USE_FACET(std::messages<charT>, l)
                      : 0;
#endif
...
void cpp_regex_traits_char_layer<charT>::init()
{
...
#ifndef BOOST_NO_STD_MESSAGES
#ifndef __IBMCPP__
    typename std::messages<charT>::catalog cat =
    ...
    if ((int)cat >= 0)
    {
        ...
        {
#endif
            for (regex_constants::syntax_type i = 1;
                 i < regex_constants::syntax_max; ++i)
            {
                string_type mss =
                    this->m_pmessages->get(cat, 0, i, get_default_message(i));
...
typename cpp_regex_traits_char_layer<charT>::string_type
    cpp_regex_traits_char_layer<charT>::get_default_message(
        regex_constants::syntax_type i)
{
    const char* ptr = get_default_syntax(i);
    string_type result;
    while (ptr && *ptr)
...
//
template <>
class BOOST_REGEX_DECL cpp_regex_traits_char_layer<char>
    : public cpp_regex_traits_base<char>
{
...
template <class charT>
class cpp_regex_traits_implementation
    : public cpp_regex_traits_char_layer<charT>
{
  public:
    typedef typename cpp_regex_traits<charT>::char_class_type char_class_type;
    typedef typename std::ctype<charT>::mask native_mask_type;
    typedef typename boost::make_unsigned<native_mask_type>::type
        unsigned_native_mask_type;
    ...
    std::string error_string(regex_constants::error_type n) const
    {
        if (!m_error_strings.empty())
        {
            std::map<int, std::string>::const_iterator p =
                m_error_strings.find(n);
            return (p == m_error_strings.end())
                       ? std::string(get_default_error_string(n))
                       : p->second;
        }
    ...
    char_class_type lookup_classname(const charT* p1, const charT* p2) const
    {
        ...
        if (result == 0)
        {
            ...
            this->m_pctype->tolower(&*temp.begin(),
                                    &*temp.begin() + temp.size());
            result = lookup_classname_imp(&*temp.begin(),
                                          &*temp.begin() + temp.size());
        }
        return result;
    }
  ...
  private:
    std::map<int, std::string>
        m_error_strings; // error messages indexed by numberic ID
    std::map<string_type, char_class_type>
        m_custom_class_names; // character class names
    std::map<string_type, string_type>
        m_custom_collate_names; // collating element names
    unsigned m_collate_type;    // the form of the collation string
    charT m_collate_delim;      // the collation group delimiter
    //
    // helpers:
    //
    char_class_type lookup_classname_imp(const charT* p1,
                                         const charT* p2) const;
    void init();
#ifdef BOOST_REGEX_BUGGY_CTYPE_FACET
  public:
...
typename cpp_regex_traits_implementation<charT>::string_type
    cpp_regex_traits_implementation<charT>::transform_primary(
        const charT* p1, const charT* p2) const
{
    ...
    try
    {
        ...
        switch (m_collate_type)
        {
            case sort_C:
            case sort_unknown:
                // the best we can do is translate to lower case, then get a
                // regular sort key:
                {
                    result.assign(p1, p2);
                    this->m_pctype->tolower(&*result.begin(),
                                            &*result.begin() + result.size());
                    result = this->m_pcollate->transform(
            ...
            case sort_delim:
                // get a regular sort key, and then truncate everything after
                // the delim:
                result.assign(this->m_pcollate->transform(p1, p2));
                std::size_t i;
...
template <class charT>
typename cpp_regex_traits_implementation<charT>::string_type
    cpp_regex_traits_implementation<charT>::transform(const charT* p1,
                                                      const charT* p2) const
{
    //
    // PRECONDITIONS:
    ...
    while (result.size() && (charT(0) == *result.rbegin()))
...
#endif
        //
        // We may have NULL's used as separators between sections of the collate
        // string, an example would be Boost.Locale.  We have no way to detect
        // this case via #defines since this can be used with any
        // compiler/platform combination. Unfortunately our state machine (which
        // was devised when all implementations used underlying C language
        // API's) can't cope with that case.  One workaround is to replace each
        // character with 2, fortunately this code isn't used that much as this
        ...
        {
            if (static_cast<uchar_type>(result[i]) ==
                (std::numeric_limits<uchar_type>::max)())
            {
                result2
                    .append(1, charT((std::numeric_limits<uchar_type>::max)()))
                    .append(1, charT('b'));
            }
            else
            {
                result2
                    .append(1, static_cast<charT>(
                                   1 + static_cast<uchar_type>(result[i])))
                    .append(1, charT('b') - 1);
            }
        }
        BOOST_ASSERT(std::find(result2.begin(), result2.end(), charT(0)) ==
                     result2.end());
#ifndef BOOST_NO_EXCEPTIONS
    }
    catch (...)
...
template <class charT>
typename cpp_regex_traits_implementation<charT>::string_type
    cpp_regex_traits_implementation<charT>::lookup_collatename(
        const charT* p1, const charT* p2) const
{
    typedef
        typename std::map<string_type, string_type>::const_iterator iter_type;
...
void cpp_regex_traits_implementation<charT>::init()
{
    ...
    if ((int)cat >= 0)
    {
        ...
        // Error messages:
        //
        for (boost::regex_constants::error_type i =
                 static_cast<boost::regex_constants::error_type>(0);
             ...
             i = static_cast<boost::regex_constants::error_type>(i + 1))
        {
            ...
            string_type s =
                this->m_pmessages->get(cat, 0, i + 200, default_message);
            std::string result;
    ...
    //
    m_collate_type =
        BOOST_REGEX_DETAIL_NS::find_sort_syntax(this, &m_collate_delim);
}

template <class charT>
typename cpp_regex_traits_implementation<charT>::char_class_type
    cpp_regex_traits_implementation<charT>::lookup_classname_imp(
        const charT* p1, const charT* p2) const
{
#ifndef BOOST_REGEX_BUGGY_CTYPE_FACET
    ...
    {
        typedef typename std::map<std::basic_string<charT>,
                                  char_class_type>::const_iterator map_iter;
        map_iter pos = m_custom_class_names.find(string_type(p1, p2));
        if (pos != m_custom_class_names.end())
            return pos->second;
    }
    std::size_t state_id =
        1 + BOOST_REGEX_DETAIL_NS::get_default_class_id(p1, p2);
...
#ifdef BOOST_REGEX_BUGGY_CTYPE_FACET
template <class charT>
bool cpp_regex_traits_implementation<charT>::isctype(const charT c,
                                                     char_class_type mask) const
{
    return ((mask & ::boost::BOOST_REGEX_DETAIL_NS::char_class_space) &&
            (this->m_pctype->is(std::ctype<charT>::space, c))) ||
           ((mask & ::boost::BOOST_REGEX_DETAIL_NS::char_class_print) &&
            (this->m_pctype->is(std::ctype<charT>::print, c))) ||
           ((mask & ::boost::BOOST_REGEX_DETAIL_NS::char_class_cntrl) &&
            (this->m_pctype->is(std::ctype<charT>::cntrl, c))) ||
           ((mask & ::boost::BOOST_REGEX_DETAIL_NS::char_class_upper) &&
            (this->m_pctype->is(std::ctype<charT>::upper, c))) ||
           ((mask & ::boost::BOOST_REGEX_DETAIL_NS::char_class_lower) &&
            (this->m_pctype->is(std::ctype<charT>::lower, c))) ||
           ((mask & ::boost::BOOST_REGEX_DETAIL_NS::char_class_alpha) &&
            (this->m_pctype->is(std::ctype<charT>::alpha, c))) ||
           ((mask & ::boost::BOOST_REGEX_DETAIL_NS::char_class_digit) &&
            (this->m_pctype->is(std::ctype<charT>::digit, c))) ||
           ((mask & ::boost::BOOST_REGEX_DETAIL_NS::char_class_punct) &&
            (this->m_pctype->is(std::ctype<charT>::punct, c))) ||
           ((mask & ::boost::BOOST_REGEX_DETAIL_NS::char_class_xdigit) &&
            (this->m_pctype->is(std::ctype<charT>::xdigit, c))) ||
           ((mask & ::boost::BOOST_REGEX_DETAIL_NS::char_class_blank) &&
            (this->m_pctype->is(std::ctype<charT>::space, c)) &&
            !::boost::BOOST_REGEX_DETAIL_NS::is_separator(c)) ||
           ((mask & ::boost::BOOST_REGEX_DETAIL_NS::char_class_word) &&
            (c == '_')) ||
           ((mask & ::boost::BOOST_REGEX_DETAIL_NS::char_class_unicode) &&
            ::boost::BOOST_REGEX_DETAIL_NS::is_extended(c)) ||
           ((mask &
             ::boost::BOOST_REGEX_DETAIL_NS::char_class_vertical_space) &&
            (is_separator(c) || (c == '\v'))) ||
           ((mask &
...
template <class charT>
inline boost::shared_ptr<const cpp_regex_traits_implementation<charT>>
    create_cpp_regex_traits(const std::locale& l)
{
    cpp_regex_traits_base<charT> key(l);
    return ::boost::object_cache<
        cpp_regex_traits_base<charT>,
        cpp_regex_traits_implementation<charT>>::get(key, 5);
}
...
class cpp_regex_traits
{
  ...
  public:
    ...
    cpp_regex_traits() :
        m_pimpl(BOOST_REGEX_DETAIL_NS::create_cpp_regex_traits<charT>(
            std::locale()))
    {
    }
    ...
    bool isctype(charT c, char_class_type f) const
    {
        ...
        static const ctype_mask mask_base = static_cast<ctype_mask>(
            std::ctype<charT>::alnum | std::ctype<charT>::alpha |
            std::ctype<charT>::cntrl | std::ctype<charT>::digit |
            std::ctype<charT>::graph | std::ctype<charT>::lower |
            std::ctype<charT>::print | std::ctype<charT>::punct |
            std::ctype<charT>::space | std::ctype<charT>::upper |
            std::ctype<charT>::xdigit);

        if ((f & mask_base) &&
            (m_pimpl->m_pctype->is(static_cast<ctype_mask>(f & mask_base), c)))
            return true;
        else if ((f & BOOST_REGEX_DETAIL_NS::cpp_regex_traits_implementation<
                          charT>::mask_unicode) &&
            ...
            return true;
                          ...
                          charT>::mask_horizontal) &&
                 this->isctype(c, std::ctype<charT>::space) &&
                 !this->isctype(
                     c, BOOST_REGEX_DETAIL_NS::cpp_regex_traits_implementation<
                            charT>::mask_vertical))
            return true;
#ifdef __CYGWIN__
        //
        // Cygwin has a buggy ctype facet, see
        // https://www.cygwin.com/ml/cygwin/2012-08/msg00178.html:
        //
  ...
  private:
    boost::shared_ptr<
        const BOOST_REGEX_DETAIL_NS::cpp_regex_traits_implementation<charT>>
        m_pimpl;
    //
...
template <class charT>
boost::intmax_t cpp_regex_traits<charT>::toi(const charT*& first,
                                             const charT* last, int radix) const
{
    BOOST_REGEX_DETAIL_NS::parser_buf<charT>
        sbuf;                            // buffer for parsing numbers.
    std::basic_istream<charT> is(&sbuf); // stream for parsing numbers.

    // we do NOT want to parse any thousands separators inside the stream:
    last = std::find(
        first, last,
        BOOST_USE_FACET(std::numpunct<charT>, is.getloc()).thousands_sep());

    sbuf.pubsetbuf(const_cast<charT*>(static_cast<const charT*>(first)),
