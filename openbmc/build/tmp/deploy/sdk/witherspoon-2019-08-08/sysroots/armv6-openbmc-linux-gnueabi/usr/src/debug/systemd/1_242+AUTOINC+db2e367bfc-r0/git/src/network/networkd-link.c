...
static bool link_ipv6_enabled(Link* link)
{
    ...
    /* DHCPv6 client will not be started if no IPv6 link-local address is
     * configured. */
    return link_ipv6ll_enabled(link) ||
           network_has_static_ipv6_addresses(link->network);
}
...
static bool link_ipv6_accept_ra_enabled(Link* link)
{
    ...
    if (link->network->ipv6_accept_ra < 0)
        /* default to accept RA if ip_forward is disabled and ignore RA if
...
static int link_enable_ipv6(Link* link)
{
    ...
    r = sysctl_write_ip_property_boolean(AF_INET6, link->ifname, "disable_ipv6",
                                         disabled);
    if (r < 0)
...
#define FLAG_STRING(string, flag, old, new)                                    \
    (((old ^ new) & flag) ? ((old & flag) ? (" -" string) : (" +" string)) : "")
...
static int link_update_flags(Link* link, sd_netlink_message* m)
{
    ...
    {
        log_link_debug(
            link, "Flags change:%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s",
        ...
        unknown_flags =
            ~(IFF_LOOPBACK | IFF_MASTER | IFF_SLAVE | IFF_UP | IFF_DORMANT |
              IFF_LOWER_UP | IFF_RUNNING | IFF_MULTICAST | IFF_BROADCAST |
              IFF_POINTOPOINT | IFF_PROMISC | IFF_ALLMULTI | IFF_PORTSEL |
              IFF_AUTOMEDIA | IFF_DYNAMIC | IFF_NOARP | IFF_NOTRAILERS |
              IFF_DEBUG | IFF_ECHO);
        unknown_flags_added = ((link->flags ^ flags) & flags & unknown_flags);
        unknown_flags_removed =
            ((link->flags ^ flags) & link->flags & unknown_flags);

        /* link flags are currently at most 18 bits, let's align to
         * printing 20 */
        if (unknown_flags_added)
            log_link_debug(link, "Unknown link flags gained: %#.5x (ignoring)",
                           unknown_flags_added);

        if (unknown_flags_removed)
            log_link_debug(link, "Unknown link flags lost: %#.5x (ignoring)",
                           unknown_flags_removed);
    }
...
static int link_new(Manager* manager, sd_netlink_message* message, Link** ret)
{
    ...
    *link = (Link){
    ...
    r = sd_netlink_message_read_u32(message, IFLA_MASTER,
                                    (uint32_t*)&link->master_ifindex);
    if (r < 0)
    ...
    if (r < 0)
        log_link_debug_errno(
            link, r,
            "MAC address not found for new device, continuing without");

    if (asprintf(&link->state_file, "/run/systemd/netif/links/%d",
                 link->ifindex) < 0)
        return -ENOMEM;

    if (asprintf(&link->lease_file, "/run/systemd/netif/leases/%d",
                 link->ifindex) < 0)
        return -ENOMEM;

    if (asprintf(&link->lldp_file, "/run/systemd/netif/lldp/%d",
                 link->ifindex) < 0)
        return -ENOMEM;
...
static int link_stop_clients(Link* link)
{
    ...
    if (link->dhcp_client)
    {
        ...
        if (k < 0)
            r = log_link_warning_errno(link, k,
    ...
    if (link->ndisc)
    {
        ...
        if (k < 0)
            r = log_link_warning_errno(
                link, k, "Could not stop IPv6 Router Discovery: %m");
    }
    ...
    if (link->radv)
    {
        ...
        if (k < 0)
            r = log_link_warning_errno(
                link, k, "Could not stop IPv6 Router Advertisement: %m");
    }
...
static int link_join_netdevs_after_configured(Link* link)
{
    ...
    HASHMAP_FOREACH(netdev, link->network->stacked_netdevs, i)
    {
        ...
        log_struct(LOG_DEBUG, LOG_LINK_INTERFACE(link),
        ...
        if (r < 0)
            return log_struct_errno(
                LOG_WARNING, r, LOG_LINK_INTERFACE(link),
                LOG_NETDEV_INTERFACE(netdev),
                LOG_LINK_MESSAGE(link, "Could not join netdev '%s': %m",
                                 netdev->ifname));
    }
...
static int link_request_set_routing_policy_rule(Link* link)
{
    ...
    LIST_FOREACH(rules, rule, link->network->rules)
    {
        r = routing_policy_rule_get(
            link->manager, rule->family, &rule->from, rule->from_prefixlen,
            &rule->to, rule->to_prefixlen, rule->tos, rule->fwmark, rule->table,
            rule->iif, rule->oif, rule->protocol, &rule->sport, &rule->dport,
            &rrule);
        if (r == 0)
        {
...
static int link_request_set_routes(Link* link)
{
    ...
    /* First add the routes that enable us to talk to gateways, then add in the
     * others that need a gateway. */
    for (phase = 0; phase < _PHASE_MAX; phase++)
        LIST_FOREACH(routes, rt, link->network->static_routes)
        {

            if (in_addr_is_null(rt->family, &rt->gw) !=
                (phase == PHASE_NON_GATEWAY))
                continue;
...
void link_check_ready(Link* link)
{
    ...
    if (link_ipv6ll_enabled(link) &&
        in_addr_is_null(AF_INET6,
                        (const union in_addr_union*)&link->ipv6ll_address))
        return;
...
static int link_set_bridge_fdb(Link* link)
{
    ...
    LIST_FOREACH(static_fdb_entries, fdb_entry,
                 link->network->static_fdb_entries)
    {
        r = fdb_entry_configure(link, fdb_entry);
        if (r < 0)
            return log_link_error_errno(
                link, r, "Failed to add MAC entry to static MAC table: %m");
    }
...
static int link_request_set_addresses(Link* link)
{
    ...
    LIST_FOREACH(addresses, ad, link->network->static_addresses)
    {
        ...
        update = address_get(link, ad->family, &ad->in_addr, ad->prefixlen,
                             NULL) > 0;
    ...
    if (link_dhcp4_server_enabled(link) && (link->flags & IFF_UP))
    {
        ...
        r = sd_dhcp_server_configure_pool(
            link->dhcp_server, &address->in_addr.in, address->prefixlen,
            link->network->dhcp_server_pool_offset,
            link->network->dhcp_server_pool_size);
        if (r < 0)
            return r;
        ...
        */
        ...
        if (link->network->dhcp_server_max_lease_time_usec > 0)
        {
            r = sd_dhcp_server_set_max_lease_time(
                link->dhcp_server,
                DIV_ROUND_UP(link->network->dhcp_server_max_lease_time_usec,
                             USEC_PER_SEC));
            if (r < 0)
                return r;
        }
        ...
        if (link->network->dhcp_server_default_lease_time_usec > 0)
        {
            r = sd_dhcp_server_set_default_lease_time(
                link->dhcp_server,
                DIV_ROUND_UP(link->network->dhcp_server_default_lease_time_usec,
                             USEC_PER_SEC));
            if (r < 0)
                return r;
        }
        ...
        if (link->network->dhcp_server_emit_dns)
        {
            ...
            if (link->network->n_dhcp_server_dns > 0)
                r = sd_dhcp_server_set_dns(link->dhcp_server,
            ...
            else
            {
                ...
                {
                    log_link_debug(link,
                                   "Not emitting DNS server information on "
                                   "link, couldn't find suitable uplink.");
                    r = 0;
                }
                else
                    r = link_push_uplink_dns_to_dhcp_server(uplink,
                                                            link->dhcp_server);
            }
            if (r < 0)
        ...
        if (link->network->dhcp_server_emit_ntp)
        {
            ...
            if (link->network->n_dhcp_server_ntp > 0)
                r = sd_dhcp_server_set_ntp(link->dhcp_server,
            ...
            else
            {
                ...
                {
                    log_link_debug(link,
                                   "Not emitting NTP server information on "
                                   "link, couldn't find suitable uplink.");
                    r = 0;
                }
                else
                    r = link_push_uplink_ntp_to_dhcp_server(uplink,
                                                            link->dhcp_server);
            }
            if (r < 0)
                log_link_warning_errno(
        ...
        r = sd_dhcp_server_set_emit_router(
            link->dhcp_server, link->network->dhcp_server_emit_router);
        if (r < 0)
            return log_link_warning_errno(
                link, r, "Failed to set router emission for DHCP server: %m");
        ...
        if (!sd_dhcp_server_is_running(link->dhcp_server))
        {
            ...
            {
                log_link_warning_errno(
                    link, r, "Could not start DHCPv4 server instance: %m");
...
static int link_set_bridge_vlan(Link* link)
{
    ...
    r = br_vlan_configure(link, link->network->pvid,
                          link->network->br_vid_bitmap,
                          link->network->br_untagged_bitmap);
    if (r < 0)
...
static int link_set_proxy_arp(Link* link)
{
    ...
    r = sysctl_write_ip_property_boolean(AF_INET, link->ifname, "proxy_arp",
                                         link->network->proxy_arp > 0);
    if (r < 0)
        log_link_warning_errno(link, r,
...
int link_set_mtu(Link* link, uint32_t mtu, bool force)
{
    ...
    if (r < 0)
        return log_link_error_errno(
            link, r, "Could not allocate RTM_SETLINK message: %m");

    /* If IPv6 not configured (no static IPv6 address and IPv6LL
    ...
    if (link_ipv6_enabled(link) && mtu < IPV6_MIN_MTU)
    {
        ...
        log_link_warning(
            link,
            "Bumping MTU to " STRINGIFY(
                IPV6_MIN_MTU) ", as "
                              "IPv6 is requested and requires a minimum MTU "
                              "of " STRINGIFY(IPV6_MIN_MTU) " bytes: %m");
...
static int set_flags_handler(sd_netlink* rtnl, sd_netlink_message* m,
                             Link* link)
{
    int r;
...
static int link_set_bridge(Link* link)
{
    ...
    if (r < 0)
        return log_link_error_errno(
            link, r, "Could not append IFLA_PROTINFO attribute: %m");

    if (link->network->use_bpdu >= 0)
    {
        r = sd_netlink_message_append_u8(req, IFLA_BRPORT_GUARD,
                                         link->network->use_bpdu);
        if (r < 0)
            return log_link_error_errno(
                link, r, "Could not append IFLA_BRPORT_GUARD attribute: %m");
    }

    if (link->network->hairpin >= 0)
    {
        r = sd_netlink_message_append_u8(req, IFLA_BRPORT_MODE,
                                         link->network->hairpin);
        if (r < 0)
            return log_link_error_errno(
                link, r, "Could not append IFLA_BRPORT_MODE attribute: %m");
    }

    if (link->network->fast_leave >= 0)
    {
        r = sd_netlink_message_append_u8(req, IFLA_BRPORT_FAST_LEAVE,
                                         link->network->fast_leave);
        if (r < 0)
            return log_link_error_errno(
                link, r,
    ...
    {
        r = sd_netlink_message_append_u8(req, IFLA_BRPORT_PROTECT,
                                         link->network->allow_port_to_be_root);
        if (r < 0)
            return log_link_error_errno(
                link, r, "Could not append IFLA_BRPORT_PROTECT attribute: %m");
    }

    if (link->network->unicast_flood >= 0)
    {
        r = sd_netlink_message_append_u8(req, IFLA_BRPORT_UNICAST_FLOOD,
                                         link->network->unicast_flood);
        if (r < 0)
            return log_link_error_errno(
                link, r,
    ...
    {
        r = sd_netlink_message_append_u8(req, IFLA_BRPORT_MCAST_FLOOD,
                                         link->network->multicast_flood);
        if (r < 0)
            return log_link_error_errno(
                link, r,
    ...
    {
        r = sd_netlink_message_append_u8(req, IFLA_BRPORT_MCAST_TO_UCAST,
                                         link->network->multicast_to_unicast);
        if (r < 0)
            return log_link_error_errno(
                link, r,
    ...
    {
        r = sd_netlink_message_append_u8(req, IFLA_BRPORT_NEIGH_SUPPRESS,
                                         link->network->neighbor_suppression);
        if (r < 0)
            return log_link_error_errno(
                link, r,
    ...
    {
        r = sd_netlink_message_append_u8(req, IFLA_BRPORT_LEARNING,
                                         link->network->learning);
        if (r < 0)
            return log_link_error_errno(
                link, r, "Could not append IFLA_BRPORT_LEARNING attribute: %m");
    }

    if (link->network->cost != 0)
    {
        r = sd_netlink_message_append_u32(req, IFLA_BRPORT_COST,
                                          link->network->cost);
        if (r < 0)
            return log_link_error_errno(
                link, r, "Could not append IFLA_BRPORT_COST attribute: %m");
    }

    if (link->network->priority != LINK_BRIDGE_PORT_PRIORITY_INVALID)
    {
        r = sd_netlink_message_append_u16(req, IFLA_BRPORT_PRIORITY,
                                          link->network->priority);
        if (r < 0)
            return log_link_error_errno(
                link, r, "Could not append IFLA_BRPORT_PRIORITY attribute: %m");
    }

    r = sd_netlink_message_close_container(req);
    if (r < 0)
        return log_link_error_errno(
            link, r, "Could not append IFLA_LINKINFO attribute: %m");
...
static int link_set_bond_handler(sd_netlink* rtnl, sd_netlink_message* m,
                                 Link* link)
{
    int r;
...
static int link_set_bond(Link* link)
{
    ...
    r = sd_rtnl_message_new_link(link->manager->rtnl, &req, RTM_NEWLINK,
                                 link->network->bond->ifindex);
    if (r < 0)
        return log_link_error_errno(
            link, r, "Could not allocate RTM_SETLINK message: %m");
    ...
    if (r < 0)
        return log_link_error_errno(
            link, r, "Could not append IFLA_INFO_DATA attribute: %m");

    if (link->network->active_slave)
    {
        r = sd_netlink_message_append_u32(req, IFLA_BOND_ACTIVE_SLAVE,
                                          link->ifindex);
        if (r < 0)
            return log_link_error_errno(
                link, r,
    ...
    {
        r = sd_netlink_message_append_u32(req, IFLA_BOND_PRIMARY,
                                          link->ifindex);
        if (r < 0)
            return log_link_error_errno(
                link, r, "Could not append IFLA_BOND_PRIMARY attribute: %m");
    }
    ...
    r = netlink_call_async(link->manager->rtnl, NULL, req,
                           link_set_bond_handler, link_netlink_destroy_callback,
                           link);
    if (r < 0)
        return log_link_error_errno(link, r,
                                    "Could not send rtnetlink message: %m");
...
finish:
    if (r < 0)
    {
        ...
        log_link_error_errno(link, r, "Failed to save LLDP data to %s: %m",
                             link->lldp_file);
    }
...
static void lldp_handler(sd_lldp* lldp, sd_lldp_event event,
                         sd_lldp_neighbor* n, void* userdata)
{
    Link* link = userdata;
    int r;
    ...
    {
        /* If we received information about a new neighbor, restart the LLDP
         * "fast" logic */

        log_link_debug(link, "Received LLDP datagram from previously unknown "
                             "neighbor, restarting 'fast' LLDP transmission.");
...
static int link_acquire_ipv6_conf(Link* link)
{
    ...
    if (link_ipv6_accept_ra_enabled(link))
    {
        ...
        if (r < 0 && r != -EBUSY)
            return log_link_warning_errno(
                link, r, "Could not start IPv6 Router Discovery: %m");
    }

    if (link_radv_enabled(link))
    {
        assert(link->radv);
        assert(in_addr_is_link_local(
                   AF_INET6,
                   (const union in_addr_union*)&link->ipv6ll_address) > 0);
        ...
        if (r < 0 && r != -EBUSY)
            return log_link_warning_errno(
                link, r, "Could not start IPv6 Router Advertisement: %m");
    }
...
static int link_acquire_ipv4_conf(Link* link)
{
    ...
    if (link_ipv4ll_enabled(link))
    {
        ...
        if (r < 0)
            return log_link_warning_errno(
                link, r, "Could not acquire IPv4 link-local address: %m");
    }
...
static int link_acquire_conf(Link* link)
{
    ...
    if (link_lldp_emit_enabled(link))
    {
        ...
        if (r < 0)
            return log_link_warning_errno(
                link, r, "Failed to start LLDP transmission: %m");
    }
...
static int link_address_genmode_handler(sd_netlink* rtnl, sd_netlink_message* m,
                                        Link* link)
{
    int r;
    ...
    if (r < 0)
        log_link_warning_errno(
            link, r, "Could not set address genmode for interface: %m");
...
static int link_configure_addrgen_mode(Link* link)
{
    ...
    if (r < 0)
        return log_link_error_errno(
            link, r, "Could not open IFLA_AF_SPEC container: %m");
    ...
    else if (sysctl_read_ip_property(AF_INET6, link->ifname, "stable_secret",
                                     NULL) < 0)
        /* The file may not exist. And event if it exists, when stable_secret is
         * unset, reading the file fails with EIO. */
        ipv6ll_mode = IN6_ADDR_GEN_MODE_EUI64;
    else
        ipv6ll_mode = IN6_ADDR_GEN_MODE_STABLE_PRIVACY;

    r = sd_netlink_message_append_u8(req, IFLA_INET6_ADDR_GEN_MODE,
                                     ipv6ll_mode);
    if (r < 0)
        return log_link_error_errno(
            link, r, "Could not append IFLA_INET6_ADDR_GEN_MODE: %m");
    ...
    if (r < 0)
        return log_link_error_errno(
            link, r, "Could not close IFLA_AF_SPEC container: %m");
...
static int link_up(Link* link)
{
    ...
    if (!link->network->bridge && !link->network->bond && !link->network->vrf)
    {
        ...
        if (r < 0)
            return log_link_error_errno(
                link, r, "Could not append IFLA_MASTER attribute: %m");
    }
    ...
    if (link->network->mac)
    {
        r = sd_netlink_message_append_ether_addr(req, IFLA_ADDRESS,
                                                 link->network->mac);
        if (r < 0)
            return log_link_error_errno(link, r,
...
static int link_set_can(Link* link)
{
    ...
    r = sd_rtnl_message_new_link(link->manager->rtnl, &m, RTM_NEWLINK,
                                 link->ifindex);
    if (r < 0)
        return log_link_error_errno(link, r,
    ...
    if (link->network->can_bitrate > 0 || link->network->can_sample_point > 0)
    {
        ...
        {
            log_link_error(link, "bitrate (%zu) too big.",
                           link->network->can_bitrate);
            return -ERANGE;
        }

        log_link_debug(link, "Setting bitrate = %d bit/s", bt.bitrate);
        if (link->network->can_sample_point > 0)
            log_link_debug(link, "Setting sample point = %d.%d%%",
                           bt.sample_point / 10, bt.sample_point % 10);
        else
            log_link_debug(link, "Using default sample point");

        r = sd_netlink_message_append_data(m, IFLA_CAN_BITTIMING, &bt,
                                           sizeof(bt));
        if (r < 0)
            return log_link_error_errno(
                link, r, "Could not append IFLA_CAN_BITTIMING attribute: %m");
    }
    ...
    if (link->network->can_restart_us > 0)
    {
        ...
        if (link->network->can_restart_us == USEC_INFINITY)
            ...
            restart_ms =
                DIV_ROUND_UP(link->network->can_restart_us, USEC_PER_MSEC);

        format_timespan(time_string, FORMAT_TIMESPAN_MAX, restart_ms * 1000,
                        MSEC_PER_SEC);
        ...
        if (r < 0)
            return log_link_error_errno(
                link, r, "Could not append IFLA_CAN_RESTART_MS attribute: %m");
    }
    ...
    if (link->network->can_triple_sampling >= 0)
    {
        struct can_ctrlmode cm = {
            ...
            .flags =
                link->network->can_triple_sampling ? CAN_CTRLMODE_3_SAMPLES : 0,
        };

        log_link_debug(link, "%sabling triple-sampling",
                       link->network->can_triple_sampling ? "En" : "Dis");

        r = sd_netlink_message_append_data(m, IFLA_CAN_CTRLMODE, &cm,
                                           sizeof(cm));
        if (r < 0)
            return log_link_error_errno(
                link, r, "Could not append IFLA_CAN_CTRLMODE attribute: %m");
    }
...
static int link_down_handler(sd_netlink* rtnl, sd_netlink_message* m,
                             Link* link)
{
    int r;
...
static void link_detach_from_manager(Link* link)
{
    ...
    assert_se(hashmap_remove(link->manager->links, INT_TO_PTR(link->ifindex)) ==
              link);
    link_unref(link);
}
...
static int link_joined(Link* link)
{
    ...
    if (link->network->bridge)
    {
        ...
        if (r < 0)
            log_link_error_errno(
                link, r, "Failed to add to bridge master's slave list: %m");
    }
    ...
    if (link->network->bond)
    {
        ...
        if (r < 0)
            log_link_error_errno(
                link, r, "Failed to add to bond master's slave list: %m");
    }
...
static int link_enter_join_netdev(Link* link)
{
    ...
    if (link->network->bond)
    {
        ...
        log_struct(LOG_DEBUG, LOG_LINK_INTERFACE(link),
                   ...
                   LOG_LINK_MESSAGE(link, "Enslaving by '%s'",
        ...
        if (r < 0)
        {
            log_struct_errno(LOG_WARNING, r, LOG_LINK_INTERFACE(link),
                             LOG_NETDEV_INTERFACE(link->network->bond),
                             LOG_LINK_MESSAGE(link,
                                              "Could not join netdev '%s': %m",
                                              link->network->bond->ifname));
            link_enter_failed(link);
            return r;
        }
...
static int link_set_ipv4_forward(Link* link)
{
    ...
    if (r < 0)
        log_link_warning_errno(
            link, r, "Cannot turn on IPv4 packet forwarding, ignoring: %m");
...
static int link_set_ipv6_forward(Link* link)
{
    ...
    if (r < 0)
        log_link_warning_errno(
            link, r, "Cannot configure IPv6 packet forwarding, ignoring: %m");
...
static int link_set_ipv6_privacy_extensions(Link* link)
{
    ...
    r = sysctl_write_ip_property_int(
        AF_INET6, link->ifname, "use_tempaddr",
        (int)link->network->ipv6_privacy_extensions);
    if (r < 0)
        log_link_warning_errno(
            link, r,
...
static int link_set_ipv6_dad_transmits(Link* link)
{
    ...
    r = sysctl_write_ip_property_int(AF_INET6, link->ifname, "dad_transmits",
                                     link->network->ipv6_dad_transmits);
    if (r < 0)
        log_link_warning_errno(
            link, r, "Cannot set IPv6 dad transmits for interface: %m");
...
static int link_set_ipv6_hop_limit(Link* link)
{
    ...
    r = sysctl_write_ip_property_int(AF_INET6, link->ifname, "hop_limit",
                                     link->network->ipv6_hop_limit);
    if (r < 0)
        log_link_warning_errno(link, r,
...
static int link_set_ipv6_mtu(Link* link)
{
    ...
    r = sysctl_write_ip_property_uint32(AF_INET6, link->ifname, "mtu",
                                        link->network->ipv6_mtu);
    if (r < 0)
        log_link_warning_errno(link, r,
...
static int link_drop_foreign_config(Link* link)
{
    ...
    SET_FOREACH(address, link->addresses_foreign, i)
    {
        ...
        if (address->family == AF_INET6 &&
            in_addr_is_link_local(AF_INET6, &address->in_addr) == 1)
            continue;

        if (link_is_static_address_configured(link, address))
        {
            r = address_add(link, address->family, &address->in_addr,
                            address->prefixlen, NULL);
            if (r < 0)
                return log_link_error_errno(link, r,
    ...
    SET_FOREACH(route, link->routes_foreign, i)
    {
        ...
        {
            r = route_add(link, route->family, &route->dst,
                          route->dst_prefixlen, route->tos, route->priority,
                          route->table, NULL);
            if (r < 0)
                return r;
        }
...
static int link_configure(Link* link)
{
    ...
    if (link_lldp_rx_enabled(link))
    {
        ...
        r = sd_lldp_match_capabilities(
            link->lldp, link->network->lldp_mode == LLDP_MODE_ROUTERS_ONLY
...
int get_product_uuid_handler(sd_bus_message* m, void* userdata,
                             sd_bus_error* ret_error)
{
    ...
    if (e)
    {
        log_error_errno(sd_bus_error_get_errno(e),
                        "Could not get product UUID. Falling back to use "
                        "machine-app-specific ID as DUID-UUID: %s",
                        e->message);
        goto configure;
    }
    ...
    {
        log_error("Invalid product UUID. Falling back to use "
                  "machine-app-specific ID as DUID-UUID.");
        goto configure;
    }
...
configure:
    ...
    /* To avoid calling GetProductUUID() bus method so frequently, set the flag
     * below even if the method fails. */
...
static bool link_requires_uuid(Link* link)
{
    ...
    if (link_dhcp4_enabled(link) &&
        IN_SET(link->network->dhcp_client_identifier, DHCP_CLIENT_ID_DUID,
...
static int link_configure_duid(Link* link)
{
    ...
    if (!m->links_requesting_uuid)
    {
        ...
        if (r < 0)
        {
            ...
            log_link_warning_errno(
                link, r,
                "Failed to get product UUID. Falling back to use "
                "machine-app-specific ID as DUID-UUID: %m");
            return 1;
        }
    }
    else
...
static int link_initialized_and_synced(Link* link)
{
    ...
    /* link_configure_duid() returns 0 if it requests product UUID. In that
     * case, link_configure() is called later asynchronously. */
...
static int link_initialized_handler(sd_netlink* rtnl, sd_netlink_message* m,
                                    Link* link)
{
...
{
    _cleanup_free_ char *network_file = NULL, *addresses = NULL, *routes = NULL,
                        *dhcp4_address = NULL, *ipv4ll_address = NULL;
    ...
    r = parse_env_file(NULL, link->state_file, "NETWORK_FILE", &network_file,
                       "ADDRESSES", &addresses, "ROUTES", &routes,
                       "DHCP4_ADDRESS", &dhcp4_address, "IPV4LL_ADDRESS",
                       &ipv4ll_address);
    if (r < 0 && r != -ENOENT)
        return log_link_error_errno(link, r, "Failed to read %s: %m",
                                    link->state_file);
    ...
    if (network_file)
    {
        ...
        if (!suffix)
        {
            log_link_debug(link, "Failed to get network name from %s",
                           network_file);
            goto network_file_fail;
        }
        *suffix = '\0';

        r = network_get_by_name(link->manager, basename(network_file),
                                &network);
        if (r < 0)
        {
            log_link_debug_errno(link, r, "Failed to get network %s: %m",
...
network_file_fail:
    ...
    if (addresses)
    {
        ...
        for (;;)
        {
            ...
            {
                log_link_debug_errno(
                    link, r, "Failed to extract next address string: %m");
                continue;
            }
            if (r == 0)
                break;
            ...
            {
                log_link_debug(link,
                               "Failed to parse address and prefix length %s",
                               address_str);
                continue;
            }
            ...
            *prefixlen_str++ = '\0';
            ...
            if (r != 1)
            {
                log_link_error(link, "Failed to parse prefixlen %s",
                               prefixlen_str);
                continue;
            }
            ...
            if (r < 0)
            {
                log_link_debug_errno(link, r, "Failed to parse address %s: %m",
                                     address_str);
                continue;
            }
    ...
    if (routes)
    {
        ...
        for (;;)
        {
            ...
            *prefixlen_str++ = '\0';
            ...
            r = sscanf(prefixlen_str, "%hhu/%hhu/%" SCNu32 "/%hhu/" USEC_FMT,
                       &prefixlen, &tos, &priority, &table, &lifetime);
            if (r != 5)
            {
                log_link_debug(link,
                               "Failed to parse destination prefix length, "
                               "tos, priority, table or expiration %s",
                               prefixlen_str);
                continue;
            }
            ...
            {
                log_link_debug_errno(link, r,
                                     "Failed to parse route destination %s: %m",
                                     route_str);
                continue;
            }

            r = route_add(link, family, &route_dst, prefixlen, tos, priority,
                          table, &route);
            if (r < 0)
                ...
                !kernel_route_expiration_supported())
            {
                ...
                if (r < 0)
                    log_link_warning_errno(
                        link, r, "Could not arm route expiration handler: %m");
            }
    ...
    if (dhcp4_address)
    {
        ...
        {
            log_link_debug_errno(link, r,
                                 "Failed to parse DHCPv4 address %s: %m",
                                 dhcp4_address);
            goto dhcp4_address_fail;
        }

        r = sd_dhcp_client_new(&link->dhcp_client,
                               link->network ? link->network->dhcp_anonymize
                                             : 0);
        if (r < 0)
            return log_link_error_errno(link, r,
        ...
        if (r < 0)
            return log_link_error_errno(
                link, r, "Failed to set initial DHCPv4 address %s: %m",
                dhcp4_address);
    }
...
dhcp4_address_fail:
    ...
    if (ipv4ll_address)
    {
        ...
        {
            log_link_debug_errno(link, r,
                                 "Failed to parse IPv4LL address %s: %m",
                                 ipv4ll_address);
        ...
        if (r < 0)
            return log_link_error_errno(
                link, r, "Failed to set initial IPv4LL address %s: %m",
                ipv4ll_address);
    }
...
int link_add(Manager* m, sd_netlink_message* message, Link** ret)
{
    ...
    if (detect_container() <= 0)
    {
        ...
        {
            log_link_warning_errno(link, r,
                                   "Could not determine whether the device is "
        ...
        {
            log_link_debug(
                link, "Interface is under renaming, pending initialization.");
            return 0;
        }
...
static int link_carrier_lost(Link* link)
{
    ...
    /* Some devices reset itself while setting the MTU. This causes the DHCP
     * client fall into a loop. setting_mtu keep track whether the device got
     * reset because of setting MTU and does not drop the
    ...
    {
        log_link_debug(link, "State is %s, dropping config",
...
int link_update(Link* link, sd_netlink_message* m)
{
    ...
    if (r >= 0 && !streq(ifname, link->ifname))
    {
        ...
        log_link_info(
            link, "Interface name change detected, %s has been renamed to %s.",
            link->ifname, ifname);
    ...
    if (r >= 0 && mtu > 0)
    {
        ...
        if (link->original_mtu == 0)
        {
            ...
            log_link_debug(link, "Saved original MTU: %" PRIu32,
                           link->original_mtu);
        }
        ...
        if (link->dhcp_client)
        {
            ...
            if (r < 0)
                return log_link_warning_errno(
                    link, r, "Could not update MTU in DHCP client: %m");
        }
        ...
        if (link->radv)
        {
            ...
            if (r < 0)
                return log_link_warning_errno(
                    link, r, "Could not set MTU for Router Advertisement: %m");
        }
    }
        ...
        if (memcmp(link->mac.ether_addr_octet, mac.ether_addr_octet, ETH_ALEN))
        {
            ...
            if (link->dhcp6_client)
            {
                ...
                if (link->network->iaid_set)
                {
                    ...
                    if (r < 0)
                        return log_link_warning_errno(
                            link, r, "Could not update DHCPv6 IAID: %m");
                }

                r = sd_dhcp6_client_set_duid(
                    link->dhcp6_client, duid->type,
                    duid->raw_data_len > 0 ? duid->raw_data : NULL,
                    duid->raw_data_len);
                if (r < 0)
                    return log_link_warning_errno(
                        link, r, "Could not update DHCPv6 DUID: %m");
            }
            ...
            if (link->ndisc)
            {
                ...
                if (r < 0)
                    return log_link_warning_errno(
                        link, r, "Could not update MAC for ndisc: %m");
            }
        }
    }
...
int link_save(Link* link)
{
    ...
    if (link->network)
    {
        ...
        fprintf(f, "REQUIRED_OPER_STATE_FOR_ONLINE=%s\n",
                strempty(link_operstate_to_string(
        ...
        if (link->network->dhcp_use_dns && link->dhcp_lease)
        {
            ...
            if (r > 0)
                if (serialize_in_addrs(f, addresses, r, space,
                                       in4_addr_is_non_local) > 0)
                    space = true;
        }
        ...
        if (link->network->dhcp_use_ntp && dhcp6_lease)
        {
            ...
            r = sd_dhcp6_lease_get_ntp_addrs(dhcp6_lease, &in6_addrs);
            if (r > 0)
            {
                if (space)
                    fputc(' ', f);
        ...
        if (link->network->dhcp_use_domains != DHCP_USE_DOMAINS_NO)
        {
            if (link->dhcp_lease)
            {
                (void)sd_dhcp_lease_get_domainname(link->dhcp_lease,
                                                   &dhcp_domainname);
                (void)sd_dhcp_lease_get_search_domains(link->dhcp_lease,
                                                       &dhcp_domains);
            }
            if (dhcp6_lease)
        ...
        SET_FOREACH(route, link->routes, i)
        {
            ...
            fprintf(f, "%s%s/%hhu/%hhu/%" PRIu32 "/%" PRIu32 "/" USEC_FMT,
                    space ? " " : "", route_str, route->dst_prefixlen,
                    route->tos, route->priority, route->table, route->lifetime);
            space = true;
        }
...
fail:
    ...
    return log_link_error_errno(link, r, "Failed to save link data to %s: %m",
                                link->state_file);
}
