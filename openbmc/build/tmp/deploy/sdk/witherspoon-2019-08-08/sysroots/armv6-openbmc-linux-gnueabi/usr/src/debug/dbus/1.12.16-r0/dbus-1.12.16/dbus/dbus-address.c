...
static dbus_bool_t append_unescaped_value(DBusString* unescaped,
                                          const DBusString* escaped,
                                          int escaped_start, int escaped_len,
                                          DBusError* error)
{
    const char* p;
    const char* end;
    ...
    while (p != end)
    {
        ...
        else if (*p == '%')
        {
            ...
            {
                dbus_set_error(error, DBUS_ERROR_BAD_ADDRESS,
                               "In D-Bus address, percent character was not "
            ...
            if (!_dbus_string_hex_decode(&hex, 0, &hex_end, unescaped,
            ...
            {
                dbus_set_error(error, DBUS_ERROR_BAD_ADDRESS,
                               "In D-Bus address, percent character was "
                               "followed by characters other than hex digits");
                goto out;
            }
        }
        else
...
dbus_bool_t dbus_parse_address(const char* address,
                               DBusAddressEntry*** entry_result, int* array_len,
                               DBusError* error)
{
    DBusString str;
    ...
    while (pos < len)
    {
        ...
        if (!_dbus_string_copy_len(&str, pos, found_pos - pos, &entry->method,
                                   0))
        {
            dbus_set_error(error, DBUS_ERROR_NO_MEMORY, NULL);
            goto error;
...
char* dbus_address_unescape_value(const char* value, DBusError* error)
{
    ...
    if (!append_unescaped_value(&unescaped, &escaped, 0,
                                _dbus_string_get_length(&escaped), error))
        goto out;
...
dbus_bool_t _dbus_address_test(void)
{
    ...
    while (i < _DBUS_N_ELEMENTS(escape_tests))
    {
        ...
        if (unescaped == NULL)
        {
            _dbus_warn("Failed to unescape '%s': %s", test->escaped,
                       error.message);
    ...
    while (i < _DBUS_N_ELEMENTS(invalid_escaped_values))
    {
        ...
        unescaped =
            dbus_address_unescape_value(invalid_escaped_values[i], &error);
        if (unescaped != NULL)
        {
    ...
    _dbus_assert(len == 2);
    _dbus_assert(strcmp(dbus_address_entry_get_value(entries[0], "path"),
                        "/tmp/foo") == 0);
    _dbus_assert(
        strcmp(dbus_address_entry_get_value(entries[1], "name"), "test") == 0);
    _dbus_assert(strcmp(dbus_address_entry_get_value(entries[1], "sliff"),
                        "sloff") == 0);
