...
#define INVALID_EXTENSION                                                      \
    {                                                                          \
        0x10000, 0, NULL, NULL, NULL, NULL, NULL, NULL                         \
    }
static const EXTENSION_DEFINITION ext_defs[] = {
    {TLSEXT_TYPE_renegotiate,
     SSL_EXT_CLIENT_HELLO | SSL_EXT_TLS1_2_SERVER_HELLO | SSL_EXT_SSL3_ALLOWED |
    ...
    {TLSEXT_TYPE_server_name,
     ...
     init_server_name, tls_parse_ctos_server_name, tls_parse_stoc_server_name,
    ...
    {/*
     ...
     tls_construct_stoc_supported_groups, tls_construct_ctos_supported_groups,
     NULL},
#else
    INVALID_EXTENSION,
    INVALID_EXTENSION,
    ...
    {TLSEXT_TYPE_encrypt_then_mac,
     SSL_EXT_CLIENT_HELLO | SSL_EXT_TLS1_2_SERVER_HELLO |
         SSL_EXT_TLS1_2_AND_BELOW_ONLY,
     init_etm, tls_parse_ctos_etm, tls_parse_stoc_etm, tls_construct_stoc_etm,
     tls_construct_ctos_etm, NULL},
    ...
    {TLSEXT_TYPE_extended_master_secret,
     SSL_EXT_CLIENT_HELLO | SSL_EXT_TLS1_2_SERVER_HELLO |
         SSL_EXT_TLS1_2_AND_BELOW_ONLY,
     init_ems, tls_parse_ctos_ems, tls_parse_stoc_ems, tls_construct_stoc_ems,
     tls_construct_ctos_ems, final_ems},
    ...
    {TLSEXT_TYPE_signature_algorithms,
     SSL_EXT_CLIENT_HELLO | SSL_EXT_TLS1_3_CERTIFICATE_REQUEST, init_sig_algs,
     tls_parse_ctos_sig_algs, tls_parse_ctos_sig_algs,
     tls_construct_ctos_sig_algs, tls_construct_ctos_sig_algs, final_sig_algs},
    ...
    *res = NULL;
    ...
    while (PACKET_remaining(&extensions) > 0)
    {
        ...
        if (!verify_extension(s, context, type, exts, raw_extensions,
            ...
            (type == TLSEXT_TYPE_psk && (context & SSL_EXT_CLIENT_HELLO) != 0 &&
        ...
        if (idx < OSSL_NELEM(ext_defs) &&
            ...
            type != TLSEXT_TYPE_cookie && type != TLSEXT_TYPE_renegotiate &&
        ...
        if (thisex != NULL)
        {
            ...
            if (s->ext.debug_cb)
                s->ext.debug_cb(
                    s, !s->server, thisex->type, PACKET_data(&thisex->data),
                    PACKET_remaining(&thisex->data), s->ext.debug_arg);
        }
    }
                             ...
                             X509* x, size_t chainidx)
{
    ...
    {
        EXT_RETURN (*construct)
        (SSL * s, WPACKET * pkt, unsigned int context, X509* x,
         size_t chainidx);
        EXT_RETURN ret;
        ...
        construct =
            s->server ? thisexd->construct_stoc : thisexd->construct_ctos;
...
static int final_server_name(SSL* s, unsigned int context, int sent)
{
    ...
    if (s->ctx->ext.servername_cb != NULL)
        ret = s->ctx->ext.servername_cb(s, &altmp, s->ctx->ext.servername_arg);
    else if (s->session_ctx->ext.servername_cb != NULL)
        ret = s->session_ctx->ext.servername_cb(
            s, &altmp, s->session_ctx->ext.servername_arg);
...
static int final_key_share(SSL* s, unsigned int context, int sent)
{
     ...
     */
    if (!s->server && !sent &&
        (!s->hit || (s->ext.psk_kex_mode & TLSEXT_KEX_MODE_FLAG_KE) == 0))
    {
    ...
    if (s->server)
    {
        ...
        else
        {
            ...
            if (!s->hit || (s->ext.psk_kex_mode & TLSEXT_KEX_MODE_FLAG_KE) == 0)
            {
                /* Nothing left we can do - just fail */
                SSLfatal(s,
...
static int final_early_data(SSL* s, unsigned int context, int sent)
{
    ...
    if (s->max_early_data == 0 || !s->hit ||
        ...
        (s->ctx->allow_early_data_cb != NULL &&
         !s->ctx->allow_early_data_cb(s, s->ctx->allow_early_data_cb_data)))
    {
    ...
    else
    {
        ...
        if (!tls13_change_cipher_state(s, SSL3_CC_EARLY |
...
static int final_maxfragmentlen(SSL* s, unsigned int context, int sent)
{
     ...
     */
    if (s->server && s->hit && USE_MAX_FRAGMENT_LENGTH_EXT(s->session) && !sent)
    {
