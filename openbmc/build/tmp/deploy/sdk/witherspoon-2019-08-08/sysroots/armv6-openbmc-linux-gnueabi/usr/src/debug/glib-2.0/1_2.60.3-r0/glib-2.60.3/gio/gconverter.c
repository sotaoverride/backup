 ...
 * @bytes_read: (out): will be set to the number of bytes read from @inbuf on
 *success
 * @bytes_written: (out): will be set to the number of bytes written to @outbuf
 *on success
 ...
 *%G_CONVERTER_CONVERTED each time until all data is consumed and all output is
 *produced, then %G_CONVERTER_FINISHED is returned instead. Note, that
 *%G_CONVERTER_FINISHED may be returned even if %G_CONVERTER_INPUT_AT_END is not
 *set, for instance in a decompression converter where the end of data is
 *detectable from the data (and there might even be other data after the end of
 *the compressed data).
 *
 ...
 * Since: 2.24
 **/
GConverterResult g_converter_convert(GConverter* converter, const void* inbuf,
                                     ...
                                     GError** error)
{
    ...
    *bytes_written = 0;
    ...
    return (*iface->convert)(converter, inbuf, inbuf_size, outbuf, outbuf_size,
                             flags, bytes_read, bytes_written, error);
}
