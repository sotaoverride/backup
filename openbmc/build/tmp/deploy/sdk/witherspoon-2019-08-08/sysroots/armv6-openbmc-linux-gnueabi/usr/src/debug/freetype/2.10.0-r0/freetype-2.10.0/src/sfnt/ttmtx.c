 ...
 */
FT_LOCAL_DEF(void)
tt_face_get_metrics(TT_Face face, FT_Bool vertical, FT_UInt gindex,
                    FT_Short* abearing, FT_UShort* aadvance)
{
    FT_Error error;
    ...
    if (k > 0)
    {
        if (gindex < (FT_UInt)k)
        {
            ...
            if (FT_STREAM_SEEK(table_pos) || FT_READ_USHORT(*aadvance) ||
