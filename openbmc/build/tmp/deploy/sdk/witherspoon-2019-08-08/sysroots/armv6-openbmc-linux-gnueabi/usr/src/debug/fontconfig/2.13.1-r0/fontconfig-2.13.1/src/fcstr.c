...
static FcChar8 FcStrCaseWalkerNext(FcCaseWalker* w, const char* delims)
{
    FcChar8 r;
...
int FcStrCmpIgnoreCaseAndDelims(const FcChar8* s1, const FcChar8* s2,
...
static FcBool FcStrIsAtIgnoreBlanksAndCase(const FcChar8* s1, const FcChar8* s2)
{
    FcCaseWalker w1, w2;
    FcChar8 c1, c2;
...
const FcChar8* FcStrContainsIgnoreBlanksAndCase(const FcChar8* s1,
                                                const FcChar8* s2)
{
    while (*s1)
    {
...
const FcChar8* FcStrContainsIgnoreCase(const FcChar8* s1, const FcChar8* s2)
{
    while (*s1)
    {
...
const FcChar8* FcStrContainsWord(const FcChar8* s1, const FcChar8* s2)
{
    ...
    while (s1len >= s2len)
    {
        if (wordStart && FcStrIsAtIgnoreCase(s1, s2) &&
...
int FcStrMatchIgnoreCaseAndDelims(const FcChar8* s1, const FcChar8* s2,
                                  const FcChar8* delims)
{
    FcCaseWalker w1, w2;
    FcChar8 c1, c2;
                  ...
                  int len) /* in bytes */
{
    ...
    if ((a & 0xfc00) == 0xd800)
    {
        ...
        if ((b & 0xfc00) != 0xdc00)
            return 0;
        result =
            ((((FcChar32)a & 0x3ff) << 10) | ((FcChar32)b & 0x3ff)) + 0x10000;
    }
    else
        result = a;
    *dst = result;
...
static FcChar8* FcStrCanonAbsoluteFilename(const FcChar8* s)
{
    FcChar8* file;
    FcChar8* f;
    const FcChar8* slash;
...
static FcBool _FcStrSetGrow(FcStrSet* set, int growElements)
{
...
FcBool FcStrSetAddLangs(FcStrSet* strs, const char* languages)
{
...
/* TODO Make public */
static FcStrSet* FcStrSetReference(FcStrSet* set)
{
