...
static void colors_debug(struct ul_color_ctl* cc)
{
    ...
    for (i = 0; i < ARRAY_SIZE(cc->scores); i++)
        printf("\tscore %s = %d\n",
               i == UL_COLORFILE_DISABLE
                   ? "disable"
                   : i == UL_COLORFILE_ENABLE
                         ? "enable"
 ...
 */
static int filename_to_tokens(const char* str, const char** name,
                              size_t* namesz, const char** term, size_t* termsz,
                              int* filetype)
{
...
static int colors_readdir(struct ul_color_ctl* cc, const char* dirname)
{
    ...
    while ((d = readdir(dir)))
    {
        ...
        if (d->d_type != DT_UNKNOWN && d->d_type != DT_LNK &&
            ...
            continue;
#endif
        if (filename_to_tokens(d->d_name, &tk_name, &tk_namesz, &tk_term,
                               &tk_termsz, &type) != 0)
            continue;
        ...
        DBG(CONF, ul_debug("item '%s': score=%d "
                           ...
                           d->d_name, score, cc->scores[type], tk_namesz,
                           tk_name, tk_termsz, tk_term));
        ...
        DBG(CONF,
            ul_debug("setting '%s' from %d -to-> %d",
                     type == UL_COLORFILE_SCHEME
                         ? "scheme"
                         : type == UL_COLORFILE_DISABLE
                               ? "disable"
 ...
 */
static int colors_add_scheme(struct ul_color_ctl* cc, char* name, char* seq0)
{
...
static void colors_sort_schemes(struct ul_color_ctl* cc)
{
    ...
    qsort(cc->schemes, cc->nschemes, sizeof(struct ul_color_scheme),
          cmp_scheme_name);
}
...
static int colors_read_schemes(struct ul_color_ctl* cc)
{
    int rc = 0;
    FILE* f = NULL;
    char buf[BUFSIZ], cn[129], seq[129];
...
{
    __UL_INIT_DEBUG_FROM_ENV(termcolors, TERMCOLORS_DEBUG_, 0,
                             TERMINAL_COLORS_DEBUG);
}
...
{
    size_t i;
    static const char* modes[] = {[UL_COLORMODE_AUTO] = "auto",
        ...
        switch (c)
        {
            ...
            default:
                fprintf(
                    ...
                    " -c, --color <red|blue|...>      color for the test "
                    "message\n"
                    " -C, --color-scheme <name>       color for the test "
                    "message\n"
