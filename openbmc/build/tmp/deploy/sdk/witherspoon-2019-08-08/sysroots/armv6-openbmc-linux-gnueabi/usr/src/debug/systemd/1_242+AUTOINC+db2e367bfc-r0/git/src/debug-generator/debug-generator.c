                                   ...
                                   void* data)
{
    ...
    else if (proc_cmdline_key_streq(key, "systemd.debug_shell"))
    {
        ...
        if (value)
        {
            ...
            if (r < 0)
                log_error("Failed to parse systemd.debug_shell= argument '%s', "
                          "ignoring.",
                          value);
            else
                arg_debug_shell = r;
    ...
    else if (streq(key, "systemd.unit"))
    {
        ...
        if (r < 0)
            return log_error_errno(r, "Failed to set default unit %s: %m",
                                   value);
    }
    else if (!value)
    {
...
static int generate_mask_symlinks(void)
{
    ...
    STRV_FOREACH(u, arg_mask)
    {
        ...
        if (symlink("/dev/null", p) < 0)
            r = log_error_errno(errno, "Failed to create mask symlink %s: %m",
                                p);
    }
...
static int generate_wants_symlinks(void)
{
    ...
    STRV_FOREACH(u, arg_wants)
    {
        ...
        if (symlink(f, p) < 0)
            r = log_error_errno(errno, "Failed to create wants symlink %s: %m",
                                p);
    }
