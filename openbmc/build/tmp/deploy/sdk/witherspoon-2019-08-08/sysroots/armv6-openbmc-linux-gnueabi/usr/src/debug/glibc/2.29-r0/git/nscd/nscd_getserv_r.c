                          ...
                          char* buf, size_t buflen, struct servent** result)
{
    ...
    memcpy(__mempcpy(__mempcpy(key, crit, critlen), "/", 1), proto ?: "",
           protolen + 1);
...
retry:;
    ...
    if (mapped != NO_MAPPING)
    {
        struct datahead* found =
            __nscd_cache_search(type, key, keylen, mapped, sizeof serv_resp);
        ...
        if (found != NULL)
        {
            ...
            {
                uint32_t* tmp;
                alloca_aliases_len = __libc_use_alloca(
                    alloca_used + (serv_resp.s_aliases_cnt * sizeof(uint32_t)));
    ...
    *result = NULL;
    ...
    if (serv_resp.found == 1)
    {
       ...
       align the pointer and the base of the h_addr_list pointers.  */
        ...
        align2 =
            ((__alignof__(char*) -
              ((cp + align1 + serv_resp.s_name_len + serv_resp.s_proto_len) -
               ((char*)0))) &
             (__alignof__(char*) - 1));
        if (buflen < (align1 + serv_resp.s_name_len + serv_resp.s_proto_len +
                      align2 + (serv_resp.s_aliases_cnt + 1) * sizeof(char*)))
        {
        no_room:
            __set_errno(ERANGE);
            retval = ERANGE;
        ...
        if (s_name == NULL)
        {
            ...
            if (serv_resp.s_aliases_cnt > 0)
            {
                ...
                {
                    aliases_len =
                        malloc(serv_resp.s_aliases_cnt * sizeof(uint32_t));
                    if (aliases_len == NULL)
                    {
                        retval = ENOMEM;
                        goto out_close;
        ...
        if (__builtin_expect((const char*)aliases_list + total_len > recend, 0))
        {
