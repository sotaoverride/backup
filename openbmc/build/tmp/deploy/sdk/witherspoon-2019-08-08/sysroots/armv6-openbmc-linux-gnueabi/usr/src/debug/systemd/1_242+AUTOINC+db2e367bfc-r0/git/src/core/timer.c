...
static int timer_add_default_dependencies(Timer* t)
{
    ...
    r = unit_add_dependency_by_name(UNIT(t), UNIT_BEFORE, SPECIAL_TIMERS_TARGET,
    ...
    if (MANAGER_IS_SYSTEM(UNIT(t)->manager))
    {
        r = unit_add_two_dependencies_by_name(
            UNIT(t), UNIT_AFTER, UNIT_REQUIRES, SPECIAL_SYSINIT_TARGET, true,
    ...
    return unit_add_two_dependencies_by_name(
        UNIT(t), UNIT_BEFORE, UNIT_CONFLICTS, SPECIAL_SHUTDOWN_TARGET, true,
        UNIT_DEPENDENCY_DEFAULT);
}
...
static int timer_add_trigger_dependencies(Timer* t)
{
    ...
    return unit_add_two_dependencies(UNIT(t), UNIT_BEFORE, UNIT_TRIGGERS, x,
...
static int timer_setup_persistent(Timer* t)
{
    ...
    if (MANAGER_IS_SYSTEM(UNIT(t)->manager))
    {
        ...
        t->stamp_path =
            strappend("/var/lib/systemd/timers/stamp-", UNIT(t)->id);
    }
    else
    {
        const char* e;
        ...
        else
        {
            ...
            t->stamp_path =
                strjoin(h, "/.local/share/systemd/timers/stamp-", UNIT(t)->id);
        }
    }
...
static void timer_dump(Unit* u, FILE* f, const char* prefix)
{
    ...
    fprintf(f,
            ...
            format_timespan(buf, sizeof(buf), t->accuracy_usec, 1), prefix,
            yes_no(t->remain_after_elapse), prefix, yes_no(t->on_clock_change),
    ...
    LIST_FOREACH(value, v, t->values)
    {
        ...
        if (v->base == TIMER_CALENDAR)
        {
            ...
            fprintf(f, "%s%s: %s\n", prefix, timer_base_to_string(v->base),
                    strna(p));
        }
        else
        {
...
static void timer_enter_dead(Timer* t, TimerResult f)
{
    ...
    unit_log_result(UNIT(t), t->result == TIMER_SUCCESS,
...
static void add_random(Timer* t, usec_t* v)
{
    ...
    log_unit_debug(UNIT(t), "Adding %s random time.",
...
static void timer_enter_waiting(Timer* t, bool time_change)
{
    ...
    LIST_FOREACH(value, v, t->values)
    {
        ...
        if (v->base == TIMER_CALENDAR)
        {
             ...
             * if the scheduled time has already passed, set the time when
             * systemd first started as the scheduled time.
             * Also, we don't have to check t->persistent since the logic
             * implicitly express true. */
            if (v->next_elapse <
                UNIT(t)
                    ->manager->timestamps[MANAGER_TIMESTAMP_USERSPACE]
                    .realtime)
                v->next_elapse =
                    UNIT(t)
                        ->manager->timestamps[MANAGER_TIMESTAMP_USERSPACE]
            ...
            if (!found_realtime)
                ...
                t->next_elapse_realtime =
                    MIN(t->next_elapse_realtime, v->next_elapse);
        ...
        else
        {
            ...
            v->next_elapse =
                usec_add(usec_shift_clock(base, CLOCK_MONOTONIC,
            ...
            if (!found_monotonic)
                ...
                t->next_elapse_monotonic_or_boottime =
                    MIN(t->next_elapse_monotonic_or_boottime, v->next_elapse);

            found_monotonic = true;
        }
    }

    if (!found_monotonic && !found_realtime && !t->on_timezone_change &&
        !t->on_clock_change)
    {
    ...
    if (found_monotonic)
    {
        ...
        left = usec_sub_unsigned(
            t->next_elapse_monotonic_or_boottime,
            triple_timestamp_by_clock(&ts, TIMER_MONOTONIC_CLOCK(t)));
        log_unit_debug(UNIT(t), "Monotonic timer elapses in %s.",
        ...
        else
        {
            ...
            r = sd_event_add_time(
                UNIT(t)->manager->event, &t->monotonic_event_source,
            ...
            (void)sd_event_source_set_description(t->monotonic_event_source,
                                                  "timer-monotonic");
        }
    }
    ...
    if (found_realtime)
    {
        ...
        log_unit_debug(
            UNIT(t), "Realtime timer elapses at %s.",
        ...
        else
        {
            r = sd_event_add_time(
                ...
                t->next_elapse_realtime, t->accuracy_usec, timer_dispatch, t);
            if (r < 0)
                goto fail;

            (void)sd_event_source_set_description(t->realtime_event_source,
                                                  "timer-realtime");
        }
    }
...
static void timer_enter_running(Timer* t)
{
    ...
    r = manager_add_job(UNIT(t)->manager, JOB_START, trigger, JOB_REPLACE, NULL,
                        &error, NULL);
    if (r < 0)
        goto fail;
    ...
    if (t->stamp_path)
        touch_file(t->stamp_path, true, t->last_trigger.realtime, UID_INVALID,
...
fail:
    log_unit_warning(UNIT(t), "Failed to queue unit startup job: %s",
...
static int timer_start(Unit* u)
{
    ...
    if (t->stamp_path)
    {
        ...
        if (stat(t->stamp_path, &st) >= 0)
        {
            ...
            /* Load the file timestamp, but only if it is actually in the past.
            ...
            else
            {
                ...
                log_unit_warning(u,
                                 "Not using persistent file timestamp %s as it "
                                 "is in the future.",
             ...
             */
            (void)touch_file(t->stamp_path, true, USEC_INFINITY, UID_INVALID,
                             GID_INVALID, MODE_INVALID);
    }
...
static int timer_serialize(Unit* u, FILE* f, FDSet* fds)
{
    ...
    if (t->last_trigger.realtime > 0)
        (void)serialize_usec(f, "last-trigger-realtime",
                             t->last_trigger.realtime);

    if (t->last_trigger.monotonic > 0)
        (void)serialize_usec(f, "last-trigger-monotonic",
                             t->last_trigger.monotonic);

    return 0;
}

static int timer_deserialize_item(Unit* u, const char* key, const char* value,
                                  FDSet* fds)
{
    Timer* t = TIMER(u);
