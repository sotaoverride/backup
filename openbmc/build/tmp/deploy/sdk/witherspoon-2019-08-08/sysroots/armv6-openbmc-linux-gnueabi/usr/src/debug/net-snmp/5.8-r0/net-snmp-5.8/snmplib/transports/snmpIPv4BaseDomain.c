...
int netsnmp_sockaddr_in(struct sockaddr_in* addr, const char* inpeername,
                        int remote_port)
{
                         ...
                         const char* default_target)
{
    ...
    if (inpeername != NULL && *inpeername != '\0')
    {
        ...
        if (host != NULL)
        {
            DEBUGMSGTL(("netsnmp_sockaddr_in", "check destination %s\n", host));
            ...
            else
            {
                ...
                {
                    DEBUGMSGTL(
                        ("netsnmp_sockaddr_in", "couldn't resolve hostname\n"));
                    free(peername);
                    return 0;
                }
                DEBUGMSGTL(
                           ...
                           const void* data, int len)
{
    ...
    if (t && t->flags & NETSNMP_TRANSPORT_FLAG_HOSTNAME)
    {
        ...
        host = netsnmp_gethostbyaddr(&to->sin_addr, sizeof(struct in_addr),
                                     AF_INET);
