...
static GVariantSerialised
    gvs_fixed_sized_maybe_get_child(GVariantSerialised value, gsize index_)
{
...
static gsize gvs_fixed_sized_maybe_needed_size(
    GVariantTypeInfo* type_info, GVariantSerialisedFiller gvs_filler,
    const gpointer* children, gsize n_children)
{
    if (n_children)
    {
        gsize element_fixed_size;
...
static void gvs_fixed_sized_maybe_serialise(GVariantSerialised value,
...
static GVariantSerialised
    gvs_variable_sized_maybe_get_child(GVariantSerialised value, gsize index_)
{
...
static gsize gvs_variable_sized_maybe_needed_size(
    GVariantTypeInfo* type_info, GVariantSerialisedFiller gvs_filler,
    const gpointer* children, gsize n_children)
{
    if (n_children)
...
static void gvs_variable_sized_maybe_serialise(
    GVariantSerialised value, GVariantSerialisedFiller gvs_filler,
    const gpointer* children, gsize n_children)
{
    if (n_children)
    {
...
static GVariantSerialised
    gvs_fixed_sized_array_get_child(GVariantSerialised value, gsize index_)
{
    GVariantSerialised child = {
        0,
    };
...
static gsize gvs_fixed_sized_array_needed_size(
    GVariantTypeInfo* type_info, GVariantSerialisedFiller gvs_filler,
    const gpointer* children, gsize n_children)
{
    gsize element_fixed_size;
...
static void gvs_fixed_sized_array_serialise(GVariantSerialised value,
                                            GVariantSerialisedFiller gvs_filler,
                                            const gpointer* children,
                                            gsize n_children)
{
...
static gboolean gvs_fixed_sized_array_is_normal(GVariantSerialised value)
{
    ...
    for (child.data = value.data; child.data < value.data + value.size;
         child.data += child.size)
    {
...
static inline void gvs_write_unaligned_le(guchar* bytes, gsize value,
                                          guint size)
{
    union
    {
...
static gsize gvs_variable_sized_array_n_children(GVariantSerialised value)
{
    ...
    last_end = gvs_read_unaligned_le(value.data + value.size - offset_size,
                                     offset_size);
...
static GVariantSerialised
    gvs_variable_sized_array_get_child(GVariantSerialised value, gsize index_)
{
    GVariantSerialised child = {
        0,
    };
    ...
    if (index_ > 0)
    {
        ...
        start = gvs_read_unaligned_le(
            value.data + last_end + (offset_size * (index_ - 1)), offset_size);
    ...
    end = gvs_read_unaligned_le(value.data + last_end + (offset_size * index_),
                                offset_size);
...
static gsize gvs_variable_sized_array_needed_size(
    GVariantTypeInfo* type_info, GVariantSerialisedFiller gvs_filler,
    const gpointer* children, gsize n_children)
{
    guint alignment;
...
static void gvs_variable_sized_array_serialise(
    GVariantSerialised value, GVariantSerialisedFiller gvs_filler,
    const gpointer* children, gsize n_children)
{
    guchar* offset_ptr;
...
static gboolean gvs_variable_sized_array_is_normal(GVariantSerialised value)
{
    ...
    for (i = 0; i < length; i++)
    {
        ...
        this_end =
            gvs_read_unaligned_le(offsets_array + offset_size * i, offset_size);
...
static GVariantSerialised gvs_tuple_get_child(GVariantSerialised value,
                                              gsize index_)
{
     ...
     */
    if
        G_UNLIKELY(value.data == NULL && value.size != 0)
        {
            g_variant_type_info_query(child.type_info, NULL, &child.size);
    ...
    if (index_ != g_variant_type_info_n_members(value.type_info) - 1)
    {
        ...
        last_child = gvs_tuple_get_child(
            value, g_variant_type_info_n_members(value.type_info) - 1);
...
static gsize gvs_tuple_needed_size(GVariantTypeInfo* type_info,
                                   GVariantSerialisedFiller gvs_filler,
                                   const gpointer* children, gsize n_children)
{
    ...
    for (i = 0; i < n_children; i++)
    {
        ...
        g_variant_type_info_query(member_info->type_info, &alignment,
                                  &fixed_size);
        offset += (-offset) & alignment;
...
static void gvs_tuple_serialise(GVariantSerialised value,
                                GVariantSerialisedFiller gvs_filler,
                                const gpointer* children, gsize n_children)
{
    gsize offset_size;
    ...
    for (i = 0; i < n_children; i++)
    {
        ...
        if (member_info->ending_type == G_VARIANT_MEMBER_ENDING_OFFSET)
        {
            ...
            gvs_write_unaligned_le(value.data + value.size, offset,
                                   offset_size);
        }
    }
        ...
        switch (member_info->ending_type)
        {
            ...
            case G_VARIANT_MEMBER_ENDING_OFFSET:
                ...
                if (offset_ptr < offset)
                    ...
                    gvs_read_unaligned_le(value.data + offset_ptr, offset_size);
                break;
...
static inline GVariantSerialised gvs_variant_get_child(GVariantSerialised value,
                                                       gsize index_)
{
    GVariantSerialised child = {
        0,
        ...
        if (value.data[child.size] == '\0')
        {
                ...
                end == limit)
            {
                ...
                if (g_variant_type_is_definite(type))
                {
                    ...
                    g_variant_type_info_query(child.type_info, NULL,
                                              &fixed_size);
                    child_type_depth =
                        g_variant_type_info_query_depth(child.type_info);

                    if ((!fixed_size || fixed_size == child.size) &&
                        value.depth <
                            G_VARIANT_MAX_RECURSION_DEPTH - child_type_depth)
                        return child;
...
static inline gsize gvs_variant_needed_size(GVariantTypeInfo* type_info,
                                            GVariantSerialisedFiller gvs_filler,
                                            const gpointer* children,
                                            gsize n_children)
{
...
static inline void gvs_variant_serialise(GVariantSerialised value,
                                         GVariantSerialisedFiller gvs_filler,
                                         const gpointer* children,
                                         gsize n_children)
{
 ...
 */
GVariantSerialised g_variant_serialised_get_child(GVariantSerialised serialised,
                                                  gsize index_)
{
    GVariantSerialised child;
    ...
    if
        G_LIKELY(index_ < g_variant_serialised_n_children(serialised))
        {
            DISPATCH_CASES(
                ...
                g_assert(g_variant_serialised_check(child)); return child;
                                    ...
                                    const gpointer* children, gsize n_children)
{
    ...
    DISPATCH_CASES(
        ...
        gvs_ /**/,
        /**/ _serialise(serialised, gvs_filler, children, n_children);
        return;
 ...
 */
gsize g_variant_serialiser_needed_size(GVariantTypeInfo* type_info,
                                       GVariantSerialisedFiller gvs_filler,
                                       const gpointer* children,
                                       gsize n_children)
{
    DISPATCH_CASES(
        ...
        return gvs_ /**/,
        /**/ _needed_size(type_info, gvs_filler, children, n_children);
