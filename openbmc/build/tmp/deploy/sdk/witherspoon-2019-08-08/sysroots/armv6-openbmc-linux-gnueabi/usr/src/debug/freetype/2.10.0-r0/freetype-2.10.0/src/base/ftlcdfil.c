...
ft_lcd_padding(FT_BBox* cbox, FT_GlyphSlot slot, FT_Render_Mode mode)
{
    ...
    if (lcd_filter_func == ft_lcd_filter_fir)
    {
        ...
        {
            cbox->xMin -= lcd_weights[0] ? 43 : lcd_weights[1] ? 22 : 0;
            cbox->xMax += lcd_weights[4] ? 43 : lcd_weights[3] ? 22 : 0;
        }
        else if (mode == FT_RENDER_MODE_LCD_V)
        {
            cbox->yMin -= lcd_weights[0] ? 43 : lcd_weights[1] ? 22 : 0;
            cbox->yMax += lcd_weights[4] ? 43 : lcd_weights[3] ? 22 : 0;
        }
    }
}
...
FT_BASE_DEF(void)
ft_lcd_filter_fir(FT_Bitmap* bitmap, FT_Render_Mode mode,
...
static void _ft_lcd_filter_legacy(FT_Bitmap* bitmap, FT_Render_Mode mode,
                                  FT_Byte* weights)
{
    FT_UInt width = (FT_UInt)bitmap->width;
    FT_UInt height = (FT_UInt)bitmap->rows;
...
{
    static const FT_LcdFiveTapFilter default_weights = {0x08, 0x4d, 0x56, 0x4d,
                                                        0x08};
    static const FT_LcdFiveTapFilter light_weights = {0x00, 0x55, 0x56, 0x55,
                                                      0x00};
    ...
    switch (filter)
    {
        ...
        case FT_LCD_FILTER_DEFAULT:
            ft_memcpy(library->lcd_weights, default_weights,
                      FT_LCD_FILTER_FIVE_TAPS);
            library->lcd_filter_func = ft_lcd_filter_fir;
            break;

        case FT_LCD_FILTER_LIGHT:
            ft_memcpy(library->lcd_weights, light_weights,
                      FT_LCD_FILTER_FIVE_TAPS);
            library->lcd_filter_func = ft_lcd_filter_fir;
            break;
...
FT_EXPORT_DEF(FT_Error)
FT_Library_SetLcdGeometry(FT_Library library, FT_Vector* sub)
{
    FT_UNUSED(library);
    FT_UNUSED(sub);
