...
static void display_stats(void)
{
    printf("\nrndrm99 stats:\n"
           ...
           (unsigned)files_created, (unsigned)files_removed,
           (unsigned)dirs_created, (unsigned)dirs_removed,
...
static void after(const char* msg)
{
    ...
    if (diff >= 8)
    {
        printf("\nrndrm99: the following fn took more than 8 seconds: %s (took "
               "%u secs)\n",
               msg, (unsigned)diff);
        fflush(stdout);
        display_stats();
...
static void rndrm99(void)
{
    ...
    for (;;)
    {
        ...
        do
        {
            ...
            else
            {
                ...
                if (!this_size)
                 ...
                 (tests_size_parameter == 0 || size < tests_size_parameter));
        ...
        do
        {
            ...
            (tests_size_parameter != 0 && size > tests_size_parameter / 10) ||
...
{
    return "Create a directory named rndrm99_test_dir_pid, where "
