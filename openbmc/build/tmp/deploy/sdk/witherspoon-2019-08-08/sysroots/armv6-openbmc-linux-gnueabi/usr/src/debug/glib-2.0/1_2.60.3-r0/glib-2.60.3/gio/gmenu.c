...
static gboolean g_menu_is_mutable(GMenuModel* model)
{
...
static gint g_menu_get_n_items(GMenuModel* model)
{
    GMenu* menu = G_MENU(model);
...
static void g_menu_get_item_attributes(GMenuModel* model, gint position,
                                       GHashTable** table)
{
    GMenu* menu = G_MENU(model);

    *table = g_hash_table_ref(
        g_array_index(menu->items, struct item, position).attributes);
}

static void g_menu_get_item_links(GMenuModel* model, gint position,
                                  GHashTable** table)
{
    GMenu* menu = G_MENU(model);

    *table = g_hash_table_ref(
        g_array_index(menu->items, struct item, position).links);
}
 ...
 * Since: 2.32
 */
void g_menu_insert(GMenu* menu, gint position, const gchar* label,
 ...
 * Since: 2.32
 */
void g_menu_prepend(GMenu* menu, const gchar* label,
                    const gchar* detailed_action)
{
 ...
 * Since: 2.32
 */
void g_menu_append(GMenu* menu, const gchar* label,
                   const gchar* detailed_action)
{
 ...
 * Since: 2.32
 */
void g_menu_insert_section(GMenu* menu, gint position, const gchar* label,
                           GMenuModel* section)
{
    GMenuItem* menu_item;
 ...
 * Since: 2.32
 */
void g_menu_prepend_section(GMenu* menu, const gchar* label,
                            GMenuModel* section)
{
 ...
 * Since: 2.32
 */
void g_menu_insert_submenu(GMenu* menu, gint position, const gchar* label,
                           GMenuModel* submenu)
{
    GMenuItem* menu_item;
 ...
 * Since: 2.32
 */
void g_menu_prepend_submenu(GMenu* menu, const gchar* label,
                            GMenuModel* submenu)
{
...
static void g_menu_clear_item(struct item* item)
{
...
static void g_menu_finalize(GObject* object)
{
    ...
    G_OBJECT_CLASS(g_menu_parent_class)->finalize(object);
}

static void g_menu_init(GMenu* menu)
{
    menu->items = g_array_new(FALSE, FALSE, sizeof(struct item));
    menu->mutable = TRUE;
}

static void g_menu_class_init(GMenuClass* class)
{
...
static void g_menu_item_clear_cow(GMenuItem* menu_item)
{
    if (menu_item->cow)
    {
        GHashTableIter iter;
        ...
        new = g_hash_table_new_full(g_str_hash, g_str_equal, g_free,
...
static void g_menu_item_finalize(GObject* object)
{
    ...
    G_OBJECT_CLASS(g_menu_item_parent_class)->finalize(object);
}

static void g_menu_item_init(GMenuItem* menu_item)
{
...
static void g_menu_item_class_init(GMenuItemClass* class)
{
 ...
 */
static gboolean valid_attribute_name(const gchar* name)
{
    gint i;
    ...
    for (i = 1; name[i]; i++)
    {
        if (name[i] != '-' && !g_ascii_islower(name[i]) &&
                                     ...
                                     const gchar* attribute, GVariant* value)
{
    ...
    if (value != NULL)
        g_hash_table_insert(menu_item->attributes, g_strdup(attribute),
 ...
 * Since: 2.32
 */
void g_menu_item_set_attribute(GMenuItem* menu_item, const gchar* attribute,
 ...
 * Since: 2.32
 */
void g_menu_item_set_link(GMenuItem* menu_item, const gchar* link,
                          GMenuModel* model)
{
    g_return_if_fail(G_IS_MENU_ITEM(menu_item));
    ...
    if (model != NULL)
        g_hash_table_insert(menu_item->links, g_strdup(link),
                            g_object_ref(model));
    else
 ...
 * Since: 2.34
 */
gboolean g_menu_item_get_attribute(GMenuItem* menu_item, const gchar* attribute,
                                   const gchar* format_string, ...)
{
    GVariant* value;
    va_list ap;
                                             ...
                                             GVariant* target_value)
{
    ...
    g_menu_item_set_attribute_value(menu_item, G_MENU_ATTRIBUTE_ACTION,
                                    action_value);
    g_menu_item_set_attribute_value(menu_item, G_MENU_ATTRIBUTE_TARGET,
                                    target_value);
}
