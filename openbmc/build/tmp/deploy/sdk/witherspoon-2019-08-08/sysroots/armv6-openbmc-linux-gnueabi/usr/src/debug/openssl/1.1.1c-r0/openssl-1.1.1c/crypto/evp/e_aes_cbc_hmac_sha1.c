                                        ...
                                        const unsigned char* iv, int enc)
{
    ...
    if (enc)
        ret = aesni_set_encrypt_key(inkey, EVP_CIPHER_CTX_key_length(ctx) * 8,
                                    &key->ks);
    else
        ret = aesni_set_decrypt_key(inkey, EVP_CIPHER_CTX_key_length(ctx) * 8,
                                    &key->ks);
    ...
    else
    {
        ...
        { /* "TLS" mode of operation */
            ...
            if (stitch)
            {
                ...
                aesni256_cbc_sha1_dec(in + aes_off, out + aes_off, blocks,
                                      &key->ks, ctx->iv, &key->md,
                                      out + sha_off);
                                    ...
                                    void* ptr)
{
    ...
    switch (type)
    {
        ...
        case EVP_CTRL_TLS1_1_MULTIBLOCK_ENCRYPT:
        {
            ...
            return (int)tls1_1_multi_block_encrypt(
                key, param->out, param->inp, param->len, param->interleave / 4);
        }
