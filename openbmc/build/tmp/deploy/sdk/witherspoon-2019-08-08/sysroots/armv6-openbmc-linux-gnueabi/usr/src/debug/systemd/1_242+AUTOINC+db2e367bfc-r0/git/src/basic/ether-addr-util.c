                           ...
                           char buffer[ETHER_ADDR_TO_STRING_MAX])
{
    ...
    sprintf(buffer, "%02x:%02x:%02x:%02x:%02x:%02x", addr->ether_addr_octet[0],
...
static void ether_addr_hash_func(const struct ether_addr* p,
                                 struct siphash* state)
{
    siphash24_compress(p, sizeof(struct ether_addr), state);
}

DEFINE_HASH_OPS(ether_addr_hash_ops, struct ether_addr, ether_addr_hash_func,
                ether_addr_compare);
...
#define parse_fields(v)                                                        \
    ...
    if (sep == '.')
    {
        ...
        {
            ret->ether_addr_octet[2 * n] =
                ((shorts[n] & (uint16_t)0xff00) >> 8);
