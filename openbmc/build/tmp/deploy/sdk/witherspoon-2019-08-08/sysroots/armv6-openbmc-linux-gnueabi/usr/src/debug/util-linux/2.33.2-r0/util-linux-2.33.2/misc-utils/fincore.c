...
static struct colinfo infos[] = {
    [COL_PAGES] = {"PAGES", 1, SCOLS_FL_RIGHT,
                   N_("file data resident in memory in pages")},
    [COL_RES] = {"RES", 5, SCOLS_FL_RIGHT,
...
struct fincore_control
{
    ...
    unsigned int bytes : 1, noheadings : 1, raw : 1, json : 1;
};
...
static int add_output_data(struct fincore_control* ctl, const char* name,
                           off_t file_size, off_t count_incore)
{
    size_t i;
    char* tmp;
...
static int do_mincore(struct fincore_control* ctl, void* window,
                      const size_t len, const char* name, off_t* count_incore)
{
...
static int fincore_fd(struct fincore_control* ctl, int fd, const char* name,
                      off_t file_size, off_t* count_incore)
{
...
static int fincore_name(struct fincore_control* ctl, const char* name,
                        struct stat* sb, off_t* count_incore)
{
    int fd;
    int rc = 0;
...
int main(int argc, char** argv)
{
    ...
    struct fincore_control ctl = {.pagesize = getpagesize()};
