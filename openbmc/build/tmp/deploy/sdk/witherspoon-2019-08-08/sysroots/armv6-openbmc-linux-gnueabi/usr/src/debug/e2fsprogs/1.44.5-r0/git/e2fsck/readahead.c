                               ...
                               void* priv_data)
{
    ...
    e2_blkcnt_t count =
        (pr->flags & E2FSCK_RA_DBLIST_IGNORE_BLOCKCNT ? 1 : db->blockcnt);
    ...
    if (!pr->run_len || db->blk != pr->run_start + pr->run_len)
    {
        ...
        {
            pr->err =
                io_channel_cache_readahead(fs->io, pr->run_start, pr->run_len);
            dbg_printf("readahead start=%llu len=%llu err=%d\n", pr->run_start,
                       pr->run_len, (int)pr->err);
        }
        pr->run_start = db->blk;
        pr->run_len = 0;
    }
                                  ...
                                  unsigned long long count)
{
    ...
    pr.flags = flags;
    err =
        ext2fs_dblist_iterate3(dblist, readahead_dir_block, start, count, &pr);
    if (pr.err)
        return pr.err;
    if (err)
        return err;
                                         ...
                                         ext2fs_block_bitmap ra_map)
{
    ...
    {
        start = out;
        err = ext2fs_find_first_zero_block_bitmap2(ra_map, start, end, &out);
        if (err == ENOENT)
        {
            out = end;
            err = 0;
...
static errcode_t mark_bmap_range(ext2fs_block_bitmap map, blk64_t blk,
                                 unsigned int num)
{
                           ...
                           dgrp_t ngroups)
{
    ...
    err = ext2fs_allocate_block_bitmap(fs, "readahead bitmap", &ra_map);
    if (err)
        return err;

    for (i = start; i < end; i++)
    {
        err = ext2fs_super_and_bgd_loc2(fs, i, &super, &old_gdt, &new_gdt,
                                        &blocks);
        if (err)
            break;
        ...
        {
            err = mark_bmap_range(ra_map, old_gdt ? old_gdt : new_gdt, blocks);
            if (err)
                break;
        }
        ...
        if ((flags & E2FSCK_READA_BBITMAP) &&
            ...
            ext2fs_bg_free_blocks_count(fs, i) < fs->super->s_blocks_per_group)
        {
        ...
        if ((flags & E2FSCK_READA_IBITMAP) &&
            ...
            ext2fs_bg_free_inodes_count(fs, i) < fs->super->s_inodes_per_group)
        {
