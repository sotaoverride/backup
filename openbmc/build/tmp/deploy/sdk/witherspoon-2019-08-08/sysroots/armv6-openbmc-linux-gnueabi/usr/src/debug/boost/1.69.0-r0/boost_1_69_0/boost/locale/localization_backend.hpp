...
///
/// \brief this class represents a localization backend that can be used for
/// localizing your application.
///
/// Backends are usually registered inside the localization backends manager and
/// allow transparent support of different backends, so a user can switch the
/// backend by simply linking the application to the correct one.
///
/// Backends may support different tuning options, but these are the default
/// options available to the user for all of them
///
/// -# \c locale - the name of the locale in POSIX format like en_US.UTF-8
/// -# \c use_ansi_encoding - select system locale using ANSI codepages rather
/// then UTF-8 under Windows
///     by default
/// -# \c message_path - path to the location of message catalogs (vector of
/// strings)
/// -# \c message_application - the name of applications that use message
/// catalogs (vector of strings)
///
/// Each backend can be installed with a different default priotiry so when you
/// work with two different backends, you can specify priotiry so this backend
/// will be chosen according to their priority.
///
...
class localization_backend
{
  ...
  public:
    ...
    ///
    virtual void set_option(std::string const& name,
    ...
    virtual std::locale install(std::locale const& base,
                                ...
                                character_facet_type type = nochar_facet) = 0;

}; // localization_backend

///
/// \brief Localization backend manager is a class that holds various backend
/// and allows creation of their combination or selection
///
...
class BOOST_LOCALE_DECL localization_backend_manager
{
  public:
    ...
    ///
    /// Add new backend to the manager, each backend should be uniquely defined
    /// by its name.
    ///
    /// This library provides: "icu", "posix", "winapi" and "std" backends.
    ///
    void add_backend(std::string const& name,
                     std::auto_ptr<localization_backend> backend);
#endif

    ///
    /// Create new localization backend according to current settings. Ownership
    /// is passed to caller
    ///
    localization_backend* create() const;
    ///
    /// Add new backend to the manager, each backend should be uniquely defined
    /// by its name. ownership on backend is transfered
    ///
    /// This library provides: "icu", "posix", "winapi" and "std" backends.
    ///
    ...
    ///
    /// Select specific backend by name for a category \a category. It allows
    /// combining different backends for user preferences.
    ///
    void select(std::string const& backend_name,
    ...
    static localization_backend_manager
        global(localization_backend_manager const&);
    ///
