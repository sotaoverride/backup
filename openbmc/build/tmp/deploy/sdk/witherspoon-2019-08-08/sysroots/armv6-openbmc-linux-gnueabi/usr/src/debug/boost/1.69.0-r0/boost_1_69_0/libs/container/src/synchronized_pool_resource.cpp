...
class thread_mutex_lock
{
  ...
  public:
    explicit thread_mutex_lock(thread_mutex& m) : m_mut(m)
    {
        m_mut.lock();
    }
...
synchronized_pool_resource::synchronized_pool_resource(
    const pool_options& opts, memory_resource* upstream) BOOST_NOEXCEPT
    : m_mut(),
      m_pool_resource(opts, upstream)
{
}
...
synchronized_pool_resource::synchronized_pool_resource(
    memory_resource* upstream) BOOST_NOEXCEPT : m_mut(),
                                                m_pool_resource(upstream)
{
}

synchronized_pool_resource::synchronized_pool_resource(const pool_options& opts)
    BOOST_NOEXCEPT : m_mut(),
                     m_pool_resource(opts)
{
}
...
void* synchronized_pool_resource::do_allocate(std::size_t bytes,
...
void synchronized_pool_resource::do_deallocate(void* p, std::size_t bytes,
                                               std::size_t alignment) // virtual
{
    thread_mutex_lock lck(m_mut);
    (void)lck;
...
bool synchronized_pool_resource::do_is_equal(const memory_resource& other) const
...
std::size_t synchronized_pool_resource::pool_next_blocks_per_chunk(
    std::size_t pool_idx) const
{
...
std::size_t
    synchronized_pool_resource::pool_cached_blocks(std::size_t pool_idx) const
{
