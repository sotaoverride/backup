                          ...
                          const krb5_data* n1, const krb5_data* n2)
{
    ...
    Tprintf(("process_intermediates(%.*s,%.*s)\n", (int)n1->length, n1->data,
             (int)n2->length, n2->data));
    ...
    if (len1 == len2)
    {
        ...
        {
            Tprintf(
                ("equal length but different strings in path: '%.*s' '%.*s'\n",
...
static krb5_error_code maybe_join(krb5_data* last, krb5_data* buf,
                                  unsigned int bufsiz)
{
    if (buf->length == 0)
        return 0;
...
/* The input strings cannot contain any \0 bytes, according to the
   ...
   of C strings.  */
static krb5_error_code
    foreach_realm(krb5_error_code (*fn)(krb5_data* comp, void* data),
                  void* data, const krb5_data* crealm, const krb5_data* srealm,
...
static krb5_error_code check_realm_in_list(krb5_data* realm, void* data)
{
...
krb5_error_code krb5_check_transited_list(krb5_context ctx,
                                          const krb5_data* trans_in,
                                          const krb5_data* crealm,
                                          const krb5_data* srealm)
{
    krb5_data trans;
    ...
    Tprintf(("krb5_check_transited_list(trans=\"%.*s\", crealm=\"%.*s\", "
             "srealm=\"%.*s\")\n",
             (int)trans.length, trans.data, (int)crealm->length, crealm->data,
...
static krb5_error_code print_a_realm(krb5_data* realm, void* data)
{
    printf("%.*s\n", (int)realm->length, realm->data);
    return 0;
}
