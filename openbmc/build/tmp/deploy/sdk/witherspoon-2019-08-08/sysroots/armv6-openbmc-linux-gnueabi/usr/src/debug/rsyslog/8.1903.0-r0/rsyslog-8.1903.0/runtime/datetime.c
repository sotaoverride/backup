...
static const long long yearInSecs[] = {
    -63158401,  -31536001,  -1,         31535999,   63071999,   94694399,
    126230399,  157766399,  189302399,  220924799,  252460799,  283996799,
    315532799,  347155199,  378691199,  410227199,  441763199,  473385599,
    504921599,  536457599,  567993599,  599615999,  631151999,  662687999,
    694223999,  725846399,  757382399,  788918399,  820454399,  852076799,
    883612799,  915148799,  946684799,  978307199,  1009843199, 1041379199,
    1072915199, 1104537599, 1136073599, 1167609599, 1199145599, 1230767999,
    1262303999, 1293839999, 1325375999, 1356998399, 1388534399, 1420070399,
    1451606399, 1483228799, 1514764799, 1546300799, 1577836799, 1609459199,
    1640995199, 1672531199, 1704067199, 1735689599, 1767225599, 1798761599,
    1830297599, 1861919999, 1893455999, 1924991999, 1956527999, 1988150399,
    2019686399, 2051222399, 2082758399, 2114380799, 2145916799, 2177452799,
    2208988799, 2240611199, 2272147199, 2303683199, 2335219199, 2366841599,
    2398377599, 2429913599, 2461449599, 2493071999, 2524607999, 2556143999,
    2587679999, 2619302399, 2650838399, 2682374399, 2713910399, 2745532799,
    2777068799, 2808604799, 2840140799, 2871763199, 2903299199, 2934835199,
    2966371199, 2997993599, 3029529599, 3061065599, 3092601599, 3124223999,
    3155759999, 3187295999, 3218831999, 3250454399, 3281990399, 3313526399,
    3345062399, 3376684799, 3408220799, 3439756799, 3471292799, 3502915199,
    3534451199, 3565987199, 3597523199, 3629145599, 3660681599, 3692217599,
    3723753599, 3755375999, 3786911999, 3818447999, 3849983999, 3881606399,
    3913142399, 3944678399, 3976214399, 4007836799, 4039372799, 4070908799,
    4102444799};
 ...
 */
static void timeval2syslogTime(struct timeval* tp, struct syslogTime* t,
                               const int inUTC)
{
    struct tm* tm;
    struct tm tmBuf;
    ...
    else
    {
         ...
         * It is UTC - localtime, which is the opposite sign of mins east of
         * GMT.
         */
                        ...
                        const int inUTC)
{
     ...
     *           This is achieved by adding a check to _AIX wherever _hpux is
     * checked
     */
 ...
 */
static rsRetVal ParseTIMESTAMP3339(struct syslogTime* pTime, uchar** ppszTS,
                                   int* pLenStr)
{
    uchar* pszTS = *ppszTS;
 ...
 */
static rsRetVal ParseTIMESTAMP3164(struct syslogTime* pTime, uchar** ppszTS,
                                   int* pLenStr, const int bParseTZ,
                                   const int bDetectYearAfterTime)
{
    ...
    /* first check if we have a year in front of the timestamp. some devices
     * (e.g. Brocade) do this. As it is pretty straightforward to detect and
     * chance of misinterpretation is low, we try to parse it.
     */
    if (*pszTS >= '0' && *pszTS <= '9')
    {
    ...
    /* If we look at the month (Jan, Feb, Mar, Apr, May, Jun, Jul, Aug, Sep,
     * Oct, Nov, Dec), we may see the following character sequences occur:
     *
     ...
     *
     * 2009-08-17: we now do case-insensitive comparisons, as some devices
     * obviously do not obey to the RFC-specified case. As we need to guess in
     * any case, we can ignore case in the first place -- rgerhards
     *
     * 2005-07-18, well sometimes it pays to be a bit more verbose, even in C...
     * Fixed a bug that lead to invalid detection of the data. The issue was
     * that we had an if(++pszTS == 'x') inside of some of the consturcts below.
     * However, there were also some elseifs (doing the same ++), which than
     * obviously did not check the orginal character but the next one. Now
     * removed the ++ and put it into the statements below. Was a really nasty
     * bug... I didn't detect it before june, when it first manifested. This
     * also lead to invalid parsing of the rest of the message, as the time
     * stamp was not detected to be correct. - rgerhards
     */
    switch (*pszTS++)
    {
        case 'j':
    ...
    if (year == 0 && hour > 1970 && hour < 2100)
    {
         ...
         * to how long the code survived - me no longer ;)) -- rgerhards,
         * 2008-11-18
         */
        year = hour;
    ...
    if (bParseTZ && lenStr > 2 && *pszTS == ' ')
    {
        ...
        for (++pszTS, --lenStr, i = 0;
             lenStr > 0 && i < (int)sizeof(tzstring) - 1 && *pszTS != ':' &&
             *pszTS != ' ';
             --lenStr)
            tzstring[i++] = *pszTS++;
        if (i > 0)
        {
            ...
            {
                DBGPRINTF(
                    "ParseTIMESTAMP3164: invalid TZ string '%s' -- ignored\n",
                    tzstring);
            }
            else
            {
...
static int formatTimestamp3164(struct syslogTime* ts, char* pBuf, int bBuggyDay)
{
    ...
    pBuf[3] = ' ';
    iDay = (ts->day / 10) %
           10; /* we need to write a space if the first digit is 0 */
...
static time_t syslogTime2time_t(const struct syslogTime* ts)
{
    ...
    switch (ts->month)
    {
        ...
        case 2:
            MonthInDays =
                31; // until 01 of February - leap year handling down below!
            break;
        case 3:
    ...
    /*	1) Counting how many Years have passed since 1970
        2) Counting how many Days have passed since the 01.01 of the selected
       Year (Day level) according to the Selected Month and Day. Last day
       doesn't count, it should be until last day 3) Calculating this period
       (NumberOfDays) in seconds*/

    NumberOfYears = ts->year - yearInSec_startYear - 1;
    NumberOfDays = MonthInDays + ts->day - 1;
    TimeInUnixFormat =
        (time_t)(yearInSecs[NumberOfYears] + 1) + NumberOfDays * 86400;
    ...
    if (ts->OffsetMode == '+')
        utcOffset *=
            -1; /* if timestamp is ahead, we need to "go back" to UTC */
 ...
 */
static int formatUnixTimeFromTime_t(time_t unixtime, const char* format,
                                    char* pBuf,
                                    __attribute__((unused)) uint pBufMax)
{
    ...
    if (strcmp(format, "date-rfc3164") == 0)
    {
        ...
        // MMM dd HH:mm:ss
        sprintf(pBuf, "%s %2d %.2d:%.2d:%.2d", monthNames[lt.tm_mon],
    ...
    else if (strcmp(format, "date-rfc3339") == 0)
    {
        ...
        sprintf(pBuf, "%d-%.2d-%.2dT%.2d:%.2d:%.2dZ", lt.tm_year + 1900,
...
BEGINobjQueryInterface(datetime)
    CODESTARTobjQueryInterface(datetime) if (pIf->ifVersion !=
                                             datetimeCURR_IF_VERSION)
...
finalize_it : ENDobjQueryInterface(datetime)

              /* Initialize the datetime class. Must be called as the very first
               * method before anything else is called inside this class.
               * rgerhards, 2008-02-19
               */
              BEGINAbstractObjClassInit(datetime, 1,
                                        OBJ_IS_CORE_MODULE) /* class, version */
