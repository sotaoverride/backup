...
{
    rfbTightClientPtr rtcp = (rfbTightClientPtr)rfbGetExtensionClientData(
        cl, &tightVncFileTransferExtension);
    if (rtcp == NULL)
    {
...
static void rfbVncAuthSendChallenge(rfbClientPtr cl)
{
    ...
    /* 4 byte header is alreay sent. Which is rfbSecTypeVncAuth (same as
     * rfbVncAuth). Just send the challenge. */
...
#define SECTYPE_TIGHT_FOR_RFB_3_8                                              \
    ...
    {                                                                          \
        uint32_t authResult;                                                   \
        rfbLog("rfbProcessClientSecurityType: returning securityResult for "   \
...
void rfbSendInteractionCaps(rfbClientPtr cl)
{
    ...
    if (rfbWriteExact(cl, (char*)&intr_caps, sz_rfbInteractionCapsMsg) < 0 ||
...
static rfbBool handleMessage(rfbClientPtr cl, const char* messageName,
                             void (*handler)(rfbClientPtr cl,
...
rfbBool rfbTightExtensionMsgHandler(struct _rfbClientRec* cl, void* data,
                                    const rfbClientToServerMsg* msg)
{
    switch (msg->type)
    {
        ...
        case rfbFileDownloadRequest:
            ...
            return handleMessage(cl, "rfbFileDownloadRequest",
                                 HandleFileDownloadRequest);

        case rfbFileUploadRequest:

            return handleMessage(cl, "rfbFileUploadRequest",
                                 HandleFileUploadRequest);

        case rfbFileUploadData:

            return handleMessage(cl, "rfbFileUploadDataRequest",
                                 HandleFileUploadDataRequest);

        case rfbFileDownloadCancel:

            return handleMessage(cl, "rfbFileDownloadCancelRequest",
                                 HandleFileDownloadCancelRequest);

        case rfbFileUploadFailed:

            return handleMessage(cl, "rfbFileUploadFailedRequest",
                                 HandleFileUploadFailedRequest);

        case rfbFileCreateDirRequest:

            return handleMessage(cl, "rfbFileCreateDirRequest",
...
/*
 * This method should be registered to libvncserver to handle rfbSecTypeTight
 * security type.
 */
...
rfbProtocolExtension tightVncFileTransferExtension = {
    NULL,          rfbTightExtensionInit,       NULL,
    NULL,          rfbTightExtensionMsgHandler, rfbTightExtensionClientClose,
    rfbTightUsage, rfbTightProcessArg,          NULL};

static rfbSecurityHandler tightVncSecurityHandler = {
    rfbSecTypeTight, rfbHandleSecTypeTight, NULL};
