...
void activation_deinit(Activation* activation)
{
    ...
    while ((message = c_list_first_entry(&activation->activation_messages,
                                         ActivationMessage, link)))
        activation_message_free(message);

    while ((request = c_list_first_entry(&activation->activation_requests,
                                         ActivationRequest, link)))
...
int activation_queue_message(Activation* activation, User* user,
                             NameOwner* names, PolicySnapshot* policy,
                             Message* m)
{
    ...
    r = user_charge(activation->user, &message->charges[0], user,
                    USER_SLOT_BYTES,
                    sizeof(ActivationMessage) + sizeof(Message) + m->n_data);
    r = r
            ?: user_charge(activation->user, &message->charges[1], user,
                           USER_SLOT_FDS, fdlist_count(m->fds));
    if (r)
...
int activation_queue_request(Activation* activation, User* user,
                             uint64_t sender_id, uint32_t serial)
{
    ...
    r = user_charge(activation->user, &request->charge, user, USER_SLOT_BYTES,
                    sizeof(ActivationRequest));
    if (r)
        return (r == USER_E_QUOTA) ? ACTIVATION_E_QUOTA : error_fold(r);
