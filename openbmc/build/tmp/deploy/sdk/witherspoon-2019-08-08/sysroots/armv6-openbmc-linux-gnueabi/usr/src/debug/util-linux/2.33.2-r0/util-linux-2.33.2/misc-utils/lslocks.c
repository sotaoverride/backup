...
struct lock
{
    ...
    off_t start;
    off_t end;
    unsigned int mandatory : 1, blocked : 1;
    uint64_t size;
    int id;
};
...
static int get_local_locks(struct list_head* locks)
{
    ...
    while (fgets(buf, sizeof(buf), fp))
    {
        ...
        for (tok = strtok(buf, " "), i = 0; tok; tok = strtok(NULL, " "), i++)
        {
            ...
            switch (i)
            {
                ...
                case 2: /* is this a mandatory lock? other values are advisory
                           or noinode */
...
static void add_scols_line(struct libscols_table* table, struct lock* l,
                           struct list_head* locks)
{
    size_t i;
    ...
    for (i = 0; i < ncolumns; i++)
    {
        ...
        switch (get_column_id(i))
        {
            ...
            case COL_BLOCKER:
            {
                pid_t bl = l->blocked && l->id ? get_blocker(l->id, locks) : 0;
                if (bl)
...
static void __attribute__((__noreturn__)) usage(void)
{
    ...
    fputs(
        _(" -p, --pid <pid>        display only locks held by this process\n"),
        out);
...
int main(int argc, char* argv[])
{
    ...
    while ((c = getopt_long(argc, argv, "biJp:o:nruhV", long_opts, NULL)) != -1)
    {
