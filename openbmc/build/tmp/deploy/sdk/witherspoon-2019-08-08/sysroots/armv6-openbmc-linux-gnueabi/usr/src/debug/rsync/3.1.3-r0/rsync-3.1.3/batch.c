...
static char* flag_name[] = {
    "--recurse (-r)",  "--owner (-o)",      "--group (-g)",    "--links (-l)",
    "--devices (-D)",  "--hard-links (-H)", "--checksum (-c)", "--dirs (-d)",
    "--compress (-z)", "--iconv",           "--acls (-A)",     "--xattrs (-X)",
    "--inplace",       "--append",          "--append-verify", NULL};
...
void check_batch_flags(void)
{
    ...
    for (i = 0; flag_ptr[i]; i++)
    {
        ...
        if (*flag_ptr[i] != set)
        {
            ...
            if (INFO_GTE(MISC, 1))
            {
                rprintf(FINFO, "%sing the %s option to match the batchfile.\n",
...
static int write_arg(int fd, char* arg)
{
    ...
    if (strpbrk(arg, " \"'&;|[]()$#!*?^\\") != NULL)
    {
        ...
        {
            if (write(fd, s, x - s + 1) != x - s + 1 || write(fd, "'", 1) != 1)
                ret = -1;
        }
        len = strlen(s);
        if (write(fd, s, len) != len || write(fd, "'", 1) != 1)
            ret = -1;
        return ret;
    }
...
void write_batch_shell_file(int argc, char* argv[], int file_arg_cnt)
{
    ...
    for (i = 1; i < argc - file_arg_cnt; i++)
    {
        ...
        if (strncmp(p, "--files-from", 12) == 0 ||
            strncmp(p, "--filter", 8) == 0 || strncmp(p, "--include", 9) == 0 ||
            ...
            strncmp(p, "--only-write-batch", len = 18) == 0)
        {
            ...
            if (p[len] == '=')
            {
                if (write(fd, "=", 1) != 1 || write_arg(fd, p + len + 1) < 0)
                    err = 1;
            }
        }
        else
    ...
    if (!(p = check_for_hostspec(argv[argc - 1], &p, &i)))
        p = argv[argc - 1];
    if (write(fd, " ${1:-", 6) != 6 || write_arg(fd, p) < 0)
        err = 1;
    write_byte(fd, '}');
    if (filter_list.head)
