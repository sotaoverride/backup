...
static const FcObjectType* FcObjectFindById(FcObject object)
{
...
FcBool FcNameRegisterObjectTypes(const FcObjectType* types, int ntypes)
{
    /* Deprecated. */
    return FcFalse;
}

FcBool FcNameUnregisterObjectTypes(const FcObjectType* types, int ntypes)
{
    /* Deprecated. */
    return FcFalse;
}
...
FcObjectSet* FcObjectGetSet(void)
{
    int i;
    FcObjectSet* os = NULL;
...
static FcValue FcNameConvert(FcType type, FcChar8* string)
{
    ...
    switch ((int)v.type)
    {
        ...
        case FcTypeMatrix:
            FcMatrixInit(&m);
            sscanf((char*)string, "%lg %lg %lg %lg", &m.xx, &m.xy, &m.yx,
                   &m.yy);
...
static const FcChar8* FcNameFindNext(const FcChar8* cur, const char* delim,
                                     FcChar8* save, FcChar8* last)
{
    FcChar8 c;
...
static FcBool FcNameUnparseString(FcStrBuf* buf, const FcChar8* string,
...
FcBool FcNameUnparseValue(FcStrBuf* buf, FcValue* v0, FcChar8* escape)
{
    FcChar8 temp[1024];
...
FcBool FcNameUnparseValueList(FcStrBuf* buf, FcValueListPtr v, FcChar8* escape)
{
    while (v)
    {
...
FcChar8* FcNameUnparseEscaped(FcPattern* pat, FcBool escape)
{
    FcStrBuf buf, buf2;
    ...
    if (e)
    {
        if (!FcNameUnparseValueList(&buf, FcPatternEltValues(e),
                                    escape ? (FcChar8*)FC_ESCAPE_FIXED : 0))
            goto bail0;
    }
    ...
    if (e)
    {
        ...
        if (!FcNameUnparseString(&buf2, (FcChar8*)"-", 0))
            goto bail0;
        if (!FcNameUnparseValueList(&buf2, FcPatternEltValues(e),
                                    escape ? (FcChar8*)FC_ESCAPE_FIXED : 0))
            goto bail0;
    ...
    for (i = 0; i < NUM_OBJECT_TYPES; i++)
    {
        ...
        o = &FcObjects[i];
        if (!strcmp(o->object, FC_FAMILY) || !strcmp(o->object, FC_SIZE))
            continue;
        ...
        if (e)
        {
            if (!FcNameUnparseString(&buf, (FcChar8*)":", 0))
                goto bail0;
            if (!FcNameUnparseString(&buf, (FcChar8*)o->object,
                                     escape ? (FcChar8*)FC_ESCAPE_VARIABLE : 0))
                goto bail0;
            if (!FcNameUnparseString(&buf, (FcChar8*)"=", 0))
                goto bail0;
            if (!FcNameUnparseValueList(&buf, FcPatternEltValues(e),
                                        escape ? (FcChar8*)FC_ESCAPE_VARIABLE
                                               : 0))
                goto bail0;
        }
    }
