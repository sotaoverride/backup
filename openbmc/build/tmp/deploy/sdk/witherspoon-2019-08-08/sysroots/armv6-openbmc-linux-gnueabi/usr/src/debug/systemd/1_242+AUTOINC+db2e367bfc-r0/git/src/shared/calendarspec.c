...
/* An arbitrary limit on the length of the chains of components. We don't want
 * to build a very long linked list, which would be slow to iterate over and
 * might cause our stack to overflow. It's unlikely that legitimate uses require
 * more than a few linked compenents anyway. */
...
static int component_compare(CalendarComponent* const* a,
                             CalendarComponent* const* b)
{
    int r;
...
int calendar_spec_to_string(const CalendarSpec* c, char** p)
{
    ...
    else if (IN_SET(c->dst, 0, 1))
    {
        ...
        /* If daylight saving is explicitly on or off, let's show the used
         * timezone. */
...
{
    _cleanup_(chain_freep) CalendarComponent *year = NULL, *month = NULL,
                                             *day = NULL, *hour = NULL,
                                             *minute = NULL, *us = NULL;
    struct tm tm;
    int r;
...
static int prepend_component(const char** p, bool usec, unsigned nesting,
                             CalendarComponent** c)
{
...
{
    _cleanup_(chain_freep) CalendarComponent *first = NULL, *second = NULL,
                                             *third = NULL;
    const char* t;
    int r;
    ...
    /* Already the end? A ':' as separator? In that case this was a time, not a
     * date */
    if (IN_SET(*t, 0, ':'))
        return 0;
    ...
    *c = (CalendarSpec){
    ...
    else
    {
        ...
        else
        {
            ...
            if (last_space != NULL &&
                timezone_is_valid(last_space + 1, LOG_DEBUG))
            {
...
static int find_matching_component(const CalendarSpec* spec,
                                   const CalendarComponent* c, struct tm* tm,
                                   int* val)
{
...
static int calendar_spec_next_usec_impl(const CalendarSpec* spec, usec_t usec,
                                        usec_t* next)
{
    struct tm tm;
    time_t t;
...
int calendar_spec_next_usec(const CalendarSpec* spec, usec_t usec, usec_t* next)
{
    ...
    if (r == 0)
    {
        ...
        shared->return_value =
            calendar_spec_next_usec_impl(spec, usec, &shared->next);
