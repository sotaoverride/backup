...
netsnmp_feature_child_of(interface_all, libnetsnmpmibs)
    netsnmp_feature_child_of(interface, interface_all)
        netsnmp_feature_child_of(interface_access_entry_set_admin_status,
                                 interface_all)
...
void init_interface(void)
{
    snmpd_register_config_handler("interface", _parse_interface_config,
                                  _free_interface_config, "name type speed");
}
 ...
 */
netsnmp_container* netsnmp_access_interface_container_init(u_int flags)
{
    ...
    {
        netsnmp_container* container2 = netsnmp_container_find(
            "access_interface_by_name:access_interface:table_container");
        if (NULL == container2)
            return NULL;
                                             ...
                                             u_int free_flags)
{
    ...
    {
        snmp_log(LOG_ERR,
                 "invalid container for netsnmp_access_interface_free\n");
        return;
    }
    ...
    if (!(free_flags & NETSNMP_ACCESS_INTERFACE_FREE_DONT_CLEAR))
    {
        ...
        CONTAINER_CLEAR(
            container,
            (netsnmp_container_obj_func*)_access_interface_entry_release, NULL);
    }
                                                ...
                                                oid index)
{
    ...
    if (NULL == container)
    {
        snmp_log(LOG_ERR, "invalid container for "
                                               ...
                                               const char* name)
{
    ...
    {
        snmp_log(LOG_ERR, "secondary index missing for "
                          "netsnmp_access_interface_entry_get_by_name\n");
        return NULL;
    }
...
/**
 */
netsnmp_interface_entry* netsnmp_access_interface_entry_create(const char* name,
                                                               oid if_index)
{
...
int Interface_Scan_Next(short* index, char* name,
                        netsnmp_interface_entry** entry, void* dc)
{
    int returnIndex = 0;
    int ret;
    if (index)
...
int Interface_Scan_NextInt(int* index, char* name,
                           netsnmp_interface_entry** entry, void* dc)
{
 ...
 */
#ifndef NETSNMP_ACCESS_INTERFACE_NOARCH
int netsnmp_access_interface_entry_set_admin_status(
    netsnmp_interface_entry* entry, int ifAdminStatus)
{
    int rc;
...
static void _access_interface_entry_save_name(const char* name, oid index)
{
    ...
    else if (index != (oid)tmp)
    {
        NETSNMP_LOGONCE((LOG_ERR,
                         "IfIndex of an interface changed. Such "
 ...
 * @retval -1 : error
 */
int netsnmp_access_interface_entry_update_stats(
    netsnmp_interface_entry* prev_vals, netsnmp_interface_entry* new_vals)
{
    ...
    if ((NULL == prev_vals) || (NULL == new_vals) ||
        ...
        (0 !=
         strncmp(prev_vals->name, new_vals->name, strlen(prev_vals->name))))
        return -1;
    ...
    if (NULL == prev_vals->old_stats)
    {
        ...
        memcpy(prev_vals->old_stats, &prev_vals->stats,
               sizeof(prev_vals->stats));
    }

    if (0 != netsnmp_c64_check32_and_update(
                 &prev_vals->stats.ibytes, &new_vals->stats.ibytes,
                 &prev_vals->old_stats->ibytes, &need_wrap_check))
        DEBUGMSGTL(
            ("access:interface", "Error expanding ifHCInOctets to 64bits\n"));

    if (new_vals->ns_flags & NETSNMP_INTERFACE_FLAGS_CALCULATE_UCAST)
    {
        if (0 != netsnmp_c64_check32_and_update(
                     &prev_vals->stats.iall, &new_vals->stats.iall,
                     &prev_vals->old_stats->iall, &need_wrap_check))
    ...
    {
        if (0 != netsnmp_c64_check32_and_update(
                     &prev_vals->stats.iucast, &new_vals->stats.iucast,
                     &prev_vals->old_stats->iucast, &need_wrap_check))
            DEBUGMSGTL(("access:interface",
    ...
    if (0 != netsnmp_c64_check32_and_update(
                 &prev_vals->stats.imcast, &new_vals->stats.imcast,
                 &prev_vals->old_stats->imcast, &need_wrap_check))
        DEBUGMSGTL(("access:interface",
                    "Error expanding ifHCInMulticastPkts to 64bits\n"));

    if (0 != netsnmp_c64_check32_and_update(
                 &prev_vals->stats.ibcast, &new_vals->stats.ibcast,
                 &prev_vals->old_stats->ibcast, &need_wrap_check))
        DEBUGMSGTL(("access:interface",
                    "Error expanding ifHCInBroadcastPkts to 64bits\n"));

    if (0 != netsnmp_c64_check32_and_update(
                 &prev_vals->stats.obytes, &new_vals->stats.obytes,
                 &prev_vals->old_stats->obytes, &need_wrap_check))
        DEBUGMSGTL(
            ("access:interface", "Error expanding ifHCOutOctets to 64bits\n"));

    if (0 != netsnmp_c64_check32_and_update(
                 &prev_vals->stats.oucast, &new_vals->stats.oucast,
                 &prev_vals->old_stats->oucast, &need_wrap_check))
        DEBUGMSGTL(("access:interface",
                    "Error expanding ifHCOutUcastPkts to 64bits\n"));

    if (0 != netsnmp_c64_check32_and_update(
                 &prev_vals->stats.omcast, &new_vals->stats.omcast,
                 &prev_vals->old_stats->omcast, &need_wrap_check))
        DEBUGMSGTL(("access:interface",
                    "Error expanding ifHCOutMulticastPkts to 64bits\n"));

    if (0 != netsnmp_c64_check32_and_update(
                 &prev_vals->stats.obcast, &new_vals->stats.obcast,
                 &prev_vals->old_stats->obcast, &need_wrap_check))
        DEBUGMSGTL(("access:interface",
 ...
 * @retval -1 : error
 */
int netsnmp_access_interface_entry_calculate_stats(
    netsnmp_interface_entry* entry)
{
                                        ...
                                        netsnmp_interface_entry* rhs)
{
    ...
    if ((NULL == lhs) || (NULL == rhs) || (NULL == lhs->name) ||
        (NULL == rhs->name) ||
