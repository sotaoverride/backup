...
void init_udp(void)
{
    ...
    reginfo = netsnmp_create_handler_registration(
        "udp", udp_handler, udp_oid, OID_LENGTH(udp_oid), HANDLER_CAN_RONLY);
    rc =
    ...
    netsnmp_inject_handler(
        reginfo,
        netsnmp_get_cache_handler(UDP_STATS_CACHE_TIMEOUT, udp_load, udp_free,
...
int udp_handler(netsnmp_mib_handler* handler,
                netsnmp_handler_registration* reginfo,
        ...
        case MODE_GET:
            for (request = requests; request; request = request->next)
            {
                ...
                switch (subid)
                {
                     ...
                     * together, so we need to isolate the UDP-specific bits.
                     */
...
#ifdef hpux11
int udp_load(netsnmp_cache* cache, void* vmagic)
{
    int fd;
    struct nmparms p;
    unsigned int ulen;
    ...
    {
        DEBUGMSGTL(("mibII/udpScalar",
                    "Failed to load UDP object %d (hpux11)\n", magic));
        return (-1); /* error */
    }
...
int udp_load(netsnmp_cache* cache, void* vmagic)
{
    ...
    {
        DEBUGMSGTL(
            ("mibII/udpScalar", "Failed to load UDP scalar Group (linux)\n"));
    }
    else
    {
...
int udp_load(netsnmp_cache* cache, void* vmagic)
{
    ...
    if (magic == UDPNOPORTS)
    {
        ...
        {
            DEBUGMSGTL(("mibII/udpScalar",
                        "Failed to load UDP object %d (solaris)\n", magic));
            return -1;
        }
        else
        {
            DEBUGMSGTL(
                ("mibII/udpScalar", "Loaded UDP object %d (solaris)\n", magic));
     ...
     */
    ret_value = getMibstat(MIB_UDP, &udpstat, sizeof(mib2_udp_t), GET_FIRST,
                           &Get_everything, NULL);

    if (ret_value < 0)
    {
        DEBUGMSGTL(
            ("mibII/udpScalar", "Failed to load UDP scalar Group (solaris)\n"));
    }
    else
    {
...
int udp_load(netsnmp_cache* cache, void* vmagic)
{
    ...
    {
        DEBUGMSGTL(
            ("mibII/udpScalar", "Failed to load UDP scalar Group (netbsd)\n"));
    }
    else
    {
...
int udp_load(netsnmp_cache* cache, void* vmagic)
{
    ...
    {
        DEBUGMSGTL(
            ("mibII/udpScalar", "Failed to load UDP scalar Group (win32)\n"));
    }
    else
    {
...
int udp_load(netsnmp_cache* cache, void* vmagic)
{
    ...
    {
        DEBUGMSGTL(
            ("mibII/udpScalar", "Failed to load UDP scalar Group (AIX)\n"));
    }
    else
    {
        ret_value = 0;
...
int udp_load(netsnmp_cache* cache, void* vmagic)
{
    ...
    {
        DEBUGMSGTL(
            ("mibII/udpScalar", "Failed to load UDP scalar Group (sysctl)\n"));
    }
    else
    {
...
int udp_load(netsnmp_cache* cache, void* vmagic)
{
    ...
    {
        DEBUGMSGTL(("mibII/udpScalar",
                    "Failed to load UDP scalar Group (tcpipstats)\n"));
    }
    else
    {
        DEBUGMSGTL(
            ("mibII/udpScalar", "Loaded UDP scalar Group (tcpipstats)\n"));
    }
    return ret_value;
}
...
int udp_load(netsnmp_cache* cache, void* vmagic)
{
    ...
    {
        DEBUGMSGTL(
            ("mibII/udpScalar", "Failed to load UDP scalar Group (udpstat)\n"));
    }
    else
    {
...
void udp_free(netsnmp_cache* cache, void* magic)
{
#if defined(_USE_PERFSTAT_PROTOCOL)
