    ...
    else
    {
        ...
        if (ctx == NULL)
        {
            pam_syslog(pamh, LOG_CRIT,
                       "calloc(): failed to allocate memory: %s",
                       strerror(errno));
            return PAM_BUF_ERR;
        }
        ctx_clear(ctx);
        ...
        if ((pwent != NULL) && (pwent->pw_uid < cfg->minimum_uid))
        {
            if (cfg->debug)
                pam_syslog(pamh, LOG_DEBUG,
                           "uid below minimum_uid; user=%s uid=%ld", *username,
                           (long)pwent->pw_uid);
...
#define map(i)                                                                 \
    ...
    switch (rc)
    {
        ...
        default:
            pam_syslog(pamh, LOG_ERR, "unknown NSLCD_PAM_* code returned: %d",
                       rc);
            return PAM_ABORT;
    }
}
                               ...
                               struct nslcd_resp* authz_resp)
{
    PAM_REQUEST(
        ...
        /* write the request parameters */
        WRITE_STRING(fp, username);
        WRITE_STRING(fp, service); WRITE_STRING(fp, ruser);
        WRITE_STRING(fp, rhost); WRITE_STRING(fp, tty);
                               ...
                               struct nslcd_resp* resp)
{
    PAM_REQUEST(
        ...
        WRITE_STRING(fp, username);
        WRITE_STRING(fp, service); WRITE_STRING(fp, ruser);
        WRITE_STRING(fp, rhost); WRITE_STRING(fp, tty); WRITE_INT32(fp, asroot);
        WRITE_STRING(fp, oldpasswd); WRITE_STRING(fp, newpasswd),
                                     /* read the result entry */
                                     READ_PAM_CODE(fp, resp->res);
        READ_STRING(fp, resp->msg);)
}
                        ...
                        const char** argv)
{
    ...
    {
        rc = nslcd_request_config_get(
            pamh, &cfg, NSLCD_CONFIG_PAM_PASSWORD_PROHIBIT_MESSAGE, &resp);
        if ((rc == PAM_SUCCESS) && (resp.msg[0] != '\0'))
        {
            /* we silently ignore errors to get the configuration option */
            pam_syslog(pamh, LOG_NOTICE,
                       "password change prohibited: %s; user=%s", resp.msg,
                       username);
            if (!cfg.no_warn)
    ...
    if (!cfg.nullok && ((passwd == NULL) || (passwd[0] == '\0')))
    {
        if (cfg.debug)
            pam_syslog(pamh, LOG_DEBUG,
                       "user has empty password, access denied");
        return PAM_AUTH_ERR;
    }
...
int pam_sm_acct_mgmt(pam_handle_t* pamh, int flags, int argc, const char** argv)
{
    ...
    if (rc != PAM_SUCCESS)
    {
        ...
        if ((msg == NULL) || (msg[0] == '\0'))
        {
            ...
            pam_syslog(pamh, LOG_NOTICE, "%s; user=%s; err=%s", msg, username,
                       pam_strerror(pamh, rc));
    ...
    if ((authz_resp.msg[0] != '\0') && (!cfg.no_warn))
    {
        ...
        pam_syslog(pamh, LOG_INFO, "%s; user=%s", authz_resp.msg, username);
    }
    if ((ctx->saved_authz.msg[0] != '\0') && (!cfg.no_warn))
    {
        pam_info(pamh, "%s", ctx->saved_authz.msg);
        pam_syslog(pamh, LOG_INFO, "%s; user=%s", ctx->saved_authz.msg,
                   username);
    }
    return PAM_SUCCESS;
}
                        ...
                        const char** argv)
{
    ...
    if (rc != PAM_SUCCESS)
        return remap_pam_rc(rc, &cfg);
    /* do the nslcd request */
    rc = nslcd_request_sess_o(pamh, &cfg, username, service, ruser, rhost, tty,
                              &(ctx->saved_session));
    if (rc != PAM_SUCCESS)
        return remap_pam_rc(rc, &cfg);
    /* debug log */
                         ...
                         const char** argv)
{
    ...
    if (rc != PAM_SUCCESS)
        return remap_pam_rc(rc, &cfg);
    /* do the nslcd request */
    rc = nslcd_request_sess_c(pamh, &cfg, username, service, ruser, rhost, tty,
                              ctx->saved_session.msg);
    if (rc != PAM_SUCCESS)
        return remap_pam_rc(rc, &cfg);
    /* debug log */
...
int pam_sm_chauthtok(pam_handle_t* pamh, int flags, int argc, const char** argv)
{
    ...
    if (flags & PAM_PRELIM_CHECK)
    {
        ...
        /* TODO: perhaps this can be combined with the nslcd_request_exists()
         * call above */
        pwent = pam_modutil_getpwnam(args->pamh, username);
        myuid = getuid();
            ...
            (!(flags & PAM_CHANGE_EXPIRED_AUTHTOK)))
        {
            ...
            if (myuid == 0)
            {
                rc = nslcd_request_authc(pamh, &cfg, "", service, ruser, rhost,
                                         tty, "", &resp, NULL);
            ...
            /* try to  authenticate with the LDAP administrator password by
               passing an empty username to the authc request */
        ...
        else
        {
            ...
            rc = pam_get_authtok(pamh, PAM_OLDAUTHTOK,
    ...
    rc =
        pam_get_item(pamh, PAM_OLDAUTHTOK, (PAM_ITEM_CONST void**)&oldpassword);
    if (rc != PAM_SUCCESS)
        return rc;
    ...
    if (rc != PAM_SUCCESS)
    {
        pam_syslog(pamh, LOG_NOTICE, "password change failed: %s; user=%s", msg,
                   username);
        if ((rc != PAM_IGNORE) && (!cfg.no_warn))
            pam_error(pamh, "%s", msg);
        return rc;
...
struct pam_module PAM_NAME(modstruct) = {
    "pam_" MODULE_NAME, pam_sm_authenticate, pam_sm_setcred,
