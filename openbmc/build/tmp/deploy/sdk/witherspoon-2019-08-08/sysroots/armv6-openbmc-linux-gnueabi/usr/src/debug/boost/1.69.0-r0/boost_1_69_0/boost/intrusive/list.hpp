...
class list_impl
{
  ...
  public:
    ...
    //!   constructor throws (this does not happen with predefined
    //!   Boost.Intrusive hooks).
    explicit list_impl(const value_traits& v_traits) : data_(v_traits)
    {
        this->priv_size_traits().set_size(size_type(0));
        node_algorithms::init_header(this->get_root_node());
    }
    ...
    //!
    //! <b>Complexity</b>: Linear in distance(b, e). No copy constructors are
    //! called.
    //!
    //! <b>Throws</b>: If value_traits::node_traits::node
    //!   constructor throws (this does not happen with predefined
    ...
    //!   Boost.Intrusive hooks) or the move constructor of value traits throws.
    ...
    //!   (ie. no code is generated). Otherwise it detaches all elements from
    //!   this. In this case the objects in the list are not deleted (i.e. no
    //!   destructors are called), but the hooks according to the ValueTraits
    ...
    void push_front(reference value)
    {
        ...
        BOOST_INTRUSIVE_SAFE_HOOK_DEFAULT_ASSERT(
            !safemode_or_autounlink || node_algorithms::inited(to_insert));
        node_algorithms::link_before(
            node_traits::get_next(this->get_root_node()), to_insert);
        this->priv_size_traits().increment();
    }
    ...
    reference front()
    {
        return *priv_value_traits().to_value_ptr(
            node_traits::get_next(this->get_root_node()));
    }

    //! <b>Effects</b>: Returns a const_reference to the first element of the
    //! list.
    //!
    //! <b>Throws</b>: Nothing.
    //!
    ...
    //! <b>Effects</b>: Returns a const_reference to the last element of the
    //! list.
    //!
    //! <b>Throws</b>: Nothing.
    //!
    ...
    {
        return *priv_value_traits().to_value_ptr(
            detail::uncast(node_traits::get_previous(this->get_root_node())));
    }
    ...
    {
        return const_iterator(detail::uncast(this->get_root_node()),
                              this->priv_value_traits_ptr());
    }
    ...
    //!   of list.
    //!
    //! <b>Effects</b>: Returns a const reference to the list associated to the
    //! end iterator
    //!
    //! <b>Throws</b>: Nothing.
    //!
    ...
    //! <b>Throws</b>: Nothing.
    //!
    //! <b>Complexity</b>: Linear to the number of elements contained in the
    //! list.
    ...
    {
        node_algorithms::swap_nodes(this->get_root_node(),
                                    other.get_root_node());
    ...
    //! <b>Throws</b>: Nothing.
    //!
    //! <b>Complexity</b>: Linear to the number of erased elements if it's a
    //! safe-mode
    //!   or auto-unlink value, or constant-time size is enabled. Constant-time
    //!   otherwise.
    //!
    //! <b>Note</b>: Invalidates the iterators (but not the references) to the
    //!   erased elements.
    ...
    void clone_from(const list_impl& src, Cloner cloner, Disposer disposer)
    {
        ...
        detail::exception_disposer<list_impl, Disposer> rollback(*this,
                                                                 disposer);
    ...
    //! <b>Throws</b>: If cloner throws. Basic guarantee.
    template <class Cloner, class Disposer>
    void clone_from(BOOST_RV_REF(list_impl) src, Cloner cloner,
                    Disposer disposer)
    {
        this->clear_and_dispose(disposer);
        detail::exception_disposer<list_impl, Disposer> rollback(*this,
    ...
    //!
    //! <b>Effects</b>: Inserts the range pointed by b and e before the position
    //! p.
    //!   No copy constructors are called.
    //!
    //! <b>Throws</b>: Nothing.
    ...
    //!
    //! <b>Effects</b>: Clears the list and inserts the range pointed by b and
    //! e.
    ...
    //!
    //! <b>Effects</b>: Transfers all the elements of list x to this list,
    //! before the
    //!   the element pointed by p. No destructors or copy constructors are
    //!   called.
    //!
    //! <b>Throws</b>: Nothing.
    //!
    ...
    //! elements of
    //!    this list. Iterators of this list and all the references are not
    //!    invalidated.
    void splice(const_iterator p, list_impl& x)
    {
        if (!x.empty())
        {
            node_algorithms::transfer(p.pointed_node(),
                                      x.begin().pointed_node(),
                                      x.end().pointed_node());
    ...
    //!
    //! <b>Effects</b>: Transfers the value pointed by new_ele, from list x to
    //! this list,
    //!   before the element pointed by p. No destructors or copy constructors
    //!   are called. If p == new_ele or p == ++new_ele, this function is a null
    //!   operation.
    //!
    //! <b>Throws</b>: Nothing.
    //!
    ...
    //!
    //! <b>Effects</b>: Transfers the range pointed by f and e from list x to
    //! this list,
    //!   before the element pointed by p. No destructors or copy constructors
    //!   are called.
    //!
    ...
    //!   list. Iterators of this list and all the references are not
    //!   invalidated.
    void splice(const_iterator p, list_impl& x, const_iterator f,
                const_iterator e)
    {
        if (constant_time_size)
            this->splice(
                p, x, f, e,
                node_algorithms::distance(f.pointed_node(), e.pointed_node()));
        else
            this->splice(p, x, f, e,
                         1); // intrusive::iterator_distance is a dummy value
    }
                ...
                const_iterator e, size_type n)
    {
        if (n)
        {
            ...
            {
                BOOST_INTRUSIVE_INVARIANT_ASSERT(
                    n == node_algorithms::distance(f.pointed_node(),
                                                   e.pointed_node()));
                node_algorithms::transfer(p.pointed_node(), f.pointed_node(),
                                          e.pointed_node());
                size_traits& thist = this->priv_size_traits();
                size_traits& xt = x.priv_size_traits();
                thist.increase(n);
    ...
    //! <b>Effects</b>: This function sorts the list *this according to
    //! std::less<value_type>.
    //!   The sort is stable, that is, the relative order of equivalent elements
    //!   is preserved.
    //!
    //! <b>Throws</b>: If value_traits::node_traits::node
    //!   constructor throws (this does not happen with predefined
    //!   Boost.Intrusive hooks) or std::less<value_type> throws. Basic
    //!   guarantee.
    //!
    //! <b>Notes</b>: Iterators and references are not invalidated.
    //!
    //! <b>Complexity</b>: The number of comparisons is approximately N log N,
    //! where N
    //!   is the list's size.
    void sort()
    {
    ...
    //! <b>Requires</b>: p must be a comparison function that induces a strict
    //! weak ordering
    //!
    //! <b>Effects</b>: This function sorts the list *this according to p. The
    //! sort is
    //!   stable, that is, the relative order of equivalent elements is
    //!   preserved.
    //!
    //! <b>Throws</b>: If value_traits::node_traits::node
    //!   constructor throws (this does not happen with predefined
    //!   Boost.Intrusive hooks) or the predicate throws. Basic guarantee.
    //!
    ...
    void sort(Predicate p)
    {
            ...
            node_traits::get_previous(this->get_root_node()))
        {
            ...
            detail::array_initializer<list_impl, 64> counter(
                this->priv_value_traits());
            int fill = 0;
            while (!this->empty())
            {
    ...
    //! <b>Effects</b>: This function removes all of x's elements and inserts
    //! them
    //!   in order into *this according to std::less<value_type>. The merge is
    //!   stable; that is, if an element from *this is equivalent to one from x,
    //!   then the element from *this will precede the one from x.
    //!
    ...
    //! <b>Requires</b>: p must be a comparison function that induces a strict
    //! weak
    //!   ordering and both *this and x must be sorted according to that
    //!   ordering The lists x and *this must be distinct.
    //!
    //! <b>Effects</b>: This function removes all of x's elements and inserts
    //! them
    //!   in order into *this. The merge is stable; that is, if an element from
    //!   *this is equivalent to one from x, then the element from *this will
    //!   precede the one from x.
    //!
    ...
    //!
    //! <b>Complexity</b>: Linear time. It performs exactly size() comparisons
    //! for equality.
    //!
    //! <b>Note</b>: The relative order of elements that are not removed is
    //! unchanged,
    ...
    void remove_and_dispose(const_reference value, Disposer disposer)
    {
        this->remove_and_dispose_if(
            detail::equal_to_value<const_reference>(value), disposer);
    }
    ...
    //!
    //! <b>Complexity</b>: Linear time. It performs exactly size() calls to the
    //! predicate.
    //!
    //! <b>Note</b>: The relative order of elements that are not removed is
    //! unchanged,
    ...
    void remove_if(Pred pred)
    {
        ...
        node_algorithms::stable_partition(
            node_traits::get_next(root_node), root_node,
            detail::key_nodeptr_comp<Pred, value_traits>(
                pred, &this->priv_value_traits()),
            info);
        // Invariants preserved by stable_partition so erase can be safely
        // called The first element might have changed so calculate it again
        this->erase(const_iterator(node_traits::get_next(root_node),
                                   this->priv_value_traits_ptr()),
                    const_iterator(info.beg_2st_partition,
                                   this->priv_value_traits_ptr()),
    ...
    void remove_and_dispose_if(Pred pred, Disposer disposer)
    {
        ...
        // Invariants preserved by stable_partition so erase can be safely
        // called The first element might have changed so calculate it again
        this->erase_and_dispose(const_iterator(node_traits::get_next(root_node),
                                               this->priv_value_traits_ptr()),
                                const_iterator(info.beg_2st_partition,
                                               this->priv_value_traits_ptr()),
                                disposer);
    ...
    void unique()
    {
        this->unique_and_dispose(std::equal_to<value_type>(),
    ...
    //! <b>Throws</b>: If pred throws. Basic guarantee.
    //!
    //! <b>Complexity</b>: Linear time (size()-1 comparisons equality
    //! comparisons).
    //!
    //! <b>Note</b>: The relative order of elements that are not removed is
    //! unchanged,
    ...
    //! <b>Requires</b>: value must be a reference to a value inserted in a
    //! list.
    //!
    //! <b>Effects</b>: This function returns a const_iterator pointing to the
    //! element
    //!
    //! <b>Throws</b>: Nothing.
    //!
    ...
    //! <b>Requires</b>: value must be a const reference to a value inserted in
    //! a list.
    //!
    //! <b>Effects</b>: This function returns an iterator pointing to the
    //! element.
    //!
    //! <b>Throws</b>: Nothing.
    //!
    ...
    static const_iterator s_iterator_to(const_reference value)
    {
        BOOST_STATIC_ASSERT((!stateful_value_traits));
        reference r =
            *detail::uncast(pointer_traits<const_pointer>::pointer_to(value));
        BOOST_INTRUSIVE_INVARIANT_ASSERT(
            !node_algorithms::inited(value_traits::to_node_ptr(r)));
        return const_iterator(value_traits::to_node_ptr(r),
                              const_value_traits_ptr());
    }
    ...
    {
        BOOST_INTRUSIVE_INVARIANT_ASSERT(!node_algorithms::inited(
            this->priv_value_traits().to_node_ptr(value)));
        return iterator(this->priv_value_traits().to_node_ptr(value),
                        this->priv_value_traits_ptr());
    }
    ...
    const_iterator iterator_to(const_reference value) const
    {
        ...
        return const_iterator(this->priv_value_traits().to_node_ptr(r),
                              this->priv_value_traits_ptr());
    }
    ...
    void check() const
    {
        ...
        // header's next and prev either both point to header (empty list) or
        // neither does
        BOOST_INTRUSIVE_INVARIANT_ASSERT(
            (node_traits::get_next(header_ptr) == header_ptr) ==
  ...
  private:
        ...
        priv_container_from_end_iterator(const const_iterator& end_iterator)
    {
        ...
        root_plus_size* r =
            detail::parent_from_member<root_plus_size, header_holder_type>(
                h, &root_plus_size::m_header);
        data_t* d = detail::parent_from_member<data_t, root_plus_size>(
            r, &data_t::root_plus_size_);
        list_impl* s =
            detail::parent_from_member<list_impl, data_t>(d, &list_impl::data_);
        return *s;
    }
    /// @endcond
};

//! Helper metafunction to define a \c list that yields to the same type when
...
{
    /// @cond
    typedef typename pack_options<list_defaults,
    ...
    typedef list_impl<value_traits, typename packed_options::size_type,
                              ...
                              >::type
{
    ...
    BOOST_STATIC_ASSERT(
        (detail::is_same<typename Base::value_traits::value_type, T>::value));
  ...
  public:
    ...
    explicit list(const value_traits& v_traits) : Base(v_traits)
    {
    }
    ...
    list(BOOST_RV_REF(list) x) : Base(BOOST_MOVE_BASE(Base, x))
    {
    }

    list& operator=(BOOST_RV_REF(list) x)
    {
        return static_cast<list&>(
