                          ...
                          int is_bak)
{
    ...
    if (is_bak && blkid_probe_is_wholedisk(pr) && sb->s_dev_size != pr->size)
        return 0;
                        ...
                        __attribute__((__unused__)))
{
    ...
    swp = valid[1] && (!valid[0] || le64_to_cpu(sbp->s_last_cno) >
                                        le64_to_cpu(sbb->s_last_cno));
    sb = swp ? sbb : sbp;

    DBG(LOWPROBE, ul_debug("nilfs2: primary=%d, backup=%d, swap=%d", valid[0],
                           valid[1], swp));
