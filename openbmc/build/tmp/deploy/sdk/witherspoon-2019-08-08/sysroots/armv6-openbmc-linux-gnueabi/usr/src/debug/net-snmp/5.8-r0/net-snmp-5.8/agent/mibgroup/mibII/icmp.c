...
#if defined(HAVE_LIBPERFSTAT_H) &&                                             \
    (defined(aix4) || defined(aix5) || defined(aix6) || defined(aix7)) &&      \
    !defined(FIRST_PROTOCOL)
...
static int solaris_read_icmp_stat(ICMP_STAT_STRUCTURE*);
static int solaris_read_icmp_msg_stat(ICMP_STAT_STRUCTURE*,
                                      struct icmp4_msg_mib*, int*);
#ifdef NETSNMP_ENABLE_IPV6
static int solaris_read_icmp6_stat(struct icmp6_mib*);
static int solaris_read_icmp6_msg_stat(struct icmp6_mib*, struct icmp6_msg_mib*,
                                       int*);
#endif
#endif
...
struct icmp_msg_stats_table_entry
    icmp_msg_stats_table[ICMP_MSG_STATS_IPV4_COUNT + ICMP_MSG_STATS_IPV6_COUNT];

int icmp_stats_load(netsnmp_cache* cache, void* vmagic)
{
        ...
        switch (i)
        {
            ...
            case 1:
                ...
                icmp_stats_table[i].icmpStatsOutErrors =
                    v6icmp.icmp6OutDestUnreachs + v6icmp.icmp6OutPktTooBigs +
                    v6icmp.icmp6OutTimeExcds + v6icmp.icmp6OutParmProblems;
#endif /* NETSNMP_ENABLE_IPV6 */
                break;
        }
...
int icmp_msg_stats_load(netsnmp_cache* cache, void* vmagic)
{
    ICMP_STAT_STRUCTURE v4icmp;
        ...
        while (255 >= k)
        {
            if (v4icmpmsg.vals[k].InType)
            {
                ...
                icmp_msg_stats_table[i].icmpMsgStatsInPkts =
                    v4icmpmsg.vals[k].InType;
                icmp_msg_stats_table[i].flags =
            ...
            if (v4icmpmsg.vals[k].OutType)
            {
                ...
                icmp_msg_stats_table[i].icmpMsgStatsOutPkts =
                    v4icmpmsg.vals[k].OutType;
                icmp_msg_stats_table[i].flags =
                    icmp_msg_stats_table[i].flags | ICMP_MSG_STATS_HAS_OUT;
                inc = 1; /* Set this if we found a valid entry */
            }
            if (inc)
            {
    ...
    else
    {
        ...
        icmp_msg_stats_table[i].flags =
            ICMP_MSG_STATS_HAS_IN | ICMP_MSG_STATS_HAS_OUT;
        icmp_msg_stats_table[i].ipVer = 1;
        i++;
    ...
    else
    {
        ...
        icmp_msg_stats_table[i].icmpMsgStatsInPkts =
            v6icmp.icmp6InGroupMembResponses;
        icmp_msg_stats_table[i].icmpMsgStatsOutPkts =
            v6icmp.icmp6OutGroupMembResponses;
        icmp_msg_stats_table[i].ipVer = 2;
        icmp_msg_stats_table[i].flags =
            ICMP_MSG_STATS_HAS_IN | ICMP_MSG_STATS_HAS_OUT;
        i++;
        ...
        icmp_msg_stats_table[i].icmpMsgStatsInPkts =
            v6icmp.icmp6InRouterAdvertisements;
        icmp_msg_stats_table[i].icmpMsgStatsOutPkts = 0;
        icmp_msg_stats_table[i].ipVer = 2;
        icmp_msg_stats_table[i].flags = ICMP_MSG_STATS_HAS_IN;
        i++;
        ...
        icmp_msg_stats_table[i].icmpMsgStatsInPkts =
            v6icmp.icmp6InNeighborSolicits;
        icmp_msg_stats_table[i].icmpMsgStatsOutPkts =
            v6icmp.icmp6OutNeighborSolicits;
        icmp_msg_stats_table[i].ipVer = 2;
        icmp_msg_stats_table[i].flags =
            ICMP_MSG_STATS_HAS_IN | ICMP_MSG_STATS_HAS_OUT;
        i++;
        ...
        icmp_msg_stats_table[i].icmpMsgStatsInPkts =
            v6icmp.icmp6InNeighborAdvertisements;
        icmp_msg_stats_table[i].icmpMsgStatsOutPkts =
            v6icmp.icmp6OutNeighborAdvertisements;
        icmp_msg_stats_table[i].ipVer = 2;
        icmp_msg_stats_table[i].flags =
            ICMP_MSG_STATS_HAS_IN | ICMP_MSG_STATS_HAS_OUT;
        i++;
...
netsnmp_variable_list* icmp_stats_next_entry(void** loop_context,
                                             void** data_context,
...
netsnmp_variable_list* icmp_stats_first_entry(void** loop_context,
                                              void** data_context,
                                              netsnmp_variable_list* index,
                                              netsnmp_iterator_info* data)
{
...
netsnmp_variable_list* icmp_msg_stats_next_entry(void** loop_context,
                                                 void** data_context,
                                                 netsnmp_variable_list* index,
                                                 netsnmp_iterator_info* data)
{
...
netsnmp_variable_list* icmp_msg_stats_first_entry(void** loop_context,
                                                  void** data_context,
                                                  netsnmp_variable_list* index,
                                                  netsnmp_iterator_info* data)
{
...
void init_icmp(void)
{
    ...
    scalar_reginfo = netsnmp_create_handler_registration(
        "icmp", icmp_handler, icmp_oid, OID_LENGTH(icmp_oid),
        HANDLER_CAN_RONLY);
    rc = netsnmp_register_scalar_group(scalar_reginfo, ICMPINMSGS,
                                       ICMPOUTADDRMASKREPS);
    if (rc != SNMPERR_SUCCESS)
         ...
         */
#ifndef hpux11
    rc = netsnmp_inject_handler(
        scalar_reginfo,
        netsnmp_get_cache_handler(ICMP_STATS_CACHE_TIMEOUT, icmp_load,
                                  icmp_free, icmp_oid, OID_LENGTH(icmp_oid)));
    if (rc != SNMPERR_SUCCESS)
        goto bail;
#endif
#ifdef USES_SNMP_DESIGNED_ICMPSTAT
    ...
    table_reginfo = netsnmp_create_handler_registration(
        "icmpStatsTable", icmp_stats_table_handler, icmp_stats_tbl_oid,
    ...
    netsnmp_inject_handler(
        table_reginfo, netsnmp_get_cache_handler(
                           ICMP_STATS_CACHE_TIMEOUT, icmp_load, icmp_free,
    ...
    if (!msg_stats_table_info)
        goto bail;
    netsnmp_table_helper_add_indexes(msg_stats_table_info, ASN_INTEGER,
                                     ASN_INTEGER, 0);
    ...
    msg_stats_reginfo = netsnmp_create_handler_registration(
        "icmpMsgStatsTable", icmp_msg_stats_table_handler,
    ...
    netsnmp_inject_handler(
        msg_stats_reginfo,
        netsnmp_get_cache_handler(ICMP_STATS_CACHE_TIMEOUT, icmp_load,
                                  icmp_free, icmp_msg_stats_tbl_oid,
                                  OID_LENGTH(icmp_msg_stats_tbl_oid)));
#endif /* USES_SNMP_DESIGNED_ICMPSTAT */

#ifdef USING_MIBII_IP_MODULE
    if (++ip_module_count == 2)
        REGISTER_SYSOR_TABLE(
            ip_module_oid, ip_module_oid_len,
...
int icmp_handler(netsnmp_mib_handler* handler,
    ...
    switch (reqinfo->mode)
    {
        case MODE_GET:
            for (request = requests; request; request = request->next)
            {
                ...
                DEBUGMSGOID(
                    ("mibII/icmp", requestvb->name, requestvb->name_length));
                DEBUGMSG(("mibII/icmp", "\n"));
...
#ifdef USES_SNMP_DESIGNED_ICMPSTAT
int icmp_stats_table_handler(netsnmp_mib_handler* handler,
                             netsnmp_handler_registration* reginfo,
                             netsnmp_agent_request_info* reqinfo,
                             netsnmp_request_info* requests)
{
    ...
    switch (reqinfo->mode)
    {
        case MODE_GET:
            ...
            {
                requestvb = request->requestvb;
                entry = (struct icmp_stats_table_entry*)
                ...
                DEBUGMSG(("mibII/icmpStatsTable",
                          " In %d InErr %d Out %d OutErr %d\n",
                ...
                switch (subid)
                {
                    case ICMP_STAT_INMSG:
                        snmp_set_var_typed_value(
                            requestvb, ASN_COUNTER,
                            (u_char*)&entry->icmpStatsInMsgs, sizeof(uint32_t));
                        break;
                    case ICMP_STAT_INERR:
                        snmp_set_var_typed_value(
                            requestvb, ASN_COUNTER,
                            (u_char*)&entry->icmpStatsInErrors,
                            sizeof(uint32_t));
                        break;
                    case ICMP_STAT_OUTMSG:
                        snmp_set_var_typed_value(
                            requestvb, ASN_COUNTER,
                            (u_char*)&entry->icmpStatsOutMsgs,
                            sizeof(uint32_t));
                        break;
                    case ICMP_STAT_OUTERR:
                        snmp_set_var_typed_value(
                            requestvb, ASN_COUNTER,
                            (u_char*)&entry->icmpStatsOutErrors,
                            sizeof(uint32_t));
                        break;
                    default:
                        snmp_log(
                            LOG_WARNING,
                            "mibII/icmpStatsTable: Unrecognised column (%d)\n",
                            (int)subid);
                }
            }
            break;
        ...
        case MODE_SET_UNDO:
            snmp_log(LOG_WARNING,
                     "mibII/icmpStatsTable: Unsupported mode (%d)\n",
                     reqinfo->mode);
            break;
#endif /* !NETSNMP_NO_WRITE_SUPPORT */
        default:
            snmp_log(LOG_WARNING,
                     "mibII/icmpStatsTable: Unrecognised mode (%d)\n",
                     reqinfo->mode);
            break;
    }
...
int icmp_msg_stats_table_handler(netsnmp_mib_handler* handler,
                                 netsnmp_handler_registration* reginfo,
                                 netsnmp_agent_request_info* reqinfo,
                                 netsnmp_request_info* requests)
{
    ...
    switch (reqinfo->mode)
    {
        case MODE_GET:
            ...
            {
                requestvb = request->requestvb;
                entry = (struct icmp_msg_stats_table_entry*)
                    netsnmp_extract_iterator_context(request);
                if (!entry)
                    continue;
                table_info = netsnmp_extract_table_info(request);
                ...
                DEBUGMSGOID(("mibII/icmpMsgStatsTable",
                             ...
                             request->requestvb->name_length));
                DEBUGMSG(("mibII/icmpMsgStatsTable",
                          " In %d Out %d Flags 0x%x\n",
                          entry->icmpMsgStatsInPkts, entry->icmpMsgStatsOutPkts,
                          entry->flags));
                ...
                switch (subid)
                {
                    case ICMP_MSG_STAT_IN_PKTS:
                        if (entry->flags & ICMP_MSG_STATS_HAS_IN)
                        {
                            snmp_set_var_typed_value(
                                requestvb, ASN_COUNTER,
                                (u_char*)&entry->icmpMsgStatsInPkts,
                                sizeof(uint32_t));
                        }
                        else
                        {
                    ...
                    case ICMP_MSG_STAT_OUT_PKTS:
                        if (entry->flags & ICMP_MSG_STATS_HAS_OUT)
                        {
                            snmp_set_var_typed_value(
                                requestvb, ASN_COUNTER,
                                (u_char*)&entry->icmpMsgStatsOutPkts,
                                sizeof(uint32_t));
                        }
                        else
                        {
                    ...
                    default:
                        snmp_log(LOG_WARNING,
                                 "mibII/icmpMsgStatsTable: Unrecognised column "
                                 "(%d)\n",
                                 (int)subid);
                }
            }
...
#ifdef hpux11
int icmp_load(netsnmp_cache* cache, void* vmagic)
{
    int fd;
    struct nmparms p;
    unsigned int ulen;
    ...
    {
        DEBUGMSGTL(
            ("mibII/icmp", "Failed to load ICMP object %d (hpux11)\n", magic));
        return (-1); /* error */
    }
...
int icmp_load(netsnmp_cache* cache, void* vmagic)
{
    ...
    ret_value = getMibstat(MIB_ICMP, &icmpstat, sizeof(mib2_icmp_t), GET_FIRST,
                           &Get_everything, NULL);
                                   ...
                                   &Get_everything, NULL)) == 0)
    {
        ...
        {
            DEBUGMSGTL(
                ("mibII/icmp", "Failed to load ICMP6 Group (solaris)\n"));
        }
        else
        {
        ...
        icmp6stat.icmp6OutNeighborAdvertisements +=
            ifstat.ipv6IfIcmpOutNeighborAdvertisements;
        icmp6stat.icmp6InNeighborAdvertisements +=
            ifstat.ipv6IfIcmpInNeighborAdvertisements;
        icmp6stat.icmp6OutNeighborSolicits +=
            ifstat.ipv6IfIcmpOutNeighborSolicits;
        icmp6stat.icmp6InNeighborSolicits +=
            ifstat.ipv6IfIcmpInNeighborSolicits;
        icmp6stat.icmp6OutRedirects += ifstat.ipv6IfIcmpOutRedirects;
        icmp6stat.icmp6InRedirects += ifstat.ipv6IfIcmpInRedirects;
        icmp6stat.icmp6InGroupMembQueries +=
            ifstat.ipv6IfIcmpInGroupMembQueries;
        icmp6stat.icmp6OutGroupMembResponses +=
            ifstat.ipv6IfIcmpOutGroupMembResponses;
        icmp6stat.icmp6InGroupMembResponses +=
            ifstat.ipv6IfIcmpInGroupMembResponses;
        icmp6stat.icmp6OutGroupMembReductions +=
            ifstat.ipv6IfIcmpOutGroupMembReductions;
        icmp6stat.icmp6InGroupMembReductions +=
            ifstat.ipv6IfIcmpInGroupMembReductions;
        req = GET_NEXT;
    }
#endif
...
static int solaris_read_icmp_stat(ICMP_STAT_STRUCTURE* mib)
{
    *mib = icmpstat;
    return 0;
}

static int solaris_read_icmp_msg_stat(ICMP_STAT_STRUCTURE* mib,
                                      struct icmp4_msg_mib* msg_mib, int* flag)
{
    *mib = icmpstat;
    *flag = 0;
    return 0;
...
#ifdef NETSNMP_ENABLE_IPV6
static int solaris_read_icmp6_stat(struct icmp6_mib* mib)
{
    *mib = icmp6stat;
    return 0;
}

static int solaris_read_icmp6_msg_stat(struct icmp6_mib* mib,
                                       struct icmp6_msg_mib* msg_mib, int* flag)
{
    *mib = icmp6stat;
    *flag = 0;
    return 0;
...
void icmp_free(netsnmp_cache* cache, void* magic)
{
#if defined(_USE_PERFSTAT_PROTOCOL)
