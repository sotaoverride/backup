                             ...
                             void* userdata)
{
    ...
    /* If there is a background thread running the poll() for us, tell it to
     * exit the poll() */
...
static int rebuild(AvahiSimplePoll* s)
{
    ...
    if (s->n_watches + 1 > s->max_pollfds)
    {
        ...
        if (!(n = avahi_realloc(s->pollfds,
                                sizeof(struct pollfd) * s->max_pollfds)))
            return -1;
...
int avahi_simple_poll_prepare(AvahiSimplePoll* s, int timeout)
{
    ...
    assert(s);
    assert(s->state == STATE_INIT || s->state == STATE_DISPATCHED ||
...
int avahi_simple_poll_run(AvahiSimplePoll* s)
{
    ...
    for (;;)
    {
        ...
        if (s->poll_func(s->pollfds, s->n_pollfds, s->prepared_timeout,
                         s->poll_func_userdata) < 0)
        {
...
static int system_poll(struct pollfd* ufds, unsigned int nfds, int timeout,
