...
GNode* g_node_insert(GNode* parent, gint position, GNode* node)
{
    ...
    if (position > 0)
        return g_node_insert_before(parent, g_node_nth_child(parent, position),
                                    node);
    else if (position == 0)
...
static gboolean g_node_traverse_pre_order(GNode* node, GTraverseFlags flags,
...
static gboolean g_node_depth_traverse_pre_order(GNode* node,
                                                ...
                                                gpointer data)
{
    if (node->children)
    {
        ...
        while (child)
        {
            ...
            current = child;
            child = current->next;
            if (g_node_depth_traverse_pre_order(current, flags, depth, func,
                                                data))
                return TRUE;
        }
    }
...
static gboolean g_node_traverse_post_order(GNode* node, GTraverseFlags flags,
                                           GNodeTraverseFunc func,
                                           gpointer data)
{
    if (node->children)
...
static gboolean g_node_depth_traverse_post_order(GNode* node,
                                                 GTraverseFlags flags,
                                                 guint depth,
                                                 GNodeTraverseFunc func,
                                                 gpointer data)
    ...
    if (node->children)
    {
        ...
        if (depth)
        {
            ...
            while (child)
            {
                ...
                current = child;
                child = current->next;
                if (g_node_depth_traverse_post_order(current, flags, depth,
                                                     func, data))
                    return TRUE;
            }
        }
...
static gboolean g_node_traverse_in_order(GNode* node, GTraverseFlags flags,
                                         GNodeTraverseFunc func, gpointer data)
{
    if (node->children)
    {
...
static gboolean g_node_depth_traverse_in_order(GNode* node,
                                               GTraverseFlags flags,
                                               guint depth,
                                               GNodeTraverseFunc func,
                                               gpointer data)
{
    if (node->children)
    {
        ...
        if (depth)
        {
            ...
            if (g_node_depth_traverse_in_order(current, flags, depth, func,
                                               data))
                return TRUE;
        ...
        else if ((flags & G_TRAVERSE_NON_LEAFS) && func(node, data))
            return TRUE;
    }
    else if ((flags & G_TRAVERSE_LEAFS) && func(node, data))
        return TRUE;
...
static gboolean g_node_traverse_level(GNode* node, GTraverseFlags flags,
                                      guint level, GNodeTraverseFunc func,
                                      gpointer data, gboolean* more_levels)
{
    if (level == 0)
    {
        if (node->children)
    ...
    else
    {
        ...
        while (node)
        {
            if (g_node_traverse_level(node, flags, level - 1, func, data,
                                      more_levels))
                return TRUE;
...
static gboolean g_node_depth_traverse_level(GNode* node, GTraverseFlags flags,
                                            gint depth, GNodeTraverseFunc func,
                                            gpointer data)
{
    guint level;
...
void g_node_traverse(GNode* root, GTraverseType order, GTraverseFlags flags,
                     gint depth, GNodeTraverseFunc func, gpointer data)
{
    g_return_if_fail(root != NULL);
    ...
    switch (order)
    {
        ...
        case G_POST_ORDER:
            if (depth < 0)
                ...
                g_node_depth_traverse_post_order(root, flags, depth, func,
                                                 data);
            break;
        case G_IN_ORDER:
            if (depth < 0)
...
static void g_node_count_func(GNode* node, GTraverseFlags flags, guint* n)
{
    if (node->children)
    {
        GNode* child;
