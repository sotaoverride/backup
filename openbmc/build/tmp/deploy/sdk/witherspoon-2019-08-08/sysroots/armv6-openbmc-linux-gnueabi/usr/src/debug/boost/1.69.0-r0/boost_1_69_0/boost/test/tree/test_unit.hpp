...
//
/// @file
/// Defines @ref boost::unit_test::test_unit "test_unit", @ref
/// boost::unit_test::test_case "test_case",
/// @ref boost::unit_test::test_suite "test_suite" and @ref
...
class BOOST_TEST_DECL test_unit
{
  public:
    ...
    typedef BOOST_READONLY_PROPERTY(std::vector<std::string>,
                                    (test_unit)) label_list_t;

    typedef boost::function<test_tools::assertion_result(test_unit_id)>
        precondition_t;
    typedef BOOST_READONLY_PROPERTY(std::vector<precondition_t>,
                                    (test_unit)) precond_list_t;
    ...
    {
        return p_run_status == RS_ENABLED;
    }
    ...
    precond_list_t
        p_preconditions; ///< user supplied preconditions for this test unit;

    // Public r/w properties
    readwrite_property<std::string> p_name; ///< name for this test unit
    readwrite_property<std::string>
        p_description; ///< description for this test unit
    readwrite_property<unsigned>
        p_timeout; ///< timeout for the test unit execution in seconds
    readwrite_property<counter_t>
        p_expected_failures; ///< number of expected failures in this test unit

    readwrite_property<run_status>
        p_default_status; ///< run status obtained by this unit during setup
                          ///< phase
    readwrite_property<run_status>
        p_run_status; ///< run status assigned to this unit before execution
                      ///< phase after applying all filters

    readwrite_property<counter_t>
        p_sibling_rank; ///< rank of this test unit amoung siblings of the same
                        ///< parent

    readwrite_property<decor_list_t>
        p_decorators; ///< automatically assigned decorators; execution is
                      ///< delayed till framework::finalize_setup_phase function
    readwrite_property<fixture_list_t>
        p_fixtures; ///< fixtures associated with this test unit

  protected:
    ~test_unit();
    // Constructor
    test_unit(const_string tu_name, const_string tc_file, std::size_t tc_line,
              test_unit_type t);
...
class BOOST_TEST_DECL test_case : public test_unit
{
  public:
    ...
    // Public property
    typedef BOOST_READONLY_PROPERTY(boost::function<void()>,
                                    (test_case)) test_func;
...
class BOOST_TEST_DECL test_suite : public test_unit
{
  public:
    ...
    // Constructor
    explicit test_suite(const_string ts_name, const_string ts_file,
                        std::size_t ts_line);
     ...
     */
    void add(test_unit* tu, counter_t expected_failures = 0,
             unsigned timeout = 0);
    ...
    /// @overload
    void add(test_unit_generator const& gen,
             decorator::collector_t& decorators);

    /// @overload
    void add(boost::shared_ptr<test_unit_generator> gen_ptr,
             decorator::collector_t& decorators);
  ...
  protected:
    ...
    friend BOOST_TEST_DECL void traverse_test_tree(test_suite const&,
                                                   test_tree_visitor&, bool);
    friend class framework::state;
    virtual ~test_suite()
    {
    }
    ...
    children_per_rank m_ranked_children; ///< maps child sibling rank to list of
                                         ///< children with that rank

    std::vector<std::pair<boost::shared_ptr<test_unit_generator>,
                          std::vector<decorator::base_ptr>>>
        m_generators; /// lazy evaluation
};
...
struct user_tc_method_invoker
{
    ...
    user_tc_method_invoker(shared_ptr<InstanceType> inst,
        ...
        m_inst(inst),
        m_test_method(test_method)
    {
    }

    void operator()()
    {
        ((*m_inst).*m_test_method)();
    }
...
inline test_case* make_test_case(boost::function<void()> const& test_func,
                                 const_string tc_name, const_string tc_file,
                                 std::size_t tc_line)
{
    return new test_case(ut_detail::normalize_test_case_name(tc_name), tc_file,
                         tc_line, test_func);
}
...
inline test_case* make_test_case(void (UserTestCase::*test_method)(),
                                 const_string tc_name, const_string tc_file,
                                 std::size_t tc_line,
                                 boost::shared_ptr<InstanceType> user_test_case)
{
    return new test_case(
        ut_detail::normalize_test_case_name(tc_name), tc_file, tc_line,
        ut_detail::user_tc_method_invoker<InstanceType, UserTestCase>(
            user_test_case, test_method));
}
