 ...
 * Instead, it contains 2 sets of bitflags.  These bitflags indicate what
 * messages are supported by the server. rect->w contains byte count
 */
...
typedef struct _rfbFileTransferMsg
{
    ...
    uint8_t contentParam; /*  Other possible content classification (Dir or File
                             name, etc..) */
    uint8_t
        pad; /* It appears that UltraVNC *forgot* to Swap16IfLE(contentParam) */
    uint32_t size; /*  FileSize or packet index or error or other  */
    /*  uint32_t sizeH;		 Additional 32Bits params to handle big values. Only
     * for V2 (we want backward compatibility between all V1 versions) */
    uint32_t length;
...
#define rfbFileTransferVersion                                                 \
    2 /*  v1 is the old FT version ( <= 1.0.0 RC18 versions) */

/*  FileTransfer Content types and Params defines */
#define rfbDirContentRequest                                                   \
    1 /*  Client asks for the content of a given Server directory */
...
#define rfbFileTransferRequest                                                 \
    3 /*  Client asks the server for the transfer of a given file */
#define rfbFileHeader                                                          \
    4 /*  First packet of a file transfer, containing file's features */
#define rfbFilePacket 5 /*  One chunk of the file */
#define rfbEndOfFile                                                           \
    6 /*  End of file transfer (the file has been received or error) */
#define rfbAbortFileTransfer                                                   \
    7 /*  The file transfer must be aborted, whatever the state */
#define rfbFileTransferOffer                                                   \
    8 /*  The client offers to send a file to the server */
#define rfbFileAcceptHeader 9 /*  The server accepts or rejects the file */
#define rfbCommand                                                             \
    10 /*  The Client sends a simple command (File Delete, Dir create etc...)  \
        */
#define rfbCommandReturn                                                       \
    11 /*  The Client receives the server's answer about a simple command */
#define rfbFileChecksums                                                       \
    12 /*  The zipped checksums of the destination file (Delta Transfer) */
...
#define rfbRDirRecursiveList                                                   \
    3 /*  Request a server directory content recursive sorted list */
#define rfbRDirRecursiveSize                                                   \
    4 /*  Request a server directory content recursive size */
...
#define rfbRErrorCmd                                                           \
    0xFFFFFFFF /*  Error when a command fails on remote side (ret in "size"    \
                  field) */

#define sz_rfbBlockSize                                                        \
    8192 /*  Size of a File Transfer packet (before compression) */
#define rfbZipDirectoryPrefix                                                  \
    "!UVNCDIR-\0" /*  Transferred directory are zipped in a file with this     \
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 * RDV Cache Encoding.
 * special is not used at this point, can be used to reset cache or other
 * specials just put it to make sure we don't have to change the encoding again.
 */
