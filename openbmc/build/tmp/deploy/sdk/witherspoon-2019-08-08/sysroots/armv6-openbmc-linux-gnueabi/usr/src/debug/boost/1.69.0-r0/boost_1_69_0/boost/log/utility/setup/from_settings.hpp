 ...
 * \date   11.10.2009
 *
 * The header contains definition of facilities that allows to initialize the
 ...
 *
 * \b Throws: An <tt>std::exception</tt>-based exception if the provided
 ...
 *
 * The function registers a factory for a sink. The factory will be called to
 * create sink instance when the parser discovers the specified sink type in the
 * settings file. The factory must accept a map of parameters [parameter name ->
 * parameter value] that it may use to initialize the sink. The factory must
 * return a non-NULL pointer to the constructed sink instance.
 *
 * \param sink_name The custom sink name. Must point to a zero-terminated
 * sequence of characters, must not be NULL. \param factory Pointer to the
 * custom sink factory. Must not be NULL.
 */
template <typename CharT>
BOOST_LOG_SETUP_API void
...
{
    typedef sink_factory<typename FactoryT::char_type> factory_base;
    register_sink_factory(sink_name.c_str(),
                          boost::static_pointer_cast<factory_base>(factory));
}
