...
int bus_parse_unit_info(sd_bus_message* message, UnitInfo* u)
{
    ...
    return sd_bus_message_read(message, "(ssssssouso)", &u->id, &u->description,
                               &u->load_state, &u->active_state, &u->sub_state,
                               &u->following, &u->unit_path, &u->job_id,
                               &u->job_type, &u->job_path);
}

#define DEFINE_BUS_APPEND_PARSE_PTR(bus_type, cast_type, type, parse_func)     \
    static int bus_append_##parse_func(sd_bus_message* m, const char* field,   \
                                       const char* eq)                         \
    {                                                                          \
        type val;                                                              \
        int r;                                                                 \
...
DEFINE_BUS_APPEND_PARSE_PTR("t", uint64_t, unsigned long,
...
static int bus_append_string(sd_bus_message* m, const char* field,
                             const char* eq)
{
    int r;
...
static int bus_append_byte_array(sd_bus_message* m, const char* field,
...
static int bus_append_parse_sec_rename(sd_bus_message* m, const char* field,
                                       const char* eq)
{
    char* n;
    usec_t t;
...
static int bus_append_parse_size(sd_bus_message* m, const char* field,
                                 const char* eq, uint64_t base)
{
    uint64_t v;
    int r;
...
static int bus_append_exec_command(sd_bus_message* m, const char* field,
                                   const char* eq)
{
    ...
    do
    {
        switch (*eq)
        {
            ...
            case '+':
            case '!':
                /* The bus API doesn't support +, ! and !! currently,
                 * unfortunately. :-( */
                return log_error_errno(SYNTHETIC_ERRNO(EOPNOTSUPP),
                                       "Sorry, but +, ! and !! are currently "
                                       "not supported for transient services.");
    ...
    if (explicit_path)
    {
        r = extract_first_word(&eq, &path, NULL,
...
static int bus_append_ip_address_access(sd_bus_message* m, int family,
                                        ...
                                        unsigned char prefixlen)
{
    ...
    r = sd_bus_message_append_array(m, 'y', prefix,
...
static int bus_append_cgroup_property(sd_bus_message* m, const char* field,
                                      const char* eq)
{
    int r;
    ...
    if (STR_IN_SET(field, "CPUAccounting", "MemoryAccounting", "IOAccounting",
                   "BlockIOAccounting", "TasksAccounting", "IPAccounting"))

        return bus_append_parse_boolean(m, field, eq);

    if (STR_IN_SET(field, "CPUWeight", "StartupCPUWeight", "IOWeight",
                   "StartupIOWeight"))
    ...
    if (STR_IN_SET(field, "MemoryMin", "MemoryLow", "MemoryHigh", "MemoryMax",
                   "MemorySwapMax", "MemoryLimit", "TasksMax"))
    {
        ...
        if (r >= 0)
        {
            ...
            /* When this is a percentage we'll convert this into a relative
             * value in the range 0…UINT32_MAX and pass it in the MemoryLowScale
             * property (and related ones). This way the physical memory size
             * can be determined server-side. */

            n = strjoina(field, "Scale");
            r = sd_bus_message_append(
                m, "(sv)", n, "u",
                (uint32_t)(((uint64_t)r * UINT32_MAX) / 1000U));
            if (r < 0)
                return bus_log_create_error(r);
    ...
    if (streq(field, "CPUQuota"))
    {
        ...
        if (isempty(eq))
            r = sd_bus_message_append(m, "(sv)", "CPUQuotaPerSecUSec", "t",
                                      USEC_INFINITY);
        else
        {
    ...
    if (streq(field, "DeviceAllow"))
    {
        ...
        else
        {
            ...
            r = sd_bus_message_append(m, "(sv)", field, "a(ss)", 1, path,
                                      strempty(rwm));
        }
    ...
    if (cgroup_io_limit_type_from_string(field) >= 0 ||
        STR_IN_SET(field, "BlockIOReadBandwidth", "BlockIOWriteBandwidth"))
    {
            ...
            else
            {
                ...
                if (r < 0)
                    return log_error_errno(
                        r, "Failed to parse byte value %s: %m", bandwidth);
            }

            r = sd_bus_message_append(m, "(sv)", field, "a(st)", 1, path,
                                      bytes);
        }
    ...
    if (STR_IN_SET(field, "IODeviceWeight", "BlockIODeviceWeight"))
    {
        ...
        else
        {
            ...
            if (r < 0)
                return log_error_errno(r, "Failed to parse %s value %s: %m",
                                       field, weight);
    ...
    if (streq(field, "IODeviceLatencyTargetSec"))
    {
        ...
        if (isempty(eq))
            r = sd_bus_message_append(m, "(sv)", field_usec, "a(st)",
                                      USEC_INFINITY);
        else
        {
            ...
            r = sd_bus_message_append(m, "(sv)", field_usec, "a(st)", 1, path,
                                      usec);
        }
    ...
    if (STR_IN_SET(field, "IPAddressAllow", "IPAddressDeny"))
    {
        ...
        else if (streq(eq, "link-local"))
        {
            ...
            prefix.in6 = (struct in6_addr){.s6_addr32[0] = htobe32(0xfe800000)};
        ...
        else if (streq(eq, "multicast"))
        {
            ...
            prefix.in6 = (struct in6_addr){.s6_addr32[0] = htobe32(0xff000000)};
        ...
        else
        {
            for (;;)
            {
                ...
                if (r == -ENOMEM)
                    return log_oom();
                if (r < 0)
                    return log_error_errno(r, "Failed to parse %s: %s", field,
                                           eq);

                r = in_addr_prefix_from_string_auto(word, &family, &prefix,
                                                    &prefixlen);
                if (r < 0)
                    return log_error_errno(
                        r, "Failed to parse IP address prefix: %s", word);
...
static int bus_append_automount_property(sd_bus_message* m, const char* field,
                                         const char* eq)
{
...
static int bus_append_execute_property(sd_bus_message* m, const char* field,
                                       const char* eq)
{
    const char* suffix;
    int r;

    if (STR_IN_SET(field, "User", "Group", "UtmpIdentifier", "UtmpMode",
                   "PAMName", "TTYPath", "WorkingDirectory", "RootDirectory",
                   "SyslogIdentifier", "ProtectSystem", "ProtectHome",
                   "SELinuxContext", "RootImage", "RuntimeDirectoryPreserve",
                   "Personality", "KeyringMode", "NetworkNamespacePath"))

        return bus_append_string(m, field, eq);

    if (STR_IN_SET(field, "IgnoreSIGPIPE", "TTYVHangup", "TTYReset",
                   "TTYVTDisallocate", "PrivateTmp", "PrivateDevices",
                   "PrivateNetwork", "PrivateUsers", "PrivateMounts",
                   "NoNewPrivileges", "SyslogLevelPrefix",
                   "MemoryDenyWriteExecute", "RestrictRealtime", "DynamicUser",
                   "RemoveIPC", "ProtectKernelTunables", "ProtectKernelModules",
    ...
    if (STR_IN_SET(field, "ReadWriteDirectories", "ReadOnlyDirectories",
                   "InaccessibleDirectories", "ReadWritePaths", "ReadOnlyPaths",
                   "InaccessiblePaths", "RuntimeDirectory", "StateDirectory",
    ...
    if (STR_IN_SET(field, "RuntimeDirectoryMode", "StateDirectoryMode",
                   "CacheDirectoryMode", "LogsDirectoryMode",
                   "ConfigurationDirectoryMode", "UMask"))
    ...
    if (streq(field, "EnvironmentFile"))
    {
        ...
        if (isempty(eq))
            r = sd_bus_message_append(m, "(sv)", "EnvironmentFiles", "a(sb)",
                                      0);
        else
    ...
    if (streq(field, "StandardInputText"))
    {
        ...
        /* Note that we don't expand specifiers here, but that should be OK, as
         * this is a programmatic interface anyway */
    ...
    if (streq(field, "StandardInputData"))
    {
        ...
        if (r < 0)
            return log_error_errno(r, "Failed to decode base64 data '%s': %m",
                                   eq);
    ...
    if ((suffix = startswith(field, "Limit")))
    {
        ...
        if (rl >= 0)
        {
            ...
            if (r < 0)
                return log_error_errno(r, "Failed to parse resource limit: %s",
                                       eq);
    ...
    if (STR_IN_SET(field, "CapabilityBoundingSet", "AmbientCapabilities"))
    {
        ...
        if (r < 0)
            return log_error_errno(r, "Failed to parse %s value %s: %m", field,
                                   eq);
    ...
    if (streq(field, "CPUAffinity"))
    {
        ...
        if (r < 0)
            return log_error_errno(r, "Failed to parse %s value: %s", field,
                                   eq);
...
static int bus_append_kill_property(sd_bus_message* m, const char* field,
                                    const char* eq)
{
...
static int bus_append_mount_property(sd_bus_message* m, const char* field,
                                     const char* eq)
{
...
static int bus_append_path_property(sd_bus_message* m, const char* field,
                                    const char* eq)
{
    int r;
    ...
    if (STR_IN_SET(field, "PathExists", "PathExistsGlob", "PathChanged",
                   "PathModified", "DirectoryNotEmpty"))
    {
        ...
        if (isempty(eq))
            ...
            r = sd_bus_message_append(m, "(sv)", "Paths", "a(ss)", 1, field,
                                      eq);
        if (r < 0)
            return bus_log_create_error(r);
...
static int bus_append_service_property(sd_bus_message* m, const char* field,
                                       const char* eq)
{
    int r;

    if (STR_IN_SET(field, "PIDFile", "Type", "Restart", "BusName",
                   "NotifyAccess", "USBFunctionDescriptors",
                   "USBFunctionStrings"))

        return bus_append_string(m, field, eq);

    if (STR_IN_SET(field, "PermissionsStartOnly", "RootDirectoryStartOnly",
                   "RemainAfterExit", "GuessMainPID"))

        return bus_append_parse_boolean(m, field, eq);

    if (STR_IN_SET(field, "RestartSec", "TimeoutStartSec", "TimeoutStopSec",
                   "RuntimeMaxSec", "WatchdogSec"))
    ...
    if (STR_IN_SET(field, "ExecStartPre", "ExecStart", "ExecStartPost",
                   "ExecReload", "ExecStop", "ExecStopPost"))

        return bus_append_exec_command(m, field, eq);

    if (STR_IN_SET(field, "RestartPreventExitStatus", "RestartForceExitStatus",
                   "SuccessExitStatus"))
    {
        ...
        for (p = eq;;)
        {
            ...
            if (r == -ENOMEM)
                return log_oom();
            if (r < 0)
                return log_error_errno(r, "Invalid syntax in %s: %s", field,
                                       eq);
            ...
            if (r < 0)
            {
                ...
                if (val < 0)
                    return log_error_errno(
                        r, "Invalid status or signal %s in %s: %m", word,
                        field);
...
static int bus_append_socket_property(sd_bus_message* m, const char* field,
                                      const char* eq)
{
    int r;

    if (STR_IN_SET(field, "Accept", "Writable", "KeepAlive", "NoDelay",
                   "FreeBind", "Transparent", "Broadcast", "PassCredentials",
                   "PassSecurity", "ReusePort", "RemoveOnStop",
    ...
    if (STR_IN_SET(field, "Backlog", "MaxConnections",
                   "MaxConnectionsPerSource", "KeepAliveProbes",
                   "TriggerLimitBurst"))
    ...
    if (STR_IN_SET(field, "TimeoutSec", "KeepAliveTimeSec",
                   "KeepAliveIntervalSec", "DeferAcceptSec",
    ...
    if (STR_IN_SET(field, "ExecStartPre", "ExecStartPost", "ExecReload",
                   "ExecStopPost"))

        return bus_append_exec_command(m, field, eq);

    if (STR_IN_SET(field, "SmackLabel", "SmackLabelIPIn", "SmackLabelIPOut",
                   "TCPCongestion", "BindToDevice", "BindIPv6Only",
                   "FileDescriptorName", "SocketUser", "SocketGroup"))
    ...
    if (STR_IN_SET(field, "ListenStream", "ListenDatagram",
                   "ListenSequentialPacket", "ListenNetlink", "ListenSpecial",
                   "ListenMessageQueue", "ListenFIFO", "ListenUSBFunction"))
    {

        if (isempty(eq))
            r = sd_bus_message_append(m, "(sv)", "Listen", "a(ss)", 0);
        else
            r = sd_bus_message_append(m, "(sv)", "Listen", "a(ss)", 1,
                                      field + STRLEN("Listen"), eq);
        if (r < 0)
            return bus_log_create_error(r);
...
static int bus_append_timer_property(sd_bus_message* m, const char* field,
                                     const char* eq)
{
    int r;
    ...
    if (STR_IN_SET(field, "OnActiveSec", "OnBootSec", "OnStartupSec",
                   "OnUnitActiveSec", "OnUnitInactiveSec"))
    {
        ...
        else
        {
            ...
            r = parse_sec(eq, &t);
            if (r < 0)
                return log_error_errno(r, "Failed to parse %s=%s: %m", field,
                                       eq);

            r = sd_bus_message_append(m, "(sv)", "TimersMonotonic", "a(st)", 1,
                                      field, t);
        }
        if (r < 0)
            return bus_log_create_error(r);
    ...
    if (streq(field, "OnCalendar"))
    {
        ...
        if (isempty(eq))
            ...
            r = sd_bus_message_append(m, "(sv)", "TimersCalendar", "a(ss)", 1,
                                      field, eq);
        if (r < 0)
            return bus_log_create_error(r);
...
static int bus_append_unit_property(sd_bus_message* m, const char* field,
                                    const char* eq)
{
    ...
    if (STR_IN_SET(field, "Description", "SourcePath", "OnFailureJobMode",
    ...
    if (STR_IN_SET(field, "StopWhenUnneeded", "RefuseManualStart",
                   "RefuseManualStop", "AllowIsolate", "IgnoreOnIsolate",
                   "DefaultDependencies"))

        return bus_append_parse_boolean(m, field, eq);

    if (STR_IN_SET(field, "JobTimeoutSec", "JobRunningTimeoutSec",
    ...
    if (t >= 0)
    {
        ...
        else
        {
            ...
            r = sd_bus_message_append(m, "(sv)",
                                      is_condition ? "Conditions" : "Asserts",
                                      "a(sbbs)", 1, field, trigger, negate, p);
        }
        if (r < 0)
            return bus_log_create_error(r);
...
int bus_append_unit_property_assignment(sd_bus_message* m, UnitType t,
                                        const char* assignment)
{
    ...
    if (!eq)
        return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Not an assignment: %s",
                               assignment);
    ...
    return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Unknown assignment: %s",
                           assignment);
}

int bus_append_unit_property_assignment_many(sd_bus_message* m, UnitType t,
                                             char** l)
{
    char** i;
    int r;
                                               ...
                                               size_t* n_changes)
{
    ...
    while ((r = sd_bus_message_read(m, "(sss)", &type, &path, &source)) > 0)
    {
        ...
        if (ch < 0)
        {
            log_notice("Manager reported unknown change type \"%s\" for path "
                       "\"%s\", ignoring.",
                       type, path);
            continue;
        }
...
int unit_load_state(sd_bus* bus, const char* name, char** load_state)
{
    ...
    /* This function warns on it's own, because otherwise it'd be awkward to
     * pass the dbus error message around. */

    r = sd_bus_get_property_string(bus, "org.freedesktop.systemd1", path,
                                   "org.freedesktop.systemd1.Unit", "LoadState",
                                   &error, load_state);
    if (r < 0)
        return log_error_errno(r, "Failed to get load state of %s: %s", name,
