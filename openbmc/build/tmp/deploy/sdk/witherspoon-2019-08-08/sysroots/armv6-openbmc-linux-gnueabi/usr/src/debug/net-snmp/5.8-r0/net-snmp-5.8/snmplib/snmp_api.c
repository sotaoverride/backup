...
netsnmp_feature_child_of(statistics, libnetsnmp)
    netsnmp_feature_child_of(snmp_api, libnetsnmp)
        netsnmp_feature_child_of(oid_is_subtree, snmp_api)
            netsnmp_feature_child_of(snmpv3_probe_contextEngineID_rfc5343,
                                     snmp_api)
...
struct snmp_internal_session
{
    ...
    int (*hook_realloc_build)(netsnmp_session*, netsnmp_pdu*, u_char**, size_t*,
                              size_t*);
...
static const char* api_errors[-SNMPERR_MAX + 1] = {
    ...
    "USM unknown security name (no such user exists)", /* SNMPERR_USM_UNKNOWNSECURITYNAME
                                                        */
    "USM unsupported security level (this user has not been configured for "
    "that level of security)", /* SNMPERR_USM_UNSUPPORTEDSECURITYLEVEL */
...
static const char* secLevelName[] = {"BAD_SEC_LEVEL", "noAuthNoPriv",
                                     "authNoPriv", "authPriv"};
...
{
    u_int max =
        netsnmp_ds_get_int(NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_MSG_SEND_MAX);
    if (0 == max)
 ...
 */
void snmp_error(netsnmp_session* psess, int* p_errno, int* p_snmp_errno,
                char** p_str)
{
    ...
    if (psess->s_errno)
    {
        ...
        if (error == NULL)
            error = "Unknown Error";
        snprintf(&buf[strlen(buf)], sizeof(buf) - strlen(buf), " (%s)", error);
    }
    buf[sizeof(buf) - 1] = '\0';
    *p_str = strdup(buf);
}
...
static void _init_snmp(void)
{
    ...
    netsnmp_ds_set_boolean(NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_REVERSE_ENCODE,
...
static void register_default_handlers(void)
{
...
#endif /* !defined(NETSNMP_DISABLE_SNMPV1) ||                                  \
          !defined(NETSNMP_DISABLE_SNMPV2C) */
    ...
    netsnmp_ds_register_config(
        ASN_BOOLEAN, "snmp", "noContextEngineIDDiscovery",
        NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_NO_DISCOVERY);
...
void init_snmp(const char* type)
{
    ...
    init_vacm();
#if defined(NETSNMP_USE_OPENSSL) && defined(HAVE_LIBSSL) &&                    \
    NETSNMP_TRANSPORT_TLSBASE_DOMAIN
    netsnmp_certs_init();
#endif
...
static struct session_list* _sess_copy(netsnmp_session* in_session)
{
    struct session_list* slp;
    ...
    isp = (struct snmp_internal_session*)calloc(
        1, sizeof(struct snmp_internal_session));
    ...
    {
        session->peername = netsnmp_strdup_and_null(
            (u_char*)in_session->peername, strlen(in_session->peername));
    ...
    {
        int retry =
            netsnmp_ds_get_int(NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_RETRIES);
        if (retry < 0)
    ...
    {
        int timeout =
            netsnmp_ds_get_int(NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_TIMEOUT);
        if (timeout <= 0)
...
int snmpv3_probe_contextEngineID_rfc5343(void* slp, netsnmp_session* session)
{
    ...
    if (NULL != response->variables && NULL != response->variables->name &&
        snmp_oid_compare(response->variables->name,
                         response->variables->name_length, snmpEngineIDoid,
                         snmpEngineIDoid_len) == 0 &&
        ASN_OCTET_STR == response->variables->type &&
        NULL != response->variables->val.string &&
        response->variables->val_len > 0)
    {
        session->contextEngineID = netsnmp_memdup(
            response->variables->val.string, response->variables->val_len);
        if (!session->contextEngineID)
        {
            snmp_log(LOG_ERR, "failed rfc5343 contextEngineID probing: memory "
                              "allocation failed\n");
            return SNMP_ERR_GENERR;
        }
        ...
        session->securityEngineID = netsnmp_memdup(
            response->variables->val.string, response->variables->val_len);
        if (!session->securityEngineID)
        {
            snmp_log(LOG_ERR, "failed rfc5343 securityEngineID probing: memory "
                              "allocation failed\n");
            return SNMP_ERR_GENERR;
        }
        ...
        if (snmp_get_do_debugging())
        {
            ...
            for (i = 0; i < session->securityEngineIDLen; i++)
                DEBUGMSG(
                    ("snmp_sess_open", "%02x", session->securityEngineID[i]));
 ...
 */
int netsnmp_sess_config_transport(netsnmp_container* transport_configuration,
                                  netsnmp_transport* transport)
{
    ...
    if (transport_configuration)
    {
        ...
        if (transport->f_config)
        {
            ...
            for (config_data = (netsnmp_transport_config*)ITERATOR_FIRST(iter);
                 config_data;
                                           ...
                                           netsnmp_transport* transport)
{
    ...
    {
        DEBUGMSGTL(
            ("snmp_sess", "limiting session rcv size to transport max\n"));
 ...
 */
static void* _sess_open(netsnmp_session* in_session)
{
...

    {
        ...
        {
            transport = netsnmp_tdomain_transport_full(
                "snmp", in_session->peername, in_session->local_port,
                "tcp,tcp6", NULL);
        }
        else
        {
    ...
    if (in_session->flags & SNMP_FLAGS_UDP_BROADCAST)
    {
        ...
        rc = setsockopt(transport->sock, SOL_SOCKET, SO_BROADCAST, (char*)&b,
                        sizeof(b));
...
netsnmp_session* snmp_add(
    netsnmp_session* in_session, netsnmp_transport* transport,
    int (*fpre_parse)(netsnmp_session*, netsnmp_transport*, void*, int),
    int (*fpost_parse)(netsnmp_session*, netsnmp_pdu*, int))
{
    struct session_list* slp;
...
netsnmp_session* snmp_add_full(
    ...
    int (*fpost_parse)(netsnmp_session*, netsnmp_pdu*, int),
    int (*fbuild)(netsnmp_session*, netsnmp_pdu*, u_char*, size_t*),
    int (*frbuild)(netsnmp_session*, netsnmp_pdu*, u_char**, size_t*, size_t*),
    int (*fcheck)(u_char*, size_t),
    netsnmp_pdu* (*fcreate_pdu)(netsnmp_transport*, void*, size_t))
{
    struct session_list* slp;
    slp = (struct session_list*)snmp_sess_add_ex(
        in_session, transport, fpre_parse, fparse, fpost_parse, fbuild, frbuild,
        fcheck, fcreate_pdu);
    if (slp == NULL)
    {
        return NULL;
    ...
    netsnmp_pdu* (*fcreate_pdu)(netsnmp_transport*, void*, size_t))
{
    ...
    {
        DEBUGMSGTL(
            ("snmp_sess_add", "limiting session rcv size to transport max\n"));
        slp->session->rcvMsgMaxSize = transport->msgMaxSize;
    }
    if (slp->session->sndMsgMaxSize > transport->msgMaxSize)
    {
        DEBUGMSGTL(
            ("snmp_sess_add", "limiting session snd size to transport max\n"));
...
void* snmp_sess_add(netsnmp_session* in_session, netsnmp_transport* transport,
...
#else
    snmp_log(LOG_ERR,
             "create_user_from_session called when USM wasn't compiled in");
    netsnmp_assert(0 == 1);
    return SNMP_ERR_GENERR;
#endif
}
...
static void snmp_free_session(netsnmp_session* s)
{
    if (s)
    {
...
int snmp_sess_close(void* sessp)
{
    ...
    if (isp)
    {
        ...
        while (rp)
        {
            ...
            if (orp->callback)
            {
                orp->callback(NETSNMP_CALLBACK_OP_TIMED_OUT, slp->session,
                              orp->pdu->reqid, orp->pdu, orp->cb_data);
            }
...
int snmp_close(netsnmp_session* session)
{
    ...
    { /*MTCRITICAL_RESOURCE */
        snmp_res_lock(MT_LIBRARY_ID, MT_LIB_SESSION);
        if (Sessions && Sessions->session == session)
        { /* If first entry */
            slp = Sessions;
            Sessions = slp->next;
        }
...
static void snmpv3_calc_msg_flags(int sec_level, int msg_command, u_char* flags)
{
    *flags = 0;
                        ...
                        netsnmp_session* session, netsnmp_pdu* pdu)
{
    ...
    if (pdu->securityModel == SNMP_DEFAULT_SECMODEL)
    {
        ...
        if (pdu->securityModel == SNMP_DEFAULT_SECMODEL)
        {
            pdu->securityModel = se_find_value_in_slist(
                "snmp_secmods", netsnmp_ds_get_string(NETSNMP_DS_LIBRARY_ID,
    ...
    {
        ret = snmpv3_packet_realloc_rbuild(pkt, pkt_len, offset, session, pdu,
                                           NULL, 0);
    }
    else
    {
...
static u_char* snmpv3_header_build(netsnmp_session* session, netsnmp_pdu* pdu,
                                   ...
                                   size_t length, u_char** msg_hdr_e)
{
    ...
    cp = asn_build_string(
        cp, out_length, (u_char)(ASN_UNIVERSAL | ASN_PRIMITIVE | ASN_OCTET_STR),
...
int snmpv3_header_realloc_rbuild(u_char** pkt, size_t* pkt_len, size_t* offset,
                                 netsnmp_session* session, netsnmp_pdu* pdu)
{
    ...
    rc = asn_realloc_rbuild_int(
        pkt, pkt_len, offset, 1,
        (u_char)(ASN_UNIVERSAL | ASN_PRIMITIVE | ASN_INTEGER), &sec_model,
        sizeof(sec_model));
    DEBUGINDENTLESS();
    if (rc == 0)
    {
    ...
    rc = asn_realloc_rbuild_string(
        pkt, pkt_len, offset, 1,
        (u_char)(ASN_UNIVERSAL | ASN_PRIMITIVE | ASN_OCTET_STR), &msg_flags,
        sizeof(msg_flags));
    DEBUGINDENTLESS();
    if (rc == 0)
    {
    ...
    rc = asn_realloc_rbuild_int(
        pkt, pkt_len, offset, 1,
        (u_char)(ASN_UNIVERSAL | ASN_PRIMITIVE | ASN_INTEGER), &max_size,
        sizeof(max_size));
    DEBUGINDENTLESS();
    if (rc == 0)
    {
    ...
    rc = asn_realloc_rbuild_int(
        pkt, pkt_len, offset, 1,
        (u_char)(ASN_UNIVERSAL | ASN_PRIMITIVE | ASN_INTEGER), &pdu->msgid,
        sizeof(pdu->msgid));
    DEBUGINDENTLESS();
    if (rc == 0)
    {
...
static u_char* snmpv3_scopedPDU_header_build(netsnmp_pdu* pdu, u_char* packet,
                                             size_t* out_length,
                                             u_char** spdu_e)
{
    u_char *scopedPdu, *pb;
...
#ifdef NETSNMP_USE_REVERSE_ASNENCODING
int snmpv3_scopedPDU_header_realloc_rbuild(u_char** pkt, size_t* pkt_len,
 ...
 */
int snmpv3_packet_realloc_rbuild(u_char** pkt, size_t* pkt_len, size_t* offset,
                                 netsnmp_session* session, netsnmp_pdu* pdu,
                        ...
                        size_t pdu_data_len)
{
     ...
     */
    sec_params =
        snmpv3_header_build(session, pdu, global_data, out_length, 0, NULL);
    if (sec_params == NULL)
        return -1;
...
static int _snmp_build(u_char** pkt, size_t* pkt_len, size_t* offset,
                       netsnmp_session* session, netsnmp_pdu* pdu)
{
#if !defined(NETSNMP_DISABLE_SNMPV1) || !defined(NETSNMP_DISABLE_SNMPV2C)
    u_char* h0e = NULL;
    ...
    switch (pdu->version)
    {
        ...
        case SNMP_VERSION_2c:
            ...
            else
            {
                 ...
                 */
                cp = asn_build_sequence(
                    *pkt, pkt_len, (u_char)(ASN_SEQUENCE | ASN_CONSTRUCTOR), 0);
                if (cp == NULL)
                {
                    return -1;
                }
...
u_char* snmp_pdu_build(netsnmp_pdu* pdu, u_char* cp, size_t* out_length)
{
    ...
    for (vp = pdu->variables; vp; vp = vp->next_variable)
    {
        ...
        cp =
            snmp_build_var_op(cp, vp->name, &vp->name_length, vp->type,
    ...
    if (vp && save_vp)
    {
        ...
        for (save_length = 0; save_vp; save_vp = save_vp->next_variable)
            ++save_length;
        DEBUGMSGTL(
            ("send", "trimmed %" NETSNMP_PRIz "d variables\n", save_length));
     ...
     */
    asn_build_sequence(h2, &length, (u_char)(ASN_SEQUENCE | ASN_CONSTRUCTOR),
                       cp - h2e);
 ...
 */
int snmp_pdu_realloc_rbuild(u_char** pkt, size_t* pkt_len, size_t* offset,
                            netsnmp_pdu* pdu)
{
#ifndef VPCACHE_SIZE
#define VPCACHE_SIZE 50
    ...
    do
    {
        for (i = final; i < VPCACHE_SIZE; i++)
        {
            vp = vpcache[i];
            DEBUGDUMPSECTION("send", "VarBind");
            rc = snmp_realloc_rbuild_var_op(
                pkt, pkt_len, offset, 1, vp->name, &vp->name_length, vp->type,
        ...
        if (wrapped)
        {
            ...
            for (i = 0; i < final; i++)
            {
                ...
                DEBUGDUMPSECTION("send", "VarBind");
                rc = snmp_realloc_rbuild_var_op(
                    pkt, pkt_len, offset, 1, vp->name, &vp->name_length,
                    vp->type, (u_char*)vp->val.string, vp->val_len);
                DEBUGINDENTLESS();
                if (rc == 0)
                {
                    return 0;
    ...
    if (pdu->command != SNMP_MSG_TRAP)
    {
        ...
        rc = asn_realloc_rbuild_int(
            pkt, pkt_len, offset, 1,
            (u_char)(ASN_UNIVERSAL | ASN_PRIMITIVE | ASN_INTEGER), &pdu->reqid,
            sizeof(pdu->reqid));
        DEBUGINDENTLESS();
        if (rc == 0)
        {
    ...
    else
    {
        ...
        rc = asn_realloc_rbuild_unsigned_int(
            pkt, pkt_len, offset, 1, (u_char)(ASN_TIMETICKS | ASN_PRIMITIVE),
            &pdu->time, sizeof(pdu->time));
        DEBUGINDENTLESS();
        if (rc == 0)
        {
     ...
     */
    rc = asn_realloc_rbuild_sequence(
        pkt, pkt_len, offset, 1, (u_char)pdu->command, *offset - start_offset);
    return rc;
}
#endif /* NETSNMP_USE_REVERSE_ASNENCODING */
 ...
 */
static int snmp_parse_version(u_char* data, size_t length)
{
    u_char type;
                 ...
                 u_char** after_header, netsnmp_session* sess)
{
    ...
    data =
        asn_parse_sequence(data, &asn_len, &type,
    ...
    *length -= data - cp; /* subtract off the length of the header */
    ...
    DEBUGINDENTADD(-4); /* return from global data indent */
    if (data == NULL || type != ASN_INTEGER || msg_sec_model < 1 ||
        msg_sec_model > 0x7fffffff)
    {
    ...
    if ((pdu->securityName == NULL) || (pdu->securityEngineID == NULL) ||
...
static void free_securityStateRef(netsnmp_pdu* pdu)
{
...
int snmpv3_make_report(netsnmp_pdu* pdu, int error)
{
     ...
     */
    snmp_pdu_add_variable(pdu, err_var, err_var_len, ASN_COUNTER, &ltmp,
                          sizeof(ltmp));
...
int snmpv3_get_report_type(netsnmp_pdu* pdu)
{
    ...
    if (vp->name_length == REPORT_STATS_LEN + 2)
    {
        ...
        else if (memcmp(usmStats, vp->name, REPORT_STATS_LEN * sizeof(oid)) ==
                 0)
        {
            switch (vp->name[REPORT_STATS_LEN])
            {
 ...
 */
static int _snmp_parse(void* sessp, netsnmp_session* session, netsnmp_pdu* pdu,
                       u_char* data, size_t length)
{
    ...
    switch (pdu->version)
    {
        ...
        case SNMP_VERSION_2c:
#endif
            NETSNMP_RUNTIME_PROTOCOL_CHECK_V1V2(pdu->version,
                                                unsupported_version);
...
#endif
            data = snmp_comstr_parse(data, &length, community,
                                     &community_length, &pdu->version);
            if (data == NULL)
                return -1;
            ...
            {
                data = session->authenticator(data, &length, community,
                                              community_length);
                if (data == NULL)
                {
        ...
        case SNMP_VERSION_3:
            ...
            */
            ...
            if (!netsnmp_ds_get_boolean(NETSNMP_DS_LIBRARY_ID,
                ...
                NULL != pdu->contextEngineID && pdu->contextEngineIDLen == 5 &&
                ...
                pdu->contextEngineID[4] == 0x06)
            {
                ...
                DEBUGMSGTL(
                    ("snmpv3_contextid", "starting context ID discovery\n"));
                ...
                if (NULL != pdu->variables &&
                    ...
                    ((SNMP_MSG_GET == pdu->command &&
                      snmp_oid_compare(snmpEngineIDoid, snmpEngineIDoid_len,
                                       pdu->variables->name,
                                          ...
                                          pdu->variables->name_length) > 0)))
                {
                    ...
                    ourEngineID_len = snmpv3_get_engineID((u_char*)ourEngineID,
                                                          ourEngineID_len);
                    if (0 != ourEngineID_len)
                    {
                        ...
                        snmp_pdu_add_variable(
                            pdu2, snmpEngineIDoid, snmpEngineIDoid_len,
                            ASN_OCTET_STR, ourEngineID, ourEngineID_len);
                        ...
                        if (0 == snmp_sess_send(sessp, pdu2))
                        {
                            ...
                            DEBUGMSGTL(
                                ("snmpv3_contextid", "  sent it off!\n"));

                            snmp_free_pdu(pdu2);

                            snmp_log(LOG_ERR,
                                     "sending a response to the context "
                                     "engineID probe failed\n");
                        }
                    }
                    else
                    {
...
static int snmp_parse(void* sessp, netsnmp_session* pss, netsnmp_pdu* pdu,
                      u_char* data, size_t length)
{
    int rc;
...
int snmp_pdu_parse(netsnmp_pdu* pdu, u_char* data, size_t* length)
{
    ...
    switch (pdu->command)
    {
        case SNMP_MSG_TRAP:
             ...
             */
            data =
                asn_parse_int(data, length, &type, (long*)&pdu->specific_type,
    ...
    while ((int)*length > 0)
    {
        ...
        switch ((short)vp->type)
        {
            ...
            case ASN_COUNTER64:
                ...
                p = asn_parse_unsigned_int64(
                    var_val, &len, &vp->type,
                    (struct counter64*)vp->val.counter64, vp->val_len);
                if (!p)
                    goto fail;
                break;
#ifdef NETSNMP_WITH_OPAQUE_SPECIAL_TYPES
            case ASN_OPAQUE_FLOAT:
                ...
                p = asn_parse_float(var_val, &len, &vp->type, vp->val.floatVal,
                                    vp->val_len);
                if (!p)
                    goto fail;
                break;
            ...
            case ASN_OBJECT_ID:
                ...
                p = asn_parse_objid(var_val, &len, &vp->type, objid,
                                    &vp->val_len);
                if (!p)
                    goto fail;
 ...
 * build pdu packet
 */
int netsnmp_build_packet(struct snmp_internal_session* isp, netsnmp_session* sp,
                         ...
                         size_t* pktbuf_len_p, u_char** pkt_p, size_t* len_p)
{
    ...
    {
        result =
            isp->hook_realloc_build(sp, pdu, pktbuf_p, pktbuf_len_p, &offset);
...
int _build_initial_pdu_packet(struct session_list* slp, netsnmp_pdu* pdu,
                              int bulk)
{
    netsnmp_session* session;
    struct snmp_internal_session* isp;
    ...
    {
        DEBUGMSGTL(
            ("sess_async_send", "couldn't malloc initial packet buffer\n"));
        session->s_snmp_errno = SNMPERR_MALLOC;
        return SNMPERR_MALLOC;
    }
    ...
    {
        DEBUGMSGTL(("sess_async_send",
                    "length of packet (%" NETSNMP_PRIz
                    "u) exceeded pdu maximum (%lu)\n",
...
static int _sess_async_send(void* sessp, netsnmp_pdu* pdu,
                            snmp_callback callback, void* cb_data)
{
    struct session_list* slp = (struct session_list*)sessp;
    netsnmp_session* session;
    ...
    DEBUGMSGTL(("sess_process_packet",
                "sending message id#%ld reqid#%ld len %" NETSNMP_PRIz "u\n",
    ...
    if (result < 0)
    {
        ...
        if (callback)
            callback(NETSNMP_CALLBACK_OP_SEND_FAILED, session, pdu->reqid, pdu,
                     cb_data);
        return 0;
    }
...
int snmp_sess_async_send(void* sessp, netsnmp_pdu* pdu, snmp_callback callback,
                         void* cb_data)
{
    int rc;
...
netsnmp_pdu* snmp_create_sess_pdu(netsnmp_transport* transport, void* opaque,
                                  size_t olength)
{
...
static netsnmp_pdu*
    _sess_process_packet_parse_pdu(void* sessp, netsnmp_session* sp,
                                   struct snmp_internal_session* isp,
                                   netsnmp_transport* transport, void* opaque,
                                   int olength, u_char* packetptr, int length)
{
    netsnmp_pdu* pdu;
    int ret = 0;
    int dump = 0, filter = 0;
    ...
    filter =
        netsnmp_ds_get_int(NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_FILTER_TYPE);
#endif
    if (dump || filter)
    {
        int filtered = 0;
        ...
        if (filter)
        {
            ...
            if (dropstr)
            {
                DEBUGMSGTL(("sess_process_packet:filter", "packet from %s %s\n",
    ...
    DEBUGMSGTL(("sess_process_packet",
                "received message id#%ld reqid#%ld len "
                "%u\n",
 ...
 */
static int _sess_process_packet_handle_pdu(void* sessp, netsnmp_session* sp,
                                           struct snmp_internal_session* isp,
                                           netsnmp_transport* transport,
                                           netsnmp_pdu* pdu)
{
    ...
    if (pdu->flags & UCD_MSG_FLAG_RESPONSE_PDU)
    {
        ...
        for (rp = isp->requests; rp; orp = rp, rp = rp->next_request)
        {
            ...
            if (callback == NULL ||
                callback(NETSNMP_CALLBACK_OP_RECEIVED_MESSAGE, sp, pdu->reqid,
                         pdu, magic) == 1)
            {
                if (pdu->command == SNMP_MSG_REPORT)
                {
            ...
            /*
             * MTR snmp_res_unlock(MT_LIBRARY_ID, MT_LIB_SESSION);  ?* XX lock
             * should be per session !
             */
        }
    }
    else
    {
        if (sp->callback)
        {
             ...
             */
            handled = 1;
            sp->callback(NETSNMP_CALLBACK_OP_RECEIVED_MESSAGE, sp, pdu->reqid,
 ...
 */
static int _sess_process_packet(void* sessp, netsnmp_session* sp,
                                struct snmp_internal_session* isp,
                                netsnmp_transport* transport, void* opaque,
                                int olength, u_char* packetptr, int length)
{
 ...
 */
static int _sess_read_accept(void* sessp)
{
    struct session_list* slp = (struct session_list*)sessp;
    ...
    nslp = (struct session_list*)snmp_sess_add_ex(
        sp, new_transport, isp->hook_pre, isp->hook_parse, isp->hook_post,
        isp->hook_build, isp->hook_realloc_build, isp->check_packet,
        isp->hook_create_pdu);
 ...
 */
static int _sess_read_dgram_packet(void* sessp, netsnmp_large_fd_set* fdset,
                                   snmp_rcv_packet* rcvp)
{
    ...
    rcvp->packet_len =
        netsnmp_transport_recv(transport, rcvp->packet, SNMP_MAX_RCV_MSG_SIZE,
...
int _sess_read(void* sessp, netsnmp_large_fd_set* fdset)
{
    ...
    {
        DEBUGMSGTL(
            ("sess_read", "not reading %d (fdset %p set %d)\n", transport->sock,
             fdset,
             fdset ? NETSNMP_LARGE_FD_ISSET(transport->sock, fdset) : -9));
        return 0;
    }
    ...
    if (!(transport->flags & NETSNMP_TRANSPORT_FLAG_STREAM))
    {
        ...
        rc = _sess_process_packet(sessp, sp, isp, transport, rcvp.opaque,
                                  rcvp.olength, rcvp.packet, rcvp.packet_len);
        SNMP_FREE(rcvp.packet);
    ...
    if (isp->packet == NULL)
    {
        ...
        if ((isp->packet = (u_char*)malloc(rxbuf_len)) == NULL)
        {
            DEBUGMSGTL(("sess_read",
                        "can't malloc %" NETSNMP_PRIz "u bytes for rxbuf\n",
                        rxbuf_len));
            return 0;
        }
        else
    ...
    length =
        netsnmp_transport_recv(transport, rxbuf, rxbuf_len, &opaque, &olength);
...

    {
        ...
        while (isp->packet_len > 0)
        {
            ...
            DEBUGMSGTL(("sess_read",
                        "  loop packet_len %" NETSNMP_PRIz
                        "u, PDU length %" NETSNMP_PRIz "u\n",
            ...
            if (pdulen > SNMP_MAX_PACKET_LEN)
            {
                 ...
                 */
                snmp_log(LOG_ERR, "Received broken packet. Closing session.\n");
                if (sp->callback != NULL)
                {
                    DEBUGMSGTL(
            ...
            if (pdulen > isp->packet_len || pdulen == 0)
            {
                 ...
                 */
                DEBUGMSGTL(("sess_read",
                            "pkt not complete (need %" NETSNMP_PRIz
                            "u got %" NETSNMP_PRIz "u so far)\n",
        ...
        pointer because we may need to reuse it for a later packet.  */
            ...
            if ((rc = _sess_process_packet(sessp, sp, isp, transport, ocopy,
                                           ocopy ? olength : 0, pptr, pdulen)))
            {
                /*
        ...
        DEBUGMSGTL(("sess_read",
                    "end: memmove(%p, %p, %" NETSNMP_PRIz
                    "u); realloc(%p, %" NETSNMP_PRIz "u)\n",
 ...
 * @see See also snmp_sess_select_info2_flags().
 */
int snmp_select_info2(int* numfds, netsnmp_large_fd_set* fdset,
 ...
 * @see See also snmp_sess_select_info2_flags().
 */
int snmp_sess_select_info_flags(void* sessp, int* numfds, fd_set* fdset,
                                struct timeval* timeout, int* block, int flags)
{
    ...
    netsnmp_large_fd_set_init(&lfdset, FD_SETSIZE);
    netsnmp_copy_fd_set_to_large_fd_set(&lfdset, fdset);
    rc = snmp_sess_select_info2_flags(sessp, numfds, &lfdset, timeout, block,
                                      flags);
    if (netsnmp_copy_large_fd_set_to_fd_set(fdset, &lfdset) < 0)
    {
        snmp_log(LOG_ERR, "Use snmp_sess_select_info2() for processing"
 ...
 * @see See also snmp_sess_select_info2_flags().
 */
int snmp_sess_select_info2(void* sessp, int* numfds,
                           netsnmp_large_fd_set* fdset, struct timeval* timeout,
                           int* block)
{
 ...
 *   function.
 */
int snmp_sess_select_info2_flags(void* sessp, int* numfds,
                                 ...
                                 struct timeval* timeout, int* block, int flags)
{
    ...
    DEBUGMSGTL(("sess_select", "for %s session%s: ", sessp ? "single" : "all",
                sessp ? "" : "s"));
                               ...
                               netsnmp_request_list* rp, int incr_retries)
{
    ...
    DEBUGMSGTL(("sess_process_packet",
                "resending message id#%ld reqid#%ld "
    ...
    if (result < 0)
    {
        ...
        snmp_set_detail(strerror(errno));
        if (rp->callback)
            rp->callback(NETSNMP_CALLBACK_OP_SEND_FAILED, sp, rp->pdu->reqid,
                         rp->pdu, rp->cb_data);
        return -1;
    }
    else
    {
        ...
        rp->expireM = tv;
        if (rp->callback)
            rp->callback(NETSNMP_CALLBACK_OP_RESEND, sp, rp->pdu->reqid,
                         rp->pdu, rp->cb_data);
    }
    return 0;
}
    ...
    for (rp = isp->requests; rp; rp = rp->next_request)
    {
        ...
        if ((timercmp(&rp->expireM, &now, <)))
        {
            ...
            if (rp->retries >= sp->retries)
            {
                ...
                if (callback)
                {
                    callback(NETSNMP_CALLBACK_OP_TIMED_OUT, sp, rp->pdu->reqid,
                             rp->pdu, magic);
                }
                if (orp)
                    orp->next_request = rp->next_request;
 ...
 */
int snmp_oid_ncompare(const oid* in_name1, size_t len1, const oid* in_name2,
                      size_t len2, size_t max_len)
{
    register int len;
 ...
 */
int snmp_oid_compare(const oid* in_name1, size_t len1, const oid* in_name2,
                     size_t len2)
{
    register int len;
    register const oid* name1 = in_name1;
 ...
 *          command responder applications (ie, agent).
 *
 * @return -1 if name1 < name2, 0 if name1 = name2, 1 if name1 > name2 and offpt
 * = len where name1 != name2
 */
 ...
 */
int snmp_oidtree_compare(const oid* in_name1, size_t len1, const oid* in_name2,
                         size_t len2)
{
...
int snmp_oidsubtree_compare(const oid* in_name1, size_t len1,
 ...
 */
int netsnmp_oid_equals(const oid* in_name1, size_t len1, const oid* in_name2,
                       size_t len2)
{
    register const oid* name1 = in_name1;
    register const oid* name2 = in_name2;
 ...
 * compared. Functionally, this determines if in_name2 is equal or a subtree of
 * in_name1
 * @param in_name1 A pointer to the first oid.
 * @param len1     length of the first OID (in segments, not bytes)
 * @param in_name2 A pointer to the second oid.
 ...
 */
netsnmp_variable_list* snmp_pdu_add_variable(netsnmp_pdu* pdu, const oid* name,
...
netsnmp_variable_list*
    snmp_varlist_add_variable(netsnmp_variable_list** varlist, const oid* name,
                              size_t name_length, u_char type,
                              const void* value, size_t len)
{
 ...
 */
int snmp_add_var(netsnmp_pdu* pdu, const oid* name, size_t name_length,
    ...
    switch (type)
    {
        case 'i':
            ...
            if (!_check_range(tp, ltmp, &result, value))
                break;
#endif /* NETSNMP_DISABLE_MIB_LOADING */
            snmp_pdu_add_variable(pdu, name, name_length, ASN_INTEGER, &ltmp,
                                  sizeof(ltmp));
            break;
        ...
        case 'd':
            ...
            if ('s' == type && do_hint &&
                !parse_octet_hint(tp->hint, value, &hintptr, &itmp))
            {
                if (_check_range(tp, itmp, &result,
                                 "Value does not match DISPLAY-HINT"))
                {
            ...
            if (type == 'd')
            {
                if (!snmp_decimal_to_binary(&buf, &buf_len, &value_len, 1,
                                            value))
                {
                    result = SNMPERR_VALUE;
                    snmp_set_detail(value);
        ...
        case 'b':
            ...
            SNMP_FREE(vp);
            snmp_pdu_add_variable(pdu, name, name_length, ASN_OCTET_STR, buf,
                                  tint);
            break;
...
type_error:
{
    ...
    snprintf(error_msg, sizeof(error_msg), "Type of attribute is %s, not %s",
             var_type, value);
