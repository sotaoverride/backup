 ...
 */
static int use_content_info(krb5_context context, pkinit_req_context req,
...
static krb5_error_code
    pkinit_as_req_create(krb5_context context, pkinit_context plgctx,
                         ...
                         const krb5_checksum* cksum, krb5_principal client,
                         krb5_principal server, krb5_data** as_req);

static krb5_error_code pkinit_as_rep_parse(
    krb5_context context, pkinit_context plgctx, pkinit_req_context reqctx,
    krb5_preauthtype pa_type, krb5_kdc_req* request, const krb5_data* as_rep,
...
static krb5_error_code
    pa_pkinit_gen_req(krb5_context context, pkinit_context plgctx,
                      pkinit_req_context reqctx, krb5_clpreauth_callbacks cb,
                      krb5_clpreauth_rock rock, krb5_kdc_req* request,
                      krb5_preauthtype pa_type, krb5_pa_data*** out_padata,
                      ...
                      krb5_get_init_creds_opt* gic_opt)
{
    ...
    retval = krb5_c_make_checksum(context, CKSUMTYPE_NIST_SHA, NULL, 0, der_req,
                                  &cksum);
    if (retval)
        goto cleanup;
    ...
    retval = pkinit_as_req_create(context, plgctx, reqctx, ctsec, cusec, nonce,
                                  &cksum, request->client, request->server,
                                  &out_data);
    if (retval)
    {
                         ...
                         krb5_principal server, krb5_data** as_req)
{
    ...
    switch (protocol)
    {
        case DH_PROTOCOL:
            ...
            retval = client_create_dh(
                context, plgctx->cryptoctx, reqctx->cryptoctx, reqctx->idctx,
                reqctx->opts->dh_size, &dh_params, &dh_params_len, &dh_pubkey,
                &dh_pubkey_len);
            if (retval != 0)
            {
    ...
    print_buffer_bin((unsigned char*)coded_auth_pack->data,
                     coded_auth_pack->length, "/tmp/client_auth_pack");
#endif
    ...
    switch ((int)reqctx->pa_type)
    {
        case KRB5_PADATA_PK_AS_REQ:
            ...
            else
            {
                retval = cms_signeddata_create(
                    context, plgctx->cryptoctx, reqctx->cryptoctx,
                    reqctx->idctx, CMS_SIGN_CLIENT, 1,
                    (unsigned char*)coded_auth_pack->data,
                    coded_auth_pack->length,
                    (unsigned char**)&req->signedAuthPack.data,
                    &req->signedAuthPack.length);
        ...
        case KRB5_PADATA_PK_AS_REQ_OLD:
            ...
            retval = cms_signeddata_create(
                context, plgctx->cryptoctx, reqctx->cryptoctx, reqctx->idctx,
                CMS_SIGN_DRAFT9, 1, (unsigned char*)coded_auth_pack->data,
                coded_auth_pack->length,
                (unsigned char**)&req9->signedAuthPack.data,
    ...
    {
        case KRB5_PADATA_PK_AS_REQ:
            retval = create_krb5_trustedCertifiers(
                context, plgctx->cryptoctx, reqctx->cryptoctx, reqctx->idctx,
                &req->trustedCertifiers);
            if (retval)
                goto cleanup;
            retval = create_issuerAndSerial(
                context, plgctx->cryptoctx, reqctx->cryptoctx, reqctx->idctx,
                (unsigned char**)&req->kdcPkId.data, &req->kdcPkId.length);
            if (retval)
                goto cleanup;
        ...
        case KRB5_PADATA_PK_AS_REQ_OLD:
            retval = create_issuerAndSerial(
                context, plgctx->cryptoctx, reqctx->cryptoctx, reqctx->idctx,
                (unsigned char**)&req9->kdcCert.data, &req9->kdcCert.length);
            if (retval)
                goto cleanup;
            /* Encode the as-req */
...
static krb5_error_code
    pa_pkinit_parse_rep(krb5_context context, pkinit_context plgctx,
                        pkinit_req_context reqctx, krb5_kdc_req* request,
                        krb5_pa_data* in_padata, krb5_enctype etype,
                        krb5_keyblock* as_key, krb5_data* encoded_request)
{
    krb5_error_code retval = KRB5KDC_ERR_PREAUTH_FAILED;
...
static krb5_error_code verify_kdc_san(krb5_context context,
                                      pkinit_context plgctx,
                                      pkinit_req_context reqctx,
                                      krb5_principal kdcprinc, int* valid_san,
                                      int* need_eku_checking)
{
    ...
    *need_eku_checking = 1;
    ...
    retval =
        pkinit_libdefault_strings(context, krb5_princ_realm(context, kdcprinc),
                                  KRB5_CONF_PKINIT_KDC_HOSTNAME, &cfghosts);
    ...
    retval =
        crypto_retrieve_cert_sans(context, plgctx->cryptoctx, reqctx->cryptoctx,
                                  reqctx->idctx, &princs, NULL, get_dns);
    if (retval)
    {
...
static krb5_error_code verify_kdc_eku(krb5_context context,
                                      pkinit_context plgctx,
                                      pkinit_req_context reqctx,
                                      int* eku_accepted)
{
    ...
    *eku_accepted = 0;
    ...
    retval =
        crypto_check_cert_eku(context, plgctx->cryptoctx, reqctx->cryptoctx,
                              reqctx->idctx, 1, /* kdc cert */
                              reqctx->opts->accept_secondary_eku, eku_accepted);
    if (retval)
    {
...
static krb5_error_code pkinit_as_rep_parse(
    krb5_context context, pkinit_context plgctx, pkinit_req_context reqctx,
    krb5_preauthtype pa_type, krb5_kdc_req* request, const krb5_data* as_rep,
    krb5_keyblock* key_block, krb5_enctype etype, krb5_data* encoded_request)
{
    krb5_error_code retval = KRB5KDC_ERR_PREAUTH_FAILED;
    ...
    switch (kdc_reply->choice)
    {
...
#endif
            if ((retval = cms_signeddata_verify(
                     context, plgctx->cryptoctx, reqctx->cryptoctx,
                     reqctx->idctx, CMS_SIGN_SERVER,
                     reqctx->opts->require_crl_checking,
                     (unsigned char*)kdc_reply->u.dh_Info.dhSignedData.data,
                     kdc_reply->u.dh_Info.dhSignedData.length,
                     (unsigned char**)&dh_data.data, &dh_data.length, NULL,
                     NULL, NULL)) != 0)
            {
        ...
        case choice_pa_pk_as_rep_encKeyPack:
            ...
            if ((retval = cms_envelopeddata_verify(
                     context, plgctx->cryptoctx, reqctx->cryptoctx,
                     reqctx->idctx, pa_type, reqctx->opts->require_crl_checking,
                     (unsigned char*)kdc_reply->u.encKeyPack.data,
                     kdc_reply->u.encKeyPack.length,
                     (unsigned char**)&dh_data.data, &dh_data.length)) != 0)
            {
    ...
    retval = krb5_build_principal_ext(
        context, &kdc_princ, request->server->realm.length,
        request->server->realm.data, strlen(KRB5_TGS_NAME), KRB5_TGS_NAME,
        request->server->realm.length, request->server->realm.data, 0);
    if (retval)
        goto cleanup;
    retval = verify_kdc_san(context, plgctx, reqctx, kdc_princ, &valid_san,
                            &need_eku_checking);
    if (retval)
        goto cleanup;
    if (!valid_san)
    ...
    {
        retval = verify_kdc_eku(context, plgctx, reqctx, &valid_eku);
        if (retval)
            goto cleanup;
        if (!valid_eku)
        {
    ...
    switch (kdc_reply->choice)
    {
        case choice_pa_pk_as_rep_dhInfo:
...
#endif
            if ((retval =
                     k5int_decode_krb5_kdc_dh_key_info(&k5data, &kdc_dh)) != 0)
            {
            ...
            if (kdc_reply->u.dh_Info.kdfID)
            {
                ...
                retval = pkinit_alg_agility_kdf(
                    context, &secret, kdc_reply->u.dh_Info.kdfID,
                    request->client, request->server, etype, encoded_request,
        ...
        case choice_pa_pk_as_rep_encKeyPack:
            ...
            retval = krb5_c_make_checksum(
                context, key_pack->asChecksum.checksum_type,
...
static void pkinit_client_profile(krb5_context context, pkinit_context plgctx,
                                  pkinit_req_context reqctx,
                                  ...
                                  const krb5_data* realm)
{
    ...
    pkinit_libdefault_boolean(context, realm,
                              ...
                              reqctx->opts->require_crl_checking,
                              &reqctx->opts->require_crl_checking);
    pkinit_libdefault_integer(context, realm, KRB5_CONF_PKINIT_DH_MIN_BITS,
                              reqctx->opts->dh_size, &reqctx->opts->dh_size);
        ...
        reqctx->opts->dh_size != 4096)
    {
        pkiDebug("%s: invalid value (%d) for pkinit_dh_min_bits, "
                 ...
                 __FUNCTION__, reqctx->opts->dh_size,
                 PKINIT_DEFAULT_DH_MIN_BITS);
        reqctx->opts->dh_size = PKINIT_DEFAULT_DH_MIN_BITS;
    }
    pkinit_libdefault_string(context, realm, KRB5_CONF_PKINIT_EKU_CHECKING,
                             &eku_string);
    if (eku_string != NULL)
    {
    ...
    if (reqctx->idopts->anchors == NULL)
        pkinit_libdefault_strings(context, realm, KRB5_CONF_PKINIT_ANCHORS,
                                  &reqctx->idopts->anchors);
    pkinit_libdefault_strings(context, realm, KRB5_CONF_PKINIT_POOL,
                              &reqctx->idopts->intermediates);
    pkinit_libdefault_strings(context, realm, KRB5_CONF_PKINIT_REVOKE,
                              &reqctx->idopts->crls);
    pkinit_libdefault_strings(context, realm, KRB5_CONF_PKINIT_IDENTITIES,
 ...
 */
static long long pkinit_client_get_token_flags(unsigned long pkcs11_token_flags)
{
    long long flags = 0;
 ...
 */
static krb5_error_code pkinit_client_prep_questions(
    ...
    krb5_data* encoded_previous_request, krb5_pa_data* pa_data)
{
    ...
    if (!reqctx->identity_initialized)
    {
        ...
        retval = pkinit_identity_initialize(
            context, plgctx->cryptoctx, reqctx->cryptoctx, reqctx->idopts,
            reqctx->idctx, cb, rock, request->client);
        if (retval != 0)
        {
    ...
    if (retval == 0)
    {
        cb->ask_responder_question(context, rock,
                                   KRB5_RESPONDER_QUESTION_PKINIT, encoded);
...
static void save_one_password(void* arg, const char* key, k5_json_value val)
{
...
static krb5_error_code pkinit_client_parse_answers(
    krb5_context context, krb5_clpreauth_moddata moddata,
    krb5_clpreauth_modreq modreq, krb5_clpreauth_callbacks cb,
    krb5_clpreauth_rock rock)
{
    krb5_error_code retval;
    const char* encoded;
    ...
    encoded =
        cb->get_responder_answer(context, rock, KRB5_RESPONDER_QUESTION_PKINIT);
    if (encoded == NULL)
        return 0;
...
static krb5_error_code pkinit_client_process(
    krb5_context context, krb5_clpreauth_moddata moddata,
    krb5_clpreauth_modreq modreq, krb5_get_init_creds_opt* gic_opt,
    krb5_clpreauth_callbacks cb, krb5_clpreauth_rock rock,
    krb5_kdc_req* request, krb5_data* encoded_request_body,
    krb5_data* encoded_previous_request, krb5_pa_data* in_padata,
    krb5_prompter_fct prompter, void* prompter_data, krb5_pa_data*** out_padata)
{
    krb5_error_code retval = KRB5KDC_ERR_PREAUTH_FAILED;
    ...
    if (processing_request)
    {
        ...
        if (!reqctx->identity_prompted)
        {
            ...
            pkinit_identity_set_prompter(reqctx->idctx, prompter,
                                         prompter_data);
            retval = pkinit_identity_prompt(
                context, plgctx->cryptoctx, reqctx->cryptoctx, reqctx->idopts,
                reqctx->idctx, cb, rock, reqctx->do_identity_matching,
                request->client);
...
static krb5_error_code pkinit_client_tryagain(
    krb5_context context, krb5_clpreauth_moddata moddata,
    krb5_clpreauth_modreq modreq, krb5_get_init_creds_opt* gic_opt,
    krb5_clpreauth_callbacks cb, krb5_clpreauth_rock rock,
    krb5_kdc_req* request, krb5_data* encoded_request_body,
    krb5_data* encoded_previous_request, krb5_preauthtype pa_type,
    ...
    krb5_prompter_fct prompter, void* prompter_data, krb5_pa_data*** out_padata)
{
    ...
    for (; *err_padata != NULL && !do_again; err_padata++)
    {
        ...
        switch (pa->pa_type)
        {
            ...
            case TD_INVALID_CERTIFICATES:
                ...
                retval = pkinit_process_td_trusted_certifiers(
                    context, plgctx->cryptoctx, reqctx->cryptoctx,
                    reqctx->idctx, certifiers, pa->pa_type);
                if (!retval)
                    do_again = 1;
                break;
            case TD_DH_PARAMETERS:
                ...
                retval = pkinit_process_td_dh_params(
                    context, plgctx->cryptoctx, reqctx->cryptoctx,
                    reqctx->idctx, algId, &reqctx->opts->dh_size);
                if (!retval)
                    do_again = 1;
                break;
            default:
    ...
    if (do_again)
    {
        ...
        retval = pa_pkinit_gen_req(context, plgctx, reqctx, cb, rock, request,
                                   pa_type, out_padata, prompter, prompter_data,
                                   gic_opt);
        if (retval)
            goto cleanup;
    }
...
static int pkinit_client_get_flags(krb5_context kcontext,
...
static void pkinit_client_req_init(krb5_context context,
                                   krb5_clpreauth_moddata moddata,
...
static void pkinit_client_req_fini(krb5_context context,
                                   krb5_clpreauth_moddata moddata,
                                   krb5_clpreauth_modreq modreq)
{
    pkinit_req_context reqctx = (pkinit_req_context)modreq;
...
static int pkinit_client_plugin_init(krb5_context context,
...
static krb5_error_code add_string_to_array(krb5_context context, char*** array,
                                           const char* addition)
{
    char** a = *array;
    size_t len;
...
static krb5_error_code handle_gic_opt(krb5_context context,
                                      pkinit_context plgctx, const char* attr,
                                      const char* value)
{
    krb5_error_code retval;
...
static krb5_error_code pkinit_client_gic_opt(krb5_context context,
                                             krb5_clpreauth_moddata moddata,
                                             krb5_get_init_creds_opt* gic_opt,
                                             const char* attr,
                                             const char* value)
...
krb5_error_code clpreauth_pkinit_initvt(krb5_context context, int maj_ver,
