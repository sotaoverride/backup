...
{
    char path[UNIX_PATH_MAX]; /* Not including the initial zero in abstract
                     case, so we can guarantee zero termination of abstract
                     pathnames in the get_path() API */
    gsize path_len;           /* Not including any terminating zeros */
    GUnixSocketAddressType address_type;
};

static void g_unix_socket_address_connectable_iface_init(
    GSocketConnectableIface* iface);
static gchar* g_unix_socket_address_connectable_to_string(
    GSocketConnectable* connectable);

G_DEFINE_TYPE_WITH_CODE(
    GUnixSocketAddress, g_unix_socket_address, G_TYPE_SOCKET_ADDRESS,
    G_ADD_PRIVATE(GUnixSocketAddress)
        G_IMPLEMENT_INTERFACE(G_TYPE_SOCKET_CONNECTABLE,
                              g_unix_socket_address_connectable_iface_init))

static void g_unix_socket_address_set_property(GObject* object, guint prop_id,
                                               const GValue* value,
                                               GParamSpec* pspec)
{
    ...
    switch (prop_id)
    {
        ...
        case PROP_PATH_AS_ARRAY:
            ...
            if (array)
            {
                /* Clip to fit in UNIX_PATH_MAX with zero termination or first
                 * byte */
...
static void g_unix_socket_address_get_property(GObject* object, guint prop_id,
                                               GValue* value, GParamSpec* pspec)
{
    ...
    switch (prop_id)
    {
        ...
        case PROP_PATH_AS_ARRAY:
            ...
            g_byte_array_append(array, (guint8*)address->priv->path,
                                address->priv->path_len);
...
static gssize g_unix_socket_address_get_native_size(GSocketAddress* address)
{
    ...
    switch (addr->priv->address_type)
    {
        ...
        case G_UNIX_SOCKET_ADDRESS_ABSTRACT:
            return G_STRUCT_OFFSET(struct sockaddr_un, sun_path) +
                   addr->priv->path_len + 1;
        default:
            return sizeof(struct sockaddr_un);
    }
}

static gboolean g_unix_socket_address_to_native(GSocketAddress* address,
        ...
        case G_UNIX_SOCKET_ADDRESS_ABSTRACT_PADDED:
            if (!g_unix_socket_address_abstract_names_supported())
            {
                g_set_error_literal(error, G_IO_ERROR, G_IO_ERROR_NOT_SUPPORTED,
                                    _("Abstract UNIX domain socket addresses "
...
static void g_unix_socket_address_class_init(GUnixSocketAddressClass* klass)
{
    ...
    gsocketaddress_class->get_native_size =
        g_unix_socket_address_get_native_size;

    g_object_class_install_property(
        gobject_class, PROP_PATH,
        g_param_spec_string("path", P_("Path"), P_("UNIX socket path"), NULL,
                            G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY |
                                G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(
        gobject_class, PROP_PATH_AS_ARRAY,
        g_param_spec_boxed("path-as-array", P_("Path array"),
     ...
     */
    g_object_class_install_property(
        gobject_class, PROP_ABSTRACT,
        g_param_spec_boolean("abstract", P_("Abstract"),
                             P_("Whether or not this is an abstract address"),
                             FALSE,
                             G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY |
                                 G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(
        gobject_class, PROP_ADDRESS_TYPE,
        g_param_spec_enum("address-type", P_("Address type"),
    ...
    g_unix_socket_address_connectable_to_string(GSocketConnectable* connectable)
{
    ...
    /* Return the #GUnixSocketAddress:path, but with all non-printable
     * characters (including nul bytes) escaped to hex. */
...
{
    return g_object_new(G_TYPE_UNIX_SOCKET_ADDRESS, "path", path, "abstract",
                        FALSE, NULL);
}
                                                   ...
                                                   gint path_len)
{
    return g_unix_socket_address_new_with_type(
        path, path_len, G_UNIX_SOCKET_ADDRESS_ABSTRACT_PADDED);
}
                                                    ...
                                                    GUnixSocketAddressType type)
{
    ...
    address = g_object_new(G_TYPE_UNIX_SOCKET_ADDRESS, "path-as-array", array,
