...
static gboolean check_integer_match(guint64 expected, const gchar* value,
                                    guint32 value_size)
{
...
static gboolean check_png_info_chunk(ExpectedInfo* expected_info,
                                     const gchar* key, guint32 key_size,
                                     const gchar* value, guint32 value_size,
...
static gboolean check_thumbnail_validity(ExpectedInfo* expected_info,
        ...
        if (memcmp(contents, "tEXt", 4) == 0)
        {
            ...
            for (key_size = 0; key_size < chunk_size; key_size++)
            {
                if (key[key_size] == '\0')
                {
                    ...
                    if (!check_png_info_chunk(expected_info, key, key_size,
                                              value, value_size,
                                              &required_matches))
                        return FALSE;
                }
...
gboolean thumbnail_verify(const char* thumbnail_path, const gchar* file_uri,
                          const GLocalFileStat* file_stat_buf)
{
    ...
    if (file)
    {
        thumbnail_is_valid = check_thumbnail_validity(
            &expected_info, g_mapped_file_get_contents(file),
