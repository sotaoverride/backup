...
struct dnscache_entry_s
{
    ...
    prop_t*
        localName; /* only local name, without domain part (if configured so) */
    prop_t* ip;
...
#pragma GCC diagnostic ignored                                                 \
    "-Wmaybe-uninitialized" /* TODO: how can we fix these warnings? */
#endif
...
static inline dnscache_entry_t* findEntry(struct sockaddr_storage* addr)
{
...
static void setLocalHostName(dnscache_entry_t* etry)
{
    ...
    p = (uchar*)strchr(
        (char*)fqdnLower,
        '.'); /* find start of domain name "machine.example.com" */
    if (p == NULL)
    {                                     /* do we have a domain part? */
     ...
     * and so should be stripped also. If so, we do it and return. Please note
     * that -l list FQDNs, not just the hostname part. If it did just list the
     * hostname, the door would be wide-open for all kinds of mixing up of
     * hosts. Because of this, you'll see comparison against the full string
     * (pszHostFQDN) below.
     */
    ...
    resolveAddr(struct sockaddr_storage* addr, dnscache_entry_t* etry)
{
    ...
    if (!glbl.GetDisableDNS())
    {
        ...
        error =
            mygetnameinfo((struct sockaddr*)addr, SALEN((struct sockaddr*)addr),
        ...
        if (error == 0)
        {
            ...
            if (getaddrinfo(fqdnBuf, NULL, &hints, &res) == 0)
            {
                uchar szErrMsg[1024];
                freeaddrinfo(res);
                /* OK, we know we have evil. The question now is what to do
                 * about it. One the one hand, the message might probably be
                 * intended to harm us. On the other hand, losing the message
                 * may also harm us. Thus, the behaviour is controlled by the
                 * $DropMsgsWithMaliciousDnsPTRRecords option. If it tells us we
                 * should discard, we do so, else we proceed, but log an error
                 * message together with it. time being, we simply drop the name
                 * we obtained and use the IP - that one is OK in any way. We do
                 * also log the error message. rgerhards, 2007-07-16
                 */
                ...
                /* Please note: we deal with a malicous entry. Thus, we have
                 * crafted the snprintf() below so that all text is in front of
                 * the entry - maybe it contains characters that make the
                 * message unreadable (OK, I admit this is more or less
                 * impossible, but I am paranoid...) rgerhards, 2007-07-16
                 */
                snprintf((char*)szErrMsg, sizeof(szErrMsg),
            ...
            { /* we have a valid entry, so let's create the respective
                 properties */
                fqdnLen = strlen(fqdnBuf);
                prop.CreateStringProp(&etry->fqdn, (uchar*)fqdnBuf, fqdnLen);
                for (i = 0; i < fqdnLen; ++i)
                    fqdnBuf[i] = tolower(fqdnBuf[i]);
                prop.CreateStringProp(&etry->fqdnLowerCase, (uchar*)fqdnBuf,
                                      fqdnLen);
            }
        }
        pthread_sigmask(SIG_SETMASK, &omask, NULL);
...
static rsRetVal ATTR_NONNULL() addEntry(struct sockaddr_storage* const addr,
                                        dnscache_entry_t** const pEtry)
{
    int r;
...
static inline rsRetVal
    validateEntry(dnscache_entry_t __attribute__((unused)) * etry,
                  struct sockaddr_storage __attribute__((unused)) * addr)
{
    return RS_RET_OK;
}
 ...
 */
rsRetVal dnscacheLookup(struct sockaddr_storage* addr, prop_t** fqdn,
                        prop_t** fqdnLowerCase, prop_t** localName, prop_t** ip)
{
