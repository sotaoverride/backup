...
{
    return IN_SET(type, UNIT_SERVICE, UNIT_SOCKET, UNIT_TARGET, UNIT_DEVICE,
                  UNIT_TIMER, UNIT_PATH);
}

bool unit_type_may_template(UnitType type)
{
    return IN_SET(type, UNIT_SERVICE, UNIT_SOCKET, UNIT_TARGET, UNIT_TIMER,
                  UNIT_PATH);
}
                          ...
                          bool check_parent)
{
    ...
    /* Note that we do *not* have generic checks for /etc or /run in place,
     * since with them we couldn't discern configuration from transient or
     * generated units */
...
int unit_file_changes_add(UnitFileChange** changes, size_t* n_changes,
                          UnitFileChangeType type, const char* path,
                          const char* source)
{
...
void unit_file_dump_changes(int r, const char* verb,
                            const UnitFileChange* changes, size_t n_changes,
                            bool quiet)
{
    size_t i;
    bool logged = false;
    ...
    for (i = 0; i < n_changes; i++)
    {
        ...
        switch (changes[i].type)
        {
            case UNIT_FILE_SYMLINK:
                if (!quiet)
                    log_info("Created symlink %s %s %s.", changes[i].path,
            ...
            case UNIT_FILE_IS_DANGLING:
                if (!quiet)
                    log_info("Unit %s is an alias to a unit that is not "
                             "present, ignoring.",
                             changes[i].path);
                break;
            case -EEXIST:
                if (changes[i].source)
                    log_error_errno(changes[i].type,
                                    ...
                                    "and is a symlink to %s.",
                                    verb, changes[i].path, changes[i].source);
                else
                    log_error_errno(
                        changes[i].type,
                        "Failed to %s unit, file %s already exists.", verb,
                        changes[i].path);
                logged = true;
                break;
            case -ERFKILL:
                log_error_errno(changes[i].type,
                                "Failed to %s unit, unit %s is masked.", verb,
                                changes[i].path);
                logged = true;
                break;
            case -EADDRNOTAVAIL:
            ...
            case -ELOOP:
                log_error_errno(changes[i].type,
                                "Failed to %s unit, refusing to operate on "
                                "linked unit file %s",
                                verb, changes[i].path);
                logged = true;
                break;
            ...
            default:
                ...
                log_error_errno(changes[i].type,
                                "Failed to %s unit, file %s: %m.", verb,
                                changes[i].path);
                logged = true;
        }
    }
...
/**
 * Checks if two paths or symlinks from wd are the same, when root is the root
 * of the filesystem. wc should be the full path in the host file system.
 */
static bool chroot_symlinks_same(const char* root, const char* wd,
...
static int create_symlink(const LookupPaths* paths, const char* old_path,
                          const char* new_path, bool force,
                          UnitFileChange** changes, size_t* n_changes)
{
    ...
    {
        unit_file_changes_add(changes, n_changes, UNIT_FILE_SYMLINK, new_path,
                              old_path);
        return 1;
    }
...
static int remove_marked_symlinks_fd(Set* remove_symlinks_to, int fd,
                                     const char* path, const char* config_path,
                                     const LookupPaths* lp, bool dry_run,
                                     bool* restart, UnitFileChange** changes,
                                     size_t* n_changes)
{
    ...
    FOREACH_DIRENT(de, d, return -errno)
    {
        ...
        if (de->d_type == DT_DIR)
        {
            ...
            q = remove_marked_symlinks_fd(remove_symlinks_to, nfd, p,
                                          config_path, lp, dry_run, restart,
                                          changes, n_changes);
            if (q < 0 && r == 0)
                r = q;
        }
        else if (de->d_type == DT_LNK)
        {
            ...
            /* We remove all links pointing to a file or path that is marked, as
             * well as all files sharing
            ...
            unit_file_changes_add(changes, n_changes, UNIT_FILE_UNLINK, p,
                                  NULL);
                                  ...
                                  UnitFileChange** changes, size_t* n_changes)
{
    ...
    do
    {
        ...
        q = remove_marked_symlinks_fd(remove_symlinks_to, cfd, config_path,
                                      config_path, lp, dry_run, &restart,
                                      changes, n_changes);
        if (r == 0)
            r = q;
...
static int is_symlink_with_known_name(const UnitFileInstallInfo* i,
                                      const char* name)
{
    int r;
...
static int find_symlinks_fd(const char* root_dir, const UnitFileInstallInfo* i,
                            bool match_aliases, bool ignore_same_name, int fd,
                            const char* path, const char* config_path,
                            bool* same_name_link)
{
    ...
    FOREACH_DIRENT(de, d, return -errno)
    {
        ...
        if (de->d_type == DT_DIR)
        {
            ...
            /* This will close nfd, regardless whether it succeeds or not */
            q = find_symlinks_fd(root_dir, i, match_aliases, ignore_same_name,
                                 nfd, p, config_path, same_name_link);
            if (q > 0)
                return 1;
            if (r == 0)
                r = q;
        ...
        else if (de->d_type == DT_LNK)
        {
            ...
            if (!ignore_same_name)
                 ...
                 *
                 * If ignore_same_name is specified, we are in one of the
                 * directories which have lower priority than the unit file, and
                 * even if a file or symlink with
                 * this name was found, we should ignore it. */
                found_path = streq(de->d_name, i->name);

            /* Check if what the symlink points to matches what we are looking
             * for */
            ...
            else if (found_path || found_dest)
            {
                ...
                /* Check if symlink name is in the set of names used by
                 * [Install] */
...
static int find_symlinks(const char* root_dir, const UnitFileInstallInfo* i,
                         bool match_name, bool ignore_same_name,
                         const char* config_path, bool* same_name_link)
{
...
static int find_symlinks_in_scope(UnitFileScope scope, const LookupPaths* paths,
                                  const UnitFileInstallInfo* i, bool match_name,
                                  UnitFileState* state)
{
    ...
    /* As we iterate over the list of search paths in paths->search_path, we may
     * encounter "same name" symlinks. The ones which are "below" (i.e. have
     * lower priority) than the unit file itself are efectively masked, so we
     * should ignore them. */
    ...
    STRV_FOREACH(p, paths->search_path)
    {
        ...
        r = find_symlinks(paths->root_dir, i, match_name, ignore_same_name, *p,
                          &same_name_link);
        if (r < 0)
            return r;
        if (r > 0)
        {
            /* We found symlinks in this dir? Yay! Let's see where precisely it
             * is enabled. */
        ...
        /* Check if next iteration will be "below" the unit file (either a
         * regular file or a symlink), and hence should be ignored */
    ...
    /* Here's a special rule: if the unit we are looking for is an instance, and
     * it symlinked in the search path outside of runtime and configuration
     * directory, then we consider it statically enabled. Note we do that only
     * for instance, not for regular names, as those are merely aliases, while
     * instances explicitly instantiate
...
static void install_context_done(InstallContext* c)
{
    ...
    c->will_process = ordered_hashmap_free_with_destructor(c->will_process,
                                                           install_info_free);
    c->have_processed = ordered_hashmap_free_with_destructor(c->have_processed,
                                                             install_info_free);
}

static UnitFileInstallInfo* install_info_find(InstallContext* c,
                                              const char* name)
{
                                    ...
                                    UnitFileChange** changes, size_t* n_changes)
{
    ...
    /* Checks whether the loaded unit file is one we should process, or is
     * masked, transient or generated and thus not subject to enable/disable
     * operations. */
    ...
    {
        unit_file_changes_add(changes, n_changes, -EADDRNOTAVAIL, i->path,
                              NULL);
...
/**
 * Adds a new UnitFileInstallInfo entry under name in the
 * InstallContext.will_process hashmap, or retrieves the existing one if already
 * present.
 *
 * Returns negative on error, 0 if the unit was already known, 1 otherwise.
 */
static int install_info_add(InstallContext* c, const char* name,
...
static int config_parse_alias(const char* unit, const char* filename,
...
static int config_parse_also(const char* unit, const char* filename,
                             unsigned line, const char* section,
                             unsigned section_line, const char* lvalue,
                             int ltype, const char* rvalue, void* data,
                             void* userdata)
...
static int config_parse_default_instance(const char* unit, const char* filename,
                                         unsigned line, const char* section,
...
static int unit_file_load(InstallContext* c, UnitFileInstallInfo* info,
                          const char* path, const char* root_dir,
                          SearchFlags flags)
{
    ...
    if (!(flags & SEARCH_DROPIN))
    {
        ...
        if (unit_name_is_valid(info->name,
                               UNIT_NAME_TEMPLATE | UNIT_NAME_INSTANCE) &&
    ...
    {
        /* Operating on a drop-in file. If we aren't supposed to load the unit
         * file drop-ins don't matter, let's hence shortcut this. */

        if (!(flags & SEARCH_LOAD))
            return 0;

        fd = chase_symlinks_and_open(path, root_dir, 0,
    ...
    /* c is only needed if we actually load the file (it's referenced from
     * items[] btw, in case you wonder.) */
    assert(c);

    r = config_parse(info->name, path, f, NULL, config_item_table_lookup, items,
                                      ...
                                      SearchFlags flags)
{
    ...
    else
    {
        ...
        if (path_is_absolute(target))
            /* This is an absolute path, prefix the root so that we always deal
             * with fully qualified paths */
            info->symlink_target = prefix_root(root_dir, target);
        else
            /* This is a relative path, take it relative to the dir the symlink
             * is located in. */
...
static int unit_file_search(InstallContext* c, UnitFileInstallInfo* info,
                            const LookupPaths* paths, SearchFlags flags)
{
    ...
    if (info->path)
        return unit_file_load_or_readlink(c, info, info->path, paths->root_dir,
                                          flags);
    ...
    if (!found_unit && template)
    {
        ...
        STRV_FOREACH(p, paths->search_path)
        {
            ...
            r = unit_file_load_or_readlink(c, info, path, paths->root_dir,
                                           flags);
            if (r >= 0)
            {
                info->path = TAKE_PTR(path);
    ...
    if (!found_unit)
        return log_debug_errno(SYNTHETIC_ERRNO(ENOENT),
                               "Cannot find unit %s%s%s.", info->name,
    ...
    {
        r = unit_file_load_or_readlink(c, info, *p, paths->root_dir,
                                       flags | SEARCH_DROPIN);
        if (r < 0)
...
static int install_info_follow(InstallContext* c, UnitFileInstallInfo* i,
                               const char* root_dir, SearchFlags flags,
...
/**
 * Search for the unit file. If the unit name is a symlink, follow the symlink
 * to the target, maybe more than once. Propagate the instance name if present.
 */
static int install_info_traverse(UnitFileScope scope, InstallContext* c,
                                 const LookupPaths* paths,
                                 UnitFileInstallInfo* start, SearchFlags flags,
                                 UnitFileInstallInfo** ret)
{
        ...
        if (r == -EXDEV)
        {
                ...
                unit_name_is_valid(bn, UNIT_NAME_TEMPLATE))
            {
                ...
                if (streq(buffer, i->name))
                {
                    ...
                    /* We filled in the instance, and the target stayed the
                     * same? If so, then let's honour the link as it is. */
...
/**
 * Call install_info_add() with name_or_path as the path (if name_or_path starts
 * with "/") or the name (otherwise). root_dir is prepended to the path.
 */
static int install_info_add_auto(InstallContext* c, const LookupPaths* paths,
...
static int install_info_discover(UnitFileScope scope, InstallContext* c,
                                 const LookupPaths* paths, const char* name,
                                 SearchFlags flags, UnitFileInstallInfo** ret,
                                 UnitFileChange** changes, size_t* n_changes)
{
...
static int install_info_discover_and_check(
    UnitFileScope scope, InstallContext* c, const LookupPaths* paths,
    const char* name, SearchFlags flags, UnitFileInstallInfo** ret,
    UnitFileChange** changes, size_t* n_changes)
{

    int r;

    r = install_info_discover(scope, c, paths, name, flags, ret, changes,
                              n_changes);
    if (r < 0)
        return r;

    return install_info_may_process(ret ? *ret : NULL, paths, changes,
                                    n_changes);
}

static int install_info_symlink_alias(UnitFileInstallInfo* i,
                                      const LookupPaths* paths,
                                      const char* config_path, bool force,
                                      ...
                                      size_t* n_changes)
{
    ...
    STRV_FOREACH(s, i->aliases)
    {
        ...
        q = create_symlink(paths, i->path, alias_path, force, changes,
                           n_changes);
        if (r == 0)
            r = q;
    }
...
static int install_info_symlink_wants(UnitFileInstallInfo* i,
                                      const LookupPaths* paths,
                                      const char* config_path, char** list,
                                      const char* suffix,
                                      UnitFileChange** changes,
                                      size_t* n_changes)
{
    ...
    if (unit_name_is_valid(i->name, UNIT_NAME_TEMPLATE))
    {
        ...
        instance.name = buf;
        r = unit_file_search(NULL, &instance, paths,
...
static int install_info_apply(UnitFileInstallInfo* i, const LookupPaths* paths,
                              const char* config_path, bool force,
                              UnitFileChange** changes, size_t* n_changes)
{
    ...
    r = install_info_symlink_alias(i, paths, config_path, force, changes,
                                   n_changes);

    q = install_info_symlink_wants(i, paths, config_path, i->wanted_by,
                                   ".wants/", changes, n_changes);
    if (r == 0)
        r = q;

    q = install_info_symlink_wants(i, paths, config_path, i->required_by,
                                   ".requires/", changes, n_changes);
    if (r == 0)
        r = q;

    q = install_info_symlink_link(i, paths, config_path, force, changes,
                                  n_changes);
    /* Do not count links to the unit file towards the "carries_install_info"
     * count */
    if (r == 0 && q < 0)
        r = q;
...
static int install_context_apply(UnitFileScope scope, InstallContext* c,
                                 const LookupPaths* paths,
                                 const char* config_path, bool force,
                                 SearchFlags flags, UnitFileChange** changes,
                                 size_t* n_changes)
{
    ...
    while ((i = ordered_hashmap_first(c->will_process)))
    {
        ...
        q = ordered_hashmap_move_one(c->have_processed, c->will_process,
                                     i->name);
        if (q < 0)
            return q;
        ...
        if (i->type == UNIT_FILE_TYPE_MASKED)
        {
            unit_file_changes_add(changes, n_changes, UNIT_FILE_IS_MASKED,
                                  i->path, NULL);
            if (r >= 0)
        ...
        q = install_info_apply(i, paths, config_path, force, changes,
                               n_changes);
        if (r >= 0)
        {
            if (q < 0)
...
static int install_context_mark_for_removal(
    UnitFileScope scope, InstallContext* c, const LookupPaths* paths,
    Set** remove_symlinks_to, const char* config_path, UnitFileChange** changes,
    size_t* n_changes)
{
    ...
    while ((i = ordered_hashmap_first(c->will_process)))
    {
        ...
        r = ordered_hashmap_move_one(c->have_processed, c->will_process,
                                     i->name);
        if (r < 0)
            return r;
        ...
        {
            log_debug_errno(
                r, "Name %s leads to a dangling symlink, removing name.",
                i->name);
            unit_file_changes_add(changes, n_changes, UNIT_FILE_IS_DANGLING,
                                  i->path ?: i->name, NULL);
        }
        else if (r == -ENOENT)
        {

            if (i->auxiliary) /* some unit specified in Also= or similar is
                                 missing */
                log_debug_errno(
                    r, "Auxiliary unit of %s not found, removing name.",
                    i->name);
            else
            {
                log_debug_errno(r, "Unit %s not found, removing name.",
                                i->name);
                unit_file_changes_add(changes, n_changes, r, i->path ?: i->name,
                                      NULL);
            }
        }
        else if (r < 0)
        {
            log_debug_errno(r, "Failed to find unit %s, removing name: %m",
                            i->name);
            unit_file_changes_add(changes, n_changes, r, i->path ?: i->name,
                                  NULL);
        }
        ...
        {
            log_debug("Unit %s has type %s, ignoring.", i->name,
...
int unit_file_mask(UnitFileScope scope, UnitFileFlags flags,
                   const char* root_dir, char** files, UnitFileChange** changes,
                   size_t* n_changes)
{
    ...
    config_path = (flags & UNIT_FILE_RUNTIME) ? paths.runtime_config
                                              : paths.persistent_config;
    if (!config_path)
        return -ENXIO;
    ...
    STRV_FOREACH(i, files)
    {
        ...
        q = create_symlink(&paths, "/dev/null", path,
...
int unit_file_unmask(UnitFileScope scope, UnitFileFlags flags,
                     ...
                     UnitFileChange** changes, size_t* n_changes)
{
    ...
    q = remove_marked_symlinks(remove_symlinks_to, config_path, &paths, dry_run,
                               changes, n_changes);
    if (r >= 0)
        r = q;
...
int unit_file_link(UnitFileScope scope, UnitFileFlags flags,
                   const char* root_dir, char** files, UnitFileChange** changes,
                   size_t* n_changes)
{
    ...
    STRV_FOREACH(i, todo)
    {
        ...
        q = create_symlink(&paths, *i, new_path, !!(flags & UNIT_FILE_FORCE),
                           changes, n_changes);
        if (q < 0 && r >= 0)
            r = q;
    }
...
static int path_shall_revert(const LookupPaths* paths, const char* path)
{
    ...
    /* Checks whether the path is one where the drop-in directories shall be
     * removed. */
...
int unit_file_revert(UnitFileScope scope, const char* root_dir, char** files,
                     UnitFileChange** changes, size_t* n_changes)
{
     ...
     *
     * a) we remove all drop-in snippets added by the user ("config"), add to
     * transient units ("transient"), and added via "systemctl set-property"
     * ("control"), but not if the drop-in is generated ("generated").
     *
     * c) if there's a vendor unit file (i.e. one in /usr) we remove any
     * configured overriding unit files (i.e. in "config", but not in
     * "transient" or "control" or even "generated").
     *
     * We remove all that in both the runtime and the persistent directories, if
     * that applies.
     */
    ...
    STRV_FOREACH(i, todo)
    {
        ...
        STRV_FOREACH(j, fs)
        {
            ...
            unit_file_changes_add(changes, n_changes, UNIT_FILE_UNLINK, t,
                                  NULL);
        }
    ...
    q = remove_marked_symlinks(remove_symlinks_to, paths.runtime_config, &paths,
                               false, changes, n_changes);
    if (r >= 0)
        r = q;

    q = remove_marked_symlinks(remove_symlinks_to, paths.persistent_config,
                               &paths, false, changes, n_changes);
    if (r >= 0)
        r = q;
...
int unit_file_add_dependency(UnitFileScope scope, UnitFileFlags flags,
                             const char* root_dir, char** files,
                             ...
                             UnitFileChange** changes, size_t* n_changes)
{
    ...
    r = install_info_discover_and_check(scope, &c, &paths, target,
                                        SEARCH_FOLLOW_CONFIG_SYMLINKS,
    ...
    STRV_FOREACH(f, files)
    {
        ...
        r = install_info_discover_and_check(scope, &c, &paths, *f,
                                            SEARCH_FOLLOW_CONFIG_SYMLINKS, &i,
                                            changes, n_changes);
        if (r < 0)
            return r;
    ...
    return install_context_apply(
        scope, &c, &paths, config_path, !!(flags & UNIT_FILE_FORCE),
        SEARCH_FOLLOW_CONFIG_SYMLINKS, changes, n_changes);
}

int unit_file_enable(UnitFileScope scope, UnitFileFlags flags,
                     const char* root_dir, char** files,
                     UnitFileChange** changes, size_t* n_changes)
{
    ...
    STRV_FOREACH(f, files)
    {
        r = install_info_discover_and_check(
            scope, &c, &paths, *f, SEARCH_LOAD | SEARCH_FOLLOW_CONFIG_SYMLINKS,
            &i, changes, n_changes);
        if (r < 0)
            return r;
    ...
    return install_context_apply(scope, &c, &paths, config_path,
                                 !!(flags & UNIT_FILE_FORCE), SEARCH_LOAD,
                                 changes, n_changes);
}

int unit_file_disable(UnitFileScope scope, UnitFileFlags flags,
                      const char* root_dir, char** files,
                      UnitFileChange** changes, size_t* n_changes)
{
    ...
    r = install_context_mark_for_removal(scope, &c, &paths, &remove_symlinks_to,
                                         config_path, changes, n_changes);
    if (r < 0)
        return r;

    return remove_marked_symlinks(remove_symlinks_to, config_path, &paths,
                                  !!(flags & UNIT_FILE_DRY_RUN), changes,
                                  n_changes);
}

int unit_file_reenable(UnitFileScope scope, UnitFileFlags flags,
                       const char* root_dir, char** files,
                       UnitFileChange** changes, size_t* n_changes)
{
...
int unit_file_set_default(UnitFileScope scope, UnitFileFlags flags,
                          ...
                          UnitFileChange** changes, size_t* n_changes)
{
    ...
    if (unit_name_to_type(name) !=
        UNIT_TARGET) /* this also validates the name */
        return -EINVAL;
    ...
    r = install_info_discover_and_check(scope, &c, &paths, name, 0, &i, changes,
                                        n_changes);
    if (r < 0)
        return r;

    new_path = strjoina(paths.persistent_config, "/" SPECIAL_DEFAULT_TARGET);
    return create_symlink(&paths, i->path, new_path,
                          !!(flags & UNIT_FILE_FORCE), changes, n_changes);
}
                          ...
                          char** name)
{
    ...
    r = install_info_discover(scope, &c, &paths, SPECIAL_DEFAULT_TARGET,
                              SEARCH_FOLLOW_CONFIG_SYMLINKS, &i, NULL, NULL);
    if (r < 0)
        return r;
...
int unit_file_lookup_state(UnitFileScope scope, const LookupPaths* paths,
                           const char* name, UnitFileState* ret)
{
    ...
    r = install_info_discover(scope, &c, paths, name,
                              SEARCH_LOAD | SEARCH_FOLLOW_CONFIG_SYMLINKS, &i,
                              NULL, NULL);
    if (r < 0)
...
int unit_file_exists(UnitFileScope scope, const LookupPaths* paths,
                     const char* name)
{
    _cleanup_(install_context_done) InstallContext c = {};
    int r;
...
static int presets_find_config(UnitFileScope scope, const char* root_dir,
                               char*** files)
{
...
static int read_presets(UnitFileScope scope, const char* root_dir,
                        Presets* presets)
{
    ...
    STRV_FOREACH(p, files)
    {
        ...
        for (;;)
        {
            ...
            if (parameter)
            {
                ...
                /* Unit_name will remain the same as parameter when no instances
                 * are specified */
                r = split_pattern_into_name_and_instances(parameter, &unit_name,
                                                          &instances);
                if (r < 0)
                {
                    log_syntax(NULL, LOG_WARNING, *p, n, r,
            ...
            log_syntax(NULL, LOG_WARNING, *p, n, 0,
                       "Couldn't parse line '%s'. Ignoring.", line);
        }
    }
...
static int pattern_match_multiple_instances(const PresetRule rule,
                                            const char* unit_name, char*** ret)
{
                         ...
                         char*** instance_name_list)
{
    ...
    for (i = 0; i < presets.n_rules; i++)
        if (pattern_match_multiple_instances(presets.rules[i], name,
    ...
    {
        case PRESET_UNKNOWN:
            log_debug("Preset files don't specify rule for %s. Enabling.",
                      name);
            return 1;
        case PRESET_ENABLE:
...
int unit_file_query_preset(UnitFileScope scope, const char* root_dir,
                           const char* name)
{
...
static int execute_preset(UnitFileScope scope, InstallContext* plus,
                          InstallContext* minus, const LookupPaths* paths,
                          const char* config_path, char** files,
                          UnitFilePresetMode mode, bool force,
                          UnitFileChange** changes, size_t* n_changes)
{
    ...
    if (mode != UNIT_FILE_PRESET_ENABLE_ONLY)
    {
        ...
        r = install_context_mark_for_removal(scope, minus, paths,
                                             &remove_symlinks_to, config_path,
                                             changes, n_changes);
        if (r < 0)
            return r;

        r = remove_marked_symlinks(remove_symlinks_to, config_path, paths,
                                   false, changes, n_changes);
    }
    else
        r = 0;
    ...
    if (mode != UNIT_FILE_PRESET_DISABLE_ONLY)
    {
        ...
        q = install_context_apply(scope, plus, paths, config_path, force,
...
static int preset_prepare_one(UnitFileScope scope, InstallContext* plus,
                              InstallContext* minus, LookupPaths* paths,
                              const char* name, Presets presets,
                              UnitFileChange** changes, size_t* n_changes)
{
    ...
    r = install_info_discover(scope, &tmp, paths, name,
                              SEARCH_FOLLOW_CONFIG_SYMLINKS, &i, changes,
                              n_changes);
    if (r < 0)
        return r;
    if (!streq(name, i->name))
    ...
    if (r > 0)
    {
        if (instance_name_list)
        {
            ...
            STRV_FOREACH(s, instance_name_list)
            {
                r = install_info_discover_and_check(
                    scope, plus, paths, *s,
                    SEARCH_LOAD | SEARCH_FOLLOW_CONFIG_SYMLINKS, &i, changes,
                    n_changes);
                if (r < 0)
                    return r;
            }
...
int unit_file_preset(UnitFileScope scope, UnitFileFlags flags,
                     const char* root_dir, char** files,
                     UnitFilePresetMode mode, UnitFileChange** changes,
                     size_t* n_changes)
{
    ...
    STRV_FOREACH(i, files)
    {
        r = preset_prepare_one(scope, &plus, &minus, &paths, *i, presets,
                               changes, n_changes);
        if (r < 0)
            return r;
    }

    return execute_preset(scope, &plus, &minus, &paths, config_path, files,
                          mode, !!(flags & UNIT_FILE_FORCE), changes,
                          n_changes);
}

int unit_file_preset_all(UnitFileScope scope, UnitFileFlags flags,
                         const char* root_dir, UnitFilePresetMode mode,
                         UnitFileChange** changes, size_t* n_changes)
{
    ...
    STRV_FOREACH(i, paths.search_path)
    {
        ...
        FOREACH_DIRENT(de, d, return -errno)
        {
            ...
            /* we don't pass changes[] in, because we want to handle errors on
             * our own */
            r = preset_prepare_one(scope, &plus, &minus, &paths, de->d_name,
                                   presets, NULL, 0);
            if (r == -ERFKILL)
                ...
                r = unit_file_changes_add(changes, n_changes,
                                          UNIT_FILE_IS_DANGLING, de->d_name,
                                          NULL);
            else if (r == -EADDRNOTAVAIL) /* Ignore generated/transient units
    ...
    return execute_preset(scope, &plus, &minus, &paths, config_path, NULL, mode,
                          !!(flags & UNIT_FILE_FORCE), changes, n_changes);
}
...
static const char* const
    unit_file_change_type_table[_UNIT_FILE_CHANGE_TYPE_MAX] = {
