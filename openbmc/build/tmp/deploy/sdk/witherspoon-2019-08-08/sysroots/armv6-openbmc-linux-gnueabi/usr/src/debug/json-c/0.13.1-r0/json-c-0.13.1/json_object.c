...
const char* json_object_to_json_string_length(struct json_object* jso,
...
json_bool json_object_object_get_ex(const struct json_object* jso,
                                           ...
                                           int global_or_thread)
{
    if (global_or_thread == JSON_C_OPTION_GLOBAL)
    {
        ...
        global_serialization_float_format =
            double_format ? strdup(double_format) : NULL;
    }
    else if (global_or_thread == JSON_C_OPTION_THREAD)
    {
        ...
        _json_c_set_last_err(
            "json_c_set_option: not compiled with __thread support\n");
        return -1;
#endif
    }
    else
    {
        _json_c_set_last_err(
            "json_c_set_option: invalid global_or_thread value: %d\n",
            global_or_thread);
        return -1;
    }
    return 0;
                                      ...
                                      struct printbuf* pb, int level, int flags)
{
    return json_object_double_to_json_string_format(
        jso, pb, level, flags, (const char*)jso->_userdata);
}
...
struct json_object* json_object_new_double_s(double d, const char* ds)
{
    ...
    json_object_set_serializer(jso, json_object_userdata_to_json_string, new_ds,
...
static int json_object_copy_serializer_data(struct json_object* src,
                                            struct json_object* dst)
{
    ...
    {
        _json_c_set_last_err("json_object_deep_copy: unable to copy unknown "
                             "serializer data: %p\n",
                             dst->_to_json_string);
        return -1;
    }
...
/**
 * The default shallow copy implementation.  Simply creates a new object of the
 * same type but does *not* copy over _userdata nor retain any custom
 * serializer. If custom serializers are in use, json_object_deep_copy() must be
 * passed a shallow copy implementation that is aware of how to copy them.
 *
 * This always returns -1 or 1.  It will never return 2 since it does not copy
 * the serializer.
 */
int json_c_shallow_copy_default(json_object* src, json_object* parent,
                                           ...
                                           json_c_shallow_copy_fn* shallow_copy)
{
    ...
    shallow_copy_rc =
        shallow_copy(src, parent, key_in_parent, index_in_parent, dst);
            ...
            for (ii = 0; ii < src_array_len; ii++)
            {
                ...
                if (!jso1)
                    jso = NULL;
                             ...
                             jso1, src, NULL, ii, &jso, shallow_copy) < 0)
                {
                    json_object_put(jso);
                    return -1;
                }
                          ...
                          json_c_shallow_copy_fn* shallow_copy)
{
    ...
    rc =
        json_object_deep_copy_recursive(src, NULL, NULL, -1, dst, shallow_copy);
    if (rc < 0)
    {
        json_object_put(*dst);
        *dst = NULL;
