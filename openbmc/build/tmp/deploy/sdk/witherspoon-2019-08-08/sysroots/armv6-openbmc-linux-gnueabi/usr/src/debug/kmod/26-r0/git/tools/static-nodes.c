                       ...
                       unsigned int maj, unsigned int min)
{
    ...
    ret = fprintf(out,
                  ...
                  "\t\tMajor: %u\n"
                  "\t\tMinor: %u\n",
                  modname, devname, (type == 'c') ? "character" : "block", maj,
                  min);
    if (ret >= 0)
        return EXIT_SUCCESS;
    else
...
static int do_static_nodes(int argc, char* argv[])
{
    ...
    if (in == NULL)
    {
        ...
        {
            fprintf(stderr,
                    "Warning: /lib/modules/%s/modules.devname not found - "
                    "ignoring\n",
                    kernel.release);
            ret = EXIT_SUCCESS;
        }
...
const struct kmod_cmd kmod_cmd_static_nodes = {
    .name = "static-nodes",
    .cmd = do_static_nodes,
    .help = "outputs the static-node information installed with the currently "
            "running kernel",
};
