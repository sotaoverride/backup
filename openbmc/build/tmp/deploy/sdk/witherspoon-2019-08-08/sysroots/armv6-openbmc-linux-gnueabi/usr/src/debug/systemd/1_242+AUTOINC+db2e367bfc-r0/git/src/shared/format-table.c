...
/*
 ...
 - TableCell is a 'fake' structure, it's just used as data type to pass
 references to specific cell positions in the table. It can be easily converted
 to an index number and back.

 - TableData is where the actual data is stored: it encapsulates the data and
 formatting for a specific cell. It's 'pseudo-immutable' and ref-counted. When a
 cell's data's formatting is to be changed, we duplicate the object if the
   ref-counting is larger than 1. Note that TableData and its ref-counting is
 mostly not visible to the outside. The outside only sees Table and TableCell.

 - The Table object stores a simple one-dimensional array of references to
 TableData objects, one row after the previous one.

 - There's no special concept of a "row" or "column" in the table, and no
 special concept of the "header" row. It's all derived from the cell index: we
 know how many cells are to be stored in a row, and can determine the rest from
   that. The first row is always the header row. If header display is turned off
 we simply skip outputting the first row. Also, when sorting rows we always
 leave the first row where it is, as the header shouldn't move.

 - Note because there's no row and no column object some properties that might
 be appropriate as row/column properties are exposed as cell properties instead.
 For example, the "weight" of a column (which is used to determine where to
   add/remove space preferable when expanding/compressing tables horizontally)
 is actually made the "weight" of a cell. Given that we usually need it
 per-column though we will calculate the average across every cell of the column
   instead.

 - To make things easy, when cells are added without any explicit configured
 formatting, then we'll copy the formatting from the same cell in the previous
 cell. This is particularly useful for the "weight" of the cell (see above), as
   this means setting the weight of the cells of the header row will nicely
 propagate to all cells in the other rows.
*/
...
typedef struct TableData
{
    ...
    unsigned weight; /* the horizontal weight for this column, in case the table
                        is expanded/compressed */
    unsigned ellipsize_percent; /* 0 … 100, where to place the ellipsis when
                                   compression is needed */
    unsigned align_percent; /* 0 … 100, where to pad with spaces when expanding
                               is needed. 0: left-aligned, 100: right-aligned */

    bool uppercase; /* Uppercase string on display */

    const char* color; /* ANSI color string to use for this cell. When written
                          to terminal should not move cursor. Will automatically
                          be reset after the cell */
    char* url;         /* A URL to use for a clickable hyperlink */
    char* formatted; /* A cached textual representation of the cell data, before
                        ellipsation/alignment */
    ...
    union
    {
        ...
        uint32_t uint32;
        uint64_t uint64;
        int percent; /* we use 'int' as datatype for percent values in order to
                        match the result of parse_percent() */
...
struct Table
{
    ...
    size_t*
        display_map; /* List of columns to show (by their index). It's fine if
                        columns are listed multiple times or not at all */
    size_t n_display_map;

    size_t*
        sort_map; /* The columns to order rows by, in order of preference. */
    size_t n_sort_map;
...
static bool table_data_matches(TableData* d, TableDataType type,
                               const void* data, size_t minimum_width,
                               size_t maximum_width, unsigned weight,
...
static TableData* table_data_new(TableDataType type, const void* data,
                                 size_t minimum_width, size_t maximum_width,
                                 unsigned weight, unsigned align_percent,
                                 unsigned ellipsize_percent)
{
...
int table_add_cell_full(Table* t, TableCell** ret_cell, TableDataType type,
                        const void* data, size_t minimum_width,
                        size_t maximum_width, unsigned weight,
                        unsigned align_percent, unsigned ellipsize_percent)
{
    ...
    /* Small optimization: Pretty often adjacent cells in two subsequent lines
     * have the same data and formatting. Let's see if we can reuse the cell
     * data and ref it once more. */

    if (p && table_data_matches(p, type, data, minimum_width, maximum_width,
                                weight, align_percent, ellipsize_percent))
        d = table_data_ref(p);
    else
    {
        d = table_data_new(type, data, minimum_width, maximum_width, weight,
                           align_percent, ellipsize_percent);
        if (!d)
            return -ENOMEM;
    }

    if (!GREEDY_REALLOC(t->data, t->n_allocated,
                        MAX(t->n_cells + 1, t->n_columns)))
        return -ENOMEM;
...
int table_dup_cell(Table* t, TableCell* cell)
{
    ...
    /* Add the data of the specified cell a second time as a new cell to the
     * end. */
...
static int table_dedup_cell(Table* t, TableCell* cell)
{
    ...
    /* Helper call that ensures the specified cell's data object has a ref count
     * of 1, which we can use before changing a cell's formatting without
     * effecting every other cell's formatting that shares the same data */
    ...
    nd =
        table_data_new(od->type, od->data, od->minimum_width, od->maximum_width,
                       od->weight, od->align_percent, od->ellipsize_percent);
    if (!nd)
        return -ENOMEM;
...
int table_update(Table* t, TableCell* cell, TableDataType type,
                 const void* data)
{
    _cleanup_free_ char* curl = NULL;
    TableData *nd, *od;
    ...
    nd = table_data_new(type, data, od->minimum_width, od->maximum_width,
                        od->weight, od->align_percent, od->ellipsize_percent);
    if (!nd)
        return -ENOMEM;
...
int table_set_display(Table* t, size_t first_column, ...)
{
    ...
    for (;;)
    {
        ...
        if (!GREEDY_REALLOC(t->display_map, allocated,
                            MAX(t->n_columns, t->n_display_map + 1)))
        {
            va_end(ap);
            return -ENOMEM;
        }
...
int table_set_sort(Table* t, size_t first_column, ...)
{
    ...
    for (;;)
    {
        ...
        if (!GREEDY_REALLOC(t->sort_map, allocated,
                            MAX(t->n_columns, t->n_sort_map + 1)))
        {
            va_end(ap);
            return -ENOMEM;
        }
...
static int cell_data_compare(TableData* a, size_t index_a, TableData* b,
                             size_t index_b)
{
    assert(a);
    assert(b);
    ...
    if (a->type == b->type)
    {
        ...
        /* We only define ordering for cells of the same data type. If cells
         * with different data types are
...
static char* align_string_mem(const char* str, const char* url,
                              size_t new_length, unsigned percent)
{
    ...
    /* As with ellipsize_mem(), 'old_length' is a byte size while 'new_length'
     * is a width in character cells */
...
{
    size_t n_rows, *minimum_width, *maximum_width, display_columns,
        *requested_width, i, j, table_minimum_width, table_maximum_width,
    ...
    if (t->sort_map)
    {
        /* If sorting is requested, let's calculate an index table we use to
         * lookup the actual index to display with. */
    ...
    for (i = t->header ? 0 : 1; i < n_rows; i++)
    {
        ...
        /* Note that we don't care about ordering at this time, as we just want
         * to determine column sizes, hence we don't care for sorted[] during
         * the first pass. */
        ...
        for (j = 0; j < display_columns; j++)
        {
            ...
            /* Determine the biggest width that any cell in this column would
             * like to have */
    ...
    table_requested_width = table_minimum_width = table_maximum_width =
        display_columns - 1;

    /* Calculate the total weight for all columns, plus the minimum, maximum and
     * requested width for the table. */
    weight_sum = 0;
    for (j = 0; j < display_columns; j++)
    {
    ...
    if (t->width == (size_t)-1)
        table_effective_width = pager_have()
                                    ? table_requested_width
                                    : MIN(table_requested_width, columns());
    else
        table_effective_width = t->width;

    if (table_maximum_width != (size_t)-1 &&
    ...
    if (table_effective_width >= table_requested_width)
    {
        ...
        /* We have extra room, let's distribute it among columns according to
         * their weights. We first provide each column with what it asked for
         * and the distribute the rest.  */
        ...
        for (j = 0; j < display_columns; j++)
        {
            ...
            if (weight_sum == 0)
                ...
                width[j] = requested_width[j] +
                           (extra * column_weight[j]) / weight_sum;
    ...
    {
        /* We need to compress the table, columns can't get what they asked for.
         * We first provide each column with the minimum they need, and then
         * distribute anything left. */
        bool finalize = false;
        size_t extra;
        ...
        for (;;)
        {
            ...
            for (j = 0; j < display_columns; j++)
            {
                ...
                /* Did this column already get something assigned? If so, let's
                 * skip to the next */
                if (width[j] != (size_t)-1)
                    continue;
                ...
                if (weight_sum == 0)
                    w = minimum_width[j] +
                        extra /
                            (display_columns - j); /* avoid division by zero */
                else
                    w = minimum_width[j] +
                        (extra * column_weight[j]) / weight_sum;
                ...
                {
                    /* Never give more than requested. If we hit a column like
                     * this, there's more space to allocate to other columns
                     * which means we need to restart the iteration. However, if
                     * we hit a column like this, let's assign it the space it
                     * wanted for good early.*/
    ...
    for (i = t->header ? 0 : 1; i < n_rows; i++)
    {
        ...
        for (j = 0; j < display_columns; j++)
        {
            ...
            {
                /* Field is shorter than allocated space. Let's align with
                 * spaces */

                buffer =
                    align_string_mem(field, d->url, width[j], d->align_percent);
                if (!buffer)
                    return -ENOMEM;
            ...
            if (row == t->data) /* underline header line fully, including the
                                   column separator */
...
int table_to_json(Table* t, JsonVariant** ret)
{
    ...
    for (i = 1; i < n_rows; i++)
    {
        ...
        r = json_variant_new_object(rows + i - 1, elements,
                                    display_columns * 2);
        if (r < 0)
            goto finish;
    }
