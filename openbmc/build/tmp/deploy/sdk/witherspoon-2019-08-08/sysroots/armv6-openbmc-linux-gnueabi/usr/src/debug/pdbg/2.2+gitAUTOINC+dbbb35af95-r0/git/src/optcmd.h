...
#define OPTCMD_CAST_ARG(cnt_, parser_)                                         \
    *(OPTCMD_TYPEOF_PARSER parser_*)args[cnt_]

/* Returns a positional argument struct */
#define _OPTCMD_ARG(parser_, default_)                                         \
    {                                                                          \
        .parser = OPTCMD_PARSER_CAST(parser_), .def = default_                 \
    }
...
#define _OPTCMD_CAST_FLAGS(flag_, field_, parser_, default_)                   \
    BUILD_ASSERT(                                                              \
        !check_types_match(flag.field_, OPTCMD_TYPEOF_PARSER(parser_)));       \
 ...
 *  @pos - list of positional arguments in the form ((parser, <default value>,
 * "help text"), ...)
 *  @flag - name of flag struct to pass @cmd_func as the last parameter
 *  @flags - list of flags in the form (("--flag-name", <struct field name>,
 * parser, <default value>, "help text"), ...)
 */
#define OPTCMD_DEFINE_CMD_WITH_FLAGS(cmd_name_, cmd_func_, pos_, flag_,        \
                                     flags_)                                   \
    ...
    {                                                                          \
        ...
        return cmd_func_(                                                      \
            CPPMAGIC_MAP_CNT(OPTCMD_CAST_ARG, CPPMAGIC_EVAL pos_), flag);      \
    }                                                                          \
                                                                               \
...
#define OPTCMD_DEFINE_CMD_WITH_ARGS(cmd_name_, cmd_func_, pos_)                \
    ...
    {                                                                          \
        return cmd_func_(                                                      \
            CPPMAGIC_MAP_CNT(OPTCMD_CAST_ARG, CPPMAGIC_EVAL pos_));            \
    }                                                                          \
                                                                               \
    struct optcmd_cmd optcmd_##cmd_name_ = {                                   \
        .cmd = #cmd_name_,                                                     \
