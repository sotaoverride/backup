...
static struct fdisk_parttype gpt_parttypes[] = {
    ...
    DEF_GUID("024DEE41-33E7-11D3-9D69-0008C781F39F",
             N_("MBR partition scheme")),
    ...
    DEF_GUID("BFBFAFE7-A34F-448A-9A5B-6213EB736C22",
             N_("Lenovo boot partition")),
    ...
    DEF_GUID("EBD0A0A2-B9E5-4433-87C0-68B6B72699C7",
             N_("Microsoft basic data")),
    DEF_GUID("5808C8AA-7E8F-42E0-85D2-E1E90434CFB3",
             N_("Microsoft LDM metadata")),
    DEF_GUID("AF9B60A0-1431-4F62-BC68-3311714A69AD", N_("Microsoft LDM data")),
    DEF_GUID("DE94BBA4-06D1-4D40-A16A-BFD50179D6AC",
             N_("Windows recovery environment")),
    DEF_GUID("37AFFC90-EF7D-4E96-91C3-2D7AE055B174",
             N_("IBM General Parallel Fs")),
    DEF_GUID("E75CAF8F-F680-4CEE-AFA3-B001E56EFC2D",
             N_("Microsoft Storage Spaces")),
    ...
    DEF_GUID("7FFEC5C9-2D00-49B7-8941-3EA10A5586B7", N_("Linux plain
    dm-crypt")), DEF_GUID("CA7D7CCB-63ED-4C53-861C-1742536059CC", N_("Linux
    LUKS")),
    */
    ...
    /* same as Apple ZFS */
    DEF_GUID("6A898CC3-1DD2-11B2-99A6-080020736631",
             N_("Solaris /usr & Apple ZFS")),
    ...
    DEF_GUID("6A9283A5-1DD2-11B2-99A6-080020736631",
             N_("Solaris alternate sector")),
    ...
    DEF_GUID("45B0969E-9B03-4F30-B4C6-5EC00CEFF106",
             N_("Ceph Encrypted Journal")),
    DEF_GUID("4FBD7E29-9D25-41B8-AFD0-062C0CEFF05D", N_("Ceph OSD")),
    DEF_GUID("4FBD7E29-9D25-41B8-AFD0-5EC00CEFF05D", N_("Ceph crypt OSD")),
    DEF_GUID("89C57F98-2FE5-4DC0-89C1-F3AD0CEFF2BE",
             N_("Ceph disk in creation")),
    DEF_GUID("89C57F98-2FE5-4DC0-89C1-5EC00CEFF2BE",
             N_("Ceph crypt disk in creation")),
    ...
    DEF_GUID("5B193300-FC78-40CD-8002-E86C45580B47",
             N_("HiFive Unleashed FSBL")),
    DEF_GUID("2E54B353-1271-4842-806F-E436D6AF6985",
             N_("HiFive Unleashed BBL")),
};
...
static void gpt_debug_uuid(const char* mesg, struct gpt_guid* guid)
{
    ...
    fprintf(stderr,
            ...
            "02x\n",
            mesg, uuid[0], uuid[1], uuid[2], uuid[3], uuid[4], uuid[5], uuid[6],
...
{
    if (uuid_parse(in, (unsigned char*)guid))
    { /* BE */
...
static struct fdisk_parttype* gpt_partition_parttype(struct fdisk_context* cxt,
...
{
    return memcmp(&e->type, &GPT_UNUSED_ENTRY_GUID, sizeof(struct gpt_guid)) !=
           0;
}
...
static inline unsigned char* gpt_get_entry_ptr(struct fdisk_gpt_label* gpt,
                                               size_t i)
{
    return gpt->ents + le32_to_cpu(gpt->pheader->sizeof_partition_entry) * i;
}

static inline struct gpt_entry* gpt_get_entry(struct fdisk_gpt_label* gpt,
                                              size_t i)
{
    return (struct gpt_entry*)gpt_get_entry_ptr(gpt, i);
}

static inline struct gpt_entry* gpt_zeroize_entry(struct fdisk_gpt_label* gpt,
                                                  size_t i)
{
    return (struct gpt_entry*)memset(
        gpt_get_entry_ptr(gpt, i), 0,
...
{
    return gpt_calculate_sizeof_ents(hdr, le32_to_cpu(hdr->npartition_entries),
                                     sz);
}
...
static void gpt_fix_alternative_lba(struct fdisk_context* cxt,
                                    struct fdisk_gpt_label* gpt)
{
    ...
    esz =
        (uint64_t)le32_to_cpu(p->npartition_entries) * sizeof(struct gpt_entry);
    ...
    DBG(LABEL, ul_debug("Alternative-LBA updated to: %" PRIu64,
                        le64_to_cpu(p->alternative_lba)));
}
                                    ...
                                    struct gpt_header* header, uint64_t lba)
{
    ...
    { /* backup */
        ...
        header->partition_entry_lba =
            cpu_to_le64(cxt->total_sectors - 1ULL - esects);
    }
}
                                     ...
                                     struct gpt_header* header2)
{
    ...
    memcpy(&header->disk_guid, &header2->disk_guid, sizeof(header2->disk_guid));
...
static int get_script_u64(struct fdisk_context* cxt, uint64_t* num,
                          const char* name)
{
    const char* str;
    int pwr = 0, rc = 0;
...
static int count_first_last_lba(struct fdisk_context* cxt, uint64_t* first,
                                uint64_t* last)
{
    int rc = 0;
    uint64_t flba, llba;
    ...
    *first = *last = 0;
    ...
    if (cxt->script)
    {
        ...
        DBG(LABEL, ul_debug("FirstLBA: script=%" PRIu64 ", uefi=%" PRIu64
                            ", topology=%ju.",
        ...
        DBG(LABEL, ul_debug("LastLBA: script=%" PRIu64 ", uefi=%" PRIu64
                            ", topology=%ju.",
    ...
    /* default by topology */
    if (!*first)
        *first = flba < cxt->first_lba && cxt->first_lba < *last
                     ? cxt->first_lba
                     : flba;
    return 0;
}
                            ...
                            struct gpt_header* header, uint64_t lba)
{
     ...
     */
    header->size =
        cpu_to_le32(sizeof(struct gpt_header) - sizeof(header->reserved2));
...
static int valid_pmbr(struct fdisk_context* cxt)
{
    ...
    {
        uint64_t sz_lba =
            (uint64_t)le32_to_cpu(pmbr->partition_record[part].size_in_lba);
        if (sz_lba != cxt->total_sectors - 1ULL && sz_lba != 0xFFFFFFFFULL)
        {

            fdisk_warnx(cxt,
                        _("GPT PMBR size mismatch (%" PRIu64 " != %" PRIu64 ") "
                          "will be corrected by write."),
                        sz_lba, cxt->total_sectors - 1ULL);

            /* Note that gpt_write_pmbr() overwrites PMBR, but we want to keep
             * it valid already in memory too to disable warnings when
             * valid_pmbr() called next time */
            pmbr->partition_record[part].size_in_lba = cpu_to_le32(
...
static uint64_t last_lba(struct fdisk_context* cxt)
{
    ...
    if (S_ISBLK(s.st_mode))
        ...
        sectors = ((uint64_t)s.st_size / (uint64_t)cxt->sector_size) - 1ULL;
    else
...
static ssize_t read_lba(struct fdisk_context* cxt, uint64_t lba, void* buffer,
                        const size_t bytes)
{
                                       ...
                                       struct gpt_header* header)
{
    ...
    offset = (off_t)le64_to_cpu(header->partition_entry_lba) * cxt->sector_size;
...
static inline uint32_t gpt_entryarr_count_crc32(struct gpt_header* header,
                                                unsigned char* ents)
{
    size_t arysz = 0;
...
static int gpt_check_lba_sanity(struct fdisk_context* cxt,
                                struct gpt_header* header)
{
    int ret = 0;
...
static struct gpt_header* gpt_read_header(struct fdisk_context* cxt,
                                          uint64_t lba, unsigned char** _ents)
{
    ...
    *size = 0;
    ...
    switch (n)
    {
        ...
        case 1:
            *name = _("GPT Header");
            *offset =
                (uint64_t)GPT_PRIMARY_PARTITION_TABLE_LBA * cxt->sector_size;
...
static int gpt_get_disklabel_item(struct fdisk_context* cxt,
                                  struct fdisk_labelitem* item)
{
    ...
    switch (item->id)
    {
        ...
        case GPT_LABELITEM_ALTLBA:
            /* TRANSLATORS: The LBA (Logical Block Address) of the backup GPT
             * header. */
 ...
 */
static uint32_t check_too_big_partitions(struct fdisk_gpt_label* gpt,
                                         uint64_t sectors)
{
    size_t i;
...
static uint32_t check_overlap_partitions(struct fdisk_gpt_label* gpt)
{
    ...
    for (i = 0; i < gpt_get_nentries(gpt); i++)
        for (j = 0; j < i; j++)
        {
            ...
            {
                DBG(LABEL,
                    ul_debug("GPT partitions overlap detected [%zu vs. %zu]", i,
                             j));
                return i + 1;
            }
        }
 ...
 */
static uint64_t find_first_available(struct fdisk_gpt_label* gpt,
                                     uint64_t start)
{
    int first_moved = 0;
    uint64_t first;
...
/* Returns last available sector in the free space pointed to by start. From
 * gdisk. */
...
static uint64_t find_last_free_sector(struct fdisk_gpt_label* gpt)
{
    ...
    do
    {
        ...
        for (i = 0; i < gpt_get_nentries(gpt); i++)
        {
            ...
            if (last >= gpt_partition_start(e) && last <= gpt_partition_end(e))
            {
...
static uint64_t get_free_sectors(struct fdisk_context* cxt,
                                 struct fdisk_gpt_label* gpt,
                                 uint32_t* nsegments, uint64_t* largest_segment)
{
    uint32_t num = 0;
...
static int gpt_probe_label(struct fdisk_context* cxt)
{
    ...
    DBG(LABEL,
        ul_debug("found a %s MBR",
                 mbr_type == GPT_MBR_PROTECTIVE ? "protective" : "hybrid"));

    /* primary header */
    gpt->pheader =
        gpt_read_header(cxt, GPT_PRIMARY_PARTITION_TABLE_LBA, &gpt->ents);

    if (gpt->pheader)
        /* primary OK, try backup from alternative LBA */
        gpt->bheader = gpt_read_header(
            cxt, le64_to_cpu(gpt->pheader->alternative_lba), NULL);
    else
...
static int gpt_entry_attrs_to_string(struct gpt_entry* e, char** res)
{
    ...
    *res = calloc(1, sizeof(GPT_ATTRSTR_NOBLOCK) + sizeof(GPT_ATTRSTR_REQ) +
                         sizeof(GPT_ATTRSTR_LEGACY) + sizeof("GUID:") +
                         (GPT_ATTRBIT_GUID_COUNT * 3));
    if (!*res)
        return -errno;
...
static int gpt_entry_attrs_from_string(struct fdisk_context* cxt,
                                       struct gpt_entry* e, const char* str)
{
    const char* p = str;
    uint64_t attrs = 0;
    char* bits;
    ...
    while (p && *p)
    {
        ...
        if (strncmp(p, GPT_ATTRSTR_REQ, sizeof(GPT_ATTRSTR_REQ) - 1) == 0)
        {
            bit = GPT_ATTRBIT_REQ;
                  ...
                  isdigit((unsigned char)*(p + 5))))
        {
            ...
            errno = 0;
            bit = strtol(p, &end, 0);
            if (errno || !end || end == str || bit < GPT_ATTRBIT_GUID_FIRST ||
        ...
        if (*p && *p != ',' && !isblank(*p))
        {
            fdisk_warnx(cxt, _("failed to parse GPT attribute string '%s'"),
                        str);
            return -EINVAL;
        }
                             ...
                             struct fdisk_partition* pa)
{
    ...
    if (pa->uuid)
    {
        ...
        fdisk_info(cxt, _("Partition UUID changed from %s to %s."), old_u,
                   new_u);
    }
    ...
    {
        uint64_t xstart =
            fdisk_partition_has_start(pa) ? pa->start : gpt_partition_start(e);
        uint64_t xsize =
            fdisk_partition_has_size(pa) ? pa->size : gpt_partition_size(e);
...
static int gpt_write_pmbr(struct fdisk_context* cxt)
{
    ...
    /*
     * Set size_in_lba to the size of the disk minus one. If the size of the
     * disk is too large to be represented by a 32bit LBA (2Tb), set it to
     * 0xFFFFFFFF.
     */
...
static int gpt_write_disklabel(struct fdisk_context* cxt)
{
    ...
    if (gpt_write_partitions(cxt, gpt->pheader, gpt->ents) != 0)
        goto err1;
    if (gpt_write_header(cxt, gpt->pheader, GPT_PRIMARY_PARTITION_TABLE_LBA) !=
        0)
        goto err1;
...
static int gpt_verify_disklabel(struct fdisk_context* cxt)
{
    ...
    {
        nerror++;
        fdisk_warnx(cxt, _("Partition %u overlaps with partition %u."), ptnum,
                    ptnum + 1);
    }
    ...
    {
        nerror++;
        fdisk_warnx(cxt, _("Partition %u is too big for the disk."), ptnum);
    }
    ...
    {
        nerror++;
        fdisk_warnx(cxt, _("Partition %u ends before it starts."), ptnum);
    }
...
static int gpt_delete_partition(struct fdisk_context* cxt, size_t partnum)
{
    struct fdisk_gpt_label* gpt;
...
static int gpt_add_partition(struct fdisk_context* cxt,
                             struct fdisk_partition* pa, size_t* partno)
{
    uint64_t user_f, user_l; /* user input ranges for first and last sectors */
    uint64_t disk_f,
        disk_l; /* first and last available sector ranges on device*/
     ...
     * the first partition */
    if ((!pa || !fdisk_partition_has_start(pa)) && gpt_entry_is_used(e) &&
        disk_f < gpt_partition_start(e))
    {
        ...
        do
        {
            ...
            if (x - disk_f >= cxt->grain / cxt->sector_size)
                break;
            DBG(LABEL, ul_debug("first sector %" PRIu64
                                " addresses to small space, continue...",
                                disk_f));
            disk_f = x + 1ULL;
        } while (1);

        if (disk_f == 0)
            disk_f = find_first_available(
                gpt, le64_to_cpu(pheader->first_usable_lba));
    }
    ...
    else if (pa && fdisk_partition_has_start(pa))
    {
        ...
        {
            fdisk_warnx(cxt, _("Sector %ju already used."),
                        (uintmax_t)pa->start);
            return -ERANGE;
        }
        user_f = pa->start;
    ...
    else if (pa && fdisk_partition_has_size(pa))
    {
        ...
        DBG(LABEL, ul_debug("size defined: %ju, end: %" PRIu64
                            " (last possible: %" PRIu64 ")",
                            (uintmax_t)pa->size, user_l, dflt_l));

        if (user_l != dflt_l && !pa->size_explicit && alignment_required(cxt) &&
    ...
    else
    {
        for (;;)
        {
            ...
            fdisk_ask_set_query(
                ask, _("Last sector, +/-sectors or +/-size{K,M,G,T,P}"));
    ...
    /* Be paranoid and check against on-disk setting rather than against
     * libfdisk cxt */
    if (user_l > le64_to_cpu(pheader->last_usable_lba))
    {
        fdisk_warnx(
            cxt, _("The last usable GPT sector is %ju, but %ju is requested."),
    ...
    {
        fdisk_warnx(
            cxt, _("The first usable GPT sector is %ju, but %ju is requested."),
    ...
    DBG(LABEL, ul_debug("GPT new partition: partno=%zu, start=%" PRIu64
                        ", end=%" PRIu64 ", size=%" PRIu64 "",
                        partnum, gpt_partition_start(e), gpt_partition_end(e),
...
static int gpt_create_disklabel(struct fdisk_context* cxt)
{
    ...
    rc = gpt_mknew_header_from_bkp(cxt, gpt->bheader, last_lba(cxt),
                                   gpt->pheader);
    if (rc < 0)
        goto done;
...
static int gpt_set_disklabel_id(struct fdisk_context* cxt)
{
    ...
    gpt = self_label(cxt);
    if (fdisk_ask_string(cxt, _("Enter new disk UUID (in 8-4-4-4-12 format)"),
                         &str))
        return -EINVAL;
                                   ...
                                   uint64_t first_usable, uint64_t last_usable)
{
    ...
    /* First check if there's enough room for the table. last_lba may have
     * wrapped */
...
int fdisk_gpt_set_npartitions(struct fdisk_context* cxt, uint32_t entries)
{
    ...
    {
        fdisk_warnx(
            cxt, _("The number of the partition has to be smaller than %zu."),
    ...
    last_usable =
        cxt->total_sectors - 2ULL - (uint64_t)(new_size / cxt->sector_size);
    ...
    gpt_mknew_header_common(cxt, gpt->bheader,
    ...
    fdisk_info(
        cxt,
        _("Partition table length changed from %" PRIu32 " to %" PRIu64 "."),
        old, entries);
 ...
 * Returns: 0 on success, <0 on error.
 */
int fdisk_gpt_get_partition_attrs(struct fdisk_context* cxt, size_t partnum,
                                  uint64_t* attrs)
{
    struct fdisk_gpt_label* gpt;
 ...
 * Returns: 0 on success, <0 on error.
 */
int fdisk_gpt_set_partition_attrs(struct fdisk_context* cxt, size_t partnum,
                                  uint64_t attrs)
{
    struct fdisk_gpt_label* gpt;
    ...
    fdisk_info(
        cxt, _("The attributes on partition %zu changed to 0x%016" PRIx64 "."),
        partnum + 1, attrs);
...
static int gpt_toggle_partition_flag(struct fdisk_context* cxt, size_t i,
                                     unsigned long flag)
{
    struct fdisk_gpt_label* gpt;
    struct gpt_entry* e;
    ...
    switch (flag)
    {
        ...
        case GPT_FLAG_GUIDSPECIFIC:
            rc = fdisk_ask_number(cxt, 48, 48, 63, _("Enter GUID specific bit"),
                                  &tmp);
            if (rc)
                return rc;
            bit = tmp;
    ...
    if (flag == GPT_FLAG_GUIDSPECIFIC)
        fdisk_info(
            ...
            isset(bits, bit)
                ...
                : _("The GUID specific bit %d on partition %zu is disabled "
                    "now."),
            bit, i + 1);
    else
        fdisk_info(cxt,
...
static int gpt_entry_cmp_start(const void* a, const void* b)
{
    ...
    int au = gpt_entry_is_used(ae), bu = gpt_entry_is_used(be);
...
static int gpt_reorder(struct fdisk_context* cxt)
{
    ...
    for (i = 0, mess = 0; mess == 0 && i + 1 < nparts; i++)
        mess = gpt_entry_cmp_start((const void*)gpt_get_entry(gpt, i),
    ...
    qsort(gpt->ents, nparts, sizeof(struct gpt_entry), gpt_entry_cmp_start);
...
static const struct fdisk_field gpt_fields[] = {
    ...
    {FDISK_FIELD_SIZE, N_("Size"), 5,
     FDISK_FIELDFL_NUMBER | FDISK_FIELDFL_EYECANDY},
