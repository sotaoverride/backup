...
static int manager_timeout(sd_event_source* source, usec_t usec, void* userdata)
{
    ...
    log_info("Timed out waiting for reply from %s (%s).", strna(pretty),
...
static int manager_send_request(Manager* m)
{
     ...
     */
    assert_se(
        clock_gettime(clock_boottime_or_monotonic(), &m->trans_time_mon) >= 0);
    ...
    {
        m->pending = true;
        log_debug("Sent NTP request to %s (%s).", strna(pretty),
                  m->current_server_name->string);
    }
    else
    {
        log_debug_errno(errno, "Sending NTP request to %s (%s) failed: %m",
                        strna(pretty), m->current_server_name->string);
        return manager_connect(m);
    }

    /* re-arm timer with increasing timeout, in case the packets never arrive
     * back */
    ...
    {
        r = sd_event_add_time(m->event, &m->event_timeout,
...
static int manager_arm_timer(Manager* m, usec_t next)
{
    ...
    return sd_event_add_time(
        m->event, &m->event_timer, clock_boottime_or_monotonic(),
        now(clock_boottime_or_monotonic()) + next, 0, manager_timer, m);
}

static int manager_clock_watch(sd_event_source* source, int fd,
...
static int manager_clock_watch_setup(Manager* m)
{
    ...
    if (m->clock_watch_fd < 0)
        return log_error_errno(m->clock_watch_fd,
                               "Failed to create timerfd: %m");

    r = sd_event_add_io(m->event, &m->event_clock_watch, m->clock_watch_fd,
                        EPOLLIN, manager_clock_watch, m);
    if (r < 0)
...
static int manager_adjust_clock(Manager* m, double offset, int leap_sec)
{
    ...
    {
        tmx.modes =
            ADJ_STATUS | ADJ_NANO | ADJ_SETOFFSET | ADJ_MAXERROR | ADJ_ESTERROR;
    ...
    log_debug("  status       : %04i %s\n"
              ...
              tmx.time.tv_sec, tmx.time.tv_usec / NSEC_PER_MSEC, tmx.constant,
              (double)tmx.offset / NSEC_PER_SEC, tmx.freq, tmx.freq / 65536);

    return 0;
}

static bool manager_sample_spike_detection(Manager* m, double offset,
                                           double delay)
{
    ...
    /* calculate new jitter value from the RMS differences relative to the
     * lowest delay sample */
    jitter = m->samples_jitter;
    for (idx_min = idx_cur, i = 0; i < ELEMENTSOF(m->samples); i++)
        if (m->samples[i].delay > 0 &&
            m->samples[i].delay < m->samples[idx_min].delay)
            idx_min = i;
    ...
    /* do not accept anything worse than the maximum possible error of the best
     * sample */
    if (fabs(offset) > m->samples[idx_min].delay)
        return true;

    /* compare the difference between the current offset to the previous offset
     * and jitter */
...
static int manager_receive_response(sd_event_source* source, int fd,
                                    uint32_t revents, void* userdata)
{
    Manager* m = userdata;
    struct ntp_msg ntpmsg;
     ...
     * field) */
    if (be32toh(ntpmsg.origin_time.sec) !=
            m->trans_time.tv_sec + OFFSET_1900_1970 ||
        be32toh(ntpmsg.origin_time.frac) !=
            (unsigned long)m->trans_time.tv_nsec)
    {
    ...
    {
        log_debug("Response NTPv%d. Disconnecting.",
    ...
    {
        log_debug("Unsupported mode %d. Disconnecting.",
    ...
    if (root_distance >
        (double)m->max_root_distance_usec / (double)USEC_PER_SEC)
    {
    ...
    log_debug("NTP response:\n"
              ...
              NTP_FIELD_LEAP(ntpmsg.field), NTP_FIELD_VERSION(ntpmsg.field),
              NTP_FIELD_MODE(ntpmsg.field), ntpmsg.stratum,
              exp2(ntpmsg.precision), ntpmsg.precision, root_distance,
              ntpmsg.stratum == 1 ? ntpmsg.refid : "n/a",
              origin - OFFSET_1900_1970, receive - OFFSET_1900_1970,
              trans - OFFSET_1900_1970, dest - OFFSET_1900_1970, offset, delay,
              m->packet_count, m->samples_jitter, spike ? " spike" : "",
    ...
    log_debug("interval/delta/delay/jitter/drift " USEC_FMT
              "s/%+.3fs/%.3fs/%.3fs/%+" PRIi64 "ppm%s",
              m->poll_interval_usec / USEC_PER_SEC, offset, delay,
              m->samples_jitter, m->drift_freq / 65536,
              spike ? " (ignored)" : "");

    (void)sd_bus_emit_properties_changed(m->bus, "/org/freedesktop/timesync1",
    ...
    if (!m->good)
    {
         ...
         */
        log_info("Synchronized to time server for the first time %s (%s).",
                 strna(pretty), m->current_server_name->string);
        sd_notifyf(
            false,
            "STATUS=Synchronized to time server for the first time %s (%s).",
            strna(pretty), m->current_server_name->string);
    }
...
static int manager_listen_setup(Manager* m)
{
    ...
    return sd_event_add_io(m->event, &m->event_receive, m->server_socket,
                           EPOLLIN, manager_receive_response, m);
}
...
static int manager_begin(Manager* m)
{
    ...
    server_address_pretty(m->current_server_address, &pretty);
    log_debug("Connecting to time server %s (%s).", strna(pretty),
              m->current_server_name->string);
    sd_notifyf(false, "STATUS=Connecting to time server %s (%s).",
               strna(pretty), m->current_server_name->string);
...
void manager_set_server_address(Manager* m, ServerAddress* a)
{
    ...
    if (a)
    {
        ...
        log_debug("Selected address %s of server %s.", strna(pretty),
                  a->name->string);
    }
}

static int manager_resolve_handler(sd_resolve_query* q, int ret,
                                   const struct addrinfo* ai, Manager* m)
{
    int r;
    ...
    if (ret != 0)
    {
        log_debug("Failed to resolve %s: %s", m->current_server_name->string,
                  gai_strerror(ret));
    ...
    for (; ai; ai = ai->ai_next)
    {
        ...
        {
            log_warning("Unsuitable address protocol for %s",
                        m->current_server_name->string);
            continue;
        }

        r = server_address_new(m->current_server_name, &a,
                               (const union sockaddr_union*)ai->ai_addr,
                               ai->ai_addrlen);
        if (r < 0)
            return log_error_errno(r, "Failed to add server address: %m");

        server_address_pretty(a, &pretty);
        log_debug("Resolved address %s for %s.", pretty,
                  m->current_server_name->string);
    }

    if (!m->current_server_name->addresses)
    {
        log_error("Failed to find suitable address for host %s.",
                  m->current_server_name->string);
...
static int manager_retry_connect(sd_event_source* source, usec_t usec,
                                 void* userdata)
{
    Manager* m = userdata;
...
int manager_connect(Manager* m)
{
    ...
    if (!ratelimit_below(&m->ratelimit))
    {
        ...
        r = sd_event_add_time(m->event, &m->event_retry,
                              clock_boottime_or_monotonic(),
    ...
    if (m->current_server_address && m->current_server_address->addresses_next)
        manager_set_server_address(m,
                                   m->current_server_address->addresses_next);
    else
    {
        ...
        /* Hmm, we are through all addresses, let's look for the next host
         * instead */
        ...
        else
        {
            ...
            if (!m->current_server_name ||
                m->current_server_name->type == SERVER_LINK)
            {
                f = m->system_servers;
                if (!f)
                    f = m->link_servers;
...
static int manager_network_read_link_servers(Manager* m)
{
    ...
    STRV_FOREACH(i, ntp)
    {
        ...
        {
            log_error_errno(r,
                            "Failed to check validity of NTP server name or "
                            "address '%s': %m",
                            *i);
            goto clear;
        }
...
{
    /* Return true when the manager is sending a request, resolving a server
     * name, or in a poll interval. */
    return m->server_socket >= 0 || m->resolve_query || m->event_timer;
}

static int manager_network_event_handler(sd_event_source* s, int fd,
                                         uint32_t revents, void* userdata)
{
    Manager* m = userdata;
    ...
    /* When manager_network_read_link_servers() failed, we assume that the
     * servers are changed. */
...
static int manager_network_monitor_listen(Manager* m)
{
    ...
    if (r == -ENOENT)
    {
        log_info("systemd does not appear to be running, not listening for "
