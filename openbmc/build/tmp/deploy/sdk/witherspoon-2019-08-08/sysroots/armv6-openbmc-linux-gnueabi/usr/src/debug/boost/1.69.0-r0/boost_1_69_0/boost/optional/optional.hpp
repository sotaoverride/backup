...
class optional_base : public optional_tag
{
  ...
  protected:
    ...
    optional_base(init_value_tag, argument_type val) : m_initialized(false)
    {
        construct(val);
    }
    ...
    // creates an uninitialized optional<T>. Can throw if T::T(T const&) does
    ...
    // otherwise creates an uninitialized optional<T>. Can throw if T::T(T &&)
    // does
    optional_base(bool cond, rval_reference_type val) : m_initialized(false)
    {
        if (cond)
            construct(boost::move(val));
    }
...
#ifndef BOOST_OPTIONAL_DETAIL_NO_RVALUE_REFERENCES
    optional_base& operator=(optional_base&& rhs) BOOST_NOEXCEPT_IF(
        ::boost::is_nothrow_move_constructible<
            T>::value&& ::boost::is_nothrow_move_assignable<T>::value)
    {
    ...
    // Assigns from another _convertible_ optional<U> (deep-copies the rhs
    // value)
    template <class U>
...
#ifndef BOOST_OPTIONAL_DETAIL_NO_RVALUE_REFERENCES
    // move-assigns from another _convertible_ optional<U> (deep-moves from the
    // rhs value)
    template <class U>
    void assign(optional<U>&& rhs)
    {
        typedef BOOST_DEDUCED_TYPENAME optional<U>::rval_reference_type
            ref_type;
        if (is_initialized())
        {
            if (rhs.is_initialized())
    ...
    // UNINITIALIZED No-throw (assuming T::~T() doesn't)
  ...
  protected:
    ...
    template <class Arg>
    explicit optional_base(in_place_init_t, Arg&& arg) : m_initialized(false)
    {
        construct(in_place_init, boost::forward<Arg>(arg));
    }

    explicit optional_base(in_place_init_t) : m_initialized(false)
    {
        construct(in_place_init);
    }
    ...
    explicit optional_base(in_place_init_if_t, bool cond) : m_initialized(false)
    {
        if (cond)
            construct(in_place_init);
    }
    ...
    template <class Expr>
    void assign_expr_to_initialized(Expr&& factory,
    ...
    // argument of a one-argument T constructor. Converting constructions of
    // optional<T> from optional<U> uses this function with 'Expr' being of type
    // 'U' and relying on a converting constructor of T from U.
    template <class Expr>
    ...
    // argument of a T's assignment operator. Converting assignments of
    // optional<T> from optional<U> uses this function with 'Expr' being of type
    // 'U' and relying on a converting assignment of T from U.
    template <class Expr>
    ...
    //   optional<T> and optional<U> to the 'Expr'-taking functions even though
    //   explicit overloads are present for them. Thus, the following overload
    //   is needed to properly handle the case when the 'lhs' is another
    //   optional.
    //
    // For VC<=70 compilers this workaround dosen't work becasue the comnpiler
    // issues and error instead of choosing the wrong overload
    //
#ifndef BOOST_OPTIONAL_DETAIL_NO_RVALUE_REFERENCES
...
struct is_optional_related
    : boost::conditional<
          boost::is_base_of<
              ...
              BOOST_DEDUCED_TYPENAME boost::decay<U>::type>::value ||
              boost::is_same<BOOST_DEDUCED_TYPENAME boost::decay<U>::type,
                             none_t>::value ||
              boost::is_same<BOOST_DEDUCED_TYPENAME boost::decay<U>::type,
                             in_place_init_t>::value ||
              boost::is_same<BOOST_DEDUCED_TYPENAME boost::decay<U>::type,
                             in_place_init_if_t>::value,
    ...
    : boost::conditional<
          boost::is_base_of<
              ...
              (boost::is_constructible<T, U&&>::value &&
               !boost::is_same<
                   T, BOOST_DEDUCED_TYPENAME boost::decay<U>::type>::value),
          boost::true_type, boost::false_type>::type
{
};
...
struct is_optional_val_init_candidate
    : boost::conditional<!is_optional_related<U>::value &&
                             is_convertible_to_T_or_factory<T, U>::value,
                         boost::true_type, boost::false_type>::type
{
};
...
#define BOOST_OPTIONAL_BASE_TYPE(T)                                            \
    boost::conditional<                                                        \
...
template <class T>
class optional : public BOOST_OPTIONAL_BASE_TYPE(T)
{
  ...
  public:
    ...
    typedef BOOST_DEDUCED_TYPENAME base::reference_const_type
        reference_const_type;
#ifndef BOOST_OPTIONAL_DETAIL_NO_RVALUE_REFERENCES
    typedef BOOST_DEDUCED_TYPENAME base::rval_reference_type
        rval_reference_type;
    typedef BOOST_DEDUCED_TYPENAME base::reference_type_of_temporary_wrapper
    ...
    optional(rval_reference_type val) :
        base(optional_detail::init_value_tag(), boost::forward<T>(val))
    {
    }
#endif

    // Creates an optional<T> initialized with 'val' IFF cond is true, otherwise
    // creates an uninitialized optional. Can throw if T::T(T const&) does
    ...
    explicit optional(
        ...
        BOOST_DEDUCED_TYPENAME boost::enable_if<
            optional_detail::is_optional_constructible<T, U const&>,
            bool>::type = true
#endif
        ) :
        base()
    ...
    explicit optional(
        ...
        ,
        BOOST_DEDUCED_TYPENAME boost::enable_if<
            optional_detail::is_optional_constructible<T, U>, bool>::type = true
#endif
        ) :
        base()
    {
    ...
    explicit optional(
        Expr&& expr,
        BOOST_DEDUCED_TYPENAME boost::enable_if<
            optional_detail::is_optional_val_init_candidate<T, Expr>,
            bool>::type = true) :
    ...
    // Copy-assigns from another convertible optional<U> (converts &&
    // deep-copies the rhs value) Requires a valid conversion from U to T. Basic
    // Guarantee: If T::T( U const& ) throws, this is left UNINITIALIZED
    template <class U>
...
#ifndef BOOST_OPTIONAL_DETAIL_NO_RVALUE_REFERENCES
    // Move-assigns from another convertible optional<U> (converts && deep-moves
    // the rhs value) Requires a valid conversion from U to T. Basic Guarantee:
    // If T::T( U && ) throws, this is left UNINITIALIZED
    template <class U>
    ...
    BOOST_DEDUCED_TYPENAME boost::enable_if<
        boost::is_same<T, BOOST_DEDUCED_TYPENAME boost::decay<T_>::type>,
        optional&>::type
        operator=(T_&& val)
    {
    ...
    explicit optional(in_place_init_t) : base(in_place_init)
    {
    }
    ...
    template <class Arg>
    explicit optional(in_place_init_t, Arg& arg) : base(in_place_init, arg)
    {
    }
    ...
    (!defined BOOST_OPTIONAL_DETAIL_NO_RVALUE_REFERENCES)
    ...
    template <typename F>
    optional<typename boost::result_of<F(reference_const_type)>::type>
        map(F f) const&
    {
        if (this->has_value())
            return f(get());
    ...
    template <typename F>
    optional<
        typename boost::result_of<F(reference_type_of_temporary_wrapper)>::type>
        map(F f) &&
    {
        if (this->has_value())
    ...
    optional<typename optional_detail::optional_value_type<
        typename boost::result_of<F(reference_type)>::type>::type>
        flat_map(F f) &
    {
        if (this->has_value())
            return f(get());
    ...
    template <typename F>
    optional<typename optional_detail::optional_value_type<
        typename boost::result_of<F(reference_const_type)>::type>::type>
        flat_map(F f) const&
    {
        if (this->has_value())
            return f(get());
    ...
    template <typename F>
    optional<
        typename optional_detail::optional_value_type<typename boost::result_of<
            F(reference_type_of_temporary_wrapper)>::type>::type>
        flat_map(F f) &&
    {
        if (this->has_value())
...
class optional<T&&>
{
    BOOST_STATIC_ASSERT_MSG(sizeof(T) == 0,
                            "Optional rvalue references are illegal.");
};
#endif
...
template <class T>
inline optional<BOOST_DEDUCED_TYPENAME boost::decay<T>::type>
    make_optional(T&& v)
{
...
// behaviour is UNDEFINED. No-throw
template <class T>
inline BOOST_DEDUCED_TYPENAME optional<T>::reference_const_type
...
template <class T>
inline BOOST_DEDUCED_TYPENAME optional<T>::reference_type get(optional<T>& opt)
{
    return opt.get();
}
...
// NULL. No-throw
template <class T>
inline BOOST_DEDUCED_TYPENAME optional<T>::pointer_const_type
    get(optional<T> const* opt)
{
    return opt->get_ptr();
}
...
template <class T>
inline BOOST_DEDUCED_TYPENAME optional<T>::pointer_type get(optional<T>* opt)
{
    return opt->get_ptr();
}
...
template <class T>
inline BOOST_DEDUCED_TYPENAME optional<T>::reference_type
...
template <class T>
inline BOOST_DEDUCED_TYPENAME optional<T>::pointer_type
...
{
    BOOST_STATIC_ASSERT_MSG(sizeof(CharType) == 0,
                            "If you want to output boost::optional, include "
