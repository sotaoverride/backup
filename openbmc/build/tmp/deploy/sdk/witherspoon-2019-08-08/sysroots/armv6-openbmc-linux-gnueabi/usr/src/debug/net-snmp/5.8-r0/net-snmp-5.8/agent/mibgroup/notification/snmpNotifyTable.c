...
static oid snmpNotifyFullCompliance[] = {
    SNMP_OID_SNMPMODULES, 13, 3, 1,
    3}; /* SNMP-NOTIFICATION-MIB::snmpNotifyFullCompliance */
...
void parse_snmpNotifyTable(const char* token, char* line)
{
    ...
    line =
        read_config_read_data(ASN_OCTET_STR, line, &StorageTmp->snmpNotifyName,
    ...
    line =
        read_config_read_data(ASN_OCTET_STR, line, &StorageTmp->snmpNotifyTag,
    ...
    line = read_config_read_data(ASN_INTEGER, line, &StorageTmp->snmpNotifyType,
                                 &tmpint);
 ...
 *   The workings of this are basically the same as for var_snmpNotifyTable
 * above.
 */
unsigned char* var_snmpNotifyTable(struct variable* vp, oid* name,
                                   ...
                                   WriteMethod** write_method)
{
     ...
     */
    if ((StorageTmp = find_row_notifyTable(vp, name, length, exact, var_len,
                                           write_method)) == NULL)
    {
        found = 0;
    }
...
int write_snmpNotifyTag(int action, u_char* var_val, u_char var_val_type,
...
int write_snmpNotifyType(int action, u_char* var_val, u_char var_val_type,
                         size_t var_val_len, u_char* statP, oid* name,
                         size_t name_len)
{
    static int tmpvar;
...
int write_snmpNotifyStorageType(int action, u_char* var_val,
...
int write_snmpNotifyRowStatus(int action, u_char* var_val, u_char var_val_type,
                              size_t var_val_len, u_char* statP, oid* name,
                              size_t name_len)
{
    struct snmpNotifyTable_data* StorageTmp = NULL;
        ...
        case RESERVE1:
                ...
                (set_value == RS_CREATEANDGO || set_value == RS_CREATEANDWAIT))
            {
                ...
                snmp_varlist_add_variable(&vars, NULL, 0,
                                          ASN_PRIV_IMPLIED_OCTET_STR, NULL,
                                          0); /* snmpNotifyName */
