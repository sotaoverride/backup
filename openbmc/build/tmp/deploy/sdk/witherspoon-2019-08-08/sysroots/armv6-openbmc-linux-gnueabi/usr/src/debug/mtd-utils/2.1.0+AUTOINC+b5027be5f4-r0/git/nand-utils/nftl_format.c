...
int main(int argc, char** argv)
{
    ...
    /* Phase 1. Erasing and checking each erase zones in the NFTL partition.
       N.B. Erase Zones not used by the NFTL partition are untouched and marked
       ZONE_GOOD */
    printf(
        "Phase 1. Checking and erasing Erase Zones from 0x%08lx to 0x%08lx\n",
    ...
    /* N.B. from dump of M-System original chips, NumEraseUnits counts the 2
       Erase Unit used by MediaHeader and the FirstPhysicalEUN starts from the
       MediaHeader */
    if (do_inftl)
    {
    ...
    else
    {
        ...
        /* N.B. we reserve 2 more Erase Units for "folding" of Virtual Unit
         * Chain */
        NFTLhdr->FormattedSize =
            cpu_to_le32(part_size - ((5 + bad_zones) * meminfo.erasesize));
    ...
    pwrite_nocheck(fd, writebuf[0], 512,
                   MediaUnit1 * meminfo.erasesize + MediaUnitOff1);
    for (ezone = 0; ezone < (meminfo.size / meminfo.erasesize); ezone += 512)
    {
        pwrite_nocheck(fd, BadUnitTable + ezone, 512,
                       (MediaUnit1 * meminfo.erasesize) +
                           512 * (1 + ezone / 512));
    }
    ...
    printf("Phase 2.b Writing Spare %s Media Header and Spare Bad Unit Table\n",
           nftl);
    pwrite_nocheck(fd, writebuf[0], 512,
                   MediaUnit2 * meminfo.erasesize + MediaUnitOff2);
    for (ezone = 0; ezone < (meminfo.size / meminfo.erasesize); ezone += 512)
    {
        pwrite_nocheck(fd, BadUnitTable + ezone, 512,
                       (MediaUnit2 * meminfo.erasesize + MediaUnitOff2) +
    ...
    /* N.B. The Media Header and Bad Erase Unit Table are considered as Free
       Erase Unit by M-System i.e. their Virtual Unit Number == 0xFFFF in the
       Unit Control Information #0, but their Block Status is BLOCK_USED
       (0x5555) in their Block Control Information */
    ...
    for (ezone = MediaUnit1; ezone < (ezones + startofs / meminfo.erasesize);
         ezone++)
    {
