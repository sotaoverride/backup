...
{
  public:
    interprocess_exception(const char* err) : m_err(other_error)
    {
        try
        {
            m_str = err;
...
//! This is the exception thrown by shared interprocess_mutex family when a
//! deadlock situation is detected or when using a interprocess_condition the
//! interprocess_mutex is not locked
class BOOST_SYMBOL_VISIBLE lock_exception : public interprocess_exception
{
  public:
    lock_exception() : interprocess_exception(lock_error)
    {
    }
