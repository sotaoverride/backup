...
static char* getttyname(const char* dev, const struct stat64* mytty, int save,
                        int* dostat);

libc_freeres_ptr(static char* getttyname_name);

static char* attribute_compat_text_section getttyname(
    const char* dev, const struct stat64* mytty, int save, int* dostat)
{
...
#define UNREACHABLE_LEN strlen("(unreachable)")
        ...
        if (ttyname_buf[0] == '/' &&
            __xstat64(_STAT_VER, ttyname_buf, &st1) == 0 && is_mytty(&st, &st1))
            return ttyname_buf;
