 ...
 * 2 was terminated and then started with a new ABS_MT_TRACKING_ID. The
 * touchpoint in slot 1 maintained the same ABS_MT_TRACKING_ID and only updated
 * the coordinates. Slot 1 is the currently active slot.
 *
...
/**
 * @page backwardscompatibility Compatibility and Behavior across kernel
 * versions
 *
 ...
 *
 * By default, Check forks, making debugging harder. The test suite tries to
 * detect if it is running inside gdb and disable forking. If that doesn't work
 * for some reason, run gdb as below to avoid forking.
 *
 ...
 * the caller to parse events through libevdev_next_event(). If a caller does
 * not use libevdev_next_event(), libevdev will not update the internal state of
 * the device and thus returns outdated values.
 */
 ...
 * to the caller. Thus, the libevdev state of the device should always be
 * identical to the caller's state. It may however lag behind the actual state
 * of the device.
 */
...
enum libevdev_read_flag
{
    ...
    LIBEVDEV_READ_FLAG_FORCE_SYNC = 4, /**< Pretend the next event is a
                            SYN_DROPPED and require the caller to sync */
 ...
 * @ingroup init
 *
 * Change the fd for this device, without re-reading the actual device. If the
 * fd changes after initializing the device, for example after a VT-switch in
 * the X.org X server, this function updates the internal fd to the newly
 * opened. No check is made that new fd points to the same device. If the device
 * has changed, libevdev's behavior is undefined.
 *
 ...
 * LIBEVDEV_READ_STATUS_SYNC) ; // noop
 * @endcode
 *
 ...
 * @ref LIBEVDEV_READ_FLAG_SYNC flag set, to get the set of events that make up
 * the device state delta. This function returns @ref LIBEVDEV_READ_STATUS_SYNC
 * for each event part of that delta, until it returns -EAGAIN once all events
 ...
 *
 * @param dev The evdev device, already initialized with libevdev_set_fd()
 * @param flags Set of flags to determine behaviour. If @ref
 * LIBEVDEV_READ_FLAG_NORMAL is set, the next event is read in normal mode. If
 * @ref LIBEVDEV_READ_FLAG_SYNC is set, the next event is read in sync mode.
 ...
 */
int libevdev_set_event_value(struct libevdev* dev, unsigned int type,
 ...
 */
int libevdev_fetch_event_value(const struct libevdev* dev, unsigned int type,
                               unsigned int code, int* value);
 ...
 * @see libevdev_get_event_value
 */
int libevdev_get_slot_value(const struct libevdev* dev, unsigned int slot,
                            unsigned int code);
 ...
 * @param slot The numerical slot number, must be smaller than the total number
 * of slots on this device
 * @param code The event code to set the value for, one of ABS_MT_POSITION_X,
 * etc.
 * @param value The new value to set
 *
 * @return 0 on success, or -1 on failure.
 ...
 * @see libevdev_get_slot_value
 */
int libevdev_set_slot_value(struct libevdev* dev, unsigned int slot,
                            unsigned int code, int value);

/**
 * @ingroup mt
 *
 * Fetch the current value of the code for the given slot. This is a shortcut
 * for
 *
 * @code
 ...
 */
void libevdev_set_abs_resolution(struct libevdev* dev, unsigned int code,
                                 int resolution);
 ...
 */
void libevdev_set_abs_info(struct libevdev* dev, unsigned int code,
 ...
 * @param data If type is EV_ABS, data points to a struct input_absinfo. If type
 * is EV_REP, data points to an integer. Otherwise, data must be NULL.
 *
 * @return 0 on success or -1 otherwise
 *
 ...
 *
 * @note enabling an LED requires write permissions on the device's file
 * descriptor.
 *
 * @param dev The evdev device, already initialized with libevdev_set_fd()
 ...
 * code are valid.
 *
 * @note The ranges for types and codes are compiled into libevdev. If the
 * kernel changes the max value, libevdev will not automatically pick these up.
 *
 * @param ev The input event to check
 * @param type Input event type to compare the event against (EV_REL, EV_ABS,
 * etc.)
 ...
 *
 * @return The name of the given input prop (e.g. INPUT_PROP_BUTTONPAD) or NULL
 ...
 * @ingroup misc
 *
 * @param type The event type to return the maximum for (EV_ABS, EV_REL, etc.).
 * No max is defined for EV_SYN.
 *
 * @return The max value defined for the given event type, e.g. ABS_MAX for a
 * type of EV_ABS, or -1 for an invalid type.
 *
