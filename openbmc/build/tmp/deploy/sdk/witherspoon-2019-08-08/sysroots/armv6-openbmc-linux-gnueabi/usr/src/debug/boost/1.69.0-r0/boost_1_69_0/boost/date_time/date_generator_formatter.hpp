...
class date_generator_formatter
{
  public:
    ...
    date_generator_formatter(
        const string_type& first_str, const string_type& second_str,
        const string_type& third_str, const string_type& fourth_str,
        const string_type& fifth_str, const string_type& last_str,
        const string_type& before_str, const string_type& after_str,
                  ...
                  phrase_elements beg_pos = first)
    {
        if (beg_pos < number_of_phrase_elements)
        {
            ...
            itr += beg_pos;
            std::copy(new_strings.begin(), new_strings.end(), itr);
...
template <class date_type, class CharT, class OutItrT>
const typename date_generator_formatter<date_type, CharT, OutItrT>::char_type
    date_generator_formatter<date_type, CharT, OutItrT>::of_string[3] = {'o',
                                                                         'f'};
} // namespace date_time
} // namespace boost
