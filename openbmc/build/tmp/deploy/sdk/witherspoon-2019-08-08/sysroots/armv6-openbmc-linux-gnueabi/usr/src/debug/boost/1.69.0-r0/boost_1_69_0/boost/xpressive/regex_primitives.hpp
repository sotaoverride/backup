...
struct SubMatch
    : proto::or_<
          proto::when<basic_mark_tag,
                      push_back(proto::_data, mark_number(proto::_value))>,
          proto::when<proto::negate<basic_mark_tag>,
                      push_back(proto::_data, minus_one())>>
{
};
    ...
    struct impl : proto::transform_impl<Expr, State, Data>
    {
        typedef typename proto::shift_right<
            typename proto::unary_expr<
                keeper_tag, typename proto::dereference<State>::type>::type,
...
struct Primitives
    : proto::or_<proto::terminal<proto::_>, proto::comma<proto::_, proto::_>,
                 proto::subscript<proto::terminal<set_initializer>, proto::_>,
                 proto::assign<proto::terminal<set_initializer>, proto::_>,
                 proto::assign<proto::terminal<attribute_placeholder<proto::_>>,
                               proto::_>,
...
struct SkipGrammar
    : proto::or_<
          ...
          proto::assign<proto::terminal<mark_placeholder>,
                        SkipGrammar> // don't "skip" mark tags
          ,
          proto::subscript<SkipGrammar, proto::_> // don't put skips in actions
          ,
          proto::binary_expr<modifier_tag, proto::_, SkipGrammar> // don't skip
                                                                  // modifiers
          ,
          proto::unary_expr<lookbehind_tag, proto::_> // don't skip lookbehinds
          ,
          proto::nary_expr<proto::_, proto::vararg<SkipGrammar>> // everything
                                                                 // else is fair
                                                                 // game!
          >
{
...
struct skip_directive
{
    ...
    skip_directive(Skip const& skip) : skip_(proto::as_expr(skip))
    {
    }
    ...
    {
        typedef SkipGrammar::impl<
            typename proto::result_of::as_expr<Expr>::type, skip_type const&,
            mpl::void_&>
            skip_transform;
        ...
        operator()(Expr const& expr) const
    {
        ...
        typename result_fun::type that = {
            typename result_fun::skip_transform()(proto::as_expr(expr),
                                                  this->skip_, ignore),
            {skip_}};
        return that;
    }
...
///
/// The regex traits are used to determine which characters are blank
/// characters. To match any character that is not blank, use ~blank.
///
/// \attention blank is equivalent to /[[:blank:]]/ in perl. ~blank is
/// equivalent to /[[:^blank:]]/ in perl.
...
///
/// The regex traits are used to determine which characters are control
/// characters. To match any character that is not a control character, use
/// ~cntrl.
///
/// \attention cntrl is equivalent to /[[:cntrl:]]/ in perl. ~cntrl is
/// equivalent to /[[:^cntrl:]]/ in perl.
...
///
/// The regex traits are used to determine which characters are space
/// characters. To match any character that is not white-space, use ~space.
///
/// \attention space is equivalent to /[[:space:]]/ in perl. ~space is
/// equivalent to /[[:^space:]]/ in perl.
...
///
/// '_b' matches the zero-width sub-sequence at the beginning or the end of a
/// word. It is equivalent to (bow | eow). The regex traits are used to
/// determine what constitutes a word character. To match a non-word boundary,
/// use ~_b.
///
...
///
/// '_w' matches a single word character. The regex traits are used to determine
/// which characters are word characters. Use ~_w to match a character that is
/// not a word character.
///
/// \attention _w is like \\w in perl. ~_w is like \\W in perl.
proto::terminal<detail::posix_charset_placeholder>::type const _w = {
    {"w", false}};

///////////////////////////////////////////////////////////////////////////////
/// \brief Matches a digit character.
///
/// '_d' matches a single digit character. The regex traits are used to
/// determine which characters are digits. Use ~_d to match a character that is
/// not a digit character.
///
/// \attention _d is like \\d in perl. ~_d is like \\D in perl.
proto::terminal<detail::posix_charset_placeholder>::type const _d = {
    {"d", false}};

///////////////////////////////////////////////////////////////////////////////
/// \brief Matches a space character.
///
/// '_s' matches a single space character. The regex traits are used to
/// determine which characters are space characters. Use ~_s to match a
/// character that is not a space character.
///
/// \attention _s is like \\s in perl. ~_s is like \\S in perl.
proto::terminal<detail::posix_charset_placeholder>::type const _s = {
    {"s", false}};
...
///
/// To complement a set, apply the '~' operator. For instance, ~(set=
/// 'a','b','c') will match any character that is not an 'a', 'b', or 'c'.
///
...
/// static regexes.
///
/// \c mark_tag is the type of the global sub-match placeholders \c s0, \c s1,
/// etc.. You can use the \c mark_tag type to create your own sub-match
/// placeholders with more meaningful names. This is roughly equivalent to the
/// "named capture" feature of dynamic regular expressions.
///
/// To create a named sub-match placeholder, initialize it with a unique
/// integer. The integer must only be unique within the regex in which the
/// placeholder is used. Then you can use it within static regexes to created
/// sub-matches by assigning a sub-expression to it, or to refer back to already
/// created sub-matches.
///
/// \code
...
/// \endcode
///
/// After a successful \c regex_match() or \c regex_search(), the sub-match
/// placeholder can be used to index into the <tt>match_results\<\></tt> object
/// to retrieve the corresponding sub-match.
struct mark_tag
    : proto::extends<detail::basic_mark_tag, mark_tag, detail::regex_domain>
{
  private:
    typedef proto::extends<detail::basic_mark_tag, mark_tag,
                           detail::regex_domain>
        base_type;
  ...
  public:
    ...
    mark_tag(int mark_nbr) : base_type(mark_tag::make_tag(mark_nbr))
    {
...
///
/// After a successful regex_match() or regex_search(), the sub-match
/// placeholders can be used to index into the match_results\<\> object to
/// retrieve the Nth sub-match.
...
// detail/icase.hpp so that it can serve double-duty as regex_constants::icase,
...
/// operator are const char*, and no such operator exists. Use as_xpr("foo") >>
/// "bar" instead.
///
...
inline typename proto::terminal<
    reference_wrapper<basic_regex<BidiIter> const>>::type const
    by_ref(basic_regex<BidiIter> const& rex)
{
    reference_wrapper<basic_regex<BidiIter> const> ref(rex);
    return proto::terminal<
        reference_wrapper<basic_regex<BidiIter> const>>::type::make(ref);
}
...
typename proto::result_of::make_expr<
    proto::tag::logical_not, proto::default_domain, Expr const&>::type const
    optional(Expr const& expr)
{
    return proto::make_expr<proto::tag::logical_not, proto::default_domain>(
        boost::ref(expr));
}
...
///
/// The repeat\<\>() function creates a greedy quantifier. To make the
/// quantifier non-greedy, apply the unary minus operator, as in
/// -repeat\<M,N\>(expr).
///
    ...
    repeat(Expr const& expr)
{
    return proto::make_expr<detail::generic_quant_tag<Min, Max>,
                            proto::default_domain>(boost::ref(expr));
}
    ...
    repeat(Expr2 const& expr2)
{
    return proto::make_expr<detail::generic_quant_tag<Count, Count>,
                            proto::default_domain>(boost::ref(expr2));
}
...
template <typename Expr>
typename proto::result_of::make_expr<detail::keeper_tag, proto::default_domain,
                                     Expr const&>::type const
    keep(Expr const& expr)
{
    return proto::make_expr<detail::keeper_tag, proto::default_domain>(
        boost::ref(expr));
}
...
/// position in the sequence, but expr is not included in the match. For
/// instance, before("foo") succeeds if we are before a "foo". Look-ahead
...
template <typename Expr>
typename proto::result_of::make_expr<
    detail::lookahead_tag, proto::default_domain, Expr const&>::type const
    before(Expr const& expr)
{
    return proto::make_expr<detail::lookahead_tag, proto::default_domain>(
        boost::ref(expr));
}
...
/// position minus N in the sequence, where N is the width of expr. expr is not
/// included in the match. For instance,  after("foo") succeeds if we are after
/// a "foo". Look-behind assertions can be negated with the bit-complement
/// operator.
///
...
template <typename Expr>
typename proto::result_of::make_expr<
    detail::lookbehind_tag, proto::default_domain, Expr const&>::type const
    after(Expr const& expr)
{
    return proto::make_expr<detail::lookbehind_tag, proto::default_domain>(
        boost::ref(expr));
}
...
/// when matching the regex. The entire expression must use the same
/// traits/locale. For instance, the following specifies a locale for use with a
/// regex:
///   std::locale loc;
    ...
    imbue(Locale const& loc)
{
    detail::modifier_op<detail::locale_modifier<Locale>> mod = {
        detail::locale_modifier<Locale>(loc), regex_constants::ECMAScript};
    return mod;
}
...
///
/// <tt>skip()</tt> instructs the regex engine to skip certain characters when
/// matching a regex. It is most useful for writing regexes that ignore
/// whitespace. For instance, the following specifies a regex that skips
