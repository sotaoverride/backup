                                  ...
                                  void* userdata)
{
    ...
    printf("%-6s[%" PRI_TIME ".%06" PRI_NSEC "] %-8s %s (%s)\n",
           group == MONITOR_GROUP_UDEV ? "UDEV" : "KERNEL", ts.tv_sec,
           (nsec_t)ts.tv_nsec / 1000, strna(device_action_to_string(action)),
           devpath, subsystem);
...
static int setup_monitor(MonitorNetlinkGroup sender, sd_event* event,
                         sd_device_monitor** ret)
{
    ...
    HASHMAP_FOREACH_KEY(devtype, subsystem, arg_subsystem_filter, i)
    {
        ...
        if (r < 0)
            return log_error_errno(
                r, "Failed to apply subsystem filter '%s%s%s': %m", subsystem,
                devtype ? "/" : "", strempty(devtype));
    }
    ...
    SET_FOREACH(tag, arg_tag_filter, i)
    {
        ...
        if (r < 0)
            return log_error_errno(r, "Failed to apply tag filter '%s': %m",
                                   tag);
    }

    r = sd_device_monitor_start(monitor, device_monitor_handler,
                                INT_TO_PTR(sender));
    if (r < 0)
        return log_error_errno(r, "Failed to start device monitor: %m");

    (void)sd_event_source_set_description(
        sd_device_monitor_get_event_source(monitor),
...
static int help(void)
{
    printf("%s monitor [OPTIONS]\n\n"
           ...
           "  -p --property                            Print the event "
           "properties\n"
           "  -k --kernel                              Print kernel uevents\n"
           "  -u --udev                                Print udev events\n"
           "  -s --subsystem-match=SUBSYSTEM[/DEVTYPE] Filter events by "
           "subsystem\n"
...
{
    _cleanup_(sd_device_monitor_unrefp) sd_device_monitor *kernel_monitor =
                                                              NULL,
                                                          *udev_monitor = NULL;
