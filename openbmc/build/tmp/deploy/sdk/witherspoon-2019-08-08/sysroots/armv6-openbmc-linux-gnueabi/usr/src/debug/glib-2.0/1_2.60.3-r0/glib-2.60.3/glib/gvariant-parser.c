 ...
 *given where a basic type was expected
 * @G_VARIANT_PARSE_ERROR_CANNOT_INFER_TYPE: cannot infer the #GVariantType
 * @G_VARIANT_PARSE_ERROR_DEFINITE_TYPE_EXPECTED: an indefinite #GVariantType
 *was given where a definite type was expected
 * @G_VARIANT_PARSE_ERROR_INPUT_NOT_AT_END: extra data after parsing finished
 * @G_VARIANT_PARSE_ERROR_INVALID_CHARACTER: invalid character in number or
 *unicode escape
 * @G_VARIANT_PARSE_ERROR_INVALID_FORMAT_STRING: not a valid #GVariant format
 *string
 ...
 * @G_VARIANT_PARSE_ERROR_NO_COMMON_TYPE: could not find a common type for array
 *entries
 * @G_VARIANT_PARSE_ERROR_NUMBER_OUT_OF_RANGE: the numerical value is out of
 *range of the given type
 * @G_VARIANT_PARSE_ERROR_NUMBER_TOO_BIG: the numerical value is out of range
 *for any type
 * @G_VARIANT_PARSE_ERROR_TYPE_ERROR: cannot parse as variant of the specified
 *type
 ...
 * @G_VARIANT_PARSE_ERROR_UNTERMINATED_STRING_CONSTANT: unterminated string
 *constant
...
G_GNUC_PRINTF(5, 0)
static void parser_set_error_va(GError** error, SourceRef* location,
                                SourceRef* other, gint code,
...
static void parser_set_error(GError** error, SourceRef* location,
                             SourceRef* other, gint code, const gchar* format,
                             ...)
{
    va_list ap;
...
G_GNUC_PRINTF(5, 6)
static void token_stream_set_error(TokenStream* stream, GError** error,
                                   gboolean this_token, gint code,
                                   const gchar* format, ...)
{
    SourceRef ref;
    va_list ap;
...
static gboolean token_stream_prepare(TokenStream* stream)
{
    gint brackets = 0;
    const gchar* end;
    ...
    switch (stream->stream[0])
    {
        ...
        case '9':
            for (end = stream->stream; end != stream->end; end++)
                if (!g_ascii_isalnum(*end) && *end != '-' && *end != '+' &&
                    *end != '.')
                    break;
            break;
...
static gboolean token_stream_peek2(TokenStream* stream, gchar first_char,
                                   gchar second_char)
{
    if (!token_stream_prepare(stream))
        return FALSE;
    ...
    return stream->stream - stream->this >= 2 &&
           stream->this[0] == first_char && stream->this[1] == second_char;
}
...
static gboolean token_stream_is_keyword(TokenStream* stream)
{
    ...
    return stream->stream - stream->this >= 2 &&
           g_ascii_isalpha(stream->this[0]) && g_ascii_isalpha(stream->this[1]);
}
...
static gboolean token_stream_is_numeric(TokenStream* stream)
{
    ...
    return (stream->stream - stream->this >= 1 &&
            (g_ascii_isdigit(stream->this[0]) || stream->this[0] == '-' ||
             stream->this[0] == '+' || stream->this[0] == '.'));
}

static gboolean token_stream_peek_string(TokenStream* stream,
                                         const gchar* token)
{
...
static gboolean token_stream_require(TokenStream* stream, const gchar* token,
                                     const gchar* purpose, GError** error)
{
...
static void pattern_copy(gchar** out, const gchar** in)
{
    gint brackets = 0;
...
typedef GVariant* (*get_value_func)(AST* ast, const GVariantType* type,
                                    GError** error);
typedef GVariant* (*get_base_value_func)(AST* ast, const GVariantType* type,
                                         GError** error);
...
typedef struct
{
    ...
    GVariant* (*get_base_value)(AST* ast, const GVariantType* type,
                                GError** error);
    void (*free)(AST* ast);
} ASTClass;
...
static GVariant* ast_get_value(AST* ast, const GVariantType* type,
                               GError** error)
{
...
G_GNUC_PRINTF(5, 6)
static void ast_set_error(AST* ast, GError** error, AST* other_ast, gint code,
                          const gchar* format, ...)
{
    va_list ap;
    ...
    parser_set_error_va(error, &ast->source_ref,
                        other_ast ? &other_ast->source_ref : NULL, code, format,
                        ap);
    va_end(ap);
}

static GVariant* ast_type_error(AST* ast, const GVariantType* type,
                                GError** error)
{
    gchar* typestr;

    typestr = g_variant_type_dup_string(type);
    ast_set_error(ast, error, NULL, G_VARIANT_PARSE_ERROR_TYPE_ERROR,
                  "can not parse as value of type '%s'", typestr);
    g_free(typestr);
...
static gchar* ast_array_get_pattern(AST** array, gint n_items, GError** error)
{
    ...
    /* Find the pattern which applies to all children in the array, by l-folding
     * a coalesce operation. This will not always work: for example, the
     * GVariant:
     *    [[0], [], [nothing]]
     * has patterns:
     *    MaMN, Ma*, Mam*
     ...
     *
     * However, the pattern MamN coalesces with all three child patterns.
     * Finding this pattern would require trying all O(n_items^2) pairs, though,
     * which is expensive. Just let it fail, and require the user to provide
     * type annotations.
     */
...
static GVariant* maybe_get_value(AST* ast, const GVariantType* type,
                                 GError** error)
{
    Maybe* maybe = (Maybe*)ast;
    GVariant* value;
...
{
    static const ASTClass maybe_class = {maybe_get_pattern, maybe_get_value,
                                         NULL, maybe_free};
    AST* child = NULL;
    Maybe* maybe;
...
static GVariant* maybe_wrapper(AST* ast, const GVariantType* type,
                               GError** error)
{
    ...
    for (depth = 0, t = type; g_variant_type_is_maybe(t);
...
static GVariant* array_get_value(AST* ast, const GVariantType* type,
                                 GError** error)
{
    Array* array = (Array*)ast;
...
{
    static const ASTClass array_class = {array_get_pattern, maybe_wrapper,
                                         array_get_value, array_free};
    ...
    while (!token_stream_consume(stream, "]"))
    {
        ...
        if (need_comma &&
            !token_stream_require(stream, ",",
                                  " or ']' to follow array element", error))
            goto error;
...
static GVariant* tuple_get_value(AST* ast, const GVariantType* type,
                                 GError** error)
{
    Tuple* tuple = (Tuple*)ast;
    const GVariantType* childtype;
...
{
    static const ASTClass tuple_class = {tuple_get_pattern, maybe_wrapper,
                                         tuple_get_value, tuple_free};
    gboolean need_comma = FALSE;
    gboolean first = TRUE;
    Tuple* tuple;
    ...
    while (!token_stream_consume(stream, ")"))
    {
        ...
        if (need_comma &&
            !token_stream_require(stream, ",",
                                  " or ')' to follow tuple element", error))
            goto error;
        ...
        if (first)
        {
            if (!token_stream_require(stream, ",", " after first tuple element",
                                      error))
                goto error;
...
static GVariant* variant_get_value(AST* ast, const GVariantType* type,
                                   GError** error)
{
...
{
    static const ASTClass variant_class = {variant_get_pattern, maybe_wrapper,
                                           variant_get_value, variant_free};
    Variant* variant;
    AST* value;
...
static gchar* dictionary_get_pattern(AST* ast, GError** error)
{
    ...
    key_pattern =
        ast_array_get_pattern(dict->keys, abs(dict->n_children), error);
    ...
    result = g_strdup_printf("M%s{%c%s}", dict->n_children > 0 ? "a" : "",
...
static GVariant* dictionary_get_value(AST* ast, const GVariantType* type,
                                      GError** error)
{
    Dictionary* dict = (Dictionary*)ast;
...
static AST* dictionary_parse(TokenStream* stream, va_list* app, GError** error)
{
    static const ASTClass dictionary_class = {
        dictionary_get_pattern, maybe_wrapper, dictionary_get_value,
        dictionary_free};
    ...
    if (!only_one &&
        !token_stream_require(stream, ":",
                              " or ',' to follow dictionary entry key", error))
        goto error;
...
static GVariant* string_get_value(AST* ast, const GVariantType* type,
                                  GError** error)
{
...
/* Accepts exactly @length hexadecimal digits. No leading sign or `0x`/`0X`
 * prefix allowed. No leading/trailing space allowed. */
static gboolean unicode_unescape(const gchar* src, gint* src_ofs, gchar* dest,
                                 gint* dest_ofs, gsize length, SourceRef* ref,
                                 GError** error)
{
    gchar buffer[9];
    guint64 value = 0;
    ...
    if (value == 0 || end != buffer + length)
    {
        ...
        parser_set_error(
            error, &escape_ref, NULL, G_VARIANT_PARSE_ERROR_INVALID_CHARACTER,
...
{
    static const ASTClass string_class = {string_get_pattern, maybe_wrapper,
                                          string_get_value, string_free};
    String* string;
    SourceRef ref;
    gchar* token;
    gsize length;
        ...
        switch (token[i])
        {
            ...
            case '\\':
                switch (token[++i])
                {
                    ...
                    case 'u':
                        if (!unicode_unescape(token, &i, str, &j, 4, &ref,
                                              error))
                        {
                            g_free(token);
                            g_free(str);
                    ...
                    case 'U':
                        if (!unicode_unescape(token, &i, str, &j, 8, &ref,
                                              error))
                        {
                            g_free(token);
                            g_free(str);
...
static GVariant* bytestring_get_value(AST* ast, const GVariantType* type,
                                      GError** error)
{
...
static AST* bytestring_parse(TokenStream* stream, va_list* app, GError** error)
{
    static const ASTClass bytestring_class = {
        bytestring_get_pattern, maybe_wrapper, bytestring_get_value,
        bytestring_free};
    ByteString* string;
    SourceRef ref;
    gchar* token;
...
static GVariant* number_overflow(AST* ast, const GVariantType* type,
                                 GError** error)
{
    ast_set_error(ast, error, NULL, G_VARIANT_PARSE_ERROR_NUMBER_OUT_OF_RANGE,
                  "number out of range for type '%c'",
                  g_variant_type_peek_string(type)[0]);
    return NULL;
}

static GVariant* number_get_value(AST* ast, const GVariantType* type,
                                  GError** error)
{
    Number* number = (Number*)ast;
    const gchar* token;
    ...
    switch (*g_variant_type_peek_string(type))
    {
        ...
        case 'h':
            ...
            return g_variant_new_handle(negative ? -((gint32)abs_val)
                                                 : abs_val);
...
{
    static const ASTClass number_class = {number_get_pattern, maybe_wrapper,
                                          number_get_value, number_free};
    Number* number;
...
static GVariant* boolean_get_value(AST* ast, const GVariantType* type,
                                   GError** error)
{
...
{
    static const ASTClass boolean_class = {boolean_get_pattern, maybe_wrapper,
                                           boolean_get_value, boolean_free};
    Boolean* boolean;
...
static GVariant* positional_get_value(AST* ast, const GVariantType* type,
                                      GError** error)
{
    Positional* positional = (Positional*)ast;
    GVariant* value;
    ...
    if
        G_UNLIKELY(!g_variant_is_of_type(positional->value, type))
    return ast_type_error(ast, type, error);
...
static AST* positional_parse(TokenStream* stream, va_list* app, GError** error)
{
    static const ASTClass positional_class = {
        positional_get_pattern, positional_get_value, NULL, positional_free};
...
static GVariant* typedecl_get_value(AST* ast, const GVariantType* type,
                                    GError** error)
{
    TypeDecl* decl = (TypeDecl*)ast;
...
static AST* typedecl_parse(TokenStream* stream, va_list* app, GError** error)
{
    static const ASTClass typedecl_class = {
        typedecl_get_pattern, typedecl_get_value, NULL, typedecl_free};
    GVariantType* type;
    TypeDecl* decl;
    AST* child;
 ...
 **/
GVariant* g_variant_parse(const GVariantType* type, const gchar* text,
...
static gboolean parse_num(const gchar* num, const gchar* limit, gint* result)
{
    gchar* endptr;
    gint64 bignum;
...
static void add_last_line(GString* err, const gchar* str)
{
    const gchar* last_nl;
    gchar* chomped;
    gint i;
...
static void add_lines_from_range(GString* err, const gchar* str,
                                 const gchar* start1, const gchar* end1,
                                 const gchar* start2, const gchar* end2)
{
                                           ...
                                           const gchar* source_str)
{
     ...
     *
     *  - a:          -- usually errors from the tokeniser (eof, invalid char,
     * etc.)
    ...
    if (dash == NULL || colon < dash)
    {
        ...
        if (point >= strlen(source_str))
            ...
            add_lines_from_range(err, source_str, source_str + point,
                                 source_str + point + 1, NULL, NULL);
    }
    else
    {
        ...
        if (comma && comma < colon)
        {
            ...
            add_lines_from_range(err, source_str, source_str + start1,
                                 source_str + end1, source_str + start2,
                                 source_str + end2);
        }
        else
        {
            ...
            /* One range */
            if (!parse_num(error->message, dash, &start) ||
                !parse_num(dash + 1, colon, &end))
                goto out;

            add_lines_from_range(err, source_str, source_str + start,
                                 source_str + end, NULL, NULL);
        }
    }
