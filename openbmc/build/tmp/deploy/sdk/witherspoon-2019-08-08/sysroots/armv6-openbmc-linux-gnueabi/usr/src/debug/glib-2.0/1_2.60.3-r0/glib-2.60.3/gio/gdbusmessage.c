...
static gboolean g_memory_buffer_is_byteswapped(GMemoryBuffer* mbuf)
{
...
static guchar g_memory_buffer_read_byte(GMemoryBuffer* mbuf)
{
...
static gint16 g_memory_buffer_read_int16(GMemoryBuffer* mbuf)
{
    gint16 v;
...
static guint16 g_memory_buffer_read_uint16(GMemoryBuffer* mbuf)
{
    guint16 v;
...
static gint32 g_memory_buffer_read_int32(GMemoryBuffer* mbuf)
{
    gint32 v;
...
static guint32 g_memory_buffer_read_uint32(GMemoryBuffer* mbuf)
{
    guint32 v;
...
static gint64 g_memory_buffer_read_int64(GMemoryBuffer* mbuf)
{
    gint64 v;
...
static guint64 g_memory_buffer_read_uint64(GMemoryBuffer* mbuf)
{
    guint64 v;
...
static gsize g_nearest_pow(gsize num)
{
    gsize n = 1;
...
static void array_resize(GMemoryBuffer* mbuf, gsize size)
{
    gpointer data;
    gsize len;
...
static gboolean g_memory_buffer_write(GMemoryBuffer* mbuf, const void* buffer,
                                      gsize count)
{
    guint8* dest;
    gsize new_size;
...
static gboolean g_memory_buffer_put_byte(GMemoryBuffer* mbuf, guchar data)
{
    return g_memory_buffer_write(mbuf, &data, 1);
}

static gboolean g_memory_buffer_put_int16(GMemoryBuffer* mbuf, gint16 data)
{
...
static gboolean g_memory_buffer_put_uint16(GMemoryBuffer* mbuf, guint16 data)
{
    switch (mbuf->byte_order)
    {
        case G_DATA_STREAM_BYTE_ORDER_BIG_ENDIAN:
...
static gboolean g_memory_buffer_put_int32(GMemoryBuffer* mbuf, gint32 data)
{
    switch (mbuf->byte_order)
    {
        case G_DATA_STREAM_BYTE_ORDER_BIG_ENDIAN:
...
static gboolean g_memory_buffer_put_uint32(GMemoryBuffer* mbuf, guint32 data)
{
    switch (mbuf->byte_order)
    {
        case G_DATA_STREAM_BYTE_ORDER_BIG_ENDIAN:
...
static gboolean g_memory_buffer_put_int64(GMemoryBuffer* mbuf, gint64 data)
{
    switch (mbuf->byte_order)
    {
        case G_DATA_STREAM_BYTE_ORDER_BIG_ENDIAN:
...
static gboolean g_memory_buffer_put_uint64(GMemoryBuffer* mbuf, guint64 data)
{
    switch (mbuf->byte_order)
    {
        case G_DATA_STREAM_BYTE_ORDER_BIG_ENDIAN:
...
static gboolean g_memory_buffer_put_string(GMemoryBuffer* mbuf, const char* str)
{
...
static void g_dbus_message_get_property(GObject* object, guint prop_id,
...
static void g_dbus_message_class_init(GDBusMessageClass* klass)
{
    ...
    g_object_class_install_property(
        ...
        g_param_spec_boolean("locked", P_("Locked"),
                             P_("Whether the message is locked"), FALSE,
...
static void g_dbus_message_init(GDBusMessage* message)
{
    ...
    message->headers = g_hash_table_new_full(
        g_direct_hash, g_direct_equal, NULL, (GDestroyNotify)g_variant_unref);
}
                                             ...
                                             const gchar* method)
{
    ...
    g_return_val_if_fail(
        interface_ == NULL || g_dbus_is_interface_name(interface_), NULL);
...
GDBusMessage* g_dbus_message_new_method_reply(GDBusMessage* method_call_message)
{
    ...
    g_return_val_if_fail(g_dbus_message_get_message_type(method_call_message) ==
                             G_DBUS_MESSAGE_TYPE_METHOD_CALL,
                         NULL);
    g_return_val_if_fail(g_dbus_message_get_serial(method_call_message) != 0,
                         NULL);
    ...
    g_dbus_message_set_reply_serial(
        message, g_dbus_message_get_serial(method_call_message));
                                              ...
                                              ...)
{
    ...
    ret = g_dbus_message_new_method_error_valist(
        method_call_message, error_name, error_message_format, var_args);
    va_end(var_args);
...
GDBusMessage* g_dbus_message_new_method_error_valist(
    GDBusMessage* method_call_message, const gchar* error_name,
    const gchar* error_message_format, va_list var_args)
{
    GDBusMessage* ret;
    gchar* error_message;
 ...
 * #GDBusMessageType enumeration).
 *
 * Since: 2.26
 */
GDBusMessageType g_dbus_message_get_message_type(GDBusMessage* message)
{
    g_return_val_if_fail(G_IS_DBUS_MESSAGE(message),
 ...
 * @message: A #GDBusMessage.
 * @type: A 8-bit unsigned integer (typically a value from the #GDBusMessageType
 * enumeration).
 *
 ...
 *
 * Returns: Flags that are set (typically values from the #GDBusMessageFlags
 ...
 * #GDBusMessageFlags enumeration bitwise ORed together).
 *
...
/* TODO: need GI annotations to specify that any guchar value goes for
 * header_field */
 ...
 *
 * Returns: (transfer none) (nullable): A #GVariant with the value if the header
 * was found, %NULL otherwise. Do not free, it is owned by @message.
 *
 * Since: 2.26
 */
                                    ...
                                    GDBusMessageHeaderField header_field)
{
    ...
    return g_hash_table_lookup(message->headers,
                               GUINT_TO_POINTER(header_field));
}
 ...
 * @header_field: A 8-bit unsigned integer (typically a value from the
 * #GDBusMessageHeaderField enumeration)
 * @value: (nullable): A #GVariant to set the header field or %NULL to clear the
 * header field.
 *
                               ...
                               GVariant* value)
{
    ...
    {
        g_hash_table_insert(message->headers, GUINT_TO_POINTER(header_field),
                                     ...
                                     GUnixFDList* fd_list)
{
    ...
    if (fd_list != NULL)
    {
        ...
        g_dbus_message_set_num_unix_fds(message,
...
static guint get_type_fixed_size(const GVariantType* type)
{
...
static gboolean validate_headers(GDBusMessage* message, GError** error)
{
    ...
    switch (message->type)
    {
        ...
        case G_DBUS_MESSAGE_TYPE_METHOD_CALL:
            if (g_dbus_message_get_header(
                    message, G_DBUS_MESSAGE_HEADER_FIELD_PATH) == NULL ||
                g_dbus_message_get_header(
                    message, G_DBUS_MESSAGE_HEADER_FIELD_MEMBER) == NULL)
            {
                g_set_error_literal(error, G_IO_ERROR,
                                    G_IO_ERROR_INVALID_ARGUMENT,
                                    _("METHOD_CALL message: PATH or MEMBER "
        ...
        case G_DBUS_MESSAGE_TYPE_METHOD_RETURN:
            if (g_dbus_message_get_header(
                    message, G_DBUS_MESSAGE_HEADER_FIELD_REPLY_SERIAL) == NULL)
            {
                g_set_error_literal(error, G_IO_ERROR,
                                    G_IO_ERROR_INVALID_ARGUMENT,
                                    _("METHOD_RETURN message: REPLY_SERIAL "
                                      "header field is missing"));
                goto out;
            }
            break;
        ...
        case G_DBUS_MESSAGE_TYPE_ERROR:
            if (g_dbus_message_get_header(
                    message, G_DBUS_MESSAGE_HEADER_FIELD_ERROR_NAME) == NULL ||
                g_dbus_message_get_header(
                    message, G_DBUS_MESSAGE_HEADER_FIELD_REPLY_SERIAL) == NULL)
            {
                g_set_error_literal(error, G_IO_ERROR,
                                    G_IO_ERROR_INVALID_ARGUMENT,
                                    _("ERROR message: REPLY_SERIAL or "
                                      "ERROR_NAME header field is missing"));
                goto out;
            }
            break;
        ...
        case G_DBUS_MESSAGE_TYPE_SIGNAL:
            if (g_dbus_message_get_header(
                    message, G_DBUS_MESSAGE_HEADER_FIELD_PATH) == NULL ||
                g_dbus_message_get_header(
                    message, G_DBUS_MESSAGE_HEADER_FIELD_INTERFACE) == NULL ||
                g_dbus_message_get_header(
                    message, G_DBUS_MESSAGE_HEADER_FIELD_MEMBER) == NULL)
            {
                g_set_error_literal(error, G_IO_ERROR,
                                    G_IO_ERROR_INVALID_ARGUMENT,
                                    _("SIGNAL message: PATH, INTERFACE or "
                          ...
                          "/org/freedesktop/DBus/Local") == 0)
            {
                g_set_error_literal(
                    ...
                    _("SIGNAL message: The PATH header field is using the "
                      "reserved value /org/freedesktop/DBus/Local"));
                goto out;
            }
                          ...
                          "org.freedesktop.DBus.Local") == 0)
            {
                g_set_error_literal(
                    error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT,
                    _("SIGNAL message: The INTERFACE header field is using the "
                      "reserved value org.freedesktop.DBus.Local"));
                goto out;
            }
            break;
...
static gboolean ensure_input_padding(GMemoryBuffer* buf, gsize padding_size)
{
    gsize offset;
    gsize wanted_offset;
...
static const gchar* read_string(GMemoryBuffer* mbuf, gsize len, GError** error)
{
    gchar* str;
    const gchar* end_valid;

    if
        G_UNLIKELY(mbuf->pos + len >= mbuf->valid_len ||
                   mbuf->pos + len < mbuf->pos)
        {
            mbuf->pos = mbuf->valid_len;
    ...
    if
        G_UNLIKELY(mbuf->data[mbuf->pos + len] != '\0')
        {
    ...
    if
        G_UNLIKELY(!g_utf8_validate(str, -1, &end_valid))
        {
            gint offset;
            gchar* valid_str;
            ...
            g_set_error(
                error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT,
                _("Expected valid UTF-8 string but found invalid bytes at byte "
                  "offset %d (length of string is %d). "
                  "The valid UTF-8 string up until that point was “%s”"),
                offset, (gint)len, valid_str);
            g_free(valid_str);
            return NULL;
        }
...
static gconstpointer read_bytes(GMemoryBuffer* mbuf, gsize len, GError** error)
{
...
/* if just_align==TRUE, don't read a value, just align the input stream wrt
 * padding */

/* returns a non-floating GVariant! */
static GVariant* parse_value_from_blob(GMemoryBuffer* buf,
                                       const GVariantType* type,
                                       gboolean just_align, guint indent,
                                       GError** error)
{
    GVariant* ret;
    GError* local_error;
...
#ifdef DEBUG_SERIALIZER
    {
        ...
        g_print("%*s%s type %s from offset 0x%04x", indent, "",
                just_align ? "Aligning" : "Reading", s, (gint)buf->pos);
        g_free(s);
    }
#endif /* DEBUG_SERIALIZER */
    ...
    switch (type_string[0])
    {
        ...
        case 'a': /* G_VARIANT_TYPE_ARRAY */
            ...
            /* If we are only aligning for this array type, it is the child type
             * of another array, which is empty. So, we do not need to add
             * padding for this nonexistent array's elements: we only need to
             * align for this array itself (4 bytes). See
            ...
            if (!just_align)
            {
                ...
                if (array_len > (2 << 26))
                {
                    ...
                    g_set_error(
                        ...
                        g_dngettext(GETTEXT_PACKAGE,
                                    ...
                                    "Encountered array of length %u bytes. "
                                    "Maximum length is 2<<26 bytes (64 MiB).",
                                    array_len),
                        array_len);
                    goto fail;
                ...
                if (fixed_size != 0)
                {
                    ...
                    if (array_len % fixed_size != 0)
                    {
                        g_set_error(&local_error, G_IO_ERROR,
                                    G_IO_ERROR_INVALID_ARGUMENT,
                                    _("Encountered array of type “a%c”, "
                                      "expected to have a length a multiple "
                                      "of %u bytes, but found to be %u bytes "
                                      "in length"),
                                    g_variant_type_peek_string(element_type)[0],
                                    fixed_size, array_len);
                        goto fail;
                    }
                    ...
                    ret = g_variant_new_fixed_array(element_type, array_data,
                                                    array_len / fixed_size,
                                                    fixed_size);
                ...
                else
                {
                    ...
                    {
                        GVariant* item;
                        item = parse_value_from_blob(buf, element_type, TRUE,
                                                     indent + 2, NULL);
                        g_assert(item == NULL);
                    }
                    else
                    {
                        ...
                        while (offset < target)
                        {
                            GVariant* item;
                            item =
                                parse_value_from_blob(buf, element_type, FALSE,
                                                      indent + 2, &local_error);
                            if (item == NULL)
                            {
        ...
        default:
            if (g_variant_type_is_dict_entry(type))
            {
                ...
                if (!just_align)
                {
                    ...
                    key = parse_value_from_blob(buf, key_type, FALSE,
                                                indent + 2, &local_error);
                    if (key == NULL)
                        goto fail;
                    value_type = g_variant_type_value(type);
                    value = parse_value_from_blob(buf, value_type, FALSE,
                                                  indent + 2, &local_error);
                    if (value == NULL)
                    {
                        g_variant_unref(key);
            ...
            else if (g_variant_type_is_tuple(type))
            {
                ...
                if (!just_align)
                {
                    ...
                    {
                        GVariant* item;
                        item = parse_value_from_blob(buf, element_type, FALSE,
                                                     indent + 2, &local_error);
                        if (item == NULL)
                        {
                            g_variant_builder_clear(&builder);
            ...
            else if (g_variant_type_is_variant(type))
            {
                ...
                if (!just_align)
                {
                    ...
                    {
                        /* A D-Bus signature can contain zero or more complete
                         * types,
                         * but a GVariant has to be exactly one complete type.
                         */
                        g_set_error(&local_error, G_IO_ERROR,
                                    G_IO_ERROR_INVALID_ARGUMENT,
                                    _("Parsed value “%s” for variant is not a "
                                      "valid D-Bus signature"),
                                    sig);
                        goto fail;
                    }
                    variant_type = g_variant_type_new(sig);
                    value = parse_value_from_blob(buf, variant_type, FALSE,
                                                  indent + 2, &local_error);
            ...
            else
            {
                ...
                g_set_error(&local_error, G_IO_ERROR,
                            G_IO_ERROR_INVALID_ARGUMENT,
                            _("Error deserializing GVariant with type string "
                              "“%s” from the D-Bus wire format"),
                            s);
                g_free(s);
                goto fail;
            }
    ...
    if (ret != NULL)
    {
        if (is_leaf)
        {
            ...
            {
                s = g_strdup_printf("0x%02x '%c'", g_variant_get_byte(ret),
                                    g_variant_get_byte(ret));
            }
            else
            {
...
gssize g_dbus_message_bytes_needed(guchar* blob, gsize blob_len, GError** error)
{
    ...
    {
        g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT,
                    "Unable to determine message blob length - given blob is "
                    "malformed");
    }
    ...
    {
        g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT,
                    "Blob indicates that message exceeds maximum message "
                    "length (128MiB)");
        ret = -1;
    }
 ...
 * @capabilities: A #GDBusCapabilityFlags describing what protocol features are
 * supported.
 * @error: Return location for error or %NULL.
 *
 ...
 * Since: 2.26
 */
GDBusMessage* g_dbus_message_new_from_blob(guchar* blob, gsize blob_len,
                                           ...
                                           GError** error)
{
    ...
    switch (endianness)
    {
        ...
        default:
            g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT,
                        _("Invalid endianness value. Expected 0x6c (“l”) or "
                          "0x42 (“B”) but found value 0x%02x"),
                        endianness);
            goto out;
    }
...
#endif /* DEBUG_SERIALIZER */
    headers =
        parse_value_from_blob(&mbuf, G_VARIANT_TYPE("a{yv}"), FALSE, 2, error);
    if (headers == NULL)
        goto out;
    ...
    signature = g_dbus_message_get_header(
        message, G_DBUS_MESSAGE_HEADER_FIELD_SIGNATURE);
    if (signature != NULL)
    {
        ...
        {
            g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT,
                        _("Signature header with signature “%s” found but "
        ...
        else if (signature_str_len > 0)
        {
            ...
            {
                g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT,
                            _("Parsed value “%s” is not a valid D-Bus "
                              "signature (for body)"),
                            signature_str);
...
#endif /* DEBUG_SERIALIZER */
            message->body =
                parse_value_from_blob(&mbuf, variant_type, FALSE, 2, error);
            g_variant_type_free(variant_type);
            if (message->body == NULL)
                goto out;
        }
    ...
    else
    {
        ...
        if (message_body_len != 0)
        {
            ...
            g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT,
                        g_dngettext(GETTEXT_PACKAGE,
                                    "No signature header in message but the "
                                    "message body is %u byte",
                                    "No signature header in message but the "
                                    "message body is %u bytes",
                                    message_body_len),
                        message_body_len);
            goto out;
        }
...
static gsize ensure_output_padding(GMemoryBuffer* mbuf, gsize padding_size)
{
    gsize offset;
    gsize wanted_offset;
...
static gboolean append_value_to_blob(GVariant* value, const GVariantType* type,
                                     GMemoryBuffer* mbuf,
                                     gsize* out_padding_added, GError** error)
{
    gsize padding_added;
    const gchar* type_string;
    ...
    switch (type_string[0])
    {
        ...
        case 'a': /* G_VARIANT_TYPE_ARRAY */
        {
            ...
            if (value != NULL)
            {
                 ...
                 *    the array element type, followed by each array element.
                 * The array length is from the end of the alignment padding to
                ...
                if (g_variant_n_children(value) == 0)
                {
                    ...
                    if (!append_value_to_blob(NULL, element_type, mbuf,
                                              &padding_added_for_item, error))
                        goto fail;
                ...
                else if (fixed_size != 0)
                {
                    ...
                    array_payload_begin_offset +=
                        ensure_output_padding(mbuf, fixed_size);

                    array_len = g_variant_get_size(use_value);
                    g_memory_buffer_write(mbuf, g_variant_get_data(use_value),
                                          array_len);
                ...
                else
                {
                    ...
                    {
                        gsize padding_added_for_item;
                        if (!append_value_to_blob(
                                item, g_variant_get_type(item), mbuf,
                                &padding_added_for_item, error))
                        {
                            g_variant_unref(item);
                            goto fail;
        ...
        default:
            ...
            else if (g_variant_type_is_variant(type))
            {
                if (value != NULL)
                {
                    ...
                    g_memory_buffer_put_byte(mbuf, '\0');
                    if (!append_value_to_blob(child, g_variant_get_type(child),
                                              mbuf, NULL, error))
                    {
            ...
            {
                g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT,
                            _("Error serializing GVariant with type string "
                              "“%s” to the D-Bus wire format"),
...
static gboolean append_body_to_blob(GVariant* value, GMemoryBuffer* mbuf,
                                    GError** error)
{
    GVariant* item;
    GVariantIter iter;
    ...
    while ((item = g_variant_iter_next_value(&iter)) != NULL)
    {
        if (!append_value_to_blob(item, g_variant_get_type(item), mbuf, NULL,
                                  error))
        {
            g_variant_unref(item);
            goto fail;
 ...
 * Since: 2.26
 */
guchar* g_dbus_message_to_blob(GDBusMessage* message, gsize* out_size,
                               GDBusCapabilityFlags capabilities,
                               GError** error)
{
    GMemoryBuffer mbuf;
    ...
    {
        g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT,
                    _("Number of file descriptors in message (%d) differs from "
                      "header field (%d)"),
                    num_fds_in_message, num_fds_according_to_header);
        goto out;
    }
    ...
    {
        g_variant_builder_add(&builder, "{yv}", (guchar)GPOINTER_TO_UINT(key),
                              header_value);
    }
    header_fields = g_variant_builder_end(&builder);

    if (!append_value_to_blob(header_fields, g_variant_get_type(header_fields),
                              &mbuf, NULL, error))
    {
    ...
    if (message->body != NULL)
    {
        ...
        if (g_strcmp0(tupled_signature_str,
                      g_variant_get_type_string(message->body)) != 0)
        {
            g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT,
                        _("Message body has type signature “%s” but signature "
                          "in the header field is “%s”"),
    ...
    else
    {
        ...
        {
            g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT,
                        _("Message body is empty but signature in the header "
                          "field is “(%s)”"),
                        signature_str);
            goto out;
        }
...
static guint32 get_uint32_header(GDBusMessage* message,
                                 GDBusMessageHeaderField header_field)
{
    GVariant* value;
    guint32 ret;
    ...
    ret = 0;
    value =
        g_hash_table_lookup(message->headers, GUINT_TO_POINTER(header_field));
...
static const gchar* get_string_header(GDBusMessage* message,
                                      GDBusMessageHeaderField header_field)
{
    GVariant* value;
    const gchar* ret;
...
static const gchar* get_object_path_header(GDBusMessage* message,
                                           GDBusMessageHeaderField header_field)
{
    GVariant* value;
    const gchar* ret;
...
static const gchar* get_signature_header(GDBusMessage* message,
                                         GDBusMessageHeaderField header_field)
{
    GVariant* value;
    const gchar* ret;
...
static void set_uint32_header(GDBusMessage* message,
                              GDBusMessageHeaderField header_field,
                              guint32 value)
{
    g_dbus_message_set_header(message, header_field,
                              g_variant_new_uint32(value));
}

static void set_string_header(GDBusMessage* message,
                              GDBusMessageHeaderField header_field,
                              const gchar* value)
{
    g_dbus_message_set_header(message, header_field,
...
static void set_object_path_header(GDBusMessage* message,
                                   GDBusMessageHeaderField header_field,
                                   const gchar* value)
{
    g_dbus_message_set_header(message, header_field,
...
static void set_signature_header(GDBusMessage* message,
                                 GDBusMessageHeaderField header_field,
                                 const gchar* value)
{
    g_dbus_message_set_header(message, header_field,
 ...
 * @message: A #GDBusMessage.
 *
 * Convenience getter for the %G_DBUS_MESSAGE_HEADER_FIELD_REPLY_SERIAL header
 * field.
 *
 * Returns: The value.
 *
 ...
 *
 * Convenience setter for the %G_DBUS_MESSAGE_HEADER_FIELD_REPLY_SERIAL header
 * field.
 *
 * Since: 2.26
 */
 ...
 * @message: A #GDBusMessage.
 *
 * Convenience getter for the %G_DBUS_MESSAGE_HEADER_FIELD_NUM_UNIX_FDS header
 * field.
 *
 * Returns: The value.
 *
 ...
 * @value: The value to set.
 *
 * Convenience setter for the %G_DBUS_MESSAGE_HEADER_FIELD_NUM_UNIX_FDS header
 * field.
 *
 * Since: 2.26
 */
...
gboolean g_dbus_message_to_gerror(GDBusMessage* message, GError** error)
{
    ...
    if (error_name != NULL)
    {
        ...
        else
        {
            ...
            {
                g_dbus_error_set_dbus_error(error, error_name, "",
    ...
    {
        /* TOOD: this shouldn't happen - should check this at message
         * serialization time and disconnect the peer.
         */
        ...
        if (num_fds > 0)
        {
            for (n = 0; n < num_fds; n++)
            {
                ...
                if (fstat(fds[n], &statbuf) == 0)
                {
                    ...
                    g_string_append_printf(fs,
                                           "%s"
                                           "dev=%d:%d",
                                           fs->len > 0 ? "," : "",
                                           (gint)major(statbuf.st_dev),
                                           (gint)minor(statbuf.st_dev));
#endif
                    g_string_append_printf(fs,
                                           "%s"
                                           "mode=0%o",
                ...
                {
                    int errsv = errno;
                    g_string_append_printf(fs, "(fstat failed: %s)",
                                           g_strerror(errsv));
                }
...
GDBusMessage* g_dbus_message_copy(GDBusMessage* message, GError** error)
{
    ...
    /* see https://bugzilla.gnome.org/show_bug.cgi?id=624546#c8 for why it's
     * fine to just ref (as opposed to deep-copying) the GVariant instances
     */
    ...
    while (g_hash_table_iter_next(&iter, &header_key, (gpointer)&header_value))
        g_hash_table_insert(ret->headers, header_key,
                            g_variant_ref(header_value));
