...
static rfbBool MallocFrameBuffer(rfbClient* client)
{
    ...
    /* SECURITY: promote 'width' into uint64_t so that the multiplication does
       not overflow 'width' and 'height' are 16-bit integers per RFB protocol
       design SIZE_MAX is the maximum value that can fit into size_t
    */
    ...
    if (client->frameBuffer == NULL)
        rfbClientErr("CRITICAL: frameBuffer allocation failed, requested size "
...
static void FillRectangle(rfbClient* client, int x, int y, int w, int h,
                          uint32_t colour)
{
    int i, j;
...
static void CopyRectangle(rfbClient* client, const uint8_t* buffer, int x,
                          int y, int w, int h)
{
    int j;
...
/* TODO: test */
static void CopyRectangleFromRectangle(rfbClient* client, int src_x, int src_y,
...
#define COPY_RECT_FROM_RECT(BPP)                                               \
    {                                                                          \
        uint##BPP##_t* _buffer = ((uint##BPP##_t*)client->frameBuffer) +       \
                                 (src_y - dest_y) * client->width + src_x -    \
                                 dest_x;                                       \
        if (dest_y < src_y)                                                    \
        {                                                                      \
            for (j = dest_y * client->width; j < (dest_y + h) * client->width; \
                 j += client->width)                                           \
            {                                                                  \
                if (dest_x < src_x)                                            \
                {                                                              \
                    ...
                    {                                                          \
                        ((uint##BPP##_t*)client->frameBuffer)[j + i] =         \
                            _buffer[j + i];                                    \
                    }                                                          \
                }                                                              \
                else                                                           \
        ...
        {                                                                      \
            for (j = (dest_y + h - 1) * client->width;                         \
                 j >= dest_y * client->width; j -= client->width)              \
            {                                                                  \
                if (dest_x < src_x)                                            \
                {                                                              \
                    for (i = dest_x; i < dest_x + w; i++)                      \
...
static rfbBool rfbInitConnection(rfbClient* client)
{
    ...
    if (!client->listenSpecified)
    {
        ...
        if (client->destHost)
        {
            if (!ConnectToRFBRepeater(client, client->serverHost,
                                      client->serverPort, client->destHost,
                                      client->destPort))
                return FALSE;
        }
        else
        {
            if (!ConnectToRFBServer(client, client->serverHost,
                                    client->serverPort))
                return FALSE;
        }
    }
    ...
    if (client->appData.scaleSetting > 1)
    {
        ...
        if (!SendFramebufferUpdateRequest(
                client, client->updateRect.x / client->appData.scaleSetting,
                client->updateRect.y / client->appData.scaleSetting,
                client->updateRect.w / client->appData.scaleSetting,
                client->updateRect.h / client->appData.scaleSetting, FALSE))
            return FALSE;
    }
    else
    {
