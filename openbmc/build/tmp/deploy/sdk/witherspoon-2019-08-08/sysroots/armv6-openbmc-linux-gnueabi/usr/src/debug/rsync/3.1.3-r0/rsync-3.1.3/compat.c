...
static void check_sub_protocol(void)
{
    ...
    if (!(their_protocol = atoi(client_info)) ||
        !(dot = strchr(client_info, '.')) || !(their_sub = atoi(dot + 1)))
    {
#if SUBPROTOCOL_VERSION != 0
        if (our_sub)
            protocol_version--;
...
void set_allow_inc_recurse(void)
{
    ...
    if (!recurse || use_qsort)
        allow_inc_recurse = 0;
    else if (!am_sender && (delete_before || delete_after || delay_updates ||
                            prune_empty_dirs))
        allow_inc_recurse = 0;
    else if (am_server && !local_server && (strchr(client_info, 'i') == NULL))
        allow_inc_recurse = 0;
}
...
void setup_protocol(int f_out, int f_in)
{
    ...
    else if (protocol_version >= 30)
    {
        ...
        want_xattr_optim =
            protocol_version >= 31 && !(compat_flags & CF_AVOID_XATTR_OPTIM);
        ...
        sender_symlink_iconv =
            iconv_opt &&
            (am_server ? local_server || strchr(client_info, 's') != NULL
                       : !!(compat_flags & CF_SYMLINK_ICONV));
#endif
        ...
        use_safe_inc_flist =
            (compat_flags & CF_SAFE_FLIST) || protocol_version >= 31;
