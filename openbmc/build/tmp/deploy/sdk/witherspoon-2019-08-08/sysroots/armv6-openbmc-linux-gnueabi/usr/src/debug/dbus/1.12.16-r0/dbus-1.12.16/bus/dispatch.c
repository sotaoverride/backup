...
static dbus_bool_t send_one_message(DBusConnection* connection,
                                    BusContext* context, DBusConnection* sender,
                                    ...
                                    DBusError* error)
{
    ...
    if (!bus_context_check_security_policy(context, transaction, sender,
                                           addressed_recipient, connection,
                                           message, NULL, &stack_error))
    {
                                 ...
                                 DBusMessage* message, DBusError* error)
{
    ...
    if (addressed_recipient != NULL)
    {
        if (!bus_context_check_security_policy(
                context, transaction, sender, addressed_recipient,
                addressed_recipient, message, NULL, error))
            return FALSE;
    ...
    recipients = NULL;
    if (!bus_matchmaker_get_recipients(matchmaker, connections, sender,
...
static DBusHandlerResult bus_dispatch(DBusConnection* connection,
                                      DBusMessage* message)
{
    ...
    if (bus_connection_is_monitor(connection))
    {
        ...
        else
        {
             ...
             */
            bus_context_log(
                context, DBUS_SYSTEM_LOG_WARNING,
        ...
        strcmp(service_name, DBUS_SERVICE_DBUS) == 0) /* to bus driver */
    {
        ...
        if (!bus_context_check_security_policy(context, transaction, connection,
    ...
    else if (!bus_connection_is_active(
                 connection)) /* clients must talk to bus driver first */
    {
        if (!bus_transaction_capture(transaction, connection, NULL, message))
        {
            BUS_SET_OOM(&error);
        ...
        _dbus_verbose(
            "Received message from non-registered client. Disconnecting.\n");
        dbus_connection_close(connection);
        goto out;
    }
    else if (service_name != NULL) /* route to named service */
    {
        ...
        if (service == NULL && dbus_message_get_auto_start(message))
        {
             ...
             */
            if (!bus_activation_activate_service(activation, connection,
                                                 transaction, TRUE, message,
                                                 service_name, &error))
            {
        ...
        else if (service == NULL)
        {
            ...
            dbus_set_error(&error, DBUS_ERROR_NAME_HAS_NO_OWNER,
                           "Name \"%s\" does not exist", service_name);
            goto out;
        }
        else
        {
...
static DBusHandlerResult bus_dispatch_message_filter(DBusConnection* connection,
                                                     DBusMessage* message,
                                                     void* user_data)
{
...
{
    if (!dbus_connection_add_filter(connection, bus_dispatch_message_filter,
                                    NULL, NULL))
        return FALSE;
...
void bus_dispatch_remove_connection(DBusConnection* connection)
{
    ...
    dbus_connection_remove_filter(connection, bus_dispatch_message_filter,
                                  NULL);
}
 ...
 */
#define SEND_PENDING(connection)                                               \
...
static void spin_connection_until_authenticated(BusContext* context,
                                                DBusConnection* connection)
{
...
static void warn_unexpected_real(DBusConnection* connection,
                                 DBusMessage* message, const char* expected,
                                 const char* function, int line)
{
    if (message)
        _dbus_warn("%s:%d received message interface \"%s\" member \"%s\" "
                   "error name \"%s\" on %p, expecting %s",
                   function, line,
        ...
        _dbus_warn("%s:%d received no message on %p, expecting %s", function,
                   line, connection, expected);
}

#define warn_unexpected(connection, message, expected)                         \
    warn_unexpected_real(connection, message, expected, _DBUS_FUNCTION_NAME,   \
                         __LINE__)

static void verbose_message_received(DBusConnection* connection,
                                     DBusMessage* message)
{
    _dbus_verbose("Received message interface \"%s\" member \"%s\" error name "
                  "\"%s\" on %p\n",
                  dbus_message_get_interface(message)
                      ? dbus_message_get_interface(message)
                      : "(unset)",
...
static dbus_bool_t
    check_service_owner_changed_foreach(DBusConnection* connection, void* data)
{
    ...
    if (d->expected_kind == SERVICE_CREATED && connection == d->skip_connection)
        return TRUE;
    ...
    if (message == NULL)
    {
        block_connection_until_message_from_bus(d->context, connection,
                                                "NameOwnerChanged");
        message = pop_message_waiting_for_memory(connection);
        if (message == NULL)
        {
    ...
    reget_service_info_data:
        ...
        dbus_message_get_args(message, &error, DBUS_TYPE_STRING, &service_name,
        ...
        if ((d->expected_kind == SERVICE_CREATED &&
            ...
            (d->expected_kind == OWNER_CHANGED &&
             (!old_owner[0] || !new_owner[0])) ||
            (d->expected_kind == SERVICE_DELETED &&
             (!old_owner[0] || new_owner[0])))
        {
        ...
        if (strcmp(service_name, d->expected_service_name) != 0)
        {
            _dbus_warn("expected info on service %s, got info on %s",
                       d->expected_service_name, service_name);
            goto out;
        }
        ...
        {
            _dbus_warn("inconsistent ServiceOwnedChanged message (\"%s\" [ %s "
                       "-> %s ])",
...
static void kill_client_connection(BusContext* context,
                                   DBusConnection* connection)
{
    char* base_service;
    const char* s;
    ...
    if (bus_connection_dispatch_one_message(connection))
        _dbus_assert_not_reached("something received on connection being "
                                 "killed other than the disconnect");
    ...
    if (socd.failed)
        _dbus_assert_not_reached(
            "didn't get the expected NameOwnerChanged (deletion) messages");

    if (!check_no_leftovers(context))
        _dbus_assert_not_reached(
            "stuff left in message queues after disconnecting a client");
}
...
static void kill_client_connection_unchecked(DBusConnection* connection)
{
    ...
    if (bus_connection_dispatch_one_message(connection))
        _dbus_assert_not_reached("message other than disconnect dispatched "
...
static dbus_bool_t check_no_messages_foreach(DBusConnection* connection,
                                             void* data)
{
 ...
 */
static dbus_bool_t check_hello_message(BusContext* context,
                                       DBusConnection* connection)
{
    DBusMessage* message;
    ...
    if (message == NULL)
    {
        _dbus_warn("Did not receive a reply to %s %d on %p", "Hello", serial,
                   connection);
        goto out;
    }
    ...
    retry_get_hello_name:
                                   ...
                                   DBUS_TYPE_INVALID))
        {
            ...
            else
            {
                ...
                _dbus_warn(
                    "Did not get the expected single string argument to hello");
                goto out;
            }
        }
        ...
        socd.failed = FALSE;
        socd.skip_connection =
            connection; /* we haven't done AddMatch so won't get it ourselves */
        socd.context = context;
    ...
    retry_get_acquired_name:
        if (!dbus_message_get_args(message, &error, DBUS_TYPE_STRING, &acquired,
                                   DBUS_TYPE_INVALID))
        {
            if (dbus_error_has_name(&error, DBUS_ERROR_NO_MEMORY))
            {
                _dbus_verbose(
                    "no memory to get service name arg from acquired\n");
                dbus_error_free(&error);
                _dbus_wait_for_memory();
            ...
            {
                _dbus_assert(dbus_error_is_set(&error));
                _dbus_warn("Did not get the expected single string argument to "
                           "ServiceAcquired");
                goto out;
            }
        }
 ...
 * but the correct thing may include OOM errors.
 */
static dbus_bool_t check_double_hello_message(BusContext* context,
                                              DBusConnection* connection)
{
    DBusMessage* message;
    dbus_uint32_t serial;
 ...
 * but the correct thing may include OOM errors.
 */
static dbus_bool_t check_get_connection_unix_user(BusContext* context,
                                                  DBusConnection* connection)
{
    DBusMessage* message;
    dbus_uint32_t serial;
    ...
    if (!dbus_message_append_args(message, DBUS_TYPE_STRING, &base_service_name,
                                  DBUS_TYPE_INVALID))
    {
        dbus_message_unref(message);
        return TRUE;
    ...
    dbus_connection_ref(connection); /* because we may get disconnected */
    block_connection_until_message_from_bus(context, connection,
                                            "reply to GetConnectionUnixUser");
    ...
    retry_get_property:
                                   ...
                                   DBUS_TYPE_INVALID))
        {
            ...
            {
                _dbus_verbose(
                    "no memory to get uid by GetConnectionUnixUser\n");
                dbus_error_free(&error);
                _dbus_wait_for_memory();
            ...
            {
                _dbus_assert(dbus_error_is_set(&error));
                _dbus_warn("Did not get the expected DBUS_TYPE_UINT32 from "
                           "GetConnectionUnixUser");
                goto out;
            }
        }
                                         ...
                                         DBusConnection* connection)
{
    ...
    block_connection_until_message_from_bus(
        context, connection, "reply to GetConnectionUnixProcessID");
    ...
    if (dbus_message_get_type(message) == DBUS_MESSAGE_TYPE_ERROR)
    {
        ...
        {
            /* We are expecting this error, since we know in the test suite we
             * aren't talking to a client running on UNIX
             */
            _dbus_verbose("Windows correctly does not support "
                          "GetConnectionUnixProcessID\n");
        }
#endif
        else
    ...
    defined(__linux__) || defined(__OpenBSD__)
            ...
            goto out;
#else
            _dbus_verbose("does not support GetConnectionUnixProcessID but "
                          "perhaps that's OK?\n");
#endif
        }
    }
...
#ifdef DBUS_WIN
        warn_unexpected(connection, message,
                        "GetConnectionUnixProcessID to fail on Windows");
        goto out;
#else
        if (dbus_message_get_type(message) == DBUS_MESSAGE_TYPE_METHOD_RETURN)
        {
    ...
    retry_get_property:
        ...
        if (!dbus_message_get_args(message, &error, DBUS_TYPE_UINT32, &pid,
                                   DBUS_TYPE_INVALID))
        {
            if (dbus_error_has_name(&error, DBUS_ERROR_NO_MEMORY))
            {
        ...
        else
        {
            ...
            {
                _dbus_assert(dbus_error_is_set(&error));
                _dbus_warn("Result from GetConnectionUnixProcessID is not our "
                           "own pid");
                goto out;
            }
        }
 ...
 * but the correct thing may include OOM errors.
 */
static dbus_bool_t check_add_match(BusContext* context,
                                   DBusConnection* connection, const char* rule)
{
    DBusMessage* message;
    dbus_bool_t retval;
    dbus_uint32_t serial;
    ...
    if (message == NULL)
    {
        _dbus_warn("Did not receive a reply to %s %d on %p", "AddMatch", serial,
                   connection);
        goto out;
    }
 ...
 * but the correct thing may include OOM errors.
 */
static dbus_bool_t check_get_all_match_rules(BusContext* context,
                                             DBusConnection* connection)
{
    DBusMessage* message;
    dbus_bool_t retval;
    ...
    message =
        dbus_message_new_method_call(DBUS_SERVICE_DBUS, DBUS_PATH_DBUS,
                                     BUS_INTERFACE_STATS, "GetAllMatchRules");
...
#define NONEXISTENT_SERVICE_NAME                                               \
    "test.this.service.does.not.exist.ewuoiurjdfxcvn"
                                            ...
                                            DBusConnection* connection)
{
    ...
    flags = 0;
    if (!dbus_message_append_args(message, DBUS_TYPE_STRING, &nonexistent,
    ...
    bus_test_run_everything(context);
    block_connection_until_message_from_bus(
        context, connection, "reply to ActivateService on nonexistent");
    bus_test_run_everything(context);
                                         ...
                                         DBusConnection* connection)
{
    ...
    message = dbus_message_new_method_call(NONEXISTENT_SERVICE_NAME,
                                           ...
                                           "org.freedesktop.TestSuite", "Echo");
...
static dbus_bool_t check_base_service_activated(BusContext* context,
                                                DBusConnection* connection,
                                                ...
                                                const char** base_service_p)
{
    ...
    reget_service_name_arg:
        ...
        if (!dbus_message_get_args(message, &error, DBUS_TYPE_STRING,
                                   &base_service, DBUS_TYPE_STRING, &old_owner,
                                   ...
                                   DBUS_TYPE_INVALID))
        {
            ...
            else
            {
                _dbus_warn("Message %s doesn't have a service name: %s",
                           "NameOwnerChanged (creation)", error.message);
                goto out;
            }
        }
        ...
        if (old_owner[0])
        {
            _dbus_warn("Received an old_owner argument during base service "
                       "activation, \"%s\"",
                       old_owner);
            goto out;
        }
    ...
    {
        warn_unexpected(connection, message,
                        "NameOwnerChanged (creation) for base service");
...
static dbus_bool_t check_service_activated(BusContext* context,
                                           DBusConnection* connection,
                                           ...
                                           DBusMessage* initial_message)
{
    ...
    reget_service_name_arg:
        ...
        {
            _dbus_warn("NameOwnerChanged reports wrong base service: %s owner, "
                       "expected %s instead",
        ...
        if (old_owner[0])
        {
            _dbus_warn("expected a %s, got a %s", "NameOwnerChanged (creation)",
    ...
    {
        warn_unexpected(connection, message,
                        "NameOwnerChanged for the activated name");
    ...
    if (!dbus_message_get_args(message, &error, DBUS_TYPE_UINT32,
                               &activation_result, DBUS_TYPE_INVALID))
    {
        if (!dbus_error_has_name(&error, DBUS_ERROR_NO_MEMORY))
        {
            _dbus_warn(
                "Did not have activation result first argument to %s: %s",
    ...
    if (!check_no_leftovers(context))
    {
        _dbus_warn("Messages were left over after verifying existent "
                   "activation results");
        goto out;
    }
...
static dbus_bool_t check_service_auto_activated(BusContext* context,
                                                DBusConnection* connection,
                                                const char* activated_name,
                                                const char* base_service_name,
                                                DBusMessage* initial_message)
...
static dbus_bool_t check_service_deactivated(BusContext* context,
                                             DBusConnection* connection,
                                             const char* activated_name,
...
static dbus_bool_t check_send_exit_to_service(BusContext* context,
                                              DBusConnection* connection,
                                              const char* service_name,
                                              const char* base_service)
{
    ...
    message =
        dbus_message_new_method_call(service_name, "/org/freedesktop/TestSuite",
                                     "org.freedesktop.TestSuite", "Exit");

    if (message == NULL)
    {
        /* Do this again; we still need the service to exit... */
        if (!check_send_exit_to_service(context, connection, service_name,
                                        base_service))
            goto out;
    ...
    {
        if (!check_service_deactivated(context, connection, service_name,
                                       base_service))
            goto out;
...
static dbus_bool_t check_got_error(BusContext* context,
                                   DBusConnection* connection,
                                   const char* first_error_name, ...)
{
    ...
    if (!error_found)
    {
        _dbus_warn("Expected error %s or other, got %s instead",
                   first_error_name, dbus_message_get_error_name(message));
        goto out;
    }
                                         ...
                                         DBusConnection* connection)
{
    ...
    flags = 0;
    if (!dbus_message_append_args(message, DBUS_TYPE_STRING, &existent,
                                  DBUS_TYPE_UINT32, &flags, DBUS_TYPE_INVALID))
    {
        dbus_message_unref(message);
        return TRUE;
    ...
    else
    {
        ...
        if (!check_base_service_activated(context, connection, message,
                                          &base_service))
            goto out;
         ...
         */
        block_connection_until_message_from_bus(
            context, connection, "test service to exit or finish up");

        message = dbus_connection_borrow_message(connection);
        if (message == NULL)
        {
            _dbus_warn("Did not receive any messages after base service "
        ...
        switch (message_kind)
        {
            ...
            case GOT_ERROR:
                if (!check_got_error(context, connection,
                    ...
                    goto out;
                /* A service deleted should be coming along now after this
                 * error. We can also get the error *after* the service deleted.
                 */
            ...
            case GOT_SERVICE_DELETED:
            {
                ...
                {
                    block_connection_until_message_from_bus(
                        context, connection, "error about service exiting");
                                         ...
                                         DBusConnection* connection)
{
    ...
    flags = 0;
    if (!dbus_message_append_args(message, DBUS_TYPE_STRING, &segv_service,
                                  DBUS_TYPE_UINT32, &flags, DBUS_TYPE_INVALID))
    {
        dbus_message_unref(message);
        return TRUE;
    ...
    bus_test_run_everything(context);
    block_connection_until_message_from_bus(
        context, connection, "reply to activating segfault service");
    bus_test_run_everything(context);
 ...
 * but the correct thing may include OOM errors.
 */
static dbus_bool_t check_segfault_service_auto_start(BusContext* context,
                                                     DBusConnection* connection)
{
    DBusMessage* message;
    dbus_uint32_t serial;
    ...
    message = dbus_message_new_method_call(
        ...
        "/org/freedesktop/TestSuite", "org.freedesktop.TestSuite", "Echo");
    ...
    bus_test_run_everything(context);
    block_connection_until_message_from_bus(
        context, connection, "reply to Echo on segfault service");
    bus_test_run_everything(context);
 ...
 * but the correct thing may include OOM errors.
 */
static dbus_bool_t check_existent_hello_from_self(BusContext* context,
                                                  DBusConnection* connection)
{
    DBusMessage* message;
    dbus_uint32_t serial;
    ...
    message = dbus_message_new_method_call(
        EXISTENT_SERVICE_NAME, "/org/freedesktop/TestSuite",
        "org.freedesktop.TestSuite", "RunHelloFromSelf");

    if (message == NULL)
        return TRUE;

    text = TEST_RUN_HELLO_FROM_SELF_MESSAGE;
    if (!dbus_message_append_args(message, DBUS_TYPE_STRING, &text,
                                  DBUS_TYPE_INVALID))
    {
        dbus_message_unref(message);
        return TRUE;
     ...
     */
    block_connection_until_message_from_bus(
        context, connection, "reply from running hello from self");

    message = pop_message_waiting_for_memory(connection);
    if (message == NULL)
    {
        _dbus_warn("Failed to pop message! Should have been reply from "
                   "RunHelloFromSelf message");
        return FALSE;
    }
 ...
 * but the correct thing may include OOM errors.
 */
static dbus_bool_t check_existent_ping(BusContext* context,
                                       DBusConnection* connection)
{
    DBusMessage* message;
    dbus_uint32_t serial;
     ...
     */
    block_connection_until_message_from_bus(context, connection,
                                            "reply from running Ping");

    message = pop_message_waiting_for_memory(connection);
    if (message == NULL)
    {
        _dbus_warn(
            "Failed to pop message! Should have been reply from Ping message");
        return FALSE;
    }
 ...
 * but the correct thing may include OOM errors.
 */
static dbus_bool_t check_existent_get_machine_id(BusContext* context,
                                                 DBusConnection* connection)
{
     ...
     */
    block_connection_until_message_from_bus(context, connection,
                                            "reply from running GetMachineId");

    message = pop_message_waiting_for_memory(connection);
    if (message == NULL)
    {
        _dbus_warn("Failed to pop message! Should have been reply from "
                   "GetMachineId message");
        return FALSE;
    }
    ...
    machine_id = NULL;
    if (!dbus_message_get_args(message, NULL, DBUS_TYPE_STRING, &machine_id,
                               DBUS_TYPE_INVALID))
    {
    ...
    /* We can't check that the machine id is correct because during make check
     * it is just made up for each process separately
     */
 ...
 * but the correct thing may include OOM errors.
 */
static dbus_bool_t check_existent_service_auto_start(BusContext* context,
                                                     DBusConnection* connection)
{
    DBusMessage* message;
    DBusMessage* base_service_message;
     ...
     */
    block_connection_until_message_from_bus(
        context, connection, "reply to Echo on existent service");
    bus_test_run_everything(context);
    ...
    if (dbus_message_get_type(message) == DBUS_MESSAGE_TYPE_SIGNAL)
    {
        ...
        switch (message_kind)
        {
            case GOT_SERVICE_CREATED:
                ...
                /* Check that ServiceOwnerChanged (creation) was correctly
                 * received */
                if (!check_service_auto_activated(context, connection,
                                                  EXISTENT_SERVICE_NAME,
                                                  base_service, message))
                    goto out;
     ...
     */
    block_connection_until_message_from_bus(
        context, connection, "reply from echo message after auto-activation");
    ...
    if (!check_send_exit_to_service(context, connection, EXISTENT_SERVICE_NAME,
                                    base_service))
        goto out;
...
#define SERVICE_FILE_MISSING_NAME                                              \
    "org.freedesktop.DBus.TestSuiteEchoServiceDotServiceFileDoesNotExist"

/* returns TRUE if the correct thing happens,
 * but the correct thing may include OOM errors.
 */
static dbus_bool_t check_launch_service_file_missing(BusContext* context,
                                                     DBusConnection* connection)
{
    DBusMessage* message;
    dbus_uint32_t serial;
    ...
    block_connection_until_message_from_bus(
        context, connection,
        "reply to service file missing should fail to auto-start");
    bus_test_run_everything(context);
 ...
 * but the correct thing may include OOM errors.
 */
static dbus_bool_t check_launch_service_user_missing(BusContext* context,
                                                     DBusConnection* connection)
{
    DBusMessage* message;
    dbus_uint32_t serial;
 ...
 * but the correct thing may include OOM errors.
 */
static dbus_bool_t check_launch_service_exec_missing(BusContext* context,
                                                     DBusConnection* connection)
{
    DBusMessage* message;
    dbus_uint32_t serial;
    ...
    if (dbus_message_get_type(message) == DBUS_MESSAGE_TYPE_ERROR)
    {
        ...
        {
            _dbus_verbose(
                "could not activate as invalid service file was not added\n");
...
#define SHELL_FAIL_SERVICE_NAME                                                \
    "org.freedesktop.DBus.TestSuiteShellEchoServiceFail"
                                        ...
                                        DBusConnection* connection)
{
    ...
    block_connection_until_message_from_bus(
        context, connection,
        "reply to shell Echo on service which should fail to auto-start");
    bus_test_run_everything(context);
    ...
    {
        _dbus_warn(
            "Did not expect to successfully auto-start shell fail service");
        goto out;
    }
...
#define SHELL_SUCCESS_SERVICE_NAME                                             \
    "org.freedesktop.DBus.TestSuiteShellEchoServiceSuccess"
                                           ...
                                           DBusConnection* connection)
{
     ...
     */
    block_connection_until_message_from_bus(
        context, connection, "reply to Echo on shell success service");
    bus_test_run_everything(context);
    ...
    if (!dbus_message_get_args(
            message, NULL, DBUS_TYPE_STRING, &argv[0], DBUS_TYPE_STRING,
            &argv[1], DBUS_TYPE_STRING, &argv[2], DBUS_TYPE_STRING, &argv[3],
            DBUS_TYPE_STRING, &argv[4], DBUS_TYPE_STRING, &argv[5],
            DBUS_TYPE_STRING, &argv[6], DBUS_TYPE_INVALID))
    {
    ...
    */
    ...
    {
        _dbus_warn("Unexpected argv[1] in shell success service test "
                   "(expected: %s, got: %s)",
                   "-test", argv[1]);
        goto out;
    }
    ...
    {
        _dbus_warn("Unexpected argv[2] in shell success service test "
                   "(expected: %s, got: %s)",
                   "that", argv[2]);
        goto out;
    }
    ...
    {
        _dbus_warn("Unexpected argv[3] in shell success service test "
                   "(expected: %s, got: %s)",
                   "we get", argv[3]);
        goto out;
    }
    ...
    {
        _dbus_warn("Unexpected argv[4] in shell success service test "
                   "(expected: %s, got: %s)",
                   "back", argv[4]);
        goto out;
    }
    ...
    {
        _dbus_warn("Unexpected argv[5] in shell success service test "
                   "(expected: %s, got: %s)",
                   "--what", argv[5]);
        goto out;
    }
    ...
    {
        _dbus_warn("Unexpected argv[6] in shell success service test "
                   "(expected: %s, got: %s)",
                   "we put in", argv[6]);
        goto out;
    }
    ...
    if (!check_send_exit_to_service(context, connection,
                                    SHELL_SUCCESS_SERVICE_NAME, base_service))
        goto out;
...
static void check1_try_iterations(BusContext* context, const char* description,
                                  Check1Func func)
{
    Check1Data d;
    ...
    if (!_dbus_test_oom_handling(description, check_oom_check1_func, &d))
        _dbus_assert_not_reached("test failed");
}

static dbus_bool_t check_get_services(BusContext* context,
                                      DBusConnection* connection,
                                      const char* method, char*** services,
                                      int* len)
{
    DBusMessage* message;
    dbus_uint32_t serial;
    ...
    dbus_connection_ref(connection); /* because we may get disconnected */
    block_connection_until_message_from_bus(
        context, connection, "reply to ListActivatableNames/ListNames");
    ...
    if (message == NULL)
    {
        _dbus_warn("Did not receive a reply to %s %d on %p", method, serial,
                   connection);
        goto out;
    }
    ...
    retry_get_property:
        ...
        if (!dbus_message_get_args(message, &error, DBUS_TYPE_ARRAY,
                                   DBUS_TYPE_STRING, &srvs, &l,
                                   DBUS_TYPE_INVALID))
        {
            if (dbus_error_has_name(&error, DBUS_ERROR_NO_MEMORY))
            {
 ...
 * but the correct thing may include OOM errors.
 */
static dbus_bool_t check_list_services(BusContext* context,
                                       DBusConnection* connection)
{
    DBusMessage* message;
    DBusMessage* base_service_message;
    ...
    if (!check_get_services(context, connection, "ListActivatableNames",
                            &services, &len))
    {
        return TRUE;
    }
...
static void check2_try_iterations(BusContext* context,
                                  DBusConnection* connection,
                                  const char* description, Check2Func func)
{
    Check2Data d;
...
static dbus_bool_t bus_dispatch_test_conf(const DBusString* test_data_dir,
                                          const char* filename,
                                          dbus_bool_t use_launcher)
{
    ...
    _dbus_verbose("TODO: testing of GetConnectionUnixProcessID message skipped "
                  "for now\n");
#else
    ...
    if (use_launcher)
        if (!check_launch_service_file_missing(context, foo))
            _dbus_assert_not_reached(
                "did not get service file not found error");
...
#ifndef DBUS_WIN
static dbus_bool_t bus_dispatch_test_conf_fail(const DBusString* test_data_dir,
                                               const char* filename)
{
    BusContext* context;
    DBusConnection* foo;
...
dbus_bool_t bus_dispatch_test(const DBusString* test_data_dir)
{
    ...
    if (!bus_dispatch_test_conf(
            test_data_dir, "valid-config-files/debug-allow-all.conf", FALSE))
        return FALSE;
    ...
    if (!bus_dispatch_test_conf_fail(
            test_data_dir,
...
dbus_bool_t bus_dispatch_sha1_test(const DBusString* test_data_dir)
{
    ...
    context = bus_context_new_test(
        test_data_dir, "valid-config-files/debug-allow-all-sha1.conf");
    if (context == NULL)
        return FALSE;
    ...
    if (!check_no_leftovers(context))
    {
        _dbus_warn("Messages were left over after setting up initial SHA-1 "
                   "connection");
        _dbus_assert_not_reached("initial connection setup failed");
    }
...
dbus_bool_t bus_unix_fds_passing_test(const DBusString* test_data_dir)
{
    ...
    if (!dbus_message_append_args(m, DBUS_TYPE_UNIX_FD, one, DBUS_TYPE_UNIX_FD,
                                  two, DBUS_TYPE_UNIX_FD, two,
                                  DBUS_TYPE_INVALID))
    ...
    block_connection_until_message_from_bus(context, foo,
                                            "unix fd reception on foo");
    ...
    block_connection_until_message_from_bus(context, bar,
                                            "unix fd reception on bar");
    ...
    if (!dbus_message_get_args(m, &error, DBUS_TYPE_UNIX_FD, &x,
                               DBUS_TYPE_UNIX_FD, &y, DBUS_TYPE_UNIX_FD, &z,
                               DBUS_TYPE_INVALID))
