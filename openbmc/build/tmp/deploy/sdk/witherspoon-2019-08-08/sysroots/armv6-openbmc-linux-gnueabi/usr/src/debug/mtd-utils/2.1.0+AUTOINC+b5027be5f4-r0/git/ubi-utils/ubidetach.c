...
static const char optionsstr[] =
    ...
    "-p, --dev-path=<path to device> or alternatively, MTD device node path to "
    "detach\n"
    "-m, --mtdn=<MTD device number>  or alternatively, MTD device number to "
    "detach\n"
...
static int parse_opt(int argc, char* const argv[])
{
    ...
    if (args.mtdn == -1 && args.devn == -1 && args.dev == NULL)
        return errmsg(
            "neither MTD nor UBI devices were specified (use -h for help)");
    ...
    if (args.devn != -1)
    {
        ...
        return errmsg(
            "specify either MTD number or device node (use -h for help)");
