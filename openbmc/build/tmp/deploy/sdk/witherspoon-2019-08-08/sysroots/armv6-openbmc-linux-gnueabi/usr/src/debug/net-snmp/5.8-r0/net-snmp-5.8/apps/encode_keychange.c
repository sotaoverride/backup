...
#define USAGE                                                                  \
    "Usage: %s [-fhPvV] -t (md5|sha1) [-O \"<old_passphrase>\"][-N "           \
    "\"<new_passphrase>\"][-E [0x]<engineID>]"
...
int main(int argc, char** argv)
{
    ...
    size_t oldKu_len = SNMP_MAXBUF_SMALL, newKu_len = SNMP_MAXBUF_SMALL,
           oldkul_len = SNMP_MAXBUF_SMALL, newkul_len = SNMP_MAXBUF_SMALL,
           keychange_len = SNMP_MAXBUF_SMALL;

    char* s = NULL;
    u_char oldKu[SNMP_MAXBUF_SMALL], newKu[SNMP_MAXBUF_SMALL],
        oldkul[SNMP_MAXBUF_SMALL], newkul[SNMP_MAXBUF_SMALL],
    ...
    local_passphrase_filename =
        (char*)malloc(sizeof(PASSPHRASE_DIR) + sizeof(PASSPHRASE_FILE) + 4);
    ...
    {
        fprintf(stderr, "Unrecognized hash transform: \"%s\".\n",
    ...
    {
        engineid_len = hex_to_binary2(engineid + 2, strlen((char*)engineid) - 2,
                                      (char**)&engineid);
    ...
    if (verbose)
    {
        fprintf(stderr, "Old passphrase:\t%s\nNew passphrase:\t%s\n", oldpass,
                newpass);
    }
    ...
    rval = generate_Ku(transform_type, transform_type_len, (u_char*)oldpass,
                       strlen(oldpass), oldKu, &oldKu_len);
    QUITFUN(rval, main_quit);

    rval = generate_Ku(transform_type, transform_type_len, (u_char*)newpass,
                       strlen(newpass), newKu, &newKu_len);
    QUITFUN(rval, main_quit);

    DEBUGMSGTL(
        ("encode_keychange", "EID (%lu): ", (unsigned long)engineid_len));
    for (i = 0; i < (int)engineid_len; i++)
        DEBUGMSGTL(("encode_keychange", "%02x", (int)(engineid[i])));
    DEBUGMSGTL(("encode_keychange", "\n"));

    DEBUGMSGTL(("encode_keychange",
                "old Ku (%lu) (from %s): ", (unsigned long)oldKu_len, oldpass));
    for (i = 0; i < (int)oldKu_len; i++)
        DEBUGMSGTL(("encode_keychange", "%02x", (int)(oldKu[i])));
    DEBUGMSGTL(("encode_keychange", "\n"));

    rval = generate_kul(transform_type, transform_type_len, engineid,
                        engineid_len, oldKu, oldKu_len, oldkul, &oldkul_len);
    QUITFUN(rval, main_quit);
    ...
    rval = generate_kul(transform_type, transform_type_len, engineid,
                        engineid_len, newKu, newKu_len, newkul, &newkul_len);
    QUITFUN(rval, main_quit);
    ...
    rval =
        encode_keychange(transform_type, transform_type_len, oldkul, oldkul_len,
...
int get_user_passphrases(void)
{
    ...
    {
        fprintf(stderr, "Directory \"%s\" is accessible by group or world.\n",
                path);
        QUITFUN(SNMPERR_GENERR, get_user_passphrases_quit);
#endif /* !WIN32 */
    }
    ...
    {
        fprintf(stderr, "File \"%s\" is accessible by group or world.\n", path);
        QUITFUN(SNMPERR_GENERR, get_user_passphrases_quit);
#endif /* !WIN32 */
    }
...
get_user_passphrases_prompt:
    ...
    if (!oldpass)
    {
        oldpass = obuf = snmp_getpassphrase(
            (promptindicator) ? "Old passphrase: " : "", visible);
    }
    if (!newpass)
    {
        newpass = nbuf = snmp_getpassphrase(
            (promptindicator) ? "New passphrase: " : "", visible);
    }
 ...
 */
char* snmp_getpassphrase(const char* prompt, int bvisible)
{
    int ti = 0;
    size_t len;
