...
static const char* debug_verbosity[] = {
    ...
    /*3*/
    "ACL,BACKUP,CONNECT2,DELTASUM2,DEL2,EXIT,FILTER2,FLIST2,FUZZY,GENR,OWN,"
    "RECV,SEND,TIME",
    /*4*/ "CMD2,DELTASUM3,DEL3,EXIT2,FLIST3,ICONV2,OWN2,PROTO,TIME2",
    /*5*/ "CHDIR,DELTASUM4,FLIST4,FUZZY2,HASH,HLINK",
};

#define MAX_VERBOSITY                                                          \
    ((int)(sizeof debug_verbosity / sizeof debug_verbosity[0]) - 1)
...
#define INFO_WORD(flag, where, help)                                           \
    {                                                                          \
#flag, help, sizeof #flag - 1, INFO_##flag, where, 0                   \
    }

static struct output_struct info_words[COUNT_INFO + 1] = {
    INFO_WORD(BACKUP, W_REC, "Mention files backed up"),
    INFO_WORD(COPY, W_REC,
              "Mention files copied locally on the receiving side"),
    ...
    INFO_WORD(MISC, W_SND | W_REC,
              "Mention miscellaneous information (levels 1-2)"),
    INFO_WORD(MOUNT, W_SND | W_REC,
              "Mention mounts that were found or skipped"),
    INFO_WORD(NAME, W_SND | W_REC,
              "Mention 1) updated file/dir names, 2) unchanged names"),
    INFO_WORD(PROGRESS, W_CLI,
              "Mention 1) per-file progress or 2) total transfer progress"),
    INFO_WORD(REMOVE, W_SND, "Mention files removed on the sending side"),
    INFO_WORD(SKIP, W_REC,
              "Mention files that are skipped due to options used"),
    INFO_WORD(STATS, W_CLI | W_SRV,
              "Mention statistics at end of run (levels 1-3)"),
    INFO_WORD(SYMSAFE, W_SND | W_REC, "Mention symlinks that are unsafe"),
    {NULL, "--info", 0, 0, 0, 0}};

#define DEBUG_WORD(flag, where, help)                                          \
    {                                                                          \
#flag, help, sizeof #flag - 1, DEBUG_##flag, where, 0                  \
    }
...
static struct output_struct debug_words[COUNT_DEBUG + 1] = {
    ...
    DEBUG_WORD(CHDIR, W_CLI | W_SRV,
               "Debug when the current directory changes"),
    DEBUG_WORD(CONNECT, W_CLI, "Debug connection events (levels 1-2)"),
    DEBUG_WORD(CMD, W_CLI,
               "Debug commands+options that are issued (levels 1-2)"),
    DEBUG_WORD(DEL, W_REC, "Debug delete actions (levels 1-3)"),
    DEBUG_WORD(DELTASUM, W_SND | W_REC,
               "Debug delta-transfer checksumming (levels 1-4)"),
    ...
    DEBUG_WORD(ICONV, W_CLI | W_SRV,
               "Debug iconv character conversions (levels 1-2)"),
    DEBUG_WORD(IO, W_CLI | W_SRV, "Debug I/O routines (levels 1-4)"),
    DEBUG_WORD(OWN, W_REC,
               "Debug ownership changes in users & groups (levels 1-2)"),
...
static char* make_output_option(struct output_struct* words, short* levels,
                                uchar where)
{
    char* str = words == info_words ? "--info=" : "--debug=";
    int j, counts[MAX_OUT_LEVEL + 1], pos, skipped = 0, len = 0, max = 0,
                                           lev = 0;
    ...
    for (j = 0; words[j].name; j++)
    {
        if (words[j].flag != j)
        {
            rprintf(FERROR, "rsync: internal error on %s%s: %d != %d\n",
                    words == info_words ? "INFO_" : "DEBUG_", words[j].name,
                    words[j].flag, j);
    ...
    if (j != word_count)
    {
        rprintf(FERROR, "rsync: internal error: %s is wrong! (%d != %d)\n",
                words == info_words ? "COUNT_INFO" : "COUNT_DEBUG", j,
                word_count);
        exit_cleanup(RERR_UNSUPPORTED);
    }
    ...
    {
        if (words[j].priority == DEFAULT_PRIORITY || levels[j] == lev ||
            !(words[j].where & where))
            continue;
        if (pos)
            buf[pos++] = ',';
                               ...
                               const char* str, uchar priority)
{
    ...
    for (; str; str = s)
    {
        ...
        for (j = 0; words[j].name; j++)
        {
            if (!len || (len == words[j].namelen &&
                         strncasecmp(str, words[j].name, len) == 0))
            {
...
static void output_item_help(struct output_struct* words)
{
    ...
    const char** verbosity =
        words == info_words ? info_verbosity : debug_verbosity;
    char buf[128], *opt, *fmt = "%-10s %s\n";
    int j;

    reset_output_levels();

    rprintf(FINFO, "Use OPT or OPT1 for level 1 output, OPT2 for level 2, "
                   "etc.; OPT0 silences.\n");
    rprintf(FINFO, "\n");
    for (j = 0; words[j].name; j++)
    ...
    snprintf(buf, sizeof buf, "Set all %s options (e.g. all%d)", words[j].help,
             MAX_OUT_LEVEL);
...
static void set_output_verbosity(int level, uchar priority)
{
    ...
    for (j = 1; j <= level; j++)
    {
        parse_output_words(info_words, info_levels, info_verbosity[j],
                           priority);
        parse_output_words(debug_words, debug_levels, debug_verbosity[j],
                           priority);
    }
}
...
void limit_output_verbosity(int level)
{
    ...
    for (j = 1; j <= level; j++)
    {
        parse_output_words(info_words, info_limits, info_verbosity[j],
                           LIMIT_PRIORITY);
        parse_output_words(debug_words, debug_limits, debug_verbosity[j],
                           LIMIT_PRIORITY);
    }
...
static void print_rsync_version(enum logcode f)
{
    ...
    rprintf(f, "%s  version %s  protocol version %d%s\n", RSYNC_NAME,
            RSYNC_VERSION, PROTOCOL_VERSION, subprotocol);
    rprintf(f, "Copyright (C) 1996-2018 by Andrew Tridgell, Wayne Davison, and "
               "others.\n");
    rprintf(f, "Web site: http://rsync.samba.org/\n");
    rprintf(f, "Capabilities:\n");
    rprintf(f,
            "    %d-bit files, %d-bit inums, %d-bit timestamps, %d-bit long "
            "ints,\n",
            (int)(sizeof(OFF_T) * 8),
            (int)(sizeof dumstat->st_ino * 8), /* Don't check ino_t! */
            (int)(sizeof(time_t) * 8), (int)(sizeof(int64) * 8));
    rprintf(f,
            "    %ssocketpairs, %shardlinks, %ssymlinks, %sIPv6, batchfiles, "
            "%sinplace,\n",
            got_socketpair, hardlinks, links, ipv6, have_inplace);
    rprintf(f,
            "    %sappend, %sACLs, %sxattrs, %siconv, %ssymtimes, %sprealloc\n",
    ...
    {
        rprintf(f, "WARNING: size mismatch in SIZEOF_INT64 define (%d != %d)\n",
    ...
    rprintf(f, "rsync comes with ABSOLUTELY NO WARRANTY.  This is free "
               "software, and you\n");
    rprintf(f, "are welcome to redistribute it under certain conditions.  See "
               "the GNU\n");
...
void usage(enum logcode F)
{
    ...
    rprintf(F, "The ':' usages connect via remote shell, while '::' & "
               "'rsync://' usages connect\n");
    ...
    rprintf(F, "     --no-motd               suppress daemon-mode MOTD (see "
               "manpage caveat)\n");
    rprintf(F, " -c, --checksum              skip based on checksum, not "
               "mod-time & size\n");
    ...
    rprintf(F, " -b, --backup                make backups (see --suffix & "
               "--backup-dir)\n");
    rprintf(F, "     --backup-dir=DIR        make backups into hierarchy based "
               "in DIR\n");
    rprintf(F,
            "     --suffix=SUFFIX         set backup suffix (default %s w/o "
            "--backup-dir)\n",
            BACKUP_SUFFIX);
    ...
    rprintf(F, "     --append-verify         like --append, but with old data "
               "in file checksum\n");
    ...
    rprintf(F, "     --fake-super            store/recover privileged attrs "
               "using xattrs\n");
#endif
    rprintf(F, " -S, --sparse                turn sequences of nulls into "
               "sparse blocks\n");
#ifdef SUPPORT_PREALLOCATION
    ...
    rprintf(F, "     --delete-after          receiver deletes after transfer, "
               "not during\n");
    rprintf(F, "     --delete-excluded       also delete excluded files from "
               "destination dirs\n");
    rprintf(F, "     --ignore-missing-args   ignore missing source args "
               "without error\n");
    ...
    rprintf(F, "     --force                 force deletion of directories "
               "even if not empty\n");
    rprintf(F,
            "     --max-delete=NUM        don't delete more than NUM files\n");
    rprintf(F, "     --max-size=SIZE         don't transfer any file larger "
               "than SIZE\n");
    rprintf(F, "     --min-size=SIZE         don't transfer any file smaller "
               "than SIZE\n");
    rprintf(F,
            "     --partial               keep partially transferred files\n");
    rprintf(F, "     --partial-dir=DIR       put a partially transferred file "
               "into DIR\n");
    rprintf(F, "     --delay-updates         put all updated files into place "
               "at transfer's end\n");
    ...
    rprintf(F, "     --contimeout=SECONDS    set daemon connection timeout in "
               "seconds\n");
    rprintf(F, " -I, --ignore-times          don't skip files that match in "
               "size and mod-time\n");
    rprintf(
        F,
    ...
    rprintf(F, "     --compare-dest=DIR      also compare destination files "
               "relative to DIR\n");
    rprintf(F, "     --copy-dest=DIR         ... and include copies of "
               "unchanged files\n");
    ...
    rprintf(F, "     --skip-compress=LIST    skip compressing files with a "
               "suffix in LIST\n");
    rprintf(F, " -C, --cvs-exclude           auto-ignore files the same way "
               "CVS does\n");
    ...
    rprintf(F, "     --files-from=FILE       read list of source-file names "
               "from FILE\n");
    ...
    rprintf(F, " -8, --8-bit-output          leave high-bit chars unescaped in "
               "output\n");
    rprintf(F, " -h, --human-readable        output numbers in a "
               "human-readable format\n");
    rprintf(F, "     --progress              show progress during transfer\n");
    rprintf(F, " -P                          same as --partial --progress\n");
    rprintf(F, " -i, --itemize-changes       output a change-summary for all "
               "updates\n");
    rprintf(F, "     --out-format=FORMAT     output updates using the "
               "specified FORMAT\n");
...
#ifdef HAVE_SETVBUF
    rprintf(F, "     --outbuf=N|L|B          set output buffering to None, "
               "Line, or Block\n");
#endif
    rprintf(F, "     --write-batch=FILE      write a batched update to FILE\n");
    rprintf(F, "     --only-write-batch=FILE like --write-batch but w/o "
               "updating destination\n");
    rprintf(F,
            "     --read-batch=FILE       read a batched update from FILE\n");
    rprintf(F, "     --protocol=NUM          force an older protocol version "
               "to be used\n");
#ifdef ICONV_OPTION
    ...
    rprintf(F, "(-h) --help                  show this help (-h is --help only "
               "if used alone)\n");

    rprintf(F, "\n");
    rprintf(F, "Use \"rsync --daemon --help\" to see the daemon-mode "
...
static struct poptOption long_options[] = {
    ...
    {"modify-window", '@', POPT_ARG_INT, &modify_window, OPT_MODIFY_WINDOW, 0,
     0},
    ...
    {"remove-sent-files", 0, POPT_ARG_VAL, &remove_source_files, 2, 0,
     0}, /* deprecated */
    ...
    {"only-write-batch", 0, POPT_ARG_STRING, &batch_name, OPT_ONLY_WRITE_BATCH,
     0, 0},
...
static void daemon_usage(enum logcode F)
{
    ...
    rprintf(F, "     --bwlimit=RATE          limit socket I/O bandwidth\n");
    rprintf(
        F, "     --config=FILE           specify alternate rsyncd.conf file\n");
    ...
    rprintf(F, "\n");
    rprintf(F, "If you were not trying to invoke rsync as a daemon, avoid "
               "using any of the\n");
    rprintf(F, "daemon-specific rsync options.  See also the rsyncd.conf(5) "
               "man page.\n");
}
...
void option_error(void)
{
    ...
    {
        strlcpy(err_buf,
                "Error parsing options: option may "
...
int parse_arguments(int* argc_p, const char*** argv_p)
{
...
#ifdef ICONV_OPTION
    if (!am_daemon && protect_args <= 0 &&
        (arg = getenv("RSYNC_ICONV")) != NULL && *arg)
        ...
        switch (opt)
        {
            ...
            case OPT_SERVER:
                if (!am_server)
                {
                    ...
                    poptFreeContext(pc);
                    pc =
                        poptGetContext(RSYNC_NAME, argc, argv, long_options, 0);
                    am_server = 1;
                }
#ifdef ICONV_OPTION
                iconv_opt = NULL;
            ...
            case OPT_DAEMON:
                ...
                protect_args = 0;
                poptFreeContext(pc);
                pc = poptGetContext(RSYNC_NAME, argc, argv, long_daemon_options,
                                    0);
                while ((opt = poptGetNextOpt(pc)) != -1)
                {
                    char** cpp;
                ...
                daemon_error:
                    rprintf(FERROR, "(Type \"rsync --daemon --help\" for "
                                    "assistance with daemon mode.)\n");
                    exit_cleanup(RERR_SYNTAX);
                }
            ...
            case OPT_INCLUDE_FROM:
                ...
                if (daemon_filter_list.head)
                {
                    ...
                    else
                    {
                        ...
                        rej =
                            check_filter(&daemon_filter_list, FLOG, dir, 0) < 0;
                    }
                    free(cp);
                    if (rej)
                ...
                parse_filter_file(&filter_list, arg,
                                  rule_template(opt == OPT_INCLUDE_FROM
                                                    ? FILTRULE_INCLUDE
                                                    : 0),
            ...
            case 'F':
                ...
                {
                    case 1:
                        parse_filter_str(&filter_list, ": /.rsync-filter",
                                         rule_template(0), 0);
                        break;
                    case 2:
                        parse_filter_str(&filter_list, "- .rsync-filter",
            ...
            case 'P':
                ...
                {
                    create_refuse_error(refused_partial ? refused_partial
                                                        : refused_progress);
                    return 0;
                }
                do_progress = 1;
            ...
            case 'M':
                ...
                if (remote_option_cnt + 2 >= remote_option_alloc)
                {
                    ...
                    remote_options = realloc_array(remote_options, const char*,
                                                   remote_option_alloc);
                    if (!remote_options)
                        out_of_memory("parse_arguments");
            ...
            case OPT_DEBUG:
                arg = poptGetOptArg(pc);
                parse_output_words(debug_words, debug_levels, arg,
                                   USER_PRIORITY);
                break;
            ...
            case OPT_CHOWN:
            {
                ...
                if (len)
                {
                    if (usermap)
                    {
                        ...
                        snprintf(err_buf, sizeof err_buf,
                                 "You can only specify a user-affecting "
                                 "--chown once.\n");
                        return 0;
                    }
                ...
                if (arg && *arg)
                {
                    if (groupmap)
                    {
                        ...
                        snprintf(err_buf, sizeof err_buf,
                                 "You can only specify a group-affecting "
                                 "--chown once.\n");
                        return 0;
                    }
    ...
    if (checksum_choice && strcmp(checksum_choice, "auto") != 0 &&
        strcmp(checksum_choice, "auto,auto") != 0)
    {
        /* Call this early to verify the args and figure out if we need to force
         * --whole-file. Note that the parse function will get called again
         * later, just in case an "auto" choice needs to know the
         * protocol_version. */
    ...
    if (do_compression || def_compress_level != NOT_SPECIFIED)
    {
        ...
        else if (def_compress_level < Z_DEFAULT_COMPRESSION ||
    ...
    *argc_p = argc = count_args(argv);
    ...
    {
        snprintf(err_buf, sizeof err_buf, "--fake-super conflicts with -XX\n");
        return 0;
    }
#else
    if (am_root < 0)
    {
        snprintf(err_buf, sizeof err_buf,
                 "--fake-super requires an rsync with extended attributes "
                 "enabled\n");
        return 0;
    }
#endif
    ...
    if (write_batch > 0 || read_batch)
    {
        if (am_server)
        {
            rprintf(FINFO, "ignoring --%s-batch option sent to server\n",
    ...
    {
        snprintf(err_buf, sizeof err_buf,
                 "--delete does not work without --recursive (-r) or --dirs "
                 "(-d).\n");
        return 0;
    }
    ...
    if (munge_symlinks && !am_daemon)
    {
        ...
        strlcpy(prefix, SYMLINK_PREFIX,
                sizeof prefix); /* trim the trailing slash */
    ...
    {
        snprintf(err_buf, sizeof err_buf, "--suffix cannot be empty %s\n",
                 backup_dir_len < 0
    ...
    {
        snprintf(backup_dir_buf, sizeof backup_dir_buf, "P *%s", backup_suffix);
    ...
    if (do_progress && !am_server)
    {
        if (!log_before_transfer && INFO_EQ(NAME, 0))
            parse_output_words(info_words, info_levels, "name",
                               DEFAULT_PRIORITY);
        parse_output_words(info_words, info_levels, "flist2,progress",
                           DEFAULT_PRIORITY);
    }
    ...
    if (inplace)
    {
        ...
        if (partial_dir)
        {
            snprintf(err_buf, sizeof err_buf, "--%s cannot be used with --%s\n",
        ...
        keep_partial = 0;
#else
        snprintf(err_buf, sizeof err_buf, "--%s is not supported on this %s\n",
                 append_mode ? "append" : "inplace",
                 am_server ? "server" : "client");
        return 0;
#endif
    ...
    if (files_from)
    {
        ...
        else if ((p = check_for_hostspec(files_from, &h, &q)) != 0)
        {
            ...
            {
                snprintf(err_buf, sizeof err_buf,
                         "The --files-from sent to the server cannot specify a "
                         "host.\n");
                return 0;
            }
            files_from = p;
        ...
        else
        {
            if (sanitize_paths)
                files_from =
                    sanitize_path(NULL, files_from, NULL, 0, SP_DEFAULT);
            if (daemon_filter_list.head)
            {
                char* dir;
                if (!*files_from)
...
void server_options(char** args, int* argc_p)
{
    ...
    {
        /* Use "eFlags" alias so that cull_options doesn't think that these are
         * no-arg option letters. */
#define eFlags argstr
        ...
        {
            x += snprintf(argstr + x, sizeof argstr - x, "%d.%d",
