...
class attribute_name::repository
    : public log::aux::lazy_singleton<repository, shared_ptr<repository>>
{
    typedef log::aux::lazy_singleton<repository, shared_ptr<repository>>
        base_type;

#if !defined(BOOST_LOG_BROKEN_FRIEND_TEMPLATE_SPECIALIZATIONS)
    friend class log::aux::lazy_singleton<repository, shared_ptr<repository>>;
#else
  ...
  public:
    ...
    typedef intrusive::set_base_hook<intrusive::link_mode<intrusive::safe_link>,
                                     intrusive::optimize_size<true>>
        node_by_name_hook;

  private:
    //! An element of the attribute names repository
    struct node : public node_by_name_hook
    {
      ...
      public:
        ...
        struct order_by_name
        {
            ...
            {
                return std::strcmp(left.m_name.c_str(), right.m_name.c_str()) <
                       0;
            }
      ...
      public:
        ...
        node(id_type i, string_type const& n) : base_type(), m_id(i), m_name(n)
        {
        }
        node(node const& that) :
            base_type(), m_id(that.m_id), m_name(that.m_name)
        {
        }
    };
    ...
    typedef intrusive::set<node, intrusive::base_hook<node_by_name_hook>,
  ...
  public:
    ...
    id_type get_id_from_string(const char* name)
    {
        ...
        BOOST_LOG_EXPR_IF_MT(
            log::aux::exclusive_lock_guard<mutex_type> _(m_Mutex);)
        ...
        if (it == m_NodeSet.end() || it->m_name != name)
        {
            ...
            if (new_id >= static_cast<id_type>(attribute_name::uninitialized))
                BOOST_THROW_EXCEPTION(
                    limitation_error("Too many log attribute names"));
