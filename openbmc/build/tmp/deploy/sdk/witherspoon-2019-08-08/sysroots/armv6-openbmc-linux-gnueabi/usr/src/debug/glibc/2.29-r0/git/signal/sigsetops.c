...
int attribute_compat_text_section(__sigismember)(const __sigset_t* set, int sig)
{
    return __sigismember(set, sig);
}
compat_symbol(libc, __sigismember, __sigismember, GLIBC_2_0);

int attribute_compat_text_section(__sigaddset)(__sigset_t* set, int sig)
{
    __sigaddset(set, sig);
    return 0;
}
...
int attribute_compat_text_section(__sigdelset)(__sigset_t* set, int sig)
{
    __sigdelset(set, sig);
    return 0;
}
