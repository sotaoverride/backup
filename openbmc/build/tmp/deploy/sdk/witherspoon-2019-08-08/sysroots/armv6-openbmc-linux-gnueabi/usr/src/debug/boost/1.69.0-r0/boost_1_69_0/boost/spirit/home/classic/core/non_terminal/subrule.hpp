...
struct subrules_scanner : public ScannerT
{
    ...
    template <typename PoliciesT>
    subrules_scanner<
        typename rebind_scanner_policies<ScannerT, PoliciesT>::type, ListT>
        change_policies(PoliciesT const& policies) const
    {
        typedef subrules_scanner<BOOST_DEDUCED_TYPENAME rebind_scanner_policies<
    ...
    template <typename IteratorT>
    subrules_scanner<
        typename rebind_scanner_iterator<ScannerT, IteratorT>::type, ListT>
        change_iterator(IteratorT const& first, IteratorT const& last) const
    {
        typedef subrules_scanner<BOOST_DEDUCED_TYPENAME rebind_scanner_iterator<
...
struct subrule_list : public parser<subrule_list<FirstT, RestT>>
{
    ...
    subrule_list<FirstT,
                 subrule_list<subrule_parser<ID, DefT, ContextT>, RestT>>
        operator,(subrule_parser<ID, DefT, ContextT> const& rhs_)
    {
        return subrule_list<
            FirstT, subrule_list<subrule_parser<ID, DefT, ContextT>, RestT>>(
            first, subrule_list<subrule_parser<ID, DefT, ContextT>, RestT>(
                       rhs_, rest));
    }
...
struct subrule_parser : public parser<subrule_parser<ID, DefT, ContextT>>
{
    ...
    struct result
    {
        typedef typename impl::get_subrule_parser_result<
    ...
    inline subrule_list<
        self_t, subrule_list<subrule_parser<ID2, DefT2, ContextT2>, nil_t>>
        operator,(subrule_parser<ID2, DefT2, ContextT2> const& rhs) const
    {
        return subrule_list<
            self_t, subrule_list<subrule_parser<ID2, DefT2, ContextT2>, nil_t>>(
            *this, subrule_list<subrule_parser<ID2, DefT2, ContextT2>, nil_t>(
                       rhs, nil_t()));
    }
...
struct subrule : public parser<subrule<ID, ContextT>>,
                 ...
                 public context_aux<ContextT, subrule<ID, ContextT>>
{
    ...
    {
        typedef
            typename impl::get_subrule_result<ID, ScannerT, attr_t>::type type;
    };
