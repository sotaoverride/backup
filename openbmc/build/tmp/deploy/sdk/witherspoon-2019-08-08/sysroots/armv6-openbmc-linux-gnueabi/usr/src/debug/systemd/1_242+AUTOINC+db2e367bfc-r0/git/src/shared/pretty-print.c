...
static bool urlify_enabled(void)
{
    ...
    /* Unfortunately 'less' doesn't support links like this yet 😭, hence let's
     * disable this as long as there's a pager in effect. Let's drop this check
     * as soon as less got fixed a and enough time passed so that it's safe to
     * assume that a link-enabled 'less' version has hit most installations. */
...
int terminal_urlify(const char* url, const char* text, char** ret)
{
    ...
    /* Takes an URL and a pretty string and formats it as clickable link for the
     * terminal. See
...
int file_url_from_path(const char* path, char** ret)
{
    ...
    /* As suggested by
     * https://gist.github.com/egmontkob/eb114294efbcd5adb1944c9f3cb5feda, let's
     * include the local hostname here. Note that we don't use
     * gethostname_malloc() or gethostname_strict() since we are interested in
     * the raw string the kernel has set, whatever it may be, under the
     * assumption that terminals are not overly careful with validating the
     * strings either. */
...
void print_separator(void)
{
    ...
    /* Outputs a separator line that resolves to whitespace when copied from the
     * terminal. We do that by outputting
     * one line filled with spaces with ANSI underline set, followed by a second
     * (empty) line. */
...
static int guess_type(const char** name, bool* is_usr, bool* is_collection,
                      const char** extension)
{
    ...
    if (path_equal(*name, "environment.d"))
        /* Special case: we need to include /etc/environment in the search path,
         * even though the whole concept is called environment.d. */
        *name = "environment";
...
int conf_files_cat(const char* root, const char* name)
{
    ...
    r = conf_files_list_strv(&files, extension, root, 0,
