...
static int _parse_config_file(pam_handle_t* pamh, int ctrl, const char* file)
{
    int retval;
    ...
    while ((retval = _assemble_line(conf, buffer, BUF_SIZE)) > 0)
    {
        ...
        if (PAM_SUCCESS != retval && ILLEGAL_VAR != retval &&
            BAD_LINE != retval && PAM_BAD_ITEM != retval)
            break;
...
static int _parse_env_file(pam_handle_t* pamh, int ctrl, const char* file)
{
    ...
    while (_assemble_line(conf, buffer, BUF_SIZE) > 0)
    {
        ...
        for (i = 0; key[i] != '=' && key[i] != '\0'; i++)
            ...
            {
                pam_syslog(pamh, LOG_ERR,
                           "non-alphanumeric key '%s' in %s', ignoring", key,
                           file);
                break;
            }
        ...
        {
            pam_syslog(pamh, LOG_DEBUG, "pam_putenv(\"%s\")", key);
        }
    }
...
static int _parse_line(const pam_handle_t* pamh, char* buffer, VAR* var)
{
    /*
    ...
    /*
     * Now we check for arguments, we only support two kinds and ('cause I am
     * lazy) each one can actually be listed any number of times
     */
    ...
    while ((length = strspn(ptr, " \t")) > 0)
    {
        ...
        else
        {
            ...
            pam_syslog(pamh, LOG_ERR, "Unrecognized Option: %s - ignoring line",
                       ptr);
            return BAD_LINE;
        }
        ...
        else
        {
            ...
            if (!tmpptr)
            {
                ...
                pam_syslog(pamh, LOG_ERR, "Unterminated quoted string: %s",
                           ptr - 1);
                return BAD_LINE;
            }
            length = tmpptr - ptr;
                ...
                '\n' != *tmpptr)
            {
                ...
                pam_syslog(pamh, LOG_ERR,
                           "Quotes must cover the entire string: <%s>", ptr);
                return BAD_LINE;
            }
            quoteflg++;
        }
        if (length)
        {
            if ((*valptr = malloc(length + 1)) == NULL)
            {
                ...
                pam_syslog(pamh, LOG_CRIT, "Couldn't malloc %d bytes",
                           length + 1);
                return PAM_BUF_ERR;
            }
...
static int _check_var(pam_handle_t* pamh, VAR* var)
{
     ...
     * if OVERRIDE=value   and value turns into the empty string, DEFAULT is
     * used
     *
    ...
    if (var->override && *(var->override) && &quote != var->override)
    {
        ...
        D(("OVERRIDE variable <%s> being used: <%s>", var->name,
           var->override));
    ...
    else
    {
        ...
        {
            D(("DEFAULT variable <%s> being used: <%s>", var->name,
               var->defval));
            retval = DEFINE_VAR;
        }
        else
    ...
    { /* while there is some input to deal with */
        if ('\\' == *orig)
        {
            ...
            else
            {
                ...
                pam_syslog(pamh, LOG_ERR,
                           "Variable buffer overflow: <%s> + <%s>", tmp,
                           tmpptr);
                return PAM_BUF_ERR;
            }
            continue;
        ...
        if ('$' == *orig || '@' == *orig)
        {
            ...
            else
            {
                 ...
                 * an environment variable or a PAM_ITEM. type will tell us
                 * which
                 */
                switch (type)
                {
                ...
                if (tmpptr)
                {
                    ...
                    {
                        /* is it really a good idea to try to log this? */
                        D(("Variable buffer overflow: <%s> + <%s>", tmp,
                           tmpptr));
                        pam_syslog(pamh, LOG_ERR,
                                   "Variable buffer overflow: <%s> + <%s>", tmp,
                                   tmpptr);
...
static const char* _pam_get_item_byname(pam_handle_t* pamh, const char* name)
{
    ...
    if (itemval && (strcmp(name, "HOME") == 0 || strcmp(name, "SHELL") == 0))
    {
        ...
        return (strcmp(name, "SHELL") == 0) ? user_entry->pw_shell
                                            : user_entry->pw_dir;
    }
...
static int handle_env(pam_handle_t* pamh, int argc, const char** argv)
{
    ...
    if (user_readenv && retval == PAM_SUCCESS)
    {
        ...
        {
            if (asprintf(&envpath, "%s/%s", user_entry->pw_dir, user_env_file) <
                0)
            {
                pam_syslog(pamh, LOG_CRIT, "Out of memory");
                return PAM_BUF_ERR;
...
int pam_sm_acct_mgmt(pam_handle_t* pamh UNUSED, int flags UNUSED,
                     int argc UNUSED, const char** argv UNUSED)
{
...
int pam_sm_chauthtok(pam_handle_t* pamh UNUSED, int flags UNUSED,
                     int argc UNUSED, const char** argv UNUSED)
{
