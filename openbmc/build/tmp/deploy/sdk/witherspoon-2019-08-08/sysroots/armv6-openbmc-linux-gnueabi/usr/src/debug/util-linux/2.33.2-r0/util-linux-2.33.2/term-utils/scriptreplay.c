...
static void __attribute__((__noreturn__)) usage(void)
{
    ...
    fprintf(out, _(" %s [-t] timingfile [typescript] [divisor]\n"),
    ...
    fputs(_(" -t, --timing <file>     script timing output file\n"
            ...
            " -d, --divisor <num>     speed up or slow down execution with "
            "time divisor\n"
            " -m, --maxdelay <num>    wait at most this many seconds between "
            "updates\n"),
          out);
...
static void emit(FILE* fd, const char* filename, size_t ct)
{
    char buf[BUFSIZ];
    ...
    for (line = 1;; line++)
    {
        ...
        if (fscanf(tfile, "%lf %zu%c\n", &delay, &blk, &nl) != 3 || nl != '\n')
        {
            if (feof(tfile))
                break;
            if (ferror(tfile))
                err(EXIT_FAILURE, _("failed to read timing file %s"), tname);
            errx(EXIT_FAILURE, _("timing file %s: line %lu: unexpected format"),
                 tname, line);
        }
        delay /= divi;
