...
class unbounded_fifo_queue
{
  ...
  protected:
    ...
    explicit unbounded_fifo_queue(ArgsT const&) :
    ...
    //! Attempts to dequeue a log record ready for processing from the queue,
    ...
    //! Attempts to dequeue log record from the queue, does not block if the
    //! queue is empty
    ...
    bool dequeue_ready(record_view& rec)
    {
        ...
        {
            m_event.wait();
            if (m_interruption_requested.exchange(false,
