 ...
 */
static int ofdlock(int fd, int cmd, fcntl_lock_st* lock_arg)
{
#ifdef F_OFD_SETLKW
    int st, ofdcmd;
...
/*ARGSUSED*/
krb5_error_code krb5_lock_file(krb5_context context, int fd, int mode)
{
    int lock_flag = -1;
...
krb5_error_code krb5_lock_file(context, fd, mode) krb5_context context;
int fd;
int mode;
{
    return 0;
