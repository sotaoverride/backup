...
struct bstree_node_checker : public ExtraChecker
{
    ...
    struct return_type : public base_checker_t::return_type
    {
        BOOST_INTRUSIVE_FORCEINLINE return_type() :
            min_key_node_ptr(const_node_ptr()),
            max_key_node_ptr(const_node_ptr()), node_count(0)
        {
        }
    ...
    BOOST_INTRUSIVE_FORCEINLINE
        bstree_node_checker(const NodePtrCompare& comp,
                            ExtraChecker extra_checker) :
        base_checker_t(extra_checker),
        comp_(comp)
    {
    }
                    ...
                    return_type& check_return)
    {
        if (check_return_left.max_key_node_ptr)
            BOOST_INTRUSIVE_INVARIANT_ASSERT(
                !comp_(p, check_return_left.max_key_node_ptr));
        if (check_return_right.min_key_node_ptr)
            BOOST_INTRUSIVE_INVARIANT_ASSERT(
                !comp_(check_return_right.min_key_node_ptr, p));
        check_return.min_key_node_ptr =
            node_traits::get_left(p) ? check_return_left.min_key_node_ptr : p;
        check_return.max_key_node_ptr =
            node_traits::get_right(p) ? check_return_right.max_key_node_ptr : p;
        check_return.node_count =
            check_return_left.node_count + check_return_right.node_count + 1;
        base_checker_t::operator()(p, check_return_left, check_return_right,
                                   check_return);
    }
...
//!   A node in the search tree has references to its children and its parent.
//!   This is to allow traversal of the whole tree from a given node making the
...
//!   the leftmost node in the tree and the right pointer to the rightmost one.
...
//!                       +---------+   +---------+   +---------+   +---------+
//!
//! bstree_algorithms is configured with a NodeTraits class, which encapsulates
//! the information about the node to be manipulated. NodeTraits must support
...
class bstree_algorithms : public bstree_algorithms_base<NodeTraits>
{
  ...
  public:
    ...
    //!
    //! <b>Effects</b>: Returns the first node of the tree, the header if the
    //! tree is empty.
    //!
    ...
    BOOST_INTRUSIVE_FORCEINLINE static node_ptr
        begin_node(const const_node_ptr& header)
    {
    ...
    //! <b>Throws</b>: Nothing.
    BOOST_INTRUSIVE_FORCEINLINE static node_ptr
        end_node(const const_node_ptr& header)
    {
    ...
    //! <b>Throws</b>: Nothing.
    BOOST_INTRUSIVE_FORCEINLINE static node_ptr
        root_node(const const_node_ptr& header)
    {
    ...
    //!
    //! <b>Effects</b>: Returns true if the node is initialized by init() or
    //! init_node().
    //!
    //! <b>Complexity</b>: Constant time.
    //!
    ...
    //!  of two trees.
    //!
    //! <b>Effects</b>: Swaps two nodes. After the function node1 will be
    //! inserted
    //!   in the position node2 before the function. node2 will be inserted in
    //!   the position node1 had before the function.
    //!
    //! <b>Complexity</b>: Logarithmic.
    //!
    //! <b>Throws</b>: Nothing.
    ...
    static void swap_nodes(node_ptr node1, node_ptr node2)
    {
        ...
        node_ptr header1(base_type::get_header(node1)),
            header2(base_type::get_header(node2));
                           ...
                           node_ptr header2)
    {
        ...
        // BOOST_INTRUSIVE_INVARIANT_ASSERT((header1 != node1 && header2 !=
        // node2));
        if (header1 != header2)
        {
    ...
    //!   the node, since no rebalancing and comparison is needed. Experimental
    //!   function
    BOOST_INTRUSIVE_FORCEINLINE static void
        replace_node(node_ptr node_to_be_replaced, node_ptr new_node)
    {
        if (node_to_be_replaced == new_node)
            return;
        replace_node(node_to_be_replaced,
                     base_type::get_header(node_to_be_replaced), new_node);
    }
    ...
    //!   new_node is not equivalent to node_to_be_replaced according to the
    //!   ordering rules. This function is faster than erasing and inserting
    //!   the node, since no rebalancing or comparison is needed. Experimental
    //!   function
                             ...
                             node_ptr new_node)
    {
        ...
        node_ptr temp;
        NodeTraits::set_left(new_node,
                             NodeTraits::get_left(node_to_be_replaced));
        NodeTraits::set_right(new_node,
                              NodeTraits::get_right(node_to_be_replaced));
        NodeTraits::set_parent(new_node,
                               NodeTraits::get_parent(node_to_be_replaced));
    ...
    //! <b>Requires</b>: 'node' is a node from the tree except the leftmost
    //! node.
    //!
    ...
    //! <b>Throws</b>: Nothing.
    //!
    //! <b>Nodes</b>: If node is inserted in a tree, this function corrupts the
    //! tree.
    ...
    //! <b>Effects</b>: Returns true if node is in the same state as if called
    //! init(node)
    //!
    //! <b>Complexity</b>: Constant.
    //!
    ...
    {
        return !NodeTraits::get_parent(node) && !NodeTraits::get_left(node) &&
    ...
    //!   <tt>void disposer::operator()(const node_ptr &)</tt> for every node of
    //!   the tree
    //!    except the header.
    //!
    //! <b>Complexity</b>: Linear to the number of element of the source tree
    //! plus the.
    //!   number of elements of tree target tree when calling this function.
    //!
    //! <b>Throws</b>: If cloner functor throws. If this happens target nodes
    //! are disposed.
    template <class Disposer>
    ...
    static node_ptr unlink_leftmost_without_rebalance(node_ptr header)
    {
        ...
        if (leftmost_right)
        {
            ...
            if (is_root)
                ...
                NodeTraits::set_left(NodeTraits::get_parent(header),
                                     leftmost_right);
        }
        else if (is_root)
        {
    ...
    static std::size_t size(const const_node_ptr& header)
    {
        ...
        std::size_t i = 0;
        for (; beg != end; beg = base_type::next_node(beg))
            ++i;
        return i;
    }
    ...
    //!   links to the second tree and header2 will have links to the first
    //!   tree.
    //!
    //! <b>Complexity</b>: Constant.
    //!
    ...
    //!   ordering compatible with the strict weak ordering used to create the
    //!   the tree. KeyNodePtrCompare can compare KeyType with tree's node_ptrs.
    //!   'lower_key' must not be greater than 'upper_key' according to 'comp'.
    //!   If 'lower_key' == 'upper_key', ('left_closed' || 'right_closed') must
    //!   be true.
    //!
                      ...
                      bool left_closed, bool right_closed)
    {
        ...
        while (x)
        {
            ...
            else
            {
                ...
                //
                // Sanity check: if lower_key and upper_key are equal, then both
                // left_closed and right_closed can't be false
                BOOST_INTRUSIVE_INVARIANT_ASSERT(left_closed || right_closed ||
                                                 comp(lower_key, x) ||
                                                 comp(x, upper_key));
                return std::pair<node_ptr, node_ptr>(
                    left_closed
                        // If left_closed, then comp(x, lower_key) is already
                        // the lower_bound condition so we save one comparison
                        // and go to the next level following traditional
                        // lower_bound algo
                        ? lower_bound_loop(NodeTraits::get_left(x), x,
                                           lower_key, comp)
                        // If left-open, comp(x, lower_key) is not the
                        // upper_bound algo condition so we must recheck current
                        // 'x' node with upper_bound algo
                        : upper_bound_loop(x, y, lower_key, comp),
                    right_closed
                        // If right_closed, then comp(upper_key, x) is already
                        // the upper_bound condition so we can save one
                        // comparison and go to the next level following
                        // lower_bound algo
                        ? upper_bound_loop(NodeTraits::get_right(x), y,
                                           upper_key, comp)
                        // If right-open, comp(upper_key, x) is not the
                        // lower_bound algo condition so we must recheck current
                        // 'x' node with lower_bound algo
    ...
    //!   the tree. KeyNodePtrCompare can compare KeyType with tree's node_ptrs.
    //!
    //! <b>Effects</b>: Returns the number of elements with a key equivalent to
    //! "key"
    //!   according to "comp".
    //!
    //! <b>Complexity</b>: Logarithmic.
    ...
    //!   the tree. KeyNodePtrCompare can compare KeyType with tree's node_ptrs.
    //!
    //! <b>Effects</b>: Returns an a pair of node_ptr delimiting a range
    //! containing
    ...
    //! <b>Throws</b>: If "comp" throws.
    template <class KeyType, class KeyNodePtrCompare>
    BOOST_INTRUSIVE_FORCEINLINE static std::pair<node_ptr, node_ptr>
    ...
    //! <b>Effects</b>: Returns an a pair of node_ptr delimiting a range
    //! containing
    //!   the first element that is equivalent to "key" according to "comp" or
    //!   an empty range that indicates the position where that element would be
    //!   if there are no equivalent elements.
    //!
    //! <b>Complexity</b>: Logarithmic.
    //!
    ...
    //!
    //! <b>Effects</b>: Returns a node_ptr to the first element that is
    //!   not less than "key" according to "comp" or "header" if that element
    //!   does not exist.
    //!
    //! <b>Complexity</b>: Logarithmic.
    //!
    ...
    BOOST_INTRUSIVE_FORCEINLINE static node_ptr
        lower_bound(const const_node_ptr& header, const KeyType& key,
                    KeyNodePtrCompare comp)
    {
        return lower_bound_loop(NodeTraits::get_parent(header),
                                detail::uncast(header), key, comp);
    }
    ...
    //!   than "key" according to "comp" or "header" if that element does not
    //!   exist.
    //!
    //! <b>Complexity</b>: Logarithmic.
    //!
    ...
    BOOST_INTRUSIVE_FORCEINLINE static node_ptr
        upper_bound(const const_node_ptr& header, const KeyType& key,
                    KeyNodePtrCompare comp)
    {
        return upper_bound_loop(NodeTraits::get_parent(header),
                                detail::uncast(header), key, comp);
    }
    ...
    //!   from the set between the "insert_unique_check" that filled
    //!   "commit_data" and the call to "insert_commit".
    //!
    //!
    //! <b>Effects</b>: Inserts new_node in the set using the information
    //! obtained
    ...
    //! <b>Throws</b>: Nothing.
    //!
    //! <b>Notes</b>: This function has only sense if a "insert_unique_check"
    //! has been
    ...
    BOOST_INTRUSIVE_FORCEINLINE static void
        insert_unique_commit(node_ptr header, node_ptr new_value,
    ...
    //!   in the returned pair's boolean and fills "commit_data" that is meant
    //!   to be used with the "insert_commit" function to achieve a
    //!   constant-time insertion function.
    //!
    ...
    //! constructing
    //!   a node is expensive and the user does not want to have two equivalent
    //!   nodes in the tree: if there is an equivalent value the constructed
    //!   object must be discarded. Many times, the part of the node that is
    //!   used to impose the order is much cheaper to construct than the node
    //!   and this function offers the possibility to use that part to check if
    //!   the insertion will be successful.
    //!
    //!   If the check is successful, the user can construct the node and use
    //!   "insert_commit" to insert the node in constant-time. This gives a
    //!   total logarithmic complexity to the insertion: check(O(log(N)) +
    //!   commit(O(1)).
    //!
    //!   "commit_data" remains valid for a subsequent "insert_unique_commit"
    //!   only if no more objects are inserted or erased from the set.
    template <class KeyType, class KeyNodePtrCompare>
        ...
        )
    {
        ...
        while (x)
        {
            ...
            x = (left_child = comp(key, x))
                    ...
                    : (prev = y, NodeTraits::get_right(x));
        }

        if (pdepth)
            *pdepth = depth;

        // Since we've found the upper bound there is no other value with the
        // same key if:
    ...
    //!   amortized constant time if new_node should be inserted immediately
    //!   before "hint".
    //!
    //! <b>Throws</b>: If "comp" throws.
    //!
        ...
        )
    {
        ...
        if (hint == header || comp(key, hint))
        {
            ...
            if (hint == begin_node(header) ||
                comp((prev = base_type::prev_node(hint)), key))
            {
                commit_data.link_left =
                    unique(header) || !NodeTraits::get_left(hint);
                commit_data.node = commit_data.link_left ? hint : prev;
                if (pdepth)
                {
                    *pdepth = commit_data.node == header
                                  ? 0
                                  : depth(commit_data.node) + 1;
                }
    ...
    //!   the "header"'s tree.
    //!
    //! <b>Effects</b>: Inserts new_node into the tree, using "hint" as a hint
    //! to
    ...
    static node_ptr insert_equal(node_ptr h, node_ptr hint, node_ptr new_node,
                                 NodePtrCompare comp
#ifndef BOOST_INTRUSIVE_DOXYGEN_INVOKED
                                 ,
                                 std::size_t* pdepth = 0
    ...
    //! <b>Throws</b>: If "comp" throws.
    template <class NodePtrCompare>
    static node_ptr insert_equal_upper_bound(node_ptr h, node_ptr new_node,
                                             NodePtrCompare comp
#ifndef BOOST_INTRUSIVE_DOXYGEN_INVOKED
                                             ,
                                             std::size_t* pdepth = 0
    ...
    //! <b>Throws</b>: If "comp" throws.
    template <class NodePtrCompare>
    static node_ptr insert_equal_lower_bound(node_ptr h, node_ptr new_node,
                                             NodePtrCompare comp
#ifndef BOOST_INTRUSIVE_DOXYGEN_INVOKED
                                             ,
                                             std::size_t* pdepth = 0
    ...
    //!   once inserted according to the order of already inserted nodes. This
    //!   function does not check "pos" and this precondition must be guaranteed
    //!   by the caller.
    //!
    ...
    //! <b>Throws</b>: Nothing.
    //!
    //! <b>Note</b>: If "pos" is not the successor of the newly inserted
    //! "new_node" tree invariants might be broken.
    static node_ptr insert_before(node_ptr header, node_ptr pos,
                                  node_ptr new_node
#ifndef BOOST_INTRUSIVE_DOXYGEN_INVOKED
                                  ,
                                  std::size_t* pdepth = 0
    ...
    //! <b>Requires</b>: "header" must be the header node of a tree.
    //!   "new_node" must be, according to the used ordering, no greater than
    //!   the lowest inserted key.
    //!
    //! <b>Effects</b>: Inserts new_node into the tree before "pos".
    //!
    //! <b>Complexity</b>: Constant-time.
    ...
    static std::size_t depth(const_node_ptr node)
    {
        ...
        node_ptr p_parent;
        while (node !=
               NodeTraits::get_parent(p_parent = NodeTraits::get_parent(node)))
        {
            ++depth;
            node = p_parent;
        }
    ...
    //!   object taking a node_ptr and returning a new cloned node of it.
    ...
    //!    except the header.
    //!
    //!   Then, duplicates the entire tree pointed by "source_header" cloning
    //!   each source node with <tt>node_ptr Cloner::operator()(const node_ptr
    //!   &)</tt> to obtain the nodes of the target tree. If "cloner" throws,
    //!   the cloned target nodes are disposed using <tt>void disposer(const
    //!   node_ptr &)</tt>.
    //!
    //! <b>Complexity</b>: Linear to the number of element of the source tree
    //! plus the
                      ...
                      node_ptr target_header, Cloner cloner, Disposer disposer)
    {
        ...
        node_ptr new_root = clone_subtree(source_header, target_header, cloner,
                                          disposer, leftmost, rightmost);
    ...
    //! <b>Requires</b>: header1 and header2 must be the headers of trees tree1
    //! and tree2
    //!   respectively, z a non-header node of tree1. NodePtrCompare is the
    //!   comparison function of tree1..
    //!
    //! <b>Effects</b>: Transfers node "z" from tree1 to tree2 if tree1 does not
    //! contain
    ...
    template <class NodePtrCompare>
    BOOST_INTRUSIVE_FORCEINLINE static bool
        transfer_unique(node_ptr header1, NodePtrCompare comp, node_ptr header2,
                        node_ptr z)
    {
        data_for_rebalance ignored;
    ...
    template <class NodePtrCompare>
    BOOST_INTRUSIVE_FORCEINLINE static void
        transfer_equal(node_ptr header1, NodePtrCompare comp, node_ptr header2,
                       node_ptr z)
    {
        data_for_rebalance ignored;
    ...
    static node_ptr rebalance_subtree(node_ptr old_root)
    {
        ...
        bool super_root_is_header =
            NodeTraits::get_parent(super_root) == old_root;
                      ...
                      typename Checker::return_type& checker_return)
    {
        ...
        if (!root_node_ptr)
        {
            ...
            BOOST_INTRUSIVE_INVARIANT_ASSERT(NodeTraits::get_left(header) ==
                                             header);
            BOOST_INTRUSIVE_INVARIANT_ASSERT(NodeTraits::get_right(header) ==
                                             header);
        }
        else
        {
            ...
            while (NodeTraits::get_left(p))
            {
                p = NodeTraits::get_left(p);
            }
            BOOST_INTRUSIVE_INVARIANT_ASSERT(NodeTraits::get_left(header) == p);
            p = root_node_ptr;
            while (NodeTraits::get_right(p))
            {
                p = NodeTraits::get_right(p);
            }
            BOOST_INTRUSIVE_INVARIANT_ASSERT(NodeTraits::get_right(header) ==
                                             p);
        }
  ...
  protected:
                                ...
                                data_for_rebalance& info)
    {
        ...
        bool const transferable =
            insert_unique_check(header1, z, comp, commit_data).second;
        if (transferable)
        {
    ...
    static void erase(node_ptr header, node_ptr z, data_for_rebalance& info)
    {
        ...
        if (y != z)
        { // has two children and y is the minimum of z
            ...
            if (y != z_right)
            {
                ...
                // Since y was the successor and not the right child of z, it
                // must be a left child
                NodeTraits::set_left(x_parent, x);
            }
            else
            { // y was the right child of y so no need to fix x's position
                x_parent = y;
            }
        ...
        { // z has zero or one child, x is one child (it can be null)
        ...
        // If z had 2 children, y is the successor of z and x is the right child
        // of y
        info.x = x;
        info.y = y;
        // If z had 0/1 child, x_parent is the new parent of the old right child
        // of y (z's successor) If z had 2 children, x_parent is the new parent
        // of y (z_parent)
    ...
    static void insert_before_check(node_ptr header, node_ptr pos,
                                    insert_commit_data& commit_data
#ifndef BOOST_INTRUSIVE_DOXYGEN_INVOKED
                                    ,
                                    std::size_t* pdepth = 0
    ...
    )
    {
        ...
        {
            *pdepth =
                commit_data.node == header ? 0 : depth(commit_data.node) + 1;
        }
    }

    static void push_back_check(node_ptr header, insert_commit_data& commit_data
#ifndef BOOST_INTRUSIVE_DOXYGEN_INVOKED
                                ,
                                std::size_t* pdepth = 0
#endif
    ...
    static void push_front_check(node_ptr header,
                                 insert_commit_data& commit_data
#ifndef BOOST_INTRUSIVE_DOXYGEN_INVOKED
                                 ,
                                 std::size_t* pdepth = 0
    ...
    static void insert_equal_check(node_ptr header, node_ptr hint,
                                   node_ptr new_node, NodePtrCompare comp,
                                   insert_commit_data& commit_data
                                   /// @cond
                                   ,
                                   std::size_t* pdepth = 0
    ...
    )
    {
        if (hint == header || !comp(hint, new_node))
        {
            ...
            {
                insert_equal_upper_bound_check(header, new_node, comp,
                                               commit_data, pdepth);
            }
        }
        else
        {
            insert_equal_lower_bound_check(header, new_node, comp, commit_data,
                                           pdepth);
        }
    }

    template <class NodePtrCompare>
    static void insert_equal_upper_bound_check(node_ptr h, node_ptr new_node,
                                               NodePtrCompare comp,
                                               insert_commit_data& commit_data,
                                               std::size_t* pdepth = 0)
    {
        ...
        while (x)
        {
            ++depth;
            y = x;
            x = comp(new_node, x) ? NodeTraits::get_left(x)
    ...
    template <class NodePtrCompare>
    static void insert_equal_lower_bound_check(node_ptr h, node_ptr new_node,
                                               NodePtrCompare comp,
                                               insert_commit_data& commit_data,
                                               std::size_t* pdepth = 0)
    {
        ...
        while (x)
        {
            ++depth;
            y = x;
            x = !comp(x, new_node) ? NodeTraits::get_left(x)
                                   : NodeTraits::get_right(x);
        }
        if (pdepth)
            *pdepth = depth;
    ...
    static void insert_commit(node_ptr header, node_ptr new_node,
                              const insert_commit_data& commit_data)
    {
    ...
    // Fix header and own's parent data when replacing x with own, providing
    // own's old data with parent
    static void set_child(node_ptr header, node_ptr new_child,
                          node_ptr new_parent, const bool link_left)
    {
    ...
    static void rotate_left(node_ptr p, node_ptr p_right, node_ptr p_parent,
                            node_ptr header)
    {
    ...
    static void rotate_right(node_ptr p, node_ptr p_left, node_ptr p_parent,
                             node_ptr header)
    {
        const bool p_was_left(NodeTraits::get_left(p_parent) == p);
  ...
  private:
    ...
    {
        while (count--)
        { // compress "count" spine nodes in the tree with pseudo-root scanner
    ...
    static void vine_to_subtree(node_ptr super_root, std::size_t count)
    {
        ...
        std::size_t leaf_nodes =
            count + one_szt -
            std::size_t(one_szt << detail::floor_log2(count + one_szt));
        ...
        for (node_ptr q = super_root, p = NodeTraits::get_right(super_root); p;
    ...
    template <class Cloner, class Disposer>
    static node_ptr clone_subtree(const const_node_ptr& source_parent,
                                  node_ptr target_parent, Cloner cloner,
                                  ...
                                  node_ptr& rightmost_out)
    {
        ...
        else
        {
            ...
            dispose_subtree_disposer<Disposer> rollback(disposer,
                                                        target_sub_root);
            while (true)
            {
...
template <class ValueTraits, class NodePtrCompare, class ExtraChecker>
struct get_node_checker<BsTreeAlgorithms, ValueTraits, NodePtrCompare,
                        ExtraChecker>
{
    typedef detail::bstree_node_checker<ValueTraits, NodePtrCompare,
                                        ExtraChecker>
        type;
};
