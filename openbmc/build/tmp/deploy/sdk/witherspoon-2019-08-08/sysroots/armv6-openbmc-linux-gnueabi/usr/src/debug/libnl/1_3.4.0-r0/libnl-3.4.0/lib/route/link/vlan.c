                      ...
                      struct nlattr* xstats)
{
    ...
    if (tb[IFLA_VLAN_INGRESS_QOS])
    {
        ...
        nla_for_each_nested(nla, tb[IFLA_VLAN_INGRESS_QOS], remaining)
        {
             ...
             * That is similar to what we do with the egress map and it
             * preserves previous behavior before
             * NL_CAPABILITY_RTNL_LINK_VLAN_INGRESS_MAP_CLEAR.
             *
             * It matters only when a received object is send back to kernel to
             * modify the link.
             */
    ...
    if (tb[IFLA_VLAN_EGRESS_QOS])
    {
        ...
        vi->vi_egress_qos =
            calloc(vi->vi_egress_size, sizeof(*vi->vi_egress_qos));
...
static void vlan_dump_details(struct rtnl_link* link, struct nl_dump_params* p)
{
    ...
    if (vi->vi_mask & VLAN_HAS_INGRESS_QOS)
    {
        ...
        for (i = 0, printed = 0; i <= VLAN_PRIO_MAX; i++)
        {
            if (vi->vi_ingress_qos_mask & (1 << i))
            {
                if (printed == 0)
                    nl_dump_line(p, "      ");
                nl_dump(p, "%x -> %#08x, ", i, vi->vi_ingress_qos[i]);
                if (printed++ == 3)
                {
                    nl_dump(p, "\n");
                    printed = 0;
    ...
    {
        nl_dump_line(p, "      egress qos/socket prio -> vlan prio mapping:\n");
        for (i = 0, printed = 0; i < vi->vi_negress; i++)
        {
            if (printed == 0)
                nl_dump_line(p, "      ");
            nl_dump(p, "%#08x -> %x, ", vi->vi_egress_qos[i].vm_from,
...
static int vlan_clone(struct rtnl_link* dst, struct rtnl_link* src)
{
    ...
    vdst->vi_egress_qos = calloc(vsrc->vi_egress_size, sizeof(struct vlan_map));
