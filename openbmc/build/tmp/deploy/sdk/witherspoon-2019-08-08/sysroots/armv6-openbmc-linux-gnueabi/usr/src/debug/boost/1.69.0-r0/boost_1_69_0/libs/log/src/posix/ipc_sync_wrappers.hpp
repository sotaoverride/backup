...
#include <cerrno>
#include <cstddef>
// Use Boost.Interprocess to detect if process-shared pthread primitives are
// supported
...
struct pthread_mutex_attributes
{
    ...
    pthread_mutex_attributes()
    {
        ...
        if (BOOST_UNLIKELY(err != 0))
            BOOST_LOG_THROW_DESCR_PARAMS(
                ...
                "Failed to initialize pthread mutex attributes", (err));
    }
    ...
    BOOST_DELETED_FUNCTION(
        pthread_mutex_attributes(pthread_mutex_attributes const&))
    BOOST_DELETED_FUNCTION(
        pthread_mutex_attributes& operator=(pthread_mutex_attributes const&))
};
...
struct pthread_condition_variable_attributes
{
    ...
    pthread_condition_variable_attributes()
    {
        ...
        if (BOOST_UNLIKELY(err != 0))
            BOOST_LOG_THROW_DESCR_PARAMS(
                boost::log::system_error,
                "Failed to initialize pthread condition variable attributes",
                (err));
    }
    ...
    BOOST_DELETED_FUNCTION(pthread_condition_variable_attributes(
        pthread_condition_variable_attributes const&))
    BOOST_DELETED_FUNCTION(pthread_condition_variable_attributes& operator=(
        pthread_condition_variable_attributes const&))
};
...
struct interprocess_mutex
{
    struct auto_unlock
    {
        explicit auto_unlock(interprocess_mutex& mutex) BOOST_NOEXCEPT
            : m_mutex(mutex)
        {
        }
        ~auto_unlock()
    ...
    interprocess_mutex()
    {
        ...
        if (BOOST_UNLIKELY(err != 0))
            BOOST_LOG_THROW_DESCR_PARAMS(boost::log::system_error,
                                         "Failed to set pthread mutex type",
                                         (err));
        err =
            pthread_mutexattr_setpshared(&attrs.attrs, PTHREAD_PROCESS_SHARED);
        if (BOOST_UNLIKELY(err != 0))
            BOOST_LOG_THROW_DESCR_PARAMS(
                boost::log::system_error,
                "Failed to make pthread mutex process-shared", (err));
#if defined(BOOST_LOG_HAS_PTHREAD_MUTEX_ROBUST)
        err = pthread_mutexattr_setrobust(&attrs.attrs, PTHREAD_MUTEX_ROBUST);
        if (BOOST_UNLIKELY(err != 0))
            BOOST_LOG_THROW_DESCR_PARAMS(boost::log::system_error,
                                         "Failed to make pthread mutex robust",
                                         (err));
#endif
    ...
    void lock()
    {
        ...
        if (BOOST_UNLIKELY(err != 0))
            BOOST_LOG_THROW_DESCR_PARAMS(boost::log::system_error,
                                         "Failed to lock pthread mutex", (err));
    }
    ...
    void recover()
    {
        ...
        if (BOOST_UNLIKELY(err != 0))
            BOOST_LOG_THROW_DESCR_PARAMS(
                boost::log::system_error,
                "Failed to recover pthread mutex from a crashed thread", (err));
    }
#endif

    BOOST_DELETED_FUNCTION(interprocess_mutex(interprocess_mutex const&))
    BOOST_DELETED_FUNCTION(
        interprocess_mutex& operator=(interprocess_mutex const&))
};
...
struct interprocess_condition_variable
{
    ...
    interprocess_condition_variable()
    {
        ...
        int err =
            pthread_condattr_setpshared(&attrs.attrs, PTHREAD_PROCESS_SHARED);
        if (BOOST_UNLIKELY(err != 0))
            BOOST_LOG_THROW_DESCR_PARAMS(
                boost::log::system_error,
                "Failed to make pthread condition variable process-shared",
                (err));
        ...
        if (BOOST_UNLIKELY(err != 0))
            BOOST_LOG_THROW_DESCR_PARAMS(
                boost::log::system_error,
                "Failed to initialize pthread condition variable", (err));
    }
    ...
    void notify_one()
    {
        ...
        if (BOOST_UNLIKELY(err != 0))
            BOOST_LOG_THROW_DESCR_PARAMS(
                boost::log::system_error,
                "Failed to notify one thread on a pthread condition variable",
                (err));
    }
    ...
    void notify_all()
    {
        ...
        if (BOOST_UNLIKELY(err != 0))
            BOOST_LOG_THROW_DESCR_PARAMS(
                boost::log::system_error,
                "Failed to notify all threads on a pthread condition variable",
                (err));
    }
    ...
    void wait(interprocess_mutex& mutex)
    {
        ...
        if (BOOST_UNLIKELY(err != 0))
            BOOST_LOG_THROW_DESCR_PARAMS(
                boost::log::system_error,
                "Failed to wait on a pthread condition variable", (err));
    }

    BOOST_DELETED_FUNCTION(
        interprocess_condition_variable(interprocess_condition_variable const&))
    BOOST_DELETED_FUNCTION(interprocess_condition_variable& operator=(
        interprocess_condition_variable const&))
};

#else // defined(BOOST_INTERPROCESS_POSIX_PROCESS_SHARED)

// If there are no process-shared pthread primitives, use whatever emulation
// Boost.Interprocess implements
struct interprocess_mutex
{
    struct auto_unlock
    {
    ...
    mutex_type* mutex() BOOST_NOEXCEPT
    {
        return &m_mutex;
    }
