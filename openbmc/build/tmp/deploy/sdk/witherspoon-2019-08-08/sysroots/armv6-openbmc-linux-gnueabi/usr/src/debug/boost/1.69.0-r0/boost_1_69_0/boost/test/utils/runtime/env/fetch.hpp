...
//  Version     : $Revision$
//
//  Description : implements fetching absent parameter athuments from
//  environment
// ***************************************************************************
...
inline std::pair<cstring, bool> sys_read_var(cstring var_name)
{
    using namespace std;
...
inline void fetch_absent(parameters_store const& params,
                         runtime::arguments_store& args, ReadFunc read_func)
{
                       ...
                       params.all())
    {
        ...
        BOOST_TEST_I_ASSRT(!value.first.is_empty() ||
                               param->p_has_optional_value,
                           format_error(param->p_name)
                               << "Missing an argument value for the parameter "
                               << param->p_name << " in the environment.");
