...
/* Most of the Windows version here is based on
...
#define IS_REGULAR_FILE_HAS_ATTRIBUTE_INTEGRITY_STREAM(dwFileAttributes)       \
...
#define IS_REGULAR_FILE_HAS_ATTRIBUTE_NO_SCRUB_DATA(dwFileAttributes)          \
...
#define IS_FOLDER(dwFileAttributes)                                            \
    (!!(dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY))
...
int CreateFileListInfo(FileListInfoPtr pFileListInfo, char* path, int flag)
{
    ...
    /* While we can find a next file do...
       But ignore \. and '.. entries, which are current folder and parent folder
       respectively */
    while (
        findHandle != INVALID_HANDLE_VALUE && winFindData.cFileName[0] == '.' &&
         ...
         (winFindData.cFileName[1] == '.' && winFindData.cFileName[2] == '\0')))
    {
        ...
        strncpy_s(fullpath + basePathLength, PATH_MAX - basePathLength,
                  winFindData.cFileName, (int)strlen(winFindData.cFileName));

        if (IS_FOLDER(winFindData.dwFileAttributes))
        {
            if (AddFileListItemInfo(pFileListInfo, winFindData.cFileName, -1,
                                    0) == 0)
            {
        ...
        else if (IS_REGULAR_FILE(winFindData.dwFileAttributes))
        {
            if (flag)
            {
                unsigned int fileSize =
                    (winFindData.nFileSizeHigh * (MAXDWORD + 1)) +
                    winFindData.nFileSizeLow;
                if (AddFileListItemInfo(
                        pFileListInfo, winFindData.cFileName, fileSize,
                        FILETIME_TO_TIME_T(winFindData.ftLastWriteTime)) == 0)
                {
    ...
    while ((pDirent = readdir(pDir)))
    {
        if (strcmp(pDirent->d_name, ".") && strcmp(pDirent->d_name, ".."))
        {
            ...
            */
            ...
            {
                if (AddFileListItemInfo(pFileListInfo, pDirent->d_name, -1,
                                        0) == 0)
                {
                    rfbLog("File [%s]: Method [%s]: Add directory %s in the"
                           " list failed\n",
...
FileTransferMsg CreateFileListMsg(FileListInfo fileListInfo, char flags)
{
    ...
    length = sz_rfbFileListDataMsg + dsSize +
             GetSumOfFileNamesLength(fileListInfo) + fileListInfo.numEntries;
                                                   ...
                                                   rfbTightClientPtr rtcp)
{
    ...
    if ((rtcp->rcft.rcfd.downloadInProgress == FALSE) &&
    ...
    {
        if ((numOfBytesRead =
                 read(rtcp->rcft.rcfd.downloadFD, pBuf, SZ_RFBBLOCKSIZE)) <= 0)
        {
...
FileTransferMsg ChkFileDownloadErr(rfbClientPtr cl, rfbTightClientPtr rtcp)
{
    ...
    if ((path == NULL) || (strlen(path) == 0) || (stat(path, &stat_buf) < 0) ||
    ...
    else
    {
        ...
        {
            fileDownloadMsg =
                CreateFileDownloadZeroSizeDataMsg(stat_buf.st_mtime);
        }
    }
...
FileTransferMsg ChkFileUploadErr(rfbClientPtr cl, rfbTightClientPtr rtcp)
{
    ...
    if ((rtcp->rcft.rcfu.fName == NULL) ||
        ...
        ((rtcp->rcft.rcfu.uploadFD = creat(
              rtcp->rcft.rcfu.fName, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP |
                                         S_IROTH | S_IWOTH)) == -1))
    {
...
FileTransferMsg ChkFileUploadWriteErr(rfbClientPtr cl, rfbTightClientPtr rtcp,
                                      char* pBuf)
{
    FileTransferMsg ftm;
    ...
    numOfBytesWritten =
        write(rtcp->rcft.rcfu.uploadFD, pBuf, rtcp->rcft.rcfu.fSize);
...
void CloseUndoneFileUpload(rfbClientPtr cl, rfbTightClientPtr rtcp)
{
    ...
    if (rtcp->rcft.rcfu.uploadInProgress == TRUE)
    {
        ...
        {
            rfbLog("File [%s]: Method [%s]: Delete operation on file <%s> "
                   "failed\n",
                   __FILE__, __FUNCTION__, rtcp->rcft.rcfu.fName);
        }
...
void CreateDirectory(char* dirName)
{
    ...
    {
        rfbLog("File [%s]: Method [%s]: Create operation for directory <%s> "
               "failed\n",
