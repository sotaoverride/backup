...
static struct prlimit_desc prlimit_desc[] = {
    ...
    [LOCKS] = {"LOCKS", N_("max number of file locks held"), N_("locks"),
               RLIMIT_LOCKS},
    [MEMLOCK] = {"MEMLOCK", N_("max locked-in-memory address space"),
                 N_("bytes"), RLIMIT_MEMLOCK},
    [MSGQUEUE] = {"MSGQUEUE", N_("max bytes in POSIX mqueues"), N_("bytes"),
                  RLIMIT_MSGQUEUE},
    [NICE] = {"NICE", N_("max nice prio allowed to raise"), NULL, RLIMIT_NICE},
    [NOFILE] = {"NOFILE", N_("max number of open files"), N_("files"),
                RLIMIT_NOFILE},
    [NPROC] = {"NPROC", N_("max number of processes"), N_("processes"),
               RLIMIT_NPROC},
    [RSS] = {"RSS", N_("max resident set size"), N_("bytes"), RLIMIT_RSS},
    [RTPRIO] = {"RTPRIO", N_("max real-time priority"), NULL, RLIMIT_RTPRIO},
    [RTTIME] = {"RTTIME", N_("timeout for real-time tasks"), N_("microsecs"),
                RLIMIT_RTTIME},
    [SIGPENDING] = {"SIGPENDING", N_("max number of pending signals"),
                    N_("signals"), RLIMIT_SIGPENDING},
...
static struct colinfo infos[] = {
    ...
    [COL_HELP] = {"DESCRIPTION", 0.1, SCOLS_FL_TRUNC,
...
static int prlimit(pid_t p, int resource, const struct rlimit* new_limit,
...
static void __attribute__((__noreturn__)) usage(void)
{
    ...
    fputs(_(" -c, --core             maximum size of core files created\n"
            ...
            " -f, --fsize            maximum size of files written by the "
            "process\n"
            " -i, --sigpending       maximum number of pending signals\n"
            " -l, --memlock          maximum size a process may lock into "
            "memory\n"
            ...
            " -y, --rttime           CPU time in microseconds a process "
            "scheduled\n"
        ...
        switch (get_column_id(i))
        {
            ...
            case COL_SOFT:
                if (l->rlim.rlim_cur == RLIM_INFINITY)
                    ...
                    xasprintf(&str, "%llu",
                              (unsigned long long)l->rlim.rlim_cur);
                break;
            case COL_HARD:
                if (l->rlim.rlim_max == RLIM_INFINITY)
                    ...
                    xasprintf(&str, "%llu",
                              (unsigned long long)l->rlim.rlim_max);
                break;
            case COL_UNITS:
...
static void get_unknown_hardsoft(struct prlimit* lim)
{
    ...
    if (prlimit(pid, lim->desc->resource, NULL, &old) == -1)
        err(EXIT_FAILURE, _("failed to get old %s limit"), lim->desc->name);
...
static void do_prlimit(struct list_head* lims)
{
    ...
    list_for_each_safe(p, pnext, lims)
    {
        ...
        if (prlimit(pid, lim->desc->resource, new, old) == -1)
            err(EXIT_FAILURE,
                lim->modify ? _("failed to set the %s resource limit")
