...
{
    static const widelimb two64p8 =
        (((widelimb)1) << 64) + (((widelimb)1) << 8);
    static const widelimb two64m8 =
        (((widelimb)1) << 64) - (((widelimb)1) << 8);
 ...
 * X_1)^2 - X_3) - Z_2^3 * Y_1 * (Z_1^2 * X_2 - Z_2^2 * X_1)^3 Z_3 = (Z_1^2 *
 * X_2 - Z_2^2 * X_1) * (Z_1 * Z_2)
 *
    ...
    for (i = (num_points ? 220 : 27); i >= 0; --i)
    {
        ...
        if (gen_mul && (i <= 27))
        {
            ...
            if (!skip)
            {
                ...
                point_add(nq[0], nq[1], nq[2], nq[0], nq[1], nq[2], 1, tmp[0],
                          tmp[1], tmp[2]);
            }
            else
            {
                               ...
                               felem tmp_felems[/* num+1 */])
{
    ...
    ec_GFp_nistp_points_make_affine_internal(
        ...
        felem_is_zero_int, (void (*)(void*, const void*))felem_assign,
                               ...
                               const BIGNUM* scalars[], BN_CTX* ctx)
{
        ...
        for (i = 0; i < num_points; ++i)
        {
            ...
            if ((p_scalar != NULL) && (p != NULL))
            {
                ...
                if ((!BN_to_felem(x_out, p->X)) ||
                    (!BN_to_felem(y_out, p->Y)) || (!BN_to_felem(z_out, p->Z)))
                    goto err;
    ...
    if ((scalar != NULL) && (have_pre_comp))
    {
        ...
        batch_mul(x_out, y_out, z_out, (const felem_bytearray(*))secrets,
                  num_points, NULL, mixed, (const felem(*)[17][3])pre_comp,
                  NULL);
...
int ec_GFp_nistp224_precompute_mult(EC_GROUP* group, BN_CTX* ctx)
{
    ...
    for (i = 1; i <= 8; i <<= 1)
    {
        ...
        if (i == 8)
            break;
        point_double(pre->g_pre_comp[0][2 * i][0], pre->g_pre_comp[0][2 * i][1],
                     pre->g_pre_comp[0][2 * i][2], pre->g_pre_comp[1][i][0],
                     pre->g_pre_comp[1][i][1], pre->g_pre_comp[1][i][2]);
        for (j = 0; j < 27; ++j)
        {
            point_double(
                pre->g_pre_comp[0][2 * i][0], pre->g_pre_comp[0][2 * i][1],
                pre->g_pre_comp[0][2 * i][2], pre->g_pre_comp[0][2 * i][0],
                pre->g_pre_comp[0][2 * i][1], pre->g_pre_comp[0][2 * i][2]);
        }
    }
    for (i = 0; i < 2; i++)
    {
        ...
        point_add(pre->g_pre_comp[i][14][0], pre->g_pre_comp[i][14][1],
                  ...
                  pre->g_pre_comp[i][12][1], pre->g_pre_comp[i][12][2], 0,
                  pre->g_pre_comp[i][2][0], pre->g_pre_comp[i][2][1],
                  pre->g_pre_comp[i][2][2]);
        for (j = 1; j < 8; ++j)
        {
            ...
            point_add(pre->g_pre_comp[i][2 * j + 1][0],
                      ...
                      pre->g_pre_comp[i][2 * j][2], 0, pre->g_pre_comp[i][1][0],
                      pre->g_pre_comp[i][1][1], pre->g_pre_comp[i][1][2]);
        }
    }
