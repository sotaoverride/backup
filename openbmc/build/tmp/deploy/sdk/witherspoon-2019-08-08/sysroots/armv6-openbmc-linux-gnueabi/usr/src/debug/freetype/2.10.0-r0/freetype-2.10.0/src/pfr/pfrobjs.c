...
pfr_face_init(FT_Stream stream, FT_Face pfrface, FT_Int face_index,
              FT_Int num_params, FT_Parameter* params)
{
    ...
    /* check face index */
    {
        ...
        error =
            pfr_log_font_count(stream, face->header.log_dir_offset, &num_faces);
        if (error)
            goto Exit;
    ...
    /* load the face */
    error = pfr_log_font_load(&face->log_font, stream,
    ...
    error =
        pfr_phy_font_load(&face->phy_font, stream, face->log_font.phys_offset,
              ...
              FT_UInt gindex, FT_Int32 load_flags)
{
    ...
    error = pfr_glyph_load(&slot->glyph, face->root.stream, gps_offset,
                           gchar->gps_offset, gchar->gps_size);
    ...
    if (!error)
    {
        ...
        *outline = slot->glyph.loader->base.outline;
        ...
        if (em_metrics != em_outline)
            advance =
                FT_MulDiv(advance, (FT_Long)em_outline, (FT_Long)em_metrics);
