...
static FT_Error ft_gzip_check_header(FT_Stream stream)
{
    ...
    if (head[0] != 0x1F || head[1] != 0x8B || head[2] != Z_DEFLATED ||
...
static FT_Error ft_gzip_file_init(FT_GZipFile zip, FT_Stream stream,
                                  FT_Stream source)
{
    ...
    if (inflateInit2(zstream, -MAX_WBITS) != Z_OK || !zstream->next_in)
        error = FT_THROW(Invalid_File_Format);
...
static FT_Error ft_gzip_file_fill_input(FT_GZipFile zip)
{
    ...
    {
        size =
            stream->read(stream, stream->pos, zip->input, FT_GZIP_BUFFER_SIZE);
        if (size == 0)
        {
...
static FT_ULong ft_gzip_file_io(FT_GZipFile zip, FT_ULong pos, FT_Byte* buffer,
                                FT_ULong count)
{
    FT_ULong result = 0;
    FT_Error error;
...
static unsigned long ft_gzip_stream_io(FT_Stream stream, unsigned long offset,
