...
/* Identical to linux/capability.h's CAP_TO_MASK(), but uses an unsigned 1U
 * instead of a signed 1 for shifting left, in order to avoid complaints about
 * shifting a signed int left by 31 bits, which would make it negative. */
#define CAP_TO_MASK_CORRECTED(x) (1U << ((x)&31U))

typedef struct CapabilityQuintet
{
    /* Stores all five types of capabilities in one go. Note that we use
     * (uint64_t) -1 for unset here. This hence needs to be updated as soon as
...
{
    return q->effective != (uint64_t)-1 || q->bounding != (uint64_t)-1 ||
           q->inheritable != (uint64_t)-1 || q->permitted != (uint64_t)-1 ||
