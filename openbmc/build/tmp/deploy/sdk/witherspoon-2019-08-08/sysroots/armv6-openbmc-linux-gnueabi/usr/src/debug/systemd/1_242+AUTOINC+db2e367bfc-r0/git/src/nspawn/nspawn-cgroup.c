...
static int chown_cgroup_path(const char* path, uid_t uid_shift)
{
    ...
    FOREACH_STRING(fn, ".", "cgroup.clone_children", "cgroup.controllers",
                   "cgroup.events", "cgroup.procs", "cgroup.stat",
...
int chown_cgroup(pid_t pid, CGroupUnified unified_requested, uid_t uid_shift)
{
    ...
    if (r < 0)
        return log_error_errno(
            r, "Failed to get file system path for container cgroup: %m");
    ...
    if (unified_requested == CGROUP_UNIFIED_SYSTEMD ||
        (unified_requested == CGROUP_UNIFIED_NONE &&
         cg_unified_controller(SYSTEMD_CGROUP_CONTROLLER) > 0))
    {
...
int sync_cgroup(pid_t pid, CGroupUnified unified_requested, uid_t uid_shift)
{
    ...
    if (r < 0)
        return log_error_errno(
            r, "Failed to get control group of " PID_FMT ": %m", pid);

    /* In order to access the unified hierarchy we need to mount it */
    if (!mkdtemp(tree))
        return log_error_errno(errno, "Failed to generate temporary mount "
                                      "point for unified hierarchy: %m");
...
int create_subcgroup(pid_t pid, bool keep_unit, CGroupUnified unified_requested)
{
    ...
    /* In the unified hierarchy inner nodes may only contain subgroups, but not
     * processes. Hence, if we running in the unified hierarchy and the
     * container does the same, and we did not create a scope unit for the
     * container move us and the container into two separate subcgroups.
     *
     * Moreover, container payloads such as systemd try to manage the cgroup
     * they run in in full (i.e. including its attributes), while the host
     * systemd will only delegate cgroups for children of the cgroup created for
     * a delegation unit, instead of the cgroup itself. This means, if we'd pass
     * on the cgroup allocated from the host systemd directly to the payload,
     * the host and payload systemd might fight for the cgroup attributes.
     * Hence, let's insert an intermediary cgroup to cover that case too.
     *
     * Note that we only bother with the main hierarchy here, not with any
     * secondary ones. On the unified setup that's fine because there's only one
     * hiearchy anyway and controllers are enabled directly on it. On the legacy
     * setup, this is fine too, since delegation of controllers is generally not
     * safe there, hence we won't do it. */
    ...
    if (keep_unit)
    {
        ...
        if (r < 0)
            return log_error_errno(r, "Failed to create %s subcgroup: %m",
                                   supervisor);
    }
...
static int mount_legacy_cgroup_hierarchy(const char* dest,
                                         ...
                                         const char* hierarchy, bool read_only)
{
    ...
    if (r < 0 && r != -ENOENT)
        return log_error_errno(
            r, "Failed to determine if %s is mounted already: %m", to);
    if (r > 0)
        return 0;
    ...
    r = mount_verbose(LOG_ERR, "cgroup", to, fstype,
    ...
    if (read_only)
    {
        r = mount_verbose(LOG_ERR, NULL, to, NULL,
                          MS_BIND | MS_REMOUNT | MS_NOSUID | MS_NOEXEC |
                              MS_NODEV | MS_RDONLY,
                          NULL);
        if (r < 0)
            return r;
                                       ...
                                       const char* selinux_apifs_context)
{
    ...
    if (r < 0)
        return log_error_errno(
            r, "Failed to determine if /sys/fs/cgroup is already mounted: %m");
    if (r == 0)
    {
    ...
    for (;;)
    {
        ...
        for (;;)
        {
            ...
            if (r < 0)
                return log_error_errno(
                    r, "Failed to extract co-mounted cgroup controller: %m");
            if (r == 0)
                break;
            ...
            if (r == -EINVAL)
                return log_error_errno(
                    r, "Invalid existing symlink for combined hierarchy: %m");
            if (r < 0)
                return log_error_errno(
                    r, "Failed to create symlink for combined hierarchy: %m");
        }
    }
...
skip_controllers:
    if (unified_requested >= CGROUP_UNIFIED_SYSTEMD)
    {
        r = mount_legacy_cgroup_hierarchy("", SYSTEMD_CGROUP_CONTROLLER_HYBRID,
                                          "unified", false);
        if (r < 0)
            return r;
    }

    r = mount_legacy_cgroup_hierarchy("", SYSTEMD_CGROUP_CONTROLLER_LEGACY,
                                      "systemd", false);
    if (r < 0)
        return r;
    ...
    if (!userns)
        return mount_verbose(LOG_ERR, NULL, cgroup_root, NULL,
                             MS_REMOUNT | MS_NOSUID | MS_NOEXEC | MS_NODEV |
                                 MS_STRICTATIME | MS_RDONLY,
                             "mode=755");
                                         ...
                                         const char* selinux_apifs_context)
{
    ...
    if (r == 0)
    {
        ...
        r = tmpfs_patch_options("mode=755",
    ...
    for (;;)
    {
        ...
        if (r == -EINVAL)
        {
            ...
            r = mount_legacy_cgroup_hierarchy(dest, controller, controller,
                                              true);
            if (r < 0)
                return r;
        }
        ...
        else
        {
            ...
            {
                log_warning("Ignoring invalid combined hierarchy %s.",
                            combined);
                continue;
            }
...
skip_controllers:
    if (unified_requested >= CGROUP_UNIFIED_SYSTEMD)
    {
        r = mount_legacy_cgroup_hierarchy(
            dest, SYSTEMD_CGROUP_CONTROLLER_HYBRID, "unified", false);
        if (r < 0)
            return r;
    }

    r = mount_legacy_cgroup_hierarchy(dest, SYSTEMD_CGROUP_CONTROLLER_LEGACY,
                                      "systemd", false);
    if (r < 0)
        return r;
...
static int mount_unified_cgroups(const char* dest)
{
    ...
    if (r < 0)
        return log_error_errno(
            r, "Failed to determine if %s is mounted already: %m", p);
    if (r > 0)
    {
        ...
        if (errno != ENOENT)
            return log_error_errno(errno,
                                   "Failed to determine if mount point %s "
                                   "contains the unified cgroup hierarchy: %m",
                                   p);

        return log_error_errno(SYNTHETIC_ERRNO(EINVAL),
                               "%s is already mounted but not a unified cgroup "
                               "hierarchy. Refusing.",
                               p);
    }

    return mount_verbose(LOG_ERR, "cgroup", p, "cgroup2",
                         MS_NOSUID | MS_NOEXEC | MS_NODEV, NULL);
}

int mount_cgroups(const char* dest, CGroupUnified unified_requested,
                  bool userns, uid_t uid_shift, uid_t uid_range,
                  const char* selinux_apifs_context, bool use_cgns)
{

    if (unified_requested >= CGROUP_UNIFIED_ALL)
        return mount_unified_cgroups(dest);
    if (use_cgns)
        return mount_legacy_cgns_supported(dest, unified_requested, userns,
                                           uid_shift, uid_range,
                                           selinux_apifs_context);

    return mount_legacy_cgns_unsupported(dest, unified_requested, userns,
                                         uid_shift, uid_range,
                                         selinux_apifs_context);
}
...
int mount_systemd_cgroup_writable(const char* dest,
                                  CGroupUnified unified_requested)
{
