...
G_DEFINE_TYPE_WITH_CODE(
    GTcpConnection, g_tcp_connection, G_TYPE_SOCKET_CONNECTION,
...
static gboolean g_tcp_connection_close(GIOStream* stream,
                                       ...
                                       GError** error);
static void g_tcp_connection_close_async(GIOStream* stream, int io_priority,
                                         GCancellable* cancellable,
...
static void g_tcp_connection_get_property(GObject* object, guint prop_id,
...
static void g_tcp_connection_set_property(GObject* object, guint prop_id,
                                          const GValue* value,
                                          GParamSpec* pspec)
{
    GTcpConnection* connection = G_TCP_CONNECTION(object);
    ...
    switch (prop_id)
    {
        case PROP_GRACEFUL_DISCONNECT:
            g_tcp_connection_set_graceful_disconnect(
                connection, g_value_get_boolean(value));
            break;
...
static void g_tcp_connection_class_init(GTcpConnectionClass* class)
{
    ...
    g_object_class_install_property(
        gobject_class, PROP_GRACEFUL_DISCONNECT,
        g_param_spec_boolean(
            "graceful-disconnect", P_("Graceful Disconnect"),
            P_("Whether or not close does a graceful disconnect"), FALSE,
        ...
        else
        {
            while (TRUE)
            {
                ret = g_socket_receive_with_blocking(
                    socket, buffer, sizeof(buffer), TRUE, cancellable, error);
                if (ret < 0)
                {
                    had_error = TRUE;
                    error = NULL;
...
/* consumes @error */
static void async_close_finish(GTask* task, GError* error)
{
...
static gboolean close_read_ready(GSocket* socket, GIOCondition condition,
                                 GTask* task)
{
    GError* error = NULL;
    char buffer[1024];
    ...
    ret = g_socket_receive_with_blocking(socket, buffer, sizeof(buffer), FALSE,
