...
cf2_blues_init(CF2_Blues blues, CF2_Font font)
{
    ...
    cf2_getBlueMetrics(decoder, &blues->blueScale, &blues->blueShift,
                       &blues->blueFuzz);
    ...
    if (cf2_getLanguageGroup(decoder) == 1 &&
        (numBlueValues == 0 ||
         (numBlueValues == 4 && cf2_blueToFixed(blueValues[0]) < emBoxBottom &&
          ...
          cf2_blueToFixed(blueValues[3]) > emBoxTop)))
    {
        ...
        blues->emBoxBottomEdge.dsCoord =
            cf2_fixedRound(
                FT_MulFix(blues->emBoxBottomEdge.csCoord, blues->scale)) -
            CF2_MIN_COUNTER;
        blues->emBoxBottomEdge.scale = blues->scale;
        blues->emBoxBottomEdge.flags =
            CF2_GhostBottom | CF2_Locked | CF2_Synthetic;

        blues->emBoxTopEdge.csCoord =
            emBoxTop + CF2_FIXED_EPSILON + 2 * font->darkenY;
        blues->emBoxTopEdge.dsCoord =
            cf2_fixedRound(
                FT_MulFix(blues->emBoxTopEdge.csCoord, blues->scale)) +
            CF2_MIN_COUNTER;
        blues->emBoxTopEdge.scale = blues->scale;
        blues->emBoxTopEdge.flags = CF2_GhostTop | CF2_Locked | CF2_Synthetic;
    ...
    {
        blues->zone[blues->count].csBottomEdge = cf2_blueToFixed(blueValues[i]);
        ...
        if (i == 0)
        {
            blues->zone[blues->count].bottomZone = TRUE;
            blues->zone[blues->count].csFlatEdge =
                blues->zone[blues->count].csTopEdge;
        }
        else
        {
            blues->zone[blues->count].bottomZone = FALSE;
            blues->zone[blues->count].csFlatEdge =
                blues->zone[blues->count].csBottomEdge;
        }
    ...
    {
        blues->zone[blues->count].csBottomEdge = cf2_blueToFixed(otherBlues[i]);
        blues->zone[blues->count].csTopEdge =
    ...
    if (maxZoneHeight > 0)
    {
        if (blues->blueScale > FT_DivFix(cf2_intToFixed(1), maxZoneHeight))
        {
            /* clamp at maximum scale */
            blues->blueScale = FT_DivFix(cf2_intToFixed(1), maxZoneHeight);
        }
    ...
    if (blues->scale < blues->blueScale)
    {
        ...
        blues->boost =
            cf2_doubleToFixed(.6) -
            FT_MulDiv(cf2_doubleToFixed(.6), blues->scale, blues->blueScale);
    ...
    for (i = 0; i < blues->count; i++)
    {
        if (blues->zone[i].bottomZone)
            blues->zone[i].dsFlatEdge = cf2_fixedRound(
                FT_MulFix(blues->zone[i].csFlatEdge, blues->scale) -
                blues->boost);
        else
            blues->zone[i].dsFlatEdge = cf2_fixedRound(
                FT_MulFix(blues->zone[i].csFlatEdge, blues->scale) +
...
cf2_blues_capture(const CF2_Blues blues, CF2_Hint bottomHintEdge,
                  CF2_Hint topHintEdge)
{
    ...
    for (i = 0; i < blues->count; i++)
    {
        if (blues->zone[i].bottomZone && cf2_hint_isBottom(bottomHintEdge))
        {
                    ...
                    ADD_INT32(blues->zone[i].csTopEdge, csFuzz))
            {
                ...
                if (blues->suppressOvershoot)
                    dsNew = blues->zone[i].dsFlatEdge;
                                   ...
                                   bottomHintEdge->csCoord) >= blues->blueShift)
                {
                    /* guarantee minimum of 1 pixel overshoot */
                    dsNew =
                        FT_MIN(cf2_fixedRound(bottomHintEdge->dsCoord),
        ...
        if (!blues->zone[i].bottomZone && cf2_hint_isTop(topHintEdge))
        {
                    ...
                    ADD_INT32(blues->zone[i].csTopEdge, csFuzz))
            {
                         ...
                         blues->blueShift)
                {
                    /* guarantee minimum of 1 pixel overshoot */
                    dsNew =
                        FT_MAX(cf2_fixedRound(topHintEdge->dsCoord),
                               blues->zone[i].dsFlatEdge + cf2_intToFixed(1));
                }
    ...
    if (captured)
    {
        ...
        {
            bottomHintEdge->dsCoord =
                ADD_INT32(bottomHintEdge->dsCoord, dsMove);
