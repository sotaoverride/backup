...
static void pty_forward_disconnect(PTYForward* f)
{
    ...
    /* STDIN/STDOUT should not be nonblocking normally, so let's unconditionally
     * reset it */
...
static bool look_for_escape(PTYForward* f, const char* buffer, size_t n)
{
    ...
    for (p = buffer; p < buffer + n; p++)
    {
        ...
        if (*p == 0x1D)
        {
            ...
            if (f->escape_counter == 0 ||
                nw > f->escape_timestamp + ESCAPE_USEC)
            {
...
static int shovel(PTYForward* f)
{
           ...
           (f->stdout_writable && f->out_buffer_full > 0))
    {
        ...
        if (f->stdin_readable && f->in_buffer_full < LINE_MAX)
        {
            ...
            k = read(STDIN_FILENO, f->in_buffer + f->in_buffer_full,
                     LINE_MAX - f->in_buffer_full);
            if (k < 0)
            {
            ...
            {
                /* Check if ^] has been pressed three times within one second.
                 * If we get this we quite immediately. */
        ...
        if (f->master_readable && f->out_buffer_full < LINE_MAX)
        {
            ...
            k = read(f->master, f->out_buffer + f->out_buffer_full,
                     LINE_MAX - f->out_buffer_full);
            if (k < 0)
            {
        ...
        if (f->stdout_writable && f->out_buffer_full > 0)
        {
            ...
            else
            {
                ...
                memmove(f->out_buffer, f->out_buffer + k,
                        f->out_buffer_full - k);
                f->out_buffer_full -= k;
            }
        }
    ...
    /* If we were asked to drain, and there's nothing more to handle from the
     * master, then call the callback too. */
...
static int on_master_event(sd_event_source* e, int fd, uint32_t revents,
                           void* userdata)
{
...
static int on_stdin_event(sd_event_source* e, int fd, uint32_t revents,
                          void* userdata)
{
    PTYForward* f = userdata;
...
static int on_stdout_event(sd_event_source* e, int fd, uint32_t revents,
                           void* userdata)
{
    PTYForward* f = userdata;
...
static int on_sigwinch_event(sd_event_source* e,
...
int pty_forward_new(sd_event* event, int master, PTYForwardFlags flags,
                    PTYForward** ret)
{
    ...
    *f = (struct PTYForward){
    ...
    if (ioctl(STDOUT_FILENO, TIOCGWINSZ, &ws) < 0)
    {
        /* If we can't get the resolution from the output fd, then use our
         * internal, regular width/height,
    ...
    if (!(flags & PTY_FORWARD_READ_ONLY))
    {
        ...
        r = sd_event_add_io(f->event, &f->stdin_event_source, STDIN_FILENO,
                            EPOLLIN | EPOLLET, on_stdin_event, f);
        if (r < 0 && r != -EPERM)
            return r;

        if (r >= 0)
            (void)sd_event_source_set_description(f->stdin_event_source,
                                                  "ptyfwd-stdin");
    }

    r = sd_event_add_io(f->event, &f->stdout_event_source, STDOUT_FILENO,
                        EPOLLOUT | EPOLLET, on_stdout_event, f);
    if (r == -EPERM)
        ...
        (void)sd_event_source_set_description(f->stdout_event_source,
                                              "ptyfwd-stdout");

    r = sd_event_add_io(f->event, &f->master_event_source, master,
                        EPOLLIN | EPOLLOUT | EPOLLET, on_master_event, f);
    if (r < 0)
        return r;
    ...
    r = sd_event_add_signal(f->event, &f->sigwinch_event_source, SIGWINCH,
                            on_sigwinch_event, f);
    if (r < 0)
        return r;

    (void)sd_event_source_set_description(f->sigwinch_event_source,
                                          "ptyfwd-sigwinch");
...
void pty_forward_set_handler(PTYForward* f, PTYForwardHandler cb,
                             void* userdata)
{
    assert(f);
...
bool pty_forward_drain(PTYForward* f)
{
     ...
     *
     * - Returns true if there are no unprocessed bytes from the pty, false
     * otherwise
     *
     * - Makes sure the handler function is called the next time the number of
     * unprocessed bytes hits zero
     */
