...
void rtnl_ematch_text_set_pattern(struct rtnl_ematch* e, char* pattern,
                                  size_t len)
{
    struct text_data* t = rtnl_ematch_data(e);
...
static void text_dump(struct rtnl_ematch* e, struct nl_dump_params* p)
{
    ...
    nl_dump(p, "text(%s \"%s\"", t->cfg.algo[0] ? t->cfg.algo : "no-algo",
            t->pattern ?: "no-pattern");

    if (t->cfg.from_layer || t->cfg.from_offset)
    {
        nl_dump(p, " from %s",
                rtnl_ematch_offset2txt(t->cfg.from_layer, t->cfg.from_offset,
                                       buf, sizeof(buf)));
    }
    ...
    {
        nl_dump(p, " to %s",
                rtnl_ematch_offset2txt(t->cfg.to_layer, t->cfg.to_offset, buf,
                                       sizeof(buf)));
    }
