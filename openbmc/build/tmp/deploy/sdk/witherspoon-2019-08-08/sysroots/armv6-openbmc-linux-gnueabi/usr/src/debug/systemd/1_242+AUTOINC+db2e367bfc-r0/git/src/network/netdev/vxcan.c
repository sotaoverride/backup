...
static int netdev_vxcan_fill_message_create(NetDev* netdev, Link* link,
                                            sd_netlink_message* m)
{
    ...
    if (r < 0)
        return log_netdev_error_errno(
            netdev, r, "Could not append VXCAN_INFO_PEER attribute: %m");
    ...
    if (v->ifname_peer)
    {
        ...
        if (r < 0)
            return log_error_errno(
                r, "Failed to add vxcan netlink interface peer name: %m");
    }
...
static int netdev_vxcan_verify(NetDev* netdev, const char* filename)
{
    ...
    if (!v->ifname_peer)
    {
        log_warning("VxCan NetDev without peer name configured in %s. Ignoring",
                    filename);
        return -EINVAL;
    }
