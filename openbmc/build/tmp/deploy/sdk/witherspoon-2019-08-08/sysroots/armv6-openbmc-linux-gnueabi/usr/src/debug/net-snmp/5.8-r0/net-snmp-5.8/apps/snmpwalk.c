...
void usage(void)
{
    ...
    fprintf(stderr, "\t\t\t  I:  don't include the given OID, even if no "
    ...
    while (running)
    {
        ...
        if (status == STAT_SUCCESS)
        {
            ...
            if (response->errstat == SNMP_ERR_NOERROR)
            {
                ...
                {
                    if (snmp_oid_compare(end_oid, end_len, vars->name,
                                         vars->name_length) <= 0)
                    {
                        /*
                    ...
                    if (netsnmp_ds_get_boolean(
                            ...
                            NETSNMP_DS_WALK_TIME_RESULTS_SINGLE))
                        fprintf(stdout, "%f s: ",
                                (double)(tv_b.tv_usec - tv_a.tv_usec) /
                                        1000000 +
                        ...
                        (vars->type != SNMP_NOSUCHINSTANCE))
                    {
                         ...
                         */
                        if (check &&
                            snmp_oid_compare(name, name_length, vars->name,
                                             vars->name_length) >= 0)
                        {
