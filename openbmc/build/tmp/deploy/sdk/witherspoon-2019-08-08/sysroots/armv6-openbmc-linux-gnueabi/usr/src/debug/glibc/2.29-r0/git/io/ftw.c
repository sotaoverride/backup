...
static const int nftw_arr[] = {FTW_F,  FTW_D,  FTW_DNR, FTW_NS,
                               FTW_SL, FTW_DP, FTW_SLN};

static const int ftw_arr[] = {FTW_F, FTW_D, FTW_DNR, FTW_NS,
                              FTW_F, FTW_D, FTW_NS};
...
static inline int find_object(struct ftw_data* data, struct STAT* st)
{
...
static int process_entry(struct ftw_data* data, struct dir_data* dir,
                         const char* name, size_t namlen, int d_type)
{
    struct STAT st;
    int result = 0;
    int flag = 0;
    ...
    *((char*)__mempcpy(data->dirbuf + data->ftw.base, name, namlen)) = '\0';
    ...
    else
    {
        ...
        statres = ((data->flags & FTW_PHYS) ? LXSTAT(_STAT_VER, name, &st)
    ...
    if (result == 0 && (flag == FTW_NS || !(data->flags & FTW_MOUNT) ||
                        st.st_dev == data->dev))
    {
        if (flag == FTW_D)
        {
...
ftw_dir(struct ftw_data* data, struct STAT* st, struct dir_data* old_dir)
{
    ...
    result = open_dir_stream(old_dir == NULL ? NULL : &old_dir->streamfd, data,
                             &dir);
    if (result != 0)
    {
        if (errno == EACCES)
            ...
            int flags)
{
    ...
    data.actdir = 0;
    data.dirstreams =
        (struct dir_data**)alloca(data.maxdir * sizeof(struct dir_data*));
    ...
    *cp = '\0';
    ...
    if (result == 0)
    {
        ...
        if (((flags & FTW_PHYS) ? LXSTAT(_STAT_VER, name, &st)
                                : XSTAT(_STAT_VER, name, &st)) < 0)
        {
            if (!(flags & FTW_PHYS) && errno == ENOENT &&
...
int NFTW_NEW_NAME(const char* path, NFTW_FUNC_T func, int descriptors,
                  int flags)
{
    if (flags &
...
int attribute_compat_text_section NFTW_OLD_NAME(const char* path,
                                                NFTW_FUNC_T func,
