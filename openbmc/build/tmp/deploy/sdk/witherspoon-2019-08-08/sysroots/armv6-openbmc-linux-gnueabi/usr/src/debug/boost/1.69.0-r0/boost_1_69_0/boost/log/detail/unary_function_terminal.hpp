 ...
 * \date   21.07.2012
 *
 * The header contains attribute value extractor adapter for constructing
 * expression template terminals.
 */
...
/*!
 * \brief An adapter for a unary function to be used as a terminal in a
 * Boost.Phoenix expression
 *
 * This class is an adapter between Boost.Phoenix expression invocation protocol
 * and a unary function. It forwards the call to the base function, passing only
 * the first argument from the original call. This allows to embed value
 * extractors in template expressions.
 */
...
class unary_function_terminal
{
  ...
  public:
    ...
    struct result<ThisT(ContextT)>
    {
        typedef typename remove_cv<typename remove_reference<
            typename phoenix::result_of::env<ContextT>::type>::type>::type
            env_type;
        typedef typename env_type::args_type args_type;
        typedef typename boost::log::aux::copy_cv<ThisT, function_type>::type
            cv_function_type;

        typedef typename boost::result_of<cv_function_type(
            typename fusion::result_of::at_c<args_type, 0>::type)>::type type;
    };
  ...
  public:
    ...
    //! Copy constructor
    unary_function_terminal(unary_function_terminal const& that) :
        m_fun(that.m_fun)
    {
    }
    ...
    unary_function_terminal(ArgT1 const& arg1, ArgT2 const& arg2) :
        m_fun(arg1, arg2)
    {
    }
    //! Initializing constructor
    ...
    namespace phoenix
{
    ...
    template <typename FunT>
    struct is_nullary<custom_terminal<
        boost::log::expressions::aux::unary_function_terminal<FunT>>>
        : public mpl::false_
    {
    };
