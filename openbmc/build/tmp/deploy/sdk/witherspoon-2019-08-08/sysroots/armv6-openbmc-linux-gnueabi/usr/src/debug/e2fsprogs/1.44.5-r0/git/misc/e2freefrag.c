...
static void scan_block_bitmap(ext2_filsys fs, struct chunk_info* info)
{
    ...
    for (chunk_num = 0; chunk_num < chunks; chunk_num++)
    {
        ...
        for (blk = 0; blk < num_blks; blk++, chunk_start_blk++)
        {
            ...
            used = ext2fs_fast_test_block_bitmap2(
                fs->block_map, chunk_start_blk >> fs->cluster_ratio_bits);
            if (!used)
            {
                last_chunk_size++;
                chunk_free++;
                       ...
                       blk64_t* free_blks)
{
    ...
    retval = ext2fs_check_mount_point(fs->device_name, &mount_flags, mntpoint,
                                      PATH_MAX);
    if (retval)
    {
    ...
    *free_blks = 0;
    ...
    while (1)
    {
        ...
        for (i = 0, extent = fsmap->fmh_recs; i < fsmap->fmh_entries;
             i++, extent++)
        {
            if (!(extent->fmr_flags & FMR_OF_SPECIAL_OWNER) ||
                extent->fmr_owner != FMR_OWN_FREE)
                continue;
            update_chunk_stats(info, extent->fmr_length / fs->blocksize);
                                 ...
                                 FILE* f, blk64_t free_blks)
{
    ...
    fprintf(f, "Total blocks: %llu\nFree blocks: %llu (%0.1f%%)\n",
            ext2fs_blocks_count(fs->super), free_blks,
            (double)free_blks * 100 / ext2fs_blocks_count(fs->super));

    if (info->chunkbytes)
    {
        fprintf(f, "\nChunksize: %lu bytes (%u blocks)\n", info->chunkbytes,
                info->blks_in_chunk);
        total_chunks = (ext2fs_blocks_count(fs->super) + info->blks_in_chunk) >>
                       (info->chunkbits - info->blocksize_bits);
    ...
    for (i = 0; i < MAX_HIST; i++)
    {
        ...
        if (info->histogram.fc_chunks[i] != 0)
        {
            ...
            fprintf(f, "%5lu%c...%7s  :  %12lu  %12lu  %6.2f%%\n", start,
                    *unitp, end_str, info->histogram.fc_chunks[i],
                    info->histogram.fc_blocks[i],
                    (double)info->histogram.fc_blocks[i] * 100 / free_blks);
        }
        start = end;
        if (start == 1 << 10)
        {
    ...
    while ((c = getopt(argc, argv, "c:h")) != EOF)
    {
        switch (c)
        {
            case 'c':
                ...
                if (chunk_info.chunkbytes & (chunk_info.chunkbytes - 1))
                {
                    fprintf(stderr,
