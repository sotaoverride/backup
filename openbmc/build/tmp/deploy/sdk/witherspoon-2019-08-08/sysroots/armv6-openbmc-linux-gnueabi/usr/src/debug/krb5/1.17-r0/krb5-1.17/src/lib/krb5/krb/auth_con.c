...
krb5_error_code KRB5_CALLCONV krb5_auth_con_free(krb5_context context,
                                       ...
                                       krb5_address* remote_addr)
{
    ...
    retval = 0;
    if (local_addr)
        retval = krb5_copy_addr(context, local_addr, &auth_context->local_addr);
    else
        auth_context->local_addr = NULL;

    if (!retval && remote_addr)
        retval =
            krb5_copy_addr(context, remote_addr, &auth_context->remote_addr);
    else
    ...
    krb5_address** local_addr, krb5_address** remote_addr)
{
    ...
    {
        retval = krb5_copy_addr(context, auth_context->local_addr, local_addr);
    }
                           ...
                           krb5_address* local_port, krb5_address* remote_port)
{
    ...
    retval = 0;
    if (local_port)
        retval = krb5_copy_addr(context, local_port, &auth_context->local_port);
    else
        auth_context->local_port = NULL;

    if (!retval && remote_port)
        retval =
            krb5_copy_addr(context, remote_port, &auth_context->remote_port);
    else
...
krb5_error_code krb5_auth_con_set_safe_cksumtype(krb5_context context,
                                                 krb5_auth_context auth_context,
                                                 krb5_cksumtype cksumtype)
{
...
krb5_error_code KRB5_CALLCONV krb5_auth_con_getlocalseqnumber(
    krb5_context context, krb5_auth_context auth_context, krb5_int32* seqnumber)
{
...
krb5_error_code krb5_auth_con_setivector(krb5_context context,
                                         krb5_auth_context auth_context,
...
krb5_error_code krb5_auth_con_getivector(krb5_context context,
                                         krb5_auth_context auth_context,
                                         krb5_pointer* ivector)
{
...
krb5_error_code krb5_auth_con_setpermetypes(krb5_context context,
                                            krb5_auth_context auth_context,
...
krb5_error_code krb5_auth_con_getpermetypes(krb5_context context,
                                            krb5_auth_context auth_context,
