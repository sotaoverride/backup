               ...
               PSAux_Service psaux)
{
    ...
    if (ft_strncmp((char*)stream->cursor, "%!PS-Adobe-3.0 Resource-CIDFont",
                   31))
    {
...
Again:
    ...
    /* `StartData' or `/sfnts'                      */
    {
        ...
        for (offset = FT_STREAM_POS();; offset += 256)
        {
            ...
            FT_MEM_MOVE(buffer, buffer + read_offset + read_len - STARTDATA_LEN,
                        STARTDATA_LEN);
...
Found:
    ...
    while (cur <= limit - SFNTS_LEN)
    {
        ...
        if (cur[0] == 'S' && cur <= limit - STARTDATA_LEN &&
        ...
        else if (cur[1] == 's' && ft_strncmp((char*)cur, SFNTS, SFNTS_LEN) == 0)
        {
