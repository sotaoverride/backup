...
/* Refuse putting together variants with a larger depth than 4K by default (as a
 * protection against overflowing stacks if code processes JSON objects
 * recursively. Note that we store the depth in an uint16_t, hence make sure
 * this remains under 2^16. The value was 16k, but it was discovered to be too
 * high on llvm/x86-64. See also the issue #10738. */
#define DEPTH_MAX (4U * 1024U)
assert_cc(DEPTH_MAX <= UINT16_MAX);

typedef struct JsonSource
{
    /* When we parse from a file or similar, encodes the filename, to indicate
     * the source of a json variant */
    size_t n_ref;
    unsigned max_line;
    unsigned max_column;
    char name[];
...
struct JsonVariant
{
    union
    {
        /* We either maintain a reference counter for this variant itself, or we
         * are embedded into an array/object, in which case only that
         * surrounding object is ref-counted. (If 'embedded' is false, see
         * below.) */
        size_t n_ref;

        /* If this JsonVariant is part of an array/object, then this field
         * points to the surrounding JSON_VARIANT_ARRAY/JSON_VARIANT_OBJECT
         * object. (If 'embedded' is true, see below.) */
        JsonVariant* parent;
    };

    /* If this was parsed from some file or buffer, this stores where from, as
     * well as the source line/column */
    JsonSource* source;
    unsigned line, column;

    JsonVariantType type : 5;

    /* A marker whether this variant is embedded into in array/object or not. If
     * true, the 'parent' pointer above is valid. If false, the 'n_ref' field
     * above is valid instead. */
    bool is_embedded : 1;

    /* In some conditions (for example, if this object is part of an array of
     * strings or objects), we don't store any data inline, but instead simply
     * reference an external object and act as surrogate of it. In that case
     * this bool is set, and the external object is referenced through the
     * .reference field below. */
    bool is_reference : 1;

    /* While comparing two arrays, we use this for marking what we already have
     * seen */
    bool is_marked : 1;

    /* The current 'depth' of the JsonVariant, i.e. how many levels of member
     * variants this has */
    uint16_t depth;
    ...
    union
    {
        ...
        /* For objects and arrays we store the number of elements immediately
         * following */
        size_t n_elements;

        /* If is_reference as indicated above is set, this is where the
         * reference object is actually stored. */
        JsonVariant* reference;

        /* Strings are placed immediately after the structure. Note that when
         * this is a JsonVariant embedded into an array we might encode strings
         * up to INLINE_STRING_LENGTH characters directly inside the element,
         * while longer strings are stored as references. When this object is
         * not embedded into an array, but stand-alone we allocate the right
         * size for the whole structure, i.e. the array might be much larger
         * than INLINE_STRING_LENGTH.
         *
         * Note that because we want to allocate arrays of the JsonVariant
         * structure we specify [0] here, rather than the prettier []. If we
         * wouldn't, then this char array would have undefined size, and so
         * would the union and then the struct this is included in. And of
         * structures with undefined size we
         * can't allocate arrays (at least not easily). */
        char string[0];
    };
};

/* Inside string arrays we have a series of JasonVariant structures one after
 * the other. In this case, strings longer than INLINE_STRING_MAX are stored as
 * references, and all shorter ones inline. (This means — on x86-64 — strings up
 * to 15 chars are stored within the array elements, and all others in separate
 * allocations) */
#define INLINE_STRING_MAX                                                      \
    (sizeof(JsonVariant) - offsetof(JsonVariant, string) - 1U)

/* Let's make sure this structure isn't increased in size accidentally. This
 * check is only for our most relevant arch (x86-64). */
#ifdef __x86_64__
 ...
 *    3. A 'magic' one, i.e. one of the special JSON_VARIANT_MAGIC_XYZ values,
 * that encode a few very basic values directly in the pointer.
 *    4. A 'const string' one, i.e. a pointer to a const string.
 *
 * The four kinds of pointers can be discerned like this:
 *
 *    Detecting #1 is easy, just compare with NULL. Detecting #3 is similarly
 * easy: all magic pointers are below _JSON_VARIANT_MAGIC_MAX (which is pretty
 * low, within the first memory page, which is special on Linux and other OSes,
 * as it is a faulting page). In order to discern #2 and #4 we check the lowest
 * bit. If it's off it's #2, otherwise #4. This makes use of the fact that
 * malloc() will return "maximum aligned" memory, which definitely means the
 * pointer is even. This means we can use the uneven pointers to reference
 * static strings, as long as we make sure that all static strings used like
 * this are aligned to 2 (or higher), and that we mask the bit on access. The
 * JSON_VARIANT_STRING_CONST() macro encodes strings as JsonVariant* pointers,
 * with the bit set. */
...
static bool json_variant_is_const_string(const JsonVariant* v)
{
    ...
    /* A proper JsonVariant is aligned to whatever malloc() aligns things too,
     * which is definitely not uneven. We hence use all uneven pointers as
     * indicators for const strings. */
...
static JsonVariant* json_variant_normalize(JsonVariant* v)
{
    ...
    /* Converts json variants to their normalized form, i.e. fully dereferenced
     * and wherever possible converted to the "magic" version if there is one */
    ...
    switch (json_variant_type(v))
    {
        ...
        case JSON_VARIANT_BOOLEAN:
            return json_variant_boolean(v) ? JSON_VARIANT_MAGIC_TRUE
        ...
        case JSON_VARIANT_INTEGER:
            return json_variant_integer(v) == 0
                       ? JSON_VARIANT_MAGIC_ZERO_INTEGER
                       : v;

        case JSON_VARIANT_UNSIGNED:
            return json_variant_unsigned(v) == 0
...
#pragma GCC diagnostic ignored "-Wfloat-equal"
            return json_variant_real(v) == 0.0 ? JSON_VARIANT_MAGIC_ZERO_REAL
                                               : v;
#pragma GCC diagnostic pop

        case JSON_VARIANT_STRING:
            return isempty(json_variant_string(v))
                       ? JSON_VARIANT_MAGIC_EMPTY_STRING
                       : v;

        case JSON_VARIANT_ARRAY:
            return json_variant_elements(v) == 0
...
static JsonVariant* json_variant_conservative_normalize(JsonVariant* v)
{
    ...
    /* Much like json_variant_normalize(), but won't simplify if the variant has
     * a source/line location attached to it, in order not to lose context */
...
static int json_variant_new(JsonVariant** ret, JsonVariantType type,
                            size_t space)
{
    JsonVariant* v;
    ...
    *v = (JsonVariant){
    ...
    for (v->n_elements = 0; v->n_elements < n; v->n_elements++)
    {
        ...
        if ((v->n_elements & 1) == 0 && !json_variant_is_string(c))
            return -EINVAL; /* Every second one needs to be a string, as it is
                               the key name */
...
mismatch:
    log_debug(
        "Non-boolean JSON variant requested as boolean, returning false.");
    return false;
}
...
intmax_t json_variant_integer(JsonVariant* v)
{
    ...
    switch (v->type)
    {
        ...
        case JSON_VARIANT_UNSIGNED:
            ...
            log_debug("Unsigned integer %ju requested as signed integer and "
                      "out of range, returning 0.",
                      v->value.unsig);
            return 0;
...
#pragma GCC diagnostic pop
            ...
            log_debug("Real %Lg requested as integer, and cannot be converted "
                      "losslessly, returning 0.",
                      v->value.real);
            return 0;
        }
...
uintmax_t json_variant_unsigned(JsonVariant* v)
{
    ...
    switch (v->type)
    {
        ...
        case JSON_VARIANT_INTEGER:
            ...
            log_debug("Signed integer %ju requested as unsigned integer and "
                      "out of range, returning 0.",
                      v->value.integer);
            return 0;
...
#pragma GCC diagnostic pop
            ...
            log_debug("Real %Lg requested as unsigned integer, and cannot be "
                      "converted losslessly, returning 0.",
                      v->value.real);
            return 0;
        }
...
long double json_variant_real(JsonVariant* v)
{
    ...
    switch (v->type)
    {
        ...
        case JSON_VARIANT_INTEGER:
        {
            ...
            log_debug("Signed integer %ji requested as real, and cannot be "
                      "converted losslessly, returning 0.",
                      v->value.integer);
            return 0.0;
        }
        ...
        case JSON_VARIANT_UNSIGNED:
        {
            ...
            log_debug("Unsigned integer %ju requested as real, and cannot be "
                      "converted losslessly, returning 0.",
                      v->value.unsig);
            return 0.0;
        }
...
bool json_variant_is_negative(JsonVariant* v)
{
    ...
    /* This function is useful as checking whether numbers are negative is
     * pretty complex since we have three types of numbers. And some JSON code
     * (OCI for example) uses negative numbers to mark "not defined" numeric
     * values. */
...
mismatch:
    log_debug(
        "Non-integer JSON variant tested for negativity, returning false.");
    return false;
}
...
bool json_variant_has_type(JsonVariant* v, JsonVariantType type)
{
    ...
    /* If it's a const string, then it only can be a string, and if it is not,
     * it's not */
    if (json_variant_is_const_string(v))
        return false;
    ...
    if ((v == JSON_VARIANT_MAGIC_ZERO_INTEGER ||
         v == JSON_VARIANT_MAGIC_ZERO_UNSIGNED ||
         v == JSON_VARIANT_MAGIC_ZERO_REAL) &&
        IN_SET(type, JSON_VARIANT_INTEGER, JSON_VARIANT_UNSIGNED,
               JSON_VARIANT_REAL, JSON_VARIANT_NUMBER))
        return true;
    ...
    if (type == JSON_VARIANT_NUMBER)
        return IN_SET(rt, JSON_VARIANT_INTEGER, JSON_VARIANT_UNSIGNED,
                      JSON_VARIANT_REAL);
    ...
    /* Any integer that can be converted lossley to a real and back may also be
     * considered a real */
...
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wfloat-equal"
    /* Any real that can be converted losslessly to an integer and back may also
...
mismatch:
    log_debug("Number of elements in non-array/non-object JSON variant "
              "requested, returning 0.");
    return 0;
}
...
mismatch:
    log_debug("Element in non-array/non-object JSON variant requested by "
              "index, returning NULL.");
    return NULL;
}

JsonVariant* json_variant_by_key_full(JsonVariant* v, const char* key,
                                      JsonVariant** ret_key)
{
    size_t i;
...
mismatch:
    log_debug(
        "Element in non-object JSON variant requested by key, returning NULL.");
    if (ret_key)
        *ret_key = NULL;
...
#pragma GCC diagnostic pop
        ...
        case JSON_VARIANT_ARRAY:
        {
            ...
            for (i = 0; i < n; i++)
            {
                if (!json_variant_equal(json_variant_by_index(a, i),
                                        json_variant_by_index(b, i)))
                    return false;
            }
        ...
        case JSON_VARIANT_OBJECT:
        {
            ...
            for (i = 0; i < n; i += 2)
            {
                ...
                for (j = 0; j < n; j += 2)
                {
                    ...
                    else if (key_b->is_marked) /* In later iterations if we
                                           ...
                                           json_variant_by_index(b, j + 1)))
                    {
                        ...
                        /* In the first iteration we continue the inner loop
                         * since we want to mark everything, otherwise exit the
                         * loop quickly after we found what we were
                         * looking for. */
                        if (i != 0)
                            break;
                    }
...
int json_variant_get_source(JsonVariant* v, const char** ret_source,
                            unsigned* ret_line, unsigned* ret_column)
{
    assert_return(v, -EINVAL);

    if (ret_source)
        *ret_source =
            json_variant_is_regular(v) && v->source ? v->source->name : NULL;
...
static int print_source(FILE* f, JsonVariant* v, JsonFormatFlags flags,
                        bool whitespace)
{
    size_t w, k;
...
static int json_format(FILE* f, JsonVariant* v, JsonFormatFlags flags,
                       const char* prefix)
{
    int r;
        ...
        case JSON_VARIANT_OBJECT:
        {
            ...
            else
            {
                ...
                for (i = 0; i < n; i += 2)
                {
                    ...
                    r = json_format(f, json_variant_by_index(v, i + 1), flags,
                                    prefix2);
                    if (r < 0)
                        return r;
                }
...
void json_variant_dump(JsonVariant* v, JsonFormatFlags flags, FILE* f,
                       const char* prefix)
{
    if (!v)
        return;
    ...
    if (((flags & (JSON_FORMAT_COLOR_AUTO | JSON_FORMAT_COLOR)) ==
    ...
    if (flags & (JSON_FORMAT_PRETTY | JSON_FORMAT_SEQ | JSON_FORMAT_SSE |
                 JSON_FORMAT_NEWLINE))
        fputc('\n', f);
    if (flags & JSON_FORMAT_SSE)
...
static int json_variant_copy(JsonVariant** nv, JsonVariant* v)
{
    ...
    switch (t)
    {
        ...
        default:
            ...
            c = malloc0(offsetof(JsonVariant, reference) +
                        sizeof(JsonVariant*));
            if (!c)
                return -ENOMEM;
...
static bool json_single_ref(JsonVariant* v)
{
    ...
    /* Checks whether the caller is the single owner of the object, i.e. can get
...
static int json_variant_set_source(JsonVariant** v, JsonSource* source,
                                   unsigned line, unsigned column)
{
    ...
    /* Patch in source and line/column number. Tries to do this in-place if the
     * caller is the sole referencer of the object. If not, allocates a new
     * object, possibly a surrogate for the original one */
    ...
    {
        if (json_source_equal((*v)->source, source) && (*v)->line == line &&
            (*v)->column == column)
            return 0;

        if (json_single_ref(*v))
        { /* Sole reference? */
            json_source_unref((*v)->source);
...
static void inc_lines_columns(unsigned* line, unsigned* column, const char* s,
                              size_t n)
{
    assert(line);
    assert(column);
    ...
    while (n > 0)
    {
        ...
        else if ((signed char)*s >= 0 &&
                 *s < 127) /* Process ASCII chars quickly */
            (*column)++;
        else
        {
            int w;
            ...
            if (w < 0) /* count invalid unichars as normal characters */
                w = 1;
            else if ((size_t)w > n) /* never read more than the specified number
                                       of characters */
                w = (int)n;
        ...
        if (*c == '\\')
        {
            ...
            else if (*c == 'u')
            {
                ...
                else
                {
                    ...
                    n += utf8_encode_unichar(
                        s + n, utf16_surrogate_pair_to_unichar(x, y));
                }
...
int json_tokenize(
    const char** p, char** ret_string, JsonValue* ret_value,
    unsigned* ret_line, /* 'ret_line' returns the line at the beginning of this
                           token */
    unsigned* ret_column, void** state,
    unsigned* line, /* 'line' is used as a line state, it always reflect the
                       line we are at after the token was read */
    unsigned* column)
{
...
typedef struct JsonStack
{
    ...
    size_t n_suppress; /* When building: if > 0, suppress this many subsequent
                          elements. If == (size_t) -1, suppress all subsequent
                          elements */
} JsonStack;
...
static int json_parse_internal(const char** input, JsonSource* source,
                               JsonVariant** ret, unsigned* line,
                               unsigned* column, bool continue_end)
{
    ...
    for (;;)
    {
        ...
        token = json_tokenize(&p, &string, &value, &line_token, &column_token,
                              &tokenizer_state, line, column);
        if (token < 0)
        {
            r = token;
            goto finish;
        ...
        switch (token)
        {
            ...
            case JSON_TOKEN_OBJECT_OPEN:
                ...
                if (!IN_SET(current->expect, EXPECT_TOPLEVEL,
                            EXPECT_OBJECT_VALUE, EXPECT_ARRAY_FIRST_ELEMENT,
                            EXPECT_ARRAY_NEXT_ELEMENT))
                {
                    r = -EINVAL;
                    goto finish;
                ...
                {
                    assert(IN_SET(current->expect, EXPECT_ARRAY_FIRST_ELEMENT,
                                  EXPECT_ARRAY_NEXT_ELEMENT));
            ...
            case JSON_TOKEN_OBJECT_CLOSE:
                if (!IN_SET(current->expect, EXPECT_OBJECT_FIRST_KEY,
                            EXPECT_OBJECT_COMMA))
                {
                    r = -EINVAL;
                    goto finish;
                ...
                r = json_variant_new_object(&add, current->elements,
                                            current->n_elements);
                if (r < 0)
                    goto finish;
            ...
            case JSON_TOKEN_ARRAY_CLOSE:
                if (!IN_SET(current->expect, EXPECT_ARRAY_FIRST_ELEMENT,
                            EXPECT_ARRAY_COMMA))
                {
                    r = -EINVAL;
                    goto finish;
                ...
                r = json_variant_new_array(&add, current->elements,
                                           current->n_elements);
                if (r < 0)
                    goto finish;
            ...
            case JSON_TOKEN_STRING:
                if (!IN_SET(current->expect, EXPECT_TOPLEVEL,
                            EXPECT_OBJECT_FIRST_KEY, EXPECT_OBJECT_NEXT_KEY,
                            EXPECT_OBJECT_VALUE, EXPECT_ARRAY_FIRST_ELEMENT,
                            EXPECT_ARRAY_NEXT_ELEMENT))
                {
                    r = -EINVAL;
                ...
                if (current->expect == EXPECT_TOPLEVEL)
                    current->expect = EXPECT_END;
                else if (IN_SET(current->expect, EXPECT_OBJECT_FIRST_KEY,
                                EXPECT_OBJECT_NEXT_KEY))
        ...
        if (add)
        {
            (void)json_variant_set_source(&add, source, line_token,
                                          column_token);
...
int json_parse_file(FILE* f, const char* path, JsonVariant** ret,
                    unsigned* ret_line, unsigned* ret_column)
{
...
int json_buildv(JsonVariant** ret, va_list ap)
{
    ...
    for (;;)
    {
        ...
        size_t n_subtract =
            0; /* how much to subtract from current->n_suppress, i.e. how many
        ...
        switch (command)
        {
            ...
            case _JSON_BUILD_STRING:
            {
                ...
                if (!IN_SET(current->expect, EXPECT_TOPLEVEL,
                            EXPECT_OBJECT_VALUE, EXPECT_ARRAY_ELEMENT))
                {
                    r = -EINVAL;
                    goto finish;
                }
            ...
            case _JSON_BUILD_VARIANT:
                ...
                /* Note that we don't care for current->n_suppress here, after
                 * all the variant is already allocated anyway... */
            ...
            case _JSON_BUILD_LITERAL:
            {
                ...
                if (l)
                {
                    /* Note that we don't care for current->n_suppress here, we
                     * should generate parsing
            ...
            case _JSON_BUILD_ARRAY_BEGIN:
                ...
                stack[n_stack++] = (JsonStack){
                    ...
                    .n_suppress = current->n_suppress != 0
                                      ? (size_t)-1
        ...
        /* If a variant was generated, add it to our current variant, but only
         * if we are not supposed to suppress additions */
        ...
        /* If we are supposed to suppress items, let's subtract how many items
         * where generated from that counter. Except if the counter is (size_t)
         * -1, i.e. we shall suppress an infinite number of elements on this
         * stack level */
...
int json_log_internal(JsonVariant* variant, int level, int error,
                      ...
                      const char* format, ...)
{
    ...
    if (variant)
    {
        r = json_variant_get_source(variant, &source, &source_line,
                                    &source_column);
        if (r < 0)
            return r;
    }
    ...
    if (source && source_line > 0 && source_column > 0)
        return log_struct_internal(
            LOG_REALM_PLUS_LEVEL(LOG_REALM_SYSTEMD, level), error, file, line,
            func, "MESSAGE_ID=" SD_MESSAGE_INVALID_CONFIGURATION_STR,
            "CONFIG_FILE=%s", source, "CONFIG_LINE=%u", source_line,
            "CONFIG_COLUMN=%u", source_column,
            LOG_MESSAGE("%s:%u:%u: %s", source, source_line, source_column,
                        buffer),
            NULL);
    else
        return log_struct_internal(
            LOG_REALM_PLUS_LEVEL(LOG_REALM_SYSTEMD, level), error, file, line,
            func, "MESSAGE_ID=" SD_MESSAGE_INVALID_CONFIGURATION_STR,
            LOG_MESSAGE("%s", buffer), NULL);
}

int json_dispatch(JsonVariant* v, const JsonDispatch table[],
                  JsonDispatchCallback bad, JsonDispatchFlags flags,
                  void* userdata)
{
    ...
    for (i = 0; i < n; i += 2)
    {
        ...
        { /* Found a matching entry! :-) */
            ...
            if (found[p - table])
            {
                json_log(value, merged_flags, 0, "Duplicate object field '%s'.",
                         json_variant_string(key));
            ...
            if (p->callback)
            {
                r = p->callback(json_variant_string(key), value, merged_flags,
        ...
        { /* Didn't find a matching entry! :-( */
            ...
            {
                json_log(value, flags, 0, "Unexpected object field '%s'.",
                         json_variant_string(key));
...
int json_dispatch_boolean(const char* name, JsonVariant* variant,
...
int json_dispatch_tristate(const char* name, JsonVariant* variant,
                           JsonDispatchFlags flags, void* userdata)
{
    int* b = userdata;
...
int json_dispatch_integer(const char* name, JsonVariant* variant,
                          JsonDispatchFlags flags, void* userdata)
{
    intmax_t* i = userdata;
...
int json_dispatch_unsigned(const char* name, JsonVariant* variant,
                           JsonDispatchFlags flags, void* userdata)
{
    ...
    if (!json_variant_is_unsigned(variant))
        return json_log(variant, flags, SYNTHETIC_ERRNO(EINVAL),
                        "JSON field '%s' is not an unsigned integer.",
                        strna(name));
...
int json_dispatch_uint32(const char* name, JsonVariant* variant,
                         JsonDispatchFlags flags, void* userdata)
{
    uint32_t* u = userdata;
...
int json_dispatch_int32(const char* name, JsonVariant* variant,
                        JsonDispatchFlags flags, void* userdata)
{
    int32_t* i = userdata;
    ...
    if (json_variant_integer(variant) < INT32_MIN ||
        json_variant_integer(variant) > INT32_MAX)
        return json_log(variant, flags, SYNTHETIC_ERRNO(ERANGE),
                        "JSON field '%s' out of bounds.", strna(name));
...
int json_dispatch_string(const char* name, JsonVariant* variant,
                         JsonDispatchFlags flags, void* userdata)
{
    char** s = userdata;
    int r;
...
int json_dispatch_strv(const char* name, JsonVariant* variant,
                       JsonDispatchFlags flags, void* userdata)
{
    ...
    if (!json_variant_is_array(variant))
        return json_log(variant, SYNTHETIC_ERRNO(EINVAL), flags,
...
int json_dispatch_variant(const char* name, JsonVariant* variant,
                          JsonDispatchFlags flags, void* userdata)
{
