...
DEFINE_PRIVATE_HASH_OPS_WITH_VALUE_DESTRUCTOR(track_item_hash_ops, char,
                                              string_hash_func,
                                              string_compare_func,
                                              struct track_item,
                                              track_item_free);
...
static void bus_track_add_to_queue(sd_bus_track* track)
{
    ...
    /* Adds the bus track object to the queue of objects we should dispatch
     * next, subject to a number of conditions. */

    /* Already in the queue? */
    if (track->in_queue)
        return;

    /* if we are currently in the process of adding a new name, then let's not
     * enqueue this just yet, let's wait until the addition is complete. */
    if (track->n_adding > 0)
        return;
...
_public_ int sd_bus_track_new(sd_bus* bus, sd_bus_track** track,
...
static int on_name_owner_changed(sd_bus_message* message, void* userdata,
                                 sd_bus_error* error)
{
...
_public_ int sd_bus_track_add_name(sd_bus_track* track, const char* name)
{
    ...
    bus_track_remove_from_queue(
        track); /* don't dispatch this while we work in it */

    r = sd_bus_add_match_async(track->bus, &n->slot, match,
                               on_name_owner_changed, NULL, track);
    if (r < 0)
    {
        bus_track_add_to_queue(track);
        return r;
    ...
    /* Second, check if it is currently existing, or maybe doesn't, or maybe
     * disappeared already. */
    track->n_adding++; /* again, make sure this isn't dispatch while we are
                          working in it */
...
_public_ unsigned sd_bus_track_count(sd_bus_track* track)
{
    ...
    /* This signature really should have returned an int, so that we can
     * propagate errors. But well, ... Also, note that this returns the number
     * of names being watched, and multiple references to the same name are not
     * counted. */

    return hashmap_size(track->names);
}

_public_ const char* sd_bus_track_contains(sd_bus_track* track,
                                           const char* name)
{
...
void bus_track_close(sd_bus_track* track)
{
    ...
    /* Called whenever our bus connected is closed. If so, and our track object
     * is non-empty, dispatch it immediately, as we are closing now, but first
     * flush out all names. */
...
_public_ int sd_bus_track_set_destroy_callback(sd_bus_track* track,
...
_public_ int sd_bus_track_get_destroy_callback(sd_bus_track* track,
                                               sd_bus_destroy_t* ret)
{
    assert_return(track, -EINVAL);
