...
static bool value_node_test(struct bus_match_node* node,
                            ...
                            uint8_t value_u8, const char* value_str,
                            char** value_strv, sd_bus_message* m)
{
...
static bool value_node_same(struct bus_match_node* node,
                            enum bus_match_node_type parent_type,
                            uint8_t value_u8, const char* value_str)
{
...
int bus_match_run(sd_bus* bus, struct bus_match_node* node, sd_bus_message* m)
{
    ...
    switch (node->type)
    {
        ...
        case BUS_MATCH_LEAF:
            ...
            if (node->leaf.callback->callback)
            {
                ...
                r = node->leaf.callback->callback(m, slot->userdata,
                                                  &error_buffer);
                if (bus)
                {
        ...
        case BUS_MATCH_ARG_NAMESPACE ... BUS_MATCH_ARG_NAMESPACE_LAST:
            (void)bus_message_get_arg(m, node->type - BUS_MATCH_ARG_NAMESPACE,
                                      &test_str);
            break;

        case BUS_MATCH_ARG_HAS ... BUS_MATCH_ARG_HAS_LAST:
            (void)bus_message_get_arg_strv(m, node->type - BUS_MATCH_ARG_HAS,
                                           &test_strv);
            break;
    ...
    else
    {
        ...
        {
            if (!value_node_test(c, node->type, test_u8, test_str, test_strv,
                                 m))
                continue;
...
static int bus_match_add_compare_value(struct bus_match_node* where,
                                       ...
                                       struct bus_match_node** ret)
{
    ...
    if (c)
    {
        ...
        {
            for (n = c->child; n && !value_node_same(n, t, value_u8, value_str);
                 n = n->next)
                ;
        }
...
static int bus_match_add_leaf(struct bus_match_node* where,
...
enum bus_match_node_type bus_match_node_type_from_string(const char* k,
                                                         size_t n)
{
    assert(k);
...
static int match_component_compare(const struct bus_match_component* a,
                                   const struct bus_match_component* b)
{
    return CMP(a->type, b->type);
}

void bus_match_parse_free(struct bus_match_component* components,
...
int bus_match_parse(const char* match, struct bus_match_component** _components,
...
char* bus_match_to_string(struct bus_match_component* components,
                          unsigned n_components)
{
    ...
    for (i = 0; i < n_components; i++)
    {
        ...
        fputs(
            bus_match_node_type_to_string(components[i].type, buf, sizeof(buf)),
            f);
        fputc('=', f);
        fputc('\'', f);
...
int bus_match_remove(struct bus_match_node* root,
                     struct match_callback* callback)
{
...
void bus_match_dump(struct bus_match_node* node, unsigned level)
{
    ...
    if (node->type == BUS_MATCH_VALUE)
    {
        ...
        printf(" %p/%p\n", node->leaf.callback->callback,
               container_of(node->leaf.callback, sd_bus_slot, match_callback)
                   ->userdata);
    else
        putchar('\n');
...
enum bus_match_scope
    bus_match_get_scope(const struct bus_match_component* components,
                        unsigned n_components)
{
    ...
    for (i = 0; i < n_components; i++)
    {
        ...
        if (c->type == BUS_MATCH_INTERFACE &&
            streq_ptr(c->value_str, "org.freedesktop.DBus.Local"))
            return BUS_MATCH_LOCAL;

        if (c->type == BUS_MATCH_PATH &&
            streq_ptr(c->value_str, "/org/freedesktop/DBus/Local"))
            return BUS_MATCH_LOCAL;
    }
