...
static int mkfilter_passwd_byname(const char* name, char* buffer, size_t buflen)
{
    ...
    /* build filter */
    return mysnprintf(buffer, buflen, "(&%s(%s=%s))", passwd_filter,
...
static int mkfilter_passwd_byuid(uid_t uid, char* buffer, size_t buflen)
{
    ...
    if (uidSid != NULL)
    {
        return mysnprintf(buffer, buflen,
                          "(&%s(%s=%s\\%02x\\%02x\\%02x\\%02x))", passwd_filter,
                          attmap_passwd_uidNumber, uidSid, (int)(uid & 0xff),
                          (int)((uid >> 8) & 0xff), (int)((uid >> 16) & 0xff),
                          (int)((uid >> 24) & 0xff));
    }
    else
    {
        return mysnprintf(buffer, buflen, "(&%s(%s=%lu))", passwd_filter,
                          attmap_passwd_uidNumber, (unsigned long int)uid);
    }
}
                    ...
                    char* buf, size_t buflen)
{
    ...
    attrs[2] = NULL;
    search =
        myldap_search(session, dn, LDAP_SCOPE_BASE, passwd_filter, attrs, rcp);
    if (search == NULL)
    {
    ...
    {
        if (*rcp != LDAP_SUCCESS)
            log_log(LOG_WARNING, "%s: lookup error: %s", dn,
                    ldap_err2string(*rcp));
        return NULL;
    }
...
char* dn2uid(MYLDAP_SESSION* session, const char* dn, char* buf, size_t buflen)
{
    ...
    if ((nslcd_cfg->cache_dn2uid_positive == 0) &&
        (nslcd_cfg->cache_dn2uid_negative == 0))
    ...
    if ((dn2uid_cache != NULL) &&
        ((cacheentry = dict_get(dn2uid_cache, dn)) != NULL))
    {
        if ((cacheentry->uid != NULL) && (strlen(cacheentry->uid) < buflen))
        {
            ...
            if ((nslcd_cfg->cache_dn2uid_positive > 0) &&
                (time(NULL) <
                 (cacheentry->timestamp + nslcd_cfg->cache_dn2uid_positive)))
            {
        ...
        else
        {
            ...
            if ((nslcd_cfg->cache_dn2uid_negative > 0) &&
                (time(NULL) <
                 (cacheentry->timestamp + nslcd_cfg->cache_dn2uid_negative)))
            {
                pthread_mutex_unlock(&dn2uid_cache_mutex);
                return NULL;
            }
    ...
    if (cacheentry == NULL)
    {
        ...
        cacheentry = (struct dn2uid_cache_entry*)malloc(
            sizeof(struct dn2uid_cache_entry));
        if (cacheentry != NULL)
        {
            cacheentry->uid = NULL;
...
MYLDAP_ENTRY* uid2entry(MYLDAP_SESSION* session, const char* uid, int* rcp)
{
    ...
    for (i = 0;
         (i < NSS_LDAP_CONFIG_MAX_BASES) && ((base = passwd_bases[i]) != NULL);
         i++)
    {
                        ...
                        const uid_t* requid, uid_t calleruid)
{
    ...
    {
        passwd = get_userpassword(entry, attmap_passwd_userPassword, passbuffer,
                                  sizeof(passbuffer));
    ...
    else
    {
        ...
        for (numuids = 0;
             (numuids < MAXUIDS_PER_ENTRY) && (tmpvalues[numuids] != NULL);
             numuids++)
        {
            if (uidSid != NULL)
    ...
    {
        attmap_get_value(entry, attmap_passwd_gidNumber, gidbuf,
                         sizeof(gidbuf));
        if (gidbuf[0] == '\0')
        {
            log_log(LOG_WARNING, "%s: %s: missing", myldap_get_dn(entry),
        ...
        if ((gidbuf[0] == '\0') || (*tmp != '\0'))
        {
            log_log(LOG_WARNING, "%s: %s: non-numeric", myldap_get_dn(entry),
                    attmap_passwd_gidNumber);
            return 0;
        }
        else if ((errno != 0) || (strchr(gidbuf, '-') != NULL))
        {
            log_log(LOG_WARNING, "%s: %s: out of range", myldap_get_dn(entry),
                    attmap_passwd_gidNumber);
            return 0;
        }
    }
...
NSLCD_HANDLE_UID(
    passwd, byname, NSLCD_ACTION_PASSWD_BYNAME, char name[BUFLEN_NAME];
    char filter[BUFLEN_FILTER]; READ_STRING(fp, name);
    log_setrequest("passwd=\"%s\"", name); if (!isvalidname(name)) {
...
NSLCD_HANDLE_UID(
    passwd, byuid, NSLCD_ACTION_PASSWD_BYUID, uid_t uid;
    char filter[BUFLEN_FILTER]; READ_INT32(fp, uid);
...
NSLCD_HANDLE_UID(passwd, all, NSLCD_ACTION_PASSWD_ALL, const char* filter;
                 log_setrequest("passwd(all)"); nsswitch_check_reload();
