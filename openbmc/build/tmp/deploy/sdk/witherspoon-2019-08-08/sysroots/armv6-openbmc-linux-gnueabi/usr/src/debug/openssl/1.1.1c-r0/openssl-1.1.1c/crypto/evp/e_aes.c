...
typedef struct
{
    ...
    XTS128_CONTEXT xts;
    void (*stream)(const unsigned char* in, unsigned char* out, size_t length,
...
void bsaes_xts_encrypt(const unsigned char* inp, unsigned char* out, size_t len,
                       const AES_KEY* key1, const AES_KEY* key2,
                       const unsigned char iv[16]);
void bsaes_xts_decrypt(const unsigned char* inp, unsigned char* out, size_t len,
                       const AES_KEY* key1, const AES_KEY* key2,
                       const unsigned char iv[16]);
#endif
#ifdef AES_CTR_ASM
    ...
    if (key)
    {
        do
        {
            ...
            if (!CRYPTO_ocb128_init(
                    &octx->ocb, &octx->ksenc.ks, &octx->ksdec.ks,
                    (block128_f)aesni_encrypt, (block128_f)aesni_decrypt,
                    enc ? aesni_ocb_encrypt : aesni_ocb_decrypt))
                return 0;
        } while (0);
                                    ...
                                    const unsigned char* in, size_t len)
{
    ...
    if (EVP_CIPHER_CTX_ctrl(ctx,
                            enc ? EVP_CTRL_GCM_IV_GEN : EVP_CTRL_GCM_SET_IV_INV,
    ...
    {
        if (CRYPTO_memcmp(gctx->kma.param.t.b, in + len, EVP_GCM_TLS_TAG_LEN))
        {
                                  ...
                                  const unsigned char* iv, int enc)
{
    ...
    if (key != NULL)
    {
        ...
        memset(cctx->aes.ccm.nonce.b + 1, 0, sizeof(cctx->aes.ccm.nonce.b));
        ...
        BLOCK_CIPHER_generic(nid, keylen, 16, 0, ecb, ecb, ECB,                \
            ...
            BLOCK_CIPHER_generic(nid, keylen, 1, 16, ofb128, ofb, OFB,         \
                ...
                BLOCK_CIPHER_generic(nid, keylen, 1, 16, cfb128, cfb, CFB,     \
                                     flags | EVP_CIPH_FLAG_DEFAULT_ASN1)       \
                    BLOCK_CIPHER_generic(nid, keylen, 1, 16, cfb1, cfb1, CFB,  \
                                         flags)                                \
                        BLOCK_CIPHER_generic(nid, keylen, 1, 16, cfb8, cfb8,   \
                                             CFB, flags)                       \
                            BLOCK_CIPHER_generic(nid, keylen, 1, 16, ctr, ctr, \
                                                 CTR, flags)
                        ...
                        const unsigned char* iv, int enc)
{
    ...
    if ((mode == EVP_CIPH_ECB_MODE || mode == EVP_CIPH_CBC_MODE) && !enc)
    {
        ...
        if (HWAES_CAPABLE)
        {
            ret = HWAES_set_decrypt_key(key, EVP_CIPHER_CTX_key_length(ctx) * 8,
                                        &dat->ks.ks);
            ...
            if (BSAES_CAPABLE && mode == EVP_CIPH_CBC_MODE)
        {
            ...
            if (VPAES_CAPABLE)
        {
            ret = vpaes_set_decrypt_key(key, EVP_CIPHER_CTX_key_length(ctx) * 8,
                                        &dat->ks.ks);
                          ...
                          const unsigned char* in, size_t len)
{
    ...
    if (dat->stream.ctr)
        CRYPTO_ctr128_encrypt_ctr32(
            in, out, len, &dat->ks, EVP_CIPHER_CTX_iv_noconst(ctx),
            EVP_CIPHER_CTX_buf_noconst(ctx), &num, dat->stream.ctr);
    else
        CRYPTO_ctr128_encrypt(
            in, out, len, &dat->ks, EVP_CIPHER_CTX_iv_noconst(ctx),
            EVP_CIPHER_CTX_buf_noconst(ctx), &num, dat->block);
    EVP_CIPHER_CTX_set_num(ctx, num);
    return 1;
}
    ...
    if (ctx->encrypt)
    {
        ...
        if (gctx->ctr)
        {
            ...
            if (len >= 32 && AES_GCM_ASM(gctx))
            {
                ...
                bulk = AES_gcm_encrypt(in, out, len, gctx->gcm.key,
    ...
    else
    {
        ...
        if (gctx->ctr)
        {
            ...
            if (len >= 16 && AES_GCM_ASM(gctx))
            {
                ...
                bulk = AES_gcm_decrypt(in, out, len, gctx->gcm.key,
                                       gctx->gcm.Yi.c, gctx->gcm.Xi.u);
                gctx->gcm.len.u[1] += bulk;
            }
#endif
...
BLOCK_CIPHER_custom(NID_aes, 128, 1, 12, gcm, GCM,
    ...
    BLOCK_CIPHER_custom(NID_aes, 192, 1, 12, gcm, GCM,
        ...
        BLOCK_CIPHER_custom(NID_aes, 256, 1, 12, gcm, GCM,
            ...
            static int aes_xts_ctrl(EVP_CIPHER_CTX* c, int type, int arg,
                                    void* ptr)
{
            ...
            if (HWAES_CAPABLE)
            {
                if (enc)
                {
                    HWAES_set_encrypt_key(
                        key, EVP_CIPHER_CTX_key_length(ctx) * 4, &xctx->ks1.ks);
                           ...
                           const unsigned char* in, size_t inlen)
{
    ...
    {
        if (EVP_CIPHER_CTX_encrypting(ctx))
            rv = CRYPTO_128_wrap_pad(&wctx->ks.ks, wctx->iv, out, in, inlen,
                                     (block128_f)AES_encrypt);
        else
            rv = CRYPTO_128_unwrap_pad(&wctx->ks.ks, wctx->iv, out, in, inlen,
    ...
    if (key)
    {
        do
        {
            ...
            if (HWAES_CAPABLE)
            {
                ...
                if (!CRYPTO_ocb128_init(
                        &octx->ocb, &octx->ksenc.ks, &octx->ksdec.ks,
                        (block128_f)HWAES_encrypt, (block128_f)HWAES_decrypt,
                        enc ? HWAES_ocb_encrypt : HWAES_ocb_decrypt))
                    return 0;
                break;
            }
#endif
            ...
            AES_set_decrypt_key(key, EVP_CIPHER_CTX_key_length(ctx) * 8,
                                &octx->ksdec.ks);
            if (!CRYPTO_ocb128_init(&octx->ocb, &octx->ksenc.ks,
                                    &octx->ksdec.ks, (block128_f)AES_encrypt,
