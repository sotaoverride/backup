...
static int do_one_pass(journal_t* journal, struct recovery_info* info,
                       enum passtype pass);
static int scan_revoke_records(journal_t*, struct buffer_head*, tid_t,
                               struct recovery_info*);
...
int journal_recover(journal_t* journal)
{
    ...
    if (!sb->s_start)
    {
        ...
        journal->j_transaction_sequence =
            ext2fs_be32_to_cpu(sb->s_sequence) + 1;
        return 0;
    }
...
int journal_skip_recovery(journal_t* journal)
{
    ...
    else
    {
        ...
        jbd_debug(1, "JBD2: ignoring %d transaction%s from the journal.\n",
                  dropped, (dropped == 1) ? "" : "s");
#endif
        journal->j_transaction_sequence = ++info.end_transaction;
    }
                        ...
                        unsigned long* next_log_block, __u32* crc32_sum)
{
    ...
    *crc32_sum = crc32_be(*crc32_sum, (void*)bh->b_data, bh->b_size);
    ...
    for (i = 0; i < num_blks; i++)
    {
        ...
        {
            *crc32_sum = crc32_be(*crc32_sum, (void*)obh->b_data, obh->b_size);
        }
        put_bh(obh);
    }
    return 0;
    ...
    while (1)
    {
        ...
        switch (blocktype)
        {
            case JFS_DESCRIPTOR_BLOCK:
                ...
                if (journal_has_csum_v2or3(journal))
                    descr_csum_size = sizeof(struct journal_block_tail);
                if (descr_csum_size > 0 &&
                    !jbd2_descr_block_csum_verify(journal, bh->b_data))
                {
                    err = -EFSBADCRC;
                    brelse(bh);
                    goto failed;
                ...
                if (pass != PASS_REPLAY)
                {
                    ...
                    {
                        if (calc_chksums(journal, bh, &next_log_block,
                                         &crc32_sum))
                        {
                            put_bh(bh);
                            break;
                       ...
                       journal->j_blocksize - descr_csum_size)
                {
                    ...
                    else
                    {
                         ...
                         * here. */
                        if (journal_test_revoke(journal, blocknr,
                                                next_commit_ID))
                        {
                            brelse(obh);
                            ++info->nr_revoke_hits;
                        ...
                        if (nbh == NULL)
                        {
                            printk(KERN_ERR "JBD2: Out of memory "
                                            "during recovery.\n");
                            err = -ENOMEM;
                            brelse(bh);
                            brelse(obh);
                        ...
                        if (flags & JFS_FLAG_ESCAPE)
                        {
                            ...
                            memcpy(nbh->b_data, &magic, sizeof(magic));
                        }
            ...
            case JFS_COMMIT_BLOCK:
                 ...
                 * number. */
                if (pass == PASS_SCAN && jfs_has_feature_checksum(journal))
                {
                    ...
                    if (crc32_sum == found_chksum &&
                        ...
                        cbh->h_chksum_size == JFS_CRC32_CHKSUM_SIZE)
                        chksum_seen = 1;
                    else if (!(cbh->h_chksum_type == 0 &&
                               cbh->h_chksum_size == 0 && found_chksum == 0 &&
                               !chksum_seen))
                        /*
                ...
                if (pass == PASS_SCAN &&
                    !jbd2_commit_block_csum_verify(journal, bh->b_data))
                {
                    info->end_transaction = next_commit_ID;
            ...
            case JFS_REVOKE_BLOCK:
                ...
                err = scan_revoke_records(journal, bh, next_commit_ID, info);
                brelse(bh);
                if (err)
                    goto failed;
                continue;
...
static int jbd2_revoke_block_csum_verify(journal_t* j, void* buf)
{
