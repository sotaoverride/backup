...
struct eject_control
{
    ...
    unsigned int  /* command flags and arguments */
        a_option : 1,
        c_option : 1, d_option : 1, F_option : 1, f_option : 1, i_option : 1,
        M_option : 1, m_option : 1, n_option : 1, p_option : 1, q_option : 1,
        r_option : 1, s_option : 1, T_option : 1, t_option : 1, v_option : 1,
        X_option : 1, x_option : 1, a_arg : 1, i_arg : 1;
...
static void __attribute__((__noreturn__)) usage(void)
{
    ...
    fprintf(out, _(" %s [options] [<device>|<mountpoint>]\n"),
    ...
    fputs(
        _(" -a, --auto <on|off>         turn auto-eject feature on or off\n"
          " -c, --changerslot <slot>    switch discs on a CD-ROM changer\n"
          " -d, --default               display default device\n"
          " -f, --floppy                eject floppy\n"
          " -F, --force                 don't care about device type\n"
          " -i, --manualeject <on|off>  toggle manual eject protection on/off\n"
          " -m, --no-unmount            do not unmount device even if it is "
          "mounted\n"
...
static void parse_args(struct eject_control* ctl, int argc, char** argv)
{
    ...
    while ((c = getopt_long(argc, argv, "a:c:i:x:dfFhnqrstTXvVpmM", long_opts,
                            NULL)) != -1)
    {
        switch (c)
        {
            case 'a':
                ctl->a_option = 1;
                ctl->a_arg = parse_switch(optarg, _("argument error"), "on",
                                          "off", "1", "0", NULL);
                break;
            case 'c':
                ctl->c_option = 1;
                ctl->c_arg = strtoul_or_err(
                    optarg, _("invalid argument to --changerslot/-c option"));
                break;
            case 'x':
                ctl->x_option = 1;
                ctl->x_arg = strtoul_or_err(
                    optarg, _("invalid argument to --cdspeed/-x option"));
                break;
            case 'd':
                ctl->d_option = 1;
                break;
            ...
            case 'i':
                ctl->i_option = 1;
                ctl->i_arg = parse_switch(optarg, _("argument error"), "on",
                                          "off", "1", "0", NULL);
                break;
            case 'm':
                ctl->m_option = 1;
    ...
    if (ioctl(ctl->fd, CDROM_LOCKDOOR, ctl->i_arg) < 0)
    {
        switch (errno)
        {
            ...
            case EBUSY:
                errx(
                    EXIT_FAILURE,
                    _("other users have the drive open and not CAP_SYS_ADMIN"));
            default:
...
static int eject_scsi(const struct eject_control* ctl)
{
     ...
     * -- kzak Jun 2013
     */
    if (io_hdr.driver_status != 0 && !(io_hdr.driver_status == DRIVER_SENSE &&
                                       io_hdr.sbp && io_hdr.sbp[12] == 0x3a))
        return 0;
    ...
    switch (fork())
    {
        ...
        default: /* parent */
            ...
            if (WIFEXITED(status) == 0)
                errx(EXIT_FAILURE, _("unmount of `%s' did not exit normally"),
                     name);
 ...
 */
static int device_get_mountpoint(struct eject_control* ctl, char** devname,
                                 char** mnt)
{
    struct libmnt_fs* fs;
    int rc;
...
static char* get_disk_devname(const char* device)
{
    ...
    if (sysfs_devno_to_wholedisk(st.st_rdev, diskname, sizeof(diskname),
                                 &diskno) != 0)
        return NULL;
...
int main(int argc, char** argv)
{
    ...
    {
        verbose(&ctl,
                _("%s: disc device: %s (disk device will be used for eject)"),
                ctl.device, disk);
        free(ctl.device);
        ctl.device = disk;
        disk = NULL;
    ...
    else
    {
        ...
        if (stat(ctl.device, &st) != 0 || !S_ISBLK(st.st_mode))
            errx(EXIT_FAILURE,
                 _("%s: not found mountpoint or device "
                   "with the given name"),
                 ctl.device);
    ...
    if (ctl.c_option)
    {
        verbose(&ctl, _("%s: selecting CD-ROM disc #%ld"), ctl.device,
                ctl.c_arg);
        open_device(&ctl);
        changer_select(&ctl);
        set_device_speed(&ctl);
    ...
    if (ctl.r_option)
    {
        verbose(&ctl, _("%s: trying to eject using CD-ROM eject command"),
                ctl.device);
    ...
    if (ctl.s_option && !worked)
    {
        ...
        verbose(&ctl,
                worked ? _("SCSI eject succeeded") : _("SCSI eject failed"));
    }

    if (ctl.f_option && !worked)
    {
        verbose(&ctl, _("%s: trying to eject using floppy eject command"),
                ctl.device);
    ...
    {
        verbose(&ctl, _("%s: trying to eject using tape offline command"),
                ctl.device);
