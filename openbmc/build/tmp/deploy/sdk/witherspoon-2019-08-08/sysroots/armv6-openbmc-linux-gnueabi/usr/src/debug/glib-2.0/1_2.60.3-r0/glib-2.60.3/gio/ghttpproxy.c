...
G_DEFINE_TYPE_WITH_CODE(
    GHttpProxy, g_http_proxy, G_TYPE_OBJECT,
    G_IMPLEMENT_INTERFACE(G_TYPE_PROXY, g_http_proxy_iface_init)
        _g_io_modules_ensure_extension_points_registered();
    g_io_extension_point_implement(G_PROXY_EXTENSION_POINT_NAME,
                                   g_define_type_id, "http", 0))

static void g_http_proxy_init(GHttpProxy* proxy)
{
}

static gchar* create_request(GProxyAddress* proxy_address, gboolean* has_cred,
                             GError** error)
{
    ...
    g_string_append_printf(request,
                           ...
                           "User-Agent: GLib/%i.%i\r\n",
                           ascii_hostname, port, ascii_hostname, port,
...
static gboolean check_reply(const gchar* buffer, gboolean has_cred,
                            GError** error)
{
    gint err_code;
...
static GIOStream* g_http_proxy_connect(GProxy* proxy, GIOStream* io_stream,
                                       ...
                                       GError** error)
{
    ...
    if (G_IS_HTTPS_PROXY(proxy))
    {
        tlsconn = g_tls_client_connection_new(
            io_stream, G_SOCKET_CONNECTABLE(proxy_address), error);
        if (!tlsconn)
            goto error;
...
#ifdef DEBUG
        {
            ...
            g_tls_client_connection_set_validation_flags(
                G_TLS_CLIENT_CONNECTION(tlsconn), tls_validation_flags);
        }
#endif
...
static void connect_thread(GTask* task, gpointer source_object,
...
static void g_http_proxy_connect_async(GProxy* proxy, GIOStream* io_stream,
                                       GProxyAddress* proxy_address,
                                       GCancellable* cancellable,
...
static GIOStream* g_http_proxy_connect_finish(GProxy* proxy,
                                              GAsyncResult* result,
                                              GError** error)
{
...
static gboolean g_http_proxy_supports_hostname(GProxy* proxy)
{
    return TRUE;
}

static void g_http_proxy_class_init(GHttpProxyClass* class)
{
}
...
G_DEFINE_TYPE_WITH_CODE(
    GHttpsProxy, g_https_proxy, G_TYPE_HTTP_PROXY,
    G_IMPLEMENT_INTERFACE(G_TYPE_PROXY, g_http_proxy_iface_init)
        _g_io_modules_ensure_extension_points_registered();
    g_io_extension_point_implement(G_PROXY_EXTENSION_POINT_NAME,
                                   g_define_type_id, "https", 0))

static void g_https_proxy_init(GHttpsProxy* proxy)
{
}

static void g_https_proxy_class_init(GHttpsProxyClass* class)
{
}
