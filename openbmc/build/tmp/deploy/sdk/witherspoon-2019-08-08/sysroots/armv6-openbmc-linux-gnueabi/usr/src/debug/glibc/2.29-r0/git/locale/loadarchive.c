...
{
    off_t namehash_end =
        (h->namehash_offset + h->namehash_size * sizeof(struct namehashent));
    off_t string_end = h->string_offset + h->string_used;
    off_t locrectab_end =
        (h->locrectab_offset + h->locrectab_used * sizeof(struct locrecent));
...
struct __locale_data* _nl_load_locale_from_archive(int category,
                                                   const char** namep)
{
    ...
    if (archmapped == NULL)
    {
       ...
       just the header fails to fit in our initial mapping window.  */
        ...
        if (headsize > mapsize)
        {
            ...
            result =
                __mmap64(NULL, mapsize, PROT_READ, MAP_FILE | MAP_COPY, fd, 0);
            if (result == MAP_FAILED)
                goto close_and_out;
        }
    ...
    head = headmap.ptr;
    namehashtab = (struct namehashent*)((char*)head + head->namehash_offset);
    ...
    else
    {
       ...
       Skip over the blocks which are before the data we need.  */
        ...
        for (cnt = 0; cnt < nranges; ++cnt)
        {
            ...
            while (mapped != NULL && (mapped->from + mapped->len <=
            ...
            /* Do we have a match?  */
            if (mapped != NULL && mapped->from <= ranges[cnt].from &&
                (ranges[cnt].from + ranges[cnt].len <=
                 mapped->from + mapped->len))
            {
                /* Yep, already loaded.  */
                results[ranges[cnt].category].addr =
                    ((char*)mapped->ptr + ranges[cnt].from - mapped->from);
            ...
            addr = __mmap64(NULL, to - from, PROT_READ, MAP_FILE | MAP_COPY, fd,
                            from);
            if (addr == MAP_FAILED)
                goto close_and_out;
            ...
            do
            {
                ...
                results[ranges[cnt].category].addr =
                    ((char*)addr + ranges[cnt].from - from);
                results[ranges[cnt].category].len = ranges[cnt].len;
            } while (++cnt < upper);
    ...
    for (cnt = 0; cnt < __LC_LAST; ++cnt)
        if (cnt != LC_ALL)
        {
            lia->data[cnt] = _nl_intern_locale_data(cnt, results[cnt].addr,
                                                    results[cnt].len);
        ...
        for (category = 0; category < __LC_LAST; ++category)
            if (category != LC_ALL && dead->data[category] != NULL)
            {
                ...
                if (dead->data[category]->private.cleanup)
                    (*dead->data[category]->private.cleanup)(
                        dead->data[category]);
