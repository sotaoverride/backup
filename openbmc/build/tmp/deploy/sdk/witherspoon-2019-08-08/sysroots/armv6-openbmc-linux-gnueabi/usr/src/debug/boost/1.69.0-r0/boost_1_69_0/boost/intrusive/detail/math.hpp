...
#undef BOOST_INTRUSIVE_BSR_INTRINSIC
    ...
    ((__GNUC__ >= 4) || (__GNUC__ == 3 && __GNUC_MINOR__ >= 4)) // GCC >=3.4
...
{
    return sizeof(std::size_t) * CHAR_BIT - std::size_t(1) -
           builtin_clz_dispatch<std::size_t>::call(n);
}
...
inline std::size_t floor_log2_get_shift(std::size_t n,
                                        true_) // power of two size_t
{
    return n >> 1;
}

inline std::size_t floor_log2_get_shift(std::size_t n,
                                        false_) // non-power of two size_t
{
    return (n >> 1) + ((n & 1u) & (n != 1));
}
...
inline std::size_t floor_log2(std::size_t x, integral_constant<std::size_t, N>)
{
    ...
    std::size_t shift =
        floor_log2_get_shift(remaining_bits, bool_<Size_t_Bits_Power_2>());
    while (shift)
    {
...
inline std::size_t floor_log2(std::size_t v, integral_constant<std::size_t, 64>)
{
    ...
    v |= v >> 16;
    v |= v >> 32;
    return MultiplyDeBruijnBitPosition[((std::size_t)((v - (v >> 1)) *
...
struct sqrt2_pow_max<SizeType,
                     typename enable_if<numbits_eq<SizeType, 32>>::type>
{
...
template <class SizeType>
struct sqrt2_pow_max<SizeType,
                     typename enable_if<numbits_eq<SizeType, 64>>::type>
{
