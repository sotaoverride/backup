...
/*
 * Copyright 1994, 2007, 2008, 2009 by the Massachusetts Institute of
...
static krb5_error_code get_from_windows_dir(char** pname)
{
...
static krb5_error_code get_from_module_dir(char** pname)
{
 ...
 * *pbuffer == 0.
 */
static krb5_error_code get_from_registry(char** pbuffer, HKEY hBaseKey)
{
    HKEY hKey = 0;
    LONG rc = 0;
    DWORD size = 0;
    ...
    *pbuffer = NULL;
    ...
    if ((rc = RegOpenKeyEx(hBaseKey, key_path, 0, KEY_QUERY_VALUE, &hKey)) !=
        ERROR_SUCCESS)
    {
        /* not a real error */
        goto cleanup;
 ...
 * *pbuffer == 0.
 */
static krb5_error_code get_from_known_folder(int folderId, char** pbuffer)
{
    ...
    *pbuffer = NULL;
    ...
    if (SUCCEEDED(SHGetFolderPath(NULL, folderId /*|CSIDL_FLAG_CREATE*/, NULL,
                                  SHGFP_TYPE_CURRENT, szPath)))
    {
        size = strlen(software_suffix) + strlen("\\" DEFAULT_PROFILE_FILENAME) +
               strlen(szPath);
...
static void free_filespecs(profile_filespec_t* files)
{
    char** cp;
...
static krb5_error_code os_get_default_config_files(profile_filespec_t** pfiles,
                                                   krb5_boolean secure)
{
...
static krb5_error_code add_kdc_config_file(profile_filespec_t** pfiles)
{
    char* file = NULL;
    size_t count = 0;
...
/* Set the profile paths in the context.  If secure is set to TRUE
   ...
   it.  */
static krb5_error_code os_init_paths(krb5_context ctx, krb5_boolean kdc)
{
    krb5_error_code retval = 0;
    ...
    if ((retval == PROF_SECTION_NOTOP) || (retval == PROF_SECTION_SYNTAX) ||
        (retval == PROF_RELATION_SYNTAX) || (retval == PROF_EXTRA_CBRACE) ||
...
krb5_error_code k5_os_init_context(krb5_context ctx, profile_t profile,
                                   krb5_flags flags)
{
...
krb5_error_code KRB5_CALLCONV krb5_get_profile(krb5_context ctx,
                                               profile_t* profile)
{
    return profile_copy(ctx->profile, profile);
}

krb5_error_code krb5_set_config_files(krb5_context ctx, const char** filenames)
{
    krb5_error_code retval = 0;
    profile_t profile;

    retval = profile_init_flags(filenames, PROFILE_INIT_ALLOW_MODULE, &profile);
    if (retval)
        return retval;
...
krb5_error_code KRB5_CALLCONV krb5_get_default_config_files(char*** pfilenames)
{
    if (!pfilenames)
        return EINVAL;
...
void k5_os_free_context(krb5_context ctx)
{
    krb5_os_context os_ctx;
