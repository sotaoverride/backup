...
static struct rtnl_meta_value* meta_alloc(uint8_t type, uint16_t id,
                                          uint8_t shift, void* data, size_t len)
{
...
static int meta_parse(struct rtnl_ematch* e, void* data, size_t len)
{
    ...
    v = meta_alloc(TCF_META_TYPE(hdr->left.kind), TCF_META_ID(hdr->left.kind),
    ...
    v = meta_alloc(TCF_META_TYPE(hdr->right.kind), TCF_META_ID(hdr->right.kind),
...
static void dump_value(struct rtnl_meta_value* v, struct nl_dump_params* p)
{
    ...
    switch (v->mv_type)
    {
        case TCF_META_TYPE_INT:
            ...
            {
                nl_dump(p, "%u", *(uint32_t*)(v + 1));
            }
            else
            {
                nl_dump(p, "%s", int_id2str(v->mv_id, buf, sizeof(buf)));
        ...
        case TCF_META_TYPE_VAR:
            ...
            {
                nl_dump(p, "%s", var_id2str(v->mv_id, buf, sizeof(buf)));
