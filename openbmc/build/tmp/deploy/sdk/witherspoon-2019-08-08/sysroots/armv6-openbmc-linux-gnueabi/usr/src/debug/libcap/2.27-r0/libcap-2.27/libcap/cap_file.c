                         ...
                         int bytes)
{
    ...
    while (i < __CAP_BLKS)
    {
        result->u[i].flat[CAP_INHERITABLE] = result->u[i].flat[CAP_PERMITTED] =
                       ...
                       int* bytes_p)
{
    ...
    for (i = 0; i < tocopy; i++)
    {
        ...
        if (eff_not_zero && ((~(cap_d->u[i].flat[CAP_EFFECTIVE])) &
                             (cap_d->u[i].flat[CAP_PERMITTED] |
                              cap_d->u[i].flat[CAP_INHERITABLE])))
        {
            errno = EINVAL;
...
cap_t cap_get_fd(int fildes)
{
    ...
    if (result)
    {
        ...
        sizeofcaps =
            fgetxattr(fildes, XATTR_NAME_CAPS, &rawvfscap, sizeof(rawvfscap));
...
cap_t cap_get_file(const char* filename)
{
    ...
    if (result)
    {
        ...
        /* fill the capability sets via a system call */
        sizeofcaps =
            getxattr(filename, XATTR_NAME_CAPS, &rawvfscap, sizeof(rawvfscap));
        if (sizeofcaps < ssizeof(rawvfscap.magic_etc))
        {
            cap_free(result);
            result = NULL;
