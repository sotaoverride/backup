...
static void g_dummy_file_finalize(GObject* object)
{
    GDummyFile* dummy;
...
static void g_dummy_file_class_init(GDummyFileClass* klass)
{
...
static void g_dummy_file_init(GDummyFile* dummy)
{
}
...
static gboolean g_dummy_file_is_native(GFile* file)
{
    return FALSE;
}

static char* g_dummy_file_get_basename(GFile* file)
{
...
static char* g_dummy_file_get_path(GFile* file)
{
    return NULL;
}

static char* g_dummy_file_get_uri(GFile* file)
{
    return g_strdup(G_DUMMY_FILE(file)->text_uri);
}

static char* g_dummy_file_get_parse_name(GFile* file)
{
    return g_strdup(G_DUMMY_FILE(file)->text_uri);
}

static GFile* g_dummy_file_get_parent(GFile* file)
{
    GDummyFile* dummy = G_DUMMY_FILE(file);
    GFile* parent;
    char* dirname;
...
static GFile* g_dummy_file_dup(GFile* file)
{
    GDummyFile* dummy = G_DUMMY_FILE(file);
...
static guint g_dummy_file_hash(GFile* file)
{
    GDummyFile* dummy = G_DUMMY_FILE(file);
...
static gboolean g_dummy_file_equal(GFile* file1, GFile* file2)
{
...
static gboolean uri_same_except_path(GDecodedUri* a, GDecodedUri* b)
{
...
static gboolean g_dummy_file_prefix_matches(GFile* parent, GFile* descendant)
{
    ...
    {
        remainder =
            match_prefix(descendant_dummy->text_uri, parent_dummy->text_uri);
        if (remainder != NULL && *remainder == '/')
        {
            while (*remainder == '/')
                remainder++;
...
static char* g_dummy_file_get_relative_path(GFile* parent, GFile* descendant)
{
    GDummyFile* parent_dummy = G_DUMMY_FILE(parent);
    GDummyFile* descendant_dummy = G_DUMMY_FILE(descendant);
    const char* remainder;
...
static GFile* g_dummy_file_resolve_relative_path(GFile* file,
                                                 const char* relative_path)
{
    ...
    else
    {
        ...
        if (g_path_is_absolute(relative_path))
            ...
            new_decoded_uri.path =
                g_build_filename(new_decoded_uri.path, relative_path, NULL);
...
static GFile* g_dummy_file_get_child_for_display_name(GFile* file,
...
static gboolean g_dummy_file_has_uri_scheme(GFile* file, const char* uri_scheme)
{
    GDummyFile* dummy = G_DUMMY_FILE(file);
...
static char* g_dummy_file_get_uri_scheme(GFile* file)
{
    GDummyFile* dummy = G_DUMMY_FILE(file);
...
{
    GDecodedUri* decoded;
    const char *p, *in, *hier_part_start, *hier_part_end, *query_start,
        *fragment_start;
    char* out;
    char c;
    ...
    *out = 0;
    ...
    if (query_start)
    {
        ...
        {
            decoded->query =
                g_strndup(query_start, fragment_start - query_start);
    ...
    if (hier_part_start[0] == '/' && hier_part_start[1] == '/')
    {
        ...
        */
        ...
        if (userinfo_end)
        {
            ...
            decoded->userinfo =
                unescape_string(userinfo_start, userinfo_end, NULL);
...
static gboolean is_valid(char c, const char* reserved_chars_allowed)
{
    ...
    if (reserved_chars_allowed && strchr(reserved_chars_allowed, c) != NULL)
        return TRUE;
...
static char* _g_encode_uri(GDecodedUri* decoded)
{
    ...
    if (decoded->host != NULL)
    {
        if (decoded->userinfo)
        {
            ...
            g_string_append_encoded(uri, decoded->userinfo,
                                    SUB_DELIM_CHARS ":");
