...
static gssize g_converter_input_stream_read(GInputStream* stream, void* buffer,
                                            gsize count,
...
static gssize g_converter_input_stream_read_nonblocking(
    GPollableInputStream* stream, void* buffer, gsize size, GError** error);

static GSource*
    g_converter_input_stream_create_source(GPollableInputStream* stream,
                                           GCancellable* cancellable);

static void g_converter_input_stream_pollable_iface_init(
    GPollableInputStreamInterface* iface);

G_DEFINE_TYPE_WITH_CODE(
    GConverterInputStream, g_converter_input_stream, G_TYPE_FILTER_INPUT_STREAM,
    ...
    g_converter_input_stream_class_init(GConverterInputStreamClass* klass)
{
    ...
    g_object_class_install_property(
        ...
        g_param_spec_object("converter", P_("Converter"),
                            P_("The converter object"), G_TYPE_CONVERTER,
                                           ...
                                           GConverter* converter)
{
    ...
    stream = g_object_new(G_TYPE_CONVERTER_INPUT_STREAM, "base-stream",
                          base_stream, "converter", converter, NULL);
...
static gssize fill_input_buffer(GConverterInputStream* stream,
                                gsize at_least_size, gboolean blocking,
                                GCancellable* cancellable, GError** error)
{
    ...
    nread = g_pollable_stream_read(
        base_stream, priv->input_buffer.data + priv->input_buffer.end,
        buffer_tailspace(&priv->input_buffer), blocking, cancellable, error);
                            ...
                            GError** error)
{
    ...
    if (buffer_data_size(&priv->input_buffer) == 0 && total_bytes_read == 0 &&
        !priv->at_input_end)
    {
    ...
    /* First try to convert any available data (or state) directly to the user
     * buffer: */
    if (!priv->finished)
    {
        my_error = NULL;
        res = g_converter_convert(
            priv->converter, buffer_data(&priv->input_buffer),
            buffer_data_size(&priv->input_buffer), buffer, count,
            priv->at_input_end ? G_CONVERTER_INPUT_AT_END : 0, &bytes_read,
            &bytes_written, &my_error);
        if (res != G_CONVERTER_ERROR)
        {
            ...
            if (res == G_CONVERTER_FINISHED)
                priv->finished = TRUE; /* We're done converting */
                 ...
                 !g_error_matches(my_error, G_IO_ERROR,
                                  G_IO_ERROR_PARTIAL_INPUT) &&
                 !g_error_matches(my_error, G_IO_ERROR, G_IO_ERROR_NO_SPACE))
        {
    ...
    while (TRUE)
    {
        ...
        if (res != G_CONVERTER_ERROR)
        {
            ...
            if (buffer_data_size(&priv->converted_buffer) == 0 &&
                res != G_CONVERTER_FINISHED)
            ...
            total_bytes_read =
                MIN(count, buffer_data_size(&priv->converted_buffer));
            ...
            !priv->at_input_end)
        {
            /* Need more data */
            my_error2 = NULL;
            nread = fill_input_buffer(
                cstream, buffer_data_size(&priv->input_buffer) + 4096, blocking,
                cancellable, &my_error2);
            if (nread < 0)
            {
        ...
        if (g_error_matches(my_error, G_IO_ERROR, G_IO_ERROR_NO_SPACE))
        {
             ...
             * Note: if we actually grow the buffer (as opposed to compacting
             * it), this will double the size, not just add one byte. */
...
static gboolean g_converter_input_stream_can_poll(GPollableInputStream* stream)
{
    ...
    return (
        G_IS_POLLABLE_INPUT_STREAM(base_stream) &&
...
static gssize g_converter_input_stream_read_nonblocking(
    GPollableInputStream* stream, void* buffer, gsize count, GError** error)
{
    return read_internal(G_INPUT_STREAM(stream), buffer, count, FALSE, NULL,
                         error);
}
                                           ...
                                           GCancellable* cancellable)
{
    ...
    if (g_pollable_input_stream_is_readable(stream))
        ...
        base_source = g_pollable_input_stream_create_source(
            G_POLLABLE_INPUT_STREAM(base_stream), NULL);
