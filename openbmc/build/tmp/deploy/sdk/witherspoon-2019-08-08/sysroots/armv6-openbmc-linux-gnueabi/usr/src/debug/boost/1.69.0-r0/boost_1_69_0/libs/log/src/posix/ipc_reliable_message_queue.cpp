 ...
 *
 * This file provides an interprocess message queue implementation on POSIX
 * platforms.
 */
...
// 32-bit atomic ops are required to be able to place atomic<uint32_t> in the
...
{
  private:
    //! Header of an allocation block within the message queue. Placed at the
    //! beginning of the block within the shared memory segment.
    struct block_header
    {
        ...
        {
            return static_cast<size_type>(boost::alignment::align_up(
                sizeof(block_header), data_alignment));
        }
    ...
    //! Header of the message queue. Placed at the beginning of the shared
    //! memory segment.
    struct header
    {
        // Increment this constant whenever you change the binary layout of the
        // queue (apart from this header structure)
        enum
        {
            abi_version = 0
        };

        // !!! Whenever you add/remove members in this structure, also modify
        // get_abi_tag() function accordingly !!!

        //! A tag value to ensure the correct binary layout of the message queue
        //! data structures. Must be placed first and always have a fixed size
        //! and alignment.
        uint32_t m_abi_tag;
        //! Padding to protect against alignment changes in Boost.Atomic. Don't
        //! use BOOST_ALIGNMENT to ensure portability.
        unsigned char
            m_padding[BOOST_LOG_CPU_CACHE_LINE_SIZE - sizeof(uint32_t)];
        //! Reference counter. Also acts as a flag indicating that the queue is
        //! constructed (i.e. the queue is constructed when the counter is not
        //! 0).
        ...
        header(uint32_t capacity, size_type block_size) :
            m_abi_tag(get_abi_tag()), m_capacity(capacity),
            m_block_size(block_size), m_size(0u), m_put_pos(0u), m_get_pos(0u)
        {
            // Must be initialized last. m_ref_count is zero-initialized
            // initially.
        ...
        static uint32_t get_abi_tag() BOOST_NOEXCEPT
        {
            ...
            boost::log::aux::murmur3_32 hash(
                boost::log::aux::make_fourcc('r', 'e', 'l', 'q'));
            ...
            // The members in the sequence below must be enumerated in the same
            // order as they are declared in the header structure. The ABI tag
            // is supposed change whenever a member changes size or offset from
...
#undef BOOST_LOG_MIX_HEADER_MEMBER
        ...
        block_header* get_block(uint32_t index) const BOOST_NOEXCEPT
        {
            ...
            unsigned char* p =
                const_cast<unsigned char*>(
                    reinterpret_cast<const unsigned char*>(this)) +
                boost::alignment::align_up(sizeof(header),
                                           BOOST_LOG_CPU_CACHE_LINE_SIZE);
            p += static_cast<std::size_t>(m_block_size) *
  ...
  private:
    ...
    //! The mask for selecting bits that constitute size values from 0 to
    //! (block_size - 1)
  ...
  public:
    ...
    implementation(open_mode::create_only_tag, object_name const& name,
                   uint32_t capacity, size_type block_size,
                   overflow_policy oflow_policy, permissions const& perms) :
        m_shared_memory(boost::interprocess::create_only, name.c_str(),
                        boost::interprocess::read_write,
                        boost::interprocess::permissions(perms.get_native())),
        m_region(), m_overflow_policy(oflow_policy), m_block_size_mask(0u),
        m_block_size_log2(0u), m_stop(false), m_name(name)
    {
        create_region(capacity, block_size);
    }

    //! The constructor creates a new shared memory segment or opens the
    //! existing one
    implementation(open_mode::open_or_create_tag, object_name const& name,
                   uint32_t capacity, size_type block_size,
                   overflow_policy oflow_policy, permissions const& perms) :
        m_shared_memory(boost::interprocess::open_or_create, name.c_str(),
                        boost::interprocess::read_write,
                        boost::interprocess::permissions(perms.get_native())),
        m_region(), m_overflow_policy(oflow_policy), m_block_size_mask(0u),
        m_block_size_log2(0u), m_stop(false), m_name(name)
    ...
    implementation(open_mode::open_only_tag, object_name const& name,
                   overflow_policy oflow_policy) :
        m_shared_memory(boost::interprocess::open_only, name.c_str(),
                        boost::interprocess::read_write),
        m_region(), m_overflow_policy(oflow_policy), m_block_size_mask(0u),
        m_block_size_log2(0u), m_stop(false), m_name(name)
    {
        ...
        if (!m_shared_memory.get_size(shmem_size))
            BOOST_LOG_THROW_DESCR(setup_error,
                                  "Boost.Log interprocess message queue cannot "
                                  "be opened: shared memory segment not found");
    ...
    operation_result send(void const* message_data, size_type message_size)
    {
        ...
        lock_queue();
        boost::log::ipc::aux::interprocess_mutex::auto_unlock unlock(
            hdr->m_mutex);
        ...
        while (true)
        {
            ...
            if (oflow_policy == fail_on_overflow)
                ...
                BOOST_LOG_THROW_DESCR(capacity_limit_reached,
  ...
  private:
    ...
    static std::size_t estimate_region_size(uint32_t capacity,
                                            size_type block_size) BOOST_NOEXCEPT
    {
    ...
    void create_region(uint32_t capacity, size_type block_size)
    {
        const std::size_t shmem_size =
            estimate_region_size(capacity, block_size);
        m_shared_memory.truncate(shmem_size);
        boost::interprocess::mapped_region(
            m_shared_memory, boost::interprocess::read_write, 0u, shmem_size)
            .swap(m_region);
    ...
    void adopt_region(std::size_t shmem_size)
    {
        if (shmem_size < sizeof(header))
            BOOST_LOG_THROW_DESCR(
                setup_error, "Boost.Log interprocess message queue cannot be "
                             "opened: shared memory segment size too small");
        ...
        header* const hdr = get_header();
        BOOST_CONSTEXPR_OR_CONST unsigned int wait_loops = 200u,
                                              spin_loops = 16u, spins = 16u;
        for (unsigned int i = 0; i < wait_loops; ++i)
        {
            uint32_t ref_count =
                hdr->m_ref_count.load(boost::memory_order_acquire);
            while (ref_count > 0u)
            {
                if (hdr->m_ref_count.compare_exchange_weak(
                        ref_count, ref_count + 1u, boost::memory_order_acq_rel,
        ...
        BOOST_LOG_THROW_DESCR(
            setup_error,
            "Boost.Log interprocess message queue cannot be opened: shared "
            "memory segment is not initialized by creator for too long");
        ...
        try
        {
            ...
            if (hdr->m_abi_tag != header::get_abi_tag())
                BOOST_LOG_THROW_DESCR(
                    setup_error, "Boost.Log interprocess message queue cannot "
                                 "be opened: the queue ABI is incompatible");

            if (!boost::log::aux::is_power_of_2(hdr->m_block_size))
                BOOST_LOG_THROW_DESCR(
                    setup_error,
                    "Boost.Log interprocess message queue cannot be opened: "
                    "the queue block size is not a power of 2");
    ...
    void lock_queue()
    {
        ...
        {
            // The mutex is locked by the current thread, but the previous owner
    ...
    //! Returns the number of allocation blocks that are required to store
    //! user's payload of the specified size
    uint32_t estimate_block_count(size_type size) const BOOST_NOEXCEPT
    {
        // ceil((size + get_header_overhead()) / block_size)
        return static_cast<uint32_t>(
            (size + block_header::get_header_overhead() + m_block_size_mask) >>
            m_block_size_log2);
    }

    //! Puts the message to the back of the queue
    void enqueue_message(void const* message_data, size_type message_size,
                         uint32_t block_count)
    {
        ...
        size_type write_size = (std::min)(
            static_cast<size_type>((capacity - pos) * block_size -
        ...
        if (BOOST_UNLIKELY(pos >= capacity))
        {
            ...
            pos -= capacity;
            message_data =
                static_cast<const unsigned char*>(message_data) + write_size;
    ...
    //! Retrieves the next message and invokes the handler to store the message
    //! contents
...
BOOST_LOG_API void reliable_message_queue::create(object_name const& name,
                                                  uint32_t capacity,
                                                  ...
                                                  permissions const& perms)
{
    ...
    if (!boost::log::aux::is_power_of_2(block_size))
        BOOST_THROW_EXCEPTION(std::invalid_argument(
            "Interprocess message queue block size is not a power of 2"));
    try
    {
        m_impl = new implementation(
            open_mode::create_only, name, capacity,
            static_cast<size_type>(boost::alignment::align_up(
                block_size, BOOST_LOG_CPU_CACHE_LINE_SIZE)),
            oflow_policy, perms);
    }
    ...
    catch (boost::interprocess::interprocess_exception& e)
    {
        BOOST_THROW_EXCEPTION(
            boost::enable_error_info(system_error(
                boost::system::error_code(e.get_native_error(),
...
BOOST_LOG_API void reliable_message_queue::open_or_create(
    object_name const& name, uint32_t capacity, size_type block_size,
    overflow_policy oflow_policy, permissions const& perms)
{
    BOOST_ASSERT(m_impl == NULL);
    if (!boost::log::aux::is_power_of_2(block_size))
    ...
    {
        m_impl = new implementation(
            open_mode::open_or_create, name, capacity,
            static_cast<size_type>(boost::alignment::align_up(
                block_size, BOOST_LOG_CPU_CACHE_LINE_SIZE)),
            oflow_policy, perms);
    }
...
BOOST_LOG_API void reliable_message_queue::open(object_name const& name,
                                                overflow_policy oflow_policy,
                                                permissions const&)
{
    BOOST_ASSERT(m_impl == NULL);
...
BOOST_LOG_API reliable_message_queue::size_type
...
BOOST_LOG_API reliable_message_queue::operation_result
...
BOOST_LOG_API bool reliable_message_queue::try_send(void const* message_data,
                                                    size_type message_size)
{
    BOOST_ASSERT(m_impl != NULL);
    try
...
BOOST_LOG_API reliable_message_queue::operation_result
    reliable_message_queue::do_receive(receive_handler handler, void* state)
{
    BOOST_ASSERT(m_impl != NULL);
    try
    {
...
BOOST_LOG_API bool
    reliable_message_queue::do_try_receive(receive_handler handler, void* state)
{
    BOOST_ASSERT(m_impl != NULL);
    try
    {
...
BOOST_LOG_API void reliable_message_queue::fixed_buffer_receive_handler(
    void* state, const void* data, size_type size)
{
    fixed_buffer_state* p = static_cast<fixed_buffer_state*>(state);
    if (BOOST_UNLIKELY(size > p->size))
        BOOST_THROW_EXCEPTION(
            bad_alloc("Buffer too small to receive the message"));
