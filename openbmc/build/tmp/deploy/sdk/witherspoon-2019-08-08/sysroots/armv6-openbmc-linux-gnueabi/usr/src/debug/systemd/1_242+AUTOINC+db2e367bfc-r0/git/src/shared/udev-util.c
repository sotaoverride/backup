                           ...
                           ResolveNameTiming* ret_resolve_name_timing)
{
    ...
    _cleanup_free_ char *log_val = NULL, *children_max = NULL,
                        *exec_delay = NULL, *event_timeout = NULL,
                        *resolve_names = NULL;
    int r;

    r = parse_env_file(NULL, "/etc/udev/udev.conf", "udev_log", &log_val,
                       "children_max", &children_max, "exec_delay", &exec_delay,
                       "event_timeout", &event_timeout, "resolve_names",
                       &resolve_names);
    if (r == -ENOENT)
        return 0;
    if (r < 0)
    ...
    if (log_val)
    {
        ...
        if (r < 0)
            log_debug_errno(r,
                            "/etc/udev/udev.conf: failed to set udev log level "
                            "'%s', ignoring: %m",
                            log);
    }
    ...
    if (ret_children_max && children_max)
    {
        ...
        if (r < 0)
            log_notice_errno(r,
                             "/etc/udev/udev.conf: failed to set parse "
                             "children_max=%s, ignoring: %m",
                             children_max);
    }
    ...
    if (ret_resolve_name_timing && resolve_names)
    {
        ...
        if (t < 0)
            log_notice("/etc/udev/udev.conf: failed to set parse "
                       "resolve_names=%s, ignoring.",
                       resolve_names);
        else
...
static int device_monitor_handler(sd_device_monitor* monitor, sd_device* device,
                                  void* userdata)
{
    ...
    if (sd_device_get_sysname(device, &sysname) >= 0 &&
        streq(sysname, data->sysname))
    {
...
int device_wait_for_initialization(sd_device* device, const char* subsystem,
                                   sd_device** ret)
{
    ...
    /* Wait until the device is initialized, so that we can get access to the
     * ID_PATH property */
    ...
    if (r < 0)
        return log_error_errno(
            r, "Failed to add %s subsystem match to monitor: %m", subsystem);
    ...
    /* Check again, maybe things changed. Udev will re-read the db if the device
