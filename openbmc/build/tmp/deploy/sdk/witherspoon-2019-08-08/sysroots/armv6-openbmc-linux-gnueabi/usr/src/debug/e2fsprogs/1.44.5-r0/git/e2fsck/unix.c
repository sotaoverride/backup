...
static void usage(e2fsck_t ctx)
{
    ...
    fprintf(stderr, "%s",
            _("\nEmergency help:\n"
              ...
              "badblock list\n"
              " -f                   Force checking even if filesystem is "
              "marked clean\n"));
    fprintf(
        stderr, "%s",
        _(""
...
static void show_stats(e2fsck_t ctx)
{
    ...
    inodes_used = (fs->super->s_inodes_count - fs->super->s_free_inodes_count);
    blocks = ext2fs_blocks_count(fs->super);
    blocks_used =
        (ext2fs_blocks_count(fs->super) - ext2fs_free_blocks_count(fs->super));
    ...
    if (inodes_used > 0)
    {
        ...
        frag_percent_total =
            ((10000 * (ctx->fs_fragmented + ctx->fs_fragmented_dir)) /
             inodes_used);
    ...
    {
        log_out(ctx,
                _("%s: %u/%u files (%0d.%d%% non-contiguous), "
                  "%llu/%llu blocks\n"),
                ctx->device_name, inodes_used, inodes, frag_percent_total / 10,
                frag_percent_total % 10, blocks_used, blocks);
        return;
    }
    profile_get_boolean(ctx->profile, "options", "report_features", 0, 0, &i);
    if (verbose && i)
    {
        log_out(ctx, "\nFilesystem features:");
        ...
        for (i = 0; i < 3; i++, mask++)
        {
            for (j = 0, m = 1; j < 32; j++, m <<= 1)
            {
                if (*mask & m)
                {
                    log_out(ctx, " %s", e2p_feature2string(i, m));
                    printed++;
                }
            }
        }
    ...
    log_out(ctx,
            ...
            inodes_used, 100.0 * inodes_used / inodes, inodes);
    log_out(ctx,
            P_("%12u non-contiguous file (%0d.%d%%)\n",
               "%12u non-contiguous files (%0d.%d%%)\n", ctx->fs_fragmented),
            ctx->fs_fragmented, frag_percent_file / 10, frag_percent_file % 10);
    log_out(ctx,
            P_("%12u non-contiguous directory (%0d.%d%%)\n",
               "%12u non-contiguous directories (%0d.%d%%)\n",
               ctx->fs_fragmented_dir),
            ctx->fs_fragmented_dir, frag_percent_dir / 10,
            frag_percent_dir % 10);
    log_out(ctx,
            _("             # of inodes with ind/dind/tind blocks: "
              "%u/%u/%u\n"),
    ...
    log_out(ctx,
            P_("%12llu block used (%2.2f%%, out of %llu)\n",
    ...
    log_out(
        ...
        ctx->fs_directory_count);
    log_out(ctx,
            P_("%12u character device file\n", "%12u character device files\n",
               ctx->fs_chardev_count),
            ctx->fs_chardev_count);
    log_out(ctx,
            P_("%12u block device file\n", "%12u block device files\n",
    ...
    log_out(
        ctx,
        P_("%12u symbolic link", "%12u symbolic links", ctx->fs_symlinks_count),
        ctx->fs_symlinks_count);
    log_out(ctx,
            P_(" (%u fast symbolic link)\n", " (%u fast symbolic links)\n",
...
static void check_mount(e2fsck_t ctx)
{
    ...
    retval = ext2fs_check_if_mounted(ctx->filesystem_name, &ctx->mount_flags);
    if (retval)
    {
    ...
    if (d)
    {
        while ((de = readdir(d)) != NULL)
        {
            if (!strncmp(".", de->d_name, 1))
                continue;
            snprintf(fname, sizeof(fname), "/proc/acpi/ac_adapter/%s/state",
                     de->d_name);
            f = fopen(fname, "r");
            if (!f)
                continue;
...
static void check_if_skip(e2fsck_t ctx)
{
    ...
    profile_get_boolean(ctx->profile, "options", "broken_system_clock", 0, 0,
                        &broken_system_clock);
    if (ctx->flags & E2F_FLAG_TIME_INSANE)
        broken_system_clock = 1;
    profile_get_boolean(ctx->profile, "options", "defer_check_on_battery", 0, 1,
             ...
             (ctx->now < lastcheck))
    {
        ...
        if (batt)
            reason = 0;
             ...
             ((ctx->now - lastcheck) >= ((time_t)fs->super->s_checkinterval)))
    {
    ...
    if ((pctx.ino != pctx.ino2) && !(ctx->options & E2F_OPT_READONLY) &&
    ...
    if ((pctx.blk != pctx.blk2) && !(ctx->options & E2F_OPT_READONLY) &&
    ...
    log_out(
        ctx, _("%s: clean, %u/%u files, %llu/%llu blocks"), ctx->device_name,
    ...
    if (next_check <= 5)
    {
        ...
        {
            if (batt)
                log_out(ctx, "%s", _(" (check deferred; on battery)"));
            else
                log_out(ctx, "%s", _(" (check after next mount)"));
        }
        else
...
static struct percent_tbl e2fsck_tbl = {5, {0, 70, 90, 92, 95, 100}};
static char bar[128], spaces[128];

static float calc_percent(struct percent_tbl* tbl, int pass, int curr, int max)
{
    float percent;
                           ...
                           unsigned int dpynum)
{
    ...
    if ((tick == ctx->progress_last_time) && (fixed_percent != 0) &&
        (fixed_percent != 1000))
        return 0;
    ...
    printf("%s%s: |%s%s", ctx->start_meta, label, bar + (sizeof(bar) - (i + 1)),
...
static int e2fsck_update_progress(e2fsck_t ctx, int pass, unsigned long cur,
                                  unsigned long max)
{
    char buf[1024];
    float percent;
    ...
    else
    {
        ...
        e2fsck_simple_progress(ctx, ctx->device_name, percent, 0);
    }
    return 0;
}
...
static void parse_extended_opts(e2fsck_t ctx, const char* opts)
{
    ...
    for (token = buf; token && *token; token = next)
    {
        ...
        if (strcmp(token, "ea_ver") == 0)
        {
            ...
            if (*p || ((ea_ver != 1) && (ea_ver != 2)))
            {
                fprintf(stderr, "%s", _("Invalid EA version.\n"));
                extended_usage++;
                continue;
            }
        ...
        else if (strcmp(token, "readahead_kb") == 0)
        {
            ...
            if (*p)
            {
                fprintf(stderr, "%s", _("Invalid readahead buffer size.\n"));
                extended_usage++;
                continue;
            }
...
static errcode_t PRS(int argc, char* argv[], e2fsck_t* ret_ctx)
{
    ...
    *ret_ctx = ctx;
    ...
    while ((c = getopt(argc, argv,
                       "panyrcC:B:dE:fvtFVM:b:I:j:P:l:L:N:SsDkz:")) != EOF)
        switch (c)
        {
            case 'C':
                ctx->progress = e2fsck_update_progress;
                ...
                if (fd < 0)
                {
                    fprintf(stderr,
                            ...
                            ctx->progress_fd, error_message(errno));
                    fatal_error(
                        ctx,
            ...
            case 'a':
                ...
                {
                conflict_opt:
                    fatal_error(ctx, _("Only one of the options -p/-a, -n or "
                                       "-y may be specified."));
                }
            ...
            case 'I':
                ...
                break;
            case 'j':
                ctx->journal_name = blkid_get_devname(ctx->blkid, optarg, NULL);
                if (!ctx->journal_name)
                {
    ...
    if (optind != argc - 1)
        usage(ctx);
    if ((ctx->options & E2F_OPT_NO) && (ctx->options & E2F_OPT_COMPRESS_DIRS))
    {
    ...
    profile_get_boolean(ctx->profile, "options", "report_time", 0, 0, &c);
    if (c)
        ctx->options |= E2F_OPT_TIME | E2F_OPT_TIME2;
    profile_get_boolean(ctx->profile, "options", "report_verbose", 0, 0, &c);
    if (c)
        verbose = 1;

    profile_get_boolean(ctx->profile, "options", "no_optimize_extents", 0, 0,
                        &c);
    if (c)
        ctx->options |= E2F_OPT_NOOPT_EXTENTS;

    profile_get_boolean(ctx->profile, "options", "inode_count_fullmap", 0, 0,
                        &c);
    if (c)
        ctx->options |= E2F_OPT_ICOUNT_FULLMAP;
    ...
    {
        profile_get_integer(ctx->profile, "options", "readahead_mem_pct", 0, -1,
                            &c);
        if (c >= 0 && c <= 100)
            ctx->readahead_kb = phys_mem_kb * c / 100;
        profile_get_integer(ctx->profile, "options", "readahead_kb", 0, -1, &c);
        if (c >= 0)
            ctx->readahead_kb = c;
        if (ctx->readahead_kb != ~0ULL && ctx->readahead_kb > phys_mem_kb)
    ...
    if (flush)
    {
        ...
        if (fd < 0)
        {
            com_err("open", errno, _("while opening %s for flushing"),
                    ctx->filesystem_name);
            fatal_error(ctx, 0);
        }
        if ((retval = ext2fs_sync_device(fd, 1)))
        {
            com_err("ext2fs_sync_device", retval, _("while trying to flush %s"),
                    ctx->filesystem_name);
            fatal_error(ctx, 0);
        }
        close(fd);
    ...
    {
        fprintf(stderr, "%s",
                _("The -c and the -l/-L options may not "
    ...
    if (jbd_debug)
    {
        ...
        if (res != 1)
        {
            fprintf(stderr, _("E2FSCK_JBD_DEBUG \"%s\" not an integer\n\n"),
                    jbd_debug);
            exit(1);
        }
    }
                             ...
                             ext2_filsys* ret_fs)
{
    ...
    *ret_fs = NULL;
    ...
    {
        int blocksize;
        for (blocksize = EXT2_MIN_BLOCK_SIZE; blocksize <= EXT2_MAX_BLOCK_SIZE;
             blocksize *= 2)
        {
            if (*ret_fs)
            {
        ...
        retval = ext2fs_open2(ctx->filesystem_name, ctx->io_options, flags, 0,
                              0, io_ptr, ret_fs);

    if (retval == 0)
    {
        (*ret_fs)->priv_data = ctx;
        e2fsck_set_bitmap_type(*ret_fs, EXT2FS_BMAP64_RBTREE, "default", NULL);
    }
    return retval;
}
...
static errcode_t e2fsck_check_mmp(ext2_filsys fs, e2fsck_t ctx)
{
    ...
    {
        log_out(ctx,
                _("MMP interval is %u seconds and total wait "
...
check_error:
    ...
    {
        com_err(ctx->program_name, retval, "%s", _("while checking MMP block"));
        dump_mmp_msg(fs->mmp_buf, NULL);
    }
    else if (retval == EXT2_ET_MMP_FSCK_ON || retval == EXT2_ET_MMP_UNKNOWN_SEQ)
    {
        com_err(ctx->program_name, retval, "%s", _("while checking MMP block"));
    ...
    else if (retval == EXT2_ET_MMP_CSUM_INVALID)
    {
        ...
        com_err(ctx->program_name, retval, "%s", _("while reading MMP block"));
    return retval;
}
...
static int e2fsck_setup_tdb(e2fsck_t ctx, io_manager* io_ptr)
{
    ...
    if (!tdb_dir)
    {
        profile_get_string(ctx->profile, "defaults", "undo_dir", 0,
    ...
    {
        retval = errno;
        com_err(ctx->program_name, retval, _("while trying to delete %s"),
                tdb_file);
        goto errout;
    }
...
int main(int argc, char* argv[])
{
    ...
    if (!(ctx->options & E2F_OPT_PREEN) && !(ctx->options & E2F_OPT_NO) &&
        !(ctx->options & E2F_OPT_YES))
    {
        if (!ctx->interactive)
            fatal_error(ctx, _("need terminal for interactive repairs"));
    }
...
restart:
    ...
    if (!ctx->superblock && !(ctx->options & E2F_OPT_PREEN) &&
        !(ctx->flags & E2F_FLAG_SB_SPECIFIED) &&
        ((retval == EXT2_ET_BAD_MAGIC) || (retval == EXT2_ET_SB_CSUM_INVALID) ||
        ...
        if (!fs || (fs->group_desc_count > 1))
        {
            ...
            if ((orig_retval == 0) && retval != 0)
            {
                if (fs)
                    ext2fs_close_free(&fs);
                log_out(ctx,
                        _("%s: %s while using the "
                          "backup blocks"),
                        ctx->program_name, error_message(retval));
                log_out(ctx,
                        _("%s: going back to original "
                          "superblock\n"),
                        ctx->program_name);
        ...
        fs && fs->super)
    {
        ...
        features[2] =
            (sb->s_feature_ro_compat & ~EXT2_LIB_FEATURE_RO_COMPAT_SUPP);
...
failure:
    if (retval)
    {
        ...
        if (retval == EXT2_ET_REV_TOO_HIGH)
        {
            ...
            log_out(ctx,
                    _("You must have %s access to the "
                      "filesystem or be root\n"),
                    (ctx->options & E2F_OPT_READONLY) ? "r/o" : "r/w");
        else if (retval == ENXIO)
            log_out(ctx, "%s", _("Possibly non-existent or swap device?\n"));
        else if (retval == EBUSY)
            log_out(ctx, "%s",
                    _("Filesystem mounted or opened "
            ...
            log_out(ctx, "%s",
                    _("Disk write-protected; use the "
        ...
        else
        {
            ...
            if (retval == EXT2_ET_BAD_MAGIC)
                check_plausibility(ctx->filesystem_name, CHECK_FS_EXIST, NULL);
        }
        fatal_error(ctx, 0);
    }
    /*
    ...
    if (!(ctx->flags & E2F_FLAG_GOT_DEVSIZE))
    {
        ...
        pctx.errcode = ext2fs_get_device_size2(ctx->filesystem_name, blocksize,
                                               &ctx->num_blocks);
        /*
    ...
    if (sb->s_rev_level > E2FSCK_CURRENT_REV)
    {
        com_err(ctx->program_name, EXT2_ET_REV_TOO_HIGH,
                _("while trying to open %s"), ctx->filesystem_name);
    get_newer:
     ...
     */
    if (ctx->device_name == 0 && (sb->s_volume_name[0] != 0))
    {
        ctx->device_name =
            string_copy(ctx, sb->s_volume_name, sizeof(sb->s_volume_name));
    }
    ...
    if (ext2fs_has_feature_mmp(fs->super) && (flags & EXT2_FLAG_SKIP_MMP))
    {
    ...
    if ((ctx->mount_flags & (EXT2_MF_MOUNTED | EXT2_MF_BUSY)) == 0)
    {
        ...
        if (retval)
        {
            com_err(ctx->program_name, retval,
                    _("while checking journal for %s"), ctx->device_name);
            fatal_error(ctx, _("Cannot proceed with file system check"));
        }
    }
    ...
    if (ext2fs_has_feature_journal_needs_recovery(sb))
    {
        ...
        else
        {
            ...
            {
                com_err(ctx->program_name, retval,
                        _("while recovering journal of %s"), ctx->device_name);
            }
            ext2fs_close_free(&ctx->fs);
    ...
    features[2] = (sb->s_feature_ro_compat & ~EXT2_LIB_FEATURE_RO_COMPAT_SUPP);
print_unsupp_features:
    if (features[0] || features[1] || features[2])
    {
        ...
        log_err(ctx, _("%s has unsupported feature(s):"), ctx->filesystem_name);
        ...
        for (i = 0; i < 3; i++, mask++)
        {
            ...
            {
                if (*mask & m)
                    log_err(ctx, " %s", e2p_feature2string(i, m));
            }
        }
        log_err(ctx, "\n");
        goto get_newer;
     ...
     */
    if (ctx->superblock && !(ctx->options & E2F_OPT_READONLY))
        ext2fs_mark_super_dirty(fs);
    ...
    if (sb->s_jnl_backup_type == EXT3_JNL_BACKUP_BLOCKS)
        journal_size =
            (sb->s_jnl_blocks[15] << (32 - 20)) | (sb->s_jnl_blocks[16] >> 20);
    else
        journal_size = -1;
    ...
    if (!ctx->invalid_bitmaps && (ctx->flags & E2F_FLAG_JOURNAL_INODE))
    {
        if (fix_problem(ctx, PR_6_RECREATE_JOURNAL, &pctx))
        {
            if (journal_size < 1024)
                journal_size =
                    ext2fs_default_journal_size(ext2fs_blocks_count(fs->super));
            if (journal_size < 0)
            {
            ...
            log_out(ctx, _("Creating journal (%d blocks): "), journal_size);
            fflush(stdout);
            retval = ext2fs_add_journal_inode(fs, journal_size, 0);
            if (retval)
            {
                log_out(ctx,
                        "%s: while trying to create "
                        "journal\n",
...
no_journal:
    ...
    if (ext2fs_has_feature_shared_blocks(ctx->fs->super) &&
        (ctx->options & E2F_OPT_UNSHARE_BLOCKS) && (ctx->options & E2F_OPT_NO))
...
skip_write:
    if (!ext2fs_test_valid(fs) ||
        ((exit_value & FSCK_CANCELED) && (sb->s_state & EXT2_ERROR_FS)))
    {
        log_out(ctx,
    ...
    if (exit_value & FSCK_CANCELED)
    {
        ...
        profile_get_boolean(ctx->profile, "options", "allow_cancellation", 0, 0,
                            &allow_cancellation);
        exit_value &= ~FSCK_NONDESTRUCT;
        if (allow_cancellation && ext2fs_test_valid(fs) &&
            (sb->s_state & EXT2_VALID_FS) && !(sb->s_state & EXT2_ERROR_FS))
            exit_value = 0;
    }
    else
        show_stats(ctx);
