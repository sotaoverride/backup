...
{
    hosts->by_address =
        hashmap_free_with_destructor(hosts->by_address, etc_hosts_item_free);
    hosts->by_name = hashmap_free_with_destructor(hosts->by_name,
...
static int parse_line(EtcHosts* hosts, unsigned nr, const char* line)
{
    ...
    if (r < 0)
        return log_error_errno(
            r, "/etc/hosts:%u: failed to extract address: %m", nr);
    assert(r > 0); /* We already checked that the line is not empty, so it
                      should contain *something* */

    r = in_addr_ifindex_from_string_auto(address_str, &address.family,
                                         &address.address, NULL);
    if (r < 0)
    {
        log_warning_errno(
            r, "/etc/hosts:%u: address '%s' is invalid, ignoring: %m", nr,
            address_str);
        return 0;
    }
    ...
    if (r > 0)
        /* This is an 0.0.0.0 or :: item, which we assume means that we shall
         * map the specified hostname to nothing. */
        item = NULL;
    else
    {
        /* If this is a normal address, then simply add entry mapping it to the
         * specified names */

        item = hashmap_get(hosts->by_address, &address);
        if (!item)
        {
            r = hashmap_ensure_allocated(&hosts->by_address,
    ...
    for (;;)
    {
        ...
        if (r < 0)
            return log_error_errno(
                r, "/etc/hosts:%u: couldn't extract host name: %m", nr);
        if (r == 0)
            break;
        ...
        {
            log_warning_errno(
                r, "/etc/hosts:%u: hostname \"%s\" is not valid, ignoring.", nr,
                name);
            continue;
        }
        ...
        if (!item)
        {
            /* Optimize the case where we don't need to store any addresses, by
             * storing only the name in a dedicated Set instead of the hashmap
             */
        ...
        if (!GREEDY_REALLOC(bn->addresses, bn->n_allocated,
                            bn->n_addresses + 1))
            return log_oom();
...
static int manager_etc_hosts_read(Manager* m)
{
    ...
    if (m->etc_hosts_last != USEC_INFINITY &&
        m->etc_hosts_last + ETC_HOSTS_RECHECK_USEC > ts)
        return 0;
    ...
    if (m->etc_hosts_mtime != USEC_INFINITY)
    {
        ...
        /* Did the mtime change? If not, there's no point in re-reading the
         * file. */
    ...
    /* Take the timestamp at the beginning of processing, so that any changes
     * made later are read on the next invocation */
    r = fstat(fileno(f), &st);
    if (r < 0)
...
int manager_etc_hosts_lookup(Manager* m, DnsQuestion* q, DnsAnswer** answer)
{
    ...
    if (r > 0)
    {
        ...
        /* We have an address in /etc/hosts that matches the queried name. Let's
         * return successful. Actual data we'll only return if the request was
         * for PTR. */
    ...
    {
        /* Check if name was listed with no address. If yes, continue to return
         * an answer. */
    ...
    for (i = 0; bn && i < bn->n_addresses; i++)
    {
        ...
        r = dns_resource_record_new_address(&rr, bn->addresses[i]->family,
                                            &bn->addresses[i]->address,
                                            bn->name);
        if (r < 0)
            return r;
