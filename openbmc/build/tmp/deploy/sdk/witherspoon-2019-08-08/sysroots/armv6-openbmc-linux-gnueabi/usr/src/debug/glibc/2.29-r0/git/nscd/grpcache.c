...
const struct iovec grp_iov_disabled = {.iov_base = (void*)&disabled,
                          ...
                          int errval)
{
    ...
    else
    {
        ...
        int key_offset;
        n = snprintf(buf, buf_len, "%d%c%n%s", grp->gr_gid, '\0', &key_offset,
                     (char*)key) +
            1;
        ...
        total =
            (offsetof(struct dataset, strdata) + gr_mem_cnt * sizeof(uint32_t) +
       ...
       stored on the stack.  */
        if (!dataset_temporary)
        {
            ...
            if ((req->type == GETGRBYNAME || db->propagate) &&
                __builtin_expect(cache_add(GETGRBYNAME, gr_name, gr_name_len,
...
static int lookup(int type, union keytype key, struct group* resultbufp,
                  char* buffer, size_t buflen, struct group** grp)
{
    if (type == GETGRBYNAME)
...
time_t readdgrbyname(struct database_dyn* db, struct hashentry* he,
                     struct datahead* dh)
{
    request_header req = {.type = GETGRBYNAME, .key_len = he->len};
    union keytype u = {.v = db->data + he->key};

    return addgrbyX(db, -1, &req, u, db->data + he->key, he->owner, he, dh);
}

void addgrbygid(struct database_dyn* db, int fd, request_header* req, void* key,
                uid_t uid)
{
    char* ep;
                    ...
                    struct datahead* dh)
{
    ...
    request_header req = {.type = GETGRBYGID, .key_len = he->len};
    union keytype u = {.g = gid};
