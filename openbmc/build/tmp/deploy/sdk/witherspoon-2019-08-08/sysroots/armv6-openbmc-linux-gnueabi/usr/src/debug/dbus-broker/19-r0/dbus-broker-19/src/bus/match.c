...
static int match_keys_assign(MatchKeys* keys, const char* key, size_t n_key,
                             const char* value)
{
    ...
    else if (match_key_equal("sender", key, n_key))
    {
        ...
        {
            /*
             * Usually rules are indexed by sender, however, we also allow rules
             * that match on a sender ID to not be indexed, in case the peer
             * with the ID does not yet exits. Therefore, we must also remember
             * the sender id.
             */
    ...
    {
        if (keys->arg0namespace || keys->filter.args[0] ||
            keys->filter.argpaths[0])
            return MATCH_E_INVALID;
        if (!dbus_validate_namespace(value, strlen(value)))
            return MATCH_E_INVALID;
        keys->arg0namespace = value;
    }
    else if (n_key >= strlen("arg") &&
             match_key_equal("arg", key, strlen("arg")))
    {
        unsigned int i = 0;
...
static int match_parse_key(const char** match, const char** keyp,
                           size_t* n_keyp)
{
    const char* key;
    size_t n_key = 0;
...
static bool match_string_prefix(const char* string, const char* prefix,
                                char delimiter, bool delimiter_included)
{
    char* tail;
...
static bool match_keys_match_metadata(MatchKeys* keys,
                                      MessageMetadata* metadata)
{
    ...
    if (keys->path_namespace &&
        !match_string_prefix(metadata->fields.path, keys->path_namespace, '/',
                             false))
        return false;

    if (keys->arg0namespace &&
        !(metadata->args[0].element == 's' &&
          match_string_prefix(metadata->args[0].value, keys->arg0namespace, '.',
                              false)))
        return false;

    for (unsigned int i = 0;
         i < keys->filter.n_args || i < keys->filter.n_argpaths; i++)
    {
        if (keys->filter.args[i] &&
            !(metadata->args[0].element == 's' &&
              c_string_equal(keys->filter.args[i], metadata->args[i].value)))
            return false;

        if (keys->filter.argpaths[i])
        {
            if (!match_string_prefix(metadata->args[i].value,
                                     keys->filter.argpaths[i], '/', true) &&
                !match_string_prefix(keys->filter.argpaths[i],
                                     metadata->args[0].value, '/', true))
                return false;
        }
    }

    if (keys->filter.type != DBUS_MESSAGE_TYPE_INVALID &&
        keys->filter.type != metadata->header.type)
        return false;

    if (keys->filter.sender != ADDRESS_ID_INVALID &&
        keys->filter.sender != metadata->sender_id)
        return false;

    if (keys->filter.interface &&
        !c_string_equal(keys->filter.interface, metadata->fields.interface))
        return false;

    if (keys->filter.member &&
        !c_string_equal(keys->filter.member, metadata->fields.member))
        return false;

    if (keys->filter.path &&
        !c_string_equal(keys->filter.path, metadata->fields.path))
        return false;
...
static int match_registry_by_path_compare(CRBTree* tree, void* k, CRBNode* rb)
{
    MatchRegistryByPath* registry =
        c_container_of(rb, MatchRegistryByPath, registry_node);
    const char *path1 = k ?: "", *path2 = registry->path;

    return strcmp(path1, path2);
}

static int match_registry_by_path_new(MatchRegistryByPath** registryp,
                                      const char* path)
{
    MatchRegistryByPath* registry;
    size_t n_path;
...
static MatchRegistryByPath*
    match_registry_by_path_ref(MatchRegistryByPath* registry)
{
    if (!registry)
        return NULL;
...
static MatchRegistryByPath*
    match_registry_by_path_unref(MatchRegistryByPath* registry)
{
...
static void match_registry_by_path_link(MatchRegistryByPath* registry,
                                        CRBTree* tree, CRBNode* parent,
                                        CRBNode** slot)
{
    c_rbtree_add(tree, parent, slot, &registry->registry_node);
}

static int match_registry_by_interface_compare(CRBTree* tree, void* k,
                                               CRBNode* rb)
{
    MatchRegistryByInterface* registry =
        c_container_of(rb, MatchRegistryByInterface, registry_node);
    const char *interface1 = k ?: "", *interface2 = registry->interface;

    return strcmp(interface1, interface2);
}

static int match_registry_by_interface_new(MatchRegistryByInterface** registryp,
                                           const char* interface)
{
    ...
    *registry =
        (MatchRegistryByInterface)MATCH_REGISTRY_BY_INTERFACE_INIT(*registry);
...
static MatchRegistryByInterface*
    match_registry_by_interface_ref(MatchRegistryByInterface* registry)
{
    if (!registry)
        return NULL;
...
static MatchRegistryByInterface*
    match_registry_by_interface_unref(MatchRegistryByInterface* registry)
{
    if (!registry || --registry->n_refs > 0)
        return NULL;
...
static void
    match_registry_by_interface_link(MatchRegistryByInterface* registry,
                                     MatchRegistryByPath* registry_by_path,
                                     CRBNode* parent, CRBNode** slot)
{
    c_rbtree_add(&registry_by_path->interface_tree, parent, slot,
...
static int match_registry_by_member_compare(CRBTree* tree, void* k, CRBNode* rb)
{
    MatchRegistryByMember* registry =
        c_container_of(rb, MatchRegistryByMember, registry_node);
    const char *member1 = k ?: "", *member2 = registry->member;

    return strcmp(member1, member2);
}

static int match_registry_by_member_new(MatchRegistryByMember** registryp,
                                        const char* member)
{
    MatchRegistryByMember* registry;
    size_t n_member;
...
static MatchRegistryByMember*
    match_registry_by_member_ref(MatchRegistryByMember* registry)
{
    if (!registry)
        return NULL;
...
static MatchRegistryByMember*
    match_registry_by_member_unref(MatchRegistryByMember* registry)
{
    if (!registry || --registry->n_refs > 0)
        return NULL;
...
static void match_registry_by_member_link(
    MatchRegistryByMember* registry,
    MatchRegistryByInterface* registry_by_interface, CRBNode* parent,
    CRBNode** slot)
{
    c_rbtree_add(&registry_by_interface->member_tree, parent, slot,
                 &registry->registry_node);
    registry->registry_by_interface =
...
static int match_keys_compare(MatchKeys* key1, MatchKeys* key2)
{
    ...
    if ((r = c_string_compare(key1->sender, key2->sender)) ||
        ...
        (r = c_string_compare(key1->filter.interface,
                              key2->filter.interface)) ||
    ...
    for (size_t i = 0; i < key1->filter.n_args || i < key1->filter.n_argpaths;
         i++)
    {
        if ((r = c_string_compare(key1->filter.args[i],
                                  key2->filter.args[i])) ||
            (r = c_string_compare(key1->filter.argpaths[i],
                                  key2->filter.argpaths[i])))
            return r;
    }
...
static int match_registry_by_keys_compare(CRBTree* tree, void* k, CRBNode* rb)
{
    MatchRegistryByKeys* registry =
        c_container_of(rb, MatchRegistryByKeys, registry_node);
    MatchKeys *keys1 = k, *keys2 = &registry->keys;

    return match_keys_compare(keys1, keys2);
}

static int match_registry_by_keys_new(MatchRegistryByKeys** registryp,
                                      MatchKeys* keys)
{
    MatchRegistryByKeys* registry;
...
static MatchRegistryByKeys*
    match_registry_by_keys_ref(MatchRegistryByKeys* registry)
{
    if (!registry)
        return NULL;
...
static MatchRegistryByKeys*
    match_registry_by_keys_unref(MatchRegistryByKeys* registry)
{
    if (!registry || --registry->n_refs > 0)
        return NULL;
...
static void
    match_registry_by_keys_link(MatchRegistryByKeys* registry,
                                MatchRegistryByMember* registry_by_member,
                                CRBNode* parent, CRBNode** slot)
{
    c_rbtree_add(&registry_by_member->keys_tree, parent, slot,
                 &registry->registry_node);
    registry->registry_by_member =
...
static int match_rule_new(MatchRule** rulep, MatchOwner* owner, User* user,
                          const char* string)
{
    ...
    *rule = (MatchRule)MATCH_RULE_NULL(*rule);
    ...
    r = user_charge(user, &rule->charge[0], NULL, USER_SLOT_BYTES,
                    sizeof(*rule) + n_string);
...
static void match_rule_link_by_keys(MatchRule* rule,
                                    MatchRegistryByKeys* registry)
{
...
static int match_rule_link_by_member(MatchRule* rule,
                                     MatchRegistryByMember* registry)
{
    _c_cleanup_(match_registry_by_keys_unrefp)
        MatchRegistryByKeys* registry_by_keys = NULL;
    CRBNode **slot, *parent;
    int r;

    slot =
        c_rbtree_find_slot(&registry->keys_tree, match_registry_by_keys_compare,
                           &rule->keys, &parent);
    if (!slot)
    {
        registry_by_keys = match_registry_by_keys_ref(
            c_rbnode_entry(parent, MatchRegistryByKeys, registry_node));
    }
    else
    {
...
static int match_rule_link_by_interface(MatchRule* rule,
                                        MatchRegistryByInterface* registry)
{
    _c_cleanup_(match_registry_by_member_unrefp)
        MatchRegistryByMember* registry_by_member = NULL;
    CRBNode **slot, *parent;
    int r;

    slot = c_rbtree_find_slot(&registry->member_tree,
                              match_registry_by_member_compare,
                              rule->keys.filter.member, &parent);
    if (!slot)
    {
        registry_by_member = match_registry_by_member_ref(
            c_rbnode_entry(parent, MatchRegistryByMember, registry_node));
    }
    else
    {
        r = match_registry_by_member_new(&registry_by_member,
                                         rule->keys.filter.member);
        if (r)
            return error_trace(r);

        match_registry_by_member_link(registry_by_member, registry, parent,
                                      slot);
    }
...
static int match_rule_link_by_path(MatchRule* rule,
                                   MatchRegistryByPath* registry)
{
    _c_cleanup_(match_registry_by_interface_unrefp)
        MatchRegistryByInterface* registry_by_interface = NULL;
    CRBNode **slot, *parent;
    int r;

    slot = c_rbtree_find_slot(&registry->interface_tree,
                              match_registry_by_interface_compare,
                              rule->keys.filter.interface, &parent);
    if (!slot)
    {
        registry_by_interface = match_registry_by_interface_ref(
            c_rbnode_entry(parent, MatchRegistryByInterface, registry_node));
    }
    else
    {
        r = match_registry_by_interface_new(&registry_by_interface,
                                            rule->keys.filter.interface);
        if (r)
            return error_trace(r);

        match_registry_by_interface_link(registry_by_interface, registry,
                                         parent, slot);
    }
...
{
    _c_cleanup_(match_registry_by_path_unrefp)
        MatchRegistryByPath* registry_by_path = NULL;
    CRBTree* tree;
    CRBNode **slot, *parent;
    int r;
    ...
    slot = c_rbtree_find_slot(tree, match_registry_by_path_compare,
                              rule->keys.filter.path, &parent);
    if (!slot)
    {
        registry_by_path = match_registry_by_path_ref(
            c_rbnode_entry(parent, MatchRegistryByPath, registry_node));
    }
    else
    {
        r = match_registry_by_path_new(&registry_by_path,
...
void match_rule_unlink(MatchRule* rule)
{
    if (rule->registry)
    {
        ...
        rule->registry_by_keys =
            match_registry_by_keys_unref(rule->registry_by_keys);
        rule->registry = NULL;
    }
}
 ...
 */
int match_owner_ref_rule(MatchOwner* owner, MatchRule** rulep, User* user,
                         const char* rule_string)
{
    ...
    slot = c_rbtree_find_slot(&owner->rule_tree, match_rule_compare,
                              &rule->keys, &parent);
    if (!slot)
    {
        /* one already exists, take a ref on that instead and drop the one we
         * created */
        match_rule_user_unref(rule);
        rule =
            match_rule_user_ref(c_container_of(parent, MatchRule, owner_node));
    }
    else
    {
 ...
 */
int match_owner_find_rule(MatchOwner* owner, MatchRule** rulep,
                          const char* rule_string)
{
    _c_cleanup_(match_keys_freep) MatchKeys* keys = NULL;
    int r;
    ...
    *rulep = c_rbtree_find_entry(&owner->rule_tree, match_rule_compare, keys,
...
static void
    match_registry_by_keys_get_destinations(MatchRegistryByKeys* registry,
                                            CList* destinations)
{
    MatchRule* rule;
    ...
    c_list_for_each_entry(rule, &registry->rule_list, registry_link)
    {
        if (c_list_is_linked(&rule->owner->destinations_link))
            /* only link a destination once, despite matching in several
             * different ways */
            continue;
...
static void
    match_registry_by_member_get_destinations(MatchRegistryByMember* registry,
                                              CList* destinations,
                                              MessageMetadata* metadata)
{
    MatchRegistryByKeys* registry_by_keys;

    c_rbtree_for_each_entry_postorder(registry_by_keys, &registry->keys_tree,
                                      registry_node)
    {
...
static void match_registry_by_interface_get_destinations(
    MatchRegistryByInterface* registry, CList* destinations,
    MessageMetadata* metadata)
{
    MatchRegistryByMember* registry_by_member;

    registry_by_member = c_rbtree_find_entry(
        &registry->member_tree, match_registry_by_member_compare, NULL,
        MatchRegistryByMember, registry_node);
    if (registry_by_member)
        match_registry_by_member_get_destinations(registry_by_member,
                                                  destinations, metadata);

    if (metadata->fields.member)
    {
        registry_by_member = c_rbtree_find_entry(
            &registry->member_tree, match_registry_by_member_compare,
            metadata->fields.member, MatchRegistryByMember, registry_node);
        if (registry_by_member)
            match_registry_by_member_get_destinations(registry_by_member,
                                                      destinations, metadata);
    }
...
static void
    match_registry_by_path_get_destinations(MatchRegistryByPath* registry,
                                            CList* destinations,
                                            MessageMetadata* metadata)
{
    MatchRegistryByInterface* registry_by_interface;

    registry_by_interface = c_rbtree_find_entry(
        &registry->interface_tree, match_registry_by_interface_compare, NULL,
        MatchRegistryByInterface, registry_node);
    if (registry_by_interface)
        match_registry_by_interface_get_destinations(registry_by_interface,
                                                     destinations, metadata);

    if (metadata->fields.interface)
    {
        registry_by_interface = c_rbtree_find_entry(
            &registry->interface_tree, match_registry_by_interface_compare,
            metadata->fields.interface, MatchRegistryByInterface,
            registry_node);
        if (registry_by_interface)
            match_registry_by_interface_get_destinations(
                registry_by_interface, destinations, metadata);
    }
}

static void match_registry_get_destinations(CRBTree* tree, CList* destinations,
                                            MessageMetadata* metadata)
{
    MatchRegistryByPath* registry_by_path;

    registry_by_path =
        c_rbtree_find_entry(tree, match_registry_by_path_compare, NULL,
                            MatchRegistryByPath, registry_node);
    if (registry_by_path)
        match_registry_by_path_get_destinations(registry_by_path, destinations,
                                                metadata);

    if (metadata->fields.path)
    {
        registry_by_path = c_rbtree_find_entry(
            tree, match_registry_by_path_compare, metadata->fields.path,
            MatchRegistryByPath, registry_node);
        if (registry_by_path)
            match_registry_by_path_get_destinations(registry_by_path,
                                                    destinations, metadata);
    }
}

void match_registry_get_subscribers(MatchRegistry* registry,
                                    CList* destinations,
                                    MessageMetadata* metadata)
{
    match_registry_get_destinations(&registry->subscription_tree, destinations,
                                    metadata);
}

void match_registry_get_monitors(MatchRegistry* registry, CList* destinations,
                                 MessageMetadata* metadata)
{
    match_registry_get_destinations(&registry->monitor_tree, destinations,
                                    metadata);
}
...
static void match_registry_by_keys_flush(MatchRegistryByKeys* registry)
{
    ...
    c_list_for_each_entry_safe(rule, rule_safe, &registry->rule_list,
                               registry_link) match_rule_unlink(rule);
...
static void match_registry_by_member_flush(MatchRegistryByMember* registry)
{
    ...
    c_rbtree_for_each_entry_safe(registry_by_keys, registry_by_keys_safe,
                                 &registry->keys_tree, registry_node)
    {
...
static void
    match_registry_by_interface_flush(MatchRegistryByInterface* registry)
{
    MatchRegistryByMember *registry_by_member, *registry_by_member_safe;

    c_rbtree_for_each_entry_safe(registry_by_member, registry_by_member_safe,
                                 &registry->member_tree, registry_node)
    {
        match_registry_by_member_ref(registry_by_member);
...
{
    MatchRegistryByInterface *registry_by_interface,
        *registry_by_interface_safe;

    c_rbtree_for_each_entry_safe(registry_by_interface,
                                 registry_by_interface_safe,
                                 &registry->interface_tree, registry_node)
    {
        match_registry_by_interface_ref(registry_by_interface);
...
void match_registry_flush(MatchRegistry* registry)
{
    ...
    c_rbtree_for_each_entry_safe(registry_by_path, registry_by_path_safe,
                                 &registry->subscription_tree, registry_node)
    {
