...
#if defined(NETSNMP_TRANSPORT_DTLSUDP_DOMAIN) ||                               \
    defined(NETSNMP_TRANSPORT_TLSTCP_DOMAIN)
...
netsnmp_feature_require(tdomain_support) netsnmp_feature_require(
    tdomain_transport_oid) netsnmp_feature_want(netsnmp_tlstmAddr_get_serverId)
                                         ...
                                         void* filterArg)
{
    ...
    for (targaddrs = get_addrTable(); targaddrs; targaddrs = targaddrs->next)
    {
         ...
         * legal row?
         */
        if (targaddrs->tDomain == NULL || targaddrs->tAddress == NULL ||
                ...
                for (i = 0; i < numtags && !matched; i++)
                {
                    if (strcmp(buf, tags[i]) == 0)
                    {
                        ...
                        if (targaddrs->storageType != ST_READONLY &&
                            targaddrs->sess &&
                            param->updateTime >= targaddrs->sessionCreationTime)
                        {
                            /*
                        ...
                        if (targaddrs->sess == NULL)
                        {
                            ...
                            t = netsnmp_tdomain_transport_oid(
                                targaddrs->tDomain, targaddrs->tDomainLen,
    ...
    defined(NETSNMP_TRANSPORT_TLSTCP_DOMAIN)
                            if (!tls)
                            {
                                ...
                                cert = netsnmp_cert_find(
                                    NS_CERT_IDENTITY, NS_CERTKEY_TARGET_PARAM,
                                    targaddrs->paramsData);
                                ...
                                DEBUGMSGTL(("target_sessions",
                                            ...
                                            buf));
                                cert = netsnmp_cert_find(NS_CERT_REMOTE_PEER,
                            ...
                            {
                                snmp_log(LOG_ERR,
                                         "unsupported mpModel/secModel combo "
                                         "%d/%d for target %s\n",
                            ...
                            if (param->mpModel == SNMP_VERSION_3)
                            {
                                thissess.securityName = netsnmp_memdup_nt(
                                    param->secNameData, param->secNameLen,
                            ...
                            {
                                thissess.community = netsnmp_memdup_nt(
                                    param->secNameData, param->secNameLen,
                        ...
                        if (targaddrs->sess)
                        {
                            if (NULL == targaddrs->sess->paramName)
                                targaddrs->sess->paramName = netsnmp_memdup_nt(
                                    param->paramNameData, param->paramNameLen,
                                    NULL);
