...
static int link_proc(struct ext2_dir_entry* dirent, int offset, int blocksize,
                     char* buf, void* priv_data)
{
    ...
    if (csum_size && curr_rec_len == ls->fs->blocksize && !dirent->inode)
    {
    ...
    if (csum_size && curr_rec_len == ls->fs->blocksize - EXT2_DIR_REC_LEN(1) &&
        offset == EXT2_DIR_REC_LEN(1) && dirent->name[0] == '.' &&
        dirent->name[1] == '.')
    {
        curr_rec_len -= csum_size;
        ls->err = ext2fs_set_rec_len(ls->fs, curr_rec_len, dirent);
                      ...
                      ext2_ino_t ino, int flags)
{
    ...
    retval = ext2fs_dir_iterate(fs, dir, DIRENT_FLAG_INCLUDE_EMPTY, 0,
                                link_proc, &ls);
    if (retval)
        return retval;
    if (ls.err)
