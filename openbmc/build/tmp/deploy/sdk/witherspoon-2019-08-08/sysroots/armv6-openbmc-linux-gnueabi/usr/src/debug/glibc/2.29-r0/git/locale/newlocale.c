    ...
    locale_t __newlocale(int category_mask, const char* locale, locale_t base)
{
    ...
    {
        if (__argz_create_sep(locpath_var, ':', &locale_path,
                              &locale_path_len) != 0)
            return NULL;
        ...
        while ((cp = strchr(np, '=')) != NULL)
        {
            for (cnt = 0; cnt < __LC_LAST; ++cnt)
                if (cnt != LC_ALL &&
                    ...
                    memcmp(
                        np,
                        (_nl_category_names.str + _nl_category_name_idxs[cnt]),
                        cp - np) == 0)
                    break;
    ...
    for (cnt = 0; cnt < __LC_LAST; ++cnt)
    {
        ...
        {
            result.__locales[cnt] = _nl_find_locale(
                locale_path, locale_path_len, cnt, &newnames[cnt]);
...
update:
{
    ...
    result_ptr->__ctype_b =
        (const unsigned short int*)ctypes[_NL_ITEM_INDEX(_NL_CTYPE_CLASS)]
            .string +
        128;
    result_ptr->__ctype_tolower =
        (const int*)ctypes[_NL_ITEM_INDEX(_NL_CTYPE_TOLOWER)].string + 128;
    result_ptr->__ctype_toupper =
        (const int*)ctypes[_NL_ITEM_INDEX(_NL_CTYPE_TOUPPER)].string + 128;
}
