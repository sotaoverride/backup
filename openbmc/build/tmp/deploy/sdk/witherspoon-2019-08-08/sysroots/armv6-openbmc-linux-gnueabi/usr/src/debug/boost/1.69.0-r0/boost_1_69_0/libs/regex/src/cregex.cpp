...
unsigned int RegEx::SetExpression(const char* p, bool icase)
{
    boost::uint_fast32_t f =
        icase ? regex::normal | regex::icase : regex::normal;
                         ...
                         match_flag_type flags)
{
    ...
    unsigned int result = regex_grep(BOOST_REGEX_DETAIL_NS::pred3(v, p, this),
                                     p, end, pdata->e, flags);
    if (result)
        pdata->update();
    return result;
...
void BuildFileList(std::list<std::string>* pl, const char* files, bool recurse)
{
    ...
    if (recurse)
    {
        ...
        if (*buf == 0)
        {
            ...
            BOOST_REGEX_DETAIL_NS::overflow_error_if_not_zero(
                BOOST_REGEX_DETAIL_NS::strcat_s(
                    buf, MAX_PATH, directory_iterator::separator()));
            BOOST_REGEX_DETAIL_NS::overflow_error_if_not_zero(
        ...
        while (dstart != dend)
        {
            ...
            if (std::strlen(dstart.path()) +
                    std::strlen(directory_iterator::separator()) +
                    std::strlen(ptr) >=
                MAX_PATH)
            {
    ...
    switch (pdata->t)
    {
        ...
        case BOOST_REGEX_DETAIL_NS::RegExData::type_pf:
            ...
            return pdata->fm[i].matched
                       ? pdata->fm[i].second - pdata->fm[i].first
                       : RegEx::npos;
#endif
        case BOOST_REGEX_DETAIL_NS::RegExData::type_copy:
        {
    ...
    char* f1, char* f2, const char* i1, const char* i2)
{
    ...
    if (insert_len > remove_len)
    {
        ...
        std::copy_backward(begin() + insert_pos + remove_len,
                           begin() + org_size, end());
