...
static kdb_log_context* create_log_context(krb5_context context)
{
...
static inline krb5_boolean time_equal(const kdbe_time_t* a,
                                      const kdbe_time_t* b)
{
    return a->seconds == b->seconds && a->useconds == b->useconds;
}

static void time_current(kdbe_time_t* out)
{
...
static void sync_update(kdb_hlog_t* ulog, kdb_ent_header_t* upd)
{
...
static void sync_header(kdb_hlog_t* ulog)
{
    if (!pagesize)
        pagesize = getpagesize();
...
static krb5_boolean check_sno(kdb_log_context* log_ctx, kdb_sno_t sno,
 ...
 * (UPDATE_OK).
 */
static update_status_t get_sno_status(kdb_log_context* log_ctx,
...
static krb5_error_code extend_file_to(int fd, unsigned int new_size)
{
 ...
 */
static krb5_error_code resize(kdb_hlog_t* ulog, uint32_t ulogentries,
                              int ulogfd, unsigned int recsize)
{
 ...
 * timestamp. */
static void set_dummy(kdb_log_context* log_ctx, kdb_sno_t sno,
                      const kdbe_time_t* kdb_time)
{
    kdb_hlog_t* ulog = log_ctx->ulog;
...
static void reset_ulog(kdb_log_context* log_ctx)
{
 ...
 * deadlock may result.
 */
static krb5_error_code lock_ulog(krb5_context context, int mode)
{
...
static void unlock_ulog(krb5_context context)
{
 ...
 */
static krb5_error_code store_update(kdb_log_context* log_ctx,
                                    kdb_incr_update_t* upd)
{
    ...
    xdrmem_create(&xdrs, (char*)indx_log->entry_data, indx_log->kdb_entry_size,
                  XDR_ENCODE);
