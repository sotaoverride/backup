...
#define UNIX_AUDIT                                                             \
    3 /* print more things than debug..                                        \
...
#define UNIX_BROKEN_SHADOW                                                     \
    22                      /* ignore errors reading password aging            \
...
#define UNIX_ALGO_ROUNDS                                                       \
    25                        /* optional number of rounds for new             \
                 password hash algorithms */
#define UNIX_BLOWFISH_PASS 26 /* new password hashes will use blowfish */
#define UNIX_MIN_PASS_LEN 27  /* min length for password */
#define UNIX_QUIET 28         /* Don't print informational messages */
#define UNIX_NO_PASS_EXPIRY                                                    \
    29 /* Don't check for password expiration if not used for authentication   \
        */
...
#define UNIX_DES_CRYPT(ctrl)                                                   \
    (off(UNIX_MD5_PASS, ctrl) && off(UNIX_BIGCRYPT, ctrl) &&                   \
     off(UNIX_SHA256_PASS, ctrl) && off(UNIX_SHA512_PASS, ctrl) &&             \
     off(UNIX_BLOWFISH_PASS, ctrl))
...
static const UNIX_Ctrls unix_args[UNIX_CTRLS_] = {
    ...
    /* UNIX_BLOWFISH_PASS */
    {"blowfish", _ALL_ON_ ^ (0260420000), 0200000000, 1},
    /* UNIX_MIN_PASS_LEN */ {"minlen=", _ALL_ON_, 0400000000, 0},
    /* UNIX_QUIET */ {"quiet", _ALL_ON_, 01000000000, 0},
    /* UNIX_NO_PASS_EXPIRY */ {"no_pass_expiry", _ALL_ON_, 02000000000, 0},
    /* UNIX_DES */ {"des", _ALL_ON_ ^ (0260420000), 0, 1},
    /* UNIX_NULLOK_SECURE */
    {"nullok_secure", _ALL_ON_ ^ (01000), 02000000000, 0},
};
...
extern int _make_remark(pam_handle_t* pamh, unsigned int ctrl, int type,
                        const char* text);
extern int _set_ctrl(pam_handle_t* pamh, int flags, int* remember, int* rounds,
                     int* pass_min_len, int argc, const char** argv);
extern int _unix_getpwnam(pam_handle_t* pamh, const char* name, int files,
                          int nis, struct passwd** ret);
extern int _unix_comesfromsource(pam_handle_t* pamh, const char* name,
                                 int files, int nis);
extern int _unix_blankpasswd(pam_handle_t* pamh, unsigned int ctrl,
                             const char* name);
extern int _unix_verify_password(pam_handle_t* pamh, const char* name,
                                 const char* p, unsigned int ctrl);
extern int _unix_read_password(pam_handle_t* pamh, unsigned int ctrl,
...
extern int _unix_run_verify_binary(pam_handle_t* pamh, unsigned int ctrl,
