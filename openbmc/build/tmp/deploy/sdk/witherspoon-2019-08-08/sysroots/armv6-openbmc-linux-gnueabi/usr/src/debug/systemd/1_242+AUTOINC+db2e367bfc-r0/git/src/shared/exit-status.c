...
const char* exit_status_to_string(int status, ExitStatusLevel level)
{
     ...
     * 200…241 │ systemd's private error codes (might be extended to 254 in
     * future development) 242…254 │ (Currently unmapped, but see above)
     *
     *   255   │ EXIT_EXCEPTION (We use this to propagate exit-by-signal events.
     * It's frequently used by others apps (like bash) │ to indicate exit reason
     * that cannot really be expressed in a single exit status value — such as a
     * propagated │ signal or such, and we follow that logic here.)
     */
...
bool is_clean_exit(int code, int status, ExitClean clean,
                   ExitStatusSet* success_status)
{
    ...
    /* If a daemon does not implement handlers for some of the signals that's
     * not considered an unclean shutdown */
    if (code == CLD_KILLED)
        return (clean == EXIT_CLEAN_DAEMON &&
                IN_SET(status, SIGHUP, SIGINT, SIGTERM, SIGPIPE)) ||
               (success_status &&
...
bool exit_status_set_test(ExitStatusSet* x, int code, int status)
{
    ...
    if (IN_SET(code, CLD_KILLED, CLD_DUMPED) &&
        set_contains(x->signal, INT_TO_PTR(status)))
        return true;
