    ...
    void init_mteTriggerConf(void)
{
    ...
    snmpd_register_const_config_handler(
        "monitor", parse_mteMonitor, NULL,
        "triggername [-I] [-i OID | -o OID]* [-e event] expression ");
    snmpd_register_const_config_handler(
        "defaultMonitors", parse_default_mteMonitors, NULL, "yes|no");
     ...
     */
    snmpd_register_config_handler("_mteTTable", parse_mteTTable, NULL, NULL);
    snmpd_register_config_handler("_mteTDTable", parse_mteTDTable, NULL, NULL);
    snmpd_register_config_handler("_mteTExTable", parse_mteTExTable, NULL,
                                  NULL);
    snmpd_register_config_handler("_mteTBlTable", parse_mteTBlTable, NULL,
                                  NULL);
    snmpd_register_config_handler("_mteTThTable", parse_mteTThTable, NULL,
                                  NULL);
    ...
    snmp_register_callback(SNMP_CALLBACK_APPLICATION,
                           SNMPD_CALLBACK_PRE_UPDATE_CONFIG, clear_mteTTable,
                           NULL);
}
 ...
 */
struct mteTrigger* _find_mteTrigger_entry(const char* owner, char* tname)
{
    ...
    snmp_set_var_typed_value(&tname_var, ASN_PRIV_IMPLIED_OCTET_STR, tname,
                             strlen(tname));
...
struct mteTrigger* _find_typed_mteTrigger_entry(const char* owner, char* tname,
                                                int type)
{
...
int _mteTrigger_callback_enable(int majorID, int minorID, void* serverargs,
                                void* clientarg)
{
...
void parse_mteMonitor(const char* token, const char* line)
{
    ...
    long flags = MTE_TRIGGER_FLAG_ENABLED | MTE_TRIGGER_FLAG_ACTIVE |
                 MTE_TRIGGER_FLAG_FIXED | MTE_TRIGGER_FLAG_VWILD |
                 MTE_TRIGGER_FLAG_SYSUPT | MTE_TRIGGER_FLAG_VALID;
    long idx = 0;
    long startup = 1; /* ??? or 0 */
    long repeat = 600;
    ...
    DEBUGMSGTL(
        ("disman:event:conf", "Parsing disman monitor config (%s)\n", line));
        ...
        if (buf[0] == '-')
        {
            switch (buf[1])
            {
                ...
                case 'i':
                    ...
                    cp = copy_nword_const(cp, buf, SPRINT_MAX_LEN);
                    object =
                        mteObjects_addOID("snmpd.conf", tname, idx, buf, 0);
                    if (!object)
                    {
                ...
                case 'o': /*  object  */
                    ...
                    cp = copy_nword_const(cp, buf, SPRINT_MAX_LEN);
                    object =
                        mteObjects_addOID("snmpd.conf", tname, idx, buf, 1);
                    if (!object)
                    {
                        snmp_log(LOG_ERR, "Unknown payload OID: %s\n", buf);
                        config_perror("Unknown payload OID");
            ...
            else
            {
                ...
                switch (test)
                {
                    ...
                    case MTE_TRIGGER_BOOLEAN:
                        ...
                        cp = NULL; /* To terminate the processing loop */
                        DEBUGMSGTL(("disman:event:conf",
                                    "%s: Bool (%s, %d, %ld)\n", tname,
                                    oid_name_buf, op, value));
                        break;
    ...
    if (NULL == sess)
    {
        ...
        if (NULL == sess)
        {
            config_perror("You must specify a default user name using the "
                          "agentSecName token\n");
            mteObjects_removeEntries("snmpd.conf", tname);
            return;
        }
    ...
    switch (test)
    {
        ...
        case MTE_TRIGGER_THRESHOLD:
            ...
            if (cp && *cp != '\0')
            {
                ...
                {
                    config_perror(
                        "Delta-threshold on delta-samples not supported");
                    mteObjects_removeEntries("snmpd.conf", tname);
                    return;
                }
...
char* _parse_mteTCols(char* line, struct mteTrigger* entry, int bcomp)
{
    ...
    else
    {
        ...
        line = read_config_read_data(ASN_UNSIGNED, line, &tmp, NULL);
        entry->flags |=
            (tmp & (MTE_TRIGGER_FLAG_VWILD | MTE_TRIGGER_FLAG_CWILD |
...
void parse_mteTDTable(const char* token, char* line)
{
    ...
    DEBUGMSGTL(
        ("disman:event:conf", "Parsing mteTriggerDeltaTable config... "));
...
void parse_mteTExTable(const char* token, char* line)
{
    ...
    DEBUGMSGTL(
        ("disman:event:conf", "Parsing mteTriggerExistenceTable config...  "));
...
char* _parse_mteTBlCols(char* line, struct mteTrigger* entry, int bcomp)
{
    ...
    if (bcomp)
    {
        ...
        line = read_config_read_data(ASN_INTEGER, line, &entry->mteTBoolValue,
                                     NULL);
        line = read_config_read_data(ASN_UNSIGNED, line, &tmp, NULL);
        if (tmp == TV_TRUE)
            entry->flags |= MTE_TRIGGER_FLAG_BSTART;
...
void parse_mteTBlTable(const char* token, char* line)
{
    ...
    DEBUGMSGTL(
        ("disman:event:conf", "Parsing mteTriggerBooleanTable config...  "));
...
char* _parse_mteTThCols(char* line, struct mteTrigger* entry, int bcomp)
{
    ...
    line =
        read_config_read_data(ASN_UNSIGNED, line, &entry->mteTThStartup, NULL);
    line =
        read_config_read_data(ASN_INTEGER, line, &entry->mteTThRiseValue, NULL);
    line =
        read_config_read_data(ASN_INTEGER, line, &entry->mteTThFallValue, NULL);
    line = read_config_read_data(ASN_INTEGER, line, &entry->mteTThDRiseValue,
                                 NULL);
    line = read_config_read_data(ASN_INTEGER, line, &entry->mteTThDFallValue,
                                 NULL);
...
void parse_mteTThTable(const char* token, char* line)
{
    ...
    DEBUGMSGTL(
        ("disman:event:conf", "Parsing mteTriggerThresholdTable config...  "));
...
void parse_mteTriggerTable(const char* token, char* line)
{
    ...
    DEBUGMSGTL(
        ("disman:event:conf", "Parsing previous mteTriggerTable config...  "));
...
int store_mteTTable(int majorID, int minorID, void* serverarg, void* clientarg)
{
    ...
    for (row = netsnmp_tdata_row_first(trigger_table_data); row;
         row = netsnmp_tdata_row_next(trigger_table_data, row))
    {
        ...
        cptr = read_config_store_data(ASN_OCTET_STR, cptr, &cp, &tint);
        tint =
            entry->flags & (MTE_TRIGGER_FLAG_VWILD | MTE_TRIGGER_FLAG_CWILD |
                            MTE_TRIGGER_FLAG_ENABLED | MTE_TRIGGER_FLAG_ACTIVE);
        cptr = read_config_store_data(ASN_UNSIGNED, cptr, &tint, NULL);
...
int clear_mteTTable(int majorID, int minorID, void* serverarg, void* clientarg)
{
    ...
    {
        struct mteTrigger* entry =
            (struct mteTrigger*)netsnmp_tdata_remove_and_delete_row(
                trigger_table_data, row);
        if (entry)
        {
