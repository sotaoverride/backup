...
struct fdisk_sun_label
{
    ...
    struct sun_disklabel*
        header; /* on-disk data (pointer to cxt->firstsector) */
};
...
static void set_partition(struct fdisk_context* cxt, size_t i, uint32_t start,
                          uint32_t stop, uint16_t sysid)
{
...
static int sun_probe_label(struct fdisk_context* cxt)
{
    ...
    {
        fdisk_warnx(
            cxt, _("Detected sun disklabel with wrong vtoc.sanity [0x%08x]."),
...
static int sun_create_disklabel(struct fdisk_context* cxt)
{
    ...
    if (cxt->geom.heads && cxt->geom.sectors)
    {
        ...
        if (blkdev_get_sectors(cxt->dev_fd, (unsigned long long*)&llsectors) ==
            0)
        {
            ...
            llcyls =
                llsectors / (cxt->geom.heads * cxt->geom.sectors * sec_fac);
    ...
    snprintf((char*)sunlabel->label_id, sizeof(sunlabel->label_id),
             ...
             (uintmax_t)cxt->geom.cylinders, be16_to_cpu(sunlabel->acyl),
             cxt->geom.heads, (uintmax_t)cxt->geom.sectors);

    if (cxt->geom.cylinders * cxt->geom.heads * cxt->geom.sectors >= 150 * 2048)
    {
        ndiv =
            cxt->geom.cylinders -
            (50 * 2048 / (cxt->geom.heads * cxt->geom.sectors)); /* 50M swap */
    }
    else
...
static int sun_toggle_partition_flag(struct fdisk_context* cxt, size_t i,
                                     unsigned long flag)
{
    struct sun_disklabel* sunlabel;
    struct sun_info* p;
                      ...
                      uint32_t* lens, uint32_t* start, uint32_t* stop)
{
    ...
    *stop = cxt->geom.cylinders * sectors_per_cylinder;
    ...
    for (i = 0; i < cxt->label->nparts_max; i++)
    {
        ...
        {
            starts[i] =
                be32_to_cpu(part->start_cylinder) * sectors_per_cylinder;
    ...
    for (k = 0; k < 7; k++)
    {
        for (i = 0; i < SUN_MAXPARTITIONS; i++)
        {
            if (k && (lens[i] % (cxt->geom.heads * cxt->geom.sectors)))
                fdisk_warnx(cxt,
                            _("Partition %u doesn't end on cylinder boundary."),
                            i + 1);
            if (lens[i])
            {
                for (j = 0; j < i; j++)
...
static int is_free_sector(struct fdisk_context* cxt, fdisk_sector_t s,
                          uint32_t starts[], uint32_t lens[])
{
    size_t i;

    for (i = 0; i < cxt->label->nparts_max; i++)
    {
        if (lens[i] && starts[i] <= s && starts[i] + lens[i] > s)
            return 0;
    }
    return 1;
}

static int sun_add_partition(struct fdisk_context* cxt,
                             struct fdisk_partition* pa, size_t* partno)
{
    ...
    else
    {
        ...
        for (;;)
        {
            ...
            else if (n == 2)
            {
                fdisk_ask_number_set_low(ask, 0);     /* minimal */
                fdisk_ask_number_set_default(ask, 0); /* default */
                fdisk_ask_number_set_high(
                    ask, fdisk_scround(cxt, stop)); /* maximal */
            }
            else
            {
                fdisk_ask_number_set_low(
                    ask, fdisk_scround(cxt, start)); /* minimal */
                fdisk_ask_number_set_default(
                    ask, fdisk_scround(cxt, start)); /* default */
                fdisk_ask_number_set_high(
                    ask, fdisk_scround(cxt, stop)); /* maximal */
            }
    ...
    stop =
        cxt->geom.cylinders * cxt->geom.heads * cxt->geom.sectors; /* ancient */
    stop2 = stop;
    for (i = 0; i < cxt->label->nparts_max; i++)
    {
    ...
    else
    {
        ...
        if (whole_disk)
        {
            fdisk_ask_number_set_low(ask,
                                     fdisk_scround(cxt, stop2)); /* minimal */
            fdisk_ask_number_set_default(
                ask, fdisk_scround(cxt, stop2)); /* default */
            fdisk_ask_number_set_high(ask,
                                      fdisk_scround(cxt, stop2)); /* maximal */
            fdisk_ask_number_set_base(ask, 0);
        }
        else if (n == 2 && !first)
        {
            fdisk_ask_number_set_low(ask,
                                     fdisk_scround(cxt, first)); /* minimal */
            fdisk_ask_number_set_default(
                ask, fdisk_scround(cxt, stop2)); /* default */
            fdisk_ask_number_set_high(ask,
                                      fdisk_scround(cxt, stop2)); /* maximal */
        ...
        else
        {
            fdisk_ask_number_set_low(ask,
                                     fdisk_scround(cxt, first)); /* minimal */
            fdisk_ask_number_set_default(
                ask, fdisk_scround(cxt, stop)); /* default */
            fdisk_ask_number_set_high(ask,
                                      fdisk_scround(cxt, stop)); /* maximal */
            fdisk_ask_number_set_base(ask, fdisk_scround(cxt, first));
        }

        fdisk_ask_number_set_wrap_negative(ask,
    ...
    if (n == 2 && !first)
    {
        ...
        {
            fdisk_warnx(cxt,
                        _("You haven't covered the whole disk with the 3rd "
                          "partition, but your value\n"
                          "%lu %s covers some other partition. Your entry has "
                          "been changed\n"
                          "to %lu %s"),
                        (unsigned long)fdisk_scround(cxt, last),
                        fdisk_get_unit(cxt, FDISK_SINGULAR),
                        (unsigned long)fdisk_scround(cxt, stop),
                        fdisk_get_unit(cxt, FDISK_SINGULAR));
            last = stop;
        }
    }
    ...
    DBG(LABEL, ul_debug("SUN new partition #%zu: first=%u, last=%u, sys=%d", n,
                        first, last, sys));
...
static int sun_delete_partition(struct fdisk_context* cxt, size_t partnum)
{
    struct sun_disklabel* sunlabel;
    struct sun_partition* part;
    struct sun_info* info;
    ...
    if (partnum == 2 && be16_to_cpu(info->id) == SUN_TAG_WHOLEDISK &&
        !part->start_cylinder &&
...
static int sun_get_disklabel_item(struct fdisk_context* cxt,
                                  struct fdisk_labelitem* item)
{
    ...
    switch (item->id)
    {
        case SUN_LABELITEM_LABELID:
            ...
            item->type = 's';
            item->data.str = *sunlabel->label_id
                                 ? strndup((char*)sunlabel->label_id,
                                           sizeof(sunlabel->label_id))
                                 : NULL;
            break;
        case SUN_LABELITEM_VTOCID:
            item->name = _("Volume ID");
            item->type = 's';
            item->data.str = *sunlabel->vtoc.volume_id
                                 ? strndup((char*)sunlabel->vtoc.volume_id,
                                           sizeof(sunlabel->vtoc.volume_id))
                                 : NULL;
            break;
...
static struct fdisk_parttype* sun_get_parttype(struct fdisk_context* cxt,
                                               size_t n)
{
    struct sun_disklabel* sunlabel = self_disklabel(cxt);
    struct fdisk_parttype* t;
    ...
    t = fdisk_label_get_parttype_from_code(
        cxt->label, be16_to_cpu(sunlabel->vtoc.infos[n].id));
    return t
               ?: fdisk_new_unknown_parttype(
                      be16_to_cpu(sunlabel->vtoc.infos[n].id), NULL);
}
                             ...
                             struct fdisk_partition* pa)
{
    ...
    start =
        be32_to_cpu(part->start_cylinder) * cxt->geom.heads * cxt->geom.sectors;
    ...
    {
        if (asprintf(&pa->attrs, "%c%c", flags & SUN_FLAG_UNMNT ? 'u' : ' ',
 ...
 * @cxt: context
 *
 * Sets number of extra sectors per cylinder. This function uses libfdisk Ask
 * API for dialog with user.
 *
 * Returns: 0 on success, <0 on error.
 */
...
static int sun_set_partition(struct fdisk_context* cxt, size_t i,
                             struct fdisk_partition* pa)
{
    struct sun_disklabel* sunlabel;
    struct sun_partition* part;
    ...
    if (pa->type)
    {
        ...
        if (cxt->script == NULL && t->code == SUN_TAG_LINUX_SWAP &&
            !part->start_cylinder)
        {
            int yes, rc;
            ...
            rc = fdisk_ask_yesno(
                ...
                _("It is highly recommended that the partition at offset 0\n"
                  "is UFS, EXT2FS filesystem or SunOS swap. Putting Linux "
                  "swap\n"
...
static int sun_partition_is_used(struct fdisk_context* cxt, size_t i)
{
    struct sun_disklabel* sunlabel;
