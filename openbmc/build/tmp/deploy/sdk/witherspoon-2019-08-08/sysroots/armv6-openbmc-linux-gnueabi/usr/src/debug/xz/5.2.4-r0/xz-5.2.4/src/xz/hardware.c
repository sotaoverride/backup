...
extern void hardware_threads_set(uint32_t n)
{
    if (n == 0)
    {
...
extern void hardware_memlimit_set(uint64_t new_memlimit, bool set_compress,
                                  bool set_decompress, bool is_percentage)
{
    if (is_percentage)
    {
...
extern uint64_t hardware_memlimit_get(enum operation_mode mode)
{
    ...
    const uint64_t memlimit =
        mode == MODE_COMPRESS ? memlimit_compress : memlimit_decompress;
...
extern void hardware_memlimit_show(void)
{
    ...
    else
    {
        ...
        memlimit_show(_("Total amount of physical memory (RAM): "), total_ram);
