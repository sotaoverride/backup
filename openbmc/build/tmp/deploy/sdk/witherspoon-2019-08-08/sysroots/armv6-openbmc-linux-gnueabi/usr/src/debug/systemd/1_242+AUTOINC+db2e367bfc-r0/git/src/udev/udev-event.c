...
UdevEvent* udev_event_new(sd_device* dev, usec_t exec_delay_usec,
                          sd_netlink* rtnl)
{
    UdevEvent* event;
                                ...
                                char* dest, size_t l)
{
    ...
    switch (entry->type)
    {
        ...
        case SUBST_MINOR:
        {
            ...
            xsprintf(buf, "%u",
                     r < 0 ? 0
                           : entry->type == SUBST_MAJOR ? major(devnum)
                                                        : minor(devnum));
            l = strpcpy(&s, l, buf);
            break;
        }
        ...
        case SUBST_ATTR:
        {
            ...
            /* try to read the attribute of the parent device, other matches
             * have selected */
            if (!val && event->dev_parent && event->dev_parent != dev)
                (void)sd_device_get_sysattr_value(event->dev_parent, attr,
                                                  &val);
...
ssize_t udev_event_apply_format(UdevEvent* event, const char* src, char* dest,
                                size_t size, bool replace_whitespace)
{
    const char* from;
    char* s;
    size_t l;
    ...
    subst:
        ...
        if (subst_len < 0)
        {
            if (format_dollar)
                ...
                log_device_warning_errno(
                    event->dev, subst_len,
                    "Failed to apply format '%%%c', ignoring: %m", entry->fmt);
...
static int on_spawn_io(sd_event_source* s, int fd, uint32_t revents,
                       void* userdata)
{
    ...
    {
        if (errno != EAGAIN)
            log_error_errno(errno, "Failed to read stdout of '%s': %m",
                            spawn->cmd);
...
static int on_spawn_timeout(sd_event_source* s, uint64_t usec, void* userdata)
{
    ...
    log_error("Spawned process '%s' [" PID_FMT "] timed out after %s, killing",
              spawn->cmd, spawn->pid,
              format_timespan(timeout, sizeof(timeout), spawn->timeout_usec,
                              USEC_PER_SEC));

    return 1;
}

static int on_spawn_timeout_warning(sd_event_source* s, uint64_t usec,
                                    void* userdata)
{
    Spawn* spawn = userdata;
    char timeout[FORMAT_TIMESPAN_MAX];
    ...
    log_warning("Spawned process '%s' [" PID_FMT
                "] is taking longer than %s to complete",
                spawn->cmd, spawn->pid,
                format_timespan(timeout, sizeof(timeout),
                                spawn->timeout_warn_usec, USEC_PER_SEC));

    return 1;
}

static int on_spawn_sigchld(sd_event_source* s, const siginfo_t* si,
                            void* userdata)
{
    Spawn* spawn = userdata;
    int ret = -EIO;
        ...
        case CLD_EXITED:
            if (si->si_status == 0)
                ...
                log_full(spawn->accept_failure ? LOG_DEBUG : LOG_WARNING,
                         "Process '%s' failed with exit code %i.", spawn->cmd,
                         si->si_status);
            ret = si->si_status;
            break;
        case CLD_KILLED:
        case CLD_DUMPED:
            log_error("Process '%s' terminated by signal %s.", spawn->cmd,
...
static int spawn_wait(Spawn* spawn)
{
    ...
    if (spawn->timeout_usec > 0)
    {
        ...
        if (age_usec < spawn->timeout_usec)
        {
                ...
                spawn->timeout_warn_usec > age_usec)
            {
                ...
                r = sd_event_add_time(
                    e, NULL, CLOCK_MONOTONIC, usec + spawn->timeout_warn_usec,
                    USEC_PER_SEC, on_spawn_timeout_warning, spawn);
                if (r < 0)
                    return r;
            }
            ...
            r = sd_event_add_time(e, NULL, CLOCK_MONOTONIC,
                                  usec + spawn->timeout_usec, USEC_PER_SEC,
    ...
    {
        r = sd_event_add_io(e, NULL, spawn->fd_stdout, EPOLLIN, on_spawn_io,
                            spawn);
        if (r < 0)
            return r;
    }
    ...
    {
        r = sd_event_add_io(e, NULL, spawn->fd_stderr, EPOLLIN, on_spawn_io,
                            spawn);
        if (r < 0)
            return r;
    }

    r = sd_event_add_child(e, NULL, spawn->pid, WEXITED, on_spawn_sigchld,
                           spawn);
    if (r < 0)
        return r;
...
int udev_event_spawn(UdevEvent* event, usec_t timeout_usec, bool accept_failure,
                     const char* cmd, char* result, size_t ressize)
{
    ...
    if (result || log_get_max_level() >= LOG_INFO)
        if (pipe2(outpipe, O_NONBLOCK | O_CLOEXEC) != 0)
            return log_error_errno(
                errno, "Failed to create pipe for command '%s': %m", cmd);
    ...
    if (isempty(argv[0]))
        return log_error_errno(SYNTHETIC_ERRNO(EINVAL), "Invalid command '%s'",
                               cmd);
    ...
    r = safe_fork("(spawn)", FORK_RESET_SIGNALS | FORK_DEATHSIG | FORK_LOG,
                  &pid);
    if (r < 0)
        return log_error_errno(
            r, "Failed to fork() to execute command '%s': %m", cmd);
    if (r == 0)
    {
    ...
    r = spawn_wait(&spawn);
    if (r < 0)
        return log_error_errno(r, "Failed to wait for spawned command '%s': %m",
                               cmd);
...
static int rename_netif(UdevEvent* event)
{
    ...
    /* Set ID_RENAMING boolean property here, and drop it in the corresponding
     * move uevent later. */
    ...
    if (r < 0)
        log_device_warning_errno(
            dev, r, "Failed to update properties with new name '%s': %m",
            event->name);

    log_device_debug(dev, "Network interface %i is renamed from '%s' to '%s'",
                     ifindex, oldname, event->name);
...
static int update_devnode(UdevEvent* event)
{
    ...
    apply = device_for_action(dev, DEVICE_ACTION_ADD) || event->owner_set ||
            event->group_set || event->mode_set;
    return udev_node_add(dev, apply, event->mode, event->uid, event->gid,
                         event->seclabel_list);
}
                                          ...
                                          UdevRules* rules)
{
    ...
    if (r < 0)
        log_device_debug_errno(
            dev, r, "Failed to read database under /run/udev/data/: %m");

    r = device_tag_index(dev, NULL, false);
    if (r < 0)
        log_device_debug_errno(dev, r,
                               "Failed to remove corresponding tag files under "
                               "/run/udev/tag/, ignoring: %m");
    ...
    (void)udev_rules_apply_to_event(rules, event, timeout_usec,
                                    properties_list);
...
static int udev_event_on_move(UdevEvent* event)
{
    ...
    if (event->dev_db_clone && sd_device_get_devnum(dev, NULL) < 0)
    {
        ...
        if (r < 0)
            log_device_debug_errno(dev, r,
                                   "Failed to copy properties from cloned "
                                   "sd_device object, ignoring: %m");
    }
    ...
    if (r < 0)
        return log_device_debug_errno(
            dev, r, "Failed to remove 'ID_RENAMING' property, ignoring: %m");

    return 0;
}

int udev_event_execute_rules(UdevEvent* event, usec_t timeout_usec,
                             Hashmap* properties_list, UdevRules* rules)
{
    ...
    {
        event_execute_rules_on_remove(event, timeout_usec, properties_list,
                                      rules);
        return 0;
    }
    ...
    if (r < 0)
        log_device_debug_errno(
            dev, r, "Failed to clone sd_device object, ignoring: %m");
    ...
    if (r < 0)
        log_device_debug_errno(
            dev, r, "Failed to set initialization timestamp, ignoring: %m");

    /* (re)write database file */
    r = device_tag_index(dev, event->dev_db_clone, true);
    if (r < 0)
        log_device_debug_errno(
            dev, r, "Failed to update tags under /run/udev/tag/, ignoring: %m");
...
void udev_event_execute_run(UdevEvent* event, usec_t timeout_usec)
{
    ...
    ORDERED_HASHMAP_FOREACH_KEY(val, cmd, event->run_list, i)
    {
        ...
        else
        {
            ...
            (void)udev_event_spawn(event, timeout_usec, false, command, NULL,
                                   0);
        }
    }
}
