...
{
    int ret = -1;
    UI_STRING* s = general_allocate_prompt(ui, prompt, prompt_freeable, type,
...
static int general_allocate_boolean(
    UI* ui, const char* prompt, const char* action_desc, const char* ok_chars,
    const char* cancel_chars, int prompt_freeable, enum UI_string_types type,
    int input_flags, char* result_buf)
{
    int ret = -1;
    UI_STRING* s;
...
{
    return general_allocate_string(ui, prompt, 0, UIT_PROMPT, flags, result_buf,
...
int UI_add_verify_string(UI* ui, const char* prompt, int flags,
                         ...
                         const char* test_buf)
{
    return general_allocate_string(ui, prompt, 0, UIT_VERIFY, flags, result_buf,
...
{
    return general_allocate_string(ui, text, 0, UIT_INFO, 0, NULL, 0, 0, NULL);
}
...
{
    return general_allocate_string(ui, text, 0, UIT_ERROR, 0, NULL, 0, 0, NULL);
}
...
int UI_dup_error_string(UI* ui, const char* text)
{
    ...
    return general_allocate_string(ui, text_copy, 1, UIT_ERROR, 0, NULL, 0, 0,
                                   NULL);
}
...
int UI_process(UI* ui)
{
    ...
    if (ui->flags & UI_FLAG_PRINT_ERRORS)
        ERR_print_errors_cb((int (*)(const char*, size_t, void*))print_error,
                            (void*)ui);
    ...
    switch (uis->type)
    {
        ...
        case UIT_VERIFY:
        {
            ...
            if (len < uis->_.string_data.result_minsize)
            {
                ...
                ERR_add_error_data(5, "You must type in ", number1, " to ",
                                   number2, " characters");
                return -1;
            }
