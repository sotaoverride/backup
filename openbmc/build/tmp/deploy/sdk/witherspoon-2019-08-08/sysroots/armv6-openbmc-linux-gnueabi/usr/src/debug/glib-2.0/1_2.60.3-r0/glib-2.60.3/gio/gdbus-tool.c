...
static void modify_argv0_for_command(gint* argc, gchar** argv[],
                                     const gchar* command)
{
    gchar* s;
    gchar* program_name;
...
static GOptionContext* command_option_context_new(const gchar* parameter_string,
                                                  const gchar* summary,
...
static void print_methods_and_signals(GDBusConnection* c, const gchar* name,
                                      const gchar* path, gboolean print_methods,
                                      gboolean print_signals)
{
    ...
    result = g_dbus_connection_call_sync(
        c, name, path, "org.freedesktop.DBus.Introspectable", "Introspect",
        NULL, G_VARIANT_TYPE("(s)"), G_DBUS_CALL_FLAGS_NONE, 3000, /* 3 secs */
        NULL, &error);
    if (result == NULL)
    {
    ...
    for (n = 0; node->interfaces != NULL && node->interfaces[n] != NULL; n++)
    {
        ...
        for (m = 0; print_methods && iface->methods != NULL &&
                    iface->methods[m] != NULL;
             m++)
        {
            const GDBusMethodInfo* method = iface->methods[m];
            g_print("%s.%s \n", iface->name, method->name);
        }
        for (m = 0; print_signals && iface->signals != NULL &&
                    iface->signals[m] != NULL;
             m++)
        {
...
static void print_paths(GDBusConnection* c, const gchar* name,
                        const gchar* path)
{
    GVariant* result;
    GError* error;
...
static void print_names(GDBusConnection* c, gboolean include_unique_names)
{
    ...
    result = g_dbus_connection_call_sync(
        ...
        G_DBUS_CALL_FLAGS_NONE, 3000, /* 3 secs */
        NULL, &error);
    if (result == NULL)
    {
        g_printerr(_("Error: %s\n"), error->message);
    ...
    result = g_dbus_connection_call_sync(
        ...
        G_VARIANT_TYPE("(as)"), G_DBUS_CALL_FLAGS_NONE, 3000, /* 3 secs */
        NULL, &error);
    if (result == NULL)
    {
        g_printerr(_("Error: %s\n"), error->message);
...
static const GOptionEntry connection_entries[] = {
    {"system", 'y', 0, G_OPTION_ARG_NONE, &opt_connection_system,
     N_("Connect to the system bus"), NULL},
    {"session", 'e', 0, G_OPTION_ARG_NONE, &opt_connection_session,
     N_("Connect to the session bus"), NULL},
    {"address", 'a', 0, G_OPTION_ARG_STRING, &opt_connection_address,
...
static GOptionGroup* connection_get_group(void)
{
    ...
    g = g_option_group_new("connection", N_("Connection Endpoint Options:"),
...
static GDBusConnection* connection_get_dbus_connection(GError** error)
{
    ...
    if (!opt_connection_system && !opt_connection_session &&
        opt_connection_address == NULL)
    {
        g_set_error(error, G_IO_ERROR, G_IO_ERROR_FAILED,
                    _("No connection endpoint specified"));
        goto out;
    }
    else if ((opt_connection_system &&
              (opt_connection_session || opt_connection_address != NULL)) ||
             (opt_connection_session &&
              (opt_connection_system || opt_connection_address != NULL)) ||
             (opt_connection_address != NULL &&
              (opt_connection_system || opt_connection_session)))
    {
        g_set_error(error, G_IO_ERROR, G_IO_ERROR_FAILED,
...
static GPtrArray* call_helper_get_method_in_signature(
    GDBusConnection* c, const gchar* dest, const gchar* path,
    const gchar* interface_name, const gchar* method_name, GError** error)
{
    GPtrArray* ret;
    GVariant* result;
    ...
    result = g_dbus_connection_call_sync(
        c, dest, path, "org.freedesktop.DBus.Introspectable", "Introspect",
        NULL, G_VARIANT_TYPE("(s)"), G_DBUS_CALL_FLAGS_NONE, 3000, /* 3 secs */
        NULL, error);
    if (result == NULL)
        goto out;
    ...
    {
        g_set_error(error, G_IO_ERROR, G_IO_ERROR_FAILED,
                    _("Warning: According to introspection data, interface "
                      "“%s” does not exist\n"),
                    interface_name);
        goto out;
    }
    ...
    method_info =
        g_dbus_interface_info_lookup_method(interface_info, method_name);
    if (method_info == NULL)
    {
        g_set_error(error, G_IO_ERROR, G_IO_ERROR_FAILED,
                    _("Warning: According to introspection data, method “%s” "
                      "does not exist on interface “%s”\n"),
                    method_name, interface_name);
        goto out;
    }

    ret = g_ptr_array_new_with_free_func((GDestroyNotify)g_variant_type_free);
    for (n = 0; method_info->in_args != NULL && method_info->in_args[n] != NULL;
         n++)
    {
        g_ptr_array_add(ret,
                        g_variant_type_new(method_info->in_args[n]->signature));
    }
...
static GVariant* _g_variant_parse_me_harder(GVariantType* type,
...
static const GOptionEntry emit_entries[] = {
    {"dest", 'd', 0, G_OPTION_ARG_STRING, &opt_emit_dest,
     N_("Optional destination for signal (unique name)"), NULL},
    {"object-path", 'o', 0, G_OPTION_ARG_STRING, &opt_emit_object_path,
     N_("Object path to emit signal on"), NULL},
    {"signal", 's', 0, G_OPTION_ARG_STRING, &opt_emit_signal,
     N_("Signal and interface name"), NULL},
    {NULL}};

static gboolean handle_emit(gint* argc, gchar** argv[],
                            gboolean request_completion,
                            ...
                            const gchar* completion_prev)
{
    ...
    o = command_option_context_new(NULL, _("Emit a signal."), emit_entries,
                                   request_completion);
    g_option_context_add_group(o, connection_get_group());

    complete_names = FALSE;
    if (request_completion && *argc > 1 &&
        g_strcmp0((*argv)[(*argc) - 1], "--dest") == 0)
    {
        complete_names = TRUE;
    ...
    if (request_completion && *argc > 1 &&
        g_strcmp0((*argv)[(*argc) - 1], "--object-path") == 0)
    {
        complete_paths = TRUE;
        remove_arg((*argc) - 1, argc, argv);
    }
    ...
    if (request_completion && *argc > 1 &&
        g_strcmp0((*argv)[(*argc) - 1], "--signal") == 0)
    {
    ...
    if (request_completion && opt_emit_dest != NULL &&
        g_strcmp0("--dest", completion_prev) == 0)
    {
    ...
    if (opt_emit_dest == NULL && opt_emit_object_path == NULL &&
        request_completion)
    {
        g_print("--dest \n");
    }
    ...
    if (opt_emit_dest != NULL && opt_emit_object_path != NULL &&
        complete_signals)
    {
        print_methods_and_signals(c, opt_emit_dest, opt_emit_object_path, FALSE,
                                  TRUE);
        goto out;
    }
    ...
    for (n = 1; n < (guint)*argc; n++)
    {
        ...
        error = NULL;
        value = g_variant_parse(NULL, (*argv)[n], NULL, NULL, &error);
        if (value == NULL)
        {
            gchar* context;
    ...
    if (!g_dbus_connection_emit_signal(c, opt_emit_dest, opt_emit_object_path,
...
static const GOptionEntry call_entries[] = {
    {"dest", 'd', 0, G_OPTION_ARG_STRING, &opt_call_dest,
     N_("Destination name to invoke method on"), NULL},
    {"object-path", 'o', 0, G_OPTION_ARG_STRING, &opt_call_object_path,
     N_("Object path to invoke method on"), NULL},
    {"method", 'm', 0, G_OPTION_ARG_STRING, &opt_call_method,
     N_("Method and interface name"), NULL},
    {"timeout", 't', 0, G_OPTION_ARG_INT, &opt_call_timeout,
     N_("Timeout in seconds"), NULL},
    {NULL}};

static gboolean handle_call(gint* argc, gchar** argv[],
                            gboolean request_completion,
                            const gchar* completion_cur,
                            const gchar* completion_prev)
{
    ...
    o = command_option_context_new(NULL,
    ...
    if (request_completion && *argc > 1 &&
        g_strcmp0((*argv)[(*argc) - 1], "--method") == 0)
    {
    ...
    if (complete_methods)
    {
        print_methods_and_signals(c, opt_call_dest, opt_call_object_path, TRUE,
                                  FALSE);
        goto out;
    }
    ...
    in_signature_types = call_helper_get_method_in_signature(
        c, opt_call_dest, opt_call_object_path, interface_name, method_name,
        &error);
    ...
    for (n = 1; n < (guint)*argc; n++)
    {
        ...
        if (in_signature_types != NULL)
        {
            if (parm >= in_signature_types->len)
            {
                ...
                if (parm == in_signature_types->len)
                {
                    g_printerr("Warning: Introspection data indicates %d "
                               "parameters but more was passed\n",
        ...
        if (value == NULL)
        {
            ...
            if (value == NULL)
            {
                if (type != NULL)
                {
                    ...
                    g_printerr(
                        _("Error parsing parameter %d of type “%s”: %s\n"),
                        parm + 1, s, context);
                    g_free(s);
                }
                else
    ...
    result = g_dbus_connection_call_sync(
        c, opt_call_dest, opt_call_object_path, interface_name, method_name,
        parameters, NULL, G_DBUS_CALL_FLAGS_NONE,
        opt_call_timeout > 0 ? opt_call_timeout * 1000 : opt_call_timeout, NULL,
        &error);
    if (result == NULL)
    {
        g_printerr(_("Error: %s\n"), error->message);
            ...
            in_signature_types != NULL)
        {
            if (in_signature_types->len > 0)
            {
                ...
                for (n = 0; n < in_signature_types->len; n++)
                {
                    ...
                    g_string_append_len(s, g_variant_type_peek_string(type),
                                        g_variant_type_get_string_length(type));
                }

                g_printerr("(According to introspection data, you need to pass "
                           "'%s')\n",
                           s->str);
...
static void dump_annotation(const GDBusAnnotationInfo* o, guint indent,
                            gboolean ignore_indent)
{
    guint n;
    g_print("%*s@%s(\"%s\")\n", ignore_indent ? 0 : indent, "", o->key,
            o->value);
    for (n = 0; o->annotations != NULL && o->annotations[n] != NULL; n++)
        dump_annotation(o->annotations[n], indent + 2, FALSE);
}

static void dump_arg(const GDBusArgInfo* o, guint indent,
                     const gchar* direction, gboolean ignore_indent,
                     gboolean include_newline)
{
    ...
    g_print("%*s%s%s %s%s", ignore_indent ? 0 : indent, "", direction,
            o->signature, o->name, include_newline ? ",\n" : "");
}
...
static void dump_method(const GDBusMethodInfo* o, guint indent)
{
    ...
    for (n = 0, m = 0; o->in_args != NULL && o->in_args[n] != NULL; n++, m++)
    {
        ...
        dump_arg(o->in_args[n], indent + name_len + 1, "in  ", ignore_indent,
                 include_newline);
    }
    for (n = 0; o->out_args != NULL && o->out_args[n] != NULL; n++, m++)
    {
        gboolean ignore_indent = (m == 0);
        gboolean include_newline = (m != total_num_args - 1);
        dump_arg(o->out_args[n], indent + name_len + 1, "out ", ignore_indent,
                 include_newline);
    }
    g_print(");\n");
}
...
static void dump_signal(const GDBusSignalInfo* o, guint indent)
{
    ...
    for (n = 0; o->args != NULL && o->args[n] != NULL; n++)
    {
        ...
        dump_arg(o->args[n], indent + name_len + 1, "", ignore_indent,
                 include_newline);
    }
    g_print(");\n");
}

static void dump_property(const GDBusPropertyInfo* o, guint indent,
                          GVariant* value)
{
    const gchar* access;
    guint n;
    ...
    if (o->flags == G_DBUS_PROPERTY_INFO_FLAGS_READABLE)
        ...
        access = "writeonly";
    else if (o->flags == (G_DBUS_PROPERTY_INFO_FLAGS_READABLE |
...
static void dump_interface(GDBusConnection* c, const gchar* name,
                           const GDBusInterfaceInfo* o, guint indent,
                           const gchar* object_path)
{
        ...
        o->properties != NULL)
    {
        GVariant* result;
        result = g_dbus_connection_call_sync(
            c, name, object_path, "org.freedesktop.DBus.Properties", "GetAll",
            g_variant_new("(s)", o->name), NULL, G_DBUS_CALL_FLAGS_NONE, 3000,
            NULL, NULL);
        if (result != NULL)
        {
        ...
        else
        {
            ...
            {
                result = g_dbus_connection_call_sync(
                    c, name, object_path, "org.freedesktop.DBus.Properties",
                    "Get",
...
static gboolean introspect_do(GDBusConnection* c, const gchar* object_path,
                              guint indent);

static void dump_node(GDBusConnection* c, const gchar* name,
                      const GDBusNodeInfo* o, guint indent,
                      const gchar* object_path, gboolean recurse)
{
    guint n;
        ...
        for (n = 0; o->interfaces != NULL && o->interfaces[n] != NULL; n++)
        {
            if (opt_introspect_only_properties)
            {
                if (o->interfaces[n]->properties != NULL &&
                    o->interfaces[n]->properties[0] != NULL)
                    dump_interface(c, name, o->interfaces[n], indent + 2,
                                   object_path);
            }
            else
            {
        ...
        for (n = 0; o->nodes != NULL && o->nodes[n] != NULL; n++)
        {
            if (recurse)
            {
                ...
                if (g_variant_is_object_path(o->nodes[n]->path))
                {
                    ...
                    {
                        g_print("Skipping path %s that is not enclosed by "
                                "parent %s\n",
                ...
                else
                {
                    if (g_strcmp0(object_path, "/") == 0)
                        ...
                        child_path = g_strdup_printf("%s/%s", object_path,
                                                     o->nodes[n]->path);
                    introspect_do(c, child_path, indent + 2);
                }
                g_free(child_path);
...
static const GOptionEntry introspect_entries[] = {
    {"dest", 'd', 0, G_OPTION_ARG_STRING, &opt_introspect_dest,
     N_("Destination name to introspect"), NULL},
    {"object-path", 'o', 0, G_OPTION_ARG_STRING, &opt_introspect_object_path,
     N_("Object path to introspect"), NULL},
    {"xml", 'x', 0, G_OPTION_ARG_NONE, &opt_introspect_xml, N_("Print XML"),
     NULL},
    {"recurse", 'r', 0, G_OPTION_ARG_NONE, &opt_introspect_recurse,
     N_("Introspect children"), NULL},
    {"only-properties", 'p', 0, G_OPTION_ARG_NONE,
     &opt_introspect_only_properties, N_("Only print properties"), NULL},
    {NULL}};
                              ...
                              guint indent)
{
    ...
    error = NULL;
    result = g_dbus_connection_call_sync(
        c, opt_introspect_dest, object_path,
        "org.freedesktop.DBus.Introspectable", "Introspect", NULL,
        G_VARIANT_TYPE("(s)"), G_DBUS_CALL_FLAGS_NONE, 3000, /* 3 sec */
        NULL, &error);
    if (result == NULL)
    {
        g_printerr(_("Error: %s\n"), error->message);
    ...
    else
    {
        ...
        dump_node(c, opt_introspect_dest, node, indent, object_path,
...
static gboolean handle_introspect(gint* argc, gchar** argv[],
                                  gboolean request_completion,
                                  const gchar* completion_cur,
                                  const gchar* completion_prev)
{
    ...
    if (request_completion && opt_introspect_object_path != NULL &&
        !opt_introspect_recurse)
    {
        g_print("--recurse \n");
    }
...
static void monitor_signal_cb(GDBusConnection* connection,
...
static void monitor_on_name_appeared(GDBusConnection* connection,
                                     const gchar* name, const gchar* name_owner,
                                     gpointer user_data)
{
    g_print("The name %s is owned by %s\n", name, name_owner);
    g_assert(monitor_filter_id == 0);
    monitor_filter_id = g_dbus_connection_signal_subscribe(
        connection, name_owner, NULL,                      /* any interface */
        NULL,                                              /* any member */
        opt_monitor_object_path, NULL,                     /* arg0 */
        G_DBUS_SIGNAL_FLAGS_NONE, monitor_signal_cb, NULL, /* user_data */
        NULL); /* user_data destroy notify */
}

static void monitor_on_name_vanished(GDBusConnection* connection,
...
static const GOptionEntry monitor_entries[] = {
    {"dest", 'd', 0, G_OPTION_ARG_STRING, &opt_monitor_dest,
     N_("Destination name to monitor"), NULL},
    {"object-path", 'o', 0, G_OPTION_ARG_STRING, &opt_monitor_object_path,
     N_("Object path to monitor"), NULL},
    {NULL}};

static gboolean handle_monitor(gint* argc, gchar** argv[],
                               gboolean request_completion,
                               const gchar* completion_cur,
                               const gchar* completion_prev)
{
    ...
    if (g_dbus_connection_get_unique_name(c) == NULL)
    {
        if (!request_completion)
            g_printerr(
                _("Error: can’t monitor a non-message-bus connection\n"));
        goto out;
    }
    ...
    g_bus_watch_name_on_connection(
        c, opt_monitor_dest, G_BUS_NAME_WATCHER_FLAGS_AUTO_START,
        monitor_on_name_appeared, monitor_on_name_vanished, NULL, NULL);
...
static gboolean opt_wait_activate_cb(const gchar* option_name,
...
static const GOptionEntry wait_entries[] = {
    {"activate", 'a', G_OPTION_FLAG_OPTIONAL_ARG, G_OPTION_ARG_CALLBACK,
     ...
     N_("Service to activate before waiting for the other one (well-known "
        "name)"),
     "[NAME]"},
...
static void wait_name_appeared_cb(GDBusConnection* connection,
                                  const gchar* name, const gchar* name_owner,
                                  gpointer user_data)
{
...
static gboolean wait_timeout_cb(gpointer user_data)
{
    WaitState* wait_state = user_data;
...
static gboolean handle_wait(gint* argc, gchar** argv[],
                            gboolean request_completion,
                            const gchar* completion_cur,
                            const gchar* completion_prev)
{
    ...
    /*
     * Try and disentangle the command line arguments, with the aim of
     * supporting: gdbus wait --session --activate ActivateName WaitName gdbus
     * wait --session --activate ActivateAndWaitName gdbus wait --activate
     * --session ActivateAndWaitName gdbus wait --session WaitName
     */
    ...
    if (activate_service != NULL && (!g_dbus_is_name(activate_service) ||
    ...
    {
        activate_watch_id = g_bus_watch_name_on_connection(
            c, activate_service, G_BUS_NAME_WATCHER_FLAGS_AUTO_START, NULL,
            NULL, NULL, NULL);
    }
    else
    {
    ...
    watch_id = g_bus_watch_name_on_connection(
        c, wait_service, G_BUS_NAME_WATCHER_FLAGS_NONE, wait_name_appeared_cb,
        NULL, &wait_state, NULL);

    /* Safety timeout. */
    if (opt_wait_timeout_secs > 0)
        timer_id = g_timeout_add_seconds(opt_wait_timeout_secs, wait_timeout_cb,
                                         &wait_state);
...
static gchar* pick_word_at(const gchar* s, gint cursor,
...
again:
    ...
    {
        if (handle_emit(&argc, &argv, request_completion, completion_cur,
                        completion_prev))
            ret = 0;
        goto out;
    }
    ...
    {
        if (handle_call(&argc, &argv, request_completion, completion_cur,
                        completion_prev))
            ret = 0;
        goto out;
    }
    ...
    {
        if (handle_introspect(&argc, &argv, request_completion, completion_cur,
                              completion_prev))
            ret = 0;
        goto out;
    }
    ...
    {
        if (handle_monitor(&argc, &argv, request_completion, completion_cur,
                           completion_prev))
            ret = 0;
        goto out;
    }
    ...
    {
        if (handle_wait(&argc, &argv, request_completion, completion_cur,
                        completion_prev))
            ret = 0;
        goto out;
    }
    else if (g_strcmp0(command, "complete") == 0 && argc == 4 &&
             !request_completion)
    {
      ...
      completion_debug ("----");
        ...
        if (!g_shell_parse_argv(completion_line, &completion_argc,
                                &completion_argv, NULL))
        {
        ...
        completion_prev = NULL;
        completion_cur =
            pick_word_at(completion_line, completion_point, &cur_begin);
        if (cur_begin > 0)
        {
            gint prev_end;
            for (prev_end = cur_begin - 1; prev_end >= 0; prev_end--)
            {
                ...
                {
                    completion_prev =
                        pick_word_at(completion_line, prev_end, NULL);
                    break;
                }
            }
        }
